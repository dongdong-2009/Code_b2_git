/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/response_plans/PlanDataAccess/src/PlanDataWriter.cpp $
  *  @author:  Mahaveer Pareek
  *  @version: $Revision: #8 $
  *
  *  Last modification: $DateTime: 2015/08/28 15:04:26 $
...
//		 strSql = database->prepareSQLStatement(STEP_DETAILES_SELECT_63551, nvlValue, planId);
        ...
        do
        {
            ...
            for( unsigned int unParamIndex = 0u; unParamIndex < dataPtr->getNumRows(); ++unParamIndex )
            {
                ...
                refStepParam.value = CORBA::string_dup( dataPtr->getStringData( unParamIndex, "PARAMETERVALUE" ).c_str() );
            }
...
//		std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_SELECT_33506, parent,
		...
		dataBase->prepareSQLStatement(strSql, NODE_TREE_SELECT_33506, parent,
			parseForApostrophe(originalName), parseForApostrophe(copyNamePrefix.c_str()));
    ...
    std::string PlanDataWriterImp::getOperatorName(const std::string& session)
    {
        try
        {
            ...
            try
            {
                ...
                std::auto_ptr<TA_Base_Core::IProfile> iOvrProfile(TA_Base_Core::ProfileAccessFactory::getInstance().getProfile(ptrSession->getOverrideProfileKey(), false));
                strProfile += " / ";

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/bus/response_plans/active_plans_display/src/PlanActiveInstanceListCtrl.cpp $
...
namespace
{
    ...
    // Column labels
    const char *const PLAN_COLUMN_LABEL = "Plan";			    // Column 1 - Plan
    const char *const CATEGORY_COLUMN_LABEL = "Category";	    // Column 2 - Category
    const char *const INSTANCE_COLUMN_LABEL = "Instance";	    // Column 3 - Instance
    const char *const LOCATION_COLUMN_LABEL = "Location";       // Column 4 - Location
    const char *const OWNER_COLUMN_LABEL = "Owner";			    // Column 5 - Owner
    const char *const ACTIVATION_COLUMN_LABEL = "Activation";	// Column 6 - Activation time
    const char *const STATE_COLUMN_LABEL = "State";		    	// Column 7 - State
    const char *const STEP_COLUMN_LABEL = "Step";		    	// Column 8 - Step
    const char *const REMARKS_COLUMN_LABEL = "Remarks";		    // Column 9 - Remarks
    ...
    *itemData = activePlanDetail;
    ...
    if (!changedOwner)
    {
	    SetItemText(position, PLAN_COLUMN_INDEX, _T(StringUtilities::extractPlanName(activePlanDetail.path.in())
			+ CString(activePlanDetail.plan.param.in())));
        ...
        SetItemText(position, ACTIVATION_COLUMN_INDEX, _T(activePlanDetail.timeOfInstantiation.in()));
...
void CPlanActiveInstanceListCtrl::InitialiseActivePlanList()
{
	if (!m_initialised) // want to do this once only
	{
		...
		//
		// Plan Column
		InsertColumn(PLAN_COLUMN_INDEX, _T(PLAN_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 180 : 0);
		// Category Column
		InsertColumn(CATEGORY_COLUMN_INDEX, _T(CATEGORY_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 180 : 0);
		// Instance Column
		InsertColumn(INSTANCE_COLUMN_INDEX, _T(INSTANCE_COLUMN_LABEL), LVCFMT_RIGHT, 65);
		// Location Column
		InsertColumn(LOCATION_COLUMN_INDEX, _T(LOCATION_COLUMN_LABEL), LVCFMT_LEFT, 70);
		// Owner Column 
		InsertColumn(OWNER_COLUMN_INDEX, _T(OWNER_COLUMN_LABEL), LVCFMT_LEFT, 150);
		// Activation Column 
		InsertColumn(ACTIVATION_COLUMN_INDEX, _T(ACTIVATION_COLUMN_LABEL), LVCFMT_LEFT, 135);
		// State Column
		InsertColumn(STATE_COLUMN_INDEX, _T(STATE_COLUMN_LABEL), LVCFMT_LEFT, 85);
		// Step Column
		InsertColumn(STEP_COLUMN_INDEX, _T(STEP_COLUMN_LABEL), LVCFMT_LEFT,	m_planPathVisible? 185 : 200);
		// Remarks Column
		// CL-14921++ Item 5
		//InsertColumn(REMARKS_COLUMN_INDEX, _T(REMARKS_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 170 : 185); 
		InsertColumn(REMARKS_COLUMN_INDEX, _T(REMARKS_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 435 : 450); 
		// ++CL-14921

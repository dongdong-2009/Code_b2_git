/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_store_library/src/AlarmFilterByProfile.cpp $
  * @author:  Ripple
  * @version: $Revision: #8 $
  *
  * Last modification: $DateTime: 2011/06/10 14:26:36 $
...
bool AlarmFilterByProfile::includeAlarm(const TA_Base_Core::AlarmDetailCorbaDef& alarm)
{
	...
	std::string strList(alarm.alarmParameterList.in());

	char const*   flagPosition = strstr(alarm.alarmParameterList,OperatorMaintenanceFlag);
	if (flagPosition!=NULL)
	{
		// Get the setting (OMFlag:M, or OMFlag:O, or OMFlag:B, or OMFlag:)
		char flagValue = *(flagPosition+strlen(OperatorMaintenanceFlag));

		switch (flagValue)
		{
			case 'M': alarmAction = TA_Base_Bus::AlarmActions::VIEW_MAINTENANCE;
					  break;
			case 'O': alarmAction = TA_Base_Bus::AlarmActions::VIEW_OPERATIONAL;
					  break;
			case 'B': alarmAction = TA_Base_Bus::AlarmActions::VIEW_BOTH;
					  break;
			default:
				{
					// still using the alarm action TA_Base_Bus::AlarmActions::VIEW_ALARM
				}
		}           
	}
...
void AlarmFilterByProfile::reset()
{
	...
	TA_Base_Core::ThreadGuard guard(m_lock);
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "AlarmFilterByProfile::reset");
	m_cache.clear();
	FUNCTION_EXIT;
}
// ++yanrong.
...
// 		{
	...
	{
		LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, 
			"Unable to determine visibility rights for alarm '%s' type %u, excluding",alarm->alarmID,alarm->messageTypeKey);//TD14337 yezh++
	}

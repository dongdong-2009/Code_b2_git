...
namespace TA_Base_Bus
{
	...
	void AllAlarmManager::deleteAllAlarmItems()
	{
		...
		try
		{
			if(g_objAllAlarms != NULL && !g_objAllAlarms->empty())
			{
                ...
                std::for_each(g_objAllAlarms->begin(), g_objAllAlarms->end(),
                    boost::bind(&AllAlarmManager::deleteItem, this, _1));
            }
	...
	void AllAlarmManager::UpdateDutyIndex(LocationSubSytemList& changes, ICheckVisibility* pObjFilter, IDutyAlarmAccess* pObjDutyAlarmAccess)
	{
		...
		try
		{
			...
			for (iterLoc; iterLoc != changes.end(); iterLoc++)
			{
				...
				pObjDutyAlarmAccess->removeLocSubSystemDutyAlarms(iterLoc->first, iterLoc->second);
	...
	unsigned long AllAlarmManager::removeLocationAlarms(unsigned long locationKey)
	{	 
		TA_Base_Core::ThreadGuard guard(m_MapLock);
			
		return removeLocationAlarmsInternal(locationKey);
	}
	...
	unsigned long AllAlarmManager::removeLocationAlarmsInternal(unsigned long locationKey)
	{
		...
		try
		{
			...
			allalarms_by_locSubSystem::iterator lIter = locSubSystem_index.lower_bound( boost::make_tuple(locationKey) );
			allalarms_by_locSubSystem::iterator hIter = locSubSystem_index.upper_bound( boost::make_tuple(locationKey) );
			...
			while(lIter != hIter)
			{
				...
				++lRes;
				lIter = locSubSystem_index.lower_bound( boost::make_tuple(locationKey) );
				hIter = locSubSystem_index.upper_bound( boost::make_tuple(locationKey) );
			}
		}
		catch(...)
		{

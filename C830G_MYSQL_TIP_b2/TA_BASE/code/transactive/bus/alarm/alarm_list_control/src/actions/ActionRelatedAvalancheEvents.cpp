 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionRelatedAvalancheEvents.cpp $
 * @author:  Ben Robbins
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Bus
{
    ...
    bool ActionRelatedAvalancheEvents::isAvailable(IAlarmRetriever& alarmRetriever)
    {
        ...
        if (alarmRetriever.getRightsChecker().canPerformAction( getEventViewerKey(alarmRetriever),
...
//            }
    ...
    unsigned long ActionRelatedAvalancheEvents::getEventViewerKey(IAlarmRetriever& alarmRetriever)
    {
        ...
        if (0 == m_eventViewerKey)
        {
			...
			TA_Base_Core::IEntityData* pentitydata = 
				alarmRetriever.getDatabaseCache().searchEntityDataList(TA_Base_Core::EventViewer::getStaticType());
			//	unsigned long entitykey;
			if(pentitydata == NULL)
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "ActionIncidentLogSheet::isAvailable entitydata NOT EXIST" );
				this->m_eventViewerKey = 0;
			}
			else{
				m_eventViewerKey = pentitydata->getKey();
			}

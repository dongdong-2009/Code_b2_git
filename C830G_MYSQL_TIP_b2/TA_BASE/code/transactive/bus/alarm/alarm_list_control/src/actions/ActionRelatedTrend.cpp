 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionRelatedTrend.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Bus
{
    ...
    bool ActionRelatedTrend::isAvailable(IAlarmRetriever& alarmRetriever)
    {
        ...
        try
        {
			...
			std::auto_ptr<TA_Base_Bus::AlarmDataItem> pAlarm(pdataTemp);
            entityKey =  pAlarm->assocEntityKey; //selectedAlarms[0]->assocEntityKey;
        }
        ...
        // take care of missing entity for trend viewer
         unsigned long trendViewerEntityKey = getTrendViewerKey(alarmRetriever);
    ...
    unsigned long ActionRelatedTrend::getTrendViewerKey(IAlarmRetriever& alarmRetriever)
    {
        FUNCTION_ENTRY("getTrendViewerKey");

        if (0 == m_trendViewerKey)
        {
			//Li Zhongyan change 20243
			TA_Base_Core::IEntityData* pentitydata = 
				alarmRetriever.getDatabaseCache().searchEntityDataList(TA_Base_Core::HistoryViewerData::getStaticType());
			//	unsigned long entitykey;
			if(pentitydata == NULL)
			{
				...
				this->m_trendViewerKey = pentitydata->getKey();
			}
            /*try
            {

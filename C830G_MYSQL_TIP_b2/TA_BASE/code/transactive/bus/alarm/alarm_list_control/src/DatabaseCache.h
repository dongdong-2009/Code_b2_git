/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/DatabaseCache.h $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
...
namespace TA_Base_Bus
{
    ...
    public:
		 ...
		 * 
		 * load the entityMapLimitedSize from runParameter, 
		 * default value is 5000, if it is 0, means we don't limit the entity map's size.
		 *
		 */
		...
		/**
         * isEntityDataListLoad
         *
         * This judges whether m_entitylist is loaded already
         */
		bool isEntityDataListLoad();


		/**
         * searchEntityDataList
         * 
         * This searches the EntityData from the database and stores them in the m_entitylist and get the IEntityData
         */
		TA_Base_Core::IEntityData* searchEntityDataList(const std::string& type);

		/**
         * loadEntityDataList
         * 
         * This loads the EntityData from the database and stores them in the m_entitylist
         */
		void loadEntityDataList();
    ...
    private:
		...
		std::list<TA_Base_Core::IEntityData*>                      m_entityLimitedList;
		unsigned long											   m_entityCacheMaxSize;

        TA_Base_Core::NonReEntrantThreadLockable                   m_alarmTypeMapLock;
        std::map<std::string, std::string>                    m_alarmTypes;
		std::map<std::string, TA_Base_Core::IEntityData*>				m_entitylist; // li zhongyan add
		bool m_entitydatalistload;
    };
}

 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionJobRequest.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Bus
{
    ...
    void ActionJobRequest::performAction(IAlarmRetriever& alarmRetriever)
    {
		...
		try
		{
            ...
            if( strcmp(ackedBy.c_str(), "") == 0 )// Alarm still yet to be acknowledged
			{
		...
		if (canAckAlarm)
		{
			try
			{
				...
				m_alarmHelper.operatorAcknowledgeAlarmsEx(alarmAckList,alarmNoExists);

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/authentication_library/src/AuthenticationLibrary.cpp $
  * @author:  Ripple
  * @version: $Revision: #17 $
  *
  * Last modification: $DateTime: 2015/05/07 16:15:15 $
...
#include <iomanip>
//#pragma comment(lib,"cots/openssl/openssl-1.0.0d/build/lib/windows/libeay32.lib")
...
namespace TA_Base_Bus
{
                                          ...
                                          const std::string & password)
    {
        ...
        try
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "[TD15901] Requesting session for user: %lu",userId);
			...
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "[TD15901] New session '%s' requested.",
                    sessionId.in() );
                           ...
                           const std::string & password)
    {
        ...
        try
        {
			...
			//CL-21875 -- Update Override Information to Database
                                  ...
                                  const std::string & password)
    {
        ...
        try
        {
            ...
            {
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "could not find sessionID <%s> whein changing profile",
						sessionId.c_str() );
			...
			if ( ! KeepSessionAliveThreadManager::getInstance().isSameLocationId(sessionId, locationId) )
			{
	            ...
	            try
				{
					newSessionIdStr = requestSession(oldSession.OperatorId[0], profileId, locationId, oldSession.WorkstationId, password);
				}
				catch(...)
				{
					throw;

...
namespace TA_Base_Bus
{
	...
	void AuthenPeerTask::getActiveSessionsImpl( const std::string & noUseSessionId)
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"begin getActiveSessionsImpl at %d",m_locationId);
		m_pAuthLib->getActiveSessionsForOneLocation( noUseSessionId, *m_pAuthAgent, m_vtActiveSessionInfo );
		LOG2(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "end getActiveSessionsImpl get active sessions, its session size = %d at location %d", m_vtActiveSessionInfo.size(), m_locationId);
	}

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/scada/proxy_library/src/DataPointBulkPollingSynchronizer.cpp $
  * @author:  Darren Sampson
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2011/06/02 14:32:44 $
...
namespace TA_Base_Bus
{
          ...
          m_timeLastConnected(time(NULL))
    {
		...
		unsigned long corbaTimeout = atol(TA_Base_Core::RunParams::getInstance().get("ProxyBulkPollCorbaTimeOut").c_str());
		if ( (corbaTimeout < BULK_POLL_COMMS_ERROR_RETRY_PERIOD_SECS) || (corbaTimeout > SET_BAD_QUALITY_RETRY_PERIOD_SECS * 12) )
		{
			corbaTimeout = BULK_POLL_COMMS_ERROR_RETRY_PERIOD_SECS;
		}
		m_agentInterface->setObjectTimeout(corbaTimeout); // in secs
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "ProxyBulkPollCorbaTimeOut is %u", corbaTimeout);
    ...
    void DataPointBulkPollingSynchronizer::run()
    {
        while (!isTerminating())
        {
            try
            {
                ...
                while ( currentRequest.requestType != EmptyQueue )
                {   
                ...
                if ( !isTerminating() )
                {
                    ...
                    {
                        LOG_GENERIC( SourceInfo, DebugUtil::DebugTrace, "BulkPoll %s - polling", m_agentName.c_str());
    ...
    void DataPointBulkPollingSynchronizer::addEntity ( const RequestItem & requestItem )
    {
        ...
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "AddDataPoint %s:%u",
    ...
    void DataPointBulkPollingSynchronizer::removeEntity ( const RequestItem & requestItem )
    {
        ...
        if (itr != m_syncEntities.end())
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "RemoveDataPoint %s:%u",
                getAgentName().c_str(), requestItem.entityKey );

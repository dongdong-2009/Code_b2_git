...
namespace TA_Base_Bus
{
		...
		const ScadaCorbaTypes::UTag& value, CORBA::ULong triggeringEntity, const char* triggeringEntityValue)
	{
		...
		TA_Base_Bus::DataPoint* dataPoint = m_specificAgent->getDataPoint(dpKey);
			...
			if(0 != dataPoint)
			{
				...
				dataPoint->WriteDataPointValue(sessionId, datapointValue, triggeringEntity, triggeringEntityValue, true);
			}
			else
			{
				TA_THROW_CORBA(TA_Base_Core::NotAvailableException());
		...
		const ScadaCorbaTypes::UTag& value, CORBA::ULong triggeringEntity, const char* triggeringEntityValue)
	{
		...
		TA_Base_Bus::DataPoint* dataPoint = m_specificAgent->getDataPoint(dpKey);
			...
			if(0 != dataPoint)
			{
				TA_Base_Bus::DataPointValue datapointValue;
				convertDataPointValue(value, datapointValue);
				dataPoint->WriteDataPointValue(sessionId, datapointValue, triggeringEntity, triggeringEntityValue, false);
			}
			else
			{
				TA_THROW_CORBA(TA_Base_Core::NotAvailableException());
	...
	void DataPointAccessCorbaDef_Impl::submitWriteRequest(CORBA::ULong dpKey, const char* sessionId, const ScadaCorbaTypes::UTag& value, 
		CORBA::ULong triggeringEntity, const char* triggeringEntityValue,CORBA::Boolean isLogingEvent, const ScadaCorbaTypes::STimeStamp &cancelTime,
		const char* controlUUID)
	{
		FUNCTION_ENTRY("DataPointAccessCorbaDef_Impl::writeDataPointValueWithDeadTime");
		...
		if(m_operationMode == TA_Base_Core::Control)
		{
			...
			if(0 != dataPoint)
			{
				...
				timeb deadTime;
				convertDataPointValue(value, datapointValue);
				converTimeStamp(cancelTime, deadTime);
				
				dataPoint->submitWriteRequest(sessionId, datapointValue, triggeringEntity, triggeringEntityValue,
					isLogingEvent, deadTime, controlUUID);
			}
			else
			{
				TA_THROW_CORBA(TA_Base_Core::NotAvailableException());

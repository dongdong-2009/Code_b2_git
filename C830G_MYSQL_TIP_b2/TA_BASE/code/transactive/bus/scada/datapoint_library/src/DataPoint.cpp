...
namespace TA_Base_Bus
{
	...
	static const long          NULL_TIMER_ID	= -1;
	...
	m_previousFieldValueQualityStatus( QUALITY_BAD_NOT_CONNECTED ),
    ...
    m_lastOperatorTimestamp( SCADA_DEFAULT_TIMEB ),
    m_forcedValueTimestamp( SCADA_DEFAULT_TIMEB ),
    m_fieldValueTimestamp( SCADA_DEFAULT_TIMEB ),
	...
	m_alarmModificationHelper(TA_Base_Core::AlarmHelperManager::getInstance().getModificationAlarmHelper())
	{
        ...
        m_workItem.reset(new DataPointWorkItem(*this));
	...
	DataPoint::~DataPoint()
	{
		...
		m_threadPool->removeWorkItem ( m_workItem );
		...
		//remove handl to ScadaUtilities
    ...
    void DataPoint::updateSynState(const TA_Base_Bus::DpDynamicSyncUpdate& toupdate)
    {
        ...
        {		
           case CompleteDynamicSyncStateUpdate:
                 //alarm properties udpate will be handled in the corresponding child datapoint type
            ...
            case LastOperatorUpdate:
        	    ...
        	    break;
            default:
				LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Receive unknown update type[%d] in Monitor mode datapoint [%d]", toupdate.updateType, m_entityKey );
                break;
        }
        FUNCTION_EXIT;
    }
    
    void DataPoint::updateSynInhibitState(const TA_Base_Bus::InhibitState& updateinhibit)
    {
        if (updateinhibit.outputInhibit == TA_Base_Bus::InhibitControl)
        {
	...
	void DataPoint::setLaunchingConditionResultString(const std::string& newResultString)
	{
		...
		m_persistence->writePersistedValue( DP_LAUNCH_RESULT_STRING, m_entityKey, m_controlState.lccResult, m_fieldValueQualityStatus );
	}
    ...
    void DataPoint::setInputInhibitState(const std::string& sessionId, TA_Base_Bus::EInputInhibitState inhibitstate)
    {
        ...
        TA_Base_Bus :: EDataPointServiceState previousServiceState = m_serviceState;
        //TD19392
        EInputInhibitState previousInputInhibitState = m_previousInputInhibitState;
        //
        if ( inhibitstate == m_previousInputInhibitState ) 
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                         "Input Inhibit state already set to desired value. Ignoring request" );
            return;
        }
        else
        {
            m_previousInputInhibitState = inhibitstate;
            //reset the internal input inhibit number to NoInputInhibit first 
        ...
        case InhibitAlarm:
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[XF] DataPoint::setInputInhibitState(): InhibitAlarm");
              ...
              updateMMSAlarmInhibitedStatus(sessionId, true);                  
              //TD19392
              if(previousInputInhibitState != inhibitstate && previousInputInhibitState==InhibitScan)
              {
                  TA_Base_Core::DescriptionParameters offDPs;
                  offDPs.clear();
                  NameValuePair pair2 ( "OnOff", "OFF" );
                  offDPs.push_back ( &pair2 );
                  submitAuditMessage( sessionId, offDPs, DataPointAudit::ScanInhibitionSucceeded );
              }
              //
               // log auditing event
        ...
        case InhibitMmsAlarm:
              ...
              updateMMSAlarmInhibitedStatus(sessionId, true);                  
              //TD19392
              if(previousInputInhibitState != inhibitstate && previousInputInhibitState==InhibitAlarm)
              {
                  TA_Base_Core::DescriptionParameters offDPs;
                  offDPs.clear();
                  NameValuePair pair2 ( "OnOff", "OFF" );
                  offDPs.push_back ( &pair2 );
                  submitAuditMessage( sessionId, offDPs, DataPointAudit::AlarmInhibitionSucceeded );
              }
              else if(previousInputInhibitState != inhibitstate && previousInputInhibitState==InhibitScan)
              {
                  TA_Base_Core::DescriptionParameters offDPs;
                  offDPs.clear();
                  NameValuePair pair2 ( "OnOff", "OFF" );
        ...
        default:
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                         "Unknown input inhibit state: %d", inhibitstate );
	...
	{
        FUNCTION_ENTRY("updateControlInhibitedStatus");
		if ( TA_Base_Core::Monitor == m_operationMode )
		{
			throw( TA_Base_Core::OperationModeException() );
		}
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                         "Control inhibit already set to desired value. Ignoring request" );
            return;
        }
    ...
    bool DataPoint::setForcedValue( const DataPointState & newState )
    {
        ...
        if ( ( m_forcedValue != newState.getValue() ) || ( m_forcedValue.getValueAsString() != newState.getValue().getValueAsString() ) || ( m_forcedValueQualityStatus != newState.getStatus() ) )
        {
            ...
            m_forcedEngValue = engState.getValue();
            
            // if the data point is in control mode
            if ( TA_Base_Core::Control == m_operationMode )
            {
                // if quality is good, then persist to database
                 if ( TA_Base_Bus::QUALITY_GOOD_LOCAL_OVERRIDE == newState.getStatus() )
                {
                    result = m_persistence->writePersistedValue( DP_MANUALLY_DRESSED_VALUE, m_entityKey, engState.getValue().getPersistString(),
			 ...
			 m_controlState.triggerringDpValue != controlStatus.triggerringDpValue)
		{
			...
			if ( TA_Base_Core::Control == m_operationMode )
			{
                ...
                if ( false == isRequestedBySystem ) //limin++, SystemSession should not access db
                {
					...
					//Only persist control state only as the other is the same
					if(TA_Base_Bus::written == m_controlState.controlState )
					{
						struct timeb currentSystemTime;
						ftime ( &currentSystemTime );
	...
	void DataPoint::threadedTimerExpired(long timerId, void* userData)
	{
		FUNCTION_ENTRY("threadedTimerExpired");
		ThreadGuard guard( m_lock );
		{
			ThreadGuard guard ( m_timerLock );
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "DataPoint [%s] control m_timerId[%d] Expired, currentState=%d", m_name.c_str(), m_timerId, m_controlState.controlState);
			if(m_timerId != timerId)
			{
				LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "control timer expired,but timerId is wrong,m_timerId=%d,Expired timerId=%d",m_timerId, timerId );
				return;
			}
			m_timerId = NULL_TIMER_ID;
		}

		TA_Base_Bus::DataPointControl newState;
		switch( m_controlState.controlState )
        {
		case TA_Base_Bus::pending:                    
		case TA_Base_Bus::written:
			newState = m_controlState;
			newState.controlState = failed;
			setControlState ( newState );
			break;
		default:
			LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "control m_timerId[%d] Expired,state not changed,currentState=%d", m_timerId, m_controlState.controlState);
			break;
		}
		FUNCTION_EXIT;
	}
	
	bool DataPoint::startControlTimeOut(unsigned long waitTime)
	{
		FUNCTION_ENTRY("startControlTimeOut");
		
		ThreadGuard guard ( m_timerLock );
		if( m_timerId != NULL_TIMER_ID )
		{// only allow one control, so this area should not be executed.
			SingletonTimerUtil::getInstance().stopPeriodicTimeOutClock ( this, m_timerId );
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "stop control timer, TimerId=%d", m_timerId);
		}
		
		//start control timer
		m_timerId = SingletonTimerUtil::getInstance().startPeriodicTimeOutClock(this, waitTime, true, NULL);
		LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "DataPoint [%s] start control timer, TimerId=%d, waitTime=%d ms", m_name.c_str(), m_timerId, waitTime);
		FUNCTION_EXIT;
		return NULL_TIMER_ID != m_timerId;
	}

	bool DataPoint::stopControlTimeOut()
	{
		FUNCTION_ENTRY("stopControlTimeOut");
		
		ThreadGuard guard ( m_timerLock );
		if( m_timerId != NULL_TIMER_ID )
		{// only allow one control, so this area should not be executed.
			SingletonTimerUtil::getInstance().stopPeriodicTimeOutClock ( this, m_timerId );
			LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "DataPoint [%s] stop control timer, TimerId=%d", m_name.c_str(), m_timerId);
			m_timerId = NULL_TIMER_ID;
		}
		FUNCTION_EXIT;
		return NULL_TIMER_ID == m_timerId;
    ...
    void DataPoint::WriteDataPointValue( const std::string& sessionId,
						...
						const unsigned long triggeringEntityKey,
						const std::string& triggeringEntityValue,
						const bool isLoggingEvent)
	{
		...
		try
		{
			writeDataPointWithChecking( sessionId, value, isLoggingEvent, triggeringEntityKey, triggeringEntityValue, SCADA_DEFAULT_TIMEB);
		}
		catch(TA_Base_Core::ScadaCommonException &)
		{
			throw( TA_Base_Core::BadParameterException() );
		}
		// this is old interface, old client may not catch UnderControlException exception, catching here.
		catch (ScadaCorbaTypes::UnderControlException &)
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "DataPoint [%s] throw UnderControlException, session[%s].", m_name.c_str(), sessionId.c_str());
		}
	...
	void DataPoint::submitWriteRequest( const std::string& sessionId,
								  const TA_Base_Bus::DataPointValue& value,
								  unsigned long triggeringDpKey, const std::string& triggeringDpValue,
								  const bool isLoggingEvent,
								  timeb cancelTime, std::string controlUUID)
	{
		FUNCTION_ENTRY("submitWriteRequest");
		...
		if (controlUUID != "")
		{
			ThreadGuard guard(m_ControlUUIDListLock);
			std::vector <std::string>::const_iterator itr;
			for ( itr = m_ControlUUIDList.begin(); itr != m_ControlUUIDList.end(); itr++ )
			{
				if (*itr == controlUUID)
				{
					LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "DataPoint [%s] throw ExistControlUUIDException[%s].", m_name.c_str(), controlUUID.c_str());
					FUNCTION_EXIT;
					throw ScadaCorbaTypes::ExistControlUUIDException(controlUUID.c_str());
				}
			}
		...
		try
		{
			writeDataPointWithChecking( sessionId, value, isLoggingEvent, triggeringDpKey, triggeringDpValue, cancelTime);
	...
	void DataPoint::writeDataPointWithChecking( const std::string& sessionId,
							 const TA_Base_Bus::DataPointValue & value,
							 bool isLoggingEvent,
							 ...
							 timeb cancelTime)
	{
		FUNCTION_ENTRY("writeDataPointWithChecking");

		ThreadGuard guard( m_lock );

		if (CompletedWithError != m_controlCommandState &&
			CompletedWithSuccess != m_controlCommandState)
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "DataPoint [%s] throw UnderControlException.", m_name.c_str());
			FUNCTION_EXIT;
			throw ScadaCorbaTypes::UnderControlException();
		}

		// ---
		
		writeDataPoint ( sessionId, value, isLoggingEvent, triggeringDpKey, triggeringDpValue, cancelTime);
									...
									timeb cancelWriteTime)
	{
		...
		if(HEARTBEART_TRIGGER == triggeringDpKey)
		{
			try
			{
				...
				DataPointWriteRequest * req =
					new DataPointWriteRequest( this, rawVal, sessionID, TA_Base_Bus::aca_DATA_POINT_WRITE,
					m_commandId, true, engVal);
		...
		if ( true == deviceRequestToBeSubmitted )
		{
			...
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Add new datapoint write request for %s", m_name.c_str() );
			...
			if (0 != cancelWriteTime.time || 0 != cancelWriteTime.millitm)
			{
				if (currentSystemTime.time > cancelWriteTime.time ||
					(currentSystemTime.time == cancelWriteTime.time && currentSystemTime.millitm >= cancelWriteTime.millitm))
				{
					trigDp.controlState = TA_Base_Bus::failed;
					LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "write datapoint %s failed. because cancel time arrived.", m_name.c_str() );
					//this call will generate new command id
					setControlState ( trigDp, isSystemSession );//limin++, SystemSession should not access db
				...
				unsigned long waitTime = (cancelWriteTime.time - currentSystemTime.time) * 1000;
				waitTime = waitTime + cancelWriteTime.millitm - currentSystemTime.millitm;
				startControlTimeOut(waitTime);
			}

			// create a write request and put it into the queue, which then processed by the specific device Agent
			DataPointWriteRequest * req = new DataPointWriteRequest( this, rawVal, sessionID, TA_Base_Bus::aca_DATA_POINT_WRITE,
																	m_commandId, false, engVal);
	...
	bool DataPoint::executeLaunchCondition(
		const std::string& sessionID,
		const DpValue& engVal)
	{
		FUNCTION_ENTRY("executeLaunchCondition");
		{
			TA_Base_Bus::DataPointControl trigDp;

			trigDp = m_controlState;
			...
			if ( true == evaluateLaunchCondition() )
			{
                ...
                if ( false == isSystemSessionID( sessionID) ) //limin++, SystemSession should not access db
                {
	...
	bool DataPoint::isWriteRequestExpired(unsigned long commandId)
	{
		FUNCTION_ENTRY("isWriteRequestExpired");
		ThreadGuard guard( m_lock );
		...
		if (TA_Base_Bus::CompletedWithError == m_controlCommandState ||
			TA_Base_Bus::CompletedWithSuccess == m_controlCommandState)
		{
			FUNCTION_EXIT;
			return true;
		}
	...
	void DataPoint::setToControlMode()
	{
		...
		if ( m_lastSavedOperationMode != m_operationMode )
		{
...

            {
                ...
                for( ; alarmIter != m_alarms.end(); ++alarmIter )
                {
                    (*alarmIter )->setOperationMode ( inControl );
                }
            }
		...
		if ( TA_Base_Core::Control == m_operationMode )
		{
			...
			else
			{
                ...
                response = TA_Base_Bus::ScadaUtilities::getInstance().isActionAllowedWithLocSub (
                    actionKey, m_entityKey, m_locationKey, m_subsystemKey, sessionID );
	...
	void DataPoint::initialiseData()
	{
		...
		if ( 0 == m_abeExpression.compare ( NotLoaded ) )
		{
			m_abeExpression = m_dataPointEntityAccess->getABEExpression();
			m_abeTimeoutPeriodSeconds = m_dataPointEntityAccess->getABETimeoutPeriodSeconds();
		}
	...
	void DataPoint::saveLastOperatorInfo ( const std::string & sessionID, const timeb & timestamp )
	{
		...
		if ( TA_Base_Core::Control == m_operationMode )
		{
            ...
            {             
				...
				resultStream<<m_lastOperatorTimestamp.time;
				PersistRequest * persistReq = new PersistRequest( DP_LAST_OPERATOR_TIMESTAMP, m_entityKey, resultStream.str(), m_fieldValueQualityStatus );
				m_persistQ->insert( persistReq );
            }
	...
	void DataPoint::configureControlStatus()
	{
		...
		else if ( "written" == configControlStatus )
		{
			...
			m_controlState.controlState = TA_Base_Bus::failed;	// when restart, set to failed, otherwise if this is control mode, it will be written for ever.
		}
		else if ( "pending" == configControlStatus )
		{
			//m_controlState.controlState = TA_Base_Bus::pending;
			m_controlState.controlState = TA_Base_Bus::failed;
		}
    ...
    void DataPoint::submitUpdate(boost::shared_ptr<TA_Base_Bus::DpDynamicSyncUpdate> dpState )
	{
		...
		if ( TA_Base_Core::Control == m_operationMode )
		{
            ...
            // is enabled
			if ( ( false == m_forcedState ) && ( true == m_logChanges ) && (TA_Base_Bus::ValueUpdate == dpState->updateType) && 
				( !((m_previousFieldValueQualityStatus == QUALITY_BAD_NOT_CONNECTED) && (dpState->value.quality == QUALITY_GOOD_NO_SPECIFIC_REASON)) ) 
				)//TD18579, only the value update is considered as state change
            {
                ...
                if ( (TA_Base_Bus::DptNoAlarm == m_previousAlarmState && 
                     ...
                     (TA_Base_Bus::DptNoAlarm != m_previousAlarmState && 
                      TA_Base_Bus::DptNoAlarm != m_alarmState) )
                {
                    TA_Base_Core::Event stateChange( new DataPointStateChangeEvent( dpState, *this, TA_Base_Core::DataPointAudit::DataPointStateChange ) );
                    TA_Base_Core::EventProcessingLibrary::instance()->processEvent( stateChange );
                }
            }
    ...
    DataPointWorkItem::DataPointWorkItem(DataPoint& parent)
		: m_parent (parent)
	{
	}

	DataPointWorkItem::~DataPointWorkItem ()
	{
	}

	void DataPointWorkItem::executeWorkItem()
	{
		//
		// callback from ThreadPool for scheduled work
		//

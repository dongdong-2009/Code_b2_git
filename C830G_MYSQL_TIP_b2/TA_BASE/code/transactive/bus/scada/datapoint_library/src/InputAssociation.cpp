/**
  ...
  * any form.
  *
  * Source : $File: //depot/4669_T01271350/TA_BASE/transactive/bus/scada/datapoint_library/src/InputAssociation.cpp $
  * @author  HoaVu
  * @version $Revision: #3 $
...
namespace TA_Base_Bus
{
    ...
    m_workItem ()
	{
        ...
        m_workItem.reset(new InputAssociationWorkItem(*this));
	...
	void InputAssociation::resolveCurrentDataPointState()
	{
        ...
        if ( m_inputDataPointRef->isConfigValid() )
        {
            try
            {
				...
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Received EntityUpdateEvent from DataPointProxy of DataPoint, NAME:%s, VALUE:%s", getInputDataPointName().c_str(), value.getValueAsString().c_str() );
	...
	void InputAssociation::updateCurrentState ( const DpValue & dpv,
												const timeb & tmstamp,
	...
	void InputAssociation::executeWorkItem()
	{
		...
		{
			//TD13275, change LOG to LOG_GENERIC
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Start of callback for latest changes of DataPoint, NAME:%s", getInputDataPointName().c_str());
			...
			//TD13275, change LOG to LOG_GENERIC
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "End of callback for latest changes of DataPoint, NAME:%s", getInputDataPointName().c_str());
		}
		// else do nothing		
	}
    ...
    InputAssociationWorkItem::InputAssociationWorkItem(InputAssociation& parent)
		: m_parent (parent)
	{
	}

	InputAssociationWorkItem::~InputAssociationWorkItem ()
	{
	}

	void InputAssociationWorkItem::executeWorkItem()
	{
		//
		// callback from ThreadPool for scheduled work
		//

...
namespace TA_Base_Bus
{
	...
	const unsigned long ControlUUIDListMaxSize(20);
	...
	public:
		 ...
		 *
		 * Call back when the timer has expired.
		 *
		 */
		 ...
		 * startControlTimeOut
		 *
		 * start control time out timer.
		 *
		 * @param waitTime not equal 0.
		 */
		virtual bool startControlTimeOut(unsigned long waitTime);

		/**
		 * stopControlTimeOut
		 *
		 * stop control time out timer.
		 *
		 */
		virtual bool stopControlTimeOut();
		...
		/**
	      *  submitWriteRequest
	      * 
	      *  write the datapoint value with cancel time and control uuid
	      * 
	      * @return virtual void& 
	      * 
	      */
        virtual void submitWriteRequest( const std::string& sessionId,
								  const TA_Base_Bus::DataPointValue& value,
								  unsigned long triggeringDpKey, const std::string& triggeringDpValue,
	   ...
	   /**
		...
		/**
		 * executeLaunchCondition
		 *
		 * execute launch condition check, if passed return true;
		 *
		 */
		virtual bool executeLaunchCondition(
			const std::string& sessionID,
			const DpValue& engVal);

		/**
		 * isWriteRequestExpired
		 *
		 * check the command;
		 * 
		 * commandId
		 */
		virtual bool isWriteRequestExpired(unsigned long commandId);
    ...
    protected:
        ...
        virtual void writeDataPoint( const std::string& sessionId,
							 ...
							 timeb cancelTime = SCADA_DEFAULT_TIMEB);
	...
	private:
	    /**
	      ...
	      * @exception <exceptions> Optional
	      */
        void updateSynInhibitState(const TA_Base_Bus::InhibitState& updateinhibit);
		 ...
		 * @exception <exceptions> Optional
		 */
		void writeDataPointWithChecking( const std::string& sessionId,
							 const TA_Base_Bus::DataPointValue & value,
							 bool isLoggingEvent,
    ...
    protected:
		...
		TA_Base_Core::ReEntrantThreadLockable		m_timerLock;
		long										m_timerId;

		TA_Base_Core::ReEntrantThreadLockable		m_ControlUUIDListLock;
		std::vector<std::string>					m_ControlUUIDList;
	};

    class DataPointWorkItem : public TA_Base_Core::IWorkItem
	{

	public:

		DataPointWorkItem(DataPoint& parent);

		virtual ~DataPointWorkItem();

...
CEditableListCtrl::CEditableListCtrl() 
	: m_cellManager(NULL),m_cellEditCtrl(NULL),m_singleClick(false), m_noCheckIconIndex(-1), m_checkIconIndex(-1), m_numImages(8), m_imageList(NULL), m_buttonProcessor(NULL)
{
}

CEditableListCtrl::CEditableListCtrl(bool singleClick, int numImages) 
	: m_cellManager(NULL),m_cellEditCtrl(NULL),m_singleClick(singleClick),m_noCheckIconIndex(-1), m_checkIconIndex(-1),m_numImages(numImages), m_imageList(NULL), m_buttonProcessor(NULL)
{
}

CEditableListCtrl::CEditableListCtrl(ICellManager *cellManager, bool singleClick, int numImages) 
    : m_cellManager(cellManager), m_cellEditCtrl(NULL),m_singleClick(singleClick),m_noCheckIconIndex(-1), m_checkIconIndex(-1),m_numImages(numImages), m_imageList(NULL), m_buttonProcessor(NULL)
{
...
void CEditableListCtrl::startEdit()
{
    ...
    switch (cellType)
    {
        ...
        case ICellManager::CellType::CT_SELECTION:
        {
            ...
            CListCellComboBox *combo = new CListCellComboBox(this, m_cellManager, cellRect, item, subItem, GetItemText(item, subItem), cellData);
...
void CEditableListCtrl::OnKillfocus(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	{
		if(::IsWindow(this->m_cellEditCtrl->m_hWnd))
		{

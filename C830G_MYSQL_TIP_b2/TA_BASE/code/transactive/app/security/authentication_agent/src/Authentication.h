 ...
 * 
 * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/security/authentication_agent/src/Authentication.h $
 * @author:   Nick Jardine
 * @version:  $Revision: #2 $
 *
 * Last modification: $DateTime: 2009/01/08 10:29:24 $
...
namespace TA_Base_App
{
    ...
    public:
         ...
         */
        std::string changeProfile(const std::string & sessionId, unsigned long profileId, unsigned long locationId, const std::string & password);
         ...
         * 
         *
         * update Override Infomation to Monitor mode's AuthenticationAgent
         *
		 * @param    sessionId  The ID of the session to which the profile is to be added or cleared
		 * @param    operatorId The operator to add to the session, for addOverride only
		 * @param    profileId  The profile to add to the session, for addOverride only
		 * @param		isAddOverride  TRUE for addOverride, FALSE for clearOverride
         *
         * @exception AuthenticationAgentException  Thrown if session is not valid.
         */
		void updateMonitorOverrideInfo(const std::string& sessionId, unsigned long operatorId, unsigned long profileId, bool isAddOverride=true);

 ...
 * 
 * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/security/authentication_agent/src/SessionRegister.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #12 $
 *
...
namespace TA_Base_App
{
    SessionRegister::SessionRegister( bool isControl, unsigned long entityKey ) /*chenlei++ CL-21183*/: m_entityKey( entityKey )
    {
        ...
        if( isControl )
        {
			...
			try
			{
				...
				//ownedSessions = TA_Base_Core::SessionAccessFactory::getInstance().getOwnedSessions( entityKey );
				//-+chenlei
			}
            ...
            for( it=ownedSessions.begin(); it!=ownedSessions.end(); it++ )
            {
				...
				if ( !(*it)->getHasEnded() )
				{
					...
					//chenlei+- CL-21183
					//addSession(sessionInfo, true);
					TA_Base_Core::ThreadGuard guardB( m_activeTimesAccessLock );
					m_sessionActiveTimes.insert( std::make_pair( sessionInfo.SessionId, time(0) ) );
					//+-chenlei CL-21183
				}
				// ++ly
        ...
        if (!exists)
        {
			...
			*/
			...
			else
			{
				...
				if ( getOwnedSessionInfoFromDb(sessionId, tmpSession) )
				{
					/*
					...
					*/	
					...
					{
						TA_Base_Core::ThreadGuard guardB( m_activeTimesAccessLock );
						m_sessionActiveTimes.insert( std::make_pair( tmpSession.SessionId, time(0) ) );
					}
	...
	//bool SessionRegister::isGhostSession( const std::string &sessionId ) const
	//{
	//	TA_Base_Core::ThreadGuard guard( m_ghostSessionLock );

...
namespace
{
	...
	const unsigned short RESPONSE_TIME( 2 );	// CL-20822++
};
...
namespace TA_Base_App
{
    ...
    std::string Authentication::requestSession(unsigned long userId, unsigned long profileId, unsigned long locationId, 
        unsigned long workstationId, const std::string & password)
    {
    ...
    void Authentication::changePassword(const std::string & sessionId, const std::string & oldPassword, const std::string & newPassword)
    {
        ...
        if (0 != user.password.compare(oldPassword) )
        {
        ...
        unsigned long locationId, const std::string & password)
    {
		...
		sendStateUpdateMessage(m_sessionRegister.getSession(sessionId),ProfileUpdate);
        sendSessionNotification(m_sessionRegister.getSession(sessionId),ProfileUpdate);
        ...
        unsigned long locationId, const std::string & password)
    {
		...
		if (profileId == session.ProfileId[TA_Base_Bus::Authentication::BASE_PROFILE])
        {
            // The user is actually logged into this profile already. Throw an AuthenticationSecurityException of type
            // INVALID_PROFILE
            TA_THROW(TA_Base_Core::AuthenticationSecurityException("User already logged in with specified profile - no need to create new session",
        ...
        //    session.WorkstationId,profileId,m_localLocationId,m_entityKey) );
        ...
        // Place the session on the monitored queue. The queue will take responsibility for the life-time of the session
        // object, so do not need to delete it in here.
        m_sessionQueue.push( theSession.release() );
        ...
        unsigned long locationId,const std::string& password)
    {
        ...
        // Check the password, System Agent Operator do not need to check password.
		if(!TA_Base_Core::OperatorAccessFactory::getInstance().getOperator(user.userId,false)->isSystemOperator())
		{
			if (0 != user.password.compare(password) )
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug, "Authentication::checkUserSecurity!=>Wrong Password, db password: %s, passed password:%s", user.password.c_str(), password.c_str());
    ...
    void Authentication::sendStateUpdateMessage(const TA_Base_Bus::SessionInfo & session, EUpdateType updateType)
    {
        ...
        if ((updateType == TA_Base_Bus::TA_Security::SessionStart)
			|| (updateType == TA_Base_Bus::TA_Security::ProfileUpdate))
        {
			if (updateType == TA_Base_Bus::TA_Security::SessionStart)
			{
				sessionInfo.updateType = TA_Base_Bus::TA_Security::SessionStart;
	...
	void Authentication::updateMonitorOverrideInfo(const std::string& sessionId, unsigned long operatorId, unsigned long profileId,
		bool isAddOverride /*TRUE for addOverride, FALSE for clearOverride*/ )
	{
		std::string s_isAddOrClear = (isAddOverride == true)?"Add Override":"Clear Override";

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
			"Begin updateMonitorOverrideInfo, SessionId: %s, OperatorId: %lu, ProfileId: %lu, Add or Clear: %s",
			sessionId.c_str(), operatorId, profileId, s_isAddOrClear.c_str());

		if(isAddOverride == true)
			m_sessionRegister.addOverride(sessionId,operatorId,profileId);
		else
			m_sessionRegister.clearOverrides(sessionId);

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "End updateMonitorOverrideInfo");
	}
}

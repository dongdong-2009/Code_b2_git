 ...
 * 
 * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/security/duty_agent/src/TransferableRegionStateConverter.cpp $
 * @author:   Gregg Kirkpatrick
 * @version:  $Revision: #3 $
 *
 * Last modification: $DateTime: 2010/03/19 10:28:37 $
...
namespace TA_Base_App
{
	...
	void TransferableRegionStateConverter::regionAdd(const TA_Base_Bus::DutyPrimitive& session)
	{
		...
		// gongzong++ CL-21236
		// same info wo only need one copy, otherwise duty mgr will display bewildering information,
		// like the follow example:
		// Location  |  Operator  |  Profile
		// OCC       |            |
		// OCC       |            |
		// DBG       |            |
		// DBG       |            |
		// we filter same NULL line. and we don't need increase the container.
		if (m_alreadyAdd)
		{
			return;
		}
	...
	void TransferableRegionStateConverter::regionRemove(const TA_Base_Bus::DutyPrimitive& session)
	{
		...
		//this ghost session information will cause DutyManager cannot get transferable region duty

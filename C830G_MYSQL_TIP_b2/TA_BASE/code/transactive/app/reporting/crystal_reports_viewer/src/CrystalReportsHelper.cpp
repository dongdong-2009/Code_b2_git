/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/reporting/crystal_reports_viewer/src/CrystalReportsHelper.cpp $
  * @author:  B. Carbonaro
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2009/04/16 17:00:41 $
...
namespace TA_Base_App
{
    namespace TA_Reporting
    {
		...
		: m_pIApplication( NULL ), m_pIReport( NULL ), m_ccbstrReportSource( "" ),
							...
							if (SUCCEEDED(hr))
							{
								...
								for (i = 1; i <= lIndex; i++)
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										//TD 13191
										std::string connectionString = TA_Base_Core::RunParams::getInstance().get(RPARAM_DBCONNECTION);

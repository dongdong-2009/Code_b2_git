...
namespace TA_Base_App
{
    ...
    BOOL RestoreArchiveDlg::OnInitDialog() 
    {
        ...
        m_archiveList.InsertColumn( DATE_COL,  _T("Date"), LVCFMT_LEFT, dateWidth, DATE_COL);
        m_archiveList.InsertColumn( STATUS_COL,  _T("Status"), LVCFMT_LEFT, statusWidth, STATUS_COL);
    ...
    void RestoreArchiveDlg::populateList()
    {
        ...
        for( RestoreArchiveHelper::FileDetailsVector::iterator it = m_fileDetails.begin(); it != m_fileDetails.end(); ++it )
        {
			...
			std::string dateString = TA_ArchiveManager::convertTimeTToStringDate( it->date );
			it->dateStr = dateString;
            m_archiveList.SetItemText( item, DATE_COL, dateString.c_str() );
    ...
    bool RestoreArchiveDlg::getTickedItems( RestoreArchiveHelper::DatesVector& restoredDates, std::string& volumeLabel )
    {
	    ...
	    for ( int item = 0; item < itemCount; item++ )
        {
            ...
            if( ( 1 == state ) && ( (m_archiveList.GetItemText( item, STATUS_COL ) == RestoreArchiveHelper::RESTORABLE.c_str() ) ||
											(m_archiveList.GetItemText( item, STATUS_COL ) == RestoreArchiveHelper::NOTPROCESSED.c_str())))
            {
				time_t date = getDateFromList(m_archiveList.GetItemText( item, DATE_COL ));
				if(date != -1)
				{
					restoredDates.push_back(date );
				}
            }
        }
	...
	time_t RestoreArchiveDlg::getDateFromList(CString dateString)
	{
		for( RestoreArchiveHelper::FileDetailsVector::iterator it = m_fileDetails.begin(); it != m_fileDetails.end(); ++it )
        {
			if(dateString.Compare(it->dateStr.c_str()) ==0)
			{
				return it->date;
			}
		}

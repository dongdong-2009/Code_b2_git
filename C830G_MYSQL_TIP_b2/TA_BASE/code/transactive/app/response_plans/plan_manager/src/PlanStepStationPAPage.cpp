...
// in any form.
//
// Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepStationPAPage.cpp $
// @author:  Andy Parker
// @version: $Revision: #1 $
//
...
void CPlanStepStationPAPage::DisplayParameters()
{
    ...
    try
    {
        ...
        for (int i = 0; i < zoneCnt; i++)
        {
            ...
            std::string zoneName = zone->getName ().c_str ();
...
void CPlanStepStationPAPage::UpdateParameters()
{
    ...
    {//single station
        ...
        step->getStationDestinationParameter()->setStation (station);
    }
    else
    {//all stations
...
void CPlanStepStationPAPage::OnZonesBtn ()
{
    ...
    CPAStationZones zonesDlg (m_stationName, locationKey, station, false);
...
void CPlanStepStationPAPage::OnMessagesBtn ()
{
    ...
    CPaMessagesDlg messagesDlg (PA_STATION_MESSAGE_TYPE, messagePar, false);
...
void CPlanStepStationPAPage::PopulateStationsCombo ()
{
    ...
    try
    {
        ...
        {
            TA_Base_Core::IPaStation* item = *it;
            std::string strStationName = item->getName ().c_str ();
            ...
            m_stationCombo.SetItemData (index, item->getLocationKey ());

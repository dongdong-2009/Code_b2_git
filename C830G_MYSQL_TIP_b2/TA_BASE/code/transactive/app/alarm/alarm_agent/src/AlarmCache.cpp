...
namespace TA_Base_App
{
	...
	*/
    ...
    // insert
    //
    bool AlarmCache::insert( const TA_Base_Core::AlarmDetailCorbaDef& p_alarm,
            ...
            alarm->state == TA_Base_Core::AlarmSuppressed )
        {
			...
			try  //add this try..catch. so the "current_AlarmID" can be popped up in any case. 
			{
				...
				if ( alarm->state == TA_Base_Core::AlarmOpen )
				{
					...
					p_closeTime==0? update->closeTime = curTime: update->closeTime = p_closeTime;
					...
					{
						update->closeValue = CORBA::string_dup(alarmType->getNormalValue().c_str());
					}
                ...
                initAlarmAgentQueueItemCorbaDef(queueItem);
					...
					// update display time to close time and value
					alarm->displayTime.time = update->closeTime;
					alarm->displayTime.millitm = 0;
					alarm->alarmValue = update->closeValue;
					alarm->state = TA_Base_Core::AlarmClosed;  // for event
					// CL-21508 begin
               		 AlarmQueueWrapper* pNewWrapper = new AlarmQueueWrapper( queueItem, *alarm );
					m_queue->push(  pNewWrapper ); //ok
					//	m_queue->push(queueItem);				 
					...
					doMessageSendingForUpdatedAlarm(update.get(), alarm->entityName.in(), "", alarm->assetName.in());
				}
			...
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,
				"Closing alarm with uuid %s, entitykey %u, type %u and asset %s",
				alarm->alarmID.in(),
				alarm->assocEntityKey,
				alarm->messageTypeKey,
				alarm->assetName.in());
...
//             }
                                                 ...
                                                 const std::string& p_session )
    {
        ...
        AlarmQueueWrapper* pNewWrapper = new AlarmQueueWrapper( queueItem, *alarm );
        //m_queue->push(queueItem);
        m_queue->push(  pNewWrapper );  //review is ok
        // CL-21508 end
	...
	//
	void AlarmCache::raiseExceededMaxAlarm(std::string problem/*,bool raise*/)
	{
...
//		}
    ...
    {
        FUNCTION_ENTRY( "getBatchQueueUpdate" );
	...
	std::string AlarmCache::obtainValueFromParameter(unsigned long alarmTypeKey, std::string alarmType, TA_Base_Core::AlarmDetailCorbaDef* alarm, bool isSystemAlarm /*FALSE*/, bool isClosed /*FALSE*/)
	{
		...
		if (isSystemAlarm)
		{
			//std::auto_ptr<TA_Base_Core::IAlarmTypeData> alarmTypeData = std::auto_ptr<TA_Base_Core::IAlarmTypeData>(TA_Base_Core::AlarmTypeAccessFactory::getInstance().getAlarmType( alarmType, false ));
			//if (isClosed) value = alarmTypeData->getNormalValue();
			//else value = alarmTypeData->getAlarmValue();
			//fix memory leak;			
		...
		//jinhua++ TD17255
		else if ( alarmType.find( "CctvTriggeringAlarm", 0  ) != std::string::npos )
		{
			if ( isClosed )
			{
				value = "NORMAL";
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[TD17255]obtainValueFromParameter, alarmType=%s, isClose=%d, value=%s",
				alarmType.c_str(), isClosed, value.c_str());
		}
		//++jinhua TD17255
		else // this is not system alarm and can be a datapoint type. check the param list for values
		{
			...
			if(found==true && valueParam != description.end()) // case of value found for the appropriate alarm state
			{
				...
				if (alarmType.find("DpGeneral", 0) >= 0 || alarmType.find("DpFalse", 0) >=0 || alarmType.find("DpTrue", 0) >= 0)
					isBoolDatapt = true;
			...
			{
				// well, use back the open value (this is the previous implementation)
				// Check if the text contains DpGeneral, DpFalse and DpTrue
				// Take them out // this is a quick fix for TD 12785
				bool isBoolDatapt = false;
				if (alarmType.find("DpGeneral", 0) >= 0 || alarmType.find("DpFalse", 0) >=0 || alarmType.find("DpTrue", 0) >= 0)
			...
			else // there is no value in the param list
			{
    ...
    std::string AlarmCache::getOperatorNameFromSession(const std::string& p_session)
    {
        try
        {
            std::map<std::string, std::string>::iterator p = m_operatorMap.find(p_session);
            ...
            if( m_authLib != NULL )
            {
                ...
                m_operatorMap.insert(std::pair<std::string, std::string>(p_session, name));

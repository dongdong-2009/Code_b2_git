/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/configuration/config_plugins/alarm_rules/src/AlarmRulesDetailView.cpp $
...
namespace TA_Base_App
{
    ...
    const CString AlarmRulesDetailView::CHANGE_ENTITYTYPE_DIALOG_TITLE = "Change Entity Type";
    const CString AlarmRulesDetailView::CHANGE_ALARMTYPE_DIALOG_TITLE  = "Change Alarm Type";
...
END_MESSAGE_MAP()
    ...
    void AlarmRulesDetailView::populateItemDetails(TA_Base_Core::IConfigItem* currentItem)
    {
        ...
        try
        {
            ...
            m_entityType.SetWindowText( m_currentAlarmRule->getEntityTypeName().c_str() );
    ...
    void AlarmRulesDetailView::onKillfocusEditAlarm() 
    {
        ...
        try
        {
            ...
            if (alarmTypeString == m_currentAlarmRule->getAlarmTypeName().c_str())
            {
                m_alarmType.setTextType( TA_Base_Bus::TextColourEdit::VALID );
            }
            else
            {
                m_currentAlarmRule->setAlarmTypeByName( std::string(alarmTypeString) );
    ...
    void AlarmRulesDetailView::onKillfocusEditEntity() 
    {
        ...
        try
        {
            ...
            if (entityString == m_currentAlarmRule->getEntityName().c_str())
            {
                m_entity.setTextType( TA_Base_Bus::TextColourEdit::VALID );
            }
            else
    ...
    void AlarmRulesDetailView::onKillfocusEditEntitytype() 
    {
        ...
        {
            // If the entity type string has not changed then there is no point doing an update
            if (entityTypeString == m_currentAlarmRule->getEntityTypeName().c_str())
            {
                m_entityType.setTextType( TA_Base_Bus::TextColourEdit::VALID );
            }
            else
    ...
    void AlarmRulesDetailView::onSelectionChangeComboTrigger()
    {
        ...
        try
        {
            m_currentAlarmRule->setRuleTrigger( std::string(triggerString) );
        }
        catch( ... )
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "Unknown",

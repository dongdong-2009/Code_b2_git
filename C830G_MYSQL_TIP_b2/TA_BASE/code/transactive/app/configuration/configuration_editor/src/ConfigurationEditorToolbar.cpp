/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/configuration/configuration_editor/src/ConfigurationEditorToolbar.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #2 $
  *
...
namespace TA_Base_App
{
    ...
    ConfigurationEditorToolBar::ConfigurationEditorToolBar(CFrameWnd* toolBarParent,
                                                           ComponentLibraryAccessor& libraryAccessor)
        : m_toolBarParent(toolBarParent), m_libraryAccessor(libraryAccessor)
    {
    ...
    void ConfigurationEditorToolBar::populateToolBar()
    {
...
#ifdef TDS_DEVELOP
		m_toolBar.GetToolBarCtrl().DeleteButton(1);
#endif
        m_toolBar.GetToolBarCtrl().DeleteButton(0);

        // Now add any static buttons we require
        addButton("Reload", LoadIcon( AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_CONFIG_MENU_RELOAD) ), ID_APP_RELOAD);

#ifdef TDS_DEVELOP
		//only TDSCE can load toolbar item TDSNewlog, add by hongzhi
		if (TA_Base_Core::RunParams::getInstance().isSet(RPARAM_TDSCE))
		{
			addButton("Tdslog",LoadIcon( AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_CONFIG_NEWTDSLOG) ), ID_TDSLOG_NEW);
		}
#endif
             ...
             ++iter)
        {
            ...
            // new to us.
            for (std::vector<ToolBarItem>::iterator toolbarItem = items.begin();
                 toolbarItem != items.end();
                 ++toolbarItem
                )
            {
        ...
        for (ToolBarSetup::iterator item = toolbarSetup.begin();
             item != toolbarSetup.end();
             ++item)
        {
...
#ifdef TDS_DEVELOP
	//This will show hidden tds button to the toolbar, add by hongzhi
    void ConfigurationEditorToolBar::showButton(int nID, BOOL bHide)
    {
		CToolBarCtrl& toolbar = m_toolBar.GetToolBarCtrl();
    ...
    void ConfigurationEditorToolBar::onToolBarItemSelected(UINT nID, Component* selectedComponent)
    {
        ...
        if (selectedComponent == NULL)
        {
            ...
            userMsg << menuItemStr;
            userMsg.showMsgBox(IDS_UE_030057);
            FUNCTION_EXIT;
            return;
        }
        ...
        if (configurable == NULL)
        {
            TA_Base_Bus::TransActiveMessage userMsg;
            userMsg << menuItemStr;
            userMsg.showMsgBox(IDS_UE_030058);
            FUNCTION_EXIT;
            return;
        }

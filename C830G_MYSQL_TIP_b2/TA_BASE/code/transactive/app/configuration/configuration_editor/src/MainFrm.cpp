...
/*
	...
	void CMainFrame::onTDSLogNew()
	{
		if (TA_Base_Core::RunParams::getInstance().isSet(RPARAM_TDSCE))
		{
			...
			try
			{
				std::string fileName = WriteTDSLog();			// write sql script to file
...
*/				eraseTDSLOG_FromDB();
				...
				std::string str = "SQL file created successfully on: " + fullFileName;
...
*/
	...
	const std::string CMainFrame::GetFileName()
	{
		// construct full name
	...
	std::string CMainFrame::WriteTDSLog()
	{
		...
		else
		{
			...
			try
			{
				...
				sql << "select LOG_CONTENT from TDS_SQLLOG";
				
				// Set up the columnNames vector to be passed to executeQuery()
				std::string logContent( "LOG_CONTENT" );
				
				std::vector<std::string> tdsRecord;
				tdsRecord.push_back(logContent);
				...
				TA_Base_Core::IData* data = databaseConnection->executeQuery(sql.str().c_str(), tdsRecord);
				str += data->getStringData(0, "LOG_CONTENT");
				str += "commit;";				
				if ( (data->getNumRows())>1 )
					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,"Table TDS_SQLLOG has more than 1 row!");
			}
			catch (...)
			{
	...
	void CMainFrame::eraseTDSLOG_FromDB()
	{
		...
		{
			std::stringstream sql;
			sql << "update TDS_SQLLOG set LOG_CONTENT=''";
			TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::OnlineUpdatable_Cd, TA_Base_Core::Read);
	...
	bool CMainFrame::isWorkingOnTdsMode()
	{
		...
		{
			std::stringstream sql;
			sql << "SELECT count(*) TABLE_COUNT from tds_mode where mode_name = " << "'Develop mode'";
			...
			// We are responsible for deleting the returned IData object when we're done with it
			TA_Base_Core::IData* data = databaseConnection->executeQuery(sql.str().c_str(), tdsRecord);
			unsigned long nRecordCount = data->getUnsignedLongData(0, "TABLE_COUNT");

 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/event/event_viewer/src/EventViewerDlg.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #2 $
 *
...
const int     EventViewerDlg::NAMED_FILTER_LABEL_SEPARATOR_WIDTH = 60;
...
const int     EventViewerDlg::EVENT_LEVEL_LABEL_SEPARATOR_WIDTH = 50;
...
LRESULT  EventViewerDlg::OnListUpdate(WPARAM size,LPARAM loadFlags)
{
    ...
    m_statusBar.SetPaneInfo( m_statusBar.CommandToIndex(ID_INDICATOR_SIZE), // The index of the pane in the status bar
                            ...
                            //90 + (( (int) log10(size) ) + 1)*9 ); // The size of the pane - 9 times the size of the number plus a constant
...
// 	{
// 		// And tell the list to update. The NOSCROLL style tells the list to maintain its current position (by default
// 		// it would reset back to the start of the list - which would be bad). The LVSICF_NOINVALIDATEALL is applied for
// 		// all situations EXCEPT when a refresh occurs (as new elements may be inserted into the visible area) or the
// 		// the page is empty (get redrawing issues otherwise).
// 		unsigned long flags = LVSICF_NOSCROLL|LVSICF_NOINVALIDATEALL;
// 		if ( ( loadFlags & Filter::MASK_REFRESH ) || ( 0 == size ) )
// 		{
// 			flags &= ~LVSICF_NOINVALIDATEALL;
// 		}
// 		m_eventList.SetItemCountEx( size, flags );
// 
// 		// Update the page status buttons - only if the update was from a load.
// 		if ( ! ( loadFlags & Filter::MASK_REFRESH ) )
// 		{
...
void EventViewerDlg::setUpToolBar()
{
	...
	VERIFY((tbButton.iString = m_eventsToolbar.AddStrings("Print")) != -1);
	...
	VERIFY((tbButton.iString = m_eventsToolbar.AddStrings("Export")) != -1);
	...
	VERIFY((tbButton.iString = m_eventsToolbar.AddStrings("Filter")) != -1);
	...
	VERIFY((tbButton.iString = m_eventsToolbar.AddStrings("Incident Report")) != -1);
	...
	VERIFY((tbButton.iString = m_eventsToolbar.AddStrings("Alarm Manager")) != -1);
...
void EventViewerDlg::setWindowCaption()
{
    ...
    {   
		...
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"setWindowCaption 0");
		// Set the title bar to show what the current filters are
		std::string windowText(RunParams::getInstance().get(RPARAM_APPNAME));
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"setWindowCaption come 1");
		std::string filterText(m_filter->getCurrentFilterString());
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"setWindowCaption come 2");

		if (0 < filterText.length())
		{
			windowText += " - Filtered on: " + filterText;
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"setWindowCaption come 3");
			SetWindowText(CString(windowText.c_str()));
		}
		else
		{
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"setWindowCaption come 4");
			SetWindowText(CString(windowText.c_str()));
		}
	}
    ...
    {
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError,"setWindowCaption Error");
    }

...
#define checkRadio(itemId, check)       checkButton(FooButton::Type::radio, itemId, check);
#define checkBox(itemId, check)         checkButton(FooButton::Type::checkBox, itemId, check);
#define initialiseRadio(button, group)      \
    button.setType(FooButton::Type::radio); \
...
namespace TA_Base_App
{
    ...
    void JobDialog::initialiseDateTimeCtrl(const int itemId, const COleDateTime* theTime, const COleDateTime* earliest /*=NULL*/, const COleDateTime* latest /*=NULL*/, const bool yearly /* = false */)
    {
        ...
        else
        {
            if (yearly)
            {
                dateTimeCtrl->SetFormat("dd-MMM");
                return;
            }        
            dateTimeCtrl->SetFormat("dd-MMM-yy");
...
/*				CComboBox* combo = new CComboBox;
    ...
    bool JobDialog::prepareDataToSave() 
    {
        ...
        try
        {
            ...
            if (sel == CB_ERR)
            {
                ...
                userMsg << itemName;
                userMsg.showMsgBox(IDS_UE_250006);
                return false;
            }
    ...
    void JobDialog::initialiseProfileCombo()
    {
        ...
        TA_Base_Bus::SchedulingAgentCorbaDef::ProfileID existingProfileId = m_job.getProfileId();
        ...
        if (m_readOnly)
        {
            ...
            catch (TA_Base_Core::DatabaseException& ex)
            {
                ...
                userMsg << actionName;
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_250002);
            }
            catch (...)
            {
    ...
    void JobDialog::onSelchangeProfile() 
    {
        ...
        try
        {
            ...
            {
                int pos = m_locationCtrl.AddString(m_locationMap[locations[i]]->getName().c_str());
            ...
            TA_Base_Bus::SchedulingAgentCorbaDef::LocationID locationId = (m_job.isNew()) ? m_doc.getSessionLocation() : m_job.getLocationId();

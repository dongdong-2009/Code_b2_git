...
#include "app/scheduling/scheduling_agent/src/JobInstanceCreationException.h"
...
const JobQueue::Mode JobQueue::CONTROL_MODE = ! MONITOR_MODE;
...
JobQueue::JobQueue()
    ...
    // Initialise the scheduled jobs queue to be empty
...
void JobQueue::readJobsFromDatabase()
{
    ...
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Reading jobs from database");
        ...
        for(it = jobs.begin(); it != jobs.end(); it ++)
        {
            ...
            catch(const TA_Base_Bus::SchedulingAgentCorbaDef::JobInPastException&)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn,
                            "JobInPastException caught for job with id '%s'."
                            "Ignoring job.", (*it)->getID());
    ...
    catch(...)
    {
        ...
        //
        for(IJob::IJobPtrVector::iterator deleteIt = jobs.begin();
            deleteIt != it && deleteIt != jobs.end();
            deleteIt ++)
        {
			TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobID = CORBA::string_dup((*deleteIt)->getID());
		...
		LOG_EXCEPTION_CATCH(SourceInfo, "Unknown Exception", "Unknow Exception Occurred"); // Wuzhongyi CL-21357 
        throw;
...
TA_Base_Bus::SchedulingAgentCorbaDef::JobID JobQueue::scheduleJob(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef& corbaJob)
{
    ...
    LOG_GENERIC(
        ...
        "Schedule:\n%s", std::string(corbaJob.description).c_str(), Schedule::toString( corbaJob.schedule ).c_str() );

    IJob* job = NULL;

    //
    // If the queue is in control mode, create a new job in the database.
    //
    ...
    {
        job = JobFactory::getInstance().getJobFromStore(CORBA::string_dup(corbaJob.identifier));
    }
    ...
    if(job != NULL)
    {
        ...
        return CORBA::string_dup(job->getID());
    }
...
void JobQueue::addJobToQueue(IJob* job)
{
    ...
    unsigned long minuteToStartJob = 
        job->getSchedule().getNextScheduledMinute();
    ...
    if ( minuteToStartJob != 0 )
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                    ...
                    "This is at %s",
                    job->getID(), minuteToStartJob,
                    SchedulingTimer::getFormatedTime( minuteToStartJob ).c_str());
        ...
        if (CONTROL_MODE == m_mode) // Merge from 4677_ALL_TIP
        {
            // tell those who care
...
void JobQueue::updateJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef& corbaJob, bool checkIfPastDate/*=true*/)
{
    ...
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
		"Updating job with ID '%s'.", std::string(corbaJob.identifier).c_str());
    ...
    IJob* job = JobInstanceManager::getInstance().getJob(CORBA::string_dup(corbaJob.identifier));
    ...
    if(job != NULL)
    {
        ...
        if(m_mode == CONTROL_MODE)
        {
            ...
            if ( unJobsCountAtMinute > MAX_ALLOWED_JOBS_AT_ONCE )
			{
				LOG_GENERIC(
					SourceInfo, DebugUtil::DebugError,
					"[TD16068] getNumJobsAtMinute( runTime ) > MAX_ALLOWED_JOBS_AT_ONCE");
				TA_THROW_CORBA_EXCEPTION(TA_Base_Bus::SchedulingAgentCorbaDef::TooManyJobsException());
			}
			else
			{
				LOG_GENERIC(
        ...
        //
        // Otherwise refresh the job from the database.
        //
        else
        {
...
void JobQueue::removeJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobToRemoveID, std::string sessionId /*=""*/)
{
    //
    // Lock the object
    //
...
void JobQueue::removeCompletedJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobToRemoveID)
{
    //
    // Remove the job
    //
...
//
void JobQueue::removeJobUnlocked(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobToRemoveID, bool deleteJobFromDb /*= true*/, std::string sessionId /*=""*/)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Removing job with ID '%s'.", jobToRemoveID);
    ...
    const TA_Base_Bus::SchedulingAgentCorbaDef::ScheduleCorbaDef& schedule)
{
    ...
    if ( Schedule::getNextScheduledMinute(schedule) == 0 )
    {
        ...
        TA_THROW_CORBA_EXCEPTION(TA_Base_Bus::SchedulingAgentCorbaDef::JobInPastException());
    }
...
// NOTE: THE OBJECT MUST BE LOCKED BEFORE CALLING THIS METHOD
//
JobQueue::ScheduledJobsQueue::iterator JobQueue::findScheduledJob(
...
void JobQueue::setControlMode()
{
    ...
    TA_ASSERT(m_mode != CONTROL_MODE,
             "JobQueue set to control mode when it was already in control "
             "mode");

    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "JobQueue set to control mode.");
    ...
    if(secondOfMinute > 30)
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                    "Second of minute > 30 (%d). Removing job instances.",
                    secondOfMinute);
    ...
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Second of minute < 30 (%d). Synchronising job instances.",
                secondOfMinute);
    ...
    {
        TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobID = CORBA::string_dup(instances[i].jobIdentifier);

        unsigned long minuteStarted =
            instances[i].minuteStarted;
        ...
        try
        {
            ...
            //
            if(job->getJobInstanceState(minuteStarted, errorMessage)
                        ==
               IJobInstance::NotStarted)
            {
            ...
            else
            {
                ...
                //
                JobInstanceManager::getInstance().removeJobInstance(
                    jobID, minuteStarted);
        ...
        catch(const JobInstanceStateRetrievalException& ex)
        {
			...
			// dahe++ for CL-21367
            //action << "retrieve the state of job with ID " << jobID
			action << "retrieve the state of job with description " << job->getDescription()
			// ++dahe
			
                   << " and instance ID " << minuteStarted;
...
void JobQueue::run()
{
    ...
    catch(const CORBA::Exception& ex)
    {
        ...
        errorMessage += CorbaUtil::getInstance().exceptionToString(ex);
    }
    catch(...)
    {
...
void JobQueue::processReadyJobs()
{
    ...
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, debugOutput.str().c_str());
        ...
        it ++)
    {
        ...
        debugOutput << "Job " << job->getID() << " with description '"
                    ...
                    << std::endl << job->getSchedule().toString()
                    << std::endl
                    << "scheduled for " << (*it).first << " "
                    << "minutes after the epoch." 
                    << " This is at " << SchedulingTimer::getFormatedTime( (*it).first )
                    << std::endl;
    ...
    while
    (
        ! m_scheduledJobsQueue.empty()  &&
        (*m_scheduledJobsQueue.begin()).first <= LAST_ELAPSED_MINUTE
    )
    {
        TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobID =
            (*m_scheduledJobsQueue.begin()).second;

        IJob* job = JobInstanceManager::getInstance().getJob(jobID);

        TA_ASSERT(job != NULL,
                  "JobInstanceManager should always know about "
                  "scheduled jobs");
    ...
    unsigned long minuteStarted)
{
    ...
    TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobID = job->getID();
    ...
    try
    {
        ...
        if( m_mode == CONTROL_MODE )
        {
            ...
            if( ! job->isInhibited() )
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                            "Job with ID %d executed at minute %d.",
                            job->getID(),
                            minuteStarted);
            }
            else
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                            "Job with ID %d not being started "
                            "at minute %d because it is inhibited.",
                            job->getID(),
                            minuteStarted);
            }
        }
        ...
        else
        {
            if( ! job->isInhibited() )
            {
                JobInstanceManager::getInstance().addJobInstance(
                    jobID, minuteStarted);
            }
        }
        ...
        //
        unsigned long nextMinuteToStartJob =
            job->getSchedule().getNextScheduledMinute();
        ...
        else
        {
            ...
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                        "Rescheduling Job with ID %d to run at minute %d.",
                        jobID,
                        nextMinuteToStartJob);
            //
    ...
    catch(const JobInstanceCreationException& ex)
    {
        std::string errorMessage = "Could not initialise job. ";
        errorMessage            += ex.what();
        SchedulingAlarmSender::getInstance().jobExecutionFailed(
            jobID, minuteStarted, job->getDescription(), errorMessage);
        removeCompletedJob(jobID);
...
void JobQueue::sleepTillNextMinuteStartsOrThreadTerminated()
{
    ...
    unsigned long secondOfMinute = ptime(second_clock::local_time()).time_of_day().seconds();
    if (!secondOfMinute )
    {
        minWaitMillis = 1000;
    }
    ...
    while(m_running   &&   (minWaitMillis  > 0 || 
          ptime(second_clock::local_time()).time_of_day().seconds()) != 0)
    {
...
{
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Terminating JobQueue");

...
void SchedulingMonitor::activate()
{
    ...
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Activating Scheduling Monitor");
...
void SchedulingMonitor::deactivate()
{
    ...
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Deactivating Scheduling Monitor");
    ...
    SpecialisedMessageSubscriber<StateUpdateMessageCorbaDef>* stateUpdatePtr =
        this;
...
void SchedulingMonitor::batchUpdateReceivedFromController(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobSequence& scheduledJobs)
{
    FUNCTION_ENTRY("batchUpdateReceivedFromController");

    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo,
        "Processing batch update received from controlling agent");
        ...
        for(unsigned int i = 0; i < scheduledJobs.length(); i ++)
        {
            ...
            catch(const TA_Base_Bus::SchedulingAgentCorbaDef::JobInPastException&)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn,
                            ...
                            "Ignoring job as it has probably already been run "
                            "by controlling agent.",
							std::string(scheduledJobs[i].identifier).c_str());
            }
            catch(const DataException& ex)
            {
                ...
                action <<
					...
					// dahe++ for CL-21367
                    //"schedule job with ID '" << scheduledJobs[i].identifier << "'" <<
					"schedule job with description '" << scheduledJobs[i].description << "'" <<
					// ++dahe

                    " and description '" << scheduledJobs[i].description << "'";
    ...
    {
        SchedulingAlarmSender::getInstance().schedulingMonitorError(
            "schedule a batch update", ex.what());
    }
    catch(...)
    {
        SchedulingAlarmSender::getInstance().schedulingMonitorError(
            "schedule a batch update", "unknown");
    }
}


void SchedulingMonitor::receiveSpecialisedMessage(
    const CommsMessageCorbaDef& message)
{
    ...
    if( std::string(message.messageTypeKey) ==
        TA_Base_Core::SchedulingComms::SchedulingCommsJobAdded.getTypeKey() )
    {
    ...
    //
             ...
             TA_Base_Core::SchedulingComms::SchedulingCommsJobUpdated.getTypeKey() )
    {
       	TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef* job = NULL;
    ...
    //
             ...
             TA_Base_Core::SchedulingComms::SchedulingCommsJobDeleted.getTypeKey() )
    {
		//const TA_Base_Bus::SchedulingAgentCorbaDef::JobID* jobID = NULL;
		...
		if( strcmp(jobID, "") != 0 )
		{
			TA_Base_Bus::SchedulingAgentCorbaDef::JobID strJobId;
			strcpy(strJobId, jobID);
...
void SchedulingMonitor::receiveSpecialisedMessage(
    const StateUpdateMessageCorbaDef& message)
{
    //
    // JobStarting message
    //
    if( std::string(message.messageTypeKey) ==
        TA_Base_Core::SchedulingStateUpdate::SchedulingStateUpdateJobStarting.getTypeKey() )
    {
    ...
    //
             ...
             TA_Base_Core::SchedulingStateUpdate::SchedulingStateUpdateJobCompleted.getTypeKey() )
    {
       	TA_Base_Bus::SchedulingAgentCorbaDef::JobInstanceCorbaDef* jobInstance = NULL;
...
void SchedulingMonitor::receiveJobAddedMessage(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef& corbaJob)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Job added message received. Job id: '%s'",
				std::string(corbaJob.identifier).c_str());
    ...
    action <<
		...
		// dahe++ for CL-21367
        //"schedule job with ID '" << corbaJob.identifier << "'" <<
		"schedule job with description '" << corbaJob.description << "'" <<
		// ++dahe

        " and description '" << corbaJob.description << "'";
    ...
    catch(const TA_Base_Bus::SchedulingAgentCorbaDef::JobInPastException&)
    {
        ...
        //
        LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn,
                    "JobInPastException caught. Probably job has already been"
                    "run by controlling agent.");
    }
...
void SchedulingMonitor::receiveJobUpdatedMessage(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef& updatedJob)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Job updated message received. Job id: '%s'",
				std::string(updatedJob.identifier).c_str());
    ...
    action <<
		...
		// dahe++ for CL-21367
        //"update job with ID '" << updatedJob.identifier << "'" << 
		"update job with description '" << updatedJob.description << "'" << 
		// ++dahe

        " and description '" << updatedJob.description << "'";
    ...
    {
        SchedulingAlarmSender::getInstance().schedulingMonitorError(
            action.str(), CorbaUtil::getInstance().exceptionToString(ex));
    }
...
void SchedulingMonitor::receiveJobDeletedMessage(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Job deleted message received. Job id: '%s'", jobID);
    ...
    if( NULL == job)
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "Job can not be found in JobInstanceManager. Job ID: '%s'", jobID);
        action << "delete job " << jobID;
    }

    action << "delete job with description " << job->getDescription();
...
void SchedulingMonitor::receiveJobStartingMessage(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobInstanceCorbaDef& jobInstance)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Job started message received. Job id: '%s', "
                "minute started: '%d'",
                std::string(jobInstance.jobIdentifier).c_str(),
                jobInstance.minuteStarted);
}


void SchedulingMonitor::receiveJobCompletedMessage(
    const TA_Base_Bus::SchedulingAgentCorbaDef::JobInstanceCorbaDef& jobInstance)
{
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                "Job completed message received. Job id: '%s', "
                "minute started: '%d'",
                std::string(jobInstance.jobIdentifier).c_str(),
                jobInstance.minuteStarted);
}

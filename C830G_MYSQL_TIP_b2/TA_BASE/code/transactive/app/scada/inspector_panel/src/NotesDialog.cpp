/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/scada/inspector_panel/src/NotesDialog.cpp $
...
namespace TA_Base_App
{
	...
	CNotesDialog::CNotesDialog(CWnd* pParent /*=NULL*/)
		: CDialog(CNotesDialog::IDD, pParent), m_notesChanged(false), m_equipment(NULL), 
		  ...
		  m_auditMessageUtility (TA_Base_Core::InspectorPanelAudit::Context), m_originalHeight (0)
	{
		...
		if (m_equipment != NULL)
		{
			...
			if (selectedButton == IDYES)
			{
				...
				{					
					...
					submitAuditMessage("Removed", dataNode->getAssetName(), dataNode->getEntityKey());
				}
	...
	bool CNotesDialog::updateNotes()
	{
		...
		if (m_equipment != NULL)
		{
			...
			try
			{
				...
				submitAuditMessage("Updated", dataNode->getAssetName(), dataNode->getEntityKey());
			}
			catch (TA_Base_Core::ScadaProxyException &e)
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, 
...
*/
	...
	void CNotesDialog::submitAuditMessage(std::string updateRemove, 

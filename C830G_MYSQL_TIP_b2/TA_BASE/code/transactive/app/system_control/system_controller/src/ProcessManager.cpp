...
namespace TA_Base_App
{
    ...
    void ProcessManager::cleanUp()
    {
        ...
        for (std::map<std::string, TA_Base_Core::IProcessSubjectCorbaDef_var>::iterator peerIter = m_peers.begin(); peerIter != m_peers.end(); peerIter++)
        {
            try
            {
...
// 					}
    ...
    void ProcessManager::terminateAllMonitors()
    {
        ...
        for ( MoniteredDbs::iterator it = m_moniteredDatabases.begin(); it != m_moniteredDatabases.end(); ++it )
        {
            if ( it->second != NULL )
            {
                it->second->terminate();
        ...
        for (iter = m_processes.begin(); iter != m_processes.end(); iter++)
        {
            ...
            if (process != NULL)
            {
                if (process->isServiceProcess())
                {
					...
					std::string notifyHosts = getNotificationHosts(process->getLocationKey());
    ...
    // limin++ CL-21243
    void ProcessManager::deployProcesses( TA_Base_Core::EOperationMode normalOperationMode, CORBA::Boolean isStart )
    {
        for ( EntityToProcessMap::iterator iter = m_processes.begin(); iter != m_processes.end(); ++iter )
        {
            ...
            if ( process != NULL )
            {
                if ( normalOperationMode == process->getNormalOperationMode() )
                {
                    if ( true == isStart )
                    {
                    ...
                    {
                        unsigned long processID = process->getProcessId();
                        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "processID %lu is asked to stop.", processID );
                        
                        process->stopProcess( TA_Base_Core::RequestedTerminate );
                        ...
                        //     // the initial value for time count is 0.
                        //     m_stoppingProcessIDToTimeCountMap[processID] = 0;
                        //     LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "processID %lu is put into stopping list", processID );
                        // }
                        }
                    }
                }

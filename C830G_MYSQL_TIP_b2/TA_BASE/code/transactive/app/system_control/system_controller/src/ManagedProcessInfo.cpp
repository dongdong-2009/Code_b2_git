...
namespace TA_Base_App
{
    ...
    {
        // limin++ CL-21243
        if ( true == TA_Base_Core::RunParams::getInstance().isSet( "StartSystemControllerOnly" ) )
        {
            m_statusToChangeTo = TA_Base_Core::Unstarted;
        }
        else
        {
            m_statusToChangeTo = TA_Base_Core::Startup;
    ...
    // getOperationMode
    //
    TA_Base_Core::EOperationMode ManagedProcessInfo::getOperationMode( bool is_queryRemote /* = true */, bool isLockFirst /* = true */ )
    {
        if ( !isRunning() )
        {
...
// 	}
    ...
    // getProcessData
    //
    void ManagedProcessInfo::getProcessData( TA_Base_Core::ProcessData& data, bool isLockFirst /* = true */ )
    {
        ...
        data.started = getTimeStarted( isLockFirst );

        static std::string hostname = ProcessManager::getInstance().getHostName();
        data.host = hostname.c_str();
        data.state = getProcessStatus( isLockFirst );
        data.requestedState = getStatusToChangeTo( isLockFirst );
        data.operationMode = getOperationMode( isLockFirst, isLockFirst );

        data.restarts = getNumRestarts( isLockFirst );
	...
	std::vector<std::string> ManagedProcessInfo::buildCommandLine(std::string& startPath)
	{
        ...
        if (isServiceProcess())
        {
            ...
            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "%s will be started with a notify port of %d", getEntityName().c_str(), m_notifyPort);
        }

        if ( TA_Base_Core::RunParams::getInstance().isSet( "PassOmniParamToAgent" ) )
        {
            if ( TA_Base_Core::RunParams::getInstance().isSet( "WithOmniLog" ) )
            {
                args.push_back( "--with-omni-log" );
            }

            if ( TA_Base_Core::RunParams::getInstance().isSet( "OmniToDbgLog" ) )
            {
                args.push_back( "--omni-to-dbg-log" );
            }

            if ( TA_Base_Core::RunParams::getInstance().isSet( "OmniLogTraceInvocations" ) )
            {
                args.push_back( "--omni-log-trace-invocations" );
            }

            if ( TA_Base_Core::RunParams::getInstance().isSet( "OmniLogLevel" ) )
            {
                std::ostringstream omniLogLevelArg;
                omniLogLevelArg << "--omni-log-level=" << TA_Base_Core::RunParams::getInstance().get( "OmniLogLevel" );
                args.push_back( omniLogLevelArg.str() );
            }
        }
    ...
    void ManagedProcessInfo::performStop(TA_Base_Core::EProcessStatus statusToChangeTo)
    {
        ...
        if (getProcessId() != 0)
        {
            ...
            if ( statusToChangeTo != TA_Base_Core::NotRunning )
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Sending SIGTERM to process %lu", getProcessId());

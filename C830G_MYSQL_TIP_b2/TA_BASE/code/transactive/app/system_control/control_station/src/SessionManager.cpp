...
SessionManager::~SessionManager()
{
    ...
    try
    {
        ...
        try
        {
            ...
            {
                m_controlStationImpl->deactivateAndDeleteServant();
        ...
        try
        {
            if (m_processManager != NULL)
            {
                ...
                m_processManager->deactivateAndDeleteServant();
                isCorbaShutdownRequired = true;
...
// limin++ CL-21031
class LoadBannerThread : public TA_Base_Core::Thread
{
public:

    LoadBannerThread(SessionManager& sessionManager, GraphWorxManager& graphWorxManager )
        : m_sessionManager( sessionManager ),
          m_graphWorxManager( graphWorxManager )
    {
        start();
    }

    virtual ~LoadBannerThread()
    {
        terminateAndWait();
    }
...
protected:
    ...
    virtual void run()
    {
        ...
        static unsigned int delayLaunchBanner = TA_Base_Core::getRunParamValue( "DelayLaunchBanner", 30 );
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Delay %d seconds before launch banners.", delayLaunchBanner );
        sleep( delayLaunchBanner * 1000 );

        m_sessionManager.loadBanners();
    }
...
/*
	...
	*/  
    ...
    // loadBanners(); // limin-- TEST
    LoadBannerThread loadBannerThread( *this, *m_graphWorxManager );
...
{
    if (TA_Base_Core::RunParams::getInstance().isSet("LoginAlways"))
    {
...
void SessionManager::retrieveRunParams(const std::string& hostName)
{
    ...
    try
    {
        TA_Base_Core::RunParamSeq* tmpParams = m_systemController->getParams(0,hostName.c_str());
        ...
        try
        {
			...
			*/
			...
			for (unsigned int i = 0; i < numBanners; i++)
            {
                ...
                static unsigned int delayBetweenBanner = TA_Base_Core::getRunParamValue( "DelayBetweenBanner", 3 );

                if ( i != 0 )
                {
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Delay %d seconds before launch next banner.", delayBetweenBanner );
                    Sleep( delayBetweenBanner * 1000 ); //limin++, CL-21031
                }
                OnlineUpdateListener::getInstance().registerInterest(TA_Base_Core::ENTITY_OWNER, *this, m_bannerEntities[i]->getKey());
                std::ostringstream commandLine;
                commandLine << "--entity-name=";
...
void SessionManager::isActionsPermitted(unsigned long action, ResourceList& resourceKeyList,ResourceList& permittedResourceKeyList)
{
    ...
    try
    {
		...
		std::string allResourceLog = "The Sent Resources are: ";
...
void SessionManager::processGxToTAMessage(const TA_Base_Core::CommsMessageCorbaDef& message)
{
	try
	{
		...
		{				
			if (event->entityType == TA_Base_Core::IManagedApplicationCorbaDef::EntityMessageType::ControlStation)
			{

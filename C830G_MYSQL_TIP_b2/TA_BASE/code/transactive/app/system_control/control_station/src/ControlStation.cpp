/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/system_control/control_station/src/ControlStation.cpp $
...
void ControlStationApp::archiveLogFiles(const std::string& debugFileName)
{
    ...
    if( pos != std::string::npos)
    {
        ...
        zipArchive.Open( (dirStr + "ArchivedLogs" + timeStr + ".zip").c_str(), CZipArchive::zipCreate);

        CFileFind finder;
        CFileStatus filestatus;
        
        BOOL bWorking = finder.FindFile((dirStr+"*.*").c_str());
        while (bWorking)
        {
            ...
            if(!finder.IsDots() && !finder.IsDirectory() && finder.GetFilePath().Find("zip") == -1 )
            {
                ...
                if( filestatus.m_mtime < time_to_clear_file )
                {
                    ...
                    catch(...)
                    {
                        ...
                        reason<<"Delete log file "<<(LPCSTR)filePath<<" failed.";
                        LOG(SourceInfo, DebugUtil::ExceptionCatch, "Unknown", reason.str().c_str());                        
                    }

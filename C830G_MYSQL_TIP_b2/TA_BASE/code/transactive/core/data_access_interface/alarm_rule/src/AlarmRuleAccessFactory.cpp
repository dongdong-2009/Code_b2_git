/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/alarm_rule/src/AlarmRuleAccessFactory.cpp $
...
//		TA_ASSERT(sql.size() > 0, "getAlarmRule: GetSQLStatement error");
        ...
        if (nRows == 0)
		{
			...
			TA_THROW(DataException("Alarm rule not found",DataException::NO_VALUE, strExSql));
		...
		if (nRows > 1)
		{
			...
			TA_THROW(DataException("More than one alarm rule found", DataException::NOT_UNIQUE, strExSql ));

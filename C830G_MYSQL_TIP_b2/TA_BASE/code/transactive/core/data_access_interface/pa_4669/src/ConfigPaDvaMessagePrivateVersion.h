/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/pa_4669/src/ConfigPaDvaMessagePrivateVersion.h $
  * @author Jade Lee
  * @version $Revision: #1 $
...
#if !defined ConfigPaDvaMessagePrivateVersion_H
#define ConfigPaDvaMessagePrivateVersion_H
...
namespace TA_Base_Core
{
    ...
    class ConfigPaDvaMessagePrivateVersion : public IConfigPaDvaMessagePrivateVersion
    {

    public:
        /**
         * ConfigPaDvaMessagePrivateVersion (constructor)
         *
         * Constructs a new instance of the ConfigPaDvaMessagePrivateVersion with no id. This is used when
         * creating a *new* PaDvaMessagePrivateVersion - that is, one that does not yet exist in the database.
         */
        ConfigPaDvaMessagePrivateVersion();
         ...
         *
         */
        ConfigPaDvaMessagePrivateVersion(unsigned long row, TA_Base_Core::IData& data);

        /**
         * ConfigPaDvaMessagePrivateVersion (constructor)
         *
         * Constructs a new instance of ConfigPaDvaMessagePrivateVersion with the specified id. This is used 
         * when creating a ConfigPaDvaMessagePrivateVersion based around an existing PaDvaMessagePrivateVersion in the database.
         *
         * @param key The key to this PaDvaMessagePrivateVersion in the database.
         */
        ConfigPaDvaMessagePrivateVersion(const unsigned long idKey);


        /**
         * ConfigPaDvaMessagePrivateVersion (copy constructor)
         *
         * @param ConfigPaDvaMessagePrivateVersion& - The PaDvaMessagePrivateVersion to make a copy of.
         */
        ConfigPaDvaMessagePrivateVersion( const ConfigPaDvaMessagePrivateVersion& theConfigPaDvaMessagePrivateVersion);

        
        virtual ~ConfigPaDvaMessagePrivateVersion();
    ...
    public:
         ...
         * @exception DataConfigurationException If the data contained in the PaDvaMessagePrivateVersion object
         ...
         *            all sections.
         *
         * pre: This PaDvaMessagePrivateVersion has not been deleted
         */
        void applyChanges();
         ...
         * getKey
         *
         * Returns the key for this PaDvaMessagePrivateVersion.
         *
         * @return The key for this PaDvaMessagePrivateVersion as an unsigned long.
         *
         * pre: Either - this PaDvaMessagePrivateVersion was initially loaded from the database
         *      OR     - writePaDvaMessagePrivateVersionData() has already been called
        ...
        {
            return m_paDvaMessagePrivateVersionChanges;
        }
       ...
       /**
         ...
         * hasChanged
         *
         * This should be called to determine if any part of the PaDvaMessagePrivateVersion has been changed by the user.
         *
         * @return bool - This will return true if the PaDvaMessagePrivateVersion does not match the one in the database. It 
         *                will return false if the data has not changed from that in the database.
         */
        virtual bool hasChanged()
        {
            // If the PaDvaMessagePrivateVersion changes is empty then nothing has changed
            return !m_paDvaMessagePrivateVersionChanges.empty();
        };
         ...
         * isNew
         *
         * This should be called to determine if this PaDvaMessagePrivateVersion is new. A new PaDvaMessagePrivateVersion is one that has not
         * yet been applied to the database and been given a pkey and create date.
         *
         * @return bool - This will return true if the PaDvaMessagePrivateVersion has not been applied to the database.
         */
        virtual bool isNew();
    ...
    private:
	    ...
	    /**
         * updatePaDvaMessagePrivateVersionChanges
         *
         * This updates the map recording the changes to the PaDvaMessagePrivateVersion. Whenever a call to set is made in this
         ...
         */
        virtual void updatePaDvaMessagePrivateVersionChanges(const std::string& name, const std::string& oldValue, const std::string& newValue);
         ...
         */
        virtual void updatePaDvaMessagePrivateVersionChanges(const std::string& name, const unsigned long oldValue, const unsigned long newValue);
		
        // Assignment PaDvaMessagePrivateVersion not used so it is made private
		ConfigPaDvaMessagePrivateVersion& operator=(const ConfigPaDvaMessagePrivateVersion&);


        unsigned long m_uniqueIdentifier; // We cannot use keys for identification of PaDvaMessagePrivateVersions in the
                                          // Configuration Editor because new PaDvaMessagePrivateVersions do not have keys.
        ...
        PaDvaMessagePrivateVersionHelper* m_paDvaMessagePrivateVersionHelper;


        TA_Base_Core::ItemChanges m_paDvaMessagePrivateVersionChanges; // This is a map of all changes made to the PaDvaMessagePrivateVersion so that they are
                                       // recorded and can be sent in audit or online update messages.

    };
} // closes TA_Base_Core

#endif // !defined ConfigPaDvaMessagePrivateVersion_H

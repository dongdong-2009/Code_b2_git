/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/interlocking_entity/src/InterlockVariableAccessFactory.cpp $
  * @author:  Robert Stagg
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    IInterlockVariable* InterlockVariableAccessFactory::getInterlockVariable(const std::string& key)
    {
        ...
        char sql[256];
		sprintf(sql,"select ENTITY_TYPE from LK_VARIABLE_TYPES where ENTITY_TYPE = '%s'",key.c_str());	
        ...
        if (0 == data->getNumRows()) // No entry found with the specified pkey
        {
			...
			sprintf(reasonMessage, "No data found for variable type = '%s'",key.c_str());			
        ...
        else if (1 < data->getNumRows()) // More than one entry found for the pkey
        {
            ...
            char reasonMessage[256];
			sprintf(reasonMessage, "More than one entry found for variable type = '%s'",key.c_str());			
        ...
        IInterlockVariable* theInterlockVariable;
        
        theInterlockVariable = new InterlockVariable(key);
    ...
    std::vector<IInterlockVariable*> InterlockVariableAccessFactory::getInterlockVariables()
    {
        ...
        char sql[256];
		sprintf(sql,"select ENTITY_TYPE from LK_VARIABLE_TYPES");	

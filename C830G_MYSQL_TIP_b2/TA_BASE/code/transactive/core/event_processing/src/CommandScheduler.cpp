/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/core/event_processing/src/CommandScheduler.cpp $
  * @author:  Bradley Cleaver
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2011/06/01 17:45:42 $
...
namespace TA_Base_Core
{
    ...
    void CommandScheduler::push( AbstractCommand* command )
    {
        ...
        LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Signalling scheduler thread, event queue size=%d", m_priorityQueue.size() );
        m_condition.signal();
    ...
    void CommandScheduler::expire()
    {
		...
		unsigned long  cmdsSize=cmdsToExecute.size();
        for ( cmdIt=cmdsToExecute.begin(); cmdIt!=cmdsToExecute.end(); cmdIt++ )
        {
			...
			if (cmdsSize%10==0)
			{
				LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "process event queue size=%lu.", cmdsSize );
			}
            cmdsSize--; //write log first, then --, reduce writing log for one item. 

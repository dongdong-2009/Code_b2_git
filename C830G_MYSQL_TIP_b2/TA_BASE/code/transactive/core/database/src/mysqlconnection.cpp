...
void MysqlConnection::exec( const SQLStatement& rSqlObj, IQueryAdapter*& pIQueryAdapter, bool isQuery /*= false*/, int prefetch /*= 10*/)
{
	...
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
			"MYSQL_ERROR: DBException errormsg=%s, errorcode=%d, ", e.getReason().c_str(), e.getNativeCode());
		throw e;
	}
	catch(sql::SQLException & e)
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
			"MYSQL_ERROR: exception: %s, errormsg=%s, errorcode=%d, ", e.getSQLStateCStr(), e.what(), e.getErrorCode());
		DBException db("Mysql_ERROR", e.what(), e.getErrorCode());
	...
	catch(...)
	{
		...
		strLogInfo = "Failed to exec SQLID:" + rSqlObj.strSQLID + ", SQL:";
		_LogSQLInfo(SourceInfo, DebugUtil::DebugError, strLogInfo, strSql);
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "MYSQL_ERROR: unknown database exception");
		DBException db("MysqlConnection_ERROR", "unknown database exception", 0);
		delete pQuery; pQuery = NULL;
		throw db;
	}	
...
void MysqlConnection::_LogSQLInfo(const char* pszFileName, int nLineNumber, TA_Base_Core::DebugUtil::EDebugLevel nLogLevel,

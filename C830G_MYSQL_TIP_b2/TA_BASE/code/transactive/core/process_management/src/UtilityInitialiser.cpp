...
namespace TA_Base_Core
{
    ...
    void UtilityInitialiser::initialiseCorba()
    {
        ...
        if (!RunParams::getInstance().isSet("NoNamingPort"))
        {
            if (RunParams::getInstance().isSet("CorbaNamingPort"))
            {
                port = TA_Base_Core::getRunParamValue("CorbaNamingPort", 0);
            }
            else
            {
                if (RunParams::getInstance().isSet(RPARAM_NSHOSTNAME) && RunParams::getInstance().isSet(RPARAM_ENTITYNAME) && !RunParams::getInstance().isSet(RPARAM_NOTIFYPORT))
                {

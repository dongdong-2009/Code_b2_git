/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/corba/src/ServantBase.cpp $
...
namespace TA_Base_Core
{
	...
	ServantBase::ServantBase()
		: m_deactivated( true ), m_explicitlyActivated( false ), m_nameRegistered( false )
	{
	}
	...
	void ServantBase::activateServant()
	{
		LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
		    "ServantBase::activateServant(): %s activating", gPtrToStr(this).c_str() );
		...
		{
			TA_ASSERT(true,
				"Unknown exception caught while activating servant. Make sure the orb is activated!");
		}
	}
	...
	{
		LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
		    "ServantBase::deactivateServant(): %s deactivating", gPtrToStr(this).c_str() );
		...
		for (unsigned int i = 0; i < m_POAs.size(); i++)
		{
			...
			catch ( ... )
			{
				...
				"Unknown exception", "caught while deactivating servant.");
			}
		}
		...
		try
		{
			...
			if ( !hasDeactivated() )
			{
				LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugWarn,
					"Servant deletion timeout. Object is not fully deactivated");
			}
		}
		catch ( const PortableServer::POA::ObjectNotActive& )
		{
		...
		{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::ExceptionCatch,
				"Unknown exception", "thrown while deactivating servant.");
		}
	...
	void ServantBase::addPOA(PortableServer::POA_ptr poa)
	{

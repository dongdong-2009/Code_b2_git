/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/4669/transactive/bus/pa/cachedmaps/src/PaZoneGroupCachedMap.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
...
namespace TA_IRS_Bus
{
    ...
    void PaZoneGroupCachedMap::removePaZoneGroup
    (
    ...
    )
    {
        ...
        if (0 != group.get())
        {
            TA_Base_Core::ConfigPaZoneGroup::AuditInfo auditData;
            auditData.entityKey = entityKey;
            auditData.sessionId = sessionId;

            group->deleteThisObject(auditData);     
    ...
    //throw(TA_Base_Core::DataException, TA_Base_Core::DatabaseException, TA_Base_Core::InvalidPasConfigurationException)
    {
        ...
        if (existingKey.hasBeenSet())
        {
            ...
            {
                TA_ASSERT(false, "Failed to retrieve specified group");
            }
        }
        else
        {
        ...
        if (0 == group.get())
        {
            TA_ASSERT(false, "No group to work with");
            return 0;
        }
    ...
    PaZoneGroupCachedMap::ZoneGroupFilter::ZoneGroupFilter(bool isOccZoneGroup, unsigned long depotLocationKey)
    :
    m_isOccZoneGroup(isOccZoneGroup),
    m_depotLocationKey(depotLocationKey)
    {
    }

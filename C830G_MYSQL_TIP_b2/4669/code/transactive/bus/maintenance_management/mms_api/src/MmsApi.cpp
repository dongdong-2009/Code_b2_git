...
namespace TA_IRS_Bus
{
    ...
    int MmsApi::initQ(char *qname)
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "InitQ status=%d", status);
	    ...
	    {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "failed to InitQ");
		    m_JournalOn = false;
            m_initQ = false;
            FUNCTION_EXIT;
		    return BEA_FALSE;
    ...
    int MmsApi::put_Q(char *q_name, char *target_q_name, char *msg, char *msgType, short msg_len)
    {
	    ...
	    int32 status;
	    char StrCount[4] = {0}; // for holding the message id
	    char StrDateTime[25] = {0};
	    char strMsg[MSGBUFSIZE] = {0}; // holding msg to be send
	    char strTempMsgType[5] = {0};

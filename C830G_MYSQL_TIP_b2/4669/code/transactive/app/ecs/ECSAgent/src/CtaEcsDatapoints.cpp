...
void CtaEcsDatapoints::setMonitor(bool isOnStartup)
{
    ...
    if (isOnStartup)
    {
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "-Sync-: Start to wait datapoint synchronization" );

        waitEcsDatapointSyncDone();

        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "-Sync-: Wait datapoint synchronization done" );
    }
...
void CtaEcsDatapoints::resetDataPoints()
{
	...
	else
	{
        ...
        setMasterModeLock(false,true);
        setMasterModeLockControl(false,true);
	}
...
void CtaEcsDatapoints::setIntegerDataPoint(TA_Base_Bus::DataPoint* dp, long value, const std::string& descriptionForDebug, bool fInit)
{
    ...
    case TA_Base_Bus::DPT_ENUM:
        if ( dp->getCurrentValue().getEnum() != value || fInit == true )
        {
			...
			if( 0 >= value )
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "The datapoint's value should > 0, but it is actual value is %li" , value );
				FUNCTION_EXIT;
				return;
			}
			//++dahe
...
/*unsigned*/ long CtaEcsDatapoints::getCurrentStationMode()	//dahe++ for CL-21046
{
	FUNCTION_ENTRY("getCurrentStationMode");

    /*unsigned*/ long state(TA_IRS_Bus::ECS_NONE_OR_INVALID);	//dahe++ for CL-21046

    try
    {
        state = (/*unsigned*/ long)m_theConfig.getModeStatusProxy()->getValueAsDouble();	//dahe++ for CL-21046
    }
    catch(TA_Base_Core::ScadaProxyException&)
    {
        LOG_GENERIC( SourceInfo,  TA_Base_Core::DebugUtil::DebugError, 
...
/*unsigned*/ long CtaEcsDatapoints::getCurrentMasterMode()	//dahe++ for CL-21046
{
	...
	FUNCTION_EXIT;
    return (/*unsigned*/ long)m_theConfig.getMyModeStatusDataPoint()->getCurrentValue().getFloat();	//dahe++ for CL-21046
}

void CtaEcsDatapoints::setCurrentMasterMode(/*unsigned*/ long value, bool fInit)	//dahe++ for CL-21046
{
...
void CtaEcsDatapoints::waitEcsDatapointSyncDone()
{
    for (std::vector<unsigned long>::iterator it=m_allZoneIds.begin(); it!=m_allZoneIds.end(); ++it)
    {
        if (m_theConfig.getIsMaster())
        {
            // Unknown ecs zones will be ignored
            waitDpQualityGood(m_theMaps.getMyZoneCongestionDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneEmergencyDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneEmergencyAppliedDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneEvacuationSelectedDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneAirflowSelectedDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneEvacuationAppliedDataPoint(*it));
            waitDpQualityGood(m_theMaps.getMyZoneAirflowAppliedDataPoint(*it));
        }
        else
        {
            if(m_theMaps.isZoneIdInScope(*it) == true)
    ...
    {
        // mastermode = mode status
        waitDpQualityGood(m_theConfig.getMyModeStatusDataPoint());
        waitDpQualityGood(m_theConfig.getMyModeInProgressDataPoint());
    }
    else
    {
        waitDpQualityGood(m_theConfig.getMyLastAppliedMasterModeDataPoint());
        waitDpQualityGood(m_theConfig.getMyLastAppliedStationModeDataPoint());
        waitDpQualityGood(m_theConfig.getMySelectedMasterModeDataPoint());
        waitDpQualityGood(m_theConfig.getMySelectedOverridingStationModeDataPoint());
        waitDpQualityGood(m_theConfig.getMyMasterModeLockDataPoint());
        waitDpQualityGood(m_theConfig.getMyMasterModeLockControlDataPoint());
	}
}

void CtaEcsDatapoints::waitDpQualityGood( TA_Base_Bus::DataPoint* pDatapoint )
{
    if ( NULL == pDatapoint )
    {
        return;
    ...
    // Currently, no SCADA library support. No notification of DataPoint 
    // synchronization done. We need to wait the DataPoint quality good by ourself.
    ...
    // QUALITY_GOOD_NO_SPECIFIC_REASON 192 and QUALITY_GOOD_LOCAL_OVERRIDE 216
    while (pDatapoint->getQualityStatus() < TA_Base_Bus::QUALITY_GOOD_NO_SPECIFIC_REASON)
    {
        if ( nRetryTimes > nMaxTimes )
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "-Sync-: Failed to wait %s update", pDatapoint->getDataPointName().c_str() );
            break;
        }

        TA_Base_Core::Thread::sleep( nSleepTime );

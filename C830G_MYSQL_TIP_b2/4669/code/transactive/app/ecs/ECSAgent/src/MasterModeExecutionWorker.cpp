/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ECSAgent/src/MasterModeExecutionWorker.cpp $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
    ...
    while (m_isRunning)
    {
        if (m_isEnabled)
        {
            try
            {
                ...
                default:
                    TA_ASSERT(false, "Invalid EEcsExecutionState enum used");
                }
...
void MasterModeExecutionWorker::abortIfRequired()
{
         ...
         modeState.executionState == TA_IRS_Bus::ECS_EXECUTION_COMPLETED)
    {
			 ...
			 ( modeState.toMasterMode <= TA_IRS_Bus::ECS_MAX_EMERGENCY_MASTER_MODE ) )
		{
			...
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "TD17328&TD17443 effase the last emergency one" );
    ...
    (TA_IRS_Bus::IMasterEcsAgentCorbaDef::EcsMasterModeExecutionUpdateData& modeState)
{
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "enter ECS_EXECUTION_READY state");
    ...
    (TA_IRS_Bus::IMasterEcsAgentCorbaDef::EcsMasterModeExecutionUpdateData& modeState)
{
	...
	try
    {
		...
		if (true == m_theDatapoints.getEmergency(m_theMaps.getZoneIdFromMasterMode(modeState.toMasterMode)))
		{
			m_locksManager.removeMasterModeLocks();
		}
    }
    ...
    (TA_IRS_Bus::IMasterEcsAgentCorbaDef::EcsMasterModeExecutionUpdateData& modeState)
{
    ...
    else
    {
		...
		try
		{
			...
			if ( true == m_theDatapoints.getEmergency(ecsZoneId) )
			{
...
void MasterModeExecutionWorker::processItaEcsCachedMapsUpdate(ItaEcsCachedMaps::EItaEcsCachedMapsItemKey key)
{
    ...
    for (std::vector<unsigned long>::iterator locationIt=locations.begin(); locationIt!=locations.end(); ++locationIt)
    {
		...
		catch(TA_Base_Core::CachedMappingNotFoundException& cmnfe)
		{
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
				"Cannot locate %s Station ECS Agent CORBA object - Master ECS Agent access to this station will not be possible",
				m_theMaps.getLocationName(*locationIt).c_str());

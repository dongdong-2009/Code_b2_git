...
* in any form.
*
* Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ecs_manager/src/ECSManagerGUI.cpp $
* @author:  Robert Young
* @version: $Revision: #5 $
*
* Last modification: $DateTime: 2010/01/26 11:36:50 $
...
namespace TA_IRS_App
{
    ...
    void ECSManagerGUI::createApplicationObject()
    {
        ...
        case STATION_MODES:
			{// --dialog-type set for Station Modes Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSStationModeDlg.log");
				//++zn
        ...
        case STATION_MODES_OVERVIEW:
			{	// --dialog-type set for for Proposed Station Modes Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSStationModeOverviewDlg.log");
				//++zn
        ...
        case NORMAL_MASTER_MODES:
			{	// --dialog-type set for Normal Master Modes Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSNormalMasterModeDlg.log");
				//++zn
        ...
        case EMERGENCY_MASTER_MODES:
			{	// --dialog-type set for Emergency Master Modes Dialog 
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSEmergencyMasterModeDlg.log");
				//++zn
				ECSMasterModesDlg *pDlg = new ECSMasterModesDlg( *this );
                setApplicationWnd( *pDlg );
			}
        ...
        case EMERGENCY_MASTER_MODES_FOR_ZONE:
			{// --dialog-type set for Emergency Master Modes For Zone Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSEmergencyMasterModeDlgForZone.log");
				//++zn
				ECSMasterModesDlg *pDlg = new ECSMasterModesDlg( *this );
                setApplicationWnd( *pDlg );
			}
        ...
        case CONGESTION_OPERATION_SETTINGS:
			{// --dialog-type set for Emergency Master Modes For Zone Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSCongestionParamSettingDlg.log");
				//++zn
        ...
        case TIMETABLE_CHANGE:
			{// --dialog-type set for Emergency Master Modes For Zone Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSTimetableSelectionDlg.log");
				//++zn
        ...
        case OVERALL_STATION_TIMETABLE_SETTING_MODE:
			{//// 10745g-Start		 --dialog-type set for Overall ECS Timetable Setting Dialog
				//zn++ - seperate diff ecs log file
				//TA_Base_Core::DebugUtil::getInstance().setFile("C:\\transactive\\logs\\Log_ECSOverallTimetableSelectionDlg.log");
				//++zn
        ...
        default:
			...
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "ECSManagerConfigurationAccess::getInstance().getDialogType() returned unexpected enumerated type value" );
			TA_THROW( TA_Base_Core::GenericGUIException( "Unexpected enumerated type value", TA_Base_Core::GenericGUIException::COMMAND_LINE_INVALID ) );
        }
		FUNCTION_EXIT;
    }
	...
	void ECSManagerGUI::onInitGenericGUICompleted()
	{
		...
		if (NULL == pView)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "the current view is NULL!" );
			return;
		}
		pView->init();
	};

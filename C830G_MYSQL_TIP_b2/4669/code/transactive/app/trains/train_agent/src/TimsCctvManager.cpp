...
namespace TA_IRS_App
{
    ...
    class IWorkItemDecorator : public TA_Base_Core::IWorkItem
    {
    public:
        
        IWorkItemDecorator( TA_Base_Core::IWorkItemPtr item )
            : m_item( item )
        {
            FUNCTION_ENTRY( "IWorkItemDecorator" );
            FUNCTION_EXIT;
        }
        
        virtual ~IWorkItemDecorator() {};
        virtual void executeWorkItem() = 0;
        
    protected:
        
        TA_Base_Core::IWorkItemPtr m_item;
    };
    
    
    template< typename condition_type >
        class ConditonedWorkItemDecorator : public IWorkItemDecorator
    {
    public:
        
        ConditonedWorkItemDecorator( TA_Base_Core::IWorkItemPtr item, const condition_type& condition )
            : IWorkItemDecorator( item ),
              m_condition( condition )
        {
            FUNCTION_ENTRY( "ConditonedWorkItemDecorator" );
            FUNCTION_EXIT;
        }
    ...
    class WaitForCommunicationCondition
    {
    public:
        
        WaitForCommunicationCondition( ITrainStatusManager& trainStatusManager, TA_IRS_Bus::CommonTypes::TrainIdType trainId, ITrainAgentConfiguration& configuration )
            : m_statusManager( trainStatusManager ),
              m_trainId( trainId ),
        ...
        bool operator()() const
        {
            ...
            bool canBeScheduled = false;
            
            //TrainCommsStatePtr commsState = m_statusManager.getTrainCommsState( m_trainId );
            
            for ( int i = 0; ( i < 30 ) && ( false == canBeScheduled ); ++i, TA_Base_Core::Thread::sleep( 1000 ) )
            {
                {
                    // get the comms state
                    ...
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                        "LIMINTEST CL-20746 CL-20639 [try=%d][train id=%d][communication valid=%d][communicating location=%d][location=%d][canBeScheduled=%d]",
                        i + 1, m_trainId, commsState->isTrainCommunicationValid(), commsState->getCommunicatingLocation(), m_configuration.getLocationKey(), canBeScheduled );
                    
                    // release the train state lock
                }
            }
            
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "LIMINTEST CL-20746 CL-20639 [canBeScheduled=%s]", canBeScheduled ? "true" : "false" );
    ...
    typedef ConditonedWorkItemDecorator< WaitForCommunicationCondition > WaitForCommunicationCarrierOffTask;
    typedef boost::shared_ptr< WaitForCommunicationCarrierOffTask > WaitForCommunicationCarrierOffTaskPtr;
                                            ...
                                            const AtsTrainInformation& newInformation )
    {
        ...
        // Check the agent is in control mode using AgentManager, if not return
LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"CL-20639: Processing AtsUpdate..");
        if ( false == OperationModeManager::isRunningControl() )
        {
            FUNCTION_EXIT;
            return;
        ...
        TrainCctvStatePtr cctvState = m_statusManager.getTrainCctvState( trainId );
        {
            ...
            if ( false == cctvState->isCctvActivated() )
            {
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"CCTV is InActive");
                //FUNCTION_EXIT;
                //return;
            }
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"CL-20639: WasInControl: %d isInControl: %d",wasInControlArea,isInControlArea);
...
LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"CL-20639: OLD_LOC: %d NEW_LOC: %d",oldInformation.localisation.currentLocation,newInformation.localisation.currentLocation);
                                                  ...
                                                  bool waitForCommunication )
    {
        ...
        {
            task->setScheduled();
            WaitForCommunicationCarrierOffTaskPtr waitTask( new WaitForCommunicationCarrierOffTask( task, WaitForCommunicationCondition( m_statusManager, trainId, m_configuration ) ) );
            m_threadPool.queueWorkItem( waitTask );

            // bool canBeScheduled = true;
            // 
            // if ( true == waitForCommunication )
            // {
            //     //limin++, CL-20746 wait for 30 seconds
            //     for ( int i = 0; ( i < 30 ) && ( false == canBeScheduled ); ++i, TA_Base_Core::Thread::sleep( 1000 ) )
            //     {
            //         {
            //             // get the comms state
            ...
            //                              ( commsState->getCommunicatingLocation() == m_configuration.getLocationKey() );
            // 
            //             LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "LIMINTEST CL-20746 CL-20639 [try=%d][communication valid=%d][communicating location=%d][location=%d][canBeScheduled=%d]",
            //                          i + 1, commsState->isTrainCommunicationValid(), commsState->getCommunicatingLocation(), m_configuration.getLocationKey(), canBeScheduled );
            //             // release the train state lock
            //         }
            //     }
            //     //++limin, CL-20746
            ...
            // 
            //     LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "LIMINTEST CL-20746 CL-20639 a task scheduled for (2)CarrierOff Command" ); //limin++, CL-20746
            // }
        }

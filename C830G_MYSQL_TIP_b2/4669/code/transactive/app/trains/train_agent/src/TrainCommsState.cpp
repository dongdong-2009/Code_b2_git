...
namespace TA_IRS_App
{
    ...
    TrainCommsState::TrainCommsState( TA_IRS_Bus::CommonTypes::TrainIdType trainId,
            ...
            : TA_IRS_Bus::CoreTrainState( trainId, primaryTsi ),
              ...
              m_serviceNumber( "" ),
              m_isMute( false ),
			  m_nextTtisLibraryVersion( nextTtisLibraryVersion ),
		      m_nextTtisScheduleVersion ( nextTtisScheduleVersion ) ,
	...
	unsigned short TrainCommsState::getNextTtisLibraryVersion() const
    {
	...
	unsigned short TrainCommsState::getNextTtisScheduleVersion() const
    {
        FUNCTION_ENTRY( "getNextTtisScheduleVersion" );
        FUNCTION_EXIT;
    ...
    bool TrainCommsState::isTrainCommunicationValid() const
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "LIMINTEST CL-20746: [isTrainCommunicationValid=%d]", isAtcValid() || isOa1Valid() );
   ...
   bool TrainCommsState::isAtsValid() const
	...
	void TrainCommsState::setNextTtisLibraryVersion( unsigned short version,
		time_t timestamp )
    {
        FUNCTION_ENTRY( "setTtisLibraryVersion" );
		
        if ( version != m_nextTtisLibraryVersion )
        {
            m_nextTtisLibraryVersion = version;
            m_persistedStateChanged = true;
            setDataChangedWithTimestamp( timestamp );
			//	m_persistedStateChanged = true ;
        }
	...
	void TrainCommsState::setNextTtisScheduleVersion( unsigned short version,
		time_t timestamp )
    {
        FUNCTION_ENTRY( "setTtisScheduleVersion" );
		
        if ( version != m_nextTtisScheduleVersion )
        {
            m_nextTtisScheduleVersion = version;
            m_persistedStateChanged = true;
            setDataChangedWithTimestamp( timestamp );
        }
	...
	//	setNextTtisLibraryVersion( nextTtisLibraryVersion ) ;
//		setNextTtisScheduleVersion( nextTtisScheduleVersion) ;
                                          ...
                                          unsigned long location )
    {
        ...
        if ( validity != m_atcValid )
        {
            ...
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "LIMINTEST CL-20746: [m_atcValid=%d]", m_atcValid );
        }
                                          ...
                                          unsigned long location )
    {
        ...
        if ( validity != m_oa1Valid )
        {
            ...
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "LIMINTEST CL-20746: [m_oa1Valid=%d]", m_oa1Valid );
        }
...
/*             ( serviceNumber != m_serviceNumber ) ||*/ //wuzhongyi CL-20826
             ...
             ( localisationValid != m_atsLocalisationValid ) )
        {
			...
			// modified by Bohong, for CL-20993
			if (m_atsTrack != track)
			{
				m_atsTrack = track;
				setDataChanged();
    ...
    TA_IRS_Bus::StateUpdateTypes::TrainCommsSyncState TrainCommsState::getSyncState() const
    {
		...
		syncState.nextTtisLibraryVersion = m_nextTtisLibraryVersion;
		syncState.nextTtisScheduleVersion = m_nextTtisScheduleVersion;
    ...
    void TrainCommsState::setSyncState( const TA_IRS_Bus::StateUpdateTypes::TrainCommsSyncState& syncState )
    {
		...
		m_nextTtisLibraryVersion = syncState.nextTtisLibraryVersion;
		m_nextTtisScheduleVersion = syncState.nextTtisScheduleVersion;

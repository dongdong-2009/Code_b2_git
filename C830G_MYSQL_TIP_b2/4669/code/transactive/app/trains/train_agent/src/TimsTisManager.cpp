       ...
       /*
              ...
              stateMap.end() != stateIter; ++stateIter )
        {
...

            {
				...
				trainInfo.nextlibraryVersion = commsState->getNextTtisLibraryVersion() ;
				trainInfo.nextscheduleVersion = commsState->getNextTtisScheduleVersion() ;
   ...
   */
		...
		persistedTrainStates.end() != stateIter; ++stateIter )
        {
            try
            {
				...
				TA_IRS_Core::IPersistedTrainStatePtr commsState =stateIter->second ;  //boost::dynamic_pointer_cast< TrainCommsState >( getTrainState( trainId, TA_IRS_Bus::CommonTypes::CoreResource ) );
			...
			//	TA_IRS_Bus::TrainStateLock stateLock( commsState );
				...
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Set nextTtisLibraryVersion \
					of Train Id %d to %d", trainInfo.trainId, trainInfo.nextlibraryVersion);
            ...
            //    time_t timestamp = stateIter->second->getTimestamp();
				...
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
					"Loaded persisted train states from the database. Train id is %d nextLibVersion is %d nextSchLibVersion is %d ",
                     trainInfo.trainId , trainInfo.nextlibraryVersion , trainInfo.nextscheduleVersion );
                 result.push_back( trainInfo );
            }
    ...
    TA_IRS_Bus::TtisTypes::TisLibraryVersionUpdate TimsTisManager::getLibraryVersionsForTrain( TA_IRS_Bus::CommonTypes::TrainIdType trainId )
    {
...

        {
			...
			result.nextlibraryVersion = commsState->getNextTtisLibraryVersion() ;
			result.nextscheduleVersion = commsState->getNextTtisScheduleVersion() ;
			
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Set nextTtisLibraryVersion \
				of Train Id %d to %d", result.trainId, result.nextlibraryVersion);
            ...
            if ( commandReceived )
            {
                ...
                switch ( result )
                {
                    ...
                    case TA_IRS_Bus::ProtocolTtisTypes::TtisLibraryUpgraded:
                    {
                        ...
                        switch ( upgradeType )
                        {
                            case TA_IRS_Bus::TtisTypes::TtisLibrary:
                                libraryUpdate.version = commandReceived->getLibraryVersion();
                                break;

                            case TA_IRS_Bus::TtisTypes::TtisSchedule:
                                libraryUpdate.version = commandReceived->getScheduleLibraryVersion();
                                break;

                            case TA_IRS_Bus::TtisTypes::TtisNone: // fall through
                            default:
                                // DO NOTHING
                                break;
                        }
                        ...
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "LIMINTEST TTIS UPGRADE: %s", libraryUpdate.getLogString().c_str() );
                                                  ...
                                                  time_t timestamp )
    {
...

        {
			...
			commsState->setNextTtisLibraryVersion( commandReceived->getLibraryVersion(), timestamp ) ;
			...
			libraryUpdate.nextlibraryVersion = commsState->getNextTtisLibraryVersion() ;
            libraryUpdate.nextscheduleVersion = commsState->getNextTtisScheduleVersion() ;
			
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Set nextTtisLibraryVersion \
				of Train Id %d to %d", libraryUpdate.trainId, libraryUpdate.nextlibraryVersion);
                                                  ...
                                                  time_t timestamp )
    {
...

        {
            ...
            commsState->setTtisLibraryVersion( commandReceived->getLibraryVersion(), timestamp );
            commsState->setTtisScheduleVersion( commandReceived->getScheduleLibraryVersion(), timestamp );
            commsState->setNextTtisScheduleVersion( commandReceived->getScheduleLibraryVersion(), timestamp ) ;
            libraryUpdate.libraryVersion = commsState->getTtisLibraryVersion();
            libraryUpdate.scheduleVersion = commsState->getTtisScheduleVersion();
			libraryUpdate.nextlibraryVersion = commsState->getNextTtisLibraryVersion() ;
            libraryUpdate.nextscheduleVersion = commsState->getNextTtisScheduleVersion() ;
        ...
        if ( TA_IRS_Bus::TtisTypes::TtisNone != currentDownloadType )
        {
            ...
            if ( TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadRepetition != downloadResult )
            {
                ...
                if ( TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadOk == downloadResult )
                {
					...
					if ( TA_IRS_Bus::TtisTypes::TtisLibrary == currentDownloadType  )
					{
                        commsState->setNextTtisLibraryVersion( currentDownloadVersion ,timstamp ) ;
					}
					else if ( TA_IRS_Bus::TtisTypes::TtisSchedule == currentDownloadType )
					{
                        commsState->setNextTtisScheduleVersion( currentDownloadVersion , timstamp ) ;
					}
                }

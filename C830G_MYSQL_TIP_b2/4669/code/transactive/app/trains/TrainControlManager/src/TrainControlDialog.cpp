...
namespace TA_IRS_App
{
    ...
    TrainControlDialog::TrainControlDialog( TA_Base_Bus::IGUIAccess& genericGUICallback,
                                            ...
                                            CWnd* pParent )
        : TransActiveDialog( genericGUICallback, TrainControlDialog::IDD, pParent ),
          m_rights( taRights ),
    ...
    void TrainControlDialog::DoDataExchange( CDataExchange* pDX )
    {
        ...
        DDX_Control( pDX, IDC_TrainId, m_trainList );
        DDX_Control( pDX, IDC_EDIT2, m_secondaryTSI );
        DDX_Control( pDX, IDC_EDIT1, m_primaryTSI );
        DDX_Control( pDX, IDC_EDIT3, m_resultDisplay );
        DDX_Control( pDX, IDC_TakeControl, m_takeControl );
        DDX_Control( pDX, IDC_Close, m_closeButton );
        //}}AFX_DATA_MAP
    }


    BEGIN_MESSAGE_MAP( TrainControlDialog, TransActiveDialog )
        //{{AFX_MSG_MAP(TrainControlDialog)
        ON_BN_CLICKED( IDC_Close, OnClose )
        ON_WM_DESTROY()
        ON_BN_CLICKED( IDC_TakeControl, OnTakeControl )
        ON_CBN_SELENDOK( IDC_TrainId, OnSelectTrain )
    ...
    LRESULT TrainControlDialog::OnUpdateTrain( WPARAM wParam, LPARAM lParam )
    {
...

        {
            ...
            if ( CB_ERR != updatedPosition )
            {
                ...
                m_trainList.SetItemData( updatedPosition, static_cast<DWORD>( findIter->second.trainId ) );
            }

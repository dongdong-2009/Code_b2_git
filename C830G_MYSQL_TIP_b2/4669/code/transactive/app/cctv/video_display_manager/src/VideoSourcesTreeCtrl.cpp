...
namespace TA_IRS_App
{
    ...
    void VideoSourcesTreeCtrl::insertVideoSources( std::vector<VideoInput*> sources )
    {
        ...
        if (sources.size() == 0)
        {
            ...
            HTREEITEM newItem = InsertItem("NONE CONFIGURED", 0, 0, TVI_ROOT, TVI_LAST);
        }
    ...
    void VideoSourcesTreeCtrl::insertVideoSource( VideoInput* source )
    {
        ...
        if ( (parts.size() > 0) && (parts[0] != "") )
        {
            ...
            iter != parts.end(); iter++)
            {
                ...
                {
                    // insert it
                    HTREEITEM newItem = InsertItem(iter->c_str(), 0, 0, parentGroupItem, TVI_LAST);
    ...
    void VideoSourcesTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point)
    {
            ...
            ( (pFlags == TVHT_ONITEMICON) || (pFlags == TVHT_ONITEMLABEL) ) )
        {
            if ( ( newSelection != GetSelectedItem() ) || (prevFoc != this) )
            {
                ...
                // HuangQi modify for CL-21275 start on 2010-03-05
                SCROLLINFO stInfo = { sizeof( SCROLLINFO ), 0u, 0, 0, 0u, 0, 0 };
                GetScrollInfo( SB_HORZ, &stInfo );
                // Judge whether there is a horizontal scroll bar
                if ( 0 < stInfo.nPage )
                {
                    // Show hide the scroll to avoid auto scroll
                    CWnd::ShowScrollBar( SB_HORZ, FALSE );
                    SelectItem( newSelection );
                    CWnd::ShowScrollBar( SB_HORZ, TRUE );
                }
                else
                {
                    SelectItem( newSelection );
                }
                // HuangQi modify for CL-21275 end on 2010-03-05
            }
        }
    ...
    CImageList* VideoSourcesTreeCtrl::CreateDragImageEx(HTREEITEM hItem)
    {
        ...
        GetItemImage(hItem, itemImageIndex, // CL-21275
    ...
    void VideoSourcesTreeCtrl::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult)
    {
        ...
        NM_TREEVIEW* pNMTreeView = (NM_TREEVIEW*)pNMHDR;
        HTREEITEM hDragItem = pNMTreeView->itemNew.hItem; // CL-21275
        ...
        if ( FALSE == m_isSequenceEditorTree )
        {
            ...
            if ( FALSE == CTreeCtrl::ItemHasChildren(hDragItem) ) // CL-21275
            {
                CRect itemRect;
                // unfortunately CreateDragImage does not work properly (no text)
                // for the font Arial Bold so had to write our own method to do
                // same thing!
                GetItemRect(hDragItem,&itemRect,TRUE); // CL-21275
                CImageList* dragImage = CreateDragImageEx(hDragItem); // CL-21275
                CPoint adjust;
    ...
    void VideoSourcesTreeCtrl::PreSubclassWindow()
    {
        ...
        // HuangQi add for CL-21275 start on 2010-03-05
        // Adjust window height to avoid auto scroll
        CRect rClient;
        GetClientRect( &rClient );
        SetWindowPos( NULL, 0, 0, rClient.Width(), getSuitItemHeight( rClient.Height() ), SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE );
        // HuangQi add for CL-21275 end on 2010-03-05
        ...
        if ( (nItem==NULL) || (nItem!=m_nCurItem) )
        {
           ...
           if (nItem!=NULL && !IsRectCompletelyVisible(rcText))
           {
               ...
               // move tip window over tree text
                // HuangQi modify for CL-21276 start on 2010-03-03
                if ( 88 <= sText.GetLength() )
                {
                    m_wndTip.SetWindowPos(NULL, rc.left-1, rc.top - 1, rc.Width()-60,
                        rc.Height()+24, SWP_NOZORDER|SWP_NOACTIVATE);    //TD17254    In order to wrap up the text in one screen
                }
                else
                {
                    m_wndTip.SetWindowPos(NULL, rc.left-1, rc.top - 1, rc.Width()+6,
                        rc.Height()+4, SWP_NOZORDER|SWP_NOACTIVATE);
                }
                // HuangQi modify for CL-21276 end on 2010-03-03
               if ( FALSE == tipVisible )
               {
    ...
    // HuangQi add for CL-21275 start on 2010-03-05
    // Adjust the height to avoid last item is not showed completely (ignore horizontal scroll bar)
    // since it could cause auto vertical scroll
    int VideoSourcesTreeCtrl::getSuitItemHeight( int nHeight )
    {
        short sItemHeight = GetItemHeight();
        // Reducing the height is better than rounding the height
        nHeight -= ( nHeight % static_cast<int>(sItemHeight) );
        nHeight += 4; // 3d Frame height
        return nHeight;
    }
    // HuangQi add for CL-21275 end on 2010-03-05

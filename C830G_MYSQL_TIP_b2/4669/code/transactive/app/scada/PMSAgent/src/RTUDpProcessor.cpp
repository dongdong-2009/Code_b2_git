/**
  ...
  * any form.
  *
  * Source:		$File: //depot/4669_Review_Branch/4669/transactive/app/scada/PMSAgent/src/RTUDpProcessor.cpp $
  * @author:	Raymond Pau
  * @version:	$Revision: #5 $
  *
...
namespace TA_IRS_App
{
	...
	void RTUDpProcessor::run()
	{
		...
		while ( m_threadTerminated == false )
		{
			...
			while (false == m_threadTerminated && false == queueEmpty)
            {
                ...
                LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Station system %s: start updating datapoints. now queue block size is:%d.", m_stationSystemName.c_str(), qSize);
    ...
    int RTUDpProcessor::updateDataPoints( const ModbusDataBlock & dataBlock, const ModbusDataBlock & prevDataBlock )
    {
        ...
        if ( dataBlock.quality != prevDataBlock.quality)
        {
            ...
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Station system %s: finish updating all %d datapoints.", m_stationSystemName.c_str(), updateCount);
			return updateCount;
        }
        ...
        if (updateCount > 0)
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Station system %s: finish updating %d datapoints.", m_stationSystemName.c_str(), updateCount);
        }

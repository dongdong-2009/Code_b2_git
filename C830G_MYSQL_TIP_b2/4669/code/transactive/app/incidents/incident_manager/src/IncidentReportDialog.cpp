/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/incidents/incident_manager/src/IncidentReportDialog.cpp $
  * @author:  Song Toan
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2009/04/23 12:21:20 $
...
namespace TA_IRS_App
{
	...
	BOOL IncidentReportDialog::OnInitDialog() 
	{
		...
		m_timeStartDate = COleDateTime();
		m_timeEndDate = COleDateTime();    
		m_timeEndTime = COleDateTime();
		m_timeStartTime = COleDateTime();
		...
		int nCount = arrayLocations.size();
		for( int i = 0; i < nCount; ++i )
		{
			pcomboboxLocation1->AddString( arrayLocations[i]->c_str() );
			pcomboboxLocation2->AddString( arrayLocations[i]->c_str() );
		}
	...
	void IncidentReportDialog::refreshDisplay()
	{
		...
		m_sLocation1 = m_report->getLocation1().c_str();
		m_sLocation2 = m_report->getLocation2().c_str();
	...
	void IncidentReportDialog::onSave()
	{
		...
		if (( m_delayhour > 999 ) || ( m_delaymins > 59 ) || ( m_delaysecs > 59 ))//TD-15805
		{
			...
			//if (m_uInitialDelayHours > 999)//TD-15805
			if ( atoi(m_uInitialDelayHours.GetBuffer(0)) > 999)
			{
		...
		if (( m_accumhour > 999 ) || ( m_accummins > 59 ) || ( m_accumsecs > 59 ))//TD-15805
		{
			...
			//if (m_uAccumulatedDelayHours > 999)//TD-15805
			if ( atoi(m_uAccumulatedDelayHours.GetBuffer(0)) > 999)
			{
				GetDlgItem(IDC_EDIT_ACCUMULATED_DELAY_TIME_HOURS)->PostMessage(EM_SETSEL, 0, -1);
				GetDlgItem(IDC_EDIT_ACCUMULATED_DELAY_TIME_HOURS)->SetFocus();
			}
		//	else if (m_uAccumulatedDelayHours > 59)
			else if ( atoi(m_uAccumulatedDelayMinutes.GetBuffer(0)) > 59)
			{
		...
		m_report->setLocation1( (LPCTSTR) m_sLocation1 );
		m_report->setLocation2( (LPCTSTR) m_sLocation2 );
	...
	bool IncidentReportDialog::isModified()
	{
		...
		if ( m_sLocation1 != m_report->getLocation1().c_str())
		{
			FUNCTION_EXIT;  //TD13540 - specify the function's exit point in log
			return true;
		}
		...
		if ( m_sLocation2 != m_report->getLocation2().c_str() )
 		{
			FUNCTION_EXIT;  //TD13540 - specify the function's exit point in log
			return true;
		}
	...
	void IncidentReportDialog::SetTimeRange(bool bIsEndTime,bool bIsDateChange)
	{
		...
		if (m_bIsReportNew)   //new report
		{
			...
			if (bIsEndTime)    //End time changed
			{
				...
				else if (tmEndDateTime < tmStartDateTime)
				{
			...
			{				
				...
				else if (tmStartDateTime > tmEndDateTime)
				{

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/pa_station_dva_message/src/PAStationDvaMessageDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #2 $
  *
...
namespace TA_IRS_App
{
    ...
    void PaStationDvaMessageDatabaseAccessor::loadItems()
    {
        ...
        IPaTisMessageMaps items = PaDvaMessageAccessFactory::getInstance().getAllPaDvaMessagesForConfigEditor( true ); //getAllPaDvaMessages(true);
    ...
    PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord PaStationDvaMessageDatabaseAccessor::getRecordFrom( TA_Base_Core::IConfigPaTisMessageMap& item )
    {
        PaStationDvaMessageRecord record;
        ...
        if ( TA_Base_Core::DatabaseKey::isInvalidKey( item.getTisMessageTag() ) )
        {
            record.tisMessageTag = "";
        }
        else
        {
            record.tisMessageTag.Format( "%d", item.getTisMessageTag() );
        }

        if ( TA_Base_Core::DatabaseKey::isInvalidKey( item.getTisLibrarySection() ) )
        {
            record.tisLibrarySection = "";
        }
        else
        {
            record.tisLibrarySection.Format( "%d", item.getTisLibrarySection() );
        }

        if ( TA_Base_Core::DatabaseKey::isInvalidKey( item.getTisLibraryVersion() ) )
        {
            record.tisLibraryVersion = "";
        }
        else
        {
            record.tisLibraryVersion.Format( "%d", item.getTisLibraryVersion() );
        }

        if ( TA_Base_Core::DatabaseKey::isValidKey( item.getTisMessageTag() ) &&
                TA_Base_Core::DatabaseKey::isValidKey( item.getTisLibrarySection() ) )
        {
            try
            {
                TA_Base_Core::PaDvaMessageAccessFactory::TisMessage tm = getTisMessage( item );
    ...
    CString PaStationDvaMessageDatabaseAccessor::getTypeString( const std::string type )
    {
        CString result;
        if ( 1 == type.length() )
        {
...
//        FUNCTION_EXIT;
    ...
    void PaStationDvaMessageDatabaseAccessor::refreshCachedMaps()
    {
        ...
        try
        {
            std::vector<TA_Base_Core::PaDvaMessageAccessFactory::TisMessage> messages =
                PaDvaMessageAccessFactory::getInstance().enumerateTisMessages();
    ...
    */
    ...
    TA_Base_Core::PaDvaMessageAccessFactory::TisMessage PaStationDvaMessageDatabaseAccessor::getTisMessage( unsigned long key ) const
    {
    ...
    TA_Base_Core::PaDvaMessageAccessFactory::TisMessage
    PaStationDvaMessageDatabaseAccessor::getTisMessage( IConfigPaTisMessageMap& configItem )
    {
		...
		unsigned long searchVersion = configItem.getTisLibraryVersion();

        for ( TisMessages::iterator itr = m_tisMessages.begin(); itr != m_tisMessages.end(); itr ++ )
        {
            if ( itr->second.tisMessageTag == searchTag && itr->second.tisLibrarySection == searchSection && itr->second.tisLibraryVersion == searchVersion)
            {
                return itr->second;
            }
        }

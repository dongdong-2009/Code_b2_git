/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/pa_zone_group/src/PAZoneGroupListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the PaZoneGroupDetailView as this will show the details of the item selected in this view.
  */
...
namespace TA_IRS_App
{
    ...
    const CString PaZoneGroupListView::NEW = "New";
    const CString PaZoneGroupListView::MODIFIED = "Modified";
    const CString PaZoneGroupListView::UPTODATE = "";

    const bool PaZoneGroupListView::ASCENDING = true;
    const bool PaZoneGroupListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(PaZoneGroupListView, CListView)


    PaZoneGroupListView::PaZoneGroupListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PaZoneGroupListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(PaZoneGroupComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_ZONE_GROUPS) );
    }
...
BEGIN_MESSAGE_MAP(PaZoneGroupListView, AbstractListView)
	//{{AFX_MSG_MAP(PaZoneGroupListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int PaZoneGroupListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void PaZoneGroupListView::OnInitialUpdate() 
    {
    ...
    void PaZoneGroupListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void PaZoneGroupListView::loadAllItems()
    {
        ...
        PaZoneGroupDatabaseAccessor* dbAccessor = dynamic_cast<PaZoneGroupDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long, PaZoneGroupDatabaseAccessor::PaZoneGroupRecord>::iterator iter 
    ...
    void PaZoneGroupListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        PaZoneGroupDatabaseAccessor::PaZoneGroupRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long, PaZoneGroupDatabaseAccessor::PaZoneGroupRecord>::iterator matching;
        ...
        else
        {
            m_currentDisplayedItems.insert( 
                std::map<unsigned long, PaZoneGroupDatabaseAccessor::PaZoneGroupRecord>::value_type(id, details) );
        }
    ...
    void PaZoneGroupListView::copyItem()
    {
    ...
    void PaZoneGroupListView::newItem()
    {
 ...
 /*   void PaZoneGroupListView::deleteItem()
    {
    ...
    void PaZoneGroupListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void PaZoneGroupListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(PaZoneGroupComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK PaZoneGroupListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        PaZoneGroupListView* list = reinterpret_cast<PaZoneGroupListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<PaZoneGroupListView*>(list);
        ...
        std::map<unsigned long, PaZoneGroupDatabaseAccessor::PaZoneGroupRecord>::iterator item1 
                                                        = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long, PaZoneGroupDatabaseAccessor::PaZoneGroupRecord>::iterator item2 
    ...
    void PaZoneGroupListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // PaZoneGroupListView diagnostics

    #ifdef _DEBUG
    void PaZoneGroupListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void PaZoneGroupListView::Dump(CDumpContext& dc) const
    {

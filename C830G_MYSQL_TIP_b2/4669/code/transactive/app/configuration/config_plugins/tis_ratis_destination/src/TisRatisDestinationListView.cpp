/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/tis_ratis_destination/src/TisRatisDestinationListView.cpp $
  * @author:  James
  * @version: $Revision: #1 $
  *
  ...
  * 
  * This view contains a list of all Tis Ratis Destinations available to be configured. This view must communicate
  * with the TisRatisDestinationDetailView as this will show the details of the item selected in this view.
  */
...
//using TA_IRS_App::TisRatisDestinationDatabaseAccessor::TisRatisDestinationRecord;
typedef TA_IRS_App::TisRatisDestinationDatabaseAccessor::TisRatisDestinationRecord TisRatisDestinationRecord;

using  namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    // These are the possible values for the status of each Tis Ratis Destination
    const CString TisRatisDestinationListView::NEW = "New";
    const CString TisRatisDestinationListView::MODIFIED = "Modified";
    const CString TisRatisDestinationListView::UPTODATE = "";

    const bool TisRatisDestinationListView::ASCENDING = true;
    const bool TisRatisDestinationListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(TisRatisDestinationListView, CListView)


    TisRatisDestinationListView::TisRatisDestinationListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(TisRatisDestinationListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(TisRatisDestinationComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_TIS_RATIS_DESTINATIONS) );
    }
...
BEGIN_MESSAGE_MAP(TisRatisDestinationListView, AbstractListView)
	//{{AFX_MSG_MAP(TisRatisDestinationListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int TisRatisDestinationListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void TisRatisDestinationListView::OnInitialUpdate() 
    {
    ...
    void TisRatisDestinationListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void TisRatisDestinationListView::loadAllItems()
    {
        ...
        TisRatisDestinationDatabaseAccessor* dbAccessor = dynamic_cast<TisRatisDestinationDatabaseAccessor*>(getDatabaseAccessor());
        TA_ASSERT( dbAccessor != NULL, "DB accessor could not be cast to an Tis Ratis Destination one");
        ...
        for (std::map<unsigned long,TisRatisDestinationRecord>::iterator iter = m_currentDisplayedItems.begin(); 
    ...
    void TisRatisDestinationListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        TisRatisDestinationRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long,TisRatisDestinationRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,TisRatisDestinationRecord>::value_type(id,details) );
        }
    ...
    void TisRatisDestinationListView::copyItem()
    {
    ...
    void TisRatisDestinationListView::newItem()
    {
    ...
    void TisRatisDestinationListView::deleteItem()
    {
		...
		// TD14164 ++
		TA_Base_Bus::TransActiveMessage userMsg;
		UINT response = userMsg.showMsgBox(IDS_UW_070028);
		// ++ TD14164 
        if (response == IDNO)
        {
        ...
        catch ( const TA_Base_Core::DataException& ex)
        {
            ...
            if (ex.getFailType() == TA_Base_Core::DataException::CANNOT_DELETE)
            {
				...
				CString errorMsg("it is linked to the following items:\n");
				// ++ TD14164 
				...
				TA_Base_Bus::TransActiveMessage userMsg;
				userMsg << errorMsg;
				userMsg.showMsgBox(IDS_UE_030071);
				// ++ TD14164 
            }
            else
            {
    ...
    void TisRatisDestinationListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void TisRatisDestinationListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(TisRatisDestinationComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK TisRatisDestinationListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        TisRatisDestinationListView* list = reinterpret_cast<TisRatisDestinationListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<TisRatisDestinationListView*>(list);
        ...
        std::map<unsigned long,TisRatisDestinationRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,TisRatisDestinationRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
    ...
    void TisRatisDestinationListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // TisRatisDestinationListView diagnostics

    #ifdef _DEBUG
    void TisRatisDestinationListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void TisRatisDestinationListView::Dump(CDumpContext& dc) const
    {

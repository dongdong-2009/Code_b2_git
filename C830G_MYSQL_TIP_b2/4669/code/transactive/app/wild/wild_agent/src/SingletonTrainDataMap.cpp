 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/4669/transactive/app/wild/wild_agent/src/SingletonTrainDataMap.cpp $
 * @author:  <Andy Siow>
 * @version: $Revision: #2 $
 *
...
namespace TA_IRS_App
{
	...
	TA_IRS_Bus::IAtsWildCorbaDef::AtsWildMessageCorbaDef* SingletonTrainDataMap::retrieveTrainInformation(unsigned long WILDTimestamp)
	{
		...
		for(std::map<time_t, SingletonTrainDataMap::TrainID*>::iterator mapItr = m_trainDataMap.begin(); mapItr != m_trainDataMap.end(); mapItr++)
		{
            ...
            if(static_cast<unsigned int>(abs((int)(mapItr->first - WILDTimestamp))) <= 70 /*WILDConfiguration::getInstance()->getCorrelationThreshold()*/ )//limin++, CL-21009
			{

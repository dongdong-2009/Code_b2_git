/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/4669/transactive/app/radio/radio_monitor_gui/src/RadioMonitorDlg.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    void RadioMonitorDlg::OnActivateApp( BOOL bActive, DWORD hTask )
    {
        if (!bActive)
        {
    ...
    void RadioMonitorDlg::addToSelectionList(std::vector<SelectedEntity>& subscriberSet)
    {
        ...
        for (std::vector<SelectedEntity>::iterator itr = subscriberSet.begin(); itr != subscriberSet.end(); itr ++)
        {
            ...
            RadioMonitorModel::TSISequence::iterator itr2 = 
                    std::find(currentContents.begin(), currentContents.end(), entity.tsi);

            if (itr2 != currentContents.end())
            {

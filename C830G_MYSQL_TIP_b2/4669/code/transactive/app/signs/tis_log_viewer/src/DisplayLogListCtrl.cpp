/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/4669/transactive/app/signs/tis_log_viewer/src/DisplayLogListCtrl.cpp $
  * @author Adam Radics
  * @version $Revision: #2 $
...
BEGIN_MESSAGE_MAP(DisplayLogListCtrl, ListCtrlSelNoFocus)
	...
	ON_MESSAGE(WM_ADD_DISPLAY_REQUESTS, onAddDsiplayRequests)
    // }}AFX_MSG_MAP
END_MESSAGE_MAP()
...
LRESULT DisplayLogListCtrl::onDisplayRequestLoadStart(WPARAM wParam, LPARAM lParam)
{
    ...
    if (m_loadProgressDisplay != NULL)
    {
        ...
        m_loadProgressDisplay->showProgressBar(true, 0, wParam, 100, 0);
    }
...
LRESULT DisplayLogListCtrl::onAddDsiplayRequests(WPARAM wParam, LPARAM lParam)
{
	FUNCTION_ENTRY( "onAddDisplayRequests" );
	
    // wParam is a pointer to an IDisplayRequest object
	std::vector<IDisplayRequest*> *newRequests = reinterpret_cast<std::vector<IDisplayRequest*>*>(wParam);
	
    TA_ASSERT(newRequests != NULL, "Update sent with a null request");
	
    // add it to the list
    addDisplayRequests(newRequests);
...
void DisplayLogListCtrl::addDisplayRequests(std::vector<IDisplayRequest*> *requests)
{
	FUNCTION_ENTRY( "addDisplayRequest" );	
    TA_ASSERT(requests != NULL, "Requests is null");
	int size = requests->size();
	for(int i = 0; i <size; ++i)
	{
		IDisplayRequest* request = (*requests)[i];
		if(requests != NULL)
		{
			addDisplayRequest(request);
		}
	}
	delete requests;
	requests = NULL;
...
IDisplayRequest* DisplayLogListCtrl::getLastSelectItem()
{
    ...
    // get the item data for the selected item
	int itemCount = GetItemCount();
    if (m_latestIndex != -1 && m_latestIndex < itemCount)
    {

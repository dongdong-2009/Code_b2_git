/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/signs/tis_log_viewer/src/DisplayRequestLoadingThread.cpp $
  * @author:  Adam Radics
  * @version: $Revision: #2 $
  *
...
namespace TA_IRS_App
{
    ...
    void DisplayRequestLoadingThread::loadItems(std::vector<IDisplayRequest*>& requests)
    {
		...
		int count = 0;		
		std::vector<IDisplayRequest*> *piece = new std::vector<IDisplayRequest*>;
        for (iter = requests.begin(); (iter != requests.end()) && m_running; iter++)
        {
            ...
            try
            {
				...
				if(100 == count)
				{
					sendUpdate(piece);
					count = 0;
					piece = new std::vector<IDisplayRequest*>;
				}				
	...
	void DisplayRequestLoadingThread::sendUpdate(std::vector<IDisplayRequest*> *updatePiece)
	{
		FUNCTION_ENTRY("sendUpdate");
		TA_ASSERT(m_windowToUpdate != NULL, "window to update is null");
		int ret = m_windowToUpdate->PostMessage( WM_ADD_DISPLAY_REQUESTS,reinterpret_cast<unsigned int>(updatePiece));
		if( 0 == ret )
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Error posting WM_ADD_DISPLAY_REQUESTS message.");
			delete updatePiece;
			updatePiece = NULL;
		}
		FUNCTION_EXIT;

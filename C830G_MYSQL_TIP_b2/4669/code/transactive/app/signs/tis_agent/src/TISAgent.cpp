...
//            TA_Base_Core::MessageSubscriptionManager::getInstance().unsubscribeToMessages( &m_stateUpdateSubscriber );
	...
	void TISAgent::notifyGroupOnline( const std::string& group )
	{
		...
		// TD9154
		int currentStationLibrary = StatusMonitor::getInstance()->getCurrentISCSStationLibraryVersion_central();
		int nextStationLibrary = StatusMonitor::getInstance()->getNextISCSStationLibraryVersion_central();
		
		int currentTrainLibrary = StatusMonitor::getInstance()->getCurrentISCSTrainLibraryVersion_central();
		int nextTrainLibrary = StatusMonitor::getInstance()->getNextISCSTrainLibraryVersion_central();
		...
		try
		{
			int currentOCCStationLibraryVersion;
            CORBA_CALL_RETURN( currentOCCStationLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentSTISMessageLibraryVersion, () );//limin
			int nextOCCStationLibraryVersion;
            CORBA_CALL_RETURN( nextOCCStationLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentSTISMessageLibraryVersion, () );//limin
			...
			int currentOCCTrainLibraryVersion;
            CORBA_CALL_RETURN( currentOCCTrainLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentTTISMessageLibraryVersion, () );//limin
			int nextOCCTrainLibraryVersion;
            CORBA_CALL_RETURN( nextOCCTrainLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentTTISMessageLibraryVersion, () );//limin
			...
			if( ( nextStationLibrary != nextOCCStationLibraryVersion ) &&
				( occStationLibrarySynchronised ) )
			{	
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
					"The OCC has a different next Station library version and is synchronised - setting next Station library version to the same");

				StatusMonitor::getInstance()->setNextISCSStationLibraryVersion( nextOCCStationLibraryVersion );
			}
			if( ( currentOCCStationLibraryVersion == nextOCCStationLibraryVersion ) &&
					 ( currentStationLibrary != currentOCCStationLibraryVersion ) )
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
				...
				bool haveLibrary = STISManager::getInstance()->reportStationLibraryVersionReceived( currentOCCStationLibraryVersion );
				...
				if( haveLibrary )
				{
					...
					StatusMonitor::getInstance()->setNextISCSStationLibraryVersion( currentOCCStationLibraryVersion );
					StatusMonitor::getInstance()->setCurrentISCSStationLibraryVersion( currentOCCStationLibraryVersion );
				}
			}

			// Have a look at the Train library version info
			if( ( nextTrainLibrary != nextOCCTrainLibraryVersion ) &&
				( occTrainLibrarySynchronised ) )
			{	
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
					"The OCC has a different next Train library version and is synchronised - setting next Train library version to the same");

				StatusMonitor::getInstance()->setNextISCSTrainLibraryVersion( nextOCCTrainLibraryVersion );
			}
			if( ( currentOCCTrainLibraryVersion == nextOCCTrainLibraryVersion ) &&
					 ( currentTrainLibrary != currentOCCTrainLibraryVersion ) )
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
				...
				// This station has missed out on an upgrade - check that we have the new predefined
				// library in this station database, and then force an upgrade
				bool haveLibrary = STISManager::getInstance()->reportTrainLibraryVersionReceived( currentOCCTrainLibraryVersion );
				...
				if( haveLibrary )
				{
					...
					StatusMonitor::getInstance()->setNextISCSTrainLibraryVersion( currentOCCTrainLibraryVersion );
					StatusMonitor::getInstance()->setCurrentISCSTrainLibraryVersion( currentOCCTrainLibraryVersion );
				}
			}
		}
		catch ( const TA_Base_Core::ObjectResolutionException& )
		{
			LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, 
				"Could not resolve the DBG TISAgent while attempting to check the predefined message library versions");
		}
		catch ( const CORBA::Exception& ce )
		{
			LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
				"Caught a CORBA exception while attempting to check the predefined message library versions at the OCC");
			std::string wtf = TA_Base_Core::CorbaUtil::getInstance().exceptionToString( ce );
		}
	...
	IEntity* TISAgent::createDataPointEntity ( TA_Base_Core::IEntityDataPtr EntityData )
	{
		...
		if ( 0 != theEntity )
		{
			...
			std::string address = theEntity->getDataPointAddress();
			std::string pidName = getEquipmentName( theEntity->getDataPointName() );
			std::string dpName = getShortDataPointName( theEntity->getDataPointName() );

			DataPointDetails* theDPDetails = new DataPointDetails();

			theDPDetails->dataPoint = theEntity;
			theDPDetails->dataPointName = dpName;
			theDPDetails->shortPIDName = pidName;
			theDPDetails->address = address;

			std::stringstream dpMessage("");

			dpMessage << "Created datapoint: " << theEntity->getDataPointName() << "\n" ;
			
			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, dpMessage.str().c_str() );
			
			// Check name of the datapoint - if it belongs to the 
			// the STIS or ISCS server e.g. DBG.TIS.STIS, or DBG.TIS.ISCS
			// then pass it to the status monitor (it is a resource status point, or an ISCS state point)
			// otherwise it is a PID datapoint, and should be stored in the STISManager
			if( ( address.compare("STIS") == 0 ) || ( address.compare("ISCS") == 0 ) )// this DP is one of the status points (ie not a PID)
			{
				StatusMonitor::getInstance()->addDataPoint( theDPDetails );
			}
			else if( (theDPDetails->dataPointName.compare("") !=0 ) && (theDPDetails->shortPIDName.compare("") != 0) ) // ensure this is a PID status point
			{
				STISManager::getInstance()->addDataPoint( theDPDetails );
			}
			else
			{
				delete theEntity;
	...
	void TISAgent::run()
	{
        ...
        if( m_locationType == TA_Base_Core::ILocation::OCC )
		{
			...
			int currentCDBStationLibrary = StatusMonitor::getInstance()->getCurrentISCSStationLibraryVersion_central();
			int nextCDBStationLibrary = StatusMonitor::getInstance()->getNextISCSStationLibraryVersion_central();
			bool stationLibrarySynchronised = StatusMonitor::getInstance()->getStationLibrariesSynchronised();

			int currentCDBTrainLibrary = StatusMonitor::getInstance()->getCurrentISCSTrainLibraryVersion_central();
			int nextCDBTrainLibrary = StatusMonitor::getInstance()->getNextISCSTrainLibraryVersion_central();
		...
		else // Must be a station
		{
			...
			try
			{
				...
				int currentLocalStationLibrary = StatusMonitor::getInstance()->getCurrentISCSStationLibraryVersion();
				int nextLocalStationLibrary = StatusMonitor::getInstance()->getNextISCSStationLibraryVersion();

				int currentLocalTrainLibrary = StatusMonitor::getInstance()->getCurrentISCSTrainLibraryVersion();
				int nextLocalTrainLibrary = StatusMonitor::getInstance()->getNextISCSTrainLibraryVersion();

				int currentCDBStationLibraryVersion;
				...
				int nextCDBStationLibraryVersion;
				...
				int currentCDBTrainLibraryVersion;
				...
				int nextCDBTrainLibraryVersion;

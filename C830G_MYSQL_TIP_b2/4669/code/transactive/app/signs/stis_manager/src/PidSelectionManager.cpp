/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/signs/stis_manager/src/PidSelectionManager.cpp $
  * @author:  Adam Radics
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    void PidSelectionManager::setRatisMode(bool ratisMode)
    {
        if (m_RATISMode != ratisMode)
        {
            m_RATISMode = ratisMode;
            ...
            if (m_RATISMode)
            {
                // disable all
                m_pidListCtrl.DeleteAllItems();
                m_pidListCtrl.EnableWindow(FALSE);

                m_pidGroupCombo.SetCurSel(-1);
                m_pidGroupCombo.EnableWindow(FALSE);
            ...
            else
            {
                // enable all
                refreshPIDList();
                m_pidListCtrl.EnableWindow(TRUE);

                m_pidGroupCombo.EnableWindow(TRUE);
                m_pidGroupCombo.SelectString(-1, m_activeGroupName.c_str());
    ...
    void PidSelectionManager::savePidGroup()
    {
        ...
        CreateNewGroupDlg dialog ( "STIS Manager",
        ...
        if (m_auditMessageSender != NULL)
        {
			...
			*/
            ...
            m_auditMessageSender->sendAuditMessage( TA_Base_Core::TISAudit::STISPidGroupCreated, guiEntity->getKey(),
			    desc, 
			    "", // Further description text
    ...
    void PidSelectionManager::deletePidGroup()
    {
		...
		if (m_auditMessageSender != NULL)
        {
			...
			*/
            ...
            m_auditMessageSender->sendAuditMessage( TA_Base_Core::TISAudit::STISPidGroupDeleted, guiEntity->getKey(),
			    desc, 
			    "", // Further description text
			    TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID),
			    "", // alarm ID - not required
    ...
    {
        if (m_RATISMode) // TD9001 - stop the manager from crashing when it gets the pid changes when it's in ratis mode
        {
            return;
        }
...
//		TA_Base_Core::DescriptionParameters desc;
//		TA_Base_Core::NameValuePair pidGroupName("Name", pidGroupNameStr );
//		TA_Base_Core::NameValuePair location( "PID LIST", selectedPIDNamesStream.str() );
//
//		desc.push_back(&pidGroupName);
//		desc.push_back(&location);
//
...
//			TA_ASSERT ( guiEntity != NULL, "EntityAccessFactory returned a NULL entity and yet did not throw an exception." );
//
//            m_auditMessageSender->sendAuditMessage( TA_Base_Core::TISAudit::STISPidGroupModified, guiEntity->getKey(),
//			    desc, 
//			    "", // Further description text
//			    TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID),
//			    "", // alarm ID - not required
...
//		//TES831 Events not logged!
			...
			if(( stationID == m_currentLocation ) && ( newSchematicID == m_schematicID ))
			{
				try
				{
					...
					if (parts.size() == 1)
					{
						...
						if( parts[0] == "???" ) // deselect everything
						{
							// deselect all PIDs
							deselectAllPIDs();
						}
    ...
    void PidSelectionManager::refreshPIDList()
    {
	    ...
	    for( ; itPid != selectedPids.end() ; itPid++ )
	    {  
            int pos = m_pidListCtrl.InsertItem( PidListCtrl::LOCATION, itPid->station.c_str() );

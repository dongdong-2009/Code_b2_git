/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/3001/transactive/core/data_access_interface/video_switch_agent/src/VideoOutputGroupHelper.cpp $
  * @author Katherine Thomson
  * @version $Revision: #5 $
  * Last modification: $DateTime: 2013/03/21 11:13:54 $
...
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), VI_VIDEO_OUTPUT_GROUP_STD_SELECT_62503, m_key);
//		std::string strSql  = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_SELECT_62503, m_key);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_SELECT_62503, m_key);
    ...
    void VideoOutputGroupHelper::deleteVideoOutputGroup()
    {
       ...
       /* std::stringstream deleteSql;
        ...
        deleteSql << "=" << m_key;*/
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), VI_VIDEO_OUTPUT_GROUP_STD_DELETE_62901, m_key);
//		std::string strSql  = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_DELETE_62901, m_key);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_DELETE_62901, m_key);
    ...
    void VideoOutputGroupHelper::insertNewVideoOutputGroup()
    {
      ...
      /*  std::stringstream selectSql;
        ...
        selectSql << " where " << TA_VideoSwitchAgentDAI::VIDEO_OUTPUT_GROUP_KEY_COLUMN;
        selectSql << "=" << m_key;*/
//		std::string strSql = defPrepareSQLStatement(nDbServerType, VI_VIDEO_OUTPUT_GROUP_STD_SELECT_62504, m_key);
//		std::string strSql = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_SELECT_62504, m_key);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_SELECT_62504, m_key);
        ...
        for( ; it != m_videoOutputs.end(); ++it )
        {
           /* fullInsertSql.str( "" );
			...
			//strSql = defPrepareSQLStatement(nDbServerType, VI_VIDEO_OUTPUT_GROUP_STD_INSERT_62801, m_key, it->first, it->second);
//			strSql = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_INSERT_62801, m_key, it->first, it->second);

		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_INSERT_62801, m_key, it->first, it->second);
            databaseConnection->executeModification( strSql );
        }
		  ...
		  /*  std::stringstream updateSqlPart1;
           ...
           /* std::stringstream updateSqlPart2;
            ...
            for( unsigned long i = 0; i < numRows; i++ )
            {
				...
				//		strSql = defPrepareSQLStatement(nDbServerType, VI_VIDEO_OUTPUT_GROUP_STD_UPDATE_62651, videoOutputKey, m_key, position);
//						strSql = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_UPDATE_62651, videoOutputKey, m_key, position);

		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_UPDATE_62651, videoOutputKey, m_key, position);
                        databaseConnection->executeModification( strSql );
                    }
                }
...
//				strSql = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_INSERT_62801, m_key, it->first, it->second);
		...
		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_INSERT_62801, m_key, it->first, it->second);
            ...
            //deleteSql << ")";
		//	strSql = defPrepareSQLStatement(nDbServerType, VI_VIDEO_OUTPUT_GROUP_STD_DELETE_62902, m_key, deleteSql.str());
//			strSql = databaseConnection->prepareSQLStatement(VI_VIDEO_OUTPUT_GROUP_DELETE_62902,  m_key, deleteSql.str());

		databaseConnection->prepareSQLStatement(strSql, VI_VIDEO_OUTPUT_GROUP_DELETE_62902,  m_key, deleteSql.str());
            databaseConnection->executeModification( strSql );
        }
        m_isNew = false;
    }

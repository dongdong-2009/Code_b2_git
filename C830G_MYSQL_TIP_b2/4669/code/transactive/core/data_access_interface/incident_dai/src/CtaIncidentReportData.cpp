/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/core/data_access_interface/incident_dai/src/CtaIncidentReportData.cpp $
  * @author:  Grace Koh
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2014/07/02 18:53:47 $
...
//Bug 652 (TD12313)
	...
	CtaIncidentReportData::CtaIncidentReportData(const CtaIncidentReportData& source)
        :m_stationReportData(NULL), m_incidentReportHelper(NULL),
		m_isModified( const_cast<CtaIncidentReportData&>(source).isModified() ),
		m_canClose(true),
		m_stationReportValid(true)
	{
		...
		if (m_stationReportData == NULL)
		{
			...
			else
			{
				...
				if(!isStationReportValid())
				{
					...
					m_stationReportData = new CtaStationReportData(stationId,true);
				}
			}
		}
		//Mintao ++
	...
	*/
	...
	//This method display only 7 digit unqiue number of a location
	//(excludes location name and date part of the report id)
	std::string CtaIncidentReportData::getDisplayReportId()
	{
		FUNCTION_ENTRY("getDisplayReportId");
		TA_ASSERT(m_incidentReportHelper != NULL, "Helper is NULL");	
		std::string reportId = m_incidentReportHelper->getReportId();		
		...
		if(reportId != "")
		{
			...
			//old format support which is 5 digit long
	...
	bool CtaIncidentReportData::isStationReportValid()
	{
		m_stationReportValid = true;
		std::string stationId = m_incidentReportHelper->getStationId();
		if (!stationId.empty())
		{
			...
			try
			{
				m_stationReportData->getCreatorProfile();
			}
			catch(TA_Base_Core::DataException ex)
			{
				LOG( SourceInfo, DebugUtil::ExceptionCatch, "DataException",ex.what());				
				m_stationReportValid = false;
			}
		}              			
		return m_stationReportValid;
	}
} // end TA_IRS_Core

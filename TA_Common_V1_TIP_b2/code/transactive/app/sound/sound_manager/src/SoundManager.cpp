...
inline HRESULT CServiceModule::RegisterServer(BOOL bRegTypeLib, BOOL bService)
{
    ...
    if (FAILED(hr))
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Failed to initialize COM. Error code: %lu", hr);
        return hr;
	}
	...
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Failed to open AppID: %lu", lRes);
        return lRes;
	}
    ...
    if (lRes != ERROR_SUCCESS)
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Failed to open key 833DEE1C-D5FC-4868-9C8B-07C22F54A216: %lu", lRes);
        return lRes;
	}
	...
	if (lRes != ERROR_SUCCESS)
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Failed to open key HKEY_CLASSES_ROOT: %lu", lRes);
		return lRes;
	}

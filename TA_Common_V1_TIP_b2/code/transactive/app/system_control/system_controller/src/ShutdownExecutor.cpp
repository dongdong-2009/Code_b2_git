...
namespace TA_Base_App
{
                ...
                for (size_t i = 0; i <= m_shutdownRetryTimes; ++i)
                {
                    ...
                    catch (CORBA::SystemException& e)
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException",
                                            str(boost::format("System exception occurred while calling peer shutdown executor %s - %s") % it->first % CorbaUtil::exceptionToString(e)));
                    }
                    catch (...)
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", str(boost::format("Error occurred calling peer shutdown executor %s") % it->first));
                    }
                }
            }
        }
    ...
    std::map<std::string, TA_Base_Core::IShutdownCorbaDef_var> ShutdownExecutor::resolvePeerShutdownExecutor()
    {
        ...
        for (std::vector<std::string>::iterator it = peerHostnames.begin(); it != peerHostnames.end(); ++it)
        {
            ...
            std::string peerShutdownExecutorIor = str(boost::format("corbaloc::%s:%d/ShutdownExecutor") % *it % port);

            // Attempt to resolve an IShutdownCorbaDef object
            CORBA::Object_var obj = CorbaUtil::getInstance().stringToObject(peerShutdownExecutorIor);
            ...
            if (!CORBA::is_nil(obj))
            {
                for (size_t i = 0; i <= m_shutdownRetryTimes; ++i)
                {
                    try
                    {
                        ...
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "resolved a peer shutdown executor: %s", peerShutdownExecutorIor.c_str());
                    ...
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException",
                                            str(boost::format("System exception occurred while resolving peer shutdown executor %s - %s") % peerShutdownExecutorIor % CorbaUtil::exceptionToString(e)));
                    }
                    catch (...)
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", str(boost::format("Error occurred narrowing %s") % peerShutdownExecutorIor));
                    }
                }
            }
            else
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Cannot resolve peer Shutdown Executor %s", peerShutdownExecutorIor.c_str());
            }
        }

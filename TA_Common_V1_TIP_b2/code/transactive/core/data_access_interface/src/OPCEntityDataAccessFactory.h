...
namespace TA_Base_Core
{
	struct OPCDataPointData
	{
		...
		bool		  isOutputDataPoint;
		std::string   entityName;
		OPCDataPointData(){}
		OPCDataPointData(unsigned long paraEntityKey, 
						 unsigned long paraAgentKey, 
						 unsigned long paraLocationKey,
						 bool		   paraIsOutputDataPoint,
						 std::string&  paraName)
		:
		entityKey(paraEntityKey),
		agentKey(paraAgentKey),
		locationKey(paraLocationKey),
		isOutputDataPoint(paraIsOutputDataPoint),
		entityName(paraName)
		{
		}
	};
	...
	struct OPCDataNodeData
	{
		...
		OPCDataNodeData(unsigned long paraEntityKey, 
						unsigned long paraAgentKey, 
						unsigned long paraLocationKey, 
						std::string& paraName, 
						std::string& paraShortLabel,
						std::string& paraLabel, 
						std::string& paraAssetName)
		:
		entityKey(paraEntityKey),
		agentKey(paraAgentKey),
		locationKey(paraLocationKey),
	...
	class OPCEntityDataAccessFactory
	{
		...
		private:
			...
			void getOPCDpData( IDatabase* pDatabase, const SQLStatement& sql, const SQLStatement& sqlParameter, 
				OPCDataPointDataList& dataList, EntityParameterDataList& dpParameterList);
			void assembleDpOPCData(OPCDataPointDataList& dataList,EntityParameterDataList& dpParameterList);

...
namespace TA_Base_Core
{
	...
	*/
	...
	{
		OPCDataPointDataList* pDataList = new OPCDataPointDataList();
		EntityParameterDataList dpParaList;
		...
		//Can got more entityparametervalue in the future...
		sqlParameterNames += "'";
		sqlParameterNames += DataPointEntityData::DATAPOINT_ALLOW_WRITE;
		sqlParameterNames += "'";
		sqlParameterNames += ")";
		...
		databaseConnection->prepareSQLStatement(sqlParameter, ENTITYPARAMETERVALUE_V_SELECT_3074, sqlLocationKeys.str(), dpTypeName,  sqlParameterNames);

		getOPCDpData(databaseConnection, sql, sqlParameter, *pDataList, dpParaList);
		assembleDpOPCData(*pDataList, dpParaList);
	...
	OPCDataPointDataList* OPCEntityDataAccessFactory::getOPCDpDataAtAllLocations()
	{
		...
		databaseConnection->prepareSQLStatement(sql, ENTITYTYPE_SELECT_4022, dpTypeName);
		SQLStatement sqlParameter;
		databaseConnection->prepareSQLStatement(sqlParameter, ENTITYPARAMETERVALUE_V_SELECT_3073, dpTypeName, sqlParameterNames);
	...
	*/
	...
	void OPCEntityDataAccessFactory::getOPCDpData( IDatabase* pDatabase, const SQLStatement& sql, const SQLStatement& sqlParameter, 
		OPCDataPointDataList& dataList, EntityParameterDataList& dpParameterList )
	{
		std::vector<std::string> columnNames;
		std::string entityKeyColumn("ENTITYKEY");
		std::string entityNameColumn("ENTITYNAME");
		...
		do
		{
			for (unsigned long i = 0; i < dataParameter->getNumRows() ; i++ )
			{
				...
				dpParameterList.push_back(parameterData);
			}
	...
	void OPCEntityDataAccessFactory::assembleDpOPCData(OPCDataPointDataList& dataList, EntityParameterDataList& dpParameterList)
	{
		std::map<unsigned long, OPCDataPointData*> tmpDataMap;

		OPCDataPointDataList::iterator itData;
		...
		OPCDataPointData* opcDpData = NULL;
		
		EntityParameterDataList::iterator itParameter;
		std::map<unsigned long, OPCDataPointData*>::iterator itDataMap = tmpDataMap.end();
		for (itParameter = dpParameterList.begin(); itParameter != dpParameterList.end(); ++itParameter)
		{
			...
			if ( NULL == opcDpData ) continue;

			if (itParameter->parameterName == TA_Base_Core::DataPointEntityData::DATAPOINT_ALLOW_WRITE)
			{
				opcDpData->isOutputDataPoint = TA_Base_Core::EntityHelper::getBooleanData(itParameter->parameterValue);
			}
		}
	}	

...
namespace TA_Base_Bus
{
	...
	AuthenPeerTaskManager::AuthenPeerTaskManager(AuthenticationLibrary * pAuthenLib)
	{
		...
		std::map<unsigned long, AuthenticationAgentNamedObjPtr> & authenAgentMap = 
			KeepSessionAliveThreadManager::getInstance().getAuthenAgentMap();
		TA_Assert( authenAgentMap.size() > 0 );

		std::map<unsigned long, AuthenticationAgentNamedObjPtr>::iterator it;
...
//				continue;
//			}
			AuthenticationAgentNamedObjPtr pAuthenAgent = (*it).second;
			TA_Assert( pAuthenAgent );

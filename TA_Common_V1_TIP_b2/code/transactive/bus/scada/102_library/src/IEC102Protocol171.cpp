#include "bus/scada/102_library/src/IEC102Protocol171.h"


namespace TA_Base_Bus
{
	IEC102Protocol171::IEC102Protocol171(IEC102Controller171& controller)
		:IEC102Protocol(controller),
		 m_lowVirtualAddress(0),
		 m_highVirtualAddress(0)

	{
		FUNCTION_ENTRY("IEC102Protocol171()");
		FUNCTION_EXIT;
	}

	IEC102Protocol171::~IEC102Protocol171()
	{
		FUNCTION_ENTRY("~IEC102Protocol171()");
		FUNCTION_EXIT;
	}

	void IEC102Protocol171::setVirtualAddress(unsigned short nAddress)
	{
	...
	/*************************************************************************
	send command to read current remote measurement info within specified address (171)( bytes). 
	This command is defined in YuBang document.
	*************************************************************************/
	
	std::string IEC102Protocol171::getRemoteMeasurement_C_YC_NA_2( unsigned short startAddr, unsigned short endAddr)
	{
		...
		TA_Base_Bus::C_YC_NA_2,
		m_lowAddress,m_highAddress,m_lowVirtualAddress,m_highVirtualAddress,
		ucStartAddr,ucEndAddr);
	...
	bool IEC102Protocol171::checkMatchWithRequestMessage(std::string& strMessage)
	{
	...
	void IEC102Protocol171::processData(std::vector<unsigned char> &buffer)
	{
		FUNCTION_ENTRY("processData()");
		
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s","Protocol 171 start to process data.");

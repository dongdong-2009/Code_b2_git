/**
  ...
  * any form.
  *
  * Source:		$File: //depot/TA_Common_V1_TIP/transactive/bus/scada/rtu_library/src/BaseRTUManager.cpp $
  * @author:	HoaVu
  * @version:	$Revision: #19 $
  *
...
namespace TA_Base_Bus
{
    ...
    void BaseRTUManager::setToControl()
    {
        ...
        for (i = 0; i < m_rtuDpProcessors.size(); ++i)
        {
            m_rtuDpProcessors[i]->setToControl();
        }
    ...
    void BaseRTUManager::setToMonitor()
    {
        ...
        for (i = 0; i < m_rtus.size(); ++ i)
        {
            ...
            m_rtuHasKnownStatus[m_rtus[i]->getPKey()] = false;
        }

        for (i = 0; i < m_rtuDpProcessors.size(); ++i)
        {
            m_rtuDpProcessors[i]->setToMonitor();
        }

        for (i = 0; i < m_qualityStatus.size(); i++)
        {
            m_qualityStatus[i] = TA_Base_Bus::QUALITY_BAD_NOT_CONNECTED;
        }
        
        if (NULL != m_redundancyWorker )
        {
            m_redundancyWorker->resetPrimaryRTU();
        }

        resetRTUManagerStatus();
	}
    ...
    {   
        {
            TA_Base_Core::ThreadGuard guard(m_rtuHasKnownStatusLock);
            if (false == m_rtuHasKnownStatus[rtu.getPKey()])
            {
                m_rtuHasKnownStatus[rtu.getPKey()] = true;
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "RTUManager has know rtu %s real status", rtu.getName().c_str());
            }
        }

        if (false == hasKnownAllRTUStatus())
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Detect some rtus' status still in unknown, so pass the processing.");
            return;
        }
	...
	void BaseRTUManager::addRTU(RTU& rtu)
	{
        ...
        m_rtuHasKnownStatus[rtu.getPKey()] = false;

	}	

    bool BaseRTUManager::hasKnownAllRTUStatus()
    {
        TA_Base_Core::ThreadGuard guard(m_rtuHasKnownStatusLock);
        bool knownStatus = true;

        for (int i = 0; i < m_rtus.size(); i++)
        {
            // once find any rtu in service and in the unknow status, just return false
            // just care rtu in services, because only these rtus
            // will poll data
            if (m_rtus[i]->isInService() && false == m_rtuHasKnownStatus[m_rtus[i]->getPKey()])
            {
                knownStatus = false;
                break;
            }

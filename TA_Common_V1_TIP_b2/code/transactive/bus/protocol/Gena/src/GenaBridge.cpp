...
namespace TA_Base_Bus
{
	...
	*/
	std::string GenaBridge::subscribe(const std::string& topicURL, const std::string& notification)
	{
		...
		LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "GenaBridge::subscribe=%s", topicURL.c_str());

		m_pGenaClient->resetSubscribe(topicURL,notification);
	...
	*/
	void GenaBridge::unsubscribe(const std::string& topicURL)
	{
		...
		LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "GenaBridge::unsubscribe=%s", topicURL.c_str());

		m_pGenaClient->unsubscribe(topicURL,"");
	...
	*/
	...
	{
		FUNCTION_ENTRY("onNotifyUpdate");
		LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Forwarding notification for topic %s", topicData->getTopicName().c_str());
		topicData->setTopicURI(m_topicURL);

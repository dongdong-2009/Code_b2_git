...
namespace TA_Base_Bus
{
		...
		, m_pHttpServer(0)
	{
		...
		start();
		m_pSubscriptionMgr->startSubscribeMgr();		
		FUNCTION_EXIT;
	}
	...
	GenaClient::~GenaClient()
	{
		...
		if (m_pSubscriptionMgr!=NULL) m_pSubscriptionMgr->terminateAndWait();
		stopService();
		destroyService();
		FUNCTION_EXIT;
	}
	...
	//* subscribe
	//*/
	//void GenaClient::subscribe(const std::string& strTopicName, const std::string& notification,
	//	const T_TopicFilterSet& filterSet)
	//{
	//	FUNCTION_ENTRY("subscribe");
	//	m_pSubscriptionMgr->subscribe(strTopicName, notification, filterSet);
	//	
	//	FUNCTION_EXIT;
	//	//return ulSID;
	//}
	...
	* subscribe
	*/
	//void GenaClient::subscribe(const std::string& topicURL, const std::string& notification)
	//{
	//	FUNCTION_ENTRY("subscribe");
	//	 m_pSubscriptionMgr->subscribe(topicURL, notification);
	...
	* addSubscribeTopic
	*/
	void GenaClient::addSubscribeTopic(T_ClientTopicList& topicList)
	{
		FUNCTION_ENTRY("addSubscribeTopic");
		m_pSubscriptionMgr->addSubscribeTopic(topicList);
		FUNCTION_EXIT;
	}
	...
	* subscribe
	*/
	void GenaClient::resetSubscribe(const std::string& topicURL, const std::string& notification)
	{
		FUNCTION_ENTRY("subscribe");
		m_pSubscriptionMgr->resetSubscribeTopic(topicURL, notification);
	...
	*/
	...
	void GenaClient::unsubscribe(const std::string& topicURL, const std::string& notification)
	{
		FUNCTION_ENTRY("subscribe");
		m_pSubscriptionMgr->unsubscribe(topicURL, notification);
		FUNCTION_EXIT;		 
	}
	...
	void GenaClient::startService()
	{
		...
		m_pConnectionMgr->startConnectionMgr();
		//m_pSubscriptionMgr->startSubscribeMgr();		
		FUNCTION_EXIT;
	}
	...
	void GenaClient::stopService()
	{
		...
		m_pSubscriptionMgr->stopSubscribeAllTopic();
		m_pConnectionMgr->stopConnectionMgr();
		FUNCTION_EXIT;
	}

...
namespace TA_Base_Bus
{
	...
	public:
		...
		ConnectionPtr createConnection(const std::string& connectionURL, unsigned long timeOut, int nMaxRetry, const std::string& dpAddress);
		...
		void notifyStateChange(const std::string& strServer, const GENAConnectionStatus& status);

		void startConnectionMgr();

		void stopConnectionMgr();
	...
	private:
		...
		TA_Base_Core::ReEntrantThreadLockable m_ActionConnLock;
		TA_Base_Core::ReEntrantThreadLockable m_ObserverConnLock;
	...
	private:
		//To Check if all connections are bad status
		bool isConnStatusBadAll();
	};

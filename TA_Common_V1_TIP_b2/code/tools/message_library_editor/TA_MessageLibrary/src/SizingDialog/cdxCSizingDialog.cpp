...
BOOL cdxCSizingDialog::ControlInfo::Header::OnInitDialog(UINT addSzByPcnt)
{
	...
	if(m_bSizeIcon)
	{
		...
		{
			TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::OnInitDialog()]: LoadOEMBitmap() failed.\n"));
			m_bSizeIcon	=	false;
		}
		else
		{
			...
			{
				TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::OnInitDialog()]: Can't create static icon control\n"));
				m_bSizeIcon	=	false;
			}
			else
			{
				...
				{
					TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::OnInitDialog()]: Can't get windowplacement for sizeicon.\n"));
					m_bSizeIcon	=	false;
				}
				else
				{
					...
					{
						TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::OnInitDialog()]: Can't set windowplacement for sizeicon.\n"));
						m_bSizeIcon	=	false;
					}
					else
					{
...
void cdxCSizingDialog::ControlInfo::Header::StretchWindow(UINT addSzByPcnt)
{
	...
	if(!m_pWnd->GetWindowPlacement(&wpl))
	{
		...
		TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::StretchWindow()]: GetWindowPlacement() failed for dialog with caption \"%s\" !\n"),(LPCTSTR)s);
#endif
		return;
	}
...
cdxCSizingDialog::ControlInfo *cdxCSizingDialog::ControlInfo::Header::AddSzControlEx(CWnd & ctrl, BYTE dX1pcnt, BYTE dX2pcnt, BYTE dY1pcnt, BYTE dY2pcnt)
{
		...
		(dY1pcnt == exIgnore) && (dY2pcnt == exIgnore) )
	{
		...
		TRACE1("*** WARNING[cdxCSizingDialog::ControlInfo::Header::AddSzControl()]: For control \"%s\", all modifcators are set to NONE (control will not be added).\n",(LPCTSTR)s);
#endif
		return NULL;
	}
...
bool cdxCSizingDialog::ControlInfo::Header::RestoreWindowPosition(const CString & profileSec, bool withState)
{
	...
	if(!m_pWnd->GetWindowPlacement(&wpl))
	{
		...
		if(s.IsEmpty())
			s.Format(_T("<Address_0x%lx"),(UINT)this);
		TRACE(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::RestoreWindowPosition()]: GetWindowPlacement() failed for dialog with caption \"%s\" !\n"),(LPCTSTR)s);
#endif
		return false;
	}
...
BOOL cdxCSizingPropSheet::OnInitDialog(UINT addSzByPcnt) 
{
	...
	for(i=0; psheetCtrls[i].id != BAD_ID; ++i)
	{
		...
		{
			TRACE1("*** WARNING[cdxCSizingPropSheet::OnInitDialog()]: Can't find CWnd for ID#%ld !\n",psheetCtrls[i].id);
		}
	}

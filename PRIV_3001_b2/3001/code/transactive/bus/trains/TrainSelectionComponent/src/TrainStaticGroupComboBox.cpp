...
namespace TA_IRS_Bus
{
    ...
    void TrainStaticGroupComboBox::updateGroupNames(const std::vector<std::string>& trainListNames)
    {
        ...
        {
			if(trainListNames[i].empty())
				continue;
            AddString(trainListNames[i].c_str());
        }
		
		//Double check if combo box count is not 0
		...
		int defaultSelect = 0;
		//Set index 0 as default selected which is the smallest Radio Group ID
		SetCurSel(defaultSelect);
		
		CString itemT;
		GetLBText(defaultSelect, itemT);
		SetWindowText(itemT);
		...
		//No need to reselect the old item since the default should always be index 0 which is the smallest Radio Group ID.

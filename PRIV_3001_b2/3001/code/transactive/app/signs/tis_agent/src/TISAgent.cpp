/**
  ...
  *
  * Source:    $File: //depot/PRIV_3001/3001/transactive/app/signs/tis_agent/src/TISAgent.cpp $
	...
	void TISAgent::run()
	{
		...
		else // Must be a station
		{
			...
			try
			{
                ...
                CORBA_CALL_RETURN( currentCDBStationLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentCDBSTISMessageLibraryVersion, () );
				unsigned long nextCDBStationLibraryVersion;
                CORBA_CALL_RETURN( nextCDBStationLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentCDBSTISMessageLibraryVersion, () );
				bool occStationLibrarySynchronised;
                CORBA_CALL_RETURN( occStationLibrarySynchronised, TISAgentAccessFactory::getInstance().getOccSTISAgent(), isStationLibrarySynchronisationComplete, () );

				unsigned long currentCDBTrainLibraryVersion;
                CORBA_CALL_RETURN( currentCDBTrainLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentCDBTTISMessageLibraryVersion, () );
				unsigned long nextCDBTrainLibraryVersion;
                CORBA_CALL_RETURN( nextCDBTrainLibraryVersion, TISAgentAccessFactory::getInstance().getOccSTISAgent(), getCurrentCDBTTISMessageLibraryVersion, () );
				bool occTrainLibrarySynchronised;
                CORBA_CALL_RETURN( occTrainLibrarySynchronised, TISAgentAccessFactory::getInstance().getOccSTISAgent(), isTrainLibrarySynchronisationComplete, () );

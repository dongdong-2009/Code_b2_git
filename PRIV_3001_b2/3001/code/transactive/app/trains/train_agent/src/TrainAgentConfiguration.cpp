...
namespace TA_IRS_App
{
	...
	unsigned long TrainAgentConfiguration::getTimerPEC3()
	{
		FUNCTION_ENTRY( "getTimerPEC3" );
		FUNCTION_EXIT;
		return m_globalEntityData->getTimerPEC3();
	}
	...
	unsigned long TrainAgentConfiguration::getTimerPEC5()
	{
		FUNCTION_ENTRY( "getTimerPEC5" );
		FUNCTION_EXIT;
		return m_globalEntityData->getTimerPEC5();
	}
	...
	std::vector<unsigned long> TrainAgentConfiguration::getStateSyncLocPriority()
	{
		FUNCTION_ENTRY("getStateSyncLocPriority");

		std::string strDepotSynPriority = m_globalEntityData->getStateSyncLocPriority();
		std::istringstream strDepot(strDepotSynPriority);

		std::string number_str;
		std::vector<unsigned long> depotSyncValue;
		unsigned long number;

		//Split the StateSyncLocPriority string using comma separated
		while ( getline( strDepot, number_str, ',' ) // separate at comma
			&& std::istringstream( number_str ) >> number ) {
				depotSyncValue.push_back( number );
		}

		FUNCTION_EXIT;
		return depotSyncValue;
	}

	void TrainAgentConfiguration::tokenize(const std::string& data, const std::string& delims, Tokens& tokens)
	{
		FUNCTION_ENTRY("tokenize");
		...
		{
			tokens.insert(Token(boost::lexical_cast<int>(*it++), *it));
		}

		FUNCTION_EXIT;
	}

	TrainAgentConfiguration::Tokens TrainAgentConfiguration::getTrainStatus12Map()
	{
		FUNCTION_ENTRY("getTrainStatus12Map");

		const std::string status12Data = m_globalEntityData->getTrainStatus12();
		const std::string delims("=,"); 
		Tokens status12Token;

		tokenize(status12Data, delims, status12Token);

		FUNCTION_EXIT;
		return status12Token;
	}

	unsigned char TrainAgentConfiguration::getTrainStatus12()
	{
		FUNCTION_ENTRY("getTrainStatus12");
		...
		// found it
		std::string locationName = findIter->second.name;
		boost::to_upper(locationName);

		Tokens trainStatus12Map = getTrainStatus12Map();

		unsigned char retStatus12 = 0;

		//Find the Status 12 information based on the Database parameter
		//1=TSFB,2=GDCC
		Tokens::iterator trainStatusIter = trainStatus12Map.begin();
		for (trainStatusIter; trainStatusIter != trainStatus12Map.end(); trainStatusIter++ )
		{
			std::string status12Data = trainStatusIter->second;
			boost::to_upper(status12Data);
			
			//Check if the location name is equal
			if ( locationName.compare(status12Data) == 0)
			{
				retStatus12 = trainStatusIter->first;
				break;
			}
		}

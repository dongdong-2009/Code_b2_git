...
namespace TA_IRS_App
{
                                                      ...
                                                      const std::string& sessionId )
    {
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"Operator rights =%d", m_rightsChecker.canCallTrain( sessionId ));
		...
		LOG_GENERIC (SourceInfo, 
					TA_Base_Core::DebugUtil::DebugInfo,
					"Test Log : calling RadioFunction makePrivateCall, resource ID : %s",
					primaryTsi.c_str());
		
		unsigned long lPrimaryTsi = boost::lexical_cast<unsigned long>(primaryTsi);
		m_radioAccess.makePrivateCall( sessionId,lPrimaryTsi);
...

        {
			...
			// no need to clear call state yet, for the call is still establish
            TA_IRS_Bus::CallTypes::OccCallReset callReset;
            callReset.trainId = trainId;

			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
				"clearIncomingCall: callReset trainID = %d", trainId);
    ...
    void TimsDriverCallManager::processTrainStateChanged( TA_IRS_Bus::TrainStatePtr state )
    {
		...
		unsigned long configLocation = 0;
		...
		configLocation = m_configuration.getLocationKey();
               ...
               ( communicatingLocation != m_configuration.getLocationKey() ) ) )
        {
            ...
            if ( trainCallState->hasIncomingCall() )
            {
                ...
                LOG_GENERIC( SourceInfo,
                             TA_Base_Core::DebugUtil::DebugInfo,
                             "The call driver request for train %d is no loger valid due to the train validity change,overallValid = %d, communicatingLocation = %d, configLocation = %d ",
                             state->getTrainId(), overallValid,  communicatingLocation, configLocation);
    ...
    void TimsDriverCallManager::processRequestForOccCallEvent( TA_IRS_Bus::RequestForOccCallEventPtr event )
    {
...

        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
						"Sending Client Update : OCCCallRequest for Driver Call");
		...
		LOG_GENERIC( SourceInfo,
			TA_Base_Core::DebugUtil::DebugInfo,
			"There is an incoming call from train [%d], origin [%d]",
			trainId, origin );

		//Create a OccCallCommandReceivedCommand
		TA_IRS_Bus::OccCallCommandReceivedCommandPtr command(
			new TA_IRS_Bus::OccCallCommandReceivedCommand(trainId,origin) );
    ...
    void TimsDriverCallManager::processEndOfOccCallEvent( TA_IRS_Bus::EndOfOccCallEventPtr event )
    {
        ...
        try
        {
            ...
            endRadioCallForSingleTrain( trainId,false, timestamp );
    ...
    void TimsDriverCallManager::processChangeoverStatusEvent( TA_IRS_Bus::ChangeoverStatusEventPtr event )
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"MPU changeover for train [%d], Status :[%d]",
                     trainId, static_cast<int>(event->getStatus()) );

		//DTL-2070 - Train Driver Call Issue
		//Connected Call should be ended, OCC call request no need to remove.
        // call endRadioCallForSingleTrain to do steps 1 and 2 above
        // endRadioCallForSingleTrain( trainId,true );
		
		//Check if the Status 8 event is due to ACSU Failure
		if (event->getStatus())
		{
			// get the train comms state for the train
			TrainCommsStatePtr commsState = m_statusManager.getTrainCommsState( event->getTrainId() );
			std::string primaryTsi;
			{
				// acquire the state lock
				TA_IRS_Bus::TrainStateLock lock( commsState );
			...
			//Get TSI or Resource ID
			unsigned long resourceId = boost::lexical_cast<unsigned long>(primaryTsi);

			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Train Id: %d, Resource ID : %d",
				static_cast<int>(trainId),resourceId);

			// using m_messageSender, send the TrainChangeover message
			TA_IRS_Bus::CallTypes::TrainChangeover trainChangeover;
			trainChangeover.trainId = trainId;
			trainChangeover.resourceId = resourceId;

			//Send message to the subscriber
			m_messageSender.sendClientUpdate(trainChangeover);
		}
...

        {
            ...
            if ( trainState->hasIncomingCall() )
            {
				...
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
							"Ending Radio Call Due to Failure (0 false : 1 true) : %d",
							bEndDueToFailure);

                // using m_messageSender, send the OCC Call reset message
                TA_IRS_Bus::CallTypes::OccCallReset request;
                request.trainId = trainId;
				request.bResetDueToFailure = bEndDueToFailure;
    ...
    void TimsDriverCallManager::performTimsCallReset( TA_IRS_Bus::CommonTypes::TrainIdType trainId )
    {
        ...
        try
        {
...

            {
                ...
                {
                    // clear the incoming call
					trainCallState->clearIncomingCall(timestamp);

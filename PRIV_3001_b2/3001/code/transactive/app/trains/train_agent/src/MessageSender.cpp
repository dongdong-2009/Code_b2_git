...
namespace TA_IRS_App
{
	...
	void MessageSender::sendStateUpdate( unsigned long trainId,unsigned char cameraId, unsigned long monitorId  )
	{
		FUNCTION_ENTRY( "sendStateUpdate( notify trian camera )" );
		try {
		...
		tvssCommand.videoInput[0]=cameraId;				
		...
		m_commsMessageSender->sendCommsMessage(
			TA_Base_Core::TVSSAgentComms::TrainBorneMonitorUpdate,m_configuration.getEntityKey(),
			cobTvssCommand,
			0, // not used
		...
		// Also logs the message that is sent out (full message details)
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
			"Sent message for pec active camera train id is %d, monitor id is %d, camera id is %d" , trainId, monitorId ,cameraId );
		}
		catch ( ... )
		{
    ...
    void MessageSender::sendClientUpdate( const TA_IRS_Bus::CallTypes::OccCallRequest& update )
    {
        ...
        // Also logs the message that is sent out (full message details)
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
			"Sent %s, Entitykey:%ul,Locationkey:%ul", update.getLogString().c_str(),
			m_configuration.getEntityKey(),m_configuration.getLocationKey() );
	...
	void MessageSender::sendClientUpdate( const TA_IRS_Bus::CallTypes::TrainChangeover& update )
	{
		FUNCTION_ENTRY( "sendClientUpdate( TrainChangeover )" );
		...
		// sends the appropriate type of message as per the Train Agent Message Types diagrams
		m_commsMessageSender->sendCommsMessage(
			TA_Base_Core::TrainAgentComms::TrainChangeover,
			m_configuration.getEntityKey(),
			data,
			0, // not used
			m_configuration.getLocationKey() );

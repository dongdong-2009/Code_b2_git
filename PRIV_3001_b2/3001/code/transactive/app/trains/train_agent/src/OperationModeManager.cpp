...
//		m_trainEventSubscriber->resubscribe();
	...
	void OperationModeManager::registerDutyReleaseUpdate (ISpecializedMsgProc * objObserver) 
	{
		
		std::set <ISpecializedMsgProc *>::iterator iterSpecMsgProc = m_setSpecializeMsgSubs.find(objObserver);

		if (iterSpecMsgProc == m_setSpecializeMsgSubs.end())
		{
			m_setSpecializeMsgSubs.insert(objObserver);
		}

	}
	
	void OperationModeManager::notifySubsForReleaseControl ()
	{
		
		std::set <ISpecializedMsgProc *>::iterator iterSpecMsgProc = m_setSpecializeMsgSubs.begin();

		for (; iterSpecMsgProc != m_setSpecializeMsgSubs.end(); iterSpecMsgProc++)
		{
			(*iterSpecMsgProc)->processReleaseOfCtrl();
		}

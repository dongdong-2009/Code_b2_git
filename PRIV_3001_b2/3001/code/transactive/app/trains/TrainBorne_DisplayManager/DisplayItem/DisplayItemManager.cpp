/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP/3001/transactive/app/cctv/video_display_manager/src/DisplayItemManager.cpp $
  * @author:  Adam Radics
  * @version: $Revision: #1 $
  *
...
#include "app/trains/TrainBorne_DisplayManager/DisplayItem/DisplayItemManager.h"
#include "app/trains/TrainBorne_DisplayManager/DisplayItem/BvsStage.h"
...
namespace TA_IRS_App
{
  ...
  //      }
	...
	std::string DisplayItemManager::getLocationNameStr()
	{
		return m_locationname;
	}

	unsigned long DisplayItemManager::getConsoleKey()
	{
		return m_consoleKey;
	}
    ...
    //    stageWorker.start();
        ...
        // create the barrier
        TA_Base_Core::SingleThreadBarrier barrier( /*5*/3 );
 ...
 /*       time_t startTime = time( NULL );
    ...
    void DisplayItemManager::loadQuadsFromDatabase()
    {
        ...
        try
        {
			...
			quadEntities = EntityAccessFactory::getInstance().getEntitiesOfType(TA_Base_Core::Quad::getStaticType());
			IEntityDataList::iterator it;
            ...
            for (it = quadEntities.begin(); it != quadEntities.end(); it++)
            {
				...
				std::string address = quadEntity->getAddress();
                std::string group = quadEntity->getVideoInputGroupName();
				std::string cameras = quadEntity->getIncludedCameras();
                delete quadEntity;

                // create the local object
                createQuad( key, name, desc, group,address,cameras );
            }
        }
        catch (const TA_Base_Core::DatabaseException& e)
        {
    ...
    void DisplayItemManager::loadSequencesFromDatabase()
    {
        ...
        try
        {
			...
			sequenceEntities = EntityAccessFactory::getInstance().getEntitiesOfType("TrainBorneSequence",false,false); // getEntitiesOfTypeAtLocation("TrainBorneSequence"/*TA_Base_Core::Sequence::getStaticType()*/, m_locationkey);
    ...
    void DisplayItemManager::loadMonitorsFromDatabase()
    {
        ...
        try
        {
			...
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"loadMonitorsFromDatabase location:%ld",m_locationkey);
			monitorEntities = EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation("VideoMonitor"/*TA_Base_Core::VideoMonitor::getStaticType()*/, m_locationkey);
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"Find %d monitors",monitorEntities.size());
			...
			//	TA_Base_Core::VideoMonitor* monitorEntity = (TA_Base_Core::VideoMonitor*)((void*)*it); 
                // get all details
				
                unsigned long key = (*it)->getKey();//monitorEntity->getKey();
				std::string name = (*it)->getName();//monitorEntity->getName();
				std::string desc = (*it)->getDescription();//monitorEntity->getDescription();
				std::string addr = (*it)->getAddress();
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"Monitor Key:%ld",key);
             ...
             //   delete monitorEntity;
				...
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"Load Monitor pkey:%ld,addr:%s,displayOrder:%d,consoleKey,%ld,alarmMonitor:%d",
							key,addr.c_str(),displayOrder,consolekey,bIsOverallMonitor);
            }
        }
        catch (const TA_Base_Core::DatabaseException& e)
        {
    ...
    {
        // create the local object
        Quad* quadPtr = new Quad( key, name, description, group,address,cameras);
    ...
    TrainBorneMonitor* DisplayItemManager::createMonitor( unsigned long key, 
                                                const std::string& name,
                                                const std::string& description,
    ...
    {
        // create the local object
        TrainBorneMonitor* monPtr = new TrainBorneMonitor( key, name, description,addr, displayOrder,consoleKey,bIsOverallMonitor );

        m_monitors[ name ] = monPtr;
        m_monitorsByPkey[ key ] = monPtr;
		
		if ( true == monPtr->isConsoleMonitor())
        {
	...
	void DisplayItemManager::subscribeToMessages()
    {
		...
		TA_Base_Core::MessageSubscriptionManager::getInstance().subscribeToCommsMessage( TA_Base_Core::TVSSAgentComms::TrainBorneMonitorUpdate, 
    ...
    TrainBorneMonitor* DisplayItemManager::getMonitor(std::string name)
    {
        // find and return the item in the map
        std::map<std::string, TrainBorneMonitor*>::iterator findIter = m_monitors.find(name);
    ...
    TrainBorneMonitor* DisplayItemManager::getMonitor(unsigned long pkey)
    {
        // find and return the item in the map
        std::map<unsigned long, TrainBorneMonitor*>::iterator findIter = m_monitorsByPkey.find(pkey);
	...
	std::map<unsigned long, Sequence*> DisplayItemManager::getAllSequencesByKeys()
    {
        return m_sequencesByPkey;
    }
    ...
    std::map<std::string, TrainBorneMonitor*> DisplayItemManager::getAllMonitors()
    {
        return m_monitors;
    }
    ...
    {
        for (std::map<std::string, TrainBorneMonitor*>::iterator monIter = m_monitors.begin();
    ...
    void DisplayItemManager::receiveSpecialisedMessage( const TA_Base_Core::CommsMessageCorbaDef& message )
    {
      ...
      /*  TA_Base_Bus::QuadCorbaDef::QuadState* quadState = 0;
        ...
        */
		...
		TA_Base_Bus::TVSSManagerCorbaDef::TVSSCommand* tvssCommand = 0;
		if( 0 !=( message.messageState >>= tvssCommand))
		{
			unsigned long consoleKey = message.assocEntityKey;
			m_callbackWindow->SendMessage(WM_TVSS_COMMAND,(WPARAM)tvssCommand,0l);
			return;
		}
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError,
            "Unrecognizable state update message received." );
    }
    ...
    {
        // get the monitor
		if(videoOutputState.isInAlarmMode || (!videoOutputState.isInAlarmMode && videoOutputState.isQuadInput) || (!videoOutputState.isInAlarmMode && !videoOutputState.isQuadInput && videoOutputState.currentVideoInputKey>0 ))
		{
			TA_Base_Bus::TVSSManagerCorbaDef::TVSSCommand cmd;
			
			TrainBorneMonitor* monitor = m_monitorsByPkey[outputKey];
			if(NULL==monitor)
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn,
					"processVideoOutputStateMessage.Unknown monitor Pkey:%lu",outputKey );
				return;
			}
			cmd.trainId = 0;
			cmd.monitorId = atoi(monitor->getAddress().c_str());
			cmd.cmdType = TA_Base_Bus::TVSSManagerCorbaDef::MappingMonitor;
			m_callbackWindow->SendMessage(WM_TVSS_COMMAND,(WPARAM)&cmd,0l);
		}
   ...
   /* void DisplayItemManager::processQuadStateMessage( unsigned long quadKey, const TA_Base_Bus::QuadCorbaDef::QuadState& quadState )
    ...
    void DisplayItemManager::notifyMonitorListeners(TrainBorneMonitor* modifiedItem)
    {
    ...
    void DisplayItemManager::notifySequenceListeners(Sequence* modifiedItem)
    {
        ...
        for (std::map<std::string, TrainBorneMonitor*>::iterator iter = m_monitors.begin();
    ...
    void DisplayItemManager::notifyGUIofDisplayItemUpdate(DisplayItem* itemThatChanged)
    {
        ...
        {
            TrainBorneMonitor* monitor = dynamic_cast<TrainBorneMonitor*>(output);
            if (monitor != NULL)
            {

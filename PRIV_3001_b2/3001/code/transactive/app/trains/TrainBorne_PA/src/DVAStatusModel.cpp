/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP/3001/transactive/app/trains/TrainBorne_PA/src/DVAStatusModel.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
...
{    
    ...
    catch (const TA_Base_Bus::IPAAgentCorbaDef::PaErrorException&)
    {
...
void DvaStatusModel::deleteStatusName(const std::string& str)
{
    ...
    int numRemovals = GenericStatusModel::terminateAndRemoveBroadcastIds(
                                                        broadcastIds, 
...
void DvaStatusModel::updateStatusNameArrayCache()
{
    try
    {
		...
		m_cachedStatusNames = GenericStatusModel::getDvaBroadcastZones(
                                                    CachedConfig::getInstance()->getAgentObject(),
                                                    broadcastFilter,
                                                    GenericStatusModel::IGNORE_RIGHTS_CHECKING);
    }
...
void DvaStatusModel::initializeStatusNameSubscriber(HWND hWnd)
{
    ...
    if (std::find(m_cachedStatusNames.begin(), m_cachedStatusNames.end(), str) == m_cachedStatusNames.end())
    {

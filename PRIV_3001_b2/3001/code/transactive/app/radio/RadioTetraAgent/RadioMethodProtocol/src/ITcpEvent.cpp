...
short ITcpEvent::getParameterShort(unsigned int position) const
{
	...
	return readBufferShort(position);
}
...
unsigned long ITcpEvent::getParameterLong(unsigned int position) const
{
	...
	return readBufferLong(position);
}
...
std::string ITcpEvent::getParameterShortString(unsigned int position, unsigned int eventLength) const
{
	FUNCTION_ENTRY("getParameterShortString");
	std::string result;

	for (unsigned int iLoop=0; iLoop<eventLength; iLoop++)
	{

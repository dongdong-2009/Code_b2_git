...
{
	const static int CALLBACK_MESSAGES_NUM = 152;
	CallBackMessage responses[CALLBACK_MESSAGES_NUM]=
	{
		/* MCDI messages */
	...
	MccRadioCommand::MccRadioCommand(MCC7500SDK::IAPIRequest_ptr &ptr,MCC7500SDK::METHOD_ID mid) : 
pAPIRequest(ptr), method_id(mid), clientid('\0')
{
}
MccRadioCommand::~MccRadioCommand()
{
}
void MccRadioCommand::execute()
{
	try
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "MCC7500 Client -> Server: Calling %s(%u)", getCommandName().c_str(), method_id);
		pAPIRequest->ApiRequest(method_id,in,out,result);
	}
	catch(...)
	{
		//char error[1024];
		//sprintf(error,"Exception while calling the method %s(%u)",getCommandName().c_str(),method_id);
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Exception while calling the method %s(%u)", getCommandName().c_str(), method_id);
	}
}

std::string MccRadioCommand::getCommandName()
{
	return command_name;
}
...
MccRadioEvent::MccRadioEvent(MCC7500SDK::Parameters p) : params(p), message_id(0)
{
	event_name = "";
	if(p.length()==0)
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Parameters are empty");
		throw TA_Base_Core::TransactiveException("Parameteres are empty");
	}
	message_id = p[0].V_UINT16();
}

MccRadioEvent::~MccRadioEvent()
{
}

void MccRadioEvent::printMessage()
{
}

unsigned short MccRadioEvent::getMessageType() 
{
	return message_id;
}

std::string MccRadioEvent::getEventName()
{
	return event_name;
}

char* MccProtocolHelper::getResponseName(unsigned short val)
{

...
#include "app/radio/RadioTetraAgent/RadioTcpCommunication/src/ObservedSocket.h"
...
//limin++, failover
	...
	void ObservedSocket::HexString(const std::string& data,std::string& output )
	{
	...
	void ObservedSocket::resetTerminate()
	{
		m_toTerminate = false;
	}
	...
	bool ObservedSocket::connect()
	{
		...
		close();
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Trying to connect to %s:%u",m_remoteAddress.get_host_addr(), m_remoteAddress.get_port_number());
	...
	{		
		...
		if(m_isConnected == true)
		{
			...
			TA_Base_Core::ThreadGuard guard(m_socketLock);
			int status = m_clientStream.send_n(writeBuffer.c_str(), writeBuffer.size(), &m_socketTimeout);
			...
			writeBuffer.erase( writeBuffer.begin(), writeBuffer.end() );
		}
	}
	...
	void ObservedSocket::getIncomingData()
	{
		if(m_isConnected == true) // already connected
		{
			...
			if (status > 0)
			{
				...
				std::string strHexData,strReceived;
				strReceived.append((char*)m_internalDataBuffer, bytes_transferred);
				size_t bytes_total=m_readBuffer.size()+bytes_transferred;
				m_readBuffer.reserve(bytes_total);
 				m_readBuffer.append(m_internalDataBuffer, bytes_transferred);
				HexString(strReceived,strHexData);
				// debugging
				if(bytes_transferred > 0)
				{
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
						"Socket %s:%s incoming %d bytes of data:[%s]",
								m_host.c_str(), m_service.c_str(), bytes_transferred,strHexData.c_str());
				}
			}

...
{
	FUNCTION_ENTRY("closeConnection");
	m_clientSocket->terminateAndWait();
	FUNCTION_EXIT;
}
...
void TCPConnection::Send(const ITcpMethod& method)
{
    ...
    if (DebugUtil::DebugDebug <= DebugUtil::getInstance().getLevel())
    {
        ...
        LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, 
			"Connection %s Data Size : %d bytes , Radio method data: %s", m_identifier.c_str(), dataSize,hexText.c_str());
        LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, 
...
void TCPConnection::processReceivedData( SocketData& data, ObservedSocket* socket )
{
	...
	m_event->setEventLength(length);

	// for non-fixed length profile_update event
	long profileUpdateLength = EVENT_LENGTH_BASE_PROFILE_UPDATE;
	long profileUpdateVariable = 0;
	if (length >= profileUpdateLength)
		profileUpdateVariable = data[EVENT_PROFILE_UPDATE_NUM_POSITION] * 0x0100 + data[EVENT_PROFILE_UPDATE_NUM_POSITION+1];
	profileUpdateLength += EVENT_LENGTH_VARIABLE_PROFILE_UPDATE * profileUpdateVariable;

	//check the length of the data bytes to ensure that it is correspond to the Radio Method 
	if( length==EVENT_LENGTH_PROFILE_REQUEST || length==EVENT_LENGTH_SUBSCRIBER_MONITORING || length==EVENT_LENGTH_DGNA_QUERY 
		|| length==EVENT_LENGTH_CALL_CANCEL_REQUEST || length==profileUpdateLength || length==EVENT_LENGTH_RADIO_SYSTEM_STATUS)
	{
		while (!data.empty())
		{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Reply Data Incorrect : Received %d bytes of data",length);

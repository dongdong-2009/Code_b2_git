 ...
 * in any form.
 *
 * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui_icdv4/src/RadioSelectedEntity/SelectedEntityDetails/CEntityDetailsAreaDlg.cpp $
...
#include "app/radio/radio_manager_gui_icdv4/src/RadioSelectedEntity/SelectedEntityDetails/EntityDetailsAreaStrategyFactory.h"
...
void CEntityDetailsAreaDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST, m_listLB);
...
void CEntityDetailsAreaDlg::OnEdit() 
{
	...
	if (The_RadioSystemStatus::instance()->getResourceCollectioForType(m_RadioResource.type).size() >= RadioSystemStatus::MAX_NUMBER_GROUP) {
...
HBRUSH CEntityDetailsAreaDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	FUNCTION_ENTRY("OnCtlColor");
	HBRUSH hbr; 

	bool changed = false;
	if (IDC_LIST == pWnd->GetDlgCtrlID()) {
		if (m_listLB.isReadOnly()) {
			pDC->SetBkColor(::GetSysColor(COLOR_3DFACE));  
			hbr = (HBRUSH) CreateSolidBrush(::GetSysColor(COLOR_3DFACE));
		} else {
			pDC->SetBkColor(::GetSysColor(COLOR_3DHIGHLIGHT));  
			hbr = (HBRUSH) CreateSolidBrush(::GetSysColor(COLOR_3DHIGHLIGHT));	
		}
		//m_listLB.RedrawWindow(NULL,NULL,RDW_INTERNALPAINT | RDW_ALLCHILDREN |RDW_UPDATENOW);
		//m_listLB.UpdateWindow();
	} else {

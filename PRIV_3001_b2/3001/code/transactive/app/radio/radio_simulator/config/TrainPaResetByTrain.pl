#!/usr/bin/perl
#
# This will generate a Train PA reset simulator case that can be loaded in the radio simulator
...
if ( ( $trainId == 0 ) ||
     ( $announcementId == 0 ) )
{
    print "Usage: ./TrainPaResetByTrain.pl --trainId=X --announcementId=X --result=Successful|OCC|PSC|Driver --file=<Output File Name>";
    exit;
}
...
Name = UC-PA-03.3 (/14) PA Reset by Train T:$trainIdDec Ann:$announcementIdDec
Data =
//
...
Resp = 07	0E	$trainIdHex	08	00	$announcementIdHex	****
RespSize = 281

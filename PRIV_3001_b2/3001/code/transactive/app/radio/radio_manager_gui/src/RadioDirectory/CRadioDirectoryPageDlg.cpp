 ...
 * in any form.
 *
 * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui/src/RadioDirectory/CRadioDirectoryPageDlg.cpp $
 * @author:  Glen Kidd
 * @version: $Revision: #2 $
 *
...
#include "app/radio/radio_manager_gui/src/RadioHelper.h"
...
//TD18869
	...
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_DIRECTORY_LIST, OnColumnClicked)
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
...
BOOL CRadioDirectoryPageDlg::OnInitDialog() 
{
	...
	columnAttr.sortBy = AutoSortListCtrl::ESortType::BY_CALLBACK;
	...
	m_sortDescOrder = AutoSortListCtrl::ECurrentSort::UNKNOWN;
	m_sortIdOrder = AutoSortListCtrl::ECurrentSort::UNKNOWN;
	m_columnSort = 0;

	m_directoryListCtrl.useCallback(0);
...
{	
	if (m_bDragging)
	{
		...
		{
			if (iPossibleWnd->IsKindOf(RUNTIME_CLASS(CListCtrl)))
			{
				if ((RR_TALKGROUP == m_selectedResource.type && RR_DYNGROUP == resTypeEditMode) || RR_NIL == resTypeEditMode) {
	...
	if (m_bDragging)
	{
		...
		if ( iPossibleWnd != NULL )
		{
			...
			if(iPossibleWnd->IsKindOf(RUNTIME_CLASS(CListCtrl)))
			{
				...
				if (MAX_MEMBERS < ((CSelectListCtrl *)iPossibleWnd)->GetCount()) {
				...
				int pos = ((CSelectListCtrl *)iPossibleWnd)->FindStringExact(-1,m_selectedResource.alias.c_str());
				...
				if ( pos == LB_ERR )
				{
					...
					{
						int index = ((CSelectListCtrl *)iPossibleWnd)->GetCount();
						
						((CSelectListCtrl *)iPossibleWnd)->InsertString(index,m_selectedResource.alias.c_str());	
						
						int x = m_directoryListCtrl.GetSelectionMark();
						//((CSelectListCtrl *)iPossibleWnd)->SetItemDataPtr(index,&m_ResourceTypeCollectionarray[x]);
						((CSelectListCtrl *)iPossibleWnd)->SetItemData(index,m_selectedResource.id);
					}
					else
					{
...
#else
				int curItemCount = ((CSelectListCtrl *)iPossibleWnd)->GetCount();
				...
				static TCHAR tBuf[50];
				{
					while(pos)
					{
						...
						int pos_listbox = ((CSelectListCtrl *)iPossibleWnd)->FindStringExact(-1,alias.c_str());
						//	could check if its already present...
						if ( pos_listbox == LB_ERR )
						{
							...
							if ( (m_selectedResource.type == RR_RADIO && m_tabStates[TALKGROUP_INDEX].enabled) || (m_selectedResource.type == RR_TALKGROUP && !m_tabStates[RADIO_INDEX].enabled))
							{
								...
								((CSelectListCtrl *)iPossibleWnd)->InsertItem(index,alias.c_str());	
								
								int x = m_directoryListCtrl.GetSelectionMark();
								//((CListBox *)iPossibleWnd)->SetItemDataPtr(index,&m_ResourceTypeCollectionarray[x]);
								((CSelectListCtrl *)iPossibleWnd)->SetItemData(index,atol(id.c_str()));
...
void CRadioDirectoryPageDlg::OnTabSelChange(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	m_directoryTabCtrl.RedrawWindow();
...
void CRadioDirectoryPageDlg::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (NULL != pNMListView && m_pRadioDirectoryPageDlgStrategy != NULL) 
	{
...
{
	int m_currentPage;	
	RadioResourceType type = (resource.type==RR_TRAIN) ? RR_RADIO : resource.type;
	

	for (int i = 0; i < RADIO_TABS_NUM; ++i)
	{
        if (type == m_tabStates[i].type)
        {
			m_currentPage = i;
			break;
        }
	...
	LOG_GENERIC (SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
				"Resource selected : %s", resource.alias.c_str());

	bool initialize = false;
	// refresh directory list
	if ( m_currentRadioResourceType != resource.type || m_pRadioDirectoryPageDlgStrategy == NULL)
	{
		// delete m_pRadioDirectoryPageDlgStrategy associated with the previously selected resourceType
		delete m_pRadioDirectoryPageDlgStrategy;
		m_pRadioDirectoryPageDlgStrategy = NULL;
		...
		// train call
		if (RR_TRAIN == resource.type) 
		{
			m_currentRadioResourceType = RR_RADIO;
			m_directoryListCtrl.setRadioResourceType(RR_RADIO);
		}
    ...
    if ((nIndex=m_directoryListCtrl.FindItem(&info)) != -1)
	{
		...
		if ( initialize ) 
		{
			nIndex = m_ResourceTypeCollectionarray.size() - nIndex - 1;
		}
		CSize z;
		int curPos = m_directoryListCtrl.GetScrollPos(SB_VERT) * 17;
		int pos = nIndex * 17;
		pos = pos - curPos;
		z.SetSize(0,pos);
		m_directoryListCtrl.Scroll(z);
	} 
	else if (RR_TRAIN == resource.type) 
	{
		
		int counter = 0;
		std::vector<RadioResource>::iterator iter = m_ResourceTypeCollectionarray.begin();
		for (; iter!=m_ResourceTypeCollectionarray.end(); iter++)
		{
			if ((*iter).id == resource.id) break;
			counter++;
		}
		if (iter != m_ResourceTypeCollectionarray.end()) 
		{
			CSize z;
			z.SetSize(0,counter*17); // size of one entry height
			m_directoryListCtrl.Scroll(z);
		}
	}
}
...
LRESULT CRadioDirectoryPageDlg::onNotifiedEntityEmptySelection(WPARAM pRadioResourceType, LPARAM unused)
{
	...
	if (RR_DYNGROUP == m_currentRadioResourceType || RR_PATCHGROUP == m_currentRadioResourceType || RR_MSELECT == m_currentRadioResourceType) {
		if (m_ResourceTypeCollectionarray.size() == 0 || 
			((((RadioResource)m_ResourceTypeCollectionarray.back()).id != 0) && ((*m_ResourceTypeCollectionarray.begin()).id != 0))) {
			if (m_ResourceTypeCollectionarray.size() >= RadioSystemStatus::MAX_NUMBER_GROUP) {
				FUNCTION_EXIT;
				return 0;
			}
			RadioResource r;
			r.initialise();
			r.type = m_currentRadioResourceType;
			m_ResourceTypeCollectionarray.push_back(r);
			setFilterListRowData();	
		}
		m_directoryListCtrl.SetItemState(m_ResourceTypeCollectionarray.size()-1, LVIS_SELECTED, LVIS_SELECTED);
	}
...
void CRadioDirectoryPageDlg::OnColumnClicked(NMHDR* pNMHDR, LRESULT* pResult)
{
	FUNCTION_ENTRY("OnColumnClicked");


	LPNMLISTVIEW pnmv = NULL;
	pnmv = (LPNMLISTVIEW) pNMHDR; 

	if (NULL != pnmv) {
		m_columnSort = pnmv->iSubItem;

		if (RR_TALKGROUP != m_currentRadioResourceType && RR_RADIO != m_currentRadioResourceType) {
			if (0 == m_columnSort) {
				if (AutoSortListCtrl::ECurrentSort::UNKNOWN == m_sortDescOrder) {
					m_sortDescOrder = AutoSortListCtrl::ECurrentSort::ASCENDING;
				} else if (AutoSortListCtrl::ECurrentSort::ASCENDING == m_sortDescOrder) {
					m_sortDescOrder = AutoSortListCtrl::ECurrentSort::DESCENDING;
				} else if (AutoSortListCtrl::ECurrentSort::DESCENDING == m_sortDescOrder) {
					m_sortDescOrder = AutoSortListCtrl::ECurrentSort::ASCENDING;
				}
			}
			return;
		...
		if (0 == m_columnSort) {
			...
			sortResources(m_columnSort, m_sortDescOrder);
		} else if (1 == m_columnSort) {
			if (AutoSortListCtrl::ECurrentSort::UNKNOWN == m_sortIdOrder) {
				m_sortIdOrder = AutoSortListCtrl::ECurrentSort::ASCENDING;
			} else if (AutoSortListCtrl::ECurrentSort::ASCENDING == m_sortIdOrder) {
				m_sortIdOrder = AutoSortListCtrl::ECurrentSort::DESCENDING;
			} else if (AutoSortListCtrl::ECurrentSort::DESCENDING == m_sortIdOrder) {
				m_sortIdOrder = AutoSortListCtrl::ECurrentSort::ASCENDING;
			}
			sortResources(m_columnSort, m_sortIdOrder);
		}
	}
...
void CRadioDirectoryPageDlg::sortResources(int column, AutoSortListCtrl::ECurrentSort sort)
{
	FUNCTION_ENTRY("sortResources");
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Sorting resource collection on directory list");
	if (0 == column) {
		if (AutoSortListCtrl::ECurrentSort::ASCENDING == sort || AutoSortListCtrl::ECurrentSort::UNKNOWN == sort) {
			std::sort(m_ResourceTypeCollectionarray.begin(), m_ResourceTypeCollectionarray.end(), SortAliasAsc());
		} else if (AutoSortListCtrl::ECurrentSort::DESCENDING == sort) {
			std::sort(m_ResourceTypeCollectionarray.begin(), m_ResourceTypeCollectionarray.end(), SortAliasDesc());
		}
	} else {
		if (AutoSortListCtrl::ECurrentSort::ASCENDING == sort || AutoSortListCtrl::ECurrentSort::UNKNOWN == sort) {
			std::sort(m_ResourceTypeCollectionarray.begin(), m_ResourceTypeCollectionarray.end(), SortIdAsc());
		} else if (AutoSortListCtrl::ECurrentSort::DESCENDING == sort) {
			std::sort(m_ResourceTypeCollectionarray.begin(), m_ResourceTypeCollectionarray.end(), SortIdDesc());
		}
	}
	//setFilterListRowData();
	FUNCTION_EXIT;
...
LRESULT CRadioDirectoryPageDlg::onNotifiedDbUpdate(WPARAM pOldResource, LPARAM pNewResource)
{
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "update resource alias = %s", oldRes->alias.c_str());
	if (m_currentRadioResourceType != oldRes->type) {
		FUNCTION_EXIT;
		return 0;
	}		
	std::vector<RadioResource>::iterator iter = m_ResourceTypeCollectionarray.begin();
...
LRESULT CRadioDirectoryPageDlg::onNotifiedDbDelete(WPARAM pOldResource, LPARAM lParam)
{
	FUNCTION_ENTRY("onNotifiedDbDelete");	
	RadioResource* res = (RadioResource*) pOldResource;
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "delete resource alias = %s", res->alias.c_str());
	if (m_currentRadioResourceType != res->type) {
		FUNCTION_EXIT;
		return 0;
	}
	std::vector<RadioResource>::iterator iter = m_ResourceTypeCollectionarray.begin();
	for (; iter!= m_ResourceTypeCollectionarray.end(); iter++) {
		if ((*iter).id == res->id) {
			m_ResourceTypeCollectionarray.erase(iter);
			break;
		}
	}
	setFilterListRowData();
...
LRESULT CRadioDirectoryPageDlg::onNotifiedDbNew(WPARAM pNewResource, LPARAM lParam)
{
	FUNCTION_ENTRY("onNotifiedDbNew");
	RadioResource* res = (RadioResource*) pNewResource;
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "add new resource alias = %s", res->alias.c_str());
	if (m_currentRadioResourceType != res->type) 
	{
		FUNCTION_EXIT;
		return 0;
	}
	m_ResourceTypeCollectionarray.push_back(*res);
	setFilterListRowData();
	FUNCTION_EXIT;
	return 0;
}

LRESULT CRadioDirectoryPageDlg::onNotifiedDbFinish(WPARAM wParam, LPARAM lParam)
{
	FUNCTION_ENTRY("onNotifiedDbFinish");			
	m_pRadioDirectoryPageDlgStrategy  = RadioDirectoryPageDlgStrategyFactory::getRadioDirectoryPageDlgStrategy(m_currentRadioResourceType);

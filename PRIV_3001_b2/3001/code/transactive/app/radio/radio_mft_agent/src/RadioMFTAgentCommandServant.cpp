 ...
 * in any form.
 *
 * Source:   $File: //depot/4677_ALL_TIP/4677/transactive/app/radio/radio_mft_agent/src/RadioMFTAgentCommandServant.cpp $
 * @author:  Glen Kidd
 * @version: $Revision: #1 $
 *
...
{
	// wong.peter find the correct resource first
	RadioResource r;
	RadioMFTAgentHelper::convertRadioResourceFromIDL(resource, r);
	std::string talkgroupAlias(resource.alias);
	std::vector<RadioResource> resources = m_pMFTAgentCommandImpl->getResourceCollection(RadioMFTAgentHelper::convertResourceType(resource.type));
	std::vector<RadioResource>::iterator iter = resources.begin();
	for (; iter!=resources.end(); iter++) {
		if ((*iter).alias == talkgroupAlias) {
			r = (*iter);
			break;
		}
	}
	if (iter == resources.end()) {
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "Assign and selecting resource with alias not known to Radio MFT Agent");
		r.id = 0;
	}
	
	try
	{
		m_pMFTAgentCommandImpl->broadcastTrain(r, IRadioMFTAgentCommand::CommandState::START);
	}
	catch(const TA_Base_Core::TransactiveException& e)
	{
		TA_IRS_Bus::RadioCommonExceptionsIDL::CommandFailureException ex;
	...
	{
		TA_IRS_Bus::RadioCommonExceptionsIDL::CommandFailureException ex;
		ex.reason = CORBA::string_dup("Unknown Exception occured in assignAndSelectResource");
...
{	
	// wong.peter find the correct resource first
	TA_IRS_Bus::Radio::RadioResource r = resource;
	std::string talkgroupAlias(resource.alias);
	std::vector<RadioResource> resources = m_pMFTAgentCommandImpl->getResourceCollection(RadioMFTAgentHelper::convertResourceType(resource.type));
	std::vector<RadioResource>::iterator iter = resources.begin();
	for (; iter!=resources.end(); iter++) {
	...
	CORBA::Short buttonRef)
{
	...
	try
	{
		...
		{
			case RR_MSELECT:
				m_pMFTAgentCommandImpl->assignMselMember(nResource);
				break;
			case RR_PATCHGROUP:
				m_pMFTAgentCommandImpl->assignPatchMember(nResource);
				break;
			default:
...
void RadioMFTAgentCommandServant::deassignResource(const char* session,const TA_IRS_Bus::Radio::RadioResource& resource)
{
	...
	try
	{
		...
		switch(nResource.type) {
			case RR_MSELECT:
				m_pMFTAgentCommandImpl->deassignMselMember(nResource);
				break;
			case RR_PATCHGROUP:
				m_pMFTAgentCommandImpl->deassignPatchMember(nResource);
				break;
			default:
	...
	try
	{
		...
		switch(nResource.type)
		{
			case RR_PATCHGROUP:
				...
				The_StatusManager::instance()->setActiveResource(nResource);
				break;
			case RR_MSELECT:
	...
	try
	{
		...
		switch(nResource.type)
		{
			case RR_PATCHGROUP:
				...
				The_StatusManager::instance()->setActiveResource(emptyR);
				break;
			case RR_MSELECT:
...
void RadioMFTAgentCommandServant::makePrivateCall(const char* session,const TA_IRS_Bus::Radio::RadioResource& resource,CORBA::Boolean bEmergencyMode)
{
	...
	try
	{
		...
		if (nResource.type == RR_TRAIN && nResource.id != nResource.ssi) {
			nResource.id = nResource.ssi;
			nResource.type = RR_RADIO;
		}
		{
			...
			if (The_StatusManager::instance()->isPrivateCallResourceSelected()) 
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Private Call Resource is already selected. Directly start the calling, channel = %u", channel);
				m_pMFTAgentCommandImpl->makePrivateCall(nResource, IRadioMFTAgentCommand::CommandState::ACTION);
			} 
			else 
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Private Call Resource is not selected. Select first. Channel = %u", channel);
				m_pMFTAgentCommandImpl->makePrivateCall(nResource, IRadioMFTAgentCommand::CommandState::START);
			}
		}
...
void RadioMFTAgentCommandServant::answerPrivateCall(const char* session,const TA_IRS_Bus::Radio::RadioResource& resource)
{
	...
	try
	{
		...
		if (The_StatusManager::instance()->isPrivateCallResourceSelected()) 
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Private Call Resource is already selected. Can answer private call now, channel = %u", channel);
			m_pMFTAgentCommandImpl->answerPrivateCall(nResource, IRadioMFTAgentCommand::CommandState::ACTION);
		} 
		else 
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Private Call Resource is not selected. Select First. Channel = %u", channel);
			m_pMFTAgentCommandImpl->answerPrivateCall(nResource, IRadioMFTAgentCommand::CommandState::START);
		}
...
// ++ Huangxin TD14319 
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "EndCall function");
			
			if (nResource.type == RR_TRAIN && nResource.id != nResource.ssi) 
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PEC Call end");
				nResource.id = nResource.ssi;
				nResource.type = RR_RADIO;
			}
...
void RadioMFTAgentCommandServant::getResourceByAlias(const char* alias, TA_IRS_Bus::Radio::ERadioResourceType type, TA_IRS_Bus::Radio::RadioResource_out resource)
{
    FUNCTION_ENTRY("getResourceByID");
	...
	try
	{
		...
		std::vector<RadioResource> resources = m_pMFTAgentCommandImpl->getResourceCollection(RadioMFTAgentHelper::convertResourceType(type));
		std::vector<RadioResource>::iterator iter = resources.begin();
		std::string tgName(alias);
		for (; iter!=resources.end(); iter++) {
			if (tgName.compare((*iter).alias) == 0) {
				res = (*iter);
				break;
			}
		}
...
void RadioMFTAgentCommandServant::restartConnection()
{
	m_pMFTAgentCommandImpl->restartRemoteApiConnection();
}

void RadioMFTAgentCommandServant::getRadioResourceCount(const ::TA_IRS_Bus::Radio::RadioResource& resource, ::CORBA::Short& count)
{
	FUNCTION_ENTRY("getRadioResourceCount");

	RadioResource nresource;
	RadioMFTAgentHelper::convertRadioResourceFromIDL(resource, nresource);

	count = The_StatusManager::instance()->getResourceCounter(nresource);
	FUNCTION_EXIT;
}

void RadioMFTAgentCommandServant::updateResource(const ::TA_IRS_Bus::Radio::RadioResource& oldResource, const ::TA_IRS_Bus::Radio::RadioResource& newResource)
{
	FUNCTION_ENTRY("updateResource");
	RadioResource noldResource, nnewResource;

	RadioMFTAgentHelper::convertRadioResourceFromIDL(oldResource, noldResource);
	RadioMFTAgentHelper::convertRadioResourceFromIDL(newResource, nnewResource);

	The_StatusManager::instance()->updateResource(noldResource, nnewResource);
	FUNCTION_EXIT;
}

void RadioMFTAgentCommandServant::addNewResource(const ::TA_IRS_Bus::Radio::RadioResource& newResource)
{
	FUNCTION_ENTRY("addNewResource");
	RadioResource resource;

	RadioMFTAgentHelper::convertRadioResourceFromIDL(newResource, resource);

	The_StatusManager::instance()->addNewResource(resource);
	FUNCTION_EXIT;
}

void RadioMFTAgentCommandServant::deleteResource(const ::TA_IRS_Bus::Radio::RadioResource& oldResource)
{
	FUNCTION_ENTRY("deleteResource");
	RadioResource resource;

	RadioMFTAgentHelper::convertRadioResourceFromIDL(oldResource, resource);

	The_StatusManager::instance()->deleteResource(resource);
	FUNCTION_EXIT;
}

void RadioMFTAgentCommandServant::setPrivateCallState(const ::TA_IRS_Bus::Radio::RadioResource& resource, ::TA_IRS_Bus::Radio::EPrivateCallState callState)
{
	FUNCTION_ENTRY("isPrivateCallResourceInUse");
	...
	The_StatusManager::instance()->setFeatureAttributePtCallState(nResource,(PrivateCallState)callState);

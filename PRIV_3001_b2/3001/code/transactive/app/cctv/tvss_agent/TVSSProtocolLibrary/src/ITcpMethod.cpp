...
ITcpMethod::ITcpMethod(unsigned long methodLength, unsigned long methodHeaderLength,
		    ...
		    : m_messageBuffer(0),
			  ...
			  m_messageHeaderMessageLengthStartPostion(methodHeaderLengthStartPosition)
{
	...
	setByte(m_messageHeaderMessageLengthStartPostion,m_messageSize);
}
...
{
	TA_ASSERT(newsize >= getMethodHeaderLength(), "New message length can not shorter than message header");
	...
	// Reset message length field value
	setByte(m_messageHeaderMessageLengthStartPostion, m_messageSize);
}

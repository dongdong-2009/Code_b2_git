...
// Added gradient-Dockbar (CMenuDockBar) for office 2003 style
...
public:
  ...
  CMenuLoadLib(LPCTSTR pName,LPCSTR pProgName)
  {
...
{
  static CMenuLoadLib menuInfo(_T("user32.dll"),"GetMenuInfo");
  if(menuInfo.m_pProg)
  {
...
{
  static CMenuLoadLib menuInfo(_T("user32.dll"),"SetMenuInfo");
  if(menuInfo.m_pProg)
  {
...
class CMenuBrushList : public CObList
{
public:
  CMenuBrushList(){}
  ~CMenuBrushList()
...
class CMenuBrush : public CBrush
{
public:
  UINT m_nMenuDrawMode;
  COLORREF m_BarColor;

  CMenuBrush(UINT menuDrawMode, COLORREF barColor):m_nMenuDrawMode(menuDrawMode),m_BarColor(barColor)
  {
...
CBrush* GetMenuBarBrush()
{
  ...
  static CMenuBrushList brushList;
  static CMenuBrush* lastBrush = NULL;
  ...
  while (pos)
  {
    lastBrush = (CMenuBrush*)brushList.GetNext(pos);
    if(lastBrush!=NULL && 
  ...
  brushList.AddHead(lastBrush = new CMenuBrush(nMenuDrawMode,menuBarColor));
  return lastBrush;
}
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  CMenuMemDC memDC(&lpDIS->rcItem,lpDIS->hDC);
  CDC* pDC;
...
BEGIN_MESSAGE_MAP(CMenuDialog, CMenuFrame<CDialog>)
...
CMenuDialog::CMenuDialog(LPCTSTR lpszTemplateName, CWnd* pParentWnd)
{
...
CMenuDialog::CMenuDialog(UINT nIDTemplate, CWnd* pParentWnd)
{
  m_pParentWnd = pParentWnd;
...
void CMenuDialog::OnInitMenuPopup(CMenu* pMenu, UINT nIndex, BOOL bSysMenu)
{
  ...
  CMenuFrame<CDialog>::OnInitMenuPopup(pMenu,nIndex,bSysMenu);
}
...
IMPLEMENT_DYNCREATE(CMenuMiniFrameWnd,CMiniFrameWnd)
IMPLEMENT_DYNCREATE(CMenuMDIChildWnd,CMDIChildWnd)
IMPLEMENT_DYNCREATE(CMenuMiniDockFrameWnd,CMiniDockFrameWnd);

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

IMPLEMENT_DYNCREATE(CMenuFrameWnd,CFrameWnd)

#if _MFC_VER < 0x0700

#include <../src/afximpl.h> 

BOOL CMenuFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
...
BOOL CMenuFrameWnd::Create(LPCTSTR lpszClassName,
                          ...
                          CCreateContext* pContext)
{
  if (lpszMenuName != NULL)
  {
    ...
    {
#if _MFC_VER < 0x0700 
      TRACE0("Warning: failed to load menu for CMenuFrameWnd.\n");
#else
      TRACE(traceAppMsg, 0, "Warning: failed to load menu for CMenuFrameWnd.\n");
#endif
  ...
  {
#if _MFC_VER < 0x0700 
    TRACE0("Warning: failed to create CMenuFrameWnd.\n");
#else
    TRACE(traceAppMsg, 0, "Warning: failed to create CMenuFrameWnd.\n");
#endif
...
void CMenuFrameWnd::EnableDocking(DWORD dwDockStyle)
{
...
IMPLEMENT_DYNAMIC(CMenuMDIFrameWnd,CMDIFrameWnd)

BOOL CMenuMDIFrameWnd::ShowMenu(BOOL bShow)
{
...
BOOL CMenuMDIFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
                                CWnd* pParentWnd, CCreateContext* pContext)
{
  // only do this once
  ASSERT_VALID_IDR(nIDResource);
...
BOOL CMenuMDIFrameWnd::Create( LPCTSTR lpszClassName,
                              LPCTSTR lpszWindowName,
                              DWORD dwStyle,
                              const RECT& rect,
                              CWnd* pParentWnd,
                              ...
                              CCreateContext* pContext)
{
  if (lpszMenuName != NULL)
  {
    ...
    {
#if _MFC_VER < 0x0700 
      TRACE0("Warning: failed to load menu for CMenuMDIFrameWnd.\n");
#else
      TRACE(traceAppMsg, 0, "Warning: failed to load menu for CMenuMDIFrameWnd.\n");
#endif
      PostNcDestroy();            // perhaps delete the C++ object
      return FALSE;
    }
  ...
  {
#if _MFC_VER < 0x0700 
    TRACE0("Warning: failed to create CMenuMDIFrameWnd.\n");
#else
    TRACE(traceAppMsg, 0, "Warning: failed to create CMenuMDIFrameWnd.\n");
#endif
...
// this order can be changed by calling EnableDocking repetitively
// with the exact order of priority
void CMenuMDIFrameWnd::EnableDocking(DWORD dwDockStyle)
{
  // must be CBRS_ALIGN_XXX or CBRS_FLOAT_MULTI only
  ASSERT((dwDockStyle & ~(CBRS_ALIGN_ANY|CBRS_FLOAT_MULTI)) == 0);
...
IMPLEMENT_DYNAMIC(CMenuMultiDocTemplate,CMultiDocTemplate)

CMenuMultiDocTemplate::CMenuMultiDocTemplate(UINT nIDResource, 
...
CMenuMultiDocTemplate::~CMenuMultiDocTemplate()
{
...
IMPLEMENT_DYNCREATE(CMenuDockBar, CDockBar)

CMenuDockBar::CMenuDockBar(BOOL bFloating):CDockBar(bFloating)
{
}

CMenuDockBar::~CMenuDockBar()
{
}

BEGIN_MESSAGE_MAP(CMenuDockBar, CDockBar)
  ON_WM_ERASEBKGND()
  ON_WM_NCPAINT()
END_MESSAGE_MAP()


// CMenuDockBar diagnostics

#ifdef _DEBUG
void CMenuDockBar::AssertValid() const
{
  CDockBar::AssertValid();
}

void CMenuDockBar::Dump(CDumpContext& dc) const
{
  CDockBar::Dump(dc);
}
#endif //_DEBUG
...
// CMenuDockBar message handlers
void CMenuDockBar::EraseNonClient()
{
...
BOOL CMenuDockBar::OnEraseBkgnd(CDC* pDC)
{
  MENUINFO menuInfo = {0};
  menuInfo.cbSize = sizeof(menuInfo);
  menuInfo.fMask = MIM_BACKGROUND;

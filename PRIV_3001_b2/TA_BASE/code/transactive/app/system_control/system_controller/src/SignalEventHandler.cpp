/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/app/system_control/system_controller/src/SignalEventHandler.cpp $
  * @author:  Justin Ebedes
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2018/03/26 10:11:03 $
...
namespace TA_Base_App
{
    ...
    SignalEventHandler::SignalEventHandler(SystemController* systemController)
    {
        ...
        struct WindowsSignalHandler : ACE_Event_Handler
        {
            ...
            {
                static std::map<int, std::string> map = boost::assign::map_list_of(SIGINT, "SIGINT")(SIGBREAK, "SIGBREAK");
                LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "Received %s(%d) signal", map[signum].c_str(), signum);
                SignalEventHandler::s_systemController->shutDownRequested();
    ...
    void SignalEventHandler::handleSignal(int sigNum)
    {
        ...
        if (signal(sigNum, handleSignal) == SIG_ERR)
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "Could not instantiate signal %d Handler", sigNum);
        }
    ...
    void SignalEventHandler::handleSignal(int sigNum, int pid, int errorCode)
    {
        ...
        case SIGTERM:
        {
            ...
            //s_systemController->shutDownRequested();
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                        "Received SIGTERM signal from pid: %d, code: %d, reason: %s. ignored.",
                        pid, errorCode, convertCodeToString(errorCode).c_str());
        }
            // limin--

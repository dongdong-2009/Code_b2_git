/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/app/system_control/system_controller/src/ManagedProcessInfo.cpp $
...
#ifdef _MSC_VER
    #pragma warning(disable:4819 4786 4996)
#endif
...
namespace TA_Base_App
{
          ...
          m_isGroupStartup(false)
    {
        ...
        m_terminateTimeout = m_terminateTimeout ? m_terminateTimeout : DEFAULT_TERMINATE_TIMEOUT;
        m_startupTimeout = processData->getStartupTimeout() * 1000;
        m_startupTimeout = m_startupTimeout ? m_startupTimeout : DEFAULT_STARTUP_TIMEOUT;
        m_changeControlTimeout = processData->getChangeControlTimeout() * 1000;
        m_changeControlTimeout = m_changeControlTimeout ? m_changeControlTimeout : DEFAULT_CHANGE_CONTROL_TIMEOUT;
        m_changeMonitorTimeout = processData->getChangeMonitorTimeout() * 1000;
        m_changeMonitorTimeout = m_changeMonitorTimeout ? m_changeMonitorTimeout : DEFAULT_CHANGE_MONITOR_TIMEOUT;
        m_debugLevel = TA_Base_Core::RunParams::getInstance().get(RPARAM_DEBUGLEVEL);
        m_dbConnectionFile = makeDbConnectionFile(m_locationName);
        m_startLag = TA_Base_Core::getRunParamValue(childEntityName + "StartLag", m_startLag);

        m_startupTimeout = TA_Base_Core::getRunParamValue("StartTimeout", m_startupTimeout);
        m_changeControlTimeout = TA_Base_Core::getRunParamValue("ControlTimeout", m_changeControlTimeout);
        m_changeMonitorTimeout = TA_Base_Core::getRunParamValue("MonitorTimeout", m_changeMonitorTimeout);
        m_terminateTimeout = TA_Base_Core::getRunParamValue("TerminateTimeout", m_terminateTimeout);

        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "%s will auto failback: %s", entityName.c_str(), m_shouldAutoFailback ? "true" : "false");
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "%s is a service process: %s", entityName.c_str(), m_isServiceProcess ? "true" : "false");
    }
    ...
    void ManagedProcessInfo::notifyGroupOffline(const std::string& group)
    {
                    ...
                    if (true == m_isStandbyProcess)
                    {
                        if (m_monitorGroupName == group)
                        {
                            if (m_peerState == RunningInControl || m_peerState == StartingInControl)
                            {
                                ...
                                {
                                    // auto failback is replaced by handleDoubleControlBaseOnCentralDb
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling notifyGroupOffline: %s") % CorbaUtil::exceptionToString(e)));
    ...
    void ManagedProcessInfo::notifyGroupOnline(const std::string& group)
    {
                ...
                if (!CORBA::is_nil(m_managedAgent))
                {
                    if (true == m_isStandbyProcess)
                    {
                        if (m_monitorGroupName == group)
                        {
                            ...
                            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "GroupOnline, StandbyProcess [EntityName: %s] [OperationMode: %d]", this->getEntityName().c_str(), m_normalOperationMode);
                        }
                    }
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling notifyGroupOnline: %s") % CorbaUtil::exceptionToString(e)));
    ...
    void ManagedProcessInfo::invalidateNotifyHosts(const std::vector<std::string>& invalidNotifyHosts)
    {
        ...
        try
        {
            if (isRunning())
            {
                ...
                strm << "Informing " << getEntityName() << " that the following NotifyHosts are invalid: ";

                for (unsigned int i = 0; i < invalidNotifyHosts.size(); i++)
                {
                    result[i] = invalidNotifyHosts[i].c_str();
                    strm << invalidNotifyHosts[i] << "";
                }

                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "%s", strm.str().c_str());

                TA_Base_Core::ThreadGuard guard(m_lock); // yanrong++: CL-21237
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling invalidateNotifyHosts: %s") % CorbaUtil::exceptionToString(e)));
    ...
    TA_Base_Core::EOperationMode ManagedProcessInfo::getOperationMode()
    {
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling getOperationMode: %s") % CorbaUtil::exceptionToString(e)));
    ...
    //            TA_Base_Core::Hostname::getHostname());
    ...
    bool ManagedProcessInfo::waitForStartLag()
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "%s is waiting for start lag %d ms", m_entityName.c_str(), m_startLag);

        static std::set<EProcessStatus> terminatingStatusSet = boost::assign::list_of(Terminating)(Stopped)(NotRunning);
        std::set<EProcessStatus> intersection;

        for (int i = 0, SLEEP_TIME = 100; i < m_startLag; i += SLEEP_TIME)
        {
            std::set<EProcessStatus> status = boost::assign::list_of(getProcessStatus())(getStatusToChangeTo());
            std::set_intersection(status.begin(), status.end(), terminatingStatusSet.begin(), terminatingStatusSet.end(), std::inserter(intersection, intersection.begin()));

            if (intersection.size())
            {
                // We've been told to terminate. Stop sleeping.
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Start lag for %s interrupted", m_entityName.c_str());
                return false;
            }

            if (!m_shouldWaitStartLag)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Start lag for %s interrupted deliberately.", m_entityName.c_str());
                return true;
            }
    ...
    void ManagedProcessInfo::setParam(TA_Base_Core::RunParam param)
    {
        try
        {
            if (isRunning())
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Setting following RunParam: (%s, %s)", param.name.in(), param.value.in());
                // yanrong++: CL-21237
                TA_Base_Core::ThreadGuard guard(m_lock);
            ...
            if (strcmp(param.name.in(), RPARAM_DEBUGLEVEL) == 0)
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Debug level is now %s", m_debugLevel.c_str());
            }
        }
        catch (CORBA::SystemException& e)
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling setParam: %s") % CorbaUtil::exceptionToString(e)));
    ...
    void ManagedProcessInfo::getProcessData(TA_Base_Core::ProcessData& data)
    {
        data.ProcessId = getEntityName().c_str();
        data.started = getTimeStarted();
    ...
    bool ManagedProcessInfo::isAPeer(const std::string& hostname)
    {
        ...
        for (std::vector<std::string>::const_iterator iter = m_peerHosts.begin(); iter != m_peerHosts.end(); iter++)
        {
            if ((*iter) == hostname)
            {
    ...
    void ManagedProcessInfo::run()
    {
        ...
        m_isRunning = true;
        TA_Base_Core::EProcessStatus lastProcessStatus = static_cast<TA_Base_Core::EProcessStatus>(-1);
        TA_Base_Core::EProcessStatus lastStatusToChangeTo = static_cast<TA_Base_Core::EProcessStatus>(-1);
        ...
        while (m_isRunning)
        {
            ...
            if (processStatus != lastProcessStatus || statusToChangeTo != lastStatusToChangeTo)
            {
                ...
                lastStatusToChangeTo = statusToChangeTo;
            }
            ...
            {
                for (int i = 0, STEP = 100; m_isRunning && i < ProcessManager::SLEEP_TIME_MSECS; i += STEP)
                {
                    sleep(STEP);
                }
            }
    ...
    TA_Base_Core::RunParams::ParamVector ManagedProcessInfo::getParams()
    {
        ...
        using namespace TA_Base_Core;
        RunParams::ParamVector params;

        boost::filesystem::path filename = boost::filesystem::system_complete(ProcessManager::getInstance().getDebugLogDir());
        filename /= str(boost::format("%s.%s.%d") % m_childEntityName % getDate() % getNumRestarts());
        params.push_back(RunParams::ParamNameValue(RPARAM_DEBUGFILE, filename.string()));

        params.push_back(RunParams::ParamNameValue(RPARAM_DEBUGLEVEL, getDebugLevel()));
        params.push_back(RunParams::ParamNameValue(RPARAM_STATION_DEPLOY_KEY, RunParams::getInstance().get(RPARAM_LOCATIONKEY)));
        ...
        {
            value = RPARAM_CONTROL;
        }
        else if (mode == TA_Base_Core::Monitor)
        {
            value = RPARAM_MONITOR;
        }
        else if (mode == TA_Base_Core::Standby)
        {
            value = RPARAM_STANDBY;
        }

        params.push_back(RunParams::ParamNameValue(RPARAM_OPERATIONMODE, value));
    ...
    void ManagedProcessInfo::processTerminating(TA_Base_Core::EManagedProcessTerminateCode reason)
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Process %s has instructed the System Controller that it is terminating.", getEntityName().c_str());
    ...
    // changeState
    //
    void ManagedProcessInfo::changeState(TA_Base_Core::EProcessStatus currentStatus, TA_Base_Core::EProcessStatus newStatus)
    {
        FUNCTION_ENTRY("changeState");
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "%s About to be told to change from state %d to state %d", getEntityName().c_str(), currentStatus, newStatus);
    ...
    std::vector<std::string> ManagedProcessInfo::buildCommandLine(std::string& startPath)
    {
        ...
        args.push_back(str(boost::format("--%s=%s") % RPARAM_ENTITYNAME % m_childEntityName));
        args.push_back(str(boost::format("--%s=%d") % RPARAM_MGRPORT % TA_Base_Core::getRunParamValue(RPARAM_MGRPORT, TA_Base_Core::DEFAULT_MGR_PORT)));
        args.push_back(str(boost::format("--%s=%s") % RPARAM_DEBUGLEVEL % getDebugLevel()));
        args.push_back(str(boost::format("--%s=%s") % RPARAM_CHARSET % RunParams::getInstance().get(RPARAM_CHARSET)));
        args.push_back(str(boost::format("--%s=%s") % RPARAM_DBCONNECTIONFILE % m_dbConnectionFile));

        boost::filesystem::path debugFile = boost::filesystem::system_complete(ProcessManager::getInstance().getDebugLogDir());
        debugFile /= str(boost::format("%s.%s.%d") % m_childEntityName % getDate() % getNumRestarts());
        args.push_back(str(boost::format("--%s=%s") % RPARAM_DEBUGFILE % debugFile.string()));

        DatabaseStatusList databaseStatusList = ProcessManager::getInstance().getDatabaseStatus();

        BOOST_FOREACH(DatabaseStatusPair& dbstatus, databaseStatusList)
        {
            args.push_back(str(boost::format("--%s=%s") % dbstatus.first % dbstatus.second));
        }

        TA_Base_Core::RunParamSeq& params = ProcessManager::getInstance().getLocationParams(m_locationKey);

        for (size_t i = 0; i < params.length(); ++i)
        {
            std::string param = str(boost::format("--%s=%s") % params[i].name.in() % params[i].value.in());
        ...
        if (isServiceProcess())
        {
            ...
            args.push_back(str(boost::format("--notify-port=%d") % m_notifyPort));
        }
    ...
    void ManagedProcessInfo::performStart()
    {
...
#ifdef WIN32
        IWorkItemPtr item(new SimpleWorkItem(boost::bind(&ManagedProcessInfo::waitForProcessTerminate, this, pid)));
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Started process %s with pid %lu",  getEntityName().c_str(), pid);
        ...
        for (int i = 0; i < nLoops; i++)
        {
            ...
            if (hasBeenSignalled())
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "%s with pid %lu Process was stopped or died while starting so it will be stopped", getEntityName().c_str(), pid);
                FUNCTION_EXIT;
                return;
            }
            ...
            if (stateReportedSuccessfully)
            {
                if (getProcessStatus() == TA_Base_Core::RunningStandby && true == m_isStandbyProcess)
                {
                    ...
                    {
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Standby Process %s successfully started in Control or Monitor mode", getEntityName().c_str());
                        ...
                        {
                            // auto failback is replaced by handleDoubleControlBaseOnCentralDb
                            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Will not failing back %s", getEntityName().c_str());
                            setStatusToChangeTo(TA_Base_Core::RunningMonitor);
                        }
                    ...
                    {
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Process %s successfully started in Standby mode", getEntityName().c_str());

                        if (m_peerState == RunningInControl || m_peerState == StartingInControl)
                        {
                            if (m_shouldAutoFailback && getNormalOperationMode() == TA_Base_Core::Control)
                            {
                                // (NOT) auto failback is replaced by handleDoubleControlBaseOnCentralDb
                                //setStatusToChangeTo(TA_Base_Core::RunningControl);
                                setStatusToChangeTo(TA_Base_Core::RunningMonitor);
                            }
                            else
                ...
                {
                    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Process %s successfully started in Control mode", getEntityName().c_str());
                ...
                {
                    {
                        ...
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Process %s successfully started in Monitor mode", getEntityName().c_str());

                        // We have started in monitor mode so now see if we have to failback
                        if (m_shouldAutoFailback && getNormalOperationMode() == TA_Base_Core::Control) // auto failback is replaced by handleDoubleControlBaseOnCentralDb
                        {
                        ...
                        else if (m_peerState != RunningInControl)
                        {
                            ...
                            // Switch to Control.
                            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Peer is no longer in Control. Starting %s in Control", getEntityName().c_str());
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "The process %s did not complete startup in time (%d). It will be terminated", getEntityName().c_str(), m_startupTimeout);
    ...
    void ManagedProcessInfo::performPoll()
    {
        ...
        while (attemptNum < NUM_ATTEMPTS)
        {
            ...
            for (int i = 0; i < nLoops; i++)
            {
                ...
                else if (hasBeenSignalled() &&
                         (RequestedTerminate == getTerminateStatus() || UserExit == getTerminateStatus()))
                {
                    return;
                }
            }
            ...
            catch (TA_Base_Core::IManagedProcessCorbaDef::PollException&)
            {
                ...
                LOG_EXCEPTION_CATCH(SourceInfo, "PollException", "Error polling agent. The agent detected an internal problem and threw a PollException");
            }
            catch (CORBA::SystemException& e)
            {
                // The poll failed.
                LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling poll: %s") % CorbaUtil::exceptionToString(e)));
            }
        }

        if (hasBeenSignalled() && (RequestedTerminate == getTerminateStatus() || UserExit == getTerminateStatus()))
        {
            return;
        }
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Cannot poll process %s. Will terminate it and then restart it.", getEntityName().c_str());
    ...
    void ManagedProcessInfo::performChangeToControl()
    {
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error asking the agent to change operation mode: %s") % CorbaUtil::exceptionToString(e)));
        ...
        // We should now indicate that the process should be killed
        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "The process %s did not complete changing to Control in time (%d). It will be terminated", getEntityName().c_str(), m_changeControlTimeout);
    ...
    void ManagedProcessInfo::performChangeToMonitor()
    {
        ...
        // The process has not changed successfully in the allocated time.
        // We should now indicate that the process should be killed
        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "The process %s did not complete changing to Monitor in time (%d). It will be terminated", getEntityName().c_str(), m_changeMonitorTimeout);
    ...
    void ManagedProcessInfo::performStop(TA_Base_Core::EProcessStatus statusToChangeTo)
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "performStop() for %s with statusToChangeTo=%d", getEntityName().c_str(), statusToChangeTo);
        ...
        if (getProcessId() != 0)
        {
            if (statusToChangeTo != TA_Base_Core::NotRunning && statusToChangeTo != TA_Base_Core::Terminating)
            {
                ...
                // isn't going to work).
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Calling CORBA terminate() to %s", getEntityName().c_str());
                terminateProcess();
                ...
                for (int i = 0; i < nLoops; i++)
                {
                    ...
                    else if (getProcessStatus() == TA_Base_Core::Stopped)
                    {
                        ProcessManager::getInstance().notifyPeers(getEntityName());
                        return;
                    }
            ...
            if (statusToChangeTo != TA_Base_Core::NotRunning)
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Sending SIGTERM to process %lu (%s)", getProcessId(), getEntityName().c_str());

#ifdef WIN32
                isTerminated = ProcessUtilities::terminateProcess(getProcessId());

                if (!isTerminated)
                {
                    LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "call terminateProcess failed, just sleep a while %d (killTimeout)", getKillTimeout());
            ...
            if (!isTerminated)
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "performStop Sending SIGKILL to process %lu (%s)", getProcessId(), getEntityName().c_str());
        ...
        if (!isTerminated)
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "performStop() failed to process %lu (%s)", getProcessId(), getEntityName().c_str());
        }
    ...
    void ManagedProcessInfo::terminateProcess()
    {
        ...
        try
        {
            ...
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Cannot terminate process as status is %d", getProcessStatus());
            }
        }
        catch (CORBA::SystemException& e)
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", str(boost::format("Error calling terminateProcess: %s") % CorbaUtil::exceptionToString(e)));
        }
    }
    ...
    std::string ManagedProcessInfo::getDate() const
    {
        ...
        ACE_OS::strftime(buf, 100, "%d.%m.%Y", &lt);
        return buf;
    }
        ...
        for (std::map<std::string, TA_Base_Core::ISystemControllerAdminCorbaDef_var>::iterator it = m_systemControllers.begin(); it != m_systemControllers.end(); ++it)
        {
            if (!CORBA::is_nil(it->second))
            {
                try
                {
                    ...
                    it->second->notifyStandbyAgent(getEntityName().c_str(), m_hostname.c_str());
                ...
                catch (CORBA::SystemException& e)
                {
                    LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException",
                                        str(boost::format("System exception occurred while calling notifyStandbyAgent on occ systemcontroller %s - %s") % it->first % CorbaUtil::exceptionToString(e)));
                }
                catch (...)
                {
                    LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", str(boost::format("Error occurred calling notifyStandbyAgent on occ systemcontroller %s") % it->first));
                }
            }
        }
    ...
    std::map<std::string, TA_Base_Core::ISystemControllerAdminCorbaDef_var> ManagedProcessInfo::resolveStandbySystemControllerAdmin()
    {
        ...
        TA_Base_Core::ISystemControllerData* myData = TA_Base_Core::SystemControllerAccessFactory:: getInstance().getSystemControllerDataByName(m_hostname);
        std::string myGroupName = myData->getGroupByName();
        std::vector<std::string> monitorsOfMyGroup = TA_Base_Core::GroupMonitorAccessFactory::getInstance().getSystemControllersMonitoringGroup(myGroupName);
        ...
        for (std::vector<std::string>::iterator it = monitorsOfMyGroup.begin(); it != monitorsOfMyGroup.end(); ++it)
        {
            ...
            std::string systemControllerAdminIor = str(boost::format("corbaloc::%s:%d/SystemControllerAdmin") % *it % port);

            // Attempt to resolve an IShutdownCorbaDef object
            CORBA::Object_var obj = TA_Base_Core::CorbaUtil::getInstance().stringToObject(systemControllerAdminIor);
            ...
            if (!CORBA::is_nil(obj))
            {
                try
                {
                    ...
                    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "resolved a systemControllerAdmin: %s", systemControllerAdminIor.c_str());
                }
                catch (CORBA::SystemException& e)
                {
                    LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException",
                                        str(boost::format("System exception occurred while resolving systemControllerAdmin %s - %s") % systemControllerAdminIor % CorbaUtil::exceptionToString(e)));
                }
                catch (...)
                {
                    LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", str(boost::format("Error occurred narrowing %s") % systemControllerAdminIor));
                }
            }
            else
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Cannot resolve systemControllerAdmin %s", systemControllerAdminIor.c_str());
            }
        }
    ...
    void ManagedProcessInfo::waitForProcessTerminate(unsigned long pid)
    {
#ifdef WIN32
        ...
        if (processHandle)
        {
            ...
            {
                ProcessManager::getInstance().signalReceived(getEntityName());
            }
        }
    ...
    std::string ManagedProcessInfo::makeDbConnectionFile(const std::string& locationName)
    {
        std::string dbConnectionFile = TA_Base_Core::RunParams::getInstance().get(RPARAM_DBCONNECTIONFILE);
        std::string serverLocationName = ProcessManager::getInstance().getServerEntityData()->getLocationName();

        if (locationName != serverLocationName)
        {
            dbConnectionFile = boost::regex_replace(dbConnectionFile, boost::regex("^" + serverLocationName), locationName);
        }

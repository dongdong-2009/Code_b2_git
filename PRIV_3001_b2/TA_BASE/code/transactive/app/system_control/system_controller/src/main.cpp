/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/app/system_control/system_controller/src/main.cpp $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
...
{
    SERVICE_STATUS* windowsServiceStatus = WindowsService::getInstance().getMonitorServiceStatus();
    SERVICE_STATUS_HANDLE* windowsServiceStatusHandle = WindowsService::getInstance().getMonitorServiceStatusHandle();
...
void service_main(DWORD argc, char**  argv)
{
    ...
    // Install handler
    *windowsServiceStatusHandle = RegisterServiceCtrlHandler(SERVICE_NAME, (LPHANDLER_FUNCTION) windowsServiceCtrlHandler);
...
int main(int argc, char* argv[])
{
    ...
    if (!TA_Base_Core::RunParams::getInstance().parseCmdLine(argc, argv))
    {
        ...
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugFatal, "Command line arguments in invalid format.");
            return 1;
        }
        else
        {

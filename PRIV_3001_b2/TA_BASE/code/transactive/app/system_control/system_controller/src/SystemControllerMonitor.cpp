/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/app/system_control/system_controller/src/SystemControllerMonitor.cpp $
...
namespace TA_Base_App
{
    ...
    SystemControllerMonitor& SystemControllerMonitor::getInstance()
    {
        ...
        if (s_instance == NULL)
        {
            ...
            if (s_instance == NULL)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugFatal, "New failed to allocate SystemControllerMonitor");
                exit(1);
            }
        }
        ...
        : m_groupProcessesAllStarted(false)
    {
        ...
        try
        {
            ...
            //m_peersMonitor = m_centralDbOnlineTimer(peerHostnames, m_heartbeatSendPeriod / 1000);
            m_peersMonitor.reset(new PeersMonitor(peerHostnames, m_heartbeatSendPeriod, m_heartbeatSendNumber));

            BOOST_FOREACH(std::string& peerHostname, peerHostnames)
            {
                // Get the entity data for each peer.
                TA_Base_Core::ISystemControllerDataPtr peerData(TA_Base_Core::SystemControllerAccessFactory:: getInstance().getSystemControllerDataByName(peerHostname));
                unsigned long entityKey = peerData->getServerEntityKey();
                TA_Base_Core::IEntityDataPtr entityData(TA_Base_Core::EntityAccessFactory::getInstance().getEntity(entityKey));
                m_peerEntityData[peerHostname].reset(new TA_Base_Core::ServerEntityData(entityData->getKey()));
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
                            "Will be sending heartbeats to peer %s, heartbeat period: %lu ms, heartbeat number: %lu", peerHostname.c_str(), m_heartbeatSendPeriod, m_heartbeatSendNumber);
                m_peerSCsToPulsate.push_back(PeerSCInfoPtr(new PeerSCInfo(m_hostname, peerHostname, m_heartbeatSendPeriod, m_heartbeatSendNumber)));
                m_failedPeers.push_back(peerHostname);
                raisePeerFailedAlarm(peerHostname);
            }
            ...
            {
                // I'm in a group!
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Am in group %s", myGroupName.c_str());

                // Get the hostnames of the System Controllers that are
                // monitoring my group.
                m_monitorsOfMyGroup = TA_Base_Core::GroupMonitorAccessFactory::getInstance().getSystemControllersMonitoringGroup(myGroupName);
            }
            else
            {
                // Not in a group.
            ...
            std::vector<std::string> groupsToMonitor = TA_Base_Core::GroupMonitorAccessFactory::getInstance().getGroupsMonitoredBySystemController(m_hostname);

            BOOST_FOREACH(std::string& groupName, groupsToMonitor)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Monitoring group %s", groupName.c_str());
                m_hasGroupFailed.push_back(false);
                std::vector<std::string> hostsInGroup = TA_Base_Core::SystemControllerAccessFactory::getInstance().getSystemControllersInGroup(groupName);
                m_groupMonitors.push_back(GroupMonitorPtr(new GroupMonitor(groupName, hostsInGroup, m_heartbeatSendPeriod, m_heartbeatSendNumber)));
            }
        }
    ...
    void SystemControllerMonitor::receiveHeartbeat(const char* host)
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugPeriodic, "received heartbeat from Host %s.", host);

        if (m_peersMonitor.get() != NULL)
        {
            // Notify the peer monitor of this heartbeat.
            m_peersMonitor->notifyHeartbeat(host);
        }

        // Notify the group monitor(s) of this heartbeat.
        BOOST_FOREACH(GroupMonitorPtr group, m_groupMonitors)
        {
            group->notifyHeartbeat(host);
        }
    }
    ...
    void SystemControllerMonitor::startHeartBeating()
    {
        ...
        if (m_groupProcessesAllStarted == false)
        {
            ...
            BOOST_FOREACH(PeerSCInfoPtr peer, m_peerSCsToPulsate)
            {
                peer->start();
            }
        }
        else
        {
            ...
            BOOST_FOREACH(PeerSCInfoPtr group, m_groupSCsToPulsate)
            {
                group->start();
            }
        }
    ...
    void SystemControllerMonitor::stopHeartBeating()
    {
        ...
        BOOST_FOREACH(PeerSCInfoPtr peer, m_groupSCsToPulsate)
        {
            peer->terminate();
        }
        ...
        BOOST_FOREACH(PeerSCInfoPtr peer, m_peerSCsToPulsate)
        {
            peer->terminateAndWait();
        }

        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Peer heartbeats stopped");
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Group heartbeats stopeed");
    ...
    void SystemControllerMonitor::checkForFailedPeers()
    {
        ...
        {
            // Peers have failed!
            BOOST_FOREACH(std::string& peerHostname, newFailures)
            {
                //TD16585++
                ...
                ProcessManager::getInstance().respondToPeerSCFailure(peerHostname);
        ...
        if (!newRecoveries.empty())
        {
            ...
            BOOST_FOREACH(std::string& peerHostname, newRecoveries)
            {
                closePeerFailedAlarm(peerHostname);
                ProcessManager::getInstance().respondtoPeerSCRestored(peerHostname);
            }
        }
    ...
    void SystemControllerMonitor::raisePeerFailedAlarm(const std::string& hostname)
    {
        ...
        try
        {
            ...
            std::map<std::string, TA_Base_Core::ServerEntityDataPtr>::iterator entityIter = m_peerEntityData.find(hostname);
            TA_ASSERT(entityIter != m_peerEntityData.end(), "Could not find peer entity data");
            TA_Base_Core::ServerEntityDataPtr serverData = entityIter->second;
            //TD11293,hongzhi, PISF/TST/814 - Communication failure alarm not raised at station level when occa and occb are offline
            TA_Base_Core::ServerEntityDataPtr ownServerData = ProcessManager::getInstance().getServerEntityData();
    ...
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "The following group has failed: %s. Raising alarm.", groupName.c_str());
        ...
        try
        {
            ...
            TA_Base_Core::ServerEntityDataPtr serverData = ProcessManager::getInstance().getServerEntityData();
    ...
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "The following peer is now online: %s. Closing alarm.", hostname.c_str());
    ...
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "The following group is now online: %s. Closing alarm.", groupName.c_str());
    ...
    void SystemControllerMonitor::checkForGroupProcesses()
    {
        ...
        if (ProcessManager::getInstance().haveAllAgentsStartedAtLeastOnce())
        {
            ...
            BOOST_FOREACH(std::string& groupName, m_monitorsOfMyGroup)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Will be sending heartbeats to group monitor %s", groupName.c_str());
                m_groupSCsToPulsate.push_back(PeerSCInfoPtr(new PeerSCInfo(m_hostname, groupName, m_heartbeatSendPeriod, m_heartbeatSendNumber)));
            }
    ...
    void SystemControllerMonitor::setGroupStarting(const char* hostName)
    {
        ...
        BOOST_FOREACH(GroupMonitorPtr group, m_groupMonitors)
        {
            if (group->contains(hostName))
            {
                group->setGroupStarting();
            }
        }
    }
} // namespace TA_Base_App

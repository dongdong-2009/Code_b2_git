/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/app/system_control/system_controller/src/ProcessInfo.cpp $
...
namespace TA_Base_App
{
    ...
    // Constructor
    //
    ProcessInfo::ProcessInfo(const std::string& entityName, TA_Base_Core::IProcess* processData, int pollPeriod, std::string binDir)
        : m_isRunning(false),
          m_timeStarted(0),
          m_terminateStatus(TA_Base_Core::NotKnown),
          m_wasSignalled(false),
          m_entityName(entityName),
          m_numRestarts(0),
          m_processStatus(TA_Base_Core::Unstarted),
          m_shouldWaitStartLag(true),
          m_threads(0)
    {
        m_pollPeriod = pollPeriod * 1000;
        m_entityDescription = processData->getDescription();
        m_address = boost::filesystem::system_complete(boost::filesystem::path(binDir) / processData->getAddress()).string();
        m_userName = processData->getUserName();
        m_passWord = processData->getPassWord();
        m_startLag = processData->getStartLag() * 1000;
        m_startLag = (m_startLag ? m_startLag : 5000);
        m_killTimeout = processData->getKillTimeout() * 1000;
        m_killTimeout = (m_killTimeout ? m_killTimeout : 5000);
        m_hostname = ProcessManager::getHostName();
        m_userName = TA_Base_Core::getRunParamValue("UserName", m_userName);
        m_passWord = TA_Base_Core::getRunParamValue("Password", m_passWord);
        m_startLag = TA_Base_Core::getRunParamValue("StartLag", m_startLag);
    }
    ...
    void ProcessInfo::setProcessStatus(TA_Base_Core::EProcessStatus processStatus)
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Process %s state changing from %d to %d", m_entityName.c_str(), m_processStatus, processStatus);
    ...
    void ProcessInfo::handleProcessFailure(TA_Base_Core::EManagedProcessTerminateCode reason)
    {
        ...
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Process %s has failed", getEntityName().c_str());
    ...
    void ProcessInfo::getProcessData(TA_Base_Core::ProcessData& data)
    {
        data.ProcessId = m_entityName.c_str();
        data.started = getTimeStarted();
        data.host = m_hostname.c_str();
    ...
    void ProcessInfo::raiseFailureAlarm(TA_Base_Core::EManagedProcessTerminateCode reason)
    {
        ...
        if (m_failureAlarmId == "")
        {
            ...
            TA_Base_Core::NameValuePair host("HostId", m_hostname);
            TA_Base_Core::NameValuePair terminateStatus("TerminateStatus", ProcessManager::terminateCodeToString(reason));
            TA_Base_Core::DescriptionParameters dp = boost::assign::list_of(&name)(&host)(&terminateStatus);
    ...
    bool ProcessInfo::waitForStartLag()
    {
        ...
        for (size_t i = 0, SLEEP_TIME = 100; i < m_startLag; i += SLEEP_TIME)
        {
            EProcessStatus status = getProcessStatus();

            if (terminatingStatusSet.find(status) != terminatingStatusSet.end())
            {
    ...
    void ProcessInfo::performStart()
    {
        ...
        else
        {
            ...
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Started process with pid %lu", m_processId);
        }
    }

...
namespace TA_Base_Core
{
	...
	void Thread::intervalRun()
	{
		...
		m_status=ThreadState_t::THREAD_STATE_TERMINATED;

	}

	Thread::Thread(void):
	m_priority(0),m_hHandle(0),m_status(ThreadState_t::THREAD_STATE_UNKNOWN)
	{
	   ...
	   if (m_hHandle!=0)
		...
		try
		{
			unsigned long hHandle = _beginthreadex
				...
				NULL);
			m_hHandle=hHandle;
			m_status=ThreadState_t::THREAD_STATE_RUNNING;
		}
		catch (...)
		{
			//

...
void CDataGenerator::getProfileConfig(I_ATS_ISCS_ALARM::ProfileInfo& profileInfo)
{
	TA_Base_Bus::ThreadGuard guard(m_threadLock);
...
I_ATS_ISCS_ALARM::AlarmDetail* CDataGenerator::createAlarm(CString& strAlarmID, CString& strAlarmValue, CString& strSystemName, 
	...
	pAlarmDetail = new I_ATS_ISCS_ALARM::AlarmDetail;
	...
	pAlarmDetail->state = (I_ATS_ISCS_ALARM::AlarmState)nAlarmState;
	pAlarmDetail->alarmType = strAlarmType.GetBuffer(0);
	pAlarmDetail->subsytemkey = strSubSystemName.GetBuffer(0);
	pAlarmDetail->systemkey = strSystemName.GetBuffer(0);
	pAlarmDetail->mmsState = (I_ATS_ISCS_ALARM::MmsStateType)nMMSState; //I_ATS_ISCS_ALARM::MMS_NONE;
	pAlarmDetail->omAlarm = 79;

	return pAlarmDetail;
}

I_ATS_ISCS_ALARM::AlarmDetail* CDataGenerator::createRandomAlarm()
{
	TA_Base_Bus::ThreadGuard guard(m_threadLock);
	I_ATS_ISCS_ALARM::AlarmDetail * pAlarmDetail = NULL;
	...
	pAlarmDetail->avalancheHeadID = "";
	pAlarmDetail->parentAlarmID = "";
	pAlarmDetail->state = I_ATS_ISCS_ALARM::AlarmOpen;
	pAlarmDetail->alarmType = GenATSAlarmStringProp(AlarmType);
	pAlarmDetail->systemkey = GenATSAlarmStringProp(SystemName);
	pAlarmDetail->subsytemkey = GenSubSystemName(pAlarmDetail->systemkey);
	pAlarmDetail->mmsState = I_ATS_ISCS_ALARM::MMS_NONE;
	pAlarmDetail->omAlarm = 79;

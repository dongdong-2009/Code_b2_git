...
void CAQMsgThreadsPool::run()
{
	...
	{	
		...
		if (STATE_CODE_MONITOR_AQ_SYS_EVENT == m_nPoolState ||
			STATE_CODE_WAITFOR_ACTIVE_EVENT == m_nPoolState)
		{
...
void  CAQMsgThreadsPool::_ProcessInitialEnv()
{
	...
	if (!m_pObserver)
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,
				"Init Evn,register observer, Error Code: %d, Error message: %s", Err_Register_Observer_Failed, defErrMsg(Err_Register_Observer_Failed));
		}
	}

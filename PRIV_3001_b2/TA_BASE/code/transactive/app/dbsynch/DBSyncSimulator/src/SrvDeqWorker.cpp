...
{	
	...
	m_pMonitorDeqTime = new CWorkTime(PROPAGATE_TIMER_LOG_MonitorDeqTime);
	m_pMonitorWaitACKTime= new CWorkTime(PROPAGATE_TIMER_LOG_MonitorClientEnqSendAckTime);
...
int	CSrvDeqWorker::_ServerProcessCommit(PENQACK	pReplyFrame)
{
	...
	LOG0(SourceInfo, DebugUtil::DebugDebug, "begin get lock table AQ_DEQ_TOKEN_CHECK one record for update"); 
	nFunRes = _DeqSelectForUpdate(m_pConsumerDataBase, m_strClientAgtName, m_nDeqInitOServerID, m_strQName);
	LOG0(SourceInfo, DebugUtil::DebugDebug, "end get lock table AQ_DEQ_TOKEN_CHECK one record for update");
	if (0 != nFunRes)
	{
		//check error
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Err:select AQ_DEQ_TOKEN_CHECK for update error! [PkgID=%s][EnqReqACK=%d]",
	...
	{  
		nDeqComitWorkTime = m_pTimer->getTimeDiffByMilSecond(PROPAGATE_TIMER_LOG_DEQCOMMIT);
		m_pTimer->reset(PROPAGATE_TIMER_LOG_DEQCOMMIT);	
		
		LOG4(SourceInfo, DebugUtil::DebugDebug, "end Commit [%s][DeqWorkIndex=%d][Msg.size=%d][DeqComitWorkTime=%d ms]", 
			m_strClientID.c_str(), m_nDeqWorkIndex, m_nCheckSendMsgNum, nDeqComitWorkTime);		
...
int	CSrvDeqWorker::_setAgtInfo(PAQWorkInfo pDeqEnqInfo)
{
	...
	LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, 
		"Deq Init ClientID=%s, ServerAgtName=%s, ServerID=%d", 
...
int	CSrvDeqWorker::_ServerPocessDeqData()
{
	...
	*/
	...
	nDeqDeqWorkTime = m_pTimer->getTimeDiffByMilSecond(PROPAGATE_TIMER_LOG_DEQDATA);
	...
	LOG4(SourceInfo, DebugUtil::DebugDebug, "[%s][DeqWorkIndex=%d][DeqGetMsgsCount=%d][Deq Time=%d ms]", 
...
{ 
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_UPDATE_SET_UPDATETIME_7012, strINLocationKey.c_str(), nINSRVID, strINQueueName.c_str());
...
int	CSrvDeqWorker::_DeqUpdateTimeAndSrvID(IDatabase* pINDataBase, 
...
{ 
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_UPDATE_SET_TIMEANDSRVID_7013, nINSRVID, strINLocationKey.c_str(), strINQueueName.c_str());
	try
	{
		pINDataBase->connect();
		pINDataBase->executeModification(strOracleSQL);
...
int	CSrvDeqWorker::_DeqSelectForUpdate(IDatabase* pINDataBase, 
...
{ 
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_SELECT_GET_LOCKUPDATETIME_7015, 
									...
									std::string& strINLocationKey, int& nINSRVID, std::string& strINQueueName, struct timeb* pOUTfbtime)
{
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_INSERT_ADD_UPDATETIME_7017, strINLocationKey.c_str(), nINSRVID, strINQueueName.c_str());
	try
	{
		pINDataBase->connect();
		pINDataBase->executeModification(strOracleSQL);
											  ...
											  std::string& strINQueueName, int nINSRVIDForCheck)
{
	...
	*/
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_SELECT_GET_UPDATETIME_7011, 
										 ...
										 std::string& strINQueueName, int nINSRVIDForCheck, timeb* pOUTUpdateTime)
{
	...
	select SRV_ID, UPD_TIME from AQ_DEQ_TOKEN_CHECK where  UPPER(LOCATION) = UPPER('%s') and UPPER(Q_NAME)=UPPER('%s')
	*/
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_SELECT_GET_UPDATETIME_7011, strINLocationKey.c_str(), strINQueueName.c_str());
											  ...
											  std::string& strINQueueName, int nINSRVIDForCheck)
{
	...
	if (nNotWorkSeconds * TIME_BASE_SECOND >= TIME_MIN_FIVE)//TIME_MIN_FIVE   TIME_MIN_THIRTY
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, 	
			"Err: [nOUTNotWorkSeconds*TIME_BASE_SECOND >= TIME_MIN_FIVE][%d>=%d][nGetSRVID=%d][nINSRVIDForCheck=%d]", 
			nNotWorkSeconds*TIME_BASE_SECOND, TIME_MIN_FIVE, nGetSRVID, nINSRVIDForCheck);

		nFunRes = _DeqUpdateTimeAndSrvID(pINDataBase, strINLocationKey, nINSRVIDForCheck, strINQueueName, NULL);
...
int	CSrvDeqWorker::_OracleGetDBNowTime(IDatabase* pINDataBase, struct timeb* pOUTfbtime)
{
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_SELECT_GET_DBTIMENOW_7010);
...
{  	
	...
	strOracleSQL = pINDataBase->prepareSQLStatement(DEQ_SQL_SELECT_GET_NOTWORKTIME_7016, strINLocationKey.c_str(), strINQueueName.c_str());
...
int CSrvDeqWorker::_ProcessDeqUpdateTime()
{
	...
	if ( m_nDeqWorkState == server_state_deq_DeqData ||		
		m_nDeqWorkState == server_state_deq_WaitForReply ) 
	{
		uiTimeDiff = m_pTimer->getTimeDiffByMilSecond(PROPAGATE_TIMER_DEQUPDATE);

		//update AQ_DEQ_TOKEN_CHECK.UPD_TIME every 30 seconds
		if (uiTimeDiff > TIME_SECOND_THIRTY)
		{
			m_pTimer->reset(PROPAGATE_TIMER_DEQUPDATE);

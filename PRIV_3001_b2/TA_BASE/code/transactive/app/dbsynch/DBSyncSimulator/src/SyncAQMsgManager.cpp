...
CSyncAQMsgManager::~CSyncAQMsgManager()
{
	...
	TA_AQ_Lib::g_deattach_db_driver();
}
...
void CSyncAQMsgManager::stopSynchMsg()
{
	...
	switch (g_system_variables.m_nDBType)
	{
	case enumMysqlDb:			
		if (m_pAQSynchPool)
		...
		break;
	case enumOracleDb: // TODO: server side stop
...
int  CSyncAQMsgManager::_InitDefaultVal(pg_config_parameter pSystemVar)
{
	...
	memcpy(pSystemVar->m_LogFileName, defDefault_LogFileName, defMaxFileLen - 1);
	memcpy(pSystemVar->m_szPort, defDefault_config_OracleServerPort, defServerPortLen - 1);
...
void CSyncAQMsgManager::_StartSynchThread()
{
	...
	if (RunType_NotSet != g_system_variables.m_nRunType)
	{
		nRunType = g_system_variables.m_nRunType;
	}
	else
	{
		nRunType = g_system_variables.m_nDBType;
	}

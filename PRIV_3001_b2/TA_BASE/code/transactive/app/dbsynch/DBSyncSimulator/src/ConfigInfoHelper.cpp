...
int  CConfigInfoHelper::_FillCfgData(pg_config_parameter pConfigPara)
{
	...
	{	
		pConfigPara->m_nEnqTimeOut = TIME_MIN_FIVE;
	}
	...
	{	
		pConfigPara->m_nDeqTimeOut = TIME_MIN_FIVE;
	}
	...
	{	
		pConfigPara->m_nDeqGetACKTimeOut = TIME_MIN_FIFTEEN;
	}
	...
	{	
		pConfigPara->m_nHandShareTimeOut = TIME_SECOND_TEN;
	}
	...
	{	
		pConfigPara->m_nReleaseActorTimeOut = TIME_MIN_FIVE;
	}
...
int  CConfigInfoHelper::_GetServerPortFromDB(const std::string& strHostName, pg_config_parameter pConfigPara)
{
	...
	std::string strSql   = m_pDatabase->prepareSQLStatement(DB_GET_SERVER_PORT_5002);
...
int  CConfigInfoHelper::_GetAgentNameFromDB(pg_config_parameter pConfigPara)
{
	...
	std::string strSql   = m_pDatabase->prepareSQLStatement(ORACLESYSTABLE_SELECT_5000);
...
int  CConfigInfoHelper::_GetQSubCntFromDB(mapQSubCntT& rMapSubCnt)
{
	...
	std::string strSql   = m_pDatabase->prepareSQLStatement(AQSYSMTABLE_SELECT_4050);
...
int  CConfigInfoHelper::_GetNewThreadInfoFromDB(const std::string& strQName, mapQSubCntT& rQsubMap, mapQThreadPoolT*& rPMapPool)
{
	...
	std::string strSql   = m_pDatabase->prepareSQLStatement(AQSYSSTABLE_SELECT_4051, strTable);
												 ...
												 mapQSubCntT& rQsubMap, mapQThreadPoolT*& rPMapPool)
{
	...
	std::string strSql   = m_pDatabase->prepareSQLStatement(AQSYSSTABLE_SELECT_4052, strTable, strExistSubID);

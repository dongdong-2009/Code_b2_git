...
{	 
	...
	if (0 != nFunRes)
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "send ConReq AcK error!");
	}
	else
	{
		LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "send ConReq AcK ok!");
	}
...
int IServerTcpComType::_InitClientID(CSrvInitWorker* pSrvInitWorker)
{
	...
	LOG5(SourceInfo, DebugUtil::DebugInfo,
		"_InitClientID[ClientID=%s][AgtName=%s][QName=%s][ClientIP=%s]OServerID=%d]",	
		m_strClientID.c_str(), m_strAgtName.c_str(), m_strQName.c_str(), m_strClientIP.c_str(), m_nINSRVID);
...
int IServerTcpComType::_InitSrvStream(TcpSocket* pSocket)
{
	...
	LOG1(SourceInfo, DebugUtil::DebugDebug,	"new client connected:[ClientHostName=%s]", 
		m_strClientHostName.c_str());
...
int IServerTcpComType::_GetServerAgentInfo(IDatabase *pDataBase, std::string& strServerAgtName )
{
	...
	strSql   = pDataBase->prepareSQLStatement(ORACLESYSTABLE_SELECT_5000);
...
int IServerTcpComType::_GetClientAgentInfo(IDatabase *pDataBase, const std::string& strClientName, std::string& strClientAgtName )
{
	...
	strSql   = pDataBase->prepareSQLStatement(ORACLERECIPIENT_SELECT_5001, strClientIp.c_str());

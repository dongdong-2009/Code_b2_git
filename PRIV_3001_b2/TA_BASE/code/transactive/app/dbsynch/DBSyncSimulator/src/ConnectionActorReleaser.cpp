...
{
	m_toTerminate = false;
	m_pReleaseActorWorkTime = new CWorkTime(PROPAGATE_TIMER_LOG_MonitorReleaseActor);
}

ConnectionActorReleaser::~ConnectionActorReleaser(void)
{
	ThreadGuard guardTimer(m_monitorTimerLock);
	DEF_DELETE(m_pReleaseActorWorkTime);
}
...
void ConnectionActorReleaser::_releaseInvaildActors()
{
	...
	while (iter!=releaselist.end())
	{
		...
		strClientIDGet = actor->getClientID();
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, 
			"begin free one server ClientID=%s, ClientAgtName=%s, ID=%ul",
			strClientIDGet.c_str(), strClientAgtNameGet.c_str(), nIDGet);
		//begin
		{
			ThreadGuard guardTimer(m_monitorTimerLock);
			m_pReleaseActorWorkTime->workBegin();
		}		
		actor->serverstopWrok(CurrentClientDestroy_Normal);
		DEF_DELETE(actor);
		//end
		{
			ThreadGuard guardTimer(m_monitorTimerLock);
			m_pReleaseActorWorkTime->workEnd();
		}	
		 
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, 
			"end free one server ClientID=%s, ClientAgtName=%s, ID=%ul",
			strClientIDGet.c_str(), strClientAgtNameGet.c_str(), nIDGet);
...
int ConnectionActorReleaser::getWorkTimeInfo( WorkTimeInfo& workTimeInfoGet )
{
	int nFunRes = 0;
	ThreadGuard guardTimer(m_monitorTimerLock);
	if (NULL != m_pReleaseActorWorkTime)
	{
		workTimeInfoGet.m_nReleaseActorNotWork = m_pReleaseActorWorkTime->getNotworkTime();
	}
	else
	{

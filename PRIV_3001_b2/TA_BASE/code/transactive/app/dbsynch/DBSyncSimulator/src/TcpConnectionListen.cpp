...
void CTcpConnectionListen::_StartThreadPool()
{
	...
	if (NULL == m_pTcpConnectionPool)
	{
		m_pTcpConnectionPool = CTcpConnectionPool::getInstance();
		m_pTcpConnectionPool->start();
	}
	...
	catch ( TA_Base_Core::TcpSocketException& e )
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError, 
			"create server listen port =[%s] socket error[%s]", m_strServerPort.c_str(), e.what());
...
void CTcpConnectionListen::_StopThreadPool()
{
	...
	if (NULL != m_pTcpConnectionPool)
	{
		...
		m_pTcpConnectionPool->terminateAndWait();
		DEF_DELETE(m_pTcpConnectionPool);
...
bool CTcpConnectionListen::_ServerListenSocketAccept(TcpServerSocket<TcpSocket>* serverSocket)
{
	...
	try
	{
		...
		clientSocket = serverSocket->accept(SERVER_NON_BLOCKING_ACCEPT); //SERVER_BLOCKING_ACCEPT
	...
	bRes = false;
	bRes = m_pTcpConnectionPool->poolAcceptOneNewClient(clientSocket);

/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_agent/src/ActiveBackgroundDecisionStep.cpp $
  *  @author:  Gregg Kirkpatrick
  *  @version: $Revision: #6 $
  *
  *  Last modification: $DateTime: 2014/01/24 18:53:39 $
...
namespace TA_Base_App
{
	...
	ActiveBackgroundDecisionStep::ActiveBackgroundDecisionStep( const TA_Base_Core::StepDetail& stepDetail, 
        const TA_Base_Core::BackgroundDecisionParameters& params, IActivePlanFlow& flowControl ) : ActiveStep(stepDetail, flowControl),
    m_params(params),
    m_sendPopupMessageTryTimes(0)
	{
	}

    TA_Base_Core::StepNumber ActiveBackgroundDecisionStep::getNextDecisionStep(const int status)
    {
	...
	TA_Base_Core::EActiveStepState ActiveBackgroundDecisionStep::executeImpl( void )
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Background Decision step %lu", getStepNumber() );
      ...
      TA_Base_Core::EActiveStepState stepState = TA_Base_Core::UNEXECUTED_ACTIVE_STEP_STATE;
        ...
        detail.data.bgDecision(m_params);

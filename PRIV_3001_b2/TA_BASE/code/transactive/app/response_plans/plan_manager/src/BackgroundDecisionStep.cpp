/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_manager/src/BackgroundDecisionStep.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #3 $
  *
...
void BackgroundDecisionStep::createPlanStepParameters(PlanStepParameterList &stepParameters)
{
	...
	// dahe++ for GF-75
    m_yesLabelParameter = new TextualStepParameter(*this,getParameterName(IDS_STEP_PARAMETER_YES_LABEL), "Yes");
    m_noLabelParameter = new TextualStepParameter(*this, getParameterName(IDS_STEP_PARAMETER_NO_LABEL), "No");
	m_pauseLabelParameter = new TextualStepParameter(*this, getParameterName(IDS_STEP_PARAMETER_PAUSE_LABEL), "Pause");
	// ++dahe
                                         ...
                                         PlanStepParameterList &stepParameters)
{
    ...
    TA_Base_Core::IProfile *profile = ProfileConfigAccess::instance()->getProfileByKey(parameterSet.bgDecision().profile);
    if (NULL == profile)
    {
        bValidProfile = false;
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugFatal, "Invalid profile key = %lu", parameterSet.bgDecision().profile);
    }
    m_operatorProfileDisplay = new TextualStepParameter(*this, getParameterName(IDS_STEP_PARAMETER_PROFILE_NAME), bValidProfile? profile->getName():"");

...
namespace TA_Base_App{
	...
	void ISCSRealTimeAgentWorker::checkStatusFromAgent()
	{
		...
		if(status)//if fail, set degraded mode to true
		{
		...
		{
			m_DegradedMode = true;
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "ISCSRealTimeAgentWorker in Degraded Mode");
		}
	}
		...
		while(!m_needToTerminate)
		{
			if(!m_isPaused)  //has task to do
			{
				...
				else if(m_isRefreshMode )
				{
					...
					if(!m_needToTerminate && !m_isPaused)
	...
	void ISCSRealTimeAgentWorker::loadRefreshList()
	{
		...
		try{
			...
			//set agent to healthy and degraded mode to false
			m_status = AgentStatus::Healthy;
			m_DegradedMode = false;
		...
		catch(TA_Base_Core::EventHelperException ex)
		{
			...
			if(eCode == TA_Base_Core::EventHelperException::ERR_ID_NOT_FOUND)
			{
				...
				}else 
				{
					...
					loadAllFromAgent();    //when eventID not found in agent, reload from Agent
					//m_isPaused = true;
					//m_wokerObserver->reload();
					return;
				}
			}
	...
	bool ISCSRealTimeAgentWorker::isInDegradedMode()
	{

	...
	void ISCSRealTimeWorker::run()
	{
		...
		while( !m_needToTerminate ){
			try
			{
				...
				{
					//check if both the DBworker and AgentWorker in degrade mode
					if(m_agentWorker->isInDegradedMode() && m_DBWorker->isInDegradedMode())
					{
						m_DegradedMode = true;
						//Update Status Bar 
						m_workerObserver->statusBarUpdate(TA_Base_Core::EventCategory::ISCSCategory,EventViewerConstants::STATUS_ISCS_UNHEALTHY);
					...
					else
					{
						...
						m_workerObserver->statusBarUpdate(TA_Base_Core::EventCategory::ISCSCategory,EventViewerConstants::STATUS_ISCS_READY);
					}
					...
					if(  m_agentWorker->getStatus()==AgentStatus::UnHealthy ) //!isAgentStatusSwitched() &&!m_realTimeAgentWorker->isHealthy() 
					{	
						//set DB Autofresh to true when Agent is UnHealthy
					...
					else if ( m_agentWorker->getStatus()==AgentStatus::Healthy) 
					{	
						//set DB AutoRefresh to false when Agent is healthy
						if( m_DBWorker->isAutoRefresh()) //to avoid duplicating setting
						{
							m_DBWorker ->resetAutoRefresh();
						}
						if(!m_needToTerminate)
							sleep(m_refreshRateAgent);
					}

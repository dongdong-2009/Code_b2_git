...
namespace TA_Base_App
{
	...
	void OPCDataCache::loadDbData(TA_Base_Core::OPCDataNodeDataList*& dnList, TA_Base_Core::OPCDataPointDataList*& dpList)
	{
		...
		bool bCentralizeOpcBridge = false;
		std::string strCentralizeOpcBridge = TA_Base_Core::RunParams::getInstance().get("CentralizeOpcBridge");
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "CentralizeOpcBridge = %s", strCentralizeOpcBridge.c_str());

		if ((0 == strCentralizeOpcBridge.compare("true")) || 
			(0 == strCentralizeOpcBridge.compare("True")) || 
			(0 == strCentralizeOpcBridge.compare("1")))
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "CentralizeOpcBridge = 1.");
			bCentralizeOpcBridge=true;
		}
	
		TA_Base_Core::OPCEntityDataAccessFactory & dbCon = TA_Base_Core::OPCEntityDataAccessFactory::getInstance(bCentralizeOpcBridge);

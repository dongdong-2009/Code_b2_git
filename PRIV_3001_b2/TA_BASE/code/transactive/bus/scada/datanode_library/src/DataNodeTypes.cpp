...
// 		}
		...
		TA_Base_Bus::ScadaProxyFactory& scadaProxyFactory, IEntityUpdateEventProcessor & entityEventProcessor)
	{
        ...
        for ( itr = staticConfig.childDataPointEntities.begin(); itr != staticConfig.childDataPointEntities.end(); ++itr )
        {
            try
            {
        ...
        for ( itr = staticConfig.childDataNodeEntities.begin(); itr != staticConfig.childDataNodeEntities.end(); ++itr )
        {
            try
            {
				unsigned long entityKey = (*itr)->getKey();

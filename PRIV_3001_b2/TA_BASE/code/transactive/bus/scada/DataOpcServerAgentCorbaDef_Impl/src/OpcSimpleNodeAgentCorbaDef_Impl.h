/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/scada/DataNodeAgentCorbaDef_Impl/src/OpcSimpleNodeAgentCorbaDef_Impl.h $
  ...
  * the IDataNodeAgentCorbaDef interface, providing SimpleNode configurations
  * and updates to OPCBridge services serving MFTs.
  *
  */

#ifndef OPC_DATA_NODE_AGENT_CORBA_IMPL_H
#define OPC_DATA_NODE_AGENT_CORBA_IMPL_H
...
{
	// forward declaration
	class SimpleNodeObserver;
	class SimpleNode;
    class SimpleNodeDisplayClient;
	class ISimpleNodeAgent;

	//TD15618, use timer to trigger if the observers are timeout
	class OpcSimpleNodeAgentCorbaDef_Impl : public virtual TA_Base_Core::ServantBase,
										public virtual POA_TA_Base_Bus::IDataNodeAgentCorbaDef,
										public virtual TA_Base_Core::AbstractThreadedTimeoutUser
	{
    public:
        
        OpcSimpleNodeAgentCorbaDef_Impl ( const std::string & agentName,
                                     TA_Base_Bus::ISimpleNodeAgent * specificSimpleNodeAgent );

		virtual ~OpcSimpleNodeAgentCorbaDef_Impl();
        ...
        SimpleNodeDisplayClient * findDisplayClient ( unsigned long clientId );
		
		//
		// Operations required by SimpleNode synchronization
		//
		 ...
		 * setToControl
		 *
		 * This method let the OpcSimpleNodeAgentCorbaDef_Impl instance knows that it
		 ...
		 * setOperationMode
		 *
		 * This method let the OpcSimpleNodeAgentCorbaDef_Impl instance into new mode
		 *
		 */
	...
	private:
		...
		OpcSimpleNodeAgentCorbaDef_Impl();
		OpcSimpleNodeAgentCorbaDef_Impl( const OpcSimpleNodeAgentCorbaDef_Impl & theInstance );
		OpcSimpleNodeAgentCorbaDef_Impl & operator= ( const OpcSimpleNodeAgentCorbaDef_Impl & rhs );
		 ...
		 *
		 */
		SimpleNodeObserver * findObserver ( unsigned long observerId );
		...
		TA_Base_Bus::ISimpleNodeAgent * m_specificAgent;
		std::map < unsigned long, SimpleNodeObserver * > m_listOfObservers;
		TA_Base_Core::ReEntrantThreadLockable m_observerListLock;
        
		unsigned long m_generatedClientID;
		std::map < unsigned long, SimpleNodeDisplayClient * > m_listOfDisplayClients;
		TA_Base_Core::ReEntrantThreadLockable m_clientListLock;
        SimpleNodeStateContainer * m_stateContainer;

        //SimpleNodeAgentSynHandler *m_synHandler;
        //only one SimpleNode synchronization client is allowed

...
namespace TA_Base_Bus
{
	...
	ATSAlarmInfoManager::ATSAlarmInfoManager(long maxBlockCount,long blockByteSize)
		:CacheDataManager<std::string , ATSAlarmQueueItem>(maxBlockCount,blockByteSize,"ATSAlarmInfo")
	{
...
//	}
	bool ATSAlarmInfoManager::writeCacheDataToMem(ATSAlarmQueueItem* cacheData,ShareMemoryHelper* mem, char* pos)
	{
		...
		{			
			...
			mem->writeString(mem->getCurWritePos(),cacheData->detail.strAlarmType,AlarmTypeSize);
			mem->writeString(mem->getCurWritePos(),cacheData->detail.systemkeyType,SystemKeySize);
			mem->writeString(mem->getCurWritePos(),cacheData->detail.subsytemType,SubSystemKeySize);
		}
		else
		{
			//raise error
	...
	bool ATSAlarmInfoManager::readCacheDataFromMem(ATSAlarmQueueItem* cacheData,ShareMemoryHelper* mem, char* pos)
	{		
		//Attention: if the sequence of the following fields is updated, all related component should be re-compile.
		...
		cacheData->detail.strAlarmType=mem->readString(mem->getCurReadPos(),AlarmTypeSize).c_str();
		cacheData->detail.systemkeyType=mem->readString(mem->getCurReadPos(),SystemKeySize).c_str();
		cacheData->detail.subsytemType=mem->readString(mem->getCurReadPos(),SubSystemKeySize).c_str();

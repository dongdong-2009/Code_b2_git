...
// 			}
	...
	void removeFromAlarmIndex(alarmindex_by_atsFlag::iterator& locItem)
	{
		...
		alarmindex_by_atsFlag& index_location = boost::multi_index::get<SORT_BY_ATSFLAG>(*alarmIndexWithDuty);
		index_location.erase(locItem);
		--g_pShAlarmInfo->gDutyAlarmCounter;
		if (true == bUnAck)
		{
...
// 	}
	...
	//*/HP++
	unsigned long TA_SharedAlarm::removeLocationAlarmsInternal(unsigned long locationKey, bool isATSAlarms /*=false*/)
	{
		...
		try
		{
			...
			alarmindex_by_location::iterator lIter = location_index.lower_bound(boost::make_tuple(locationKey, isATSAlarms));
			alarmindex_by_location::iterator hIter = location_index.upper_bound(boost::make_tuple(locationKey, isATSAlarms));
			
			while(lIter != hIter)
			{
				removeFromAlarmIndex(lIter);
				++removedCount;
				lIter = location_index.lower_bound(boost::make_tuple(locationKey, isATSAlarms));
				hIter = location_index.upper_bound(boost::make_tuple(locationKey, isATSAlarms));
			}
		}
		catch(...)
		{
	...
	unsigned long TA_SharedAlarm::removeATSAlarmsInternal()
	{
		...
		ACE_SEH_TRY
		{
			alarmindex_by_atsFlag& location_index = boost::multi_index::get<SORT_BY_ATSFLAG>(*alarmIndexWithDuty);
	...
	unsigned long TA_SharedAlarm::removeLocationAlarms(unsigned long locationKey, bool isATSAlarms/* = false*/)
	{
		TA_Base_Core::ProcessRWGuard   SharedAlarmGuard (m_SharedAlarmDutyLock, TA_Base_Core::WRITE_ACCESS);

		unsigned long removedCount = 0;
		removedCount=removeLocationAlarmsInternal(locationKey, isATSAlarms);
	...
	void TA_SharedAlarm::insertOrUpdateAlarm( std::vector<AlarmDetail*>& atsAlarms, bool isPassPreFilter)
	{
		...
		{   
			...
			try
			{
				...
				if (itFind == find_id.end())
				{
					if (atsAlarmDetail->state != I_ATS_ISCS_ALARM::AlarmClosed)
					{
	...
	{		
		...
		try
		{
			...
			{
			case I_ATS_ISCS_ALARM::AlarmOpen:
			case I_ATS_ISCS_ALARM::AlarmAcknowledged:
				alarmState = TA_Base_Core::AlarmOpen;
				break;
			case I_ATS_ISCS_ALARM::AlarmReturned:
	...
	AlarmDataItem* TA_SharedAlarm::_newAlarmDataItem(TA_Base_Core::AlarmItem* pAlarm)
	{
		...
		if (pAlarm != NULL)
		{
			try
			{
				...
				pAlarmDataItem->isATSAlarm			= pAlarm->isATSAlarm;

...
namespace TA_Base_Bus
{
	...
	public:
		 ...
		 */
		void insertOrUpdateAlarm( std::vector<I_ATS_ISCS_ALARM::AlarmDetail* >& atsAlarms, bool isPassPreFilter);
...
//		unsigned long getAlarmAmount();
		 ...
		 */
		unsigned long removeLocationAlarms(unsigned long locationKey, bool isATSAlarms = false);
		 ...
		 * if alarm is not found in the container 
		 *
		...
		//++Noel
		long removeLocSubSystemDutyAlarms(unsigned long ulLocationKey, std::list<unsigned long>& subSystemKeyList, bool isATSAlarms = false);
	...
	private:
		...
		unsigned long removeLocationAlarmsInternal(unsigned long locationKey, bool isATSAlarms = false);
		...
		void addAlarmItem(const I_ATS_ISCS_ALARM::AlarmDetail & alartItem, bool isPassPreFilter);

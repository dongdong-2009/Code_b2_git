...
namespace TA_Base_Bus
{
	...
	unsigned long SharedAlarmAccessFactory::removeLocationAlarms(unsigned long locationKey)
	{
		TA_Base_Core::ThreadGuard guard(m_lock);

		return m_sa->removeLocationAlarms(locationKey);
	}
	...
	long SharedAlarmAccessFactory::removeLocSubSystemDutyAlarms(unsigned long ulLocationKey, std::list<unsigned long>& subSystemKeyList, bool isATSAlarms/*=false*/)
	{
		TA_Base_Core::ThreadGuard guard(m_lock);
		return m_sa->removeLocSubSystemDutyAlarms(ulLocationKey, subSystemKeyList, isATSAlarms);

...
namespace TA_Base_Bus
{
	...
	bool MMSSubmitterDatabase::submitJobRequest(JobRequestDetailCorbaDef& jobRequest, std::string sessionID)
	{
		...
		{
			throw TA_Base_Bus::DependencyFailure("Rights could not be determined for this action.");
		}

		if(!isJobRequestPermitted)
		{
			std::string message = "Operator does not have sufficient privileges to raise Service Request.";
			throw TA_Base_Bus::SubmissionDenied(message.c_str(), true);
		}
		...
		if( 0 != strcmp("NOT_FROM_ALARM", jobRequest.alarmUID.in()) )
		{
			...
			std::string reason;
			if(false == alarmStateIsOk(jobRequest, reason) )
			{				
				throw TA_Base_Bus::SubmissionDenied(reason.c_str(), false);
			}
			...
			{
				throw TA_Base_Bus::DependencyFailure("Could not determine the MMS inhibition state.");
			}
			if( isInhibited )
			{	

				throw TA_Base_Bus::SubmissionDenied("The datapoint associated with the alarm is MMS inhibited.", false);
			}
		...
		else //not from alarm so just check global inhibit state
		{
			...
			{
				throw TA_Base_Bus::SubmissionDenied("Global MMS inhibition is applied.", false);
			}
		}
		...
		try
		{
			...
			if (ret==false)
			{
				...
				throw TA_Base_Bus::DependencyFailure(msg.c_str());
			}
		}
		catch(...)
		{		 
		...
		if( 0 != strcmp("NOT_FROM_ALARM", jobRequest.alarmUID.in()) )
		{
			...
			catch(...)
			{
				...
				throw TA_Base_Bus::DependencyFailure(reason.c_str());
			}
		}
	...
	std::string MMSSubmitterDatabase::getOperatorDesc(const std::string& sessionID)
	{
		...
		if(name.empty())
		{
			...
			throw TA_Base_Bus::DependencyFailure("The operator details could not be retrieved.");
		}
	...
	JobRequestDetailCorbaDef* MMSSubmitterDatabase::getDataFromAlarm(std::string alarmUID,unsigned long alarmLocationKey, std::string sessionID)
	{
		...
		if("NO_ERROR" != errorMessage)
		{
			...
			throw TA_Base_Bus::DependencyFailure(errorMessage.c_str());
		}
	...
	std::string MMSSubmitterDatabase::toDataPointValue(const TA_Base_Core::AlarmDetailCorbaDef& p_alarm)
    {
			...
			bool truncate /*= true*/)
		{
			...
			if(alarmMessage.length() <= ALARM_MSG_SIZE || truncate == false)
			{
				FUNCTION_EXIT;
				return alarmMessage;
			}
			//else truncate
			std::string truncated = alarmMessage.substr(0, ALARM_MSG_SIZE);
		...
		std::string& alarmDescField, std::string& problemReportField)
	{
		...
		if(untruncatedDesc.length() <= ALARM_MSG_SIZE)
		{
			alarmDescField = untruncatedDesc;
		}
		else //split the result up
		{
			alarmDescField = untruncatedDesc.substr(0, ALARM_MSG_SIZE);
			//256 - 200 - 2 = 54 max chars
			//problemReportField = untruncatedDesc.substr(ALARM_MSG_SIZE, 54);
		}
		problemReportField = "";
		FUNCTION_EXIT;
	}

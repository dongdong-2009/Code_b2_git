...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/bus/generic_gui/src/TransActiveMessage.cpp $
  * @author:  Brett Zimmerman
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2018/03/23 16:08:21 $
...
namespace
{
    struct MutexGuard
    {
        MutexGuard(omni_mutex& m) : mutex(m) { mutex.lock(); }
    ...
    public:
        ...
        static void insideHook(HWND hwnd)
        {
            stopHook();
            addMessageBox(hwnd);
        }

        static void closeAllMessageBoxes()
        {
            MutexGuard guard(g_mutex());
            LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::closeAllMessageBoxes - there are %lu MessageBoxes to close", TA_Base_Bus::TransActiveMessage::s_popupHWnd.size());

            BOOST_FOREACH(HWND hwnd, TA_Base_Bus::TransActiveMessage::s_popupHWnd)
            {
                if (::IsWindow(hwnd))
                {
                    LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::closeAllMessageBoxes - close MessageBox %p", hwnd);
                    ::PostMessage(hwnd, WM_CLOSE, 0, 0);
                }
            }

            TA_Base_Bus::TransActiveMessage::s_popupHWnd.clear();
        }
    ...
    private:
        ...
        static void startHook()
        {
            if (NULL == tss()->hook())
            {
                tss()->hook() = SetWindowsHookEx(WH_CBT, &TA_Base_Bus::TransActiveMessage::CBTProc, 0, GetCurrentThreadId());
                LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::startHook - hook=%p SetWindowsHookEx", tss()->hook());
            }
        }
        ...
        static void stopHook()
        {
            if (tss()->hook() != NULL)
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::stopHook - hook=%p UnhookWindowsHookEx", tss()->hook());
                UnhookWindowsHookEx(tss()->hook());
                tss()->hook() = NULL;
            }
        }

        static void removeAllMessageBoxes()
        {
            MutexGuard guard(g_mutex());
            LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::removeAllMessageBoxes - there are %lu MessageBoxes in this thread", tss()->hwnds().size());
            BOOST_FOREACH(HWND hwnd, tss()->hwnds())
            {
                TA_Base_Bus::TransActiveMessage::s_popupHWnd.erase(hwnd);
                LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::removeAllMessageBoxes - removed MessageBoxes %p", hwnd);
            }
            tss()->hwnds().clear();
            LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::removeAllMessageBoxes - there are %lu MessageBoxes remains", TA_Base_Bus::TransActiveMessage::s_popupHWnd.size());
        }

        static void addMessageBox(const HWND hwnd)
        {
            tss()->hwnds().insert(hwnd);
            LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "HookWrap::addMessageBox - add MessageBox HWND %p, size=%lu", hwnd, tss()->hwnds().size());
            MutexGuard guard(g_mutex());
            TA_Base_Bus::TransActiveMessage::s_popupHWnd.insert(hwnd);
        }
...
namespace TA_Base_Bus
{
    ...
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Message Box [Caption: %s] [Message: %s]", pszCaption, pszMessage);
    ...
    bool TransActiveMessage::logMessage(LPCTSTR pszCaption, LPCTSTR pszMessage, UINT uiResult, BOOL bAuditEvent)
    {
        ...
        default:
            sResult = boost::lexical_cast<std::string>(uiResult).c_str();
            break;
        }
        ...
        sMessage.Format(_T("Message Box [Caption: %s] [Message: %s] [Result: %s]"), pszCaption, pszMessage, sResult); //SL
    ...
    LRESULT CALLBACK TransActiveMessage::CBTProc(INT nCode, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (nCode == HCBT_ACTIVATE)
        {
            ...
            //s_popupHWnd.insert(hChildWnd);

            // exit CBT hook
            //UnhookWindowsHookEx(s_hhk);
            //s_hhkLock.release();
            HookWrap::insideHook(hChildWnd);
        }

...
TA_Base_Core::ALLOCATOR* TA_Base_Core::Shmem::create(const char* mapfile /*= "C:/Shared.map"*/, bool attempDeleteOldFile /*= true*/)
{
...
#endif	
		LOGBEFORECOTS("ACE::ACE_Allocator_Adapter" );		
		LOGBEFORECOTS("ACE::ACE_MMAP_MEMORY_POOL" );
		LOGBEFORECOTS("ACE::ACE_Null_Mutex" );
		LOGBEFORECOTS("ACE::ACE_Control_Block" );
		tempAlloc = new ALLOCATOR(mapfile, "Shared_Lock", 
			//&ACE_MMAP_Memory_Pool_Options(ACE_DEFAULT_BASE_ADDR,
			&ACE_MMAP_Memory_Pool_Options((char *) (1024 * 1024 * 1024),
			ACE_MMAP_Memory_Pool_Options::ALWAYS_FIXED, 1, 128*1024*1024
			));
		LOGAFTERCOTS("ACE::ACE_Control_Block");
		LOGAFTERCOTS("ACE::ACE_Null_Mutex");
		LOGAFTERCOTS("ACE::ACE_MMAP_MEMORY_POOL");
		LOGAFTERCOTS("ACE::ACE_Allocator_Adapter");
		int nError = ACE_OS::last_error();
		if (nError==0)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo," new ALLOCATOR, last_error=%d", nError);
		}
		else
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"new ALLOCATOR, last_error=%d", nError);
		}
	}
	catch(...)
	{
...
int TA_Base_Core::Shmem::mapSharedMem(const char* mapfile /*= "C:/Shared.map"*/, bool attempDeleteOldFile /*= true*/)
{
	int ret = -1;
#ifdef WIN32

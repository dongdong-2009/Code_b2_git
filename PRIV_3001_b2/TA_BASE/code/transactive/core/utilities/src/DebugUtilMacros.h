...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/core/utilities/src/DebugUtilMacros.h $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    inline void gLogExceptionDetails(const char* file, int line, const std::string& name, const std::string& details)
    {
        TA_Base_Core::DebugUtil::getInstance().log(file, line, DebugUtil::ExceptionDetails, name.c_str(), details.c_str());
    }

    inline void gLogExceptionCatch(const char* file, int line, const std::string& name, const std::string& description)
    {

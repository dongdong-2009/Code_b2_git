...
// 	}
	void RealTimeEventQueue::prepareDataForAnEvent(EventItemInfo * eventItem)
	{
		...
		assign(event->eventTypeName, eventTypeName.c_str(), EV_EVENT_TYPE_NAME_SIZE);
		...
		else //sourceTable==EventConstants::SOURCE_TABLE_EVENT
		{
			...
			if(alarmId == "")  //for non-alarm event
			{
 			...
 			assign(event->value,  value.c_str(), EV_ALARM_VALUE_SIZE);
		}
	...
	size_t RealTimeEventQueue::assign(char buf[], const char* value, size_t bufSize)
	{

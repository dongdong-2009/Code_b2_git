...
* in any form.
*
* Source:   $File: //depot/PRIV_3001/TA_BASE/transactive/core/database/src/OCIConnection.cpp $
* @author:  Ripple
* @version: $Revision: #1 $
*
...
namespace TA_Base_Core
{
	...
	*/
	...
	void OCIConnection::exec( const SQLStatement& rSqlObj, IQueryAdapter*& pIQueryAdapter, bool isQuery /*= false*/, int prefetch /*= 10*/)
	{
		...
		_LogSQLInfo(SourceInfo, DebugUtil::DebugSQL, "Begin execute the LargeSQL statement:", strSql);
		...
		try
		{
			...
			else //like the insert command
			{
				...
				_LogSQLInfo(SourceInfo, DebugUtil::DebugDebug, "The SQL modification to execute is:", strSql);
		...
		catch(ocl::OraException & e)
		{
			...
			strLogInfo = "Failed to exec SQLID:" + rSqlObj.strSQLID + ", SQL:";
			_LogSQLInfo(SourceInfo, DebugUtil::DebugError, strLogInfo, strSql);
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
				"OCI_ERROR: ocl::OraException errormsg=%s, errorcode=%d", e.message().c_str(), e.errorCode());
			DBException db("OCI_ERROR", e.message().c_str(), e.errorCode());
		...
		catch(ocl::CRException &e)
		{
			...
			_LogSQLInfo(SourceInfo, DebugUtil::DebugError, strLogInfo, strSql);
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
				"OCI_ERROR: ocl::CRException errormsg=%s, errorcode=%d", e.message().c_str(), 0);
			DBException db("OCL_ERROR", e.message().c_str(), 0);
		...
		catch(...)
		{
			...
			strLogInfo = "Failed to exec SQLID:" + rSqlObj.strSQLID + ", SQL:";
			_LogSQLInfo(SourceInfo, DebugUtil::DebugError, strLogInfo, strSql);
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "OCL_ERROR: unknown database exception");
			DBException db("OCIConnection_ERROR", "unknown database exception", 0);
			delete query; query = NULL;
			throw db;
		}	
...
// 	}
	...
	void OCIConnection::_LogSQLInfo(const char* pszFileName, int nLineNumber, TA_Base_Core::DebugUtil::EDebugLevel nLogLevel,

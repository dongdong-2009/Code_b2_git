/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/core/naming/src/NamedObject.h $
...
namespace TA_Base_Core
{
    ...
    template <typename T> void clearNamedObject(T& obj) { obj.clear();}
    template <typename T> void clearNamedObject(T* obj) { obj->clear(); }
    template <typename T> void clearNamedObject(boost::shared_ptr<T> obj) { obj->clear(); }
    template <typename T> std::string getNamedObjectDescription(const T& obj) { return obj.getDescription();}
    template <typename T> std::string getNamedObjectDescription(T* obj) { return obj->getDescription();}
    template <typename T> std::string getNamedObjectDescription(boost::shared_ptr<T> obj) { return obj->getDescription();}
...
#define CORBACALL(statement, namedObject)                                                                                                                               \
    ...
    {                                                                                                                                                                   \
        ...
        try                                                                                                                                                             \
        {                                                                                                                                                               \
            LOG_GENERIC(SourceInfo, DebugUtil::DebugCORBA, "CORBACALL BEGIN [%s]: %s", getNamedObjectDescription(namedObject).c_str(), #statement);                     \
            statement;                                                                                                                                                  \
            LOG_GENERIC(SourceInfo, DebugUtil::DebugCORBA, "CORBACALL END   [%s]: %s", getNamedObjectDescription(namedObject).c_str(), #statement);                     \
        }                                                                                                                                                               \
        ...
        {                                                                                                                                                               \
            ...
            clearNamedObject(namedObject);                                                                                                                              \
            statement;                                                                                                                                                  \
        }                                                                                                                                                               \
        catch (CORBA::TIMEOUT& e)                                                                                                                                       \
        {                                                                                                                                                               \
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::TIMEOUT", CorbaUtil::getInstance().exceptionToString(e).c_str());                                                   \
            throw;                                                                                                                                                      \
        }                                                                                                                                                               \
        catch (CORBA::SystemException& e)                                                                                                                               \
        {                                                                                                                                                               \
            LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException", CorbaUtil::getInstance().exceptionToString(e).c_str());                                           \
            clearNamedObject(namedObject);                                                                                                                              \
            statement;                                                                                                                                                  \
        }                                                                                                                                                               \
    } while(0)

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/core/data_access_interface/entity_access/src/Console.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
	...
	const std::string Console::CCTV_CLIENT_PORT = "CCTVClientPort";
    ...
    unsigned long Console::getCCTVClientPort()
    {
		std::string value = getHelper()->getParameter(CCTV_CLIENT_PORT);
		unsigned long port = 0;
		if (!value.empty())
		{
			port = EntityHelper::getUnsignedLongData(value);

  ...
  * in any form.
  *
  * Source: $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/core/data_access_interface/radio/src/RAPatchMembers.cpp $
  * @author Glen Kidd
  * @version $Revision: #1 $
...
#include "core/data_access_interface/radio/src/RAPatchMembers.h"
...
namespace TA_Base_Core
{
    ...
    RAPatchMembers::RAPatchMembers(const unsigned long key)
        : m_key(key)
    {
    }


    RAPatchMembers::~RAPatchMembers() 
	{
    }


    std::string RAPatchMembers::getSubscriberName()
    {
    ...
    std::string RAPatchMembers::getLocation()
    {
		// If the data isn't written & the key isn't set then we must reload
...
#if 0
    IRAPatchMembers::SubscriberType RAPatchMembers::getSubscriberType()
	{
		// If the data isn't written & the key isn't set then we must reload
		if ( ! m_isWritten && m_subscriberType == IRAPatchMembers::SubscriberType::INVALID)
		{
			TA_ASSERT( true, "The [key] must be set before it can be retrieved" );
		}
    ...
    unsigned long RAPatchMembers::getLocationKey()
	{
		// If the data isn't written & the key isn't set then we must reload
    ...
    void RAPatchMembers::invalidate()
    {
        m_isValidData = false;  
    }

    void RAPatchMembers::reload()
    {

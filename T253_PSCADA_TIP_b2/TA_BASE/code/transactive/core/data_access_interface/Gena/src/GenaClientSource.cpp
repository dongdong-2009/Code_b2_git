...
namespace TA_Base_Core 
{
	...
	//Server Connection Status Datapoint Address
	void GenaClientSource::setDpAddress(const std::string& dpAddress)
	{
		FUNCTION_ENTRY("setDpAddress");
		reload();

		if (m_dpAddress != dpAddress)
		{
			m_dpAddress = dpAddress;
			m_isDpAddressModified = true;
		}
		FUNCTION_EXIT;
	}

	std::string GenaClientSource::getDpAddress()
	{
		FUNCTION_ENTRY("getDpAddress");
		reload();
	...
	void GenaClientSource::reloadInternal(const unsigned long rowIdx, TA_Base_Core::IData& data)
	{
		...
		std::string dpAddressColumn = "a.DPADDRESS";
		...
		m_dpAddress = data.getStringData(rowIdx,dpAddressColumn);
	...
	void GenaClientSource::reload()
	{
		...
		columnNames.push_back(dpAddressColumn);
	...
	void GenaClientSource::createNew()
	{
		...
		databaseConnection->prepareSQLStatement(formatSQL, GENA_CLIENT_SOURCE_INSERT_170002,
			...
			databaseConnection->escapeAQSQLString(m_dpAddress));
	...
	void GenaClientSource::updateExisting()
	{
		...
		if( m_isEntityKeyModified || m_isServerURLModified || m_isMaxRetryModified || 
		    m_isTimeOutModified || m_isKeepAliveModified || m_isDpAddressModified )
		{
			// Get the database connection
			IDatabase* databaseConnection =
					TA_Base_Core::DatabaseFactory::getInstance().getDatabase(Signalling_Cd, Write);
			...
			//Datapoint Address
			if (m_isDpAddressModified)
			{
				sqlParams.add(std::string((firstField ? "" : ", ")));
				sqlParams.add(std::string("DPADDRESS"), databaseConnection->escapeAQSQLString(m_dpAddress), enumValType_STR, enumOperKey_EQUAL, true);
				firstField = false;
			}

...
namespace TA_Base_Core
{
    ...
    {
        LOG_INFO("CorbaUtil::initialise(): will attempt to pin port %d", port);
        ...
        try
        {
            ...
            if (CORBA::is_nil(m_orbVar))
            {
                int argc = 0;
                char* argv[] = { "NULL" };
                std::string portStr = str(boost::format("giop:tcp::%s") % (0 < port ? boost::lexical_cast<std::string>(port) : ""));
                ...
                std::string endPointPublish = getRunParamValue("EndPointPublish", "all(hostname)");

                const char* options[][2] =
                {
                    { "endPoint", portStr.c_str() },
                    { "endPointPublish", endPointPublish.c_str() },
                    { "giopMaxMsgSize", GIOP_MAX_MSG_SIZE },
                    { "clientCallTimeOutPeriod", timeout.c_str() },
                    { "serverCallTimeOutPeriod", timeout.c_str() },
                    { "clientTransportRule", "* unix,ssl,tcp,ziop" },
                    { "serverTransportRule", "* unix,ssl,tcp,ziop" },
                    { 0, 0 }
                };
                ...
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                            ...
                            " endPointPublish = %s\n"
                            " giopMaxMsgSize = %s\n"
                            " clientCallTimeOutPeriod = %s\n"
                            " serverCallTimeOutPeriod = %s\n"
                            " clientTransportRule = * unix,ssl,tcp,ziop\n"
                            " serverTransportRule = * unix,ssl,tcp,ziop",
                            portStr.c_str(), endPointPublish.c_str(), GIOP_MAX_MSG_SIZE, timeout.c_str(), timeout.c_str());
                ...
                if (RunParams::getInstance().isSet("WithOmniLog"))
                {
                    ...
                    else
                    {
                        ...
                        LOG_INFO("CorbaUtil::initialise(): omni will write to a separate log file "
                ...
                {
                    omniORB::setLogFunction(writeOmniToDbgLog);
                    omniORB::traceLevel = TA_Base_Core::getRunParamValue("OmniLogLevel", 40);
            ...
            {
                LOG_WARNING("CorbaUtil::Initialise() - Orb already initialised. Unable to initialise on port %d", port);
            }
        }
    ...
    void CorbaUtil::activate()
    {
        ...
        if (! m_activated)
        {
            LOG_DEBUG("CorbaUtil::activate(): activating ORB");
    ...
    void CorbaUtil::createPolicyList(PortableServer::POA_ptr poa, CORBA::PolicyList& list)
    {
...

        {
            ...
            ids[0].compression_level = 9;

            list[4] = omniZIOP::create_compression_enabling_policy(1);
            list[5] = omniZIOP::create_compression_id_level_list_policy(ids);

            omniZIOP::setGlobalPolicies(list);
        }
    }
    ...
    void CorbaUtil::shutdown()
    {
        try
        {
            LOG_DEBUG("CorbaUtil::shutdown(): shutting ORB down");
    ...
    CORBA::Object_ptr CorbaUtil::stringToObject(const std::string& uri)
    {
        std::string uri_v12 = uri;

        if (uri_v12.find("corbaloc::1.2@") == std::string::npos)
        {
            boost::algorithm::replace_first(uri_v12, "corbaloc::", "corbaloc::1.2@");
            boost::algorithm::replace_first(uri_v12, ",:", ",:1.2@");
        }

        CORBA::Object_var obj = getOrb().string_to_object(uri_v12.c_str());
        return enableZIOP(obj);
    }
    ...
    void CorbaUtil::setServantKey(ServantBase* theServant, const std::string& key)
    {
        ...
        LOG_DEBUG("CorbaUtil::setServantKey(): setting servant key of %s to %s", gPtrToStr(theServant).c_str(), key.c_str());
        ...
        try
        {
            ...
            LOG_INFO("CorbaUtil::setServantKey(): %s activated with key %s", gPtrToStr(theServant).c_str(), key.c_str());
        }
        ...
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "PortableServer::POA::ServantAlreadyActive", exceptionToString(ex));
        }
        catch (const PortableServer::POA::ObjectAlreadyActive& ex)
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "PortableServer::POA::ObjectAlreadyActive", exceptionToString(ex));
        }
        catch (const PortableServer::POA::WrongPolicy& ex)
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "PortableServer::POA::WrongPolicy", exceptionToString(ex));
            throw;
        }
    }
...
//#if defined( SOLARIS )  (Define _stricmp except for _WIN32)
#if !defined ( _WIN32 )
    ...
    std::string CorbaUtil::getObjectAddress(CORBA::Object_ptr obj)
    {
        try
        {
            ...
            return getAddressFromIOR(iorstr.in());
        }
        catch (...)
        {
        }
    ...
    std::string CorbaUtil::getAddressFromIOR(const std::string& iorstr)
    {
        size_t s = iorstr.size();
        const char* p = iorstr.c_str();
        s = (s - 4) / 2;
        p += 4;
        ...
        buf.rewindInputPtr();
        buf.setByteSwapFlag(buf.unmarshalBoolean());
        ...
        for (size_t i = 0; i < ior.profiles.length(); ++i)
        {
            ...
            {
                IIOP::unmarshalMultiComponentProfile(ior.profiles[i], pBody.components);
            }

            strm << (i ? "," : "") << (const char*)pBody.address.host << ":" << pBody.address.port;

            for (size_t index = 0; index < pBody.components.length(); index++)
            {
                IOP::TaggedComponent& c = pBody.components[index];

                if (c.tag == 3) // IOP:TAG_ALTERNATE_IIOP_ADDRESS
                {
    ...
    CORBA::Object_ptr CorbaUtil::enableZIOP(CORBA::Object_ptr obj)
    {
        static CORBA::PolicyList pl;
        ...
        try
        {
            return omniZIOP::setServerPolicies(obj, pl);
        }
        catch (const CORBA::Exception& ex)
        {
            LOG_DEBUG("failed to enable ZIOP: %s", exceptionToString(ex).c_str());
        }

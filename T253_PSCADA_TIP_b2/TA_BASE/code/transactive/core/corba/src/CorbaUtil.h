...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/core/corba/src/CorbaUtil.h $
...
namespace TA_Base_Core
{
    ...
    public:
        ...
        /**
          ...
          */
        CORBA::Object_ptr stringToObject(const std::string& uri);

        template <typename CorbaDef>
        static typename CorbaDef::_ptr_type stringToObject(const std::string& uri, unsigned long timeout = 2000)
        {
            CORBA::Object_var obj = getInstance().stringToObject(uri);
            omniORB::setClientCallTimeout(obj, timeout);
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "CorbaUtil::stringToObject - %s", uri.c_str());
            return CorbaDef::_narrow(obj);
        }

        template <typename CorbaDef, typename Port>
        static typename CorbaDef::_ptr_type stringToObject(const std::string& hostname, const Port& port, const std::string& key, unsigned long timeout = 2000)
        {
            std::stringstream ior;
            ior << "corbaloc::1.2@" << hostname << ":" << port << "/" << key;
            CORBA::Object_var obj = getInstance().stringToObject(ior.str());
            omniORB::setClientCallTimeout(obj, timeout);
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "CorbaUtil::stringToObject - %s", ior.str().c_str());
            return CorbaDef::_narrow(obj);
        }
        ...
        std::string getAddressFromIOR(const std::string& iorstr);
        static CORBA::Object_ptr enableZIOP(CORBA::Object_ptr obj);

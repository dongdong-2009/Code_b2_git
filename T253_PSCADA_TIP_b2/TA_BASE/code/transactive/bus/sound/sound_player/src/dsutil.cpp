/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/sound/sound_player/src/dsutil.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
#define KEY_SOUND_DEVICE "SoundDevice"
...
namespace TA_Base_Bus
{
		//-----------------------------------------------------------------------------
		// Name: DSEnumProc
		// Desc: Callback function pass to DirectSoundEnumerate to list all available
		//		 sound devices.
		//-----------------------------------------------------------------------------
		BOOL CALLBACK DSEnumProc(LPGUID lpGUID, 
			...
			LPVOID lpContext )
		{
			T_DEVICE_GUID* pDeviceList = (T_DEVICE_GUID*)lpContext;
			LPGUID lpTemp = NULL;
										   ...
										   DWORD dwPrimaryBitRate )
		{
			...
			if (FAILED(hr = DirectSoundEnumerate((LPDSENUMCALLBACK)DSEnumProc,
				(VOID*)&m_objDeviceList)))
			{
				return hr;
			}
			...
			//Query configured sound device output.
			HKEY hKey;
			DWORD dwLenght = 256;
			TCHAR* pszData = new TCHAR[dwLenght];

			if( RegOpenKeyEx(HKEY_CLASSES_ROOT,
				"AppID\\TA_SoundManager.exe"
				,0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
			{
				//Query registry for a configured sound device
				if (FAILED(hr = DXUtil_ReadStringRegKey( 
					hKey, 
					KEY_SOUND_DEVICE, 
					pszData, dwLenght, DEFAULT_SOUND_DEVICE )))
				{
					LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "No target sound device is configure and use system default device");
				}
				else
				{
					LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Target Sound device is configure with %s", pszData);
					
					T_DEVICE_GUID::iterator itr = m_objDeviceList.begin();
					for (itr; itr!=m_objDeviceList.end(); itr++)
					{
						if (itr->first.find(pszData)!= -1)
						{ //Device is found
							devGuid = itr->second;
							LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Target sound device found on the device list");
							break;
						}
					}
					if ((NULL == devGuid) && (strcmp(pszData, DEFAULT_SOUND_DEVICE) != 0))
					{
						LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Target sound device not found on the device list");
					}
				}
			}
			RegCloseKey(hKey);

			LOGBEFORECOTS("DirectSoundCreate8" );			
			// Create IDirectSound using the primary sound device
			if( FAILED( hr = DirectSoundCreate8( devGuid, &m_pDS, NULL ) ) )
            {
                if (!m_initFailed)
                {
                    m_initFailed = true;
                    TA_THROW(TA_Base_Core::SoundPlayerException("Error initialising DirectSound"));
                }

				return hr;//DXTRACE_ERR( TEXT("DirectSoundCreate8"), hr );
            }
            LOGAFTERCOTS("DirectSoundCreate8");
			// Set DirectSound coop level 

			LOGBEFORECOTS("SetCooperativeLevel" );
			if( FAILED( hr = m_pDS->SetCooperativeLevel( hWnd, dwCoopLevel ) ) )
				return hr;//DXTRACE_ERR( TEXT("SetCooperativeLevel"), hr );
			LOGAFTERCOTS("SetCooperativeLevel");
													   ...
													   DWORD dwPrimaryBitRate )
		{
			...
			LOGBEFORECOTS("CreateSoundBuffer" );			
			if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &pDSBPrimary, NULL ) ) )
				return hr;//DXTRACE_ERR( TEXT("CreateSoundBuffer"), hr );
            LOGAFTERCOTS("CreateSoundBuffer");
			WAVEFORMATEX wfx;
									   ...
									   DWORD dwNumBuffers )
		{
			...
			if( FAILED(hr) )
			{
				...
				if( hr != DSERR_BUFFERTOOSMALL )
                    ...
                    // Removed error message box when buffer creation unsuccessful - 02/09/03 (NickJ)
			...
			for( i=1; i<dwNumBuffers; i++ )
			{
				...
				LOGBEFORECOTS("DuplicateSoundBuffer" );				
				...
				LOGAFTERCOTS("DuplicateSoundBuffer");
			}
												...
												HANDLE hNotifyEvent )
		{
			...
			if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &pDSBuffer, NULL ) ) )
			{
				...
				if( hr == DSERR_BADFORMAT || hr == E_INVALIDARG )
					return hr;//DXTRACE_ERR_NOMSGBOX( TEXT("CreateSoundBuffer"), hr );
		...
		HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags )
		{
			...
			if( pDSB == NULL )
				return E_FAIL;//DXTRACE_ERR( TEXT("GetFreeBuffer"), E_FAIL );
			...
			LOGBEFORECOTS("Play" );	
			HRESULT res;
			res= pDSB->Play( 0, dwPriority, dwFlags );
			LOGAFTERCOTS("Play");
			return res;
				...
				if( NULL == m_hmmio )
				{
					...
					if( NULL == ( hResInfo = FindResource( NULL, strFileName, TEXT("WAVE") ) ) )
					{
						if( NULL == ( hResInfo = FindResource( NULL, strFileName, TEXT("WAV") ) ) )
							return E_FAIL;//DXTRACE_ERR_NOMSGBOX( TEXT("FindResource"), E_FAIL );
					}

					if( NULL == ( hResData = LoadResource( NULL, hResInfo ) ) )
						return E_FAIL;//DXTRACE_ERR( TEXT("LoadResource"), E_FAIL );

					if( 0 == ( dwSize = SizeofResource( NULL, hResInfo ) ) ) 
						return E_FAIL;//DXTRACE_ERR( TEXT("SizeofResource"), E_FAIL );

					if( NULL == ( pvRes = LockResource( hResData ) ) )
						return E_FAIL;//DXTRACE_ERR( TEXT("LockResource"), E_FAIL );

					m_pResourceBuffer = new CHAR[ dwSize ];
					ZeroMemory(m_pResourceBuffer, dwSize);

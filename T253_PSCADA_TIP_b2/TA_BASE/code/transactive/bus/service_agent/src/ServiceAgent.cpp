/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/service_agent/src/ServiceAgent.cpp $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2018/01/19 11:00:48 $
	...
	void ServiceAgent::locateSystemController()
	{
        ...
        for (int i = 0;i < retries;i++)
        {
		    ...
		    while( (iter != hostNames.end()) && (!haveFoundSystemController) )
		    {
			    ...
			    sprintf(procManString, "corbaloc::1.2@%s:%lu/ProcessManager", iter->c_str(), portNumber);

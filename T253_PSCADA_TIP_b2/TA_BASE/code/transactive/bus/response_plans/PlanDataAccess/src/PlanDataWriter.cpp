/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/response_plans/PlanDataAccess/src/PlanDataWriter.cpp $
  *  @author:  Mahaveer Pareek
  *  @version: $Revision: #4 $
  *
  *  Last modification: $DateTime: 2018/02/22 11:31:20 $
...
//		std::string sql = dataBase->prepareSQLStatement(STEP_DETAILES_UPDATE_63509);
		...
		dataBase->prepareSQLStatement(sql, PLAN_STEP_DETAILES_UPDATE_63509);
    ...
    std::string PlanDataWriterImp::getEditLockOwner(unsigned long plan)
    {
		TA_Base_Core::IDatabase* database = getDatabase();
...
//		std::string strSql  = database->prepareSQLStatement(NODE_TREE_SELECT_33503, plan);
		...
		database->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33503, plan);
	...
	bool PlanDataWriterImp::isPlanBeingEdited(unsigned long plan)
	{
		TA_Base_Core::IDatabase* database = getDatabase();
       /* std::ostringstream sql;
...
//		std::string strSql  = database->prepareSQLStatement(NODE_TREE_SELECT_33551, 
		TA_Base_Core::SQLStatement strSql;
		database->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33551,
    ...
    std::string PlanDataWriterImp::setEditLockInDataBase (const std::string& session, unsigned long plan )
    {
		TA_Base_Core::IDatabase* database = getDatabase();
...
//		std::string strSql = database->prepareSQLStatement(NODE_TREE_SELECT_33504, 
		TA_Base_Core::SQLStatement strSql;
		database->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33504,
...
//		strSql = database->prepareSQLStatement(NODE_TREE_SELECT_33503, plan);
    ...
    void PlanDataWriterImp::releaseEditLockInDatabase(const std::string& session, unsigned long plan )
    {
		TA_Base_Core::IDatabase* database = getDatabase();
...
//		std::string strSelectSql = database->prepareSQLStatement(NODE_TREE_SELECT_33552, plan);
		...
		database->prepareSQLStatement(strSelectSql, PLAN_NODE_TREE_SELECT_33552, plan);
...
//		std::string strUpdSql = database->prepareSQLStatement(NODE_TREE_UPDATE_33651, plan);
		...
		database->prepareSQLStatement(strUpdSql, PLAN_NODE_TREE_UPDATE_33651, plan);
    ...
    std::string PlanDataWriterImp::getPlanEditSession(unsigned long plan, const std::string& session)
	{
		TA_Base_Core::IDatabase* database = getDatabase();
       /* std::ostringstream sql;
...
//		std::string strSql  = database->prepareSQLStatement(NODE_TREE_SELECT_33552, plan);
		TA_Base_Core::SQLStatement strSql;
		database->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33552, plan);
...
//		std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_UPDATE_33652, 
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_UPDATE_33652,
			detail.nodeType, parseForApostrophe(detail.name.in()), parseForApostrophe(detail.description.in()), detail.approvalState,
			getOperatorName(session), detail.plan);

		dataBase->executeModification(strSql);
	}

    void PlanDataWriterImp::copyPlan( const std::string& session, const unsigned long planToCopy, const unsigned long parent, const std::string planNewName )
    {
        TA_Base_Core::IDatabase* pDb = getDatabase();			
        //std::ostringstream sql;
        TA_Base_Core::SQLStatement strSql;

        pDb->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_UPDATE_63502, planToCopy, parent,
            parseForApostrophe(planNewName), getOperatorName(session));

        pDb->executeProcedure(strSql);
    }

    TA_Base_Core::AuxPlanDetail* PlanDataWriterImp::exportPlan(const std::string& session, unsigned long& planId)
    {
...
//		 std::string strSql = database->prepareSQLStatement(NODE_TREE_SELECT_33553, nvlValue, planId);
        TA_Base_Core::SQLStatement strSql;
        database->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33553, nvlValue, planId, planId);
...
//		 strSql = database->prepareSQLStatement(STEP_DETAILES_SELECT_63551, nvlValue, planId);
        ...
        database->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_SELECT_63551, nvlValue, planId);
        ...
        do
        {
            ...
            for( unsigned long dataIdx = 0; dataIdx < dataPtr->getNumRows(); dataIdx++ )
            {
                ...
                refPlanStep.id = dataPtr->getUnsignedLongData( dataIdx, columns[nIndex++]);
                refPlanStep.name = dataPtr->getStringData( dataIdx, columns[nIndex++] ).c_str();
                refPlanStep.type = dataPtr->getUnsignedLongData( dataIdx, columns[nIndex++]);
                ...
                refPlanStep.graph = dataPtr->getStringData(dataIdx, columns[nIndex++]).c_str();
            }
		...
		database->prepareSQLStatement(strSql, PLAN_STEP_PARAMETERS_SELECT_63051, nvlValue, planId);
		...
		// Plan sequence flow
        std::vector<std::string> seqColumns;
        seqColumns.push_back( "P_KEY" );
        seqColumns.push_back( "ID" );
        seqColumns.push_back( "INCOMING_STEP" );
        seqColumns.push_back( "OUTGOING_STEP" );
        seqColumns.push_back( "CONDITION" );
        seqColumns.push_back( "GRAPH" );

        database->prepareSQLStatement(strSql, PLAN_SEQUENCE_FLOW_SELECT_81001, planId);

        TA_Base_Core::IData* seqData = database->executeQuery(strSql, seqColumns);
        ...
        do
        {
            ...
            std::auto_ptr<TA_Base_Core::IData> dataPtr( seqData );

			TA_Base_Core::AuxSequenceFlows& refSeqFlows = auxPlanDetail->sequenceFlows;

            refSeqFlows.length( dataPtr->getNumRows() + rowsFetched );

            for( unsigned long dataIdx = 0, sz = dataPtr->getNumRows(); dataIdx < sz; dataIdx++ )
            {
                int nColLoop = 1;
                refSeqFlows[rowsFetched + dataIdx].id = dataPtr->getUnsignedLongData( dataIdx, seqColumns[nColLoop++] );
                refSeqFlows[rowsFetched + dataIdx].incomingStep = dataPtr->getUnsignedLongData( dataIdx, seqColumns[nColLoop++] );
                refSeqFlows[rowsFetched + dataIdx].outgoingStep = dataPtr->getUnsignedLongData( dataIdx, seqColumns[nColLoop++] );
                refSeqFlows[rowsFetched + dataIdx].condition = dataPtr->getStringData( dataIdx, seqColumns[nColLoop++] ).c_str();
                refSeqFlows[rowsFetched + dataIdx].graph = dataPtr->getStringData( dataIdx, seqColumns[nColLoop++] ).c_str();
            }

            rowsFetched += dataPtr->getNumRows();
            dataPtr.reset();
        }
        while ( database->moreData( seqData ));

        return auxPlanDetail._retn();
    }

	unsigned long PlanDataWriterImp::importPlan (const std::string& session, const TA_Base_Core::AuxPlanDetail& plan, unsigned long parentId)
	{
...
//		std::string strSql = dataBase->prepareSQLStatement(STEP_DETAILES_UPDATE_63503, TA_Base_Core::PLAN_NODE,
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_UPDATE_63503, TA_Base_Core::PLAN_NODE,
...
//		strSql = dataBase->prepareSQLStatement(NODE_TREE_SELECT_33505, parentId, name);
		...
		dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33505, parentId, name);
		...
		//get the new plan id
		unsigned long planId = data->getUnsignedLongData( 0, columns[0] );

		//now update plan step details
		for ( unsigned int i = 0;  i < plan.steps.length () ; i++ )
		{	
			//std::ostringstream sqlstep;
			unsigned long stepId = TA_Base_Bus::PlanDataReader::instance()->getNextPlanSeqNumber();
...
//			strSql = dataBase->prepareSQLStatement(STEP_DETAILES_INSERT_63801, stepId, plan.steps[i].type, planId,
		    ...
		    dataBase->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_INSERT_63801, stepId, plan.steps[i].type, planId,
                plan.steps[i].position, plan.steps[i].id, parseForApostrophe(plan.steps[i].description.in()), plan.steps[i].skip,
                plan.steps[i].skippable, plan.steps[i].delay, parseForApostrophe(plan.steps[i].name.in()),
                plan.steps[i].ignoreFailure, parseForApostrophe(plan.steps[i].graph.in()));
...
//				strSql = dataBase->prepareSQLStatement( STEP_PARAMETERS_INSERT_63351, stepId, 
		        ...
		        dataBase->prepareSQLStatement(strSql,  PLAN_STEP_PARAMETERS_INSERT_63351, stepId,
		...
		// Plan sequence flow
		for(unsigned int i = 0, sz = plan.sequenceFlows.length(); i < sz; ++i)
		{
			const TA_Base_Core::AuxSequenceFlow& seqFlow = plan.sequenceFlows[i];
			unsigned long pkey = TA_Base_Bus::PlanDataReader::instance()->getNextPlanSeqNumber();
			dataBase->prepareSQLStatement(strSql, PLAN_SEQUENCE_FLOW_INSERT_81004, pkey, planId, seqFlow.id,
				seqFlow.incomingStep, seqFlow.outgoingStep, seqFlow.condition.in(), seqFlow.graph.in());
			dataBase->executeModification(strSql);
		}
        return planId;
	}
	...
	unsigned long PlanDataWriterImp::insertPlan(const std::string& session, const unsigned long parent, TA_Base_Core::ENode nodeType)
	{
		ACE_Write_Guard<ACE_RW_Mutex> guard(m_planInsertLock);
		std::string name = getNewNodeName(parent, nodeType);
...
//		std::string strSql = dataBase->prepareSQLStatement( STEP_DETAILES_UPDATE_63503, 
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql,  PLAN_STEP_DETAILES_UPDATE_63503,
			nodeType, parent, name, description, approvalState, getOperatorName(session), 1);
		dataBase->executeProcedure(strSql);
...
//		strSql = dataBase->prepareSQLStatement( NODE_TREE_SELECT_33505, parent, name);
		...
		dataBase->prepareSQLStatement(strSql,  PLAN_NODE_TREE_SELECT_33505, parent, name);
    ...
    bool PlanDataWriterImp::insertPlanType( const std::string& session, const unsigned long plan, TA_Base_Core::EPlanType planType)
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
		TA_Base_Core::SQLStatement strSql;
    ...
    void PlanDataWriterImp::updateCategory( const std::string& session, const std::string& name, const std::string& description, const unsigned long category )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();

        //		std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_UPDATE_33654, name, description, category);
        TA_Base_Core::SQLStatement strSql;
        dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_UPDATE_33654, name, description, category);

        dataBase->executeModification(strSql);
    }


    unsigned long PlanDataWriterImp::insertCategory(const std::string& session, unsigned long parent)
	{
...
//		std::string strSql = dataBase->prepareSQLStatement( NODE_TREE_INSERT_33851, TA_Base_Core::CATEGORY_NODE,
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql,  PLAN_NODE_TREE_INSERT_33851, TA_Base_Core::CATEGORY_NODE,
...
//		strSql = dataBase->prepareSQLStatement( NODE_TREE_SELECT_33505, parent, name);
    ...
    void PlanDataWriterImp::deleteCategory( const unsigned long category )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        ...
        TA_Base_Core::SQLStatement strSql;
        dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_DELETE_33901, category);

        dataBase->executeModification(strSql);
    }


    void PlanDataWriterImp::setPlanApprovalState( const std::string& session, const unsigned long plan, TA_Base_Core::EApprovalState approvalState )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        TA_Base_Core::SQLStatement strSql;

        dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_UPDATE_33701, ( approvalState + 1 ),
	...
	std::string PlanDataWriterImp::getNodeNameToCopy(unsigned long node, unsigned long parent)
	{
		TA_Base_Core::IDatabase* dataBase = getDatabase();
...
//		std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_SELECT_33506, parent,
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33506, parent,
	...
	std::string PlanDataWriterImp::getNewNodeName(unsigned long parent, TA_Base_Core::ENode nodeType)
	{
		TA_Base_Core::IDatabase* dataBase = getDatabase();

		std::string nodeName = EMPTY_STRING;
		...
		else if (nodeType == TA_Base_Core::WORKFLOW_NODE)
		{
			nodeName = NewPlanName;
		}
...
//		std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_SELECT_33507, parent, nodeType, nodeName);
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_SELECT_33506, parent, nodeName, nodeName);
		...
		while (!foundNewName)
		{
			for( unsigned long dataIdx = 0; dataIdx < nRows; dataIdx++ )
			{
				name = data->getStringData( dataIdx, columns[0] );
				int result = name.compare(newName);
    ...
    unsigned long PlanDataWriterImp::getStepNumber(const TA_Base_Core::Plan& plan, unsigned long pkey) const
    {
    ...
    void PlanDataWriterImp::updatePlanType( unsigned long nodeId, TA_Base_Core::EPlanType visibleState )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
    ...
    void PlanDataWriterImp::deletePlanType( unsigned long nodeId )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
    ...
    void PlanDataWriterImp::deleteAllStepParameters( const std::string& session, const unsigned int plan )
    {
		TA_Base_Core::IDatabase* dataBase = getDatabase();
...
//		std::string strDelSql1 = dataBase->prepareSQLStatement(STEP_PARAMETERS_DELETE_63401, plan.detail.plan); 
		TA_Base_Core::SQLStatement strDelSql;
		dataBase->prepareSQLStatement(strDelSql, PLAN_STEP_PARAMETERS_DELETE_63401, plan);

		dataBase->executeModification(strDelSql); //delete all step parameters
    }

    void PlanDataWriterImp::deleteAllSteps( const std::string& session, const unsigned int plan )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        ...
        TA_Base_Core::SQLStatement strDelSql;
        dataBase->prepareSQLStatement(strDelSql, PLAN_STEP_DETAILES_DELETE_63901, plan);

        dataBase->executeModification(strDelSql); //delete all steps
    }

    void PlanDataWriterImp::deleteAllSequenceFlows( const std::string& session, const unsigned int plan )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();

        TA_Base_Core::SQLStatement strDelSql;
        dataBase->prepareSQLStatement( strDelSql, PLAN_SEQUENCE_FLOW_DELETE_81006, plan );

        dataBase->executeModification(strDelSql); //delete all steps
    }

    void PlanDataWriterImp::insertStepDetail( const unsigned int plan, const TA_Base_Core::StepDetail& detail )
    {
        ...
        dataBase->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_INSERT_63801, detail.pkey, detail.type,
            plan, detail.position, detail.id, parseForApostrophe(detail.description.in()), detail.skip,
            detail.skippable, detail.delay, parseForApostrophe(detail.name.in()), detail.ignoreFailure,
            parseForApostrophe(detail.graph.in()));

        dataBase->executeModification(strSql);
    }

    void PlanDataWriterImp::insertSequenceFlow( const unsigned int plan, const TA_Base_Core::SequenceFlow& detail )
    {
        // Update the details of the plan
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        ...
        dataBase->prepareSQLStatement( strSql, PLAN_SEQUENCE_FLOW_INSERT_81004, detail.pkey, plan, detail.id, detail.incomingStep, detail.outgoingStep, detail.condition.in(), detail.graph.in() );

        dataBase->executeModification(strSql);
    }

    void PlanDataWriterImp::insertStepParameters( const unsigned int stepKey, const std::vector<std::string>& parameters)
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        TA_Base_Core::SQLStatement strSql;
		...
		{
	        dataBase->prepareSQLStatement(strSql, PLAN_STEP_PARAMETERS_INSERT_63351, stepKey, position++,  parseForApostrophe(*it));

			dataBase->executeModification(strSql);
		}
    }

    void PlanDataWriterImp::replacePlan( const std::string& session, const unsigned long existingPlan, const unsigned long planToOverwrite )
    {
        TA_Base_Core::IDatabase* pDb = getDatabase();			
        TA_Base_Core::SQLStatement strSql;

        pDb->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_UPDATE_63501, existingPlan, planToOverwrite,
            getOperatorName(session));

        pDb->executeProcedure(strSql);
    }

    void PlanDataWriterImp::updateNodeParent( const std::string& session, const unsigned long parent, const unsigned long plan )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
...
//			std::string strSql  = dataBase->prepareSQLStatement(NODE_TREE_UPDATE_33653, parent, plan);
		TA_Base_Core::SQLStatement strSql;
		dataBase->prepareSQLStatement(strSql, PLAN_NODE_TREE_UPDATE_33653, parent, plan);

	    dataBase->executeModification(strSql);

    }

    void PlanDataWriterImp::deletePlan( const std::string& session, const unsigned long plan )
    {
        TA_Base_Core::IDatabase* dataBase = getDatabase();
        TA_Base_Core::SQLStatement strSql;

        dataBase->prepareSQLStatement(strSql, PLAN_STEP_DETAILES_UPDATE_63505, plan);
        dataBase->executeProcedure(strSql);
    }

...
* in any form.
*
* Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/response_plans/plan_agent/src/PlanAgentLibrary.cpp $
* @author:  Ripple
* @version: $Revision: #2 $
*
...
CORBA::Boolean PlanAgentLibrary::isOwner(const char* session, const TA_Base_Core::ActivePlanId& plan)
{
	...
	CORBA_CALL_RETURN( bRet, m_planAgent, isOwner, ( session, plan ) );
	return bRet;
}

void PlanAgentLibrary::processStepResponse( const char* session, const TA_Base_Core::ActivePlanId& plan, const TA_Base_Core::InteractiveStepData& stepDetail )
{
    CORBA_CALL( m_planAgent, processStepResponse, (session, plan, stepDetail) );
}

void PlanAgentLibrary::runPlan(const char* session, TA_Base_Core::ActivePlanId& plan)
{
	CORBA_CALL( m_planAgent, runPlan, ( session, plan ) );
}
...
void PlanAgentLibrary::runOperatorTriggeredPlan(const char* session, TA_Base_Core::ActivePlanId& plan)
{
	CORBA_CALL( m_planAgent, runOperatorTriggeredPlan, ( session, plan ) );
}

void PlanAgentLibrary::customisePlan(const char* session, TA_Base_Core::ActivePlanId& plan)
{
...
void PlanAgentLibrary::takeControl(const char* session, const TA_Base_Core::ActivePlanId& plan)
{
	CORBA_CALL( m_planAgent, takeControl, ( session, plan ) );
}
...
void PlanAgentLibrary::getAllActivePlanDetails(TA_Base_Core::ActivePlanStepsDetails_out details)
{
	CORBA_CALL( m_planAgent,
...
void PlanAgentLibrary::getEditPlanLock(const char* session, TA_Base_Core::NodeId plan)
{
	CORBA_CALL( m_planAgent, getEditPlanLock, ( session, plan ) );
}
...
void PlanAgentLibrary::getSequenceFlows( TA_Base_Core::NodeId plan, TA_Base_Core::SequenceFlows_out details )
{
    CORBA_CALL( m_planAgent, getSequenceFlows, (plan, details) );
}
...
void PlanAgentLibrary::getStepPosition(TA_Base_Core::NodeId planId, TA_Base_Core::StepId stepId, TA_Base_Core::StepNumber& position)
{
	CORBA_CALL( m_planAgent, getStepPosition, ( planId, stepId, position ) );
}

void PlanAgentLibrary::getParameterNames(int stepType, TA_Base_Core::ParameterNames_out names)
{
	CORBA_CALL( m_planAgent, getParameterNames, ( stepType, names ) );
}

void PlanAgentLibrary::getParametersByStepNumber( TA_Base_Core::NodeId plan, TA_Base_Core::StepNumber position, TA_Base_Core::StepParameters_out parameters )
{
    CORBA_CALL( m_planAgent, getParametersByStepNumber, (plan, position, parameters) );
}

void PlanAgentLibrary::getParametersByStepId( TA_Base_Core::StepId step, TA_Base_Core::StepParameters_out parameters )
{
	CORBA_CALL( m_planAgent, getParametersByStepId, ( step, parameters ) );
}
...
void PlanAgentLibrary::insertPlan(const char* session, TA_Base_Core::NodeId parent, TA_Base_Core::ENode nodeType)
{
	CORBA_CALL( m_planAgent,
	            insertPlan,
	            ( session, parent, nodeType ) );
}

void PlanAgentLibrary::insertPlanIntoRoot(const char* session, TA_Base_Core::ENode nodeType)
{
	CORBA_CALL( m_planAgent,
	            insertPlanIntoRoot,
	            ( session, nodeType ) );

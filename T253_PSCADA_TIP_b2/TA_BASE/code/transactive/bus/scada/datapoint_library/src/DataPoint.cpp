/**
  ...
  * any form.
  *
  * Source:		$File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/scada/datapoint_library/src/DataPoint.cpp $
  * @author:	HoaVu
  * @version $Revision: #2 $
  *
  * Last modification : $DateTime: 2018/02/09 11:16:40 $
...
m_entityKey( dataPointEntityAccess->getKey() ),
    ...
    boost::shared_ptr<TA_Base_Bus::DataPointDisplayState> DataPoint::getDisplayState()
    {
		...
		{
			//for output datapoint, only completewithsuccess can return false.
			if (true == m_ControlInhibitedState)
			{
				//pass control state = false if control inhibit.
				dpDisplayState->alarmState = false;
			}
			else
			{
				if (m_controlCommandState != TA_Base_Bus::CompletedWithSuccess)
				{
					dpDisplayState->alarmState = true ;
				}
			}
		}
	...
	*/
	...
	// For control state to pass some information to lccResult.
	// lccResult field is not used after LCC checking finished. so we can use it to pass the information to other place.
	// Currently we use it in GZL9/13 to transmit errorcode of wf verification.
	// Agent can decide which information need to pass and how to use it.
	void DataPoint::setControlState ( TA_Base_Bus::EDataPointControlState controlStatus, std::string& strInfo, bool isRequestedBySystem ) 
	{
        FUNCTION_ENTRY("setControlState");
        TA_Base_Bus::DataPointControl toupdate;
        //only got new control status
									...
									const std::string triggeringDpValue )
	{
		...
		if ( true == deviceRequestToBeSubmitted )
		{
			...
			if (getPScadaFlag())
			{
				...
				if (CONTROL_BLINK == controlOperation)
				{
					ftime ( &currentSystemTime );
					//Just make sure CONTROL_BLINK to pass the control information for opcbridge.
					m_controlCommandState = CompletedWithError;
					saveLastOperatorInfo ( sessionID, currentSystemTime);
	...
	void DataPoint::setToStandbyMode()
	{
        ...
        if ( m_lastSavedOperationMode != m_operationMode )
        {
                ...
                (m_lastSavedOperationMode != TA_Base_Core::Monitor))
            {
			    ...
			    // a different thread
			     // m_threadPool->queueWorkItem ( this );
                DataPointWorkItem* item = new DataPointWorkItem(this);
	...
	void DataPoint::initialiseData()
	{
		...
		if (!m_abeExpression.empty() || !m_trueRccExpression.empty() || !m_falseRccExpression.empty())
		{
			m_abeTimeoutPeriodSeconds = m_dataPointEntityAccess->getABETimeoutPeriodSeconds();
		}
    ...
    DataPointWorkItem::DataPointWorkItem(DataPoint * dp)
        :
        m_dp(dp)
    {

/**
  ...
  * in any form.
  *
  * Source : $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/scada/ItaScada/src/CtaScada.cpp $
  * @author  Sean Liew
  * @version $Revision: #1 $
...
namespace TA_Base_Bus
{
	...
	*/
    ...
    /**
    * Interanl stores the datapoints that have been created for further reference.   
	*
	* @param    datapointsMap    The map stores the datapoints
	*
	*/
    void CtaScada::addAllDatapoints(std::map < unsigned long, TA_Base_Bus::DataPoint *>& dataPointList)
    {
        FUNCTION_ENTRY("CtaScada::addAllDatapoints");
        
        // for dataPointList using
        ...
        for (it = dataPointList.begin(); it != dataPointList.end(); it++)
        {
            ...
            {
                TA_Base_Core::ThreadGuard guard( m_internalDataPointEntityKeyMapLock );
                // for internal m_internalDataPointEntityKeyMap using
                std::map < unsigned long, TA_Base_Bus::DataPoint * >::iterator internalItr;
                internalItr = m_internalDataPointEntityKeyMap.find(ptrDp->getEntityKey());
            
                // if does not exist yet
                if (m_internalDataPointEntityKeyMap.end() == internalItr)
                {
                    m_internalDataPointEntityKeyMap[ptrDp->getEntityKey()] = ptrDp;
                }
            }
            
            {
                // stop multi threads accessing this list
                TA_Base_Core::ThreadGuard guard( m_internalDataPointAddressMapLock );
                // for internal InternalDataPointAddressMap using
                InternalDataPointAddressMap::iterator addrItr = m_internalDataPointAddressMap.find(ptrDp->getDataPointAddress());
                
                // if the targe has been exist
                if (addrItr != m_internalDataPointAddressMap.end())
                {
                    addrItr->second.push_back(ptrDp);
                }
                // the target has not been found
                else
                {
                    InternalDataPointVector dataPointVector;
                    dataPointVector.push_back(ptrDp);
                    m_internalDataPointAddressMap[ptrDp->getDataPointAddress()] = dataPointVector;
                }
            }
        }
        
        FUNCTION_EXIT("CtaScada::addAllDatapoints");
    }

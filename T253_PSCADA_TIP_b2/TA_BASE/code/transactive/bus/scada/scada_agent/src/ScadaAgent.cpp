...
namespace TA_Base_Bus
{
	...
	m_hasOtherBusiness(false)
	{
		...
		for (; iterDn != m_dataNodes.end(); )
		{
            ...
            // group is divided by specific numbers,
            // "||" is special handling for the last group
            if (i == j*350 || i == m_dataNodes.size())
			{
				j++;
	...
	void ScadaAgent::agentTerminate()
	{
        ...
        // because timer is singleton, the invalidate will impact golobal, so place to last
	...
	void CreateEntityWorkItem::executeWorkItem()
	{
		...
		try
        {
            // we must capture the exceptions,
            // else the m_sync cant be performed and cause m_sync->wait forever waiting
            m_parent.createEntity(m_entityData);
        }
        catch( const TransactiveException & te )
        {
            std::stringstream ss;
            ss << "TransactiveException happend when create entity [" << m_entityData->getKey() << "], the exception is: " << te.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "TransactiveException", ss.str().c_str());
        }
        catch( const std::exception & e )
        {
            std::stringstream ss;
            ss << "std::exception happend when create entity [" << m_entityData->getKey() << "], the exception is: " << e.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "std::exception", ss.str().c_str());
        }
        catch( ... )
        {
            std::stringstream ss;
            ss << "Unknown Exception happend when create entity [" << m_entityData->getKey() << "]";
            LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception ", ss.str().c_str());
        }
	...
	void InitDataPointWorkItem::executeWorkItem()
	{
        ...
        {
            std::stringstream ss;
            ss << "TransactiveException happend when init entity [" << m_dp->getEntityKey() << "], the exception is: " << te.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "TransactiveException", ss.str().c_str());
        }
        catch( const std::exception & e )
        {
            std::stringstream ss;
            ss << "std::exception happend when init entity [" << m_dp->getEntityKey() << "], the exception is: " << e.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "std::exception", ss.str().c_str());
        }
        catch( ... )
        {
            std::stringstream ss;
            ss << "Unknown Exception happend when init entity [" << m_dp->getEntityKey() << "]";
            LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception", ss.str().c_str());
        }
		unsigned long nLapsed1 = timeStart1.elapsed();
		m_sync->itemEnd();
	...
	void ChangeDataNodeOpertionModeWorkItem::executeWorkItem()
	{
        ...
        {
            std::stringstream ss;
            ss << "TransactiveException happend when change datanode [" << m_dn->getEntityKey() << "] to " << m_operationMode \
                << ", the exception is: " << te.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "TransactiveException", ss.str().c_str());
        }
        catch( const std::exception & e )
        {
            std::stringstream ss;
            ss << "std::exception happend when change datanode [" << m_dn->getEntityKey() << "] to " << m_operationMode \
        ...
        {
            std::stringstream ss;
            ss << "Unknown Exception happend when change datanode [" << m_dn->getEntityKey() << "] to " << m_operationMode;
            LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception", ss.str().c_str());
        }
		unsigned long nLapsed1 = timeStart1.elapsed();
		m_sync->itemEnd();
	...
	void ChangeDataPointOpertionModeWorkItem::executeWorkItem()
	{
        ...
        {
            std::stringstream ss;
            ss << "TransactiveException happend when change datapoint [" << m_dp->getEntityKey() << "] to " << m_operationMode \
                << ", the exception is: " << te.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "TransactiveException", ss.str().c_str());
        }
        catch( const std::exception & e )
        {
            std::stringstream ss;
            ss << "std::exception happend when change datapoint [" << m_dp->getEntityKey() << "] to " << m_operationMode \
                << ", the exception is: " << e.what();
            LOG_EXCEPTION_CATCH( SourceInfo, "std::exception", ss.str().c_str());
        }
        catch( ... )
        {
            std::stringstream ss;
            ss << "Unknown Exception happend when change datapoint [" << m_dp->getEntityKey() << "] to " << m_operationMode;
            LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception", ss.str().c_str());
        }
		unsigned long nLapsed1 = timeStart1.elapsed();
		m_sync->itemEnd();
	...
	*/
	...
	void DataNodeActivateWorkItem::executeWorkItem()
	{
		...
		for (it = m_datanodes.begin(); it != m_datanodes.end(); ++it)
		{
            ...
            {
                std::stringstream ss;
                ss << "TransactiveException happend when active datanode [" << (*it)->getEntityKey() << "], the exception is: " << te.what();
                LOG_EXCEPTION_CATCH( SourceInfo, "TransactiveException", ss.str().c_str());
            }
            catch( const std::exception & e )
            {
                std::stringstream ss;
                ss << "std::exception happend when active datanode [" << (*it)->getEntityKey() << "], the exception is: " << e.what();
                LOG_EXCEPTION_CATCH( SourceInfo, "std::exception", ss.str().c_str());
            }
            catch( ... )
            {
                std::stringstream ss;
                ss << "Unknown Exception happend when active datanode [" << (*it)->getEntityKey() << "]";
                LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception", ss.str().c_str());
            }
		}
		unsigned long nLapsed1 = timeStart1.elapsed();

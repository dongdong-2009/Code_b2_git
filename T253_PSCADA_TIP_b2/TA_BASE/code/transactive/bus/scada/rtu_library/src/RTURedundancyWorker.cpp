...
namespace TA_Base_Bus
{
    ...
    void RTURedundancyWorker::resetPrimaryRTU()
    {
        m_currentPrimaryRTU = NULL;
    }
    ...
    void SinglePrimaryRTURedundancyWorker::processRTUData(RTU& rtu, const TA_Base_Bus::DataBlock< WORD > & addressBlock)
    {
        ...
        if (0 == primaryRTUNum )
        {
            ...
            {
                m_currentPrimaryRTU = &rtu;
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "set Primary %s", m_currentPrimaryRTU->getName().c_str());
            }

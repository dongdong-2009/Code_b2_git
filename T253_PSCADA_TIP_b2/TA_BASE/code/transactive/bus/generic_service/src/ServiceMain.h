/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/generic_service/src/ServiceMain.h $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2017/11/23 14:55:43 $
  * Last modified by:  $Author: limin.zhu $
  *
  */
// ServiceMain.h : Defines the entry point for the application.
//
...
namespace TA_Base_Bus
{
	...
	public:
		...
		void initialise(const std::string& name, int defaultPortNum)
		{
			m_name = name;
	...
	protected:
		...
		VOID WINAPI serviceMain(DWORD argc, LPTSTR *argv)
		{
			...
			{
				reportEvent(std::string("Unexpected exception during deleting ServiceAgent"));
			}
		...
		VOID WINAPI ctrlHandler(DWORD opcode)
		{
			...
			case SERVICE_CONTROL_SHUTDOWN:
				...
				//TD14190, add try...catch block. If exceptions catched, distroy the serviceAgent anyway.
				try
				{
			...
			try
			{
				...
				if (scmHandle != NULL)
				{
					...
					if (serviceHandle != NULL)
					{
						...
						size_t uSize = m_name.size() + 1;
                        char* description = new char[uSize];
						memset(description, 0, uSize);

/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/security/rights_library/src/CorbaRightsLibrary.cpp $
  *  @author:  Gregg Kirkpatrick
  *  @version: $Revision: #2 $
  *
  *  Last modification: $DateTime: 2018/02/09 11:04:08 $
...
void CorbaRightsLibrary::areActionsPermittedOnSubsystemAndLocSub(
		...
		EDecisionModule& decisionModule )
	{
		...
		try
		{
			CORBA::ULong uResultActSeqLen = resultActionSeqVar->length();
			if (uResultActSeqLen == actionKeys.size())
			{
				LOG(SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "All actions got permit.");
				response = true;
				TA_Base_Core::ThreadGuard guard(m_cacheLock);
				for(CORBA::ULong idx=0; idx < uResultActSeqLen; idx++ )
				{
					m_cache.add( sessionId, locationKey, subsystemKey, resultActionSeqVar[idx], response, reasonString, decisionModule );
				}
				returnActionList=actionKeys;
			}
			else if (0 == uResultActSeqLen)
			{
				LOG(SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "All actions got deny.");
				response = false;
				TA_Base_Core::ThreadGuard guard(m_cacheLock);
				for( ActionList::iterator itr = actionKeys.begin(), end = actionKeys.end(); itr != end; itr++ )
				{
			...
			{
				LOG(SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "Some actions got permit while others got deny.");
				std::set<unsigned long> tmpActionSet;
				for( ActionList::iterator itr = actionKeys.begin(), end = actionKeys.end(); itr != end; itr++ )
				{
					tmpActionSet.insert(*itr);
				}
				...
				{
					response = true;
					std::set<unsigned long>::iterator itrFound = tmpActionSet.find(static_cast<unsigned long>(resultActionSeqVar[idx]));
					if (itrFound != tmpActionSet.end())
					{
						tmpActionSet.erase(itrFound);
					}
					m_cache.add( sessionId, locationKey, subsystemKey, resultActionSeqVar[idx], response, reasonString, decisionModule );
					returnActionList.push_back(static_cast<unsigned long>(resultActionSeqVar[idx]));
				}

				for (std::set<unsigned long>::iterator iter = tmpActionSet.begin(); iter != tmpActionSet.end(); iter++)
				{
					response = false;
					m_cache.add( sessionId, locationKey, subsystemKey, *iter, response, reasonString, decisionModule );
				}

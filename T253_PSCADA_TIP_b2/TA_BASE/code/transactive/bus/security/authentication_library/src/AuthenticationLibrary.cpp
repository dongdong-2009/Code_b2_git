/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/security/authentication_library/src/AuthenticationLibrary.cpp $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2018/01/17 10:08:31 $
...
namespace TA_Base_Bus
{
		...
		bool displayOnly)
	{
		...
		try
		{
			...
			AuthenticationAgentNamedObjPtr authAgent = 
				KeepSessionAliveThreadManager::getInstance().getAuthCorbaNameObject(locationId);
			CORBACALL(sessionId = (*authAgent)->requestSession(userId, profileId, locationId, workstationId, encrypted_Password.c_str(), displayOnly), authAgent);
		...
		const std::string & newPassword)
	{
		...
		try
		{
			...
			CORBACALL((*authAgent)->changePassword(sessionId.c_str(), encrypted_Password_old.c_str(), encrypted_Password_new.c_str()), authAgent);
		}
		CATCH_AUTHENTICATION_EXCEPTION();
		...
		const std::string & password)
	{
		...
		try
		{
			...
			CORBACALL((*authAgent)->beginOverride(sessionId.c_str(), userId, profileId, locationId, encrypted_Password.c_str()), authAgent);
	...
	void AuthenticationLibrary::endOverride(const std::string & sessionId)
	{
		...
		try
		{
			...
			CORBACALL((*authAgent)->endOverride(sessionId.c_str()), authAgent);
		}
		CATCH_AUTHENTICATION_EXCEPTION();
			...
			else
			{
				...
				if(rtnCode != 0 )
			 {
				...
				AuthenticationAgentNamedObjPtr authAgent = 
					KeepSessionAliveThreadManager::getInstance().getAuthCorbaNameObject(sessionId);
				// Call the method
				CORBACALL(sessionIdVar = (*authAgent)->changeProfile(sessionId.c_str(), profileId, locationId, encrypted_Password.c_str(), displayOnly), authAgent);
				/*
				CORBA_CALL(	authAgent,
				beginDisplayonly,
				(displayOnly)
	...
	void AuthenticationLibrary::getProfiles(const std::string & sessionId, std::vector<unsigned long> & profileVector)
	{
		...
		try
		{
			...
			CORBACALL(profiles = (*authAgent)->getProfiles(sessionId.c_str()), authAgent);
	...
	SessionInfo AuthenticationLibrary::getSessionInfo(const std::string & sessionId, const std::string & requestingSessionId)
	{
		...
		try
		{
			...
			try
			{
				...
				AuthenticationAgentNamedObjPtr authAgent = 
					KeepSessionAliveThreadManager::getInstance().getAuthCorbaNameObject(sessionId);			
				CORBACALL(session = (*authAgent)->getSessionInfo(sessionId.c_str(), requestingSessionId.c_str()), authAgent);
			}
			CATCH_AUTHENTICATION_EXCEPTION();
	...
	void AuthenticationLibrary::endSession(const std::string & sessionId)
	{
		...
		try
		{
			...
			AuthenticationAgentNamedObjPtr authAgent = 
				KeepSessionAliveThreadManager::getInstance().getAuthCorbaNameObject(sessionId);
			CORBACALL((*authAgent)->endSession(sessionId.c_str()), authAgent);
		}
		CATCH_AUTHENTICATION_EXCEPTION();
	...
	void AuthenticationLibrary::getActiveSessionsForOneLocation( const std::string & noUseSessionId, AuthenticationAgentNamedObjPtr authAgent, 
		std::vector<SessionInfo> & sessionInfoVector)
	{
		...
		try
		{
            ...
            CORBACALL(sessions = (*authAgent)->getActiveSessions(noUseSessionId.c_str()), authAgent);
	...
	/*
		...
		unsigned char* origin = (unsigned char*) data.c_str();

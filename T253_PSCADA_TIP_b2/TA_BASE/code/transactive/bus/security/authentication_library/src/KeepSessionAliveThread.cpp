/**
  ...
  *
  * Source:    $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/security/authentication_library/src/KeepSessionAliveThread.cpp $
  * @author:   Anita Lee
  * @version:  $Revision: #3 $
  *
...
namespace TA_Base_Bus
{
    ...
    void KeepSessionAliveThread::start()
    {
        m_running = true;
        Thread::start();
    }
    ...
    KeepSessionAliveThread::EHeartbeatResult KeepSessionAliveThread::sendHeartbeatForSession( const std::string& sessionId )
    {
        ...
        try
        {
			if(m_running) //
			{
				AuthenticationAgentNamedObjPtr authAgent = KeepSessionAliveThreadManager::getInstance().getAuthCorbaNameObject(sessionId);
				CORBACALL((*authAgent)->keepSessionAlive(timeNow, sessionId.c_str()), authAgent);

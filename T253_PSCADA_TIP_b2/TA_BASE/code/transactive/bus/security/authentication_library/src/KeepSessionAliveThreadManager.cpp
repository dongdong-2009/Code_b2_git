/**
  ...
  *
  * Source:    $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/security/authentication_library/src/KeepSessionAliveThreadManager.cpp $
  * @author:   Anita Lee
  * @version:  $Revision: #3 $
  *
...
namespace TA_Base_Bus
{
    ...
    {
        return *ACE_Unmanaged_Singleton<KeepSessionAliveThreadManager, ACE_Recursive_Thread_Mutex>::instance();
    }

	void KeepSessionAliveThreadManager::releaseInstance()
	{
        ACE_Unmanaged_Singleton<KeepSessionAliveThreadManager, ACE_Recursive_Thread_Mutex>::close();
	}
	...
	void KeepSessionAliveThreadManager::initAuthAgentMap()
	{
		...
		for(size_t i=0; i<authAgentNameList.size(); i++)
		{
			...
			AuthenticationAgentNamedObjPtr authAgent(new AuthenticationAgentNamedObj(corbaName));
	...
	AuthenticationAgentNamedObjPtr KeepSessionAliveThreadManager::getAuthCorbaNameObject( unsigned long locationID )
	{
	...
	AuthenticationAgentNamedObjPtr KeepSessionAliveThreadManager::getAuthCorbaNameObject( const std::string & sessionId )
	{
		if ( sessionId.size() == 0 )
		{

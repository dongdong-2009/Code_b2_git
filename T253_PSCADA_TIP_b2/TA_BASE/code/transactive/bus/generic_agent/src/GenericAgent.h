/**
  ...
  * 
  * Source:    $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/generic_agent/src/GenericAgent.h $
  * @author:   Ross Tucker
  * @version:  $Revision: #2 $
  * @see:      9999-Q01-02-0016-GenericAgent_Usage_Guidelines.doc
  *
  * Last modification: $DateTime: 2018/02/08 10:38:17 $
...
const int AGENT_USER_TYPE_NORMAL = 1;
const int AGENT_USER_TYPE_SCADA  = 2;
...
namespace TA_Base_Bus
{
	struct AgentInfo
	{
		bool FailOverFromStationToOcc;
	};

    class IGenericAgentUser; // Forward Declaration

    class GenericAgent : public virtual POA_TA_Base_Core::IManagedAgentCorbaDef,
    ...
    public:
        ...
        /**
          * Class constructor
          *
          * Construct the GenericAgent, passing in any command line arguments
          * that were supplied to the agent.
          *
          * @param Number of parameters
          * @param Array of parameter values
          * @param Pointer to object implementing the IGenericAgentUser interface
          * @param Pointer to object responsible for responding to polls from the
          *        Process Manager, or NULL if the default PollListener is required.
          *
          * NOTE: Do not delete the PollListener object when shutting down.
          *       GenericAgent will delete it for you.
          *
          * @exception GenericAgentException thrown if initialisation fails
          */
        GenericAgent(int argc, char* argv[], IGenericAgentUser* registeredUser,
                     TA_Base_Core::PollListener* pollListener = NULL, const unsigned int agentUserType = AGENT_USER_TYPE_SCADA );
		...
		// get Agent Info, AgentInfo contains extra info which can be used by Agent user
		TA_Base_Bus::AgentInfo getAgentInfo() const { return m_agentInfo; }
    private:

        // Disable the copy constructor and assignment operator
        GenericAgent( const GenericAgent& theGenericAgent);
        GenericAgent& operator=(const GenericAgent &);
        ...
        /**
          ...
          */
        void doEntityConfiguration(const unsigned int agentUserType);

        /**
          * doAgentSetup
          *
          * This method is called by the GenericAgent constructor. It's purpose
          * is to complete the GenericAgent initialisation.
          */
        void doAgentSetup();
		...
		/** 
          * dummySignalHandler
          *
          * set a dummy signal handler for signal INT (ctrl+c), which will cause
		  * oracle database abort the current database operation.
          *
          * @param sigNum  The signal received SIGINT.
          */
		...
		//AgentInfo contains extra info which can be used by Agent user
		TA_Base_Bus::AgentInfo m_agentInfo;
    };
}

#endif // !defined(GENERICAGENT)

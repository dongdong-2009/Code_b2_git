...
* in any form.
*
* Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/generic_gui/src/TransActiveWinApp.cpp $
* @author:  Karen Graham
* @version: $Revision: #3 $
*
* Last modification: $DateTime: 2018/02/08 10:17:08 $
...
namespace TA_Base_Bus
{
	...
	void TransActiveWinApp::WinHelpInternal(DWORD dwData, UINT nCmd) 
	{
		...
		void TransActiveWinApp::setTeminating()
	{
		m_terminating = true;
	}
    int TransActiveWinApp::outOfMemoryHandler( size_t size )
    {
    	if(!m_terminating) //application is not terminating
		{
            std::ostringstream oss;
            oss << "No more memory available.  Failed to allocated " << size << " bytes.";
            TA_THROW(std::exception(oss.str().c_str()));
        }

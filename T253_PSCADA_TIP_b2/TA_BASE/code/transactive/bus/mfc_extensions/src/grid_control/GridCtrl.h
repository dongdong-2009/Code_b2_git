/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/mfc_extensions/src/grid_control/GridCtrl.h $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
typedef struct _GV_ITEM { 
	...
	CString  szTextS;      // Text in cell //TD15533
    int      iImageS;      // index of the list view item’s icon //TD15533
	BOOL     bOverrided;   // If profile is  overrided or not //TD15533
...
public:
    CGridCell() 
    {
        ...
        szText.Empty();
        iImage = -1;
		szTextS.Empty(); //TD15533
        iImageS = -1; //TD15533
		bOverrided = false; //TD15533
		crBkClr = CLR_DEFAULT;
		crFgClr = CLR_DEFAULT;
        lParam = 0;
    }
    ...
    int      iImage;      // Index of the list view item’s icon 
	CString  szTextS;      // Cell text (or binary data if you wish...) //TD15533
    int      iImageS;      // Index of the list view item’s icon  //TD15533
	BOOL     bOverrided;   // If profile is  overrided or not //TD15533
    COLORREF crBkClr;     // Background colour (or CLR_DEFAULT)
    COLORREF crFgClr;     // Forground colour (or CLR_DEFAULT)
...
AFX_API_IMPORT void AFXAPI DDX_GridControl(CDataExchange* pDX, int nIDC, CGridCtrl& rControl);
#else
AFX_EXT_API void AFXAPI DDX_GridControl(CDataExchange* pDX, int nIDC, CGridCtrl& rControl);
#endif
...
#ifdef IMPORT_EXTENSIONS
class __declspec(dllimport) CGridCtrl : public CWnd
#else
...
public:
    ...
    int  GetDefaultRowHeight() const            { return m_nDefCellHeight;   }
    void SetDefaultRowHeight(int height)        { m_nDefCellHeight = height; }
    int  GetDefaultColumnWidth() const          { return m_nDefCellWidth;    }
    void SetDefaultColumnWidth(int width)       { m_nDefCellWidth = width;   }
...
protected:
    ...
    CString GetVisibleCellText(const CDC& dc, const CRect& rect, const CString& text) const;

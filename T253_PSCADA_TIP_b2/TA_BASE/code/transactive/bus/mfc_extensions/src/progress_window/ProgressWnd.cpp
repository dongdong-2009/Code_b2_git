/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/mfc_extensions/src/progress_window/ProgressWnd.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
CProgressWnd::CProgressWnd(CWnd* pParent, LPCTSTR pszTitle, BOOL bCanCancel /* = TRUE */, BOOL bSmooth /* = FALSE */)
{
    CommonConstruct();
    m_strTitle = pszTitle;

    Create(pParent, pszTitle, bCanCancel, bSmooth);
}
...
BOOL CProgressWnd::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bCanCancel /* = TRUE */, BOOL bSmooth /* = FALSE */)
{
    // if TRUE, the cancel button will be drawn and enabled
...
void CProgressWnd::SetWindowSize(int nNumTextLines, int nWindowWidth /*=390*/)
{
    ...
    {
        ProgressRect.SetRect(TextRect.left, CancelRect.top + EdgeSize.cy, 
                             CancelRect.right, CancelRect.bottom - EdgeSize.cy);
    }

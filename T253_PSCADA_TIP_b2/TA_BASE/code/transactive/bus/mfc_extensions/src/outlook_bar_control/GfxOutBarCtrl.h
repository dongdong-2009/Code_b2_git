/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/mfc_extensions/src/outlook_bar_control/GfxOutBarCtrl.h $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
#define NM_OB_ICON_SIZE_CHANGE  6
...
class AFX_EXT_CLASS CGfxOutBarCtrl : public CWnd
{
// Construction
...
// Attributes
public:
	int	crBackGroundColor, crBackGroundColor1;
	int	crTextColor;
	int	cr3dFace, crLightBorder, crHilightBorder, crShadowBorder, crDkShadowBorder;
    ...
    CImageList *    pLargeImageListDisabled;
    CImageList *    pSmallImageListDisabled;
...
public:
    ...
    DWORD GetItemData(const int index, const int folder) const;
	bool IsValidItem(const int index) const;
	bool IsValidItem(const int index, int folder) const;
    ...
    void DisableItem(const int index, const int folder, bool isDisabled = true);
    ...
    int GetItemCount(int folder) const;
	...
	CBarFolder* GetFolder( const int index );
...
private:
    ...
    bool IsItemDisabled(int folder, int index);
    CImageList* GetDisabledImageList(CImageList* oldImageList);
    HICON createGreyscaleIcon(HICON colourIcon);


    typedef std::map<int, BOOL> DisabledFolderItems;
    typedef std::map<int, DisabledFolderItems> Folders;

    Folders m_disabledItems;
};

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/bus/mfc_extensions/src/tree_ctrl_multi_sel/MltiTree.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    void CMultiTree::OnButtonDown(BOOL bLeft, UINT nFlags, CPoint point)
    {
	    ...
	    if (!hItem || (nHF & (TVHT_ONITEMRIGHT|TVHT_NOWHERE|TVHT_ONITEMINDENT))) {
		    //clicked in space, do rubber-banding provided user allowing drag selections
            if (m_bAllowDragSelection)
            {

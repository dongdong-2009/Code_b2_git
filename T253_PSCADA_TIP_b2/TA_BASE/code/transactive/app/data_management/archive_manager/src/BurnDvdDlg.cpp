...
* in any form.
*
* Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/data_management/archive_manager/src/BurnDvdDlg.cpp $
...
#include "MediaAccessWrapperFactory.h"
...
namespace
{
	...
	const CString CaptionWriteComplete = "¿ÌÂ¼Íê³É";
}
...
namespace TA_Base_App
{
		...
		try
		{
			...
			try
			{
				...
				if(!writeSuccessful)
				{
					...
					m_stageStatic.SetWindowText( DVD_WRITE_FAILED_MSG );
					appendStringToLogDisplay ( std::string( reason ) );
				...
				m_buttonCancel.SetWindowText( "Close" );
				 ...
				 {		
					 // Write successful.
					 m_stageStatic.SetWindowText( DVD_WRITE_SUCC_MSG );
	...
	BOOL BurnDvdDlg::OnInitDialog() 
	{
		...
		//  Reports write/burn progress in percent.

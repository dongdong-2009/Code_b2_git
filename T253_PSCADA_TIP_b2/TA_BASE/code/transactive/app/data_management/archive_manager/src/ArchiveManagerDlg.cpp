/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/data_management/archive_manager/src/ArchiveManagerDlg.cpp $
  * @author:  Alan Brims
  * @version: $Revision: #2 $
  *
...
namespace TA_Base_App
{
    ...
    void CArchiveManagerDlg::OnButtonSchedule() 
    {
        ...
        if( !messageForUser.empty() )
        {
            ...
            {
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_050126);
            }
    ...
    void CArchiveManagerDlg::OnButtonManual() 
    {
        ...
        if( !messageForUser.empty() )
        {
            ...
            if (messageLevel == TA_ArchiveManager::ERR)
            {
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_050127);
            }
            else if (messageLevel == TA_ArchiveManager::WARN)
            {
                UINT selectedButton = userMsg.showMsgBox(IDS_UW_050018);
        ...
        try
        {
			if( !TA_ArchiveManager::isActionPermitted( TA_Base_Bus::aca_ARCHIVE_DELETE ) )
            {
                ...
                try
                {
                    ...
                    UINT selectedButton = userMsg.showMsgBox(IDS_UE_050109);
				}

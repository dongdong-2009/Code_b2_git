/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/system_manager/src/SessionHandler.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #2 $
  *
...
namespace TA_Base_App
{
    ...
    void SessionHandler::retrieveRunParams()
    {
        ...
        try
	    {
            ...
            TA_Base_Core::RunParamSeq* tmpParams = m_systemController->getParams(0, hostName);
    ...
    void SessionHandler::checkUserPermissions()
    {
        ...
        try
        {
            std::auto_ptr<TA_Base_Core::IResource> resource( TA_Base_Core::ResourceAccessFactory::getInstance().getResourceFromEntity( m_guiEntity->getKey() ) );
            int resourceId = resource->getKey();
            TA_Base_Bus::EDecisionModule decisionModule = TA_Base_Bus::UNDEFINED_DM;
            permitted = m_rights->isActionPermittedOnResource( TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID),
                                                               resourceId,
    ...
    void SessionHandler::configureHelpFile()
    {
        ...
        try
        {
            std::auto_ptr<TA_Base_Core::IGui> gui( TA_Base_Core::GuiAccessFactory::getInstance().getGui( m_guiApplication.getApplicationType() ) );

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/system_manager/src/SystemControllerDlg.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
	...
	void SystemControllerDlg::populateHostnamesCombo()
	{
		...
		try
		{
			...
			for (std::vector<TA_Base_Core::ISystemControllerData*>::iterator iter = systemControllers.begin();
         		iter != systemControllers.end(); ++iter)
			{
                m_hostnameCombo.AddString((*iter)->getName().c_str());
			}

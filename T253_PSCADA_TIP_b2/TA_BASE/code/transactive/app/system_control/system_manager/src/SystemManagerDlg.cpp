/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/system_manager/src/SystemManagerDlg.cpp $
...
namespace TA_Base_App
{
	...
	const std::string SystemManagerDlg::STANDBY = "Standby";

	const std::string SystemManagerDlg::RUNNING_STANDBY = "Running Standby";
	const std::string SystemManagerDlg::GOING_TO_STANDBY = "Changing to Standby";
	...
	void SystemManagerDlg::init()
	{
        ...
        m_rightsLibrary = std::auto_ptr<TA_Base_Bus::RightsLibrary>(rlFactory.buildRightsLibrary());
   	    // m_rightsLibrary = std::auto_ptr<TA_Base_Bus::RightsLibrary>(rlFactory.buildRightsLibrary(TA_Base_Bus::ALWAYS_TRUE_RL)); // Use for testing
    ...
    void SystemManagerDlg::OnDebugLevel()
    {
		...
		states.push_back(GOING_TO_STANDBY);
		states.push_back(RUNNING_STANDBY);
        ...
        if ((pWnd == &m_processList) && (m_userLoggedIn))
        {
            ...
            while (pos != NULL)
            {
                ...
                if( (currStatus.compare(RUNNING_CONTROL) == 0) ||
					...
					(currStatus.compare(GOING_TO_STANDBY) == 0) ||
					(currStatus.compare(RUNNING_STANDBY) == 0) ||
    ...
    void SystemManagerDlg::setupMenu(bool loggedIn)
    {
        ...
        if(loggedIn)
        {
            ...
            pMenu->EnableMenuItem(ID_CONTROL_SHUTDOWNSYSTEMCONTROLLER,
                m_isShutdownAllowed ? MF_ENABLED : MF_GRAYED);        
    ...
    void SystemManagerDlg::checkPermissions()
    {
        ...
        try
        {
            TA_Base_Core::IEntityData* entityData = TA_Base_Core::EntityAccessFactory::getInstance().
                        getEntity(TA_Base_Core::RunParams::getInstance().get(RPARAM_HOSTNAME.c_str()));

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/system_manager/src/ProcessListCtrl.cpp $
...
namespace TA_Base_App
{
    ...
    ProcessListCtrl::ProcessListCtrl()
        : CListCtrl(),  m_isFrozen(false), m_sortedCol(PROCESS_ID_COL), m_sortAscending(true)
    {
    }
    ...
    void ProcessListCtrl::populate(std::vector<TA_Base_Core::ProcessData> processes)
    {
        ...
        {
            int pos = this->InsertItem(i, AdminManager::getInstance().getEntityDescription(
                                            processes[i].ProcessId.in()).c_str());
            ...
            this->SetItemText(pos, RESTARTS_COL, restarts.str().c_str());
                                               ...
                                               bool isManagedProcess)
    {
		...
		case TA_Base_Core::RunningStandby:
			SetItemText(insertPosition, STATUS_COL, SystemManagerDlg::RUNNING_STANDBY.c_str());
			SetItemText(insertPosition, RUNNING_SINCE_COL, processStartTime.Format(dateFormatStr.c_str()));
			//SetItemText(insertPosition, RUNNING_SINCE_COL, "");
			break;
		case TA_Base_Core::GoingToStandby:
			SetItemText(insertPosition, STATUS_COL, SystemManagerDlg::GOING_TO_STANDBY.c_str());
			SetItemText(insertPosition, RUNNING_SINCE_COL, processStartTime.Format(dateFormatStr.c_str()));
			//SetItemText(insertPosition, RUNNING_SINCE_COL, "");
			break;
        }
                                                       ...
                                                       TA_Base_Core::EOperationMode operationMode)
    {
		...
		case(TA_Base_Core::Standby):
			SetItemText(insertPosition, OPERATION_MODE_COL, SystemManagerDlg::STANDBY.c_str());
			break;
                                                  ...
                                                  bool isManagedProcess)
    {
		...
		case TA_Base_Core::RunningStandby:
			SetItemText(insertPosition, NEW_STATUS_COL, SystemManagerDlg::RUNNING_STANDBY.c_str());
			break;

		case TA_Base_Core::GoingToStandby:
			SetItemText(insertPosition, NEW_STATUS_COL, SystemManagerDlg::GOING_TO_STANDBY.c_str());
			break;
        }
    }

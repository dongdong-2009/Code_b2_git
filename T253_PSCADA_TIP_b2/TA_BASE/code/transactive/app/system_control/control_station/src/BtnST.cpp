/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/BtnST.cpp $
...
DWORD CButtonST::SetDefaultColors(BOOL bRepaint)
{
	...
	m_crColors[BTNST_COLOR_FG_OUT]		= RGB(45,45,45);//::GetSysColor(COLOR_BTNTEXT);
...
void CButtonST::AddToMenu(unsigned int type, unsigned int callbackId, const char *message)
{
	if (m_hMenu)
		::AppendMenu(m_hMenu, type, callbackId, message);
}


void CButtonST::AddMenu(CMenu* pMenu, HWND hParentWnd)
{	
	// Destroy any previous menu
	if (m_hMenu)
	{

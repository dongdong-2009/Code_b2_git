/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/ControlStation.cpp $
  * @author:  Davide Calabro
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2018/01/30 16:26:16 $
  * 
...
int ControlStationApp::ExitInstance()
{
	...
	//}
     ...
     _omniFinalCleanup* never_cleanup = new _omniFinalCleanup;
...
{
    FUNCTION_ENTRY( "ControlStationApp::archiveLogFiles" );

    boost::filesystem::path p = debugFileName;
    boost::filesystem::path dir = p.parent_path();
    std::string folderName = "ArchivedLogs_" + CTime::GetCurrentTime().Format("%Y-%m-%d_%H-%M-%S");
    boost::filesystem::path archive = dir / folderName;

    try
    {
        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "ControlStationApp::archiveLogFiles - create directory %s", folderName.c_str() );

        create_directory( archive );

        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "ControlStationApp::archiveLogFiles - move files to directory %s", folderName.c_str() );
        ...
        for ( ; it != end; ++it )
        {
            ...
            if ( !is_directory( it->status() ) && p.extension() == ".log" )
            {
                try
                {
                    LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "ControlStationApp::archiveLogFiles - ", p.filename().c_str() );

                    rename( p, archive / p.filename() );
                }
                catch ( std::exception& e )
                {
                    LOG_GENERIC( SourceInfo, DebugUtil::DebugWarn, "ControlStationApp::archiveLogFiles - can not move %s, error message: %s", p.filename().c_str(), e.what() );

                    try
                    {
                        copy_file( p, archive / p.filename() );
                    }
                    catch ( std::exception& e )
                    {
                        LOG_GENERIC( SourceInfo, DebugUtil::DebugWarn, "ControlStationApp::archiveLogFiles - can not copy %s, error message: %s", p.filename().c_str(), e.what() );
                    }
                }
            }
        }

        logsthread = new CLogsThread( archive );
        logsthread->start();
    }
    catch ( std::exception& e )
    {
        LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "ControlStationApp::archiveLogFiles - %s", e.what() );
    }

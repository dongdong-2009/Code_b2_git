/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/ProcessManager.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
struct IProcessEventObserver
{
    virtual ~IProcessEventObserver() {}
    virtual void applicationStarted() = 0;
    virtual void applicationTerminated() = 0;
};

struct ProcessEventNotifier
{
    static ProcessEventNotifier& instance()
    {
        static ProcessEventNotifier instance_;
        return instance_;
    }
    ...
    void notifyApplicationStarted( const std::string& applicationName )
    {
        if ( ! m_exit )
        {
            for ( std::set<IProcessEventObserver*>::iterator it = m_application2ObserversMap[applicationName].begin(); it != m_application2ObserversMap[applicationName].end(); ++it )
            {
                (*it)->applicationStarted();
            }
        }
    }

    void notifyApplicationTerminated( const std::string& applicationName )
    {
        if ( ! m_exit )
        {
            for ( std::set<IProcessEventObserver*>::iterator it = m_application2ObserversMap[applicationName].begin(); it != m_application2ObserversMap[applicationName].end(); ++it )
            {
                (*it)->applicationTerminated();
            }
        }
    }

    void registerObserver( IProcessEventObserver* observer, const std::string& applicationName )
    {
        m_application2ObserversMap[applicationName].insert( observer );
    }

    void deregisterObserver( IProcessEventObserver* observer, const std::string& applicationName )
    {
        m_application2ObserversMap[applicationName].erase( observer );
    }

private:

    ProcessEventNotifier::ProcessEventNotifier() : m_exit(false) {}
    bool m_exit;
    std::map< std::string, std::set<IProcessEventObserver*> > m_application2ObserversMap;
};

struct RadioManagerEventObserver : IProcessEventObserver
{
    RadioManagerEventObserver()
    {
        ProcessEventNotifier::instance().registerObserver( this, "Radio Manager" );
    }

    virtual void applicationStarted()
    {
        updateGX( "Type[RadioSessionStatus] StatusOK" );
    }

    virtual void applicationTerminated()
    {
        updateGX( "Type[RadioSessionStatus] LinkBroken" );
    }

    void updateGX( const std::string& message )
    {
        TAToGxMessageInformation *pGxMsg = new TAToGxMessageInformation();
        pGxMsg->targetSessionID = SessionManager::getInstance().getSessionDetails().getSessionId().c_str();
        pGxMsg->targetSchematicID = "";
        pGxMsg->sourceEntityName = "RadioManager";
        pGxMsg->message = message.c_str();
        ControlStationRootDialog::getInstance().PostMessage(WM_TA_TO_GX_MESSAGE, (WPARAM)pGxMsg);
    }

    ~RadioManagerEventObserver()
    {
        ProcessEventNotifier::instance().deregisterObserver( this, "Radio Manager" );
    }
};
...
m_sessionJobObject(NULL)
{
    ...
    TA_Base_Core::RunParams::getInstance().registerRunParamUser( this, RPARAM_DBCONNECTIONFILE );
                                       ...
                                       const bool isVisible /*= true*/)
{
    ...
    if ( !shouldApplicationBeRelaunched(appType, alreadyRunningApp) )
    {
        ...
        {
            LOG( SourceInfo, DebugUtil::ExceptionCatch, "ApplicationException","Application could not be given focus. We will relaunch instead.");            
        }
...
void ProcessManager::applicationHasTerminated( unsigned long processId )
{
    ...
    if ( application != NULL )
    {
        ProcessEventNotifier::instance().notifyApplicationTerminated( application->getApplicationName() );
    }
                                             ...
                                             const char* entity)
{
    ...
    if ( matchingAppIter != m_runningApplications.end() )
    {
        ...
        ProcessEventNotifier::instance().notifyApplicationStarted( matchingAppIter->second->getApplicationName() );
        return;
    }
    ...
    try
    {
        ...
        ProcessEventNotifier::instance().notifyApplicationStarted( runningApp->getApplicationName() );
    }
    catch ( DatabaseException& )
    {
        if (runningApp != NULL)
...
TA_Base_Core::RunParamSeq* ProcessManager::getParams(TA_Base_Core::ProcessId p_processId, const char* hostname)
{
    ...
    if (iter != m_runningApplications.end() )
    {
		...
		if (RunParams::getInstance().isSet(RPARAM_RESOLVE_TIMEOUT_MS))
		{
			std::string pvalue = RunParams::getInstance().get(RPARAM_RESOLVE_TIMEOUT_MS);
			params->length( insertPosition+1 );
			(*params)[insertPosition].name = CORBA::string_dup(RPARAM_RESOLVE_TIMEOUT_MS);
			(*params)[insertPosition].value = CORBA::string_dup(pvalue.c_str());
			LOGMORE( SourceInfo, "%s=%s", RPARAM_RESOLVE_TIMEOUT_MS, pvalue.c_str());
			++insertPosition;
		}
    }
...
void ProcessManager::terminateAllApplications(TA_Base_App::TA_ControlStation::ETerminateMode terminateMode/*=TA_Base_App::TA_ControlStation::LOGOUT*/)
{
    ...
    if ( exitAll )
    {
        ProcessEventNotifier::instance().onExit();
    }
...
std::vector<ApplicationProperties> ProcessManager::getApplications()
{
	...
	std::map<unsigned long, ApplicationProperties> appListMap;
	std::map<unsigned long ,unsigned long> entityResourceMatch;

    ApplicationMap::iterator iter = m_applications.begin();
    ApplicationMap::iterator iterEnd = m_applications.end();

	TA_Base_Bus::ResourceList allResourseList;
		
	//std::string entityList = "\nThe entities are:	\n";
	LOG(SourceInfo,DebugUtil::GenericLog,DebugUtil::DebugInfo,"The entities are:");
	...
	while (iter != iterEnd)
    {
        try
        {
				if( iter->second->isIconVisible() )
				{
					...
					//entityList +=app.name+":	"+_ultoa(app.key,buffer,10)+"	"+_ultoa(iter->second->getResourceKey(),buffer,10)+"\n";
					LOG(SourceInfo,DebugUtil::GenericLog,DebugUtil::DebugInfo,"%s: %lu, %lu", app.name.c_str(), app.key, iter->second->getResourceKey());
					allResourseList.push_back(iter->second->getResourceKey());
					appListMap.insert(std::pair<unsigned long, ApplicationProperties>(app.key,app));
					entityResourceMatch.insert(std::pair<unsigned long, unsigned long>(app.key, iter->second->getResourceKey()));
	...
	//LOG(SourceInfo,DebugUtil::GenericLog,DebugUtil::DebugInfo,entityList.c_str());
	//assign corba call 3 times to access available resources for the action
	TA_Base_Bus::ResourceList tempList;
	bool ResourceGot = false;
	int tryTimes = 0;

	while (!ResourceGot&& tryTimes < 3 && allResourseList.size() > 0)
	{
		tryTimes ++;

		SessionManager::getInstance().areActionsPermitted(TA_Base_Bus::aca_RUN_APPLICATION, allResourseList,tempList);
		if(tempList.size() != 0)
		{
			permitedResourceList = tempList;
			ResourceGot = true;
		TA_Base_Bus::ResourceList::iterator itertempList= tempList.begin();
		while(itertempList!=tempList.end())
		{
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "EntityKey => %lu", *itertempList);
			itertempList++;
		}
		}
	}	
	...
	std::map<unsigned long, ApplicationProperties>::iterator APPiter = appListMap.begin();
	//iter = m_applications.begin();
//	unsigned long resourceKey;
	std::map<unsigned long, unsigned long>::iterator entityResourceIter;
		while(APPiter != appListMap.end())
		{		
			entityResourceIter= entityResourceMatch.find(APPiter->first);
			if(resourceIsPermitted(entityResourceIter->second))
			{
				APPiter->second.isAccessible = true;
				applist.push_back(APPiter->second);
			}
			else
			{
				APPiter->second.isAccessible = false;
				applist.push_back(APPiter->second);
			}
 	...
 	//	appListMap.erase( appListMap.begin(), appListMap.end() );
	//	entityResourceMatch.erase( entityResourceMatch.begin(), entityResourceMatch.end() );
...
void ProcessManager::setupDbMonitors(int& pollPeriod)
{
    ...
    m_dbConnectionFileList.push_back( filename );
	...
	if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_MONITOREDDATABASE))
	{
		...
		m_monitoredDbName = connectInfo[1];
	}
...
void ProcessManager::notifyStatusChange(const std::string& DbConnectionInfo, TA_Base_Bus::DatabaseStatusCorbaDef::EState newStatus)
{
	...
	if (!connectInfo[4].empty())					
		dbName += (connectInfo[1] + "@" + connectInfo[4]);
	else
		dbName += connectInfo[1];
    ...
    for (RunningApplicationMap::iterator iter = m_runningApplications.begin(); 
        iter != m_runningApplications.end(); iter++)
    {
        process = dynamic_cast<RunningApplication*>(iter->second);
        if (process != NULL)
        {
            ...
            process->passNewParameters(runParamSeq);
        }
    }
...
void ProcessManager::updateSessionId(std::string& sessionId)
{
	...
	while (iter != iterEnd)
	{
		iter->second->updateSessionId(sessionId);
		++iter;
	}
...
void ProcessManager::onRunParamChange(const std::string& name, const std::string& value)
{
    FUNCTION_ENTRY( "onRunParamChange" );

    if ( RPARAM_DBCONNECTIONFILE == name )
    {
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "RunParam change: %s=%s", name.c_str(), value.c_str() );

        const std::string& filename = value;

        if ( std::find( m_dbConnectionFileList.begin(), m_dbConnectionFileList.end(), filename ) == m_dbConnectionFileList.end() )
        {
            m_dbConnectionFileList.push_back( filename );
            ...
            for ( TA_Base_Core::DatabaseConnectionMap::iterator it = dbMap.begin(); it != dbMap.end(); ++it )
            {
                DataConnections& dataConnections = it->second;

                for ( TA_Base_Core::DataConnections::iterator dcIt = dataConnections.begin(); dcIt != dataConnections.end(); ++dcIt )
                {
                    std::string& connectionString = dcIt->second;

                    if ( m_moniteredDatabases.find( connectionString ) == m_moniteredDatabases.end() )
                    {
                        TA_Base_Bus::SimpleDbStatusChecker* dbChecker = new TA_Base_Bus::SimpleDbStatusChecker( connectionString, pollTime, this );
                        dbChecker->start();
                        m_moniteredDatabases.insert( std::make_pair( connectionString, dbChecker ) );

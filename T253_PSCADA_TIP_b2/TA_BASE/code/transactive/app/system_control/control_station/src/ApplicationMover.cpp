/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/ApplicationMover.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
void ApplicationMover::run()
{
    ...
    try
    {
        ...
        {
            LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Attempt %d to move application %d", numTries++, static_cast<int>(m_moveInformation.processId));
...
BOOL CALLBACK ApplicationMover::WndPositionEnumHandler(HWND hWnd, LPARAM lParam)
{
	...
	if (info->processId == dwThisWindowProcessID)
	{
        ...
        else
        {
            ...
            LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Either no positioning flag is defined or there's some problems with the arguments, centering application instead.");
        }
        ...
        LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Successfully moved application %d", static_cast<int>(info->processId));

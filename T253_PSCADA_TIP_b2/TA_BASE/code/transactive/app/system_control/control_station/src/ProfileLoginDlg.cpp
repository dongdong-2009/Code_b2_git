/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/ProfileLoginDlg.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
        ...
        if (*iter != NULL && (*iter)->getKey() != 0 )
		{
			...
			try
			{
				...
				*/
				...
				if (isOnline )
				{
					if(isLocationValidToBePopulated(*iter,minLocalLocationOrderId,maxLocalLocationOrderId ))
					{
						//int index = m_locationCombo.AddString(name);
...
bool ProfileLoginDlg::isLocationValidToBePopulated(TA_Base_Core::ILocation* location, unsigned long minLocalLocationOrderId, unsigned long maxLocalLocationOrderId)
{
	...
	if( boost::iequals(currentlocaiton->getName(),"DT1") || boost::iequals(currentlocaiton->getName(),"DT16"))
	{
...
std::vector<unsigned long>  ProfileLoginDlg::getLocalOrderIdList()
{
	...
	unsigned long maxLocalLocationOrderId = ULONG_MAX;
	unsigned long minLocalLocationOrderId = 0;
	SessionDetails::LocationVector::iterator iter = locations.begin();

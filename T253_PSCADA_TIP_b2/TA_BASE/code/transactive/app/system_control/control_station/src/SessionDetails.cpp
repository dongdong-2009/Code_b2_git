/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/system_control/control_station/src/SessionDetails.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
void SessionDetails::renewSession(std::string sessionId)
{
	...
	RunParams::getInstance().set("OperatorName", m_operator->getName().c_str());
    FUNCTION_EXIT;
}
...
SessionDetails::ProfileVector SessionDetails::getListOfProfiles(TA_Base_Core::IOperator* theOperator, bool isCentral, const bool onlyOverridable /*=false*/)
{
	LOG( SourceInfo, DebugUtil::FunctionEntry,"getListOfProfiles");
...
std::string SessionDetails::getLocationDisplayName()
{
	...
	{
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DataException", "Caught while trying to retrieve location display name");
	}
	catch( DatabaseException& )
	{
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DatabaseException", "Caught while trying to retrieve location display name");
	}
	return "Unknown";
}

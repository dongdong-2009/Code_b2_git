        ...
        for (std::vector<std::string>::iterator it = peerHostnames.begin(); it != peerHostnames.end(); ++it)
        {
            ...
            if (!CORBA::is_nil(obj))
            {
                for (size_t i = 0; i <= m_shutdownRetryTimes; ++i)
                {
                    ...
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "CORBA::SystemException",
                                            str(boost::format("System exception occurred while resolving peer shutdown executor %s - %s") % peerShutdownExecutorIor.str() % CorbaUtil::exceptionToString(e)));
                    }
                    catch (...)
                    {
                        LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", str(boost::format("Error occurred narrowing %s") % peerShutdownExecutorIor.str()));
                    }
                }
            }
            else

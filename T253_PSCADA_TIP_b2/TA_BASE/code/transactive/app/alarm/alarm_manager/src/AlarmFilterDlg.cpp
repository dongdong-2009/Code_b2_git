/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/alarm/alarm_manager/src/AlarmFilterDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2018/01/30 16:53:36 $
...
#include "bus\alarm\alarm_list_control\src\TA_Filter.h"
...
namespace TA_Base_App
{
	...
	//const std::string AlarmFilterDlg::FILTER_DEFAULT("Default Filter");
	const std::string AlarmFilterDlg::FILTER_DEFAULT("No Filter");

	const std::string AlarmFilterDlg::FILTER_TOTIME("ToTime");
	const std::string AlarmFilterDlg::FILTER_FROMTIME("FromTime");
	const std::string AlarmFilterDlg::FILTER_SUBSYSTEM("Subsystem");
	const std::string AlarmFilterDlg::FILTER_SUBSYSTEMTYPE("SubsystemType");
	const std::string AlarmFilterDlg::FILTER_SYSTEM("System");
	const std::string AlarmFilterDlg::FILTER_LOCATION("Location");
	const std::string AlarmFilterDlg::FILTER_OPERATOR("Operator");
	const std::string AlarmFilterDlg::FILTER_EQUIPMENT_ID("Equipment ID");
	const std::string AlarmFilterDlg::FILTER_DESCRIPTION("Description");
	const std::string AlarmFilterDlg::FILTER_SEVERITY("Severity");
	const std::string AlarmFilterDlg::FILTER_ALARMTYPE("Alarm Type");

	const std::string AlarmFilterDlg::FILTER_ACKSTATE("AckState");
	const std::string AlarmFilterDlg::FILTER_STATE("State");
	const std::string AlarmFilterDlg::FILTER_OPEN("Open");
	const std::string AlarmFilterDlg::FILTER_CLOSE("Close");
	const std::string AlarmFilterDlg::FILTER_ACKED("Acknowledged");
	const std::string AlarmFilterDlg::FILTER_UNACKED("Unacknowledged");
	const std::string AlarmFilterDlg::FILTER_ALL("All");
	
	const std::string AlarmFilterDlg::FILTER_OMTYPE("OMType");
	const std::string AlarmFilterDlg::FILTER_OMTYPE_OPERATION("OMType Operation");
	const std::string AlarmFilterDlg::FILTER_OMTYPE_MAINTENANCE("OMType Maintenance");
	const std::string AlarmFilterDlg::FILTER_OMTYPE_BOTH("OMType Both");
	const std::string AlarmFilterDlg::FILTER_OMTYPE_ALL("OMType All");
	...
	AlarmFilterDlg::AlarmFilterDlg(TA_Base_Bus::AlarmListCtrl& alarmList, CWnd* pParent /*=NULL*/)
		...
		m_alarmList(alarmList),m_SystemDlg(NULL),
		m_subsystemTypeDlg(NULL), m_nCurFilterState(0),
	...
	AlarmFilterDlg::~AlarmFilterDlg()
	{
		...
		if (m_subsystemTypeDlg != NULL)
		{
			delete m_subsystemTypeDlg;
			m_subsystemTypeDlg = NULL;
		}
		if (m_SystemDlg != NULL)
		{
			delete m_SystemDlg;
	...
	DDX_Control(pDX, IDC_SUBSYSTEMTYPE_EDIT, m_subsystemTypeEdit);
	DDX_Control(pDX, IDC_SYSTEM_EDIT, m_systemEdit);
	...
	ON_BN_CLICKED(IDC_SUBSYSTEMTYPE_BUTTON, OnSubsystemTypeButton)
	ON_BN_CLICKED(IDC_SUBSYSTEM_BUTTON, OnSubsystemButton)
	ON_BN_CLICKED(IDC_SYSTEM_BUTTON, OnSystemButton)
...
//	ON_WM_TIMER() //TD17081
	...
	ON_COMMAND_RANGE(IDC_OMTYPE_OPERATION, IDC_OMTYPE_ALL, OnOMTypeButton)
...
// 	}
	...
	void AlarmFilterDlg::clearAllDisplays()
	{
		...
		setOMTypeButton(IDC_OMTYPE_ALL);
...
//         else
	...
	void AlarmFilterDlg::OnSubsystemButton() 
	{
		if (m_subsystemDlg == NULL)
		{
			m_subsystemDlg = new MultipleSelectionDlg("Select Subsystem...");
	...
	void AlarmFilterDlg::OnSubsystemTypeButton() 
	{
		...
		if (m_subsystemTypeDlg == NULL)
		{
			m_subsystemTypeDlg = new MultipleSelectionDlg("Select Subsystem...");
		}
		m_subsystemTypeDlg->setItems(m_subsystemTypes,true);
		m_subsystemTypeDlg->setSelection(m_selectedSubsystemTypes);
		if(m_subsystemTypeDlg->DoModal() == IDOK)
		{
			m_selectedSubsystemTypes = m_subsystemTypeDlg->getSelection();
	...
	void AlarmFilterDlg::OnSystemButton() 
	{
		...
		if (m_SystemDlg == NULL)
		{
			m_SystemDlg = new MultipleSelectionDlg("Select System...");
			m_SystemDlg->setItems(m_systemTypes,true);
		}
		m_SystemDlg->setSelection(m_selectedSystems);
		if(m_SystemDlg->DoModal() == IDOK)
		{
			m_selectedSystems = m_SystemDlg->getSelection();
			m_selectedSubsystemTypes.clear();
			populateSubsystemTypesEditCtrl();
	...
	void AlarmFilterDlg::OnLocationButton() 
	{
		if(m_locationDlg == NULL)
		{
			m_locationDlg = new MultipleSelectionDlg("Select Location...");
	...
	void AlarmFilterDlg::OnOperatorButton() 
	{
		if(m_operatorDlg == NULL)
		{
			m_operatorDlg = new MultipleSelectionDlg("Select Operators...");
	...
	void AlarmFilterDlg::OnAlarmTypeButton() 
	{
		if(m_alarmTypeDlg == NULL)
		{
			m_alarmTypeDlg = new MultipleSelectionDlg("Select Alarm Types...");
  ...
  /**
	...
	*/
	...
	void AlarmFilterDlg::OnOMTypeButton(UINT nId)
	{
		// The state of the "Closed" radio button should be the opposite
		// of the "Acked" radio button.
		setUpdateNeeded(true);
...
/////////////////////////////////////////////////////////////////////////////
			...
			switch (setting.type)
			{
				case TA_Base_Bus::Filter::FILTER_ALARM_TIME:
					...
					else if (setting.comparison == TA_Base_Bus::Filter::COMPARE_LESS_THAN_EQUAL)
					{
						...
						// Populate the date and time
						TA_Base_Bus::Filter::SDateRange dateRange = setting.value.dateRange;
						CTime toDateTime = dateRange.startTime;
						m_toDate.SetTime(&toDateTime);
						m_toTime.SetTime(&toDateTime);
                        m_fromTime.SetTime(&toDateTime);
				...
				case TA_Base_Bus::Filter::FILTER_ALARM_TIME_RANGE:
                    ...
                    m_fromTime.SetRange(&EARLY_TIME_LIMIT,&toDateTime);
                    m_toDate.SetRange(&fromDateTime,&LATEST_TIME_LIMIT);
                    m_fromDate.SetRange(&fromDateTime,&LATEST_TIME_LIMIT);
				...
				case TA_Base_Bus::Filter::FILTER_SYSTEMTYPE:
						{
							stringValue = *(setting.value.stringValue);
							// Find stringValue in m_systemTypes and then add the item to m_selectedSystems
							for(ListItems::iterator it = m_systemTypes.begin(); it != m_systemTypes.end(); ++it)
							{
								if(stringValue.compare((*it).str) == 0)
								{
	...
	/////////////////////////////////////////////////////////////////////////////
	void AlarmFilterDlg::retrieveSubsystemTypes()
	{
		m_subsystemTypes.clear();
		// get subsystems from systems
		m_subsystemTypes=SubsystemDataCache::getInstance().getSubsystemsFormSystems(m_selectedSystems);
		// sort the subsystems
		m_subsystemTypes.sort();
	}
	/////////////////////////////////////////////////////////////////////////////
	void AlarmFilterDlg::retrieveSystems()
	{
		// store the subsystems into the map and delete the temporary structure
		m_systemTypes.clear();
        SubsystemDataCache::getInstance().getAllSystems(m_systemTypes);
		// sort the subsystems
		m_systemTypes.sort();
	}
...
//			namedes += (dynamic_cast<TA_Base_Core::AlarmSeverityData*>(allSeverityData[i]))->getDescription().c_str();
	...
	void AlarmFilterDlg::populateOMType(const TA_Base_Bus::Filter::FilterDetails& setting)
	{
		switch (setting.value.alarmOMTypeValue)
		{
			case TA_Base_Bus::Filter::OMTYPE_OPERATION:
				setOMTypeButton(IDC_OMTYPE_OPERATION);
				break;
			case TA_Base_Bus::Filter::OMTYPE_MAINTENANCE:
				setOMTypeButton(IDC_OMTYPE_MAINTENANCE);
				break;
			case TA_Base_Bus::Filter::OMTYPE_BOTH:
				setOMTypeButton(IDC_OMTYPE_BOTH);
				break;
	...
	/////////////////////////////////////////////////////////////////////////////
	void AlarmFilterDlg::addOMTypeFilters(TA_Base_Bus::Filter& filter)
	{
		bool filterOnOperation  = ( static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_OPERATION))->GetCheck() == 1 )? true : false;
		bool filterOnMaintenance= ( static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_MAINTENANCE))->GetCheck() == 1)? true : false;
		bool filterOnBoth       = ( static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_BOTH))->GetCheck() == 1)? true : false;
		if(filterOnOperation)
		{
			filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
					TA_Base_Bus::Filter::COMPARE_EQUAL, TA_Base_Bus::Filter::OMTYPE_OPERATION);
		}
		else if(filterOnMaintenance)
		{
			filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
				TA_Base_Bus::Filter::COMPARE_EQUAL, TA_Base_Bus::Filter::OMTYPE_MAINTENANCE);
		}
		else if(filterOnBoth)
		{			
			filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
	...
	/////////////////////////////////////////////////////////////////////////////
	void AlarmFilterDlg::setOMTypeButton(int resourceID)
	{
		TA_ASSERT(resourceID == IDC_OMTYPE_OPERATION || resourceID == IDC_OMTYPE_MAINTENANCE
					|| resourceID == IDC_OMTYPE_BOTH || resourceID == IDC_OMTYPE_ALL, 
					"Invalid resource ID for setAckRadioButton");

		static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_OPERATION))->SetCheck(IDC_OMTYPE_OPERATION == resourceID);
		static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_MAINTENANCE))->SetCheck(IDC_OMTYPE_MAINTENANCE == resourceID);
		static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_BOTH))->SetCheck(IDC_OMTYPE_BOTH == resourceID);
		static_cast<CButton*>(GetDlgItem(IDC_OMTYPE_ALL))->SetCheck(IDC_OMTYPE_ALL == resourceID);
	}
...
//		m_okButton.EnableWindow(false);  //changxing:need not do this.
	...
	TA_Base_Bus::Filter AlarmFilterDlg::convertNamedFilterToFilter (TA_Base_Core::NamedFilter& nmFilter)
	{
		...
		try
		{
			...
			sVector = nmFilter.getStringFilter(AlarmFilterDlg::FILTER_SUBSYSTEMTYPE);
			if (sVector.size() > 0)
			{
			...
			sVector.clear();
			sVector = nmFilter.getStringFilter(AlarmFilterDlg::FILTER_SYSTEM);
			if (sVector.size() > 0)
			{
				for ( its = sVector.begin(); its != sVector.end(); ++its )
				{
			...
			// add filter on asset
			sVector.clear();
			sVector = nmFilter.getStringFilter(AlarmFilterDlg::FILTER_EQUIPMENT_ID);
			if (sVector.size() > 0) 
			{
				for ( its = sVector.begin(); its != sVector.end(); ++its )
				{
			...
			std::string omtype;
			sVector.clear();
			sVector = nmFilter.getStringFilter(AlarmFilterDlg::FILTER_OMTYPE);
			if (sVector.size() >0) omtype = sVector[0].c_str();
			if(omtype == AlarmFilterDlg::FILTER_OMTYPE_OPERATION)
			{
				filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
					TA_Base_Bus::Filter::COMPARE_EQUAL, TA_Base_Bus::Filter::OMTYPE_OPERATION);
			}
			else if(omtype == AlarmFilterDlg::FILTER_OMTYPE_MAINTENANCE)
			{
				filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
					TA_Base_Bus::Filter::COMPARE_EQUAL, TA_Base_Bus::Filter::OMTYPE_MAINTENANCE);
			}
			else if(omtype == AlarmFilterDlg::FILTER_OMTYPE_BOTH)
			{
				filter.addFilter(TA_Base_Bus::Filter::FILTER_OMTYPE,
					TA_Base_Bus::Filter::COMPARE_EQUAL, TA_Base_Bus::Filter::OMTYPE_BOTH);
			}
		...
		try
		{
			...
			for (std::vector<TA_Base_Bus::Filter::FilterDetails>::iterator iter = filterDetails.begin() ; iter != filterDetails.end() ; ++iter )
			{
				...
				switch (fDetails.type)
				{
					...
					case TA_Base_Bus::Filter::FILTER_SUBSYSTEMTYPE:
						if (fDetails.comparison == TA_Base_Bus::Filter::COMPARE_EQUAL)
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_SUBSYSTEMTYPE, *fDetails.value.stringValue);
						break;
	
					case TA_Base_Bus::Filter::FILTER_SYSTEMTYPE:
							if (fDetails.comparison == TA_Base_Bus::Filter::COMPARE_EQUAL)
								nmFilter.addStringFilter(AlarmFilterDlg::FILTER_SYSTEM, *fDetails.value.stringValue);
							break;
					...
					case TA_Base_Bus::Filter::FILTER_OMTYPE:
						...
						{
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_OMTYPE, AlarmFilterDlg::FILTER_OMTYPE_OPERATION);
						}
						else if (fDetails.value.alarmOMTypeValue == TA_Base_Bus::Filter::OMTYPE_MAINTENANCE)
						{
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_OMTYPE, AlarmFilterDlg::FILTER_OMTYPE_MAINTENANCE);
						} 
						else if (fDetails.value.alarmOMTypeValue == TA_Base_Bus::Filter::OMTYPE_BOTH)
						{
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_OMTYPE, AlarmFilterDlg::FILTER_OMTYPE_BOTH);
						} 
						else if (fDetails.value.alarmOMTypeValue == TA_Base_Bus::Filter::OMTYPE_ALL)
						{
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_OMTYPE, AlarmFilterDlg::FILTER_OMTYPE_ALL);
						} 
						break;
					...
					case TA_Base_Bus::Filter::FILTER_ASSET_NAME:
						if (fDetails.comparison == TA_Base_Bus::Filter::COMPARE_CONTAINS)
							nmFilter.addStringFilter(AlarmFilterDlg::FILTER_EQUIPMENT_ID, *fDetails.value.stringValue);
						break;
	...
	bool AlarmFilterDlg::checkInNamedFilterDatabase(std::string curName)
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Data Exception: Could not get a list of named filters!");
		}
		catch (TA_Base_Core::DatabaseException & )
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Database Exception: Could not get a list of named filters!");
		}
		catch(...)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Unknown exception occured! Could not get a list of named filters!");
		}

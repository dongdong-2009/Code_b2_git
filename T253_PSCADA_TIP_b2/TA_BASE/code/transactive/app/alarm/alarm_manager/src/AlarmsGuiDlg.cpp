 ...
 * in any form.
 *
 * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/alarm/alarm_manager/src/AlarmsGuiDlg.cpp $
 * @author:  Andy Siow
 * @version: $Revision: #2 $
 *
 * Last modification: $DateTime: 2018/01/25 16:52:25 $
...
BEGIN_MESSAGE_MAP(CAlarmsGuiDlg, TransActiveDialog)
	...
	ON_MESSAGE(WM_ON_FILTER_DLG_UPDATE,OnFilterUpdate)
	ON_WM_TIMER()
	//TD18095,jianghp
...
void CAlarmsGuiDlg::PopulateAlarmToolbar()
{
    ...
    m_alarmsToolbar.SetFont(&font,FALSE);
    ...
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Ack")) != -1);
	...
	VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Default Sort")) != -1);
	...
	VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Close Alarm")) != -1);
	...
	//hid the close alarm button for 955 project[lwk]
	...
	//Add Filters button
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Filter")) != -1);
    ...
    //Add Suppress button
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Inhibit")) != -1);
    ...
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Show Inhibits")) != -1);
	...
	//Add Ack All button
	VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Ack All")) != -1);
	...
	// Add Print button
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Print")) != -1);
	...
	// Add Export button
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Export")) != -1);
    ...
    // Add Comment button
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Comment")) != -1);
	...
	//hid the Comment button for 955 project [lwk]
    tbButton.fsState = TBSTATE_HIDDEN;
    ...
    VERIFY((tbButton.iString = m_alarmsToolbar.AddStrings("Event Viewer")) != -1);
...
void CAlarmsGuiDlg::Print()
{
	...
	//TD14929
	CString header = "Alarms Manager: " + CTime::GetCurrentTime().Format(dateFormatStr.c_str())
        + " - " + oper.c_str() + "  on console " + console.c_str();
	// ++ TD12474 
...
//	{
	...
	/*	ascSort.removeAll();
		...
		//TD ++#12719
		TA_Base_Bus::IAlarmAction* doubleClickAction = new TA_Base_Bus::ActionDisplayComment();*/
		//disable Comment column for 955 project.[lwk]

...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/TreeNodeFactory.cpp $
// @author:  Bart Golab
// @version: $Revision: #9 $
//
...
TreeNodeFactory::~TreeNodeFactory()
{
    ...
    while (!m_treeNodeMap.empty())
    {
        ...
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[Node] Tree Node deleting [%lu]", treeNode->getNodeId());
        delete treeNode;
    }
...
unsigned long TreeNodeFactory::getRootNodeId()
{
...
TreeNode *TreeNodeFactory::createTreeNode(unsigned long nodeId)
{
...
TreeNode *TreeNodeFactory::createTreeNode(const TA_Base_Core::NodeSummary &nodeSummary)
{
    ...
    else
    {
        ...
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[Node] Tree Node inserted [%lu]", treeNode->getNodeId());
    }
...
void TreeNodeFactory::createChildren(unsigned long nodeId, std::vector<TreeNode*>& children)
{
...
void TreeNodeFactory::createRootChildren(std::vector<TreeNode*>& children)
{
...
void TreeNodeFactory::retireTreeNode(unsigned long nodeId)
{
    FUNCTION_ENTRY( "retireTreeNode" );

    TreeNode* treeNode = NULL;

    if (m_treeNodeMap.find(nodeId, treeNode))
    {
        m_treeNodeMap.erase(nodeId);

        if ( NULL != treeNode )
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[Node] Tree Node deleted [%lu]", nodeId);
            delete treeNode;
        }
    }
...
TreeNode* TreeNodeFactory::getTreeNode(unsigned long nodeId)
{
...
TreeNode *TreeNodeFactory::getClosestAncestor(unsigned long nodeId)
{
...
TreeNode *TreeNodeFactory::getTreeNodeOrClosestAncestor(unsigned long nodeId)
{
...
void TreeNodeFactory::getNodeIdsInNodePath(unsigned long nodeId, std::vector<unsigned long>& nodeIds)
{
...
void TreeNodeFactory::createTreeNodes(const TA_Base_Core::SiblingNodes& nodes, std::vector<TreeNode*>& treeNodes)
{
    ...
    for (CORBA::ULong i = 0; i < nodes.length(); i++)
    {
		...
		if (TA_Base_Core::PLANTEMPLATE_NODE == nodes[i].type || TA_Base_Core::PLANTEMPLATE_INSTANCE_NODE == nodes[i].type)
			continue;

        if (( TA_Base_Core::PLAN_NODE == nodes[i].type ) || ( TA_Base_Core::WORKFLOW_NODE == nodes[i].type ))
        {
...
int TA_Base_App::TreeNodeFactory::getTreeNodeLevelInTree( unsigned long nodeId )
{
...
bool TA_Base_App::TreeNodeFactory::isTreeNodeVisible( unsigned long nodeId )
{
...
bool TA_Base_App::TreeNodeFactory::canTreeNodeBeShowed( unsigned long nodeId )
{
    return !( !Utilities::isPlanManagerConfigurationMode() && ! isTreeNodeVisible(nodeId) );
}

void TA_Base_App::TreeNodeFactory::getAllTreeNode( std::vector<TreeNode*>& refTreeNodes )
{
    return m_treeNodeMap.getItems( refTreeNodes );
}

...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanTreeDoc.cpp $
// @author:  Bart Golab
// @version: $Revision: #22 $
//
...
BEGIN_MESSAGE_MAP( CPlanTreeDoc, CPlanBrowserDoc )
    ...
    ON_COMMAND( ID_WORK_FLOW_PLAN_NEW, OnNewWorkflowPlan )
        ...
        default:
            {
                try
                {
                    ...
                    for( TreeNodeList::iterator iter = selectedNodes.begin(); iter != selectedNodes.end(); iter++ )
                    {
                        ...
                        bEnable &= treeNode->isEnable( pCmdUI->m_nID );
                    }
            ...
            do
            {
                ...
                try
                {
                    ...
                    if( inFile.Open( impFilePath, CFile::modeRead ) )
                    {
                        ...
                        UINT nBufSize = inFile.GetLength();
                        ...
                        for( int curLine = 0; curLine < newLineIdxArray.GetSize(); curLine++ )
                        {
                            ...
                            else if( currentLine.Compare( "Steps" ) == 0 )
                            {
                                ...
                                for( int step = 0; step < numSteps; step++ )
                                {
                                    ...
                                    for( int curValue = 0; curValue < newStepDetailPos.GetSize(); curValue++ )
                                    {
                                        ...
                                        {
                                            case 0:
                                                plan->steps[step].name = tmpValue.c_str();
                                                break;
                                            case 1:
                                                plan->steps[step].type = static_cast<int>( atol( tmpValue.c_str() ) );
                                                break;
                                            case 2:
                                                plan->steps[step].description = tmpValue.c_str();
                                                break;
                                            case 3:
                                                plan->steps[step].position = static_cast <TA_Base_Core::StepNumber>( atol( tmpValue.c_str() ) );
                                                break;
                                            case 4:
                                                plan->steps[step].graph = tmpValue.c_str();
                                                break;
                                            case 5:
                                                plan->steps[step].id = static_cast <TA_Base_Core::StepNumber>( atol( tmpValue.c_str() ) );
                                                break;
                                            case 6:
                            ...
                            else if( currentLine.Compare( "Sequence_Flows" ) == 0 )
                            {
                                curLine++;
                                // get number of flows
                                tmp = fileBuffer.Right( fileBuffer.GetLength() - newLineIdxArray.GetAt( curLine ) - 1 );
                                currentLine = tmp.Left( tmp.Find( "\n" ) );
                                int numFlows = atoi( currentLine.GetBuffer( 1 ) );

                                // for each flow store the details
                                for( int flow = 0; flow < numFlows; flow++ )
                                {
                                    ...
                                    {
                                        "id==", "incomingStep==", "outgoingStep==", "graph==", "condition=="
                                    };
                                    TA_Base_Core::AuxSequenceFlow seqFlow;
                                    std::string line = LPCTSTR( currentLine );
                                    std::string::size_type pos = 0, pos2;
                                    int i;
                                    for( i = 0; i < 5; ++i )
                                    {
                                        pos = line.find( searchStr[i], pos );
                                        if( std::string::npos == pos )
                                        {
                                            break;
                                        }
                                        pos += strlen( searchStr[i] );
                                        pos2 = line.find( ";", pos );
                                        std::string value;
                                        ...
                                        {
                                            case 0:
                                                seqFlow.id = TA_Base_Core::DataConversion::toUnsignedInteger( value );
                                                break;
                                            case 1:
                                                seqFlow.incomingStep = TA_Base_Core::DataConversion::toUnsignedInteger( value );
                                                break;
                                            case 2:
                                                seqFlow.outgoingStep = TA_Base_Core::DataConversion::toUnsignedInteger( value );
                                                break;
                                            case 3:
                                                seqFlow.graph = value.c_str();
                                                break;
                                            case 4:
                                                seqFlow.condition = value.c_str();
                                                break;
                                            default:
                                                break;
                                        }
                                    ...
                                    plan->sequenceFlows.length( plan->sequenceFlows.length() + 1 );
                                    plan->sequenceFlows[plan->sequenceFlows.length() - 1] = seqFlow;
                                }
                            }
                        }
                    }
            ...
            if( expFileDlg.DoModal() == IDOK )
            {
                ...
                if( outFile.Open( exportFilePath, CFile::modeCreate | CFile::modeWrite ) )
                {
                    ...
                    for( unsigned int i = 0; i < plan->steps.length(); i++ )
                    {
                        ...
                        textToWrite += ";";
						textToWrite += "graph==";
                        textToWrite += plan->steps[i].graph;
                        textToWrite += ";";
                        textToWrite += "id==";
                        textToWrite += TA_Base_Core::DataConversion::toString( plan->steps[i].id ).c_str();
                        textToWrite += ";";
                        textToWrite += "skip==";
                    ...
                    // Sequence flow
                    textToWrite = "Sequence_Flows\n";
                    textToWrite += TA_Base_Core::DataConversion::toString( plan->sequenceFlows.length() ).c_str();
                    textToWrite += "\n";
                    outFile.Write( textToWrite.GetBuffer( 1 ), textToWrite.GetLength() );
                    for( unsigned int i = 0, sz = plan->sequenceFlows.length(); i < sz; ++i )
                    {
                        ...
                        textToWrite = "id==";
                        textToWrite += TA_Base_Core::DataConversion::toString( seqFlow.id ).c_str();
                        textToWrite += ";";
                        textToWrite += "incomingStep==";
                        textToWrite += TA_Base_Core::DataConversion::toString( seqFlow.incomingStep ).c_str();
                        textToWrite += ";";
                        textToWrite += "outgoingStep==";
                        textToWrite += TA_Base_Core::DataConversion::toString( seqFlow.outgoingStep ).c_str();
                        textToWrite += ";";
                        textToWrite += "graph==";
                        textToWrite += seqFlow.graph.in();
                        textToWrite += ";";
                        textToWrite += "condition==";
                        textToWrite += seqFlow.condition.in();
                        textToWrite += ";\n";
                        outFile.Write( textToWrite.GetBuffer( 1 ), textToWrite.GetLength() );
                    }
...
void CPlanTreeDoc::OnNewPlan()
{
    ...
    {
        CWaitCursor waitCursor;
        categoryNode->newPlan(TA_Base_Core::PLAN_NODE);
    }
    catch( TA_Base_Core::TransactiveException&  ex )
    {
...
void CPlanTreeDoc::OnNewWorkflowPlan()
{
    FUNCTION_ENTRY( "OnNewWorkflowPlan" );
    ...
    {
        CWaitCursor waitCursor;
        categoryNode->newPlan(TA_Base_Core::WORKFLOW_NODE);
    }
    catch( TA_Base_Core::TransactiveException&  ex )
    {
        MessageBox::error( IDS_CREATE_NEW_PLAN, ex );
...
void CPlanTreeDoc::OnEditStep()
{
    ...
    try
    {
        ...
        if( NULL != planStep )
        {
            CPlanStepEditorDlg dlg( planStep, false );
            dlg.DoModal();
        }
    }
    catch( TA_Base_Core::TransactiveException& ex )

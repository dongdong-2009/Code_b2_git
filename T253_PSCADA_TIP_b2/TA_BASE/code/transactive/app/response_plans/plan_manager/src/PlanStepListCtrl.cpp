/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepListCtrl.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #21 $
  *
  * Last modification: $DateTime: 2015/10/16 10:27:23 $
...
void CPlanStepListCtrl::PreSubclassWindow()
{
    ...
    InsertColumn(SLC_STEP_TYPE, _T(LabelMaker::getStepListTypeColumnLabel()), LVCFMT_LEFT, 190);
    ...
    InsertColumn(SLC_STEP_REMARK, _T(LabelMaker::getStepListRemarkColumnLabel()), LVCFMT_LEFT, 400);
    ...
    if (pLVCD->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
    {
        ...
        switch (stepImage)
        {
            case PSIT_EXECUTED_STEP:
            {
                ...
                pLVCD->clrText = (planStep->hasCustomOutgoingStep()? EXECUTED_BRANCHING_STEP_COLOUR : EXECUTED_SIMPLE_STEP_COLOUR);
                break;
            }
...
void CPlanStepListCtrl::AddStep(const PlanStep& planStep)
{
    FUNCTION_ENTRY( "AddStep" );

    int item = InsertItem(GetItemCount(), _T(""), PSIT_UNEXECUTED_STEP);

    SetItemText( item, SLC_STEP_POSITION, _T( TA_Base_Bus::StringUtilities::convertToString( planStep.getStepPosition() ) ) );
    SetItemText( item, SLC_STEP_NAME, _T( planStep.getStepName().c_str() ));
	SetItemText(item, SLC_STEP_TYPE, _T( planStep.getStepTypeName().c_str() ));
    SetItemText( item, SLC_STEP_DESCRIPTION, _T( planStep.getStepDescription().c_str() ) );
    ...
    if (!m_bEditing)
    {
        SetItemText(item, SLC_STEP_REMARK, "");
    }
...
void CPlanStepListCtrl::SetSelectedSteps(unsigned long stepNumber, unsigned long stepCount, bool scrollIntoView)
{
    FUNCTION_ENTRY( "SetSelectedSteps" );

    // Find the item to select
    int topItem = FindStepByPosition(stepNumber);
    if (topItem != -1)
    {
...
void CPlanStepListCtrl::ResetCustomStepSettings()
{
    ...
    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[Pointer] %s", "ResetCustomStepSettings" );

    int itemCount = GetItemCount();
    for (int item = 0; item < itemCount; item++)
    {
        PlanStep *planStep = GetStep(item);
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[Pointer] Step [%p] used by list ctrl", planStep );
...
void CPlanStepListCtrl::UpdatePlanExecutionProgress(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
    ...
    if ( TA_Base_Core::STOPPED_ACTIVE_PLAN_STATE == activePlanDetail.activePlanState )
    {
        for ( int nItem = 0; nItem < GetItemCount(); ++nItem )
        {
            EPlanStepImageType stepImage = GetStepImage(nItem);
            CString stepRemark = !m_bEditing ? GetItemText(nItem, SLC_STEP_REMARK) : "";
            ...
            SetStepImageAndSelection(nItem, stepImage, false);

            if (!m_bEditing)
            {
                SetItemText(nItem, SLC_STEP_REMARK, stepRemark);
            }
...
void CPlanStepListCtrl::UpdateStepExecutionProgress(const TA_Base_Core::ActiveStepDetail &activeStepDetail, bool scrollIntoView)
{
    FUNCTION_ENTRY( "UpdatePlanExecutionProgress" );
    ...
    int item = FindStepById(activeStepDetail.id);
    if (item != -1)
    {
        // Set the item's image and selection state.
        ...
        if ( !m_bEditing )
        {
            SetItemText(item, SLC_STEP_REMARK, activeStepDetail.remark.in());
        }
...
void CPlanStepListCtrl::UpdateCustomStepSettings(const TA_Base_Core::StepCustomisableDetail &stepCustomDetail)
{
    ...
    // Find the item to update
    int item = FindStepById(stepCustomDetail.id);
    if (item != -1)
    {
        PlanStep *planStep = GetStep(item);
...
int CPlanStepListCtrl::FindStepById(unsigned long stepId)
{
    FUNCTION_ENTRY( "FindStepById" );
...
int CPlanStepListCtrl::FindStepByPosition(unsigned long stepPosition)
{
	FUNCTION_ENTRY( "FindStepByPostion" );

	int itemCount = GetItemCount();
	if (stepPosition >= 1 && stepPosition <= static_cast<unsigned long>(itemCount))
	{
		// Start the search with the item at position (stepNumber - 1).
		// This should result in a direct hit.
		int item = 0;
		for (item = stepPosition - 1; item < GetItemCount(); item++)
		{
			PlanStep *planStep = GetStep(item);

			if (planStep->getStepPosition() == stepPosition)
			{
				FUNCTION_EXIT;
				return item;
			}
		...
		// Item not found at or above position (stepNumber - 1). Wrap around
		// to the top and search from there.
		for (item = 0; static_cast<unsigned long>(item) < stepPosition - 1; item++)
		{
			PlanStep *planStep = GetStep(item);
...
void CPlanStepListCtrl::setEditing(const bool bEditing)
{
    // By default , m_bEditing is true and Column SLC_STEP_REMARK is inserted
    if ( bEditing && !m_bEditing )
    {
        DeleteColumn( SLC_STEP_REMARK );
    }

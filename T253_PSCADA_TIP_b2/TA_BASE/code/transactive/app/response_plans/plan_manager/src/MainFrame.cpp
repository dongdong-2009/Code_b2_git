...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/MainFrame.cpp $
// @author:  Bart Golab
// @version: $Revision: #11 $
//
...
BEGIN_MESSAGE_MAP(CMainFrame, TransActiveMDIFrame)
	...
	ON_NOTIFY(TBN_DROPDOWN, IDR_CATEGORY_NODE_TOOLBAR, OnToolbarDropDown)
	ON_NOTIFY(TBN_DROPDOWN, IDR_PLAN_NODE_TOOLBAR, OnToolbarDropDown)
	ON_NOTIFY(TBN_DROPDOWN, IDR_PLAN_NODE_ADMIN_TOOLBAR, OnToolbarDropDown)
    // }}AFX_MSG_MAP
END_MESSAGE_MAP()
...
CMainFrame::CMainFrame(TA_Base_Bus::AbstractGUIApplication* appGUI) : TransActiveMDIFrame()
{
...
CMainFrame::CMainFrame() : TransActiveMDIFrame()
{
    FUNCTION_ENTRY( "CMainFrame" );
    FUNCTION_EXIT;
}
...
void CMainFrame::OnDestroy()
{
    ...
    TA_Base_Bus::TransActiveMDIFrame::OnDestroy();
}
...
void CMainFrame::OnViewPlan(UINT nCommandID)
{
    ...
    case ID_VIEW_ALLPLANS:
        strNewTitle += " - All Plans";
        break;

    case ID_VIEW_APPROVEDPLANS:
        strNewTitle += " - Approved Plans";
        break;

    case ID_VIEW_UNAPPROVEDPLANS:
        strNewTitle += " - Unapproved Plans";
        break;

    case ID_VIEW_ACTIVEPLANS:
        strNewTitle += " - Active Plans";
        break;
...
void CMainFrame::SetBaseTitle( const std::string& strTitle )
{
...
void CMainFrame::OnToolbarDropDown( NMHDR* pnmtb, LRESULT *plr)
{
	...
	*plr = 0;
	...
	if(ID_PLAN_NEW != ptoolbar->iItem)
	{
		return;
	}

	CMenu menu;
	menu.LoadMenu(IDR_PLAN_NEW_DROP_DOWN);
	CMenu* pPopup = menu.GetSubMenu(0);

	CRect rc = m_toolBarManager.getBoundingRect(ID_PLAN_NEW);

...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/CategoryNode.cpp $
// @author:  Bart Golab
// @version: $Revision: #6 $
//
...
bool CategoryNode::isEnable(const int nAction)
{
    ...
    switch (nAction)
    {
        ...
        case ID_CATEGORY_OR_PLAN_DELETE:
            bEnable = !getNodeSummary().hasChildren && OperatorRights::getInstance().canEditCategories();
            break;
...
void CategoryNode::newPlan(TA_Base_Core::ENode nodeType)
{
    FUNCTION_ENTRY( "newPlan" );

    // Create new plan associated with this node
    PlanAgentAccess::getInstance().addPlan(getNodeId(), nodeType);

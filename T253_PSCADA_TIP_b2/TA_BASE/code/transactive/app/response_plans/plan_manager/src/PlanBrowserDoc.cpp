...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanBrowserDoc.cpp $
// @author:  Bart Golab
// @version: $Revision: #8 $
//
...
BEGIN_MESSAGE_MAP(CPlanBrowserDoc, CDocument)
    ...
    ON_COMMAND(ID_ACTIVE_PLAN_CLEARED_UPDATE, OnActivePlanClearedUpdate)
    ON_COMMAND(ID_ACTIVE_PLAN_DETAIL_UPDATE, OnActivePlanDetailUpdate)
    ON_COMMAND(ID_ACTIVE_STEP_DETAIL_UPDATE, OnActiveStepDetailUpdate)
...
CPlanBrowserDoc::~CPlanBrowserDoc()
{
    ...
    while (!m_activePlanClearedUpdateQueue.empty())
    {
        delete m_activePlanClearedUpdateQueue.front();
        m_activePlanClearedUpdateQueue.pop_front();
    }

    while (!m_activeStepDetailUpdateQueue.empty())
    {
        delete m_activeStepDetailUpdateQueue.front();
        m_activeStepDetailUpdateQueue.pop_front();
    }
...
void CPlanBrowserDoc::OnActivePlanClearedUpdate()
{
    FUNCTION_ENTRY( "OnActivePlanClearedUpdate" );

    while (!m_activePlanClearedUpdateQueue.empty())
    {
        ActivePlanClearedUpdate* activePlanClearedUpdate = m_activePlanClearedUpdateQueue.front();
        m_activePlanClearedUpdateQueue.pop_front();

        if ( NULL != activePlanClearedUpdate )
        {
            UpdateAllViews(NULL, 0, activePlanClearedUpdate);

            delete activePlanClearedUpdate;
            activePlanClearedUpdate = NULL;
        }
    }
...
void CPlanBrowserDoc::OnActiveStepDetailUpdate()
{
    FUNCTION_ENTRY( "OnActiveStepDetailUpdate" );

    while (!m_activeStepDetailUpdateQueue.empty())
    {
        ActiveStepDetailUpdate* activeStepDetailUpdate = m_activeStepDetailUpdateQueue.front();
        m_activeStepDetailUpdateQueue.pop_front();

        if (activeStepDetailUpdate)
        {
            UpdateAllViews(NULL, 0, activeStepDetailUpdate);

            delete activeStepDetailUpdate;
            activeStepDetailUpdate = NULL;
        }
    }
...
void CPlanBrowserDoc::activePlanCleared(const unsigned long ulLocationKey)
{
    FUNCTION_ENTRY( "activePlanCleared" );

    if (m_parentFrameHWND)
    {
        m_activePlanClearedUpdateQueue.push_back(new ActivePlanClearedUpdate(ulLocationKey));
        ...
        for (unsigned int attempts = 1; attempts <= MAX_SEND_UPDATE_ATTEMPTS; attempts++)
        {
            ...
            if (::PostMessage(m_parentFrameHWND, WM_COMMAND, MAKEWPARAM(ID_ACTIVE_PLAN_CLEARED_UPDATE, 0), 0))
            {
                FUNCTION_EXIT;
                return;
            }
            ...
            LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugWarn,
                "Failed to post active plan cleared update, retrying...");
        }
...
void CPlanBrowserDoc::activePlanStateChanged(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
...
// IActivePlanObserver interface
void CPlanBrowserDoc::activeStepStateChanged(const TA_Base_Core::PlanExecuteStepUpdate &activeStepDetail)
{
    FUNCTION_ENTRY( "activeStateChanged" );

    if (m_parentFrameHWND)
    {
        m_activeStepDetailUpdateQueue.push_back(new ActiveStepDetailUpdate(activeStepDetail));

        for (unsigned int attempts = 1; attempts <= MAX_SEND_UPDATE_ATTEMPTS; attempts++)
        {
            // The frame does not handle this command. It will be routed back to the document in a thread safe manner.
            if (::PostMessage(m_parentFrameHWND, WM_COMMAND, MAKEWPARAM(ID_ACTIVE_STEP_DETAIL_UPDATE, 0), 0))
            {
                FUNCTION_EXIT;
                return;
            }
            ...
            LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugWarn,
                "Failed to post active step detail update, retrying...");
        }

        LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugWarn,
            "Unable to post active step detail update after %u attempts. Update remains queued.",
            MAX_SEND_UPDATE_ATTEMPTS);
    }

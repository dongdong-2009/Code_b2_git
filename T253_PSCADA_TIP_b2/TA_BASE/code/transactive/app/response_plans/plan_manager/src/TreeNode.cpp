...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/TreeNode.cpp $
// @author:  Bart Golab
// @version: $Revision: #13 $
//
...
TA_Base_Core::ENode TreeNode::getType()
{
    FUNCTION_ENTRY("getType");

    if (NULL == m_nodeSummary)
    {
        m_nodeSummary = PlanAgentAccess::getInstance().getNodeSummary(getNodeId());
    }

    FUNCTION_EXIT;
    return m_nodeSummary->type;
}


void TreeNode::setType(const TA_Base_Core::ENode eType)
{
...
bool TreeNode::isEnable(const int nAction)
{

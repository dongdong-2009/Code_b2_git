/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepListCtrl.h $
  * @author:  Bart Golab
  * @version: $Revision: #12 $
  *
...
namespace TA_Base_App
{
    ...
    public:
        ...
        void AddStep(const PlanStep& planStep);
        PlanStep *GetStep(int item);
        void SetSelectedSteps(unsigned long stepNumber, unsigned long stepCount, bool scrollIntoView = false);
        ...
        void UpdateStepExecutionProgress(const TA_Base_Core::ActiveStepDetail &activeStepDetail, bool scrollIntoView = false);
    ...
    protected:
        int FindStepById(unsigned long stepId); // Returns -1 if step is not found
		int FindStepByPosition(unsigned long stepPosition); // Returns -1 if step is not found

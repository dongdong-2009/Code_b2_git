...
#include "app/response_plans/plan_manager/src/PlanStepEditInclusiveGatewayPage.h"
#include "app/response_plans/plan_manager/src/InclusiveGatewayStep.h"
...
InclusiveGatewayStep::InclusiveGatewayStep( IPlanNode* parent, const BasicParameter& stepDetail ) :
PlanStep(parent,m_params,false),
m_params(stepDetail),
m_parActivityParameter(m_params, *parent)
{
    FUNCTION_ENTRY( "InclusiveGatewayStep" );
    FUNCTION_EXIT;
}


InclusiveGatewayStep::InclusiveGatewayStep( IPlanNode* parent, const unsigned long stepId, const int stepType ) :
...
InclusiveGatewayStep::InclusiveGatewayStep(const InclusiveGatewayStep& planStep) :
...
InclusiveGatewayStep::~InclusiveGatewayStep()
{
    FUNCTION_ENTRY( "~InclusiveGatewayStep" );
...
PlanStep* InclusiveGatewayStep::clone()
{
    ...
    FUNCTION_EXIT;
    return new InclusiveGatewayStep(*this);
}

void InclusiveGatewayStep::createPlanStepParameters()
{
...
CPlanStepEditParameterPage* InclusiveGatewayStep::getParameterEditPage()
{
    FUNCTION_ENTRY( "getParameterEditPage" );
    FUNCTION_EXIT;
    return new CPlanStepEditInclusiveGatewayPage(this);
}

bool InclusiveGatewayStep::isParametersValid( const std::vector<std::string>& parameters )
{
    FUNCTION_ENTRY( "isParametersValid" );

    static const size_t ParameterSize = 2u;

    bool bValid = (ParameterSize == parameters.size());
    if ( !bValid )
    {
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Invalid parameter size [%u] for InclusiveGateway step [%u].", parameters.size(), ParameterSize );
    }
...
std::string InclusiveGatewayStep::getStepTypeName() const
{
	return "Inclusive gateway";
}

IActivityParameter& InclusiveGatewayStep::getActivityParameter()
{
    reloadParameters();
...
void InclusiveGatewayStep::outgoingStepChanged( const unsigned int unPrevOutgoing, const unsigned int unNewOutgoing )
{
...
bool InclusiveGatewayStep::retrieveOutgoings( std::vector<unsigned int>& vecOutgoings )
{
    for ( std::vector<ExpressionPair>::iterator itLoop = m_params.stepExpressionPairs.begin(); itLoop != m_params.stepExpressionPairs.end(); ++itLoop )
    {
...
size_t InclusiveGatewayStep::getShortestPathNum( const std::map<unsigned int, UintSet>& mapPaths, const unsigned int unStart, const unsigned int unEnd )
{
...
void InclusiveGatewayStep::updateFlowParameters()
{
    ...
    for ( std::vector<PlanStep*>::iterator itLoop = vecAllSteps.begin(); vecAllSteps.end() != itLoop; ++itLoop )
    {
        ...
        // Filter step type (require InclusiveGateway type) and step id (not this step)
...
void InclusiveGatewayStep::resetFlowParameters()
{
    m_params.pairStep = 0u;
}

void InclusiveGatewayStep::clearFlowParameters()
{
...
bool InclusiveGatewayStep::hasCustomOutgoingStep() const
{
    return true;
}

bool InclusiveGatewayStep::canIgnoreFailure() const
{
    return false;
}

bool InclusiveGatewayStep::allowMultipleIncoming()
{
    return true;
}

bool InclusiveGatewayStep::allowMultipleOutgoing()
{
    return true;
}

...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepDetailPage.cpp $
// @author:  Bart Golab
// @version: $Revision: #17 $
//
// Last modification: $DateTime: 2015/07/20 18:10:06 $
...
#include "app/response_plans/plan_manager/src/PlanInstanceNone.h"
...
BOOL CPlanStepDetailPage::OnInitDialog()
{
    ...
    {
        m_planStepsListCtrl.setEditing( true );
        m_planStepsListCtrl.ModifyStyle(LVS_SINGLESEL, 0);
    }
    else
    {
        m_planStepsListCtrl.setEditing( false );
...
void CPlanStepDetailPage::OnPlanInstanceComboItemSelected()
{
    ...
    try
    {
        ...
        else
        {
            ...
            oNonePlanInstance.setCustomStepDetails(m_planStepsListCtrl);
            oNonePlanInstance.setProgressDetails(m_planStepsListCtrl);
        }
    }
...
void CPlanStepDetailPage::OnPlanStepsListItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if (stepSelected || stepDeselected)
    {
        ...
        try
        {
            ...
            {
                addParametersToParameterList(m_stepParametersListCtrl);
            }
            else
            {
...
void CPlanStepDetailPage::addParametersToParameterList(CPlanStepParameterListCtrl &listCtrl)
{
	listCtrl.DeleteAllItems();

    std::vector<ItemPair> stepParameters;
    GetDocument()->GetCurrentStep()->getActivityParameter().getNameValuePairs( stepParameters );

    listCtrl.SetItemCount( stepParameters.size() );

    for ( std::vector<ItemPair>::iterator iter = stepParameters.begin(); iter != stepParameters.end(); iter++ )
    {
        listCtrl.AddParameter( iter->DisplayName.c_str(), iter->Item.c_str() );
	}
}
...
LRESULT CPlanStepDetailPage::OnActivePlanReset(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CPlanStepDetailPage::OnActiveStepDetailUpdate(WPARAM wParam, LPARAM lParam)
{
    FUNCTION_ENTRY( "OnActivePlanDetailUpdate" );

    TA_Base_Core::PlanExecuteStepUpdate *activeStepDetail = reinterpret_cast<TA_Base_Core::PlanExecuteStepUpdate *>(wParam);
    TA_ASSERT(activeStepDetail, "Active plan detail pointer is null");
    ...
    if (NULL != planInstance.get())
    {
        if (planInstance->getInstanceId() == m_planInstanceCombo.GetSelectedInstance())
        {
            m_planStepsListCtrl.UpdateStepExecutionProgress(activeStepDetail->detail);
        }
    }
...
void CPlanStepDetailPage::refreshPlanStepList()
{
    GetDocument()->PurgeCurrentSteps();

    m_planStepsListCtrl.DeleteAllItems();

    PlanNode *planNode = GetPlanNode();
    PlanStepList planSteps = planNode->getPlanSteps();

    m_planStepsListCtrl.SetItemCount(planSteps.size());

    for (PlanStepList::const_iterator siter = planSteps.begin(); siter != planSteps.end(); siter++)
    {
        PlanStep* pStep = *siter;
        if ( NULL != pStep )
        {
            m_planStepsListCtrl.AddStep( *pStep );
        }
    }
}
...
void CPlanStepDetailPage::ShowRelevantControls()
{
    ...
    bool bPlanApproved = GetPlanNode()->isApproved();
    bool bEditingEnabled = GetDocument()->EditingEnabled();
    bool bControlEnabled = GetDocument()->ControlEnabled();

    int showActivePlanControls = bPlanApproved ? SW_SHOW : SW_HIDE;
    ...
    m_planStepsListCtrl.setEditing(!bPlanApproved);

    m_planInstanceListCtrl.SetReadOnly(!bEditingEnabled && !bControlEnabled);
    m_planStepsListCtrl.SetReadOnly(!bEditingEnabled && !bControlEnabled);
    m_stepParametersListCtrl.SetReadOnly(!bEditingEnabled && !bControlEnabled);

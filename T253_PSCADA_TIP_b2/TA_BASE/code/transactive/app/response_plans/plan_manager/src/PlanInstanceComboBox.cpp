...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanInstanceComboBox.cpp $
// @author:  Bart Golab
// @version: $Revision: #5 $
//
...
void CPlanInstanceComboBox::AddInstance(const TA_Base_Core::NodeId& planId, const PlanInstanceId& instanceId)
{
...
void CPlanInstanceComboBox::RepositionInstance(const PlanInstanceSharedPtr& planInstance)
{
    ...
    if (item != CB_ERR)
    {
        ...
        if (!currentPositionOK)
        {
            ...
            DeleteString(item);
            AddInstance(planInstance->getActivePlanId().plan, planInstance->getInstanceId());
...
void CPlanInstanceComboBox::RemoveInstances(const unsigned long ulLocationKey)
{
    ...
    for (int item = GetCount() - 1; item >= 0; --item)
    {
        ...
        if ( removedInstance->m_location != ulLocationKey )
        {
            continue;
        }
...
void CPlanInstanceComboBox::OnDestroy()
{

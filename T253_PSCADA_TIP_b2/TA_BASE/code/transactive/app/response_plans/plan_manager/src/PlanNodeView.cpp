...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanNodeView.cpp $
// @author:  Bart Golab
// @version: $Revision: #19 $
//
...
#include "app/response_plans/plan_editor_plugin/workflow_editor/src/WorkflowHelper.h"
...
BEGIN_MESSAGE_MAP(CPlanNodeView, CScrollView)
    ...
    ON_MESSAGE(WM_PLAN_NODE_TYPE_UPGRADE, OnPlanNodeTypeUpgrade)
    // }}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CPlanNodeView::OnInitialUpdate()
{
	...
	//TA_Base_Core::ENode nodeType = GetPlanNode()->getType();

	//if(GetDocument()->EditingEnabled())
	//{
	//	if(TA_Base_Core::WORKFLOW_NODE == nodeType)
	//	{
	//        m_planPropertySheet.EnablePage(1, false);
	//        m_planPropertySheet.EnablePage(2, true);
	//	}
	//	else if(TA_Base_Core::PLAN_NODE == nodeType)
	//    {
	//        m_planPropertySheet.EnablePage(1, true);
	//        m_planPropertySheet.EnablePage(2, false);
	//	}
	//}
...
void CPlanNodeView::OnDestroy()
{
    if ( ::IsWindow( m_planPropertySheet.GetSafeHwnd() ) )
    {
        // Will call destroy window for sub pages of property sheet
        m_planPropertySheet.DestroyWindow();
    }
...
void CPlanNodeView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
    ...
    CViewUpdate<TA_Base_Core::PlanExecuteStepUpdate> *pesUpdate = dynamic_cast<CViewUpdate<TA_Base_Core::PlanExecuteStepUpdate> *>(pHint);
    if (pesUpdate)
    {
        TA_Base_Core::PlanExecuteStepUpdate activeStepDetail = pesUpdate->GetUpdateData();

        if (activeStepDetail.plan.plan == GetPlanNode()->getNodeId())
        {
            SendMessageToDescendants(WM_ACTIVE_STEP_DETAIL_UPDATE, (WPARAM) &activeStepDetail);
        }
    ...
    CViewUpdate<unsigned long> *pciUpdate = dynamic_cast<CViewUpdate<unsigned long> *>(pHint);
    if (pciUpdate)
    {
        SendMessageToDescendants(WM_ACTIVE_PLAN_RESET, (WPARAM) pciUpdate->GetUpdateData(), 0);
...
LRESULT CPlanNodeView::OnResetTreeNodeView(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	if(GetDocument()->EditingEnabled())
	{
		...
		m_planPropertySheet.Invalidate();
	}
...
LRESULT CPlanNodeView::OnPlanNodeTypeUpgrade( WPARAM wParam, LPARAM lParam )
{
    if (GetPlanNode()->getPlanNodeId() != lParam)
	{
		return 0;
	}
	...
	std::map<unsigned int, unsigned int> mapPositionToStepId;
	std::vector<PlanStep*> planStepList = GetPlanNode()->getPlanSteps();
	for(std::vector<PlanStep*>::iterator itLoop = planStepList.begin(); planStepList.end() != itLoop; ++itLoop)
	{
		PlanStep* step = *itLoop;
		if( NULL == step )
		{
			continue;
		}

		mapPositionToStepId.insert(std::make_pair(step->getStepPosition(), step->getStepId()));
	}

	WorkflowHelper::instance().autoPositions(mapPositionToStepId, mapTypeToData, mapSequenceFlows);

    GetPlanNode()->updateGraph( mapTypeToData, mapSequenceFlows );
	GetPlanNode()->activityUpdated(0u);

	if(GetDocument()->EditingEnabled())
	{
		// Disable the tab CPlanStepDetailPage
	    m_planPropertySheet.EnablePage(1, false);
	    m_planPropertySheet.EnablePage(2, true);

...
// in any form.
//
// Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanInstance.cpp $
// @author:  Bart Golab
// @version: $Revision: #16 $
//
...
void TA_Base_App::PlanInstance::getStepExecDetails( std::vector<TA_Base_Core::ActiveStepDetail>& vecDetails )
{
    getStepExecDetailMap().getItems( vecDetails );
}
...
TA_Base_Core::StepCustomisableDetail PlanInstance::getCustomStepDetail(PlanStep *planStep)
{
    ...
    if (getStepCustomDetailMap().find(planStep->getStepId()))
    {
        customStepDetail = getStepCustomDetailMap()[planStep->getStepId()];
    }
    else
    {
        customStepDetail.id = planStep->getStepId();
...
void PlanInstance::updateCmdUI(CCmdUI &cmdUI, PlanNode &planNode, PlanStepNumber stepPosition)
{
    ...
    {
        case ID_PLAN_STOP:
            cmdUI.Enable(m_ownedByCurrentSession && (isRunning || isPaused) && OperatorRights::getInstance().canControlPlans());
            break;

        case ID_PLAN_PAUSE:
            cmdUI.Enable(m_ownedByCurrentSession && (isRunning ) && OperatorRights::getInstance().canControlPlans());
            break;

        case ID_PLAN_RESUME_SELECTED_STEP:
        case ID_PLAN_RESUME:
            cmdUI.Enable(m_ownedByCurrentSession && (isPaused) && OperatorRights::getInstance().canControlPlans());
            break;

        case ID_PLAN_TAKECONTROL:
            cmdUI.Enable(!m_ownedByCurrentSession && (isRunning || isPaused) &&
                         OperatorRights::getInstance().canControlPlans() && OperatorRights::getInstance().canTakePlanOwnership());
            break;

        case ID_STEP_CUSTOMISE:
            m_customisationEnabled = m_ownedByCurrentSession && (isPaused) &&
...
void PlanInstance::addToPlanInstanceCombo(CPlanInstanceComboBox &instanceComboBox)
{
    ...
    instanceComboBox.AddInstance(getActivePlanId().plan, getInstanceId());
...
void PlanInstance::setProgressDetails(CPlanStepListCtrl &stepListCtrl)
{
    ...
    {
        stepListCtrl.UpdateStepExecutionProgress(*iter);
    }
...
void PlanInstance::update(const TA_Base_Core::PlanExecuteStepUpdate &activeStepDetail)
{
    StepExecDetailMap& stepExecDetailMap = getStepExecDetailMap();

    stepExecDetailMap[activeStepDetail.detail.id] = activeStepDetail.detail;
}
...
void PlanInstance::update(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
...

    {
        ...
        {
            std::string strCurSession = m_activePlanDetail.session.in();

            m_activePlanDetail = activePlanDetail;

            bUpdateOwnership = ( strCurSession.compare( activePlanDetail.session.in() ) != 0 );
        }
    }
...
void PlanInstance::update(const TA_Base_Core::PlanCustomisationUpdate &planCustomDetail)
{
    ...
    if (m_stepCustomDetailMap && Utilities::compareUpdateIds(planCustomDetail.customUpdateId, m_stepCustomDetailMapLastUpdateId) > 0)
    {
        ...
        stepCustomDetailMap[planCustomDetail.detail.id] = planCustomDetail.detail;
...
PlanInstance::StepExecDetailMap &PlanInstance::getStepExecDetailMap()
{
    ...
    if ( NULL == m_stepExecDetailMap)
    {
        ...
        {
            m_stepExecDetailMap->insert(executionHistory->details[i].id, executionHistory->details[i]);
        }
    }
...
PlanInstance::StepCustomDetailMap &PlanInstance::getStepCustomDetailMap()
{
    ...
    if (!m_stepCustomDetailMap)
    {
        ...
        {
            m_stepCustomDetailMap->insert(customisationHistory->details[i].id, customisationHistory->details[i]);
        }

...
void CPaMessagesDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control (pDX, IDC_TRAINPA_LIB_VERSION_TXT, m_libraryVerTxt);
    DDX_Control (pDX, IDC_TRAINPA_LIB_SECTION_TXT, m_librarySecTxt);
    DDX_Control (pDX, IDC_TRAINPA_MESSAGES_LIST, m_messagesList);
    DDX_Control (pDX, IDC_TRAINPA_PARAMETER_NAME_TXT, m_parameterNameTxt);
    // }}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CPaMessagesDlg, CPlanStepParameterEditDlg)
    // {{AFX_MSG_MAP(CPaMessagesDlg)
    ON_NOTIFY (LVN_ITEMCHANGED, IDC_TRAINPA_MESSAGES_LIST, OnMessageListItemChanged)
    // }}AFX_MSG_MAP
END_MESSAGE_MAP()

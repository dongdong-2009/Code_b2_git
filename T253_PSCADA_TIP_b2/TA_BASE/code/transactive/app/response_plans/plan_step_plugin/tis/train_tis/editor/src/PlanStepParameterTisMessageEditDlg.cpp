/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterTisMessageEditDlg.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
CPlanStepParameterTisMessageEditDlg::CPlanStepParameterTisMessageEditDlg(unsigned short libraryVersion, TisMessageStepParameter& parameter, bool canEdit) :
    ...
    m_libraryType(_T(convertToString(parameter.getMessageType()).c_str())),
    m_destinationLibraryVersion(libraryVersion),
    m_messageLibraryVersion(_T(""))
{
    FUNCTION_ENTRY( "CPlanStepParameterTisMessageEditDlg" );

    TA_ASSERT(parameter.getMessageType() != MT_UNDEFINED, "Invalid message type");

    TISMessage message(parameter.getMessage());

    if (message.messageId == TIS_UNDEFINED_MESSAGE_ID)
    {
        m_messageLibraryVersion = "Undefined";
    }
    else
    {
...
void CPlanStepParameterTisMessageEditDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_TRAINTIS_PARAMETER_NAME, m_parameterName);
    DDX_Text(pDX, IDC_TRAINTIS_LIBRARY_TYPE, m_libraryType);
    DDX_Text(pDX, IDC_TRAINTIS_DEST_LIBRARY_VERSION, m_destinationLibraryVersion);
    DDX_Text(pDX, IDC_TRAINTIS_MSG_LIBRARY_VERSION, m_messageLibraryVersion);
    DDX_Control(pDX, IDC_TRAINTIS_MESSAGE_LIST, m_messageList);
    // }}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CPlanStepParameterTisMessageEditDlg, CPlanStepParameterEditDlg)
    ...
    ON_NOTIFY(LVN_ITEMCHANGED, IDC_TRAINTIS_MESSAGE_LIST, OnMessageListItemChanged)
    // }}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CPlanStepParameterTisMessageEditDlg::OnInitDialog()
{
    ...
    bool isSTisLibrary(m_messageParameter.getMessageType() == MT_STATION);
    ...
    m_messageList.InsertColumn(TYPE_COLUMN_INDEX, _T("Type"), LVCFMT_LEFT, 90);
    m_messageList.InsertColumn(ID_COLUMN_INDEX, _T("ID"), LVCFMT_LEFT, 40);
    m_messageList.InsertColumn(MESSAGE_COLUMN_INDEX, _T("Message"), LVCFMT_LEFT, 410);
    m_messageList.InsertColumn(PRIORITY_COLUMN_INDEX, _T("Priority"), LVCFMT_LEFT, 60);

    // Populate the message list.
    TISMessage currentMessage(m_messageParameter.getMessage());
         ...
         mIt != m_predefinedMessages.end(); mIt++)
    {
        ...
        m_messageList.SetItemText(item, TYPE_COLUMN_INDEX, _T(convertToString(librarySection).c_str()));
        ...
        if (currentMessageIndex == LV_ERR && currentMessage.messageId != TIS_UNDEFINED_MESSAGE_ID)
        {
    ...
    {
        if (currentMessage.messageId != TIS_UNDEFINED_MESSAGE_ID)
        {
...
std::string CPlanStepParameterTisMessageEditDlg::convertToString(const EMessageType messageType)
{
...
std::string CPlanStepParameterTisMessageEditDlg::convertToString(const TA_Base_Core::ELibrarySection librarySection)
{
	FUNCTION_ENTRY( "convertToString" );

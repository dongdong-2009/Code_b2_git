/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_agent/src/ActivePlanLaunchGuiAppStepMessage.cpp $
  *  @author:  Bart Golab
  *  @version: $Revision: #3 $
  *
...
{
	ActivePlanLaunchGuiAppStepMessage::ActivePlanLaunchGuiAppStepMessage( const unsigned long ulStepNum, const std::vector<std::string>& vecData )
    ...
    void ActivePlanLaunchGuiAppStepMessage::execute( IActivePlan& activePlan, IMessageSender& refMsgSender ) const
    {
        const int nStatus = m_responseParam.response;

        int nWaitStatus = ActiveStep::eWaitFailed;

        if ( TA_Base_Core::PSCS_SUCCESS == nStatus )
        {

/**
  ...
  *
  * Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_mft_processor/src/RightsManager.cpp $
  * @author:  Huang Qi
  * @version: $Revision: #2 $
  *
...
void RightsManager::setInstance( RightsManager& refRighteManager )
{
	FUNCTION_ENTRY( "setInstance" );

	TA_THREADGUARD(s_singletonLock);

    s_instance = &refRighteManager;
	FUNCTION_EXIT;
}
...
void RightsManager::updateRights( const std::string& strSessionId )
{
...

    {
        ...
        m_unWorkstationId = sessionInfo.WorkstationId;
        m_unLocation = sessionInfo.LocationId;
    }
...
unsigned int RightsManager::workstationId()
{
    return m_unWorkstationId;
}

unsigned int RightsManager::locationId()
{
    return m_unLocation;
}

std::string RightsManager::sessionId()
{

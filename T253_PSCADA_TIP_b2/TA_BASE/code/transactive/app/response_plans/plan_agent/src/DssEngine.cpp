/**
  ...
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/response_plans/plan_agent/src/DssEngine.cpp $
  * @author:  Huang Qi
  * @version: $Revision: #2 $
  *
...
void DssEngine::processTriggerUpdate()
{
    ...
    ActivePlanBulletinBoard::instance()->getActiveInstances( ulPlanId, lstActivePlans, ulTriggerKey, true );
    bCurActive = !lstActivePlans.empty();
    ...
    if ( bNeedActivate )
    {
        ...
        TA_Base_Core::ActivePlanId stPlanId = { ulPlanId, 0u, 0u };

        try
        {
            PlanExecutor::instance()->run( strSession, stPlanId, ulTriggerKey );
        }
        catch ( const std::exception& expWhat )
        {
            LOG_EXCEPTION_CATCH( SourceInfo, "std::exception", expWhat.what() );

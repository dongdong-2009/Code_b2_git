/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/TA_Product_ECSPlan/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanAgentUser.cpp $
  *  @author:  Gregg Kirkpatrick
  *  @version: $Revision: #11 $
  *
  *  Last modification: $DateTime: 2015/07/09 15:29:14 $
...
namespace TA_Base_App
{
	...
	void PlanAgentUser::agentTerminate()
	{
		...
		// Wait for any running plans to end. All plans will run to completion before this method returns.
        if ( NULL != m_pDssManager )
        {
        ...
        PlanExecutor::instance()->changedToMonitor();
...
//         registerForMonitorMessages();
	...
	void PlanAgentUser::agentSetControl()
	{
        ...
        PlanExecutor::instance()->changedToControl();
...
// 		}
    ...
    void PlanAgentUser::initialisePlanAgentConfiguration()
    {
        ...
        //TD 13527 number of micro secs operator-response timed out plans and
		...
		if ( RPARAM_CONTROL == mode )
		{
            ...
            ActivePlanBulletinBoard::instance()->broadcastAgentResetMessage();
		}
        ...
        TA_Base_Bus::RpcAgency::createInstance( m_locationKey, 0u );
    ...
    void PlanAgentUser::handlePASUpdateData( TA_Base_Core::PASUpdateData* updateData, unsigned long& lastPasUpdateId )
    {
        ...
        try
        {
		    switch ( updateData->update._d() )
		    {
			    case TA_Base_Core::PASUT_PLAN_EXECUTION_HISTORY:
				    {
                        ...
                        TA_Base_Core::ExecutionHistoryUpdate& executionUpdate = updateData->update.executionHistory();
                        PlanExecutor::instance()->processExecutionHistoryUpdate( executionUpdate );
				    }
				    break;
                ...
                case TA_Base_Core::PASUT_TAKE_CONTROL:
                    {
                        ...
                        PlanExecutor::instance()->reassign(tku.session.in(), tku.plan);
                    }
                    break;
			    ...
			    case TA_Base_Core::PASUT_RUN_PLAN:
				    {
					    ...
					    PlanExecutor::instance()->run( rpsu.session.in(), rpsu.plan, false, rpsu.manuExec, true );
				    }
				    break;
			    ...
			    case TA_Base_Core::PASUT_STOP_PLAN:
				    {
					    ...
					    PlanExecutor::instance()->stop(rpsu.plan, rpsu.remark.in());
				    }
				    break;
                ...
                case TA_Base_Core::PASUT_EXECUTE_STEP:
                    {
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "StateUpdate: customiseStep(). Number:%u", receivedPASUpdateId);
                        TA_Base_Core::PlanExecuteStepUpdate& pcsu = updateData->update.executedStep();
                        PlanExecutor::instance()->executeStep(pcsu.plan, pcsu.detail);
                    }
                    break;
                ...
                case TA_Base_Core::PASUT_ACTIVE_PLAN_SHARE_DATA:
                    {
                        ...
                        TA_Base_Core::ActivePlanSharedData& ascss = updateData->update.sharedData();
                        PlanExecutor::instance()->processPlanSharedDataUpdate(ascss);
                    }
                    break;

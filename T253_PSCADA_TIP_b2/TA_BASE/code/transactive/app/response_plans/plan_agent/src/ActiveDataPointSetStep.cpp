...
namespace TA_Base_App
{
	...
	TA_Base_Core::EActiveStepState ActiveDataPointSetStep::executeImpl( void )
    {
        ...
        if ( initialize( strSession, strValue, abeWaitMillSec ))
        {
		    ...
		    {
				bool isWFDatapont = false;
				bool bRetWFInhibitCheck = false;
				bool inhibitStatus;
				//For WF datapoint, no need to send the execute command;
				std::string strDataPointName = m_pDataPoint->getEntityName();
				std::transform(strDataPointName.begin(), strDataPointName.end(), strDataPointName.begin(), toupper);
				std::string strWF= ".WF.";

				std::string::size_type pos = strDataPointName.find(strWF);

				if (pos != std::string::npos)
				{
					isWFDatapont = true;
					bRetWFInhibitCheck = TA_Base_Bus::WFGlobalInhibitionManager::getInstance().getWFGlobalInhibitState(inhibitStatus);
				}
				
				//selectWaitTime is 10s here because selectCmdConfirmTimeOut is 5s
				int selectWaitTime = 10000; // 10s
				if (false == isWFDatapont)
				{
				...
				{
					//For WF datapoint
					if (false == bRetWFInhibitCheck)
					{
						LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "WF Global Inhibit Statue unknown, can not do WF verification" );
					}
					else
					{
						if (false == inhibitStatus)
						{
							if ( sendPScadaControl( strSession, strValue, selectWaitTime, TA_Base_Bus::CONTROL_SELECT ) )
							{
								LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "DataPoint [%s] select operate successfully", m_params.dataPoint.in() );
								bSuccess = true;
							}
						}
						else
						{
							LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "WF Global Inhibit is true, no need to do WF inhibit." );
							bSuccess = true;
						}
					}
				}

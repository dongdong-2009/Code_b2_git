    ...
    UserCache::UserDetails UserCache::getUserDetails(const unsigned long userId)
    {
        ...
        try
        {
            ...
            {// scope for guarding the operator map
				...
				IConfigOperator* op = NULL;
                if (opIter == m_operatorMap.end())
                {
					LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,
						"Try to initialize the new Operator from database.  Adding new operator, %d, into map", userId);
					IOperator* newOp = OperatorAccessFactory::getInstance().getOperator(userId, true);
					if (!newOp)
					{
                    TA_THROW(AuthenticationAgentException("The specified operator is not recognised by the application") );
                }
					m_operatorMap.insert(OperatorMap::value_type(userId, dynamic_cast<IConfigOperator*>(newOp)));
					op = dynamic_cast<IConfigOperator*>(newOp);
                }
				else
				{
					op = opIter->second;

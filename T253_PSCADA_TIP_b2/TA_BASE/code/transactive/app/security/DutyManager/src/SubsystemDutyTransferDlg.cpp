/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/security/DutyManager/src/SubsystemDutyTransferDlg.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
void SubsystemDutyTransferDlg::updateTransferableSubsystems(const TA_Base_Bus::RegionKeyList& regionKeys)
{
    ...
    try
    {
        ...
        m_regionsEdit.SetWindowText(GridAssistant::generateRegionString(regionKeys).c_str());
             ...
             subsystemIt != m_transferableSubsystems.end(); subsystemIt++)
        {
            try
            {
                std::string regionName(m_dataCache.getRegionDisplayName(subsystemIt->regionKey));
                ...
                m_transferableSubsystemList.SetItemText(item, REGION_COLUMN, _T(regionName.c_str()));
                m_transferableSubsystemList.SetItemText(item, SUBSYSTEM_COLUMN, _T(subsystemName.c_str()));
                m_transferableSubsystemList.SetItemText(item, OPERATOR_COLUMN, _T(operatorName.c_str()));
                m_transferableSubsystemList.SetItemText(item, PROFILE_COLUMN, _T(profileName.c_str()));
    ...
    {
        MessageBox::error("Error while updating transferable subsystem list.");
    }	
}
...
BOOL SubsystemDutyTransferDlg::OnInitDialog() 
{
    ...
    m_transferableSubsystemList.InsertColumn(REGION_COLUMN, _T(REGION_COLUMN_HEADING), LVCFMT_LEFT, REGION_COLUMN_WIDTH);
    m_transferableSubsystemList.InsertColumn(SUBSYSTEM_COLUMN, _T(SUBSYSTEM_COLUMN_HEADING), LVCFMT_LEFT, SUBSYSTEM_COLUMN_WIDTH);
    m_transferableSubsystemList.InsertColumn(OPERATOR_COLUMN, _T(OPERATOR_COLUMN_HEADING), LVCFMT_LEFT, OPERATOR_COLUMN_WIDTH);
    m_transferableSubsystemList.InsertColumn(PROFILE_COLUMN, _T(PROFILE_COLUMN_HEADING), LVCFMT_LEFT, PROFILE_COLUMN_WIDTH);

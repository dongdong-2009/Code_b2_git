 ...
 * in any form.
 *
 * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/scada/EquipmentStatusViewer/src/EquipmentStatusListItem.h $
 * @author:  Alan Brims
 * @version: $Revision: #1 $
 *
...
{    
    ...
    protected:
		...
		CString getInfoFromSession(const std::string& session);
		/** 
		...
		static std::map<unsigned long, CString> m_consoles;
		static std::map<unsigned long, CString> m_profiles;
		static std::map<unsigned long, CString> m_operators;
    private:
        //
    ...
    //    EquipmentStatusListItem( const EquipmentStatusListItem& theSource) {};
		...
		static std::map<std::string, CString>				m_sessionInfoCache;

 ...
 * in any form.
 *
 * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/scada/EquipmentStatusViewer/src/ConfigurationDialog.cpp $
 * @author:  Rod Rolirad
 * @version: $Revision: #1 $
 *
...
//      int linesRemaining = 3;
		...
		//Launch Manager///////////////////////////////////////////////////////////
		std::vector< EquipmentReportEntity> failedItems;
        ...
        for ( ; linesRemaining > 0; --linesRemaining)
        {
            ...
            try
            {
                ...
                {
                    pItem.reset( new DataNodeListItem( it->entityKey, m_scadaProxyFactory, GetSafeHwnd() ));
                }
                else if (DATAPOINT == it->itemType)
                {
                    pItem.reset( new DataPointListItem( it->entityKey, m_scadaProxyFactory, GetSafeHwnd() ));
                }
                else
                {
		...
		{
			m_unprocessedEntities.insert(m_unprocessedEntities.end(), failedItems.begin(), failedItems.end());
		}

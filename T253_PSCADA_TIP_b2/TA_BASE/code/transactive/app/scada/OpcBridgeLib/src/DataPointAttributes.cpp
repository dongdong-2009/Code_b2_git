...
namespace TA_Base_App
{
	...
	:
	AbstractDataObjectAttributes(opcDpData.entityKey, 
								 opcDpData.agentKey, 
								 opcDpData.locationKey, 
								 opcDpData.preCreateAttrs, 
								 opcDpData.entityName),
	m_configAvaliable(false),
	m_isOutputDataPoint(opcDpData.isOutputDataPoint)
	{
    ...
    bool DataPointAttributes::isAttributeNameValid(const std::string & attributeName, EOPCATTRIBUTETYPE& attrType)
    {
		...
		else if( attributeName == DPT_ATTR_NAME_CONTROL_STATE )
        {
            attrType = E_OPC_DP_CONTROLSTATE;
        }
		else
		{
	...
	{
		//For input datapoint, only alarmstate and alarmackstate is needed.
		//For output datapoint , only control state is needed.
		if (false == m_isOutputDataPoint)
		{
		...
		{
			if (m_preCreateAttrFlags & E_OPC_DATA_ATTR_DP_CONTROLSTATE)
			{
				attrs.push_back(AttrData(E_OPC_DP_CONTROLSTATE, DPT_ATTR_NAME_CONTROL_STATE));
			}			
		}
	...
	Attribute* DataPointAttributes::newAttribute(const std::string& tagName, EOPCATTRIBUTETYPE attrType)
	{
		...
		case E_OPC_DP_ACKSTATE:
			...
			break;
		case E_OPC_DP_CONTROLSTATE:
			attribute = new DpControlState(this, tagName);
			break;			
		default:
	...
	void DataPointAttributes::initDisplayState()
	{
		...
		m_state.alarmState = false;			//use alarm state to pass the control state information.
	...
	//alarmState field will pass the diffent information for different data point.
	//For input datapoint, alarmState will pass the alarm state of this data point.
	//For output datapoint,alarmState will pass whether this output datapoint is in control process.
	...
	bool DataPointAttributes::isInControlState()
	{
		TA_Base_Core::ThreadGuard guard(m_stateLock);
		return m_state.alarmState;
	}

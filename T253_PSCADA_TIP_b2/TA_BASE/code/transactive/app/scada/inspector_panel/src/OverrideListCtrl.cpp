...
namespace TA_Base_App
{
	...
	void COverrideListCtrl::setupList()
	{
		...
		this->InsertColumn(DESCRIPTION_COLUMN, StringTableHelper(IDS_DESCRIPTION_COLUMN));
		this->InsertColumn(DATAPOINT_TYPE_COLUMN, StringTableHelper(IDS_TYPE_COLUMN),LVCFMT_CENTER);
		this->InsertColumn(OVERRIDE_ENABLE_COLUMN, StringTableHelper(IDS_OVERRIDE_COLUMN), LVCFMT_CENTER);
		this->InsertColumn(OVERRIDE_VALUE_COLUMN, StringTableHelper(IDS_OVERRIDE_VALUE_COLUMN));
		this->InsertColumn(CURRENT_VALUE_COLUMN, StringTableHelper(IDS_CURRENT_VALUE_COLUMN)); // FAT TES# 228
	...
	bool COverrideListCtrl::UpdateCellValue(CEditableListCtrl* listCtrl, int item, int subItem, CString value)
	{
						...
						if ( (strValue == "Yes") != dataPoint->isOverrideEnabled() )
						{
							...
							if (strValue == "Yes")
							{
								...
								if (!isSetForcedPermitted)
								{
									...
									statusLine << StringTableHelper(IDS_OVR_FAILED4);
									break;
								}
								...
								else
								{
									...
									if (dlg.DoModal() == IDOK)
									{
										...
										if ( overrideValue == "" )
										{
											...
											statusLine << StringTableHelper(IDS_OVR_FAILED1);
											//  [3/14/2010 benxun]
											 ...
											 ( false == dataPoint->isDerived() ) )
										{
											...
											if ( valueStream >> doubleValue )
     										{
												...
												if ( "" != dataPoint->getUnit() )
												{
														 ...
														 ( doubleValue > dataPoint->getLimit ( false ) ) )	// high eng limit
													{
														...
														statusLine << StringTableHelper(IDS_OVR_FAILED2);
														//  [3/14/2010 benxun]
														std::string* statusLineStr = new std::string(statusLine.str());
														AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);
                                                        ...
                                                        CString cstring;
                                                        AfxFormatString2(cstring, IDS_VALUE_RANGE, std::to_string(dataPoint->getLimit(true)).c_str(), std::to_string(dataPoint->getLimit(false)).c_str());
                                                        message2 << cstring;
														//  [3/14/2010 benxun]
											...
											else
											{
                                                ...
                                                CString cstring;
                                                AfxFormatString1(cstring, IDS_OVR_FAILED3, overrideValue.c_str());
                                                statusLine << cstring;
                                        ...
                                        CString cstring;
                                        AfxFormatString1(cstring, IDS_OVR_SUCCESSFUL1, overrideValue.c_str());
                                        statusLine << cstring;
							...
							else
							{
								...
								else
								{
									...
									if ( inhibitOverrideConfirm ( IDS_OVERRIDE, dataPoint, (strValue == "Yes") ) )
									{
										...
										statusLine << StringTableHelper(IDS_OVR_SUCCESSFUL2);
										//  [3/14/2010 benxun]
										std::string* statusLineStr = new std::string(statusLine.str());
										AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);
		...
		catch ( TA_Base_Core::ScadaProxyException & e )
		{
			...
			if (strValue == "Yes")
			{
                ...
                CString cstring;
                AfxFormatString1(cstring, IDS_OVR_FAILED5, overrideValue.c_str());
                statusLine << cstring;
				//  [3/14/2010 benxun]
			}
			// if the checkbox is unchecked
			else
			{
				...
				statusLine << StringTableHelper(IDS_OVR_FAILED6);
				//  [3/14/2010 benxun]
			}
			std::string* statusLineStr = new std::string(statusLine.str());
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);

...
namespace TA_Base_App
{
	...
	{
        std::string *statusString = new std::string(StringTableHelper(IDS_LOADING));
		AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
		//step1
		if(this->loadDataNodeData())
		{
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_DATA_NODE));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);	
		}
		else
		{
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_NODE_FAILED));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);	
			return false;
		}
		//step2
		...
		{
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_NODE_PROXY));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);	
		...
		{
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_NODE_PROXY_FAILED));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);	
			return false;
		}
		//step3
		...
		{	
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_DATA_POINT));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
		}
		else
		{
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_DATA_POINT_FAILED));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);	
			return false;
		}
		//step4
		if(this->createDataPointProxy())
		{
			...
			std::string *statusString = new std::string(StringTableHelper(IDS_LOADING_DATA_PROXY));
			AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
		...
		{	
			std::string *str = new std::string(StringTableHelper(IDS_LOADING_DATA_PROXY_FAILED));

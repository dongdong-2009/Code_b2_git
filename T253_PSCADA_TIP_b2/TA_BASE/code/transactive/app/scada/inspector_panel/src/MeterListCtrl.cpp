...
namespace TA_Base_App
{
  ...
  // 		this->InsertColumn(DescriptionColumn,			"Description");
		...
		this->InsertColumn(DataPointTypeColumn, StringTableHelper(IDS_TYPE_COLUMN), LVCFMT_CENTER);
		this->InsertColumn(CurrentValueColumn, StringTableHelper(IDS_CURRENT_VALUE_COLUMN));
		this->InsertColumn(MeterAdjustColumn, StringTableHelper(IDS_ADJUST_COLUMN));
				...
				try
				{
					...
					if ( valueStream >> correction )
     				{
						...
						if ( false == dataPoint->getUnit().empty() )
						{
								 ...
								 ( adjustedValue > dataPoint->getLimit ( false ) ) )	// high eng limit
							{
								...
								*/
                                CString cstring;
                                AfxFormatString1(cstring, IDS_ADJUST_FAILED_MSG, strValue.c_str());
                                statusLine << cstring;
                                cstring.Empty();
					...
					else
					{
						...
						if (0 == strValue.compare(""))
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_ADJUST_VALUE_FAILED_MSG, strValue.c_str());
                            statusLine << cstring;
					...
					if ( false == adjustFailed )
					{
						...
						if (dlg.DoModal()==IDOK)
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_ADJUST_SUCCESS, strValue.c_str());
                            statusLine << cstring;

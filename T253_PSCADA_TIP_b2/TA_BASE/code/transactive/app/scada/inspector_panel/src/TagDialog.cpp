...
namespace TA_Base_App
{
	...
	bool CTagDialog::applyTag()
	{
		...
		case IDC_RADIO_TAG:
				...
				dlg.m_tag = StringTableHelper(IDS_TAG_LAB1);
				break; 
		case IDC_RADIO_TAG_PTW:
				//dlg.m_tag = "Tag with PTW \'";
				dlg.m_tag = StringTableHelper(IDS_TAG_LAB2);
				dlg.m_tag += tagPtwId;
				dlg.m_tag += "\'";
				break;
		case IDC_RADIO_NOT_TAGGED:
				//dlg.m_tag = "Remove Tag";
				dlg.m_tag = StringTableHelper(IDS_TAG_LAB3);
				break;
		}
		...
		if (m_controlInhibitChecked)
		{
			if (inhibitString == "")
			{
				...
				inhibitString = StringTableHelper(IDS_TAG_LAB6);
			}
			else
			{
				//inhibitString += " and control";
				inhibitString += StringTableHelper(IDS_TAG_LAB7);
			}
		...
		if (inhibitString == "")
		{
			if (tagRadioButton == IDC_RADIO_NOT_TAGGED)
			{
				if (m_originalScanInhibitChecked)
				{
					...
					inhibitString = StringTableHelper(IDS_TAG_LAB8);
				}
				else if (m_originalAlarmInhibitChecked)
				{
					// inhibitString = "Enable alarm";
					inhibitString = StringTableHelper(IDS_TAG_LAB9);
				}
				...
				if (m_originalControlInhibitChecked)
				{
					if (inhibitString == "")
					{
						...
						inhibitString = StringTableHelper(IDS_TAG_LAB10);
					}
					else
					{
						//inhibitString += " and control";
				...
				if (inhibitString == "")
				{
					...
					inhibitString = StringTableHelper(IDS_TAG_LAB11);
				}
			}
			else
			{
				//inhibitString = "No Inhibition";
				inhibitString = StringTableHelper(IDS_TAG_LAB12);
			}
		}
		...
		if (apply)
		{
			...
			statusLine << StringTableHelper(IDS_TAG_LAB13);
			//  [3/15/2010 benxun]
			...
			{					
				...
				case IDC_RADIO_TAG:
					{
							 ...
							 ( TA_Base_Bus::NoOutputInhibit != inhibitState.outputInhibit ) )
						{
							//  [3/15/2010 benxun]
							//statusLine << " with";
							statusLine << StringTableHelper(IDS_TAG_LAB14);
							//  [3/15/2010 benxun]
							...
							if ( TA_Base_Bus::InhibitScan == inhibitState.inputInhibit )
							{
								...
								statusLine << StringTableHelper(IDS_TAG_LAB15);
								//  [3/15/2010 benxun]
							}
							else if ( TA_Base_Bus::InhibitAlarm == inhibitState.inputInhibit )
							{
								...
								statusLine << StringTableHelper(IDS_TAG_LAB16);
								//  [3/15/2010 benxun]
							}
							...
							if ( TA_Base_Bus::InhibitControl == inhibitState.outputInhibit )
							{
								...
								statusLine << StringTableHelper(IDS_TAG_LAB17);
								//  [3/15/2010 benxun]
							}					
							// ++wenching (TD13621)
						}
						...
						statusLine << StringTableHelper(IDS_TAG_LAB18);
						//  [3/14/2010 benxun]
				...
				case IDC_RADIO_TAG_PTW:
					{
						//statusLine << " With PTW";
						statusLine << StringTableHelper(IDS_TAG_LAB19);
						...
						{
							//statusLine << " with";
							statusLine << StringTableHelper(IDS_TAG_LAB20);
				...
				case IDC_RADIO_NOT_TAGGED:
					{
						...
						statusLine << StringTableHelper(IDS_TAG_LAB21);
						//  [3/14/2010 benxun]
						std::string* statusLineStr = new std::string(statusLine.str());
						AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);
			...
			catch (TA_Base_Core::ScadaProxyException &e)
			{
				...
				if ( IDC_RADIO_NOT_TAGGED == tagRadioButton )
				{
					...
					statusLine << StringTableHelper(IDS_TAG_LAB22);
					//  [3/14/2010 benxun]
				}
				else
				{
					...
					statusLine << StringTableHelper(IDS_TAG_LAB23);
					//  [3/14/2010 benxun]
				}
	...
	bool CTagDialog::isOkToChangeTab()
	{
        ...
        userMsg << actionName;
        UINT selectedButton = userMsg.showMsgBox(IDS_UW_010003, StringTableHelper(IDS_INSPECTORPANEL_TITLE));

...
//
// Msg.cpp: Implementation of the StringTableHelper and StringTableFormatHelper classes
//
//   StringTableHelper : Wrapper around LoadString(). CString that takes a resource string index in its c'tor
//
//   StringTableFormatHelper : Wrapper around the API FormatMessage().
//
...
//
// StringTableHelper : CString-derived class used to load strings form the resource string table
...
//        e.g.: SetWindowText(StringTableHelper(IDS_TITLE)); // Weather Forecast.
//
namespace TA_Base_App
{
    StringTableHelper::StringTableHelper(UINT nID) // nID = resource ID of string (in string table)
    {
        if (!LoadString(nID))
    ...
    StringTableFormatHelper::StringTableFormatHelper(UINT nFormatID, ...) // nFormatID = resource string containing text to format
    ...
    StringTableFormatHelper::StringTableFormatHelper(LPCTSTR lpszFormat, ...)
    {
        // Format message into temporary buffer lpszTemp
        va_list argList;

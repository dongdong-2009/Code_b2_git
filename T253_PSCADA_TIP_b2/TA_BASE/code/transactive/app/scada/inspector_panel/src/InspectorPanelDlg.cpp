...
namespace TA_Base_App
{
	...
	void CInspectorPanelDlg::updateMainDialog()
	{
		...
		std::string title = _T(StringTableHelper(IDS_INSPECTORPANEL_TITLE));;
	...
	void CInspectorPanelDlg::init()
	{
        ...
        // Registering for receive rights chaning message
        m_rightsMgr->registerObserver(*this);
...
#pragma omp master
	...
	LRESULT CInspectorPanelDlg::OnLoadDataPoints (WPARAM wParam, LPARAM lParam)
	{
		...
		{	
       		...
       		else
		    {
                ...
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_081009, StringTableHelper(IDS_INSPECTORPANEL_TITLE));
            }
		}
		catch (TA_Base_Core::ScadaProxyException& e)
		{
    ...
    LRESULT CInspectorPanelDlg::UpdatePermissions (WPARAM wParam, LPARAM lParam)
	{
        if (NULL == m_equipment)
        {
            LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Equipment is null, so exit processing...");
            return 0;
        }
        ...
        try
        {
		    ...
		    if (*temporalName==EQUIPMENT_RPARAM) 
		    {
		    	if(m_state == WaitUpdate)
		    	{
	                ...
	                {
						m_isInitialTab = false;
						std::string *statusLineStr = new std::string(StringTableHelper(IDS_CHANGEING_EQUIPMENT_MSG));
    ...
    void CInspectorPanelDlg::OnRightsChange(std::string sessionId, TA_Base_Bus::ERightsChangeType changeType, TA_Base_Bus::DutyChanges& changes)
    {
        FUNCTION_ENTRY("CInspectorPanelDlg::OnRightsChange");
        
        LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Received right chaning message.");
        AccessControlHelper::getInstance()->updateAccessControlMap();

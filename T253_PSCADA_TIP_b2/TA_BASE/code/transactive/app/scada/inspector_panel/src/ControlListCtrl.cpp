...
namespace TA_Base_App
{
	...
	void CButtonControlListCtrl::setupList()
	{
		...
		this->InsertColumn(EQUIPMENT_ATTRIBUTES_COLUMN, StringTableHelper(IDS_DESCRIPTION_COLUMN));
		this->InsertColumn(DATAPOINT_TYPE_COLUMN, StringTableHelper(IDS_TYPE_COLUMN), LVCFMT_CENTER);
		this->InsertColumn(CONTROL_STATE_COLUMN, StringTableHelper(IDS_STATE_COLUMN));
		this->InsertColumn(NEW_VALUE_COLUMN, StringTableHelper(IDS_OPERATION_COLUMN));
				...
				else
				{
					...
					if ((*dataPoints[iPoint])->isNumber() || (*dataPoints[iPoint])->isText())
					{
						...
						if (button == NULL)
						{
							...
							button = new CControlListCellButton(this, cellRect, nItem, StringTableHelper(IDS_SETUP), this);
							button->EnableWindow(false);
							m_buttonMap.insert(std::map<unsigned long,CControlListCellButton*> ::value_type(2*nItem, button));
						}
					}
					...
					if (bSetValue)
					{
						...
						for (int nItem=0; nItem < GetItemCount(); nItem++)
						{
							if (this->GetItemData(nItem) == iPoint)
							{
								...
								setButtonText( nItem, strValue, strValue );
								RefreshButtonDisplay(iPoint,nItem);
								return;
							}
						}
			...
			if ( dataPoint->isWriteable() && !dataPoint->isControlInhibited() )
			{
				...
				if (dataPoint->getPScadaFlag())
				{
...

					{
						...
						if (shouldCancelDlg)
						{
							HWND hwnd = ::FindWindow(NULL, _T(StringTableHelper(IDS_CONFIRM_CANCLE)));
	...
	std::string CButtonControlListCtrl::getControlStatusString ( TA_Base_Bus::EDataPointControlState state ) 
    {
        ...
        try
        {
            ...
            case written:
                ...
                returnString = StringTableHelper(IDS_SENT);
                break;

            case pending:
                // returnString = DPT_CONTROL_PENDING;
                returnString = StringTableHelper(IDS_PENDING);
                break;
            ...
            case failed:
                ...
                returnString = StringTableHelper(IDS_FAILED);
                break;

            case LCC_failed:
                // returnString = DPT_CONTROL_LCC_FAILED;
                returnString = StringTableHelper(IDS_LCC_FAILED_STR);
                break;

            case RCC_failed:
                // returnString = DPT_CONTROL_RCC_FAILED;
                returnString = StringTableHelper(IDS_RCC_FAILED_STR);
                break;

            case comms_failed:
                // returnString = DPT_CONTROL_COMMS_FAILED;
                returnString = StringTableHelper(IDS_COMMS_FAILED_STR);
                break;

                //Using for IEC104 Protocol control
            case selectCmdWritten:
                // returnString = DPT_CONTROL_SELECT_SENT;
                returnString = StringTableHelper(IDS_CONTROL_SELECT_SENT_STR);
                break;

            case selectCmdConfirm:
                // returnString = DPT_CONTROL_SELECT_CONFIRM;
                returnString = StringTableHelper(IDS_CONTROL_SELECT_CFR_STR);
                break;
            ...
            case execCmdWritten:
                ...
                returnString = StringTableHelper(IDS_CONTROL_EXE_SENT_STR);
                break;

            case execCmdConfirm:
                // returnString = DPT_CONTROL_EXECUTE_CONFIRM;
                returnString = StringTableHelper(IDS_CONTROL_EXE_CFM_STR);
                break;
            ...
            case cancelCmdWritten:
                ...
                returnString = StringTableHelper(IDS_CONTROL_CANCEL_SENT_STR);
                break;
			...
			if ( true == datapoint->isWriteable() )
			{
				...
				switch ( state ) 
				{
					case written:
						{
							//TD15649
							//statusLine << "Sent.";
							statusLine << StringTableHelper(IDS_SENT);
							addStatusLine = true;
							returnString = DPT_CONTROL_WRITTEN;
							break;
						}
					...
					case pending:
						{
							//statusLine << "Pending.";
							statusLine << StringTableHelper(IDS_PENDING);
							addStatusLine = true;
							returnString = DPT_CONTROL_PENDING;
							break;
						}
					...
					case conditionalSuccess:
						{
							...
							statusLine << StringTableHelper(IDS_SUCCESSFUL);
							addStatusLine = true;
							returnString = DPT_CONTROL_CONDITIONALSUCCESS;						
							break;
						}
					...
					case failed:
						{
							//statusLine << "Failed.";
							statusLine << StringTableHelper(IDS_FAILED);
							addStatusLine = true;
							returnString = DPT_CONTROL_FAILED;						
							break;
						}
					...
					case LCC_failed:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_LCC_FAILED, datapoint->getLaunchingConditionResultString().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_LCC_FAILED;
							break;
					...
					case RCC_failed:
						{
							...
							//statusLine << " failed due to Bad Return Condition.";
                            CString cstring;
                            AfxFormatString1(cstring, IDS_RCC_FAILED, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							//++CL19432
							addStatusLine = true;
							returnString = DPT_CONTROL_RCC_FAILED;				
					...
					case comms_failed:
						{
							...
							statusLine << StringTableHelper(IDS_COMMS_FAILED);
							addStatusLine = true;
							returnString = DPT_CONTROL_COMMS_FAILED;
							break;
						}
					...
					case selectCmdWritten:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SELECT_COMMAND_WRITE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_SELECT_SENT;
							break;
					...
					case selectCmdConfirm:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SELECT_COMMAND_CONFIRM, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_SELECT_CONFIRM;
							break;
					...
					case selectCmdConfirmTimeout:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SELECT_COMMAND_TIMEOUT, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_SELECT_CONFIRM;
							break;
					...
					case operateCmdTimeout:
						{
							...
							statusLine << StringTableHelper(IDS_OPERATOR_TIMEOUT);
							addStatusLine = true;
							returnString = DPT_CONTROL_OPERATOR_TIMEOUT;
							break;
						}
					...
					case execCmdWritten:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_WRITE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_EXECUTE_SENT;
							break;
					...
					case execCmdConfirm:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_CONFIRM, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_EXECUTE_CONFIRM;
							break;
					...
					case execCmdConfirmTimeout:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_TIMEOUT, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_EXECUTE_TIMEOUT;
							break;
					...
					case execCmdSuccess:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_SUCCESS, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_EXECUTE_SUCCESS;
							break;
					...
					case execCmdSuccessTimeout:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_SUCCESS_TIMEOUT, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_EXECUTE_SUC_TIMEOUT;
							break;
					...
					case cancelCmdWritten:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_CANCEL_COMMAND_WRITE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_CANCEL_SENT;
							break;
					...
					case cancelCmdConfirm:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_CANCEL_COMMAND_CONFIRM, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_CANCEL_CONFIRM;
							break;
					...
					case cancelCmdConfirmTimeout:
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_CANCEL_COMMAND_TIMEOUT, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_CANCEL_TIMEOUT;
							break;
					...
					case selectCmdCfmNegative:
						{
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SELECT_COMMAND_NEGATIVE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_FAILED;
							break;
					...
					case execCmdCfmNegative:
						{
                            CString cstring;
                            AfxFormatString1(cstring, IDS_EXE_COMMAND_NEGATIVE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_FAILED;
							break;
					...
					case cancelCmdCfmNegative:
						{
                            CString cstring;
                            AfxFormatString1(cstring, IDS_CANCEL_COMMAND_NEGATIVE, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_FAILED;
							break;
					...
					case deviceControlException:
						{
                            CString cstring;
                            AfxFormatString1(cstring, IDS_COMMAND_EXCEPTION, datapoint->getEntityName().c_str());
                            statusLine << cstring;
							addStatusLine = true;
							returnString = DPT_CONTROL_FAILED;
							break;
    ...
    void CButtonControlListCtrl::processPscadaControlCommand(int item, TA_Base_Bus::DataPointProxySmartPtr & dataPoint, std::string strValue, TA_Base_Bus::EControlOperation action)
    {
        ...
        {
        case CONTROL_SELECT:
            strAction = StringTableHelper(IDS_SELECT_ACTION);
            break;
        case CONTROL_EXECUTE:
            strAction = StringTableHelper(IDS_EXEC_ACTION);
            break;
        case CONTROL_CANCEL:
            strAction = StringTableHelper(IDS_CANCEL_ACTION);
            break;
        default:            
        ...
        catch (TA_Base_Core::AccessDeniedException& e)
        {
            if (CONTROL_SELECT == action)
                button->setIsWaitingForConfirm(false);
            CString cstring;
            AfxFormatString1(cstring, IDS_SEND_COMMAND_NO_PERMISSION, strAction.c_str());
            std::stringstream ss;
            ss << cstring;
            this->showStatusMessage(ss.str());
            std::string text = "setValue() - " + std::string(e.reason);
    ...
    void CButtonControlListCtrl::processNonPscadaControlCommand(TA_Base_Bus::DataPointProxySmartPtr& dataPoint, std::string strValue)
    {
        ...
        if (dlg.DoModal() == IDOK)
        {
            ...
            {
                this->showStatusMessage((LPCTSTR)StringTableHelper(IDS_SET_FAILED_NO_PERMISSION));

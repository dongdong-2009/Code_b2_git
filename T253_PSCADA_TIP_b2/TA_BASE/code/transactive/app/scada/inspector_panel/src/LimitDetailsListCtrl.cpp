...
namespace TA_Base_App
{
	...
	void CLimitDetailsListCtrl::setupList()
	{
		...
		this->InsertColumn(DUMMY_COLUMN,				"Dummy");
		this->InsertColumn(ALARM_TYPE_COLUMN,			StringTableHelper(IDS_ALARM_TYPE_COLUMN));
		this->InsertColumn(ALARM_ENABLED_COLUMN, StringTableHelper(IDS_ENABLE_COLUMN), LVCFMT_CENTER);
		this->InsertColumn(CURRENT_VALUE_COLUMN, StringTableHelper(IDS_CURRENT_VALUE_COLUMN));
		this->InsertColumn(THRESHOLD_VALUE_COLUMN, StringTableHelper(IDS_THRESHOLD_COLUMN));
				...
				case ALARM_ENABLED_COLUMN:
					{
						...
						if (strValue == "Yes")
						{
							...
							std::string typeOfChange = StringTableHelper(IDS_ALARM_TYPE) + alarmType;
							if ( overrideAlarmConditionConfirm ( typeOfChange, dataPoint, ( strValue == "Yes" ) ) )
							{
                                ...
                                CString cstring;
                                AfxFormatString1(cstring, IDS_ALARM_ENABLE_MSG, (LPCTSTR)alarmType);
                                statusLine << cstring;
								//  [3/14/2010 benxun]
						...
						else
						{
							...
							if ( overrideAlarmConditionConfirm ( typeOfChange, dataPoint, ( strValue == "Yes" ) ) )
							{
                                ...
                                CString cstring;
                                AfxFormatString1(cstring, IDS_ALARM_DISABLE_MSG, (LPCTSTR)alarmType);
                                statusLine << cstring;
								//  [3/14/2010 benxun]
				...
				case THRESHOLD_VALUE_COLUMN:
					{
						...
						if (cellValue.empty() || atof(cellValue.c_str()) == atof(longerValue.c_str()))
						{
                            ...
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SET_UNIT_ERR_MSG, cellValue.c_str());
                            statusLine << cstring;
							...
							atof(cellValue.c_str()) > dataPoint->getLimit(false))
						{
							...
							*/
                            CString cstring;
                            AfxFormatString1(cstring, IDS_SET_THRESHOLD_FAILED, cellValue.c_str());
						...
						// get confirmation from the operator
						std::stringstream ss;
						ss << "Set new threshold value:" <<  dataPoint->getEntityName();
						ss << ", "  << dataPoint->getDescription() << ", from ";

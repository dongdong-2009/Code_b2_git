...
namespace TA_Base_App
{
	...
	void CMainTabControl::initilizeTabDialog()
	{
		...
		AddPage(*informationDialog,IDD_INFORMATION, CString(StringTableHelper(IDS_TAB_INFO)));

		CControlDialog* controlDialog = new CControlDialog;
		m_tabDialogs[ControlTab] = controlDialog;
		AddPage(*controlDialog,IDD_CONTROL,CString(StringTableHelper(IDS_TAB_CTRL)));

		CNotesDialog* notesDialog = new CNotesDialog;
		m_tabDialogs[NotesTab] = notesDialog;
		AddPage(*notesDialog,IDD_NOTES,CString(StringTableHelper(IDS_TAB_NOTE)));

		CTagDialog* tagDialog = new CTagDialog;
		m_tabDialogs[TagTab] = tagDialog;
		AddPage(*tagDialog,IDD_TAG,CString(StringTableHelper(IDS_TAB_LABEL)));

		CInhibitDialog* inhibitDialog = new CInhibitDialog;
		m_tabDialogs[InhibitTab] = inhibitDialog;
		AddPage(*inhibitDialog,IDD_INHIBIT,CString(StringTableHelper(IDS_TAB_INHIBIT)));

		COverrideDialog* overrideDialog = new COverrideDialog;
		m_tabDialogs[OverrideTab] = overrideDialog;
		AddPage(*overrideDialog,IDD_OVERRIDE,CString(StringTableHelper(IDS_TAB_OVERRIDE)));
		
		CLimitDialog* limitDialog = new CLimitDialog;
		m_tabDialogs[LimitsTab] = limitDialog;
		AddPage(*limitDialog,IDD_LIMITS,CString(StringTableHelper(IDS_TAB_LIMIT)));

		CMeterDialog* meterDialog = new CMeterDialog;
		m_tabDialogs[MeterTab] = meterDialog;
		AddPage(*meterDialog,IDD_METER,CString(StringTableHelper(IDS_TAB_METER)));
		...
		if ( TA_Base_Core::RunParams::getInstance().isSet("StatusTab") )
		{
			...
			AddPage(*statusDialog,IDD_STATUS,CString(StringTableHelper(IDS_TAB_STATUS)));
		}

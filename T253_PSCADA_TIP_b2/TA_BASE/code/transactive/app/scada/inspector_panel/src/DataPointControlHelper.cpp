		...
		{	
			if (dataPoint->isDerived())
			{
				...
				catch (TA_Base_Core::ScadaProxyException&)
				{
                    ...
                    CString cstring;
                    AfxFormatString2(cstring, IDS_VALUE_INVALID, dataPoint->getDescription().c_str(), strValue.c_str());
			...
			else if (dataPoint->isNumber())
			{
				...
				if (valueStream >> doubleValue)
     			{
						...
						|| doubleValue < dataPoint->getLimit(true))
					{
                        ...
                        CString cstring;
                        AfxFormatString2(cstring, IDS_VALUE_OUT_RANGE, dataPoint->getDescription().c_str(), strValue.c_str());
						...
						*/
						...
						message2 << dataPoint->getDescription() << ": " << cstring << " ";
						message2 << dataPoint->getUnit() << ".";
				...
				else
				{
                    ...
                    CString cstring;
                    AfxFormatString2(cstring, IDS_INVALID_NUM_MSG, dataPoint->getDescription().c_str(), strValue.c_str());
                    message << cstring;
					//  [3/14/2010 benxun]
			...
			else if (dataPoint->isBoolean())
			{
				...
				if (strValue == "" || (strValue != trueLabel && strValue != falseLabel))
				{
					...
					std::stringstream message;
                    CString cstring;
                    AfxFormatString2(cstring, IDS_INVALID_VALUE_MSG, dataPoint->getDescription().c_str(), strValue.c_str());
                    message << cstring;
					// message << dataPoint->getDescription() << ": 值 \"" << strValue << "\" 不合法.";

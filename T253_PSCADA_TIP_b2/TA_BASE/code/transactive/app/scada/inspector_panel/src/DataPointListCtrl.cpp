...
//						||dataPoint->getStatus() == TA_Base_Bus::QUALITY_GOOD_LOCAL_OVERRIDE)
			...
			if (dataPoint->isDerived() && iconIndex == m_greenIconIndex)
			{
                ...
                if (strValue == (LPCTSTR)StringTableHelper(IDS_UNDEFINED_STR))
				{
					iconIndex = m_blueIconIndex;
				}
    ...
    std::string CAbstractDataPointListCtrl::getCurrentValueAsString(TA_Base_Bus::DataPointProxySmartPtr& dataPoint)
    {
        ...
        // If alarmState equls some special, no need showing in GUI, so clear alarmState
        if (TA_Base_Bus::DPTALM_TRUE_SHORT == alarmState ||
            TA_Base_Bus::DPTALM_FALSE_SHORT == alarmState ||
            TA_Base_Bus::DPTALM_GENERAL_SHORT == alarmState ||
            TA_Base_Bus::DPTALM_UNKNWN_SHORT == alarmState)
        {
            alarmState = "";
        }

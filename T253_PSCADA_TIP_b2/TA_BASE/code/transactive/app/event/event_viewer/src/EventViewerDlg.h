 ...
 * in any form.
 *
 * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/event/event_viewer/src/EventViewerDlg.h $
 * @author:  Nick Jardine
 * @version: $Revision: #1 $
 *
...
// Dialog Data
	...
	NamedFilterAccessFactory::FilterNameList m_FilterNameList;
	//}}AFX_DATA
...
//     void loadEvents(bool enableRefresh);
	...
	/**
     * setParaFromCommandLine
     *
     * set other parameters(ATSEventCount, EnableSCSCEvent, EnableASEvent) from the command line.
     */
	//void setParamFromCommandLine();
    ...
    // The filter object
    CombinedEventFilter* m_filter;   // This will actually act as an EventHandlerManager   
    ...
    // The time to wait between event refreshes (milliseconds).
    unsigned long m_refreshTimeDB;

	// The time to wait between event refreshes from Event Agent (milliseconds).

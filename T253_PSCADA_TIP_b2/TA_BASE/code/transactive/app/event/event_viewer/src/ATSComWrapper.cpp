...
namespace TA_Base_App
{
		...
		std::vector<TA_Base_Core::ICombinedEventData*>& events)
	{
			...
			{
				m_pSubscriber = NULL;
				LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Fail to CoCreateInstance in CLSID_ATSFilterCriteria.");
				return actCommand; // fail to QueryInterface
			}
			...
			if( m_pSubscriber != NULL )
			{
				...
				hr = m_pSubscriber ->startSubscription( pATSFilter, backloglength , &pATSEvents, &atsSubErr);
				...
				if (FAILED(hr))
				{
					LOG0(SourceInfo, m_ATS_func_debug_level ? DebugUtil::DebugDebug : DebugUtil::DebugInfo, "startSubscription failed");
					return Reconnect;
				}
				...
				{
					long size = 0;
					pATSEvents->GetSize(&size);
					LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "ATS subscriber has started a new subscription successfully, and returned total backlog events: %d ", size);
					try
					{
	...
	*/ 
	int ATSComWrapper::loadATSSystemsInfo( )
	{
		...
		{
			long err;
			HRESULT hr = pATSBridgeUtility->DoEventInfoQuery ( & pEventSystems, & pEventTypes, & pAlarmTypes, &err);
			if(err == 0 /*ATS_QUERY_OK*/)
			{
				...
				for(int i=0; i<evSize; i++)
				{
					...
					long subSysSize = 0;
					pSubSystemList->GetSize(&subSysSize);
					for( int j=0; j<subSysSize; j++)
					{
						BSTR bstrSubName;
						pSubSystemList->GetItem(j, &bstrSubName);
	...
	{
		pATSFilter->put_StartTimePeriodDefined((filterCriteria.StartTimePeriodDefined)?VARIANT_TRUE:VARIANT_FALSE);
		pATSFilter->put_EndTimePeriodDefined((filterCriteria.EndTimePeriodDefined)?VARIANT_TRUE:VARIANT_FALSE);
...
// 		adjustATSFilterTime(pATSFilter);
	...
	void  ATSComWrapper::setStartEndTimeToATSFilter ( IATSFilterCriteria * pATSfilter, 
		FilterCriteria & filterCriteria)
	{
		VARIANT_BOOL bStartTime;
		pATSfilter->get_StartTimePeriodDefined(&bStartTime);
		if(bStartTime == VARIANT_TRUE)
		{
		...
		VARIANT_BOOL bEndTime;
		pATSfilter->get_EndTimePeriodDefined(&bEndTime);
		if(bEndTime == VARIANT_TRUE)
		{
...
// 	}
	...
	void ATSComWrapper::setATSFilterStartTimeByCTime(IATSFilterCriteria *  atsfilter, CTime time)
	{
	...
	void ATSComWrapper::setATSFilterEndTimeByCTime(IATSFilterCriteria *  atsfilter, CTime time)
	{
	...
	void  ATSComWrapper:: setTMTypeToATSFilter(IATSFilterCriteria * atsfilter, FilterCriteria & filterCriteria)
	{
	...
	ICombinedEventData * ATSComWrapper::createCombinedEvent ( CComPtr<IATSEvent> pATSEvent, unsigned long key)
	{
		try
		{
			AtsEventWrap pEventWrap(pATSEvent);

			// get the creatime (timeb)
			
			ISCSDateTime iscsTime(pEventWrap.TimeStamp->Year, pEventWrap.TimeStamp->Month, pEventWrap.TimeStamp->Day,
				pEventWrap.TimeStamp->Hour, pEventWrap.TimeStamp->Minute, pEventWrap.TimeStamp->Second, pEventWrap.TimeStamp->Nanosecond);
			timeb createtime;
			...
			_itoa_s(pEventWrap.AlarmSeverity,buff, 10);
			std::string alarmSeverityName = buff;

			// MMS type:
			std::string alarmMmsState = "";
			short mmsState = pEventWrap.AlarmMMSState;
			switch(mmsState)
			{
			...
			if( /*ATSEventAlarmAvalancheType::ATS_EVENTALARMAVLTYPE_CHILD*/ 1 == pEventWrap.AlarmAvalancheType)
			{
				//alarmAVlState = itoa(TA_Base_Core::IAlarmData::CHILD_AVL, buff, 10);
				_itoa_s(TA_Base_Core::IAlarmData::CHILD_AVL, buff, 10);
				alarmAVlState = buff;
			}else if(/* ATSEventAlarmAvalancheType::ATS_EVENTALARMAVLTYPE_HEAD*/ 0 == pEventWrap.AlarmAvalancheType)
			{
			...
			short alarmState = pEventWrap.AlarmState;
			switch (alarmState)
			{
			...
			if( /*ATSEventAlarmType::ATS_EVENTALARMTYPE_BOTH*/ 2 == pEventWrap.OperationalMaintenanceType )
			{
				operation_mode = "B";
			}
			else if(/* ATSEventAlarmType::ATS_EVENTALARMTYPE_MAINTENANCE*/ 1 == pEventWrap.OperationalMaintenanceType )
			{
				operation_mode = "M";
			}else if(/*ATSEventAlarmType::ATS_EVENTALARMTYPE_OPERATIONAL*/ 0 == pEventWrap.OperationalMaintenanceType )
			{
				operation_mode = "O";
			}

			string eventID_str = pEventWrap.EventID;
			
			// to do: change CombinedEventData to CombinedEventItem
			return new CombinedEventData( TA_Base_Core::E_AUDIT_EVENT, key, createtime, alarmSeverityName, pEventWrap.Asset, pEventWrap.Description,
				pEventWrap.Value, alarmMmsState,alarmDssState, alarmAVlState, pEventWrap.Operator, alarmComment, pEventWrap.AlarmID, alarmAck, alarmStatus,
	...
	ActionCommand ATSComWrapper::stopATSSubscription(void){
		...
		if(  m_pSubscriber != NULL )
		{
			...
			long atsSubErr = -1;
			m_pSubscriber->stopSubscription(&atsSubErr);
		...
		FilterCriteria filterCriteria, unsigned long atsMaxLength)
	{
		...
		hr = pATSBridgeUtility->DoStartTimeEndTimeQuery(
			pATSFilter,
			pStartTime,
			pEndTime,
			&pATSEvents, &err);
	...
	void ATSComWrapper::truncateATSEvents (IATSEventList * pATSEvents, unsigned long atsMaxLength, bool cutSmallTime)
	{
		long evSize = 0;
		pATSEvents->GetSize(&evSize);
		if( (unsigned long)evSize <= atsMaxLength )
		{
			return;
		}
		else 
		{
			// cut some events
			unsigned long cutNum = evSize - atsMaxLength;
		...
		FilterCriteria filterCriteria, unsigned long atsMaxLength)
	{
		...
		if(filterCriteria.pkeyBoundary_ATS != "")
		{
			...
			hr = pATSBridgeUtility->DoPreviousPageQuery( pATSFilter, bstr_ATSPkey, atsMaxLength, &pATSEvents, &err);
		...
		else
		{
			...
			pATSFilter->GetEndTimePeriod(&pEndTime);
			LOG_GENERIC(SourceInfo, m_ATS_func_debug_level? DebugUtil::DebugDebug : DebugUtil::DebugInfo , "Coming into IATSEventsAPI::DoPreviousPageQuery().");
			hr = pATSBridgeUtility->DoPreviousPageQueryDate( pATSFilter, pEndTime, atsMaxLength, &pATSEvents, &err);
			LOG_GENERIC(SourceInfo, m_ATS_func_debug_level? DebugUtil::DebugDebug : DebugUtil::DebugInfo , "Come out from IATSEventsAPI::DoPreviousPageQuery().");
		...
		FilterCriteria filterCriteria, unsigned long atsMaxLength)
	{
		...
		if(filterCriteria.pkeyBoundary_ATS != "")
		{
			...
			hr = pATSBridgeUtility ->DoForwardPageQuery(pATSFilter, bstr_ATSPkey, atsMaxLength, &pATSEvents, &err);
		...
		}else
		{
			...
			hr = pATSBridgeUtility ->DoForwardPageQueryDate(pATSFilter, pStartTime, atsMaxLength, &pATSEvents, &err);

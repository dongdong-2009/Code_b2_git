/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/event/event_viewer/src/CombinedEventFilter.cpp $
  * @author:  Bradley Cleaver
  * @version: $Revision: #1 $
  *
  * Last modification: $DateTime: 2018/01/23 17:40:21 $
...
const std::string CombinedEventFilter::COLUMN_ASSET("Equipment ID");
const std::string CombinedEventFilter::COLUMN_ALARM_STATUS("Alarm State"); // add this column
...
const std::string CombinedEventFilter::COLUMN_T("T"); //add this column
const std::string CombinedEventFilter::COLUMN_MMS("M");
const std::string CombinedEventFilter::COLUMN_DSS("P"); //change from "D" to "P"
...
const std::string CombinedEventFilter::NAMEDFILTER_DEFAULT( "No Filter" );
											 ...
											 m_eventHanlderManager(NULL)
{
    ...
    m_columns.push_back( Filter::Column( COLUMN_ASSET, 270, ACTION_NO_ACTION ) ); //TD15005  
    m_columns.push_back( Filter::Column( COLUMN_DESCRIPTION, 600, ACTION_NO_ACTION ) ); //TD15005 
    m_columns.push_back( Filter::Column( COLUMN_VALUE, 167, ACTION_NO_ACTION ) );
	m_columns.push_back( Filter::Column( COLUMN_ALARM_STATUS, 94, ACTION_NO_ACTION ) );    
	m_columns.push_back( Filter::Column( COLUMN_T, 34, ACTION_NO_ACTION ) );  //add column "T"
    m_columns.push_back( Filter::Column( COLUMN_MMS, 34, ACTION_NO_ACTION ) );
    m_columns.push_back( Filter::Column( COLUMN_DSS, 34, ACTION_NO_ACTION ) );
    m_columns.push_back( Filter::Column( COLUMN_AVL, 34, ACTION_NO_ACTION ) );
    m_columns.push_back( Filter::Column( COLUMN_OPERATOR, 200, ACTION_NO_ACTION ) );
//	m_columns.push_back( Filter::Column( HIDDEN_COLUMN_EVENTID, 200, ACTION_NO_ACTION ) );  //todo: to be commented. just for testing
...
void CombinedEventFilter::populateFilterCriteria (FilterCriteria & filter)
{
	...
	// Alarm states
	ulVector = m_namedFilter->getNumericFilter(FILTER_ALARMSTATE);
	if ( !ulVector.empty() )
	{

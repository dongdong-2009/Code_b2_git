...
namespace TA_Base_App{
	...
	void EventHandler::setATSSubscriptionErr(long err)
	{
		...
		//if ats subscription error is received set ATSWorker thread to degraded mode
		if(m_ATSSubscriptionErr != 0)
		{
			if(m_workers.find(ATSCategory)!= m_workers.end())
			{
				 m_workers.find(ATSCategory)->second->setDegradeMode(true);
				 LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "setATSSubscriptionErr(), Error: %d, ATS worker set to degraded mode",err);
			}
		}

		//if ats subscription is okay set degraded mode to false
		else
		{
			if(m_workers.find(ATSCategory)!= m_workers.end())
			{
				m_workers.find(ATSCategory)->second->setDegradeMode(false);
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "setATSSubscriptionErr(), ATS worker set to normal mode");
			}
		}
	}
	...
	{	
		/*GUI is cleared before every new task*/
	...
	void EventHandler::statusBarUpdate(TA_Base_Core::EventCategory eventCategory,std::string status )
	{
		m_GUIObserver->statusBarUpdate(0, eventCategory, status); 
	}
...
// 				eventDatas[i] = NULL;
	...
	void EventHandler::mergeSort(TA_Base_Core::EventCategory eventCategory,std::vector<DisplayItem*>& b)
	{
		...
		while(i<m_eventStoreList.size() && j<b.size())
		{
			...
			else
			{
				if(updateEventIDSet)
				{
					//insert ISCS Event ids into m_eventIdSet
					if (eventCategory == ISCSCategory && m_eventIdSet.find(b[j]->getEventID())== m_eventIdSet.end())
					{
						m_eventIdSet.insert(b[j]->getEventID().c_str());
						c.push_back(b[j]);
					}
					//insert ATS Event ids into m_ATSeventIdSet
					else if(eventCategory == ATSCategory && m_ATSeventIdSet.find(b[j]->getEventID())== m_ATSeventIdSet.end())
					{
						m_ATSeventIdSet.insert(b[j]->getEventID().c_str());
						c.push_back(b[j]);
					}
					else
					{
		...
		if(j<b.size())
		{
			for(; j<b.size(); j++)
			{
				if(updateEventIDSet)
				{
					...
					//insert ATS events ids into m_ATSeventIdSet
					else if(eventCategory == ATSCategory && m_ATSeventIdSet.find(b[j]->getEventID())== m_ATSeventIdSet.end())
					{
						m_ATSeventIdSet.insert(b[j]->getEventID().c_str());
						c.push_back(b[j]);
		...
		if(!fromRear)  //cut from head
		{
			for(int i=0; i<cutNum; i++)
			{
				...
				if( tempItem!=NULL )
				{
					...
					if(m_mode==RealTimeMode)
					{	
						if(tempItem->getEventCategory()== ISCSCategory)
							m_eventIdSet.erase(tempItem->getEventID());
						else
							m_ATSeventIdSet.erase(tempItem->getEventID());
					}
	...
	void EventHandler::addCache( std::vector<DisplayItem*>& eventItems,TA_Base_Core::EventCategory eventCategory)
	{
		...
		for(int i=0; i<eventItems.size(); i++)
		{
			...
			if(updateEventIDSet)
			{
				...
				if(eventCategory == ISCSCategory && m_eventIdSet.find(tempItem->getEventID())== m_eventIdSet.end())
				{
					m_eventStoreList.push_back(tempItem);
					m_eventIdSet.insert(tempItem->getEventID());
				}	
				else if(eventCategory == ATSCategory && m_ATSeventIdSet.find(tempItem->getEventID())== m_ATSeventIdSet.end())
				{
					m_eventStoreList.push_back(tempItem);
					m_ATSeventIdSet.insert(tempItem->getEventID());
				}
	...
	{
		//clear only m_eventStoreList, m_eventIdSet and m_ATSeventIDset m_displayList not cleared. 
	...
	void EventHandler::mergeEventFromATS( std::vector<TA_Base_Core::ICombinedEventData*> & eventlist)
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Begin merging for events from ATS, total %d events", eventlist.size());

		// loop eventSeq, check the eventID, if already exist in m_ATSeventIDSet, mark it as deleted in eventSeq
		std::vector<TA_Base_Core::ICombinedEventData*> newEventList;
		std::vector<TA_Base_Core::ICombinedEventData*>::iterator itr = eventlist.begin() ;
		std::string tempEventID = "";
		TA_Base_Core::ThreadGuard guard(m_storedlistLock);
		...
		for (; itr!=eventlist.end(); itr++)
		{
			TA_Base_Core::ICombinedEventData* pEventData = *itr;
			tempEventID = pEventData->getEventID();
			if(m_ATSeventIdSet.find(tempEventID) != m_ATSeventIdSet.end())  //if can find the eventID in the set
			{
				//release the duplicate event:
				delete pEventData;
				pEventData = NULL;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/config_plugin_helper/src/AbstractListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
END_MESSAGE_MAP()
    ...
    void AbstractListView::loadAllItems()
    {
        ...
        std::map<CString,unsigned long> items;
        try
        {
        ...
        for (std::map<CString,unsigned long>::iterator iter = items.begin(); iter != items.end(); ++iter)
        {
        ...
        while(pos != NULL)
        {
            ...
            try
            {
                ...
                if (newItem !=  NULL)
                {
                    ...
                    CString newName = dynamic_cast<TA_Base_Core::IItem*>(newItem)->getName().c_str();
                    newName += AbstractDetailView::NEW_LABEL;
                    newItem->setName( std::string(newName) );
            ...
            catch ( const TA_Base_Core::DataException& ex)
            {
                ...
                if (ex.getFailType() == TA_Base_Core::DataException::CANNOT_DELETE)
                {
                    ...
                    if(!ex.getWhichData().empty())
                    {
						...
						char* tok;
					    tok = strtok(const_cast<char*>(ex.getWhichData().c_str()), ",");
    ...
    void AbstractListView::newItem()
    {
        ...
        try
        {
            ...
            {
                // Append something to the end of the name to indicate that this is a new item
                std::string newName = dynamic_cast<TA_Base_Core::IItem*>(newItem)->getName();
                newName += AbstractDetailView::NEW_LABEL;

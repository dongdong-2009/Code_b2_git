/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/alarm_plan_association/src/TA_AlarmPlanAssocConfigPlugin.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
{
    static AFX_EXTENSION_MODULE TA_AlarmPlanAssocConfigPluginDLL = { NULL, NULL };
    ...
    DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
    {
	    ...
	    {
		    TRACE0("TA_ALARMPLANASSOCCONFIGPLUGIN.DLL Initializing!\n");
		    
		    // Extension DLL one-time initialization
		    if (!AfxInitExtensionModule(TA_AlarmPlanAssocConfigPluginDLL, hInstance))
			    return 0;
		    ...
		    new CDynLinkLibrary(TA_AlarmPlanAssocConfigPluginDLL);
	    }
	    else if (dwReason == DLL_PROCESS_DETACH)
	    {
		    TRACE0("TA_ALARMPLANASSOCCONFIGPLUGIN.DLL Terminating!\n");
		    // Terminate the library before destructors are called
		    AfxTermExtensionModule(TA_AlarmPlanAssocConfigPluginDLL);
	    }
	    return 1;   // ok
    }
    ...
    {
	    return new AlarmPlanAssocComponent(componentName);
    }

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/operator/src/OperatorDetailView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
END_MESSAGE_MAP()
    ...
    void OperatorDetailView::populateItemDetails(TA_Base_Core::IConfigItem* currentOperator)
    {
        ...
        try
        {
            ...
            m_password.SetWindowText( m_currentOperator->getPassword().c_str() );
    ...
    void OperatorDetailView::onButtonPassword() 
    {
        ...
        try
        {
			...
			std::string origin_Password = m_currentOperator->getName();
			std::string encrypted_Password;
			origin_Password +="/";
			origin_Password += newPassword.GetBuffer(0);
			int rtnCode= TA_Base_Bus::AuthenticationLibrary::GetHash(origin_Password,encrypted_Password);
			...
			m_encryptPassword = std::string(encrypted_Password);

            // If this is a different location then set it in our IConfigEntity*.
            m_currentOperator->setPassword( std::string(encrypted_Password) );
    ...
    void OperatorDetailView::onButtonApply() 
    {
		...
		{
			/*
			bool isNew = m_currentOperator->isNew();
			
						if(!isNew && m_changePassword)
						{
							LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] (!isNew && m_changePassword)");
							std::auto_ptr<TA_Base_Core::IOperatorPassword> theOperatorPassword = TA_Base_Core::OperatorPasswordAccessFactory::getInstance().getOperatorPassword(m_currentOperator->getKey(),true,false);
							theOperatorPassword->changePassword(m_currentOperator->getPassword());
							theOperatorPassword->applyChanges();
							LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] (!isNew && m_changePassword)=>done");
						}
			
						LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] applyButtonPressed");*/
			
			applyButtonPressed(m_currentOperator);
			//LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] applyButtonPressed done");
/*

			if(isNew)
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] (isNew)");
				std::auto_ptr<TA_Base_Core::IOperator> theOperator( TA_Base_Core::OperatorAccessFactory::getInstance().getOperator(m_currentOperator->getName(),false));
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] etInstance().getOperator");
				std::auto_ptr<TA_Base_Core::IOperatorPassword> theOperatorPassword = TA_Base_Core::OperatorPasswordAccessFactory::getInstance().CreateOperatorPassword(theOperator->getKey());
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] getInstance().CreateOperatorPassword %s %d",m_encryptPassword.c_str(),theOperator->getKey());
				theOperatorPassword->setPassword(m_encryptPassword);
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] setPassword");
				theOperatorPassword->applyChanges();
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-15] (isNew)=>done");
			}
*/

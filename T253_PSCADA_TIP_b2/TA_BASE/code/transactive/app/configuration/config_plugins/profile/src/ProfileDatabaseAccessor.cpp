/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/profile/src/ProfileDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    std::map<CString,unsigned long> ProfileDatabaseAccessor::getItemNames(CProgressCtrl& progress)
    {
    ...
    std::vector<std::string> ProfileDatabaseAccessor::getAllDistinctParameterNames()
    {
        ...
        std::vector<std::string> parameterNames = TA_Base_Core::ProfileAccessFactory::getInstance().getAllDistinctParameterNames();

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/alarm_rules/src/SelectDataNodeDialog.cpp $
...
END_MESSAGE_MAP()
    ...
    BOOL SelectDataNodeDialog::OnInitDialog() 
    {
        ...
        m_alarmTypeKeyNameMap = m_databaseAccessor->getAllAlarmTypes();

        for(std::map<unsigned long, std::string>::iterator it =
                m_alarmTypeKeyNameMap.begin();
            it != m_alarmTypeKeyNameMap.end();
            it ++)
        {

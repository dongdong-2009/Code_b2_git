/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/action_groups/src/DutyDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    void DutyDatabaseAccessor::loadItems()
    {
        ...
        for (LoadedDuties::iterator dutyToDelete = m_duties.begin(); dutyToDelete != m_duties.end(); ++dutyToDelete)
        {
            if( (dutyToDelete->second) != NULL )
            {
                delete dutyToDelete->second;
                dutyToDelete->second = NULL;
            }
        }
        m_duties.clear();
    ...
    std::map<CString,unsigned long> DutyDatabaseAccessor::getItemNames(CProgressCtrl& progress)
    {

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/rights/src/TA_RightsConfigPlugin.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
{
    static AFX_EXTENSION_MODULE TA_RightsConfigPluginDLL = { NULL, NULL };
    ...
    DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
    {
	    ...
	    {
		    TRACE0("TA_RIGHTSCONFIGPLUGIN.DLL Initializing!\n");
		    
		    // Extension DLL one-time initialization
		    if (!AfxInitExtensionModule(TA_RightsConfigPluginDLL, hInstance))
			    return 0;
		    ...
		    new CDynLinkLibrary(TA_RightsConfigPluginDLL);
	    }
	    else if (dwReason == DLL_PROCESS_DETACH)
	    {
		    TRACE0("TA_RIGHTSCONFIGPLUGIN.DLL Terminating!\n");
		    // Terminate the library before destructors are called
		    AfxTermExtensionModule(TA_RightsConfigPluginDLL);
	    }
	    return 1;   // ok
    }
    ...
    {
	    return new RightsComponent(componentName);
    }

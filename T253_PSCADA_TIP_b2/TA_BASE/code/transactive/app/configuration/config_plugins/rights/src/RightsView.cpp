/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/rights/src/RightsView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    const CString RightsView::CHANGE_SUBSYSTEM_STATE_DIALOG_TITLE = "Change Subsystem State";
...
END_MESSAGE_MAP()
    ...
    void RightsView::setComponentInterface(RightsComponent* component)
    {
        ...
        static_cast<CTabCtrl*>(GetDlgItem(IDC_SUBSYSTEM_APP_TAB))->InsertItem(0, "Subsystems");
        static_cast<CTabCtrl*>(GetDlgItem(IDC_SUBSYSTEM_APP_TAB))->InsertItem(1, "Applications");
    ...
    void RightsView::onChangeSubsystemState() 
    {
        ...
        try
        {
            ChangeDialog::ItemsForSelection items =
                m_component->getDatabaseAccessor()->getAllSubsystemStates();

            // Now construct the dialog and then wait for the user to
            // finish with it
            ChangeDialog dlg(
                items, CHANGE_SUBSYSTEM_STATE_DIALOG_TITLE, currentSubsystemState, this);

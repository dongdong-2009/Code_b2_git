/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/plan_dss_rule/src/PlanDssRuleDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    std::map<CString,unsigned long> PlanDssRuleDatabaseAccessor::getItemNames( CProgressCtrl& progress )
    {
        FUNCTION_ENTRY("getItemNames");

        std::map<CString,unsigned long> mapNotExist;
    ...
    void PlanDssRuleDatabaseAccessor::convertItemToDisplay( TA_Base_Core::IConfigPlanDssDpRule& refFrom, PlanDssDpRuleText& refTo )
    {
        ...
        refTo.strMinimumInterval = TA_Base_Core::DataConversion::toString( refFrom.getMinimumInterval() );
		refTo.strActivedTolerantTime = TA_Base_Core::DataConversion::toString( refFrom.getTolerantTimeForActive());
		refTo.strDenyStrategy = TA_Base_Core::DenyMultipleStrategyString[refFrom.getDenyMultipleStrategy()];
		refTo.strActivateValue = refFrom.getActivateValue() ? "True" : "False";
		refTo.strNeedDeactivate = refFrom.getNeedDeactivate() ? "True" : "False";
        refTo.strAllowMultiple = refFrom.getAllowMultiple() ? "True" : "False";
		refTo.strEnable = refFrom.getEnable() ? "True" : "False";
	...
	void PlanDssRuleDatabaseAccessor::convertValidDaysToDisplay( std::string& strValidDaysInWeek, TA_Base_Core::IConfigPlanDssDpRule& refFrom )
	{
		...
		{
			strValidDaysInWeek = " Sun";
		}
		if(refFrom.getValidMonday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Mon";
		}
		if(refFrom.getValidTuesday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Tue";
		}
		if(refFrom.getValidWednesday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Wed";
		}
		if(refFrom.getValidThursday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Thu";
		}
		if(refFrom.getValidFriday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Fri";
		}
		if(refFrom.getValidSaturday())
		{
			strValidDaysInWeek = strValidDaysInWeek + " Sat";
		} 
	}
    ...
    void PlanDssRuleDatabaseAccessor::reloadLocations()
    {
        ...
        {
            m_mapLocations[(*itLoop)->getKey()] = (*itLoop)->getName();
        }
    }

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/config_plugins/agent/src/ParameterListCtrl.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    const int     ParameterListCtrl::ATTRIBUTE_UPDATE_COLUMN_WIDTH= 65;
	...
	// Image size constant
	const int ParameterListCtrl::IMAGE_SIZE = 10;
...
END_MESSAGE_MAP()
    ...
    void ParameterListCtrl::setupList()
    {
        ...
        m_imageList.Create(IDB_BITMAP_MANDATORY, IMAGE_SIZE, IMAGE_SIZE, ILC_COLOR);
	...
	void ParameterListCtrl::onDividerDoubleClick(NMHDR* pNMHDR, LRESULT* pResult) 
	{
		...
		// Adding on double the image width seems to work okay.
		SetColumnWidth(column, maxWidth + 2 * IMAGE_SIZE);

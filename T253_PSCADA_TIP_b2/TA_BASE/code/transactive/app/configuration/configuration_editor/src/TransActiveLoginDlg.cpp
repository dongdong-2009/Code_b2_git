/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/configuration_editor/src/TransActiveLoginDlg.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
END_MESSAGE_MAP()
    ...
    void TransActiveLoginDlg::populateOperatorCombo()
    {
        ...
        for (OperatorVector::iterator iter = m_allOperators.begin(); iter != m_allOperators.end(); ++iter)
        {
            try
            {
                int index = m_nameCombo.AddString( (*iter)->getDescription().c_str() );
                ...
                case(TA_Base_Core::GenericGUIException::ACCESS_DENIED):
						 {
							 ...
							 std::string actionName = "run this application";
							 userMsg << userName;
							 userMsg << actionName;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/T253_PSCADA_TIP/TA_BASE/transactive/app/configuration/configuration_editor/src/MainFrm.h $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
//#ifdef TDS_DEVELOP
	private:
		/*
		 *
		 * Develop mode -- tds, Live mode -- Live mode
		 * This is called to determine if the component is working on tds mode.
		 * add by hongzhi
		 * @ return bool - return true if ce is running tds , otherwise false
	  	 */
		bool isWorkingOnTdsMode();

		time_t m_lastClickTime;				//add by hongzhi for TDSCE
		time_t m_clickTime;
		std::fstream m_outFile;
		const std::string GetFileName(const std::string& suffix = "");
		std::string WriteTDSLog(int dbType);			// read TDS sql log from database, then write it to MFT, 0 mean oracle sql, 1 mean mysql sql
		void eraseTDSLOG_FromDB();			//erase tds sql log of table TDS_SQLLOG
		bool m_bInitComeplete;					//lock toolbar before initialize complete
//#endif
    };
}
/////////////////////////////////////////////////////////////////////////////

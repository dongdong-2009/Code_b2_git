...
BOOL CGridCtrl::MoveColumn(int iPrevColIndex, int iNewColIndex)
{
	if(iPrevColIndex == iNewColIndex)
		return FALSE;

	for(int i = 0; i < m_nRows; i++)
	{
		GRID_ROW* pRow = m_RowData[i];
		pRow->InsertAt(iNewColIndex, pRow->ElementAt(iPrevColIndex));
		pRow->RemoveAt(iNewColIndex < iPrevColIndex ? iPrevColIndex + 1 : iPrevColIndex);
	}

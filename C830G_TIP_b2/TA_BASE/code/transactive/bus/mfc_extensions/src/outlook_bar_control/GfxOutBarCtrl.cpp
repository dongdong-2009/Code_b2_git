/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/bus/mfc_extensions/src/outlook_bar_control/GfxOutBarCtrl.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
void CGfxOutBarCtrl::SetFolderText(const int index, const char * text)
{
	...
	size_t nSize = lstrlen(text)+1;
	pbf->cName = new char [ nSize ];
	memset(pbf->cName,0,nSize);
...
int CGfxOutBarCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    arialBoldFont.CreatePointFont(100, "Arial Bold");
...
CGfxOutBarCtrl::CBarFolder::CBarFolder(const char * name, DWORD exData)
{
	...
	if (name)
	{
		size_t nSize = lstrlen(name)+1;
		cName = new char[ nSize ];
		ASSERT(cName);
		memset(cName,0,nSize);
		lstrcpy(cName, name);
	}
	pLargeImageList = NULL;
	pSmallImageList = NULL;
...
CGfxOutBarCtrl::CBarItem::CBarItem(const char * name, const int image, DWORD exData)
{
	...
	{
		size_t nSize = lstrlen(name)+1;
		cItem = new char[ nSize ];
		ASSERT(cItem);
		memset(cItem,0,nSize);
		lstrcpy(cItem, name);
	}
}

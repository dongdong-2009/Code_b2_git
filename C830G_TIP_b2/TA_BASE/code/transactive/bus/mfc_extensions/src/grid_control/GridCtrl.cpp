/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/bus/mfc_extensions/src/grid_control/GridCtrl.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
void CGridCtrl::SelectCells(CCellID currentCell)
{
    ...
    // Ripple:
    // Added so we only need to single click to get the editable cell. If you want to go back
    // to double click etc then remove this line.
    OnEditCell(m_SelectionStartCell.row, m_SelectionStartCell.col, VK_LBUTTON);
}
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject)
{
	...
	size_t utxtSize = ::GlobalSize(hmem);
	CMemFile sf((BYTE*) ::GlobalLock(hmem), utxtSize);

    // CF_TEXT is ANSI text, so we need to allocate a char* buffer
    // to hold this.
	LPSTR szBuffer = new char[utxtSize + 1];
	::ZeroMemory(szBuffer, utxtSize+1);
	memset(szBuffer,0,::GlobalSize(hmem));
    if (!szBuffer)
        return FALSE;

    sf.Read(szBuffer, utxtSize);
    ::GlobalUnlock(hmem);
...
BOOL CGridCtrl::GetCellRangeRect(const CCellRange& cellRange, LPRECT lpRect) const
{
    ...
    lpRect->right  = MaxOrigin.x + GetColumnWidth(cellRange.GetMaxCol());
    lpRect->bottom = MaxOrigin.y + GetRowHeight(cellRange.GetMaxRow()-1);

    return TRUE;
}

// Returns the portion of the specified text visible in the specified rectangle,
// including an ellipsis (...) if the string does not fit the width of the rectangle.
CString CGridCtrl::GetVisibleCellText(const CDC& dc, const CRect& rect, const CString& text) const
{
    CSize textExtent = dc.GetTextExtent(text);

    if (textExtent.cx <= rect.Width())
    {
        return text;
    }

    static const CString ELLIPSIS = "...";
    int ellipsisExtent = dc.GetTextExtent(ELLIPSIS).cx;
    int maxExtent = rect.Width() - ellipsisExtent;
    int maxChars;

    ::GetTextExtentExPoint(dc, text, text.GetLength(), maxExtent, &maxChars, NULL, &textExtent);

    return (text.Left(__max(maxChars, 1)) + ELLIPSIS);
}
...
/* NOTE (TD15533): The following method is overrided in CGridCtrlS. 
 * Any changes made in here should be reflected in CGridCtrlS.
 */
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_LeftClickDownCell == m_idCurrentCell)
    {
        ...
        return;
             ...
             m_MouseMode != MOUSE_OVER_ROW_DIVIDE)        
    {
		//TD7399 - if m_bEnableSelection == TRUE, then can only do a SetFocus
		// to the cells to show that the cell is Selected
		if(m_bEnableSelection) 
		{
			SetFocusCell(-1,-1);
    ...
    {    
		// TD7399 - no selecting is done when m_bEnableSelection = FALSE
		if (!m_bEnableSelection)
			return;

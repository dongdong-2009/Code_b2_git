/**
  ...
  * in any form.
  *
  * Source : $File: //depot/4669_T01271350/TA_BASE/transactive/bus/scada/proxy_library/src/ScadaProxyManager.cpp $
  * @author  A. Parker
  * @version $Revision: #2 $
...
namespace TA_Base_Bus
{
	...
	void	ScadaProxyManager::startProcess()
	{
		...
		if(false == m_isProcessing)
		{
			...
			m_dataPointProxyManager->start();
			m_dataNodeProxyManager->start();
			m_dataPointSyncManager.startSynchronisation();
			m_dataNodeSyncManager.startSynchronisation();
			m_isProcessing = true;
	...
	void	ScadaProxyManager::stopProcess()
	{
		...
		if(true == m_isProcessing)
		{
			...
			m_dataPointProxyManager->terminateAndWait();
			m_dataNodeProxyManager->terminateAndWait();

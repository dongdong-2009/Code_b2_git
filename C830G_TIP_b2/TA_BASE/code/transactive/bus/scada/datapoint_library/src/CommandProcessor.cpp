...
namespace TA_Base_Bus
{
    ...
    void CommandProcessor::scheduleCommand(DataPointWriteRequest* dpWriteRequest)
    {
		...
		char szTemp[1024] = {0};
		sprintf(szTemp,"Command: commandId[%u], entityKey[%u], value[%s] is put into write queue.",dpWriteRequest->getCommandId(), dpWriteRequest->getDataPoint()->getEntityKey(), dpWriteRequest->getValue().getValueAsString().c_str());
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, szTemp );
        m_writeQ->insert(dpWriteRequest);

		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Current write queue size [%u].", m_writeQ->getSize() );
    }

				...
				if ((*dp)->getEntityKey() == (*itr1)->getOutputEntityKey())
				{
					...
					if ( true == isValidValue )
					{
						try
						{
							...
							(*dp)->setValue(dpValue->getValueAsString(), sessionId, triggeringDp.triggerringDpKey, (char *)triggeringDp.triggerringDpValue.c_str(), auditEventLoggingEnabled, cancelTime);

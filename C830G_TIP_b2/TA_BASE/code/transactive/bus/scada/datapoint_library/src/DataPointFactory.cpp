/**
  ...
  * in any form.
  *
  * Source : $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/scada/datapoint_library/src/DataPointFactory.cpp $
  * @author  J. Welton
  * @version $Revision: #4 $
...
namespace TA_Base_Bus
{
	...
	DataPoint * DataPointFactory::createDataPoint ( TA_Base_Core::DataPointEntityDataPtr cfg )
	{
		...
		if ( NULL == m_eventDetailsFactory )
		{
            ...
            m_eventDetailsFactory->populateEventDetails(cfg->getAgentName());
		}
        // create ScadaPersistence if not already done so
        if ( m_persistence.get() == NULL )
        {
            m_persistence = ScadaPersistencePtr( ScadaPersistence::createScadaPersistenceForAgent( ScadaPersistence::DATAPOINT, cfg->getAgent() ) );
        ...
        if (m_dpStateUpdateSender == NULL)
        {
            m_dpStateUpdateSender = new DataPointStateUpdateSender(cfg->getAgentName(), *this);
            m_commandProcessor = new CommandProcessor(m_dpStateUpdateSender, m_dpAgent);
            m_dpStateUpdateServant = new DataPointStateUpdateServant(cfg->getAgentName() + DP_STATE_UPDATE_INTERFACE, m_commandProcessor);
        }

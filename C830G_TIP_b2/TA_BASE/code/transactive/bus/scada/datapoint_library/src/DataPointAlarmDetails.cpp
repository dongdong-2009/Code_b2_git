/**
  ...
  * in any form.
  *
  * Source : $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/scada/datapoint_library/src/DataPointAlarmDetails.cpp $
  * @author  J. Welton
  * @version $Revision: #5 $
  * Last modification : $DateTime: 2011/06/22 15:33:00 $
...
namespace TA_Base_Bus
{
	...
	m_alarmHelper(AlarmHelperManager::getInstance().getAlarmHelper())
	{
        ...
        m_workItem.reset(new DataPointAlarmDetailsWorkItem(*this));
	}
	...
	void DataPointAlarmDetails::setOperationMode ( bool inControl )
	{
        m_isInControl = inControl;

        // To improve failover, will subscribe during agent startup and unsubscrip when agent terminate.
        // Don't unsubscribe when agent switch to monitor as will need to re-subscribe when change to Control.

		// only interest in Monitor mode
		/*if ( false == inControl )
		{
			// Removed handle to the EventDetailsFactory.  This is to stop the
			// DataNodeProxy to continuously send a health message to the
			// DataNodeAgent for nothing
            //TD18575, need not remove the instance here
			//if ( NULL != m_eventDetailsFactory )
			//{
			//	EventDetailsFactory::removeInstance ( m_eventDetailsFactory );
			//	m_eventDetailsFactory = NULL;
        ...
        else*/
        {
            if (!m_ackStateSubscribed)
            {
                LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "request loadAlarms for location [%u]", m_locationKey);
    ...
    DataPointAlarmDetailsWorkItem::DataPointAlarmDetailsWorkItem(DataPointAlarmDetails& parent)
		: m_parent (parent)
	{
	}

	DataPointAlarmDetailsWorkItem::~DataPointAlarmDetailsWorkItem ()
	{
	}

	void DataPointAlarmDetailsWorkItem::executeWorkItem()
	{
		//
		// callback from ThreadPool for scheduled work
		//

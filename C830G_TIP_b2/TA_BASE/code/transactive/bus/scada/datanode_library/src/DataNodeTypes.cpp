...
// 		}
	...
	bool DataNodeChildProxies::checkConfigValidForChildDataPoints(std::string & configName)
	{
		if (false == m_dataPointsHaveValidConfig)
		{
			...
			if (false == configIsGood)
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
					"Not processing datanode summary for %s, not all child datapoints have valid config[entitykey]: %d",
					configName.c_str(), entitykey );
	...
	bool DataNodeChildProxies::checkConfigValidForChildDataNodes(std::string & configName)
	{
...
// 			ChildDataNodeVectorIt itDataNode;
// 			for (itDataNode= m_dataNodes.begin();
// 			itDataNode!= m_dataNodes.end();
// 			itDataNode++)
// 			{
// 				DataNodeProxySmartPtr * pDataNode = *itDataNode;
// 				if (false == (*pDataNode)->isConfigValid() )
// 				{
// 					configIsGood = false;
// 					entitykey = (*pDataNode)->getEntityKey();
// 					break;
// 				}
// 			}
			...
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
					"Not processing datanode summary for %s, not all child datanodes have valid config[entitykey]: %d",
					configName.c_str(), entitykey );
		...
		TA_Base_Bus::ScadaProxyFactory& scadaProxyFactory, IEntityUpdateEventProcessor & entityEventProcessor)
	{
		...
		++itr )
        {
            try
            {
                ...
                DataPointProxySmartPtr* dp = getChildDataPointProxy(entityKey);
				if (NULL==dp)
				{
		...
		++itr )
        {
            ...
            {
				unsigned long entityKey = (*itr)->getKey();
                DataNodeProxySmartPtr* dn = getChildDataNodeProxy(entityKey);
				if (NULL==dn)
				{

...
//			return m_alarmSyn.getIndexinfo().latestSourceTime.time;
    ...
    void AlarmListCtrl::onGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult) 
    {
		...
		{
		// Set to unselected
		pDispInfo->item.state = 0; //Unselected
		
		  int item = pDispInfo->item.iItem;
		  // If selected
		  if (item >= 0 && item < static_cast<int>(m_alarms.size()) && m_alarms[item].isSelected )
		  {
		  pDispInfo->item.state = LVIS_SELECTED; //Selected
		  }
		  pDispInfo->item.stateMask = LVIS_SELECTED;	
		  }	    
		  
			// Handle list image items here.
			if (pDispInfo->item.mask&LVIF_IMAGE)
			{
			int item = pDispInfo->item.iItem;
            int subItem = pDispInfo->item.iSubItem;
			
			  long iconNumber = getItemIcon(item, subItem);
			  
				if (iconNumber != -1)
				{
                pDispInfo->item.iImage = iconNumber;
				}
				}
    ...
    BOOL AlarmListCtrl::OnEraseBkgnd(CDC* pDC) 
    {
		...
		if (blnDisplayIsEmpty)
        {
            ...
            if ( m_alarmStore == NULL || !m_alarmStore->isAlarmStoreValid() )
            {
                ...
                newFont.CreatePointFont(100, "Arial Bold");

 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionSuppressAlarm.cpp $
 * @author:  Anita Lee
 * @version: $Revision: #8 $
 *
 * Last modification: $DateTime: 2011/05/27 15:04:02 $
...
namespace TA_Base_Bus
{
    ...
    bool ActionSuppressAlarm::isAvailable(IAlarmRetriever& alarmRetriever)
    {
		...
		TA_Base_Core::IEntityData* pentitydata = 
				alarmRetriever.getDatabaseCache().searchEntityDataList(TA_Base_Core::RunParams::getInstance().get(RPARAM_ENTITYNAME));	 
		unsigned long entityKey = pentitydata->getKey();
		...
		for (int i=0; i<selectedAlarms.size(); i++)
		{
			...
			std::map<unsigned long, bool>::iterator matching = m_alarmTypeKeySystemMap.find(alarmDetail->messageTypeKey );
			...
			*/
            ...
            unsigned long actionClose = TA_Base_Bus::AlarmActions::getAlarmAction(alarmDetail->messageTypeKey,
                                                                        TA_Base_Bus::AlarmActions::CLOSE_ALARM);

            if( alarmRetriever.getRightsChecker().canPerformAction(alarmDetail->assocEntityKey, actionClose) )
            {
                ...
                unsigned long actionAck = TA_Base_Bus::AlarmActions::getAlarmAction(alarmDetail->messageTypeKey,
                                                              TA_Base_Bus::AlarmActions::ACK_ALARM);

                if( !(alarmRetriever.getRightsChecker().canPerformAction(alarmDetail->assocEntityKey, actionAck)) )
                {
...
//			m_selectedAlarmDetails.clear();
	...
	bool ActionSuppressAlarm::canPerformActionOnAlarm(IAlarmRetriever& alarmRetriever)
	{
		...
		if (m_pAlarmDetails.get() == NULL)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Alarm details were not retrieved. Alarm may have been removed from the list.");
			alarmRetriever.setIsActionAlreadyRunning(false);
			FUNCTION_EXIT;
			return false;
		}
...
//            try
//            {
//                // Copy the alarms into this vector for safety reasons
//                m_selectedAlarmDetails.push_back(alarmRetriever.getAlarm(*iter));
//            }
//            catch( const TA_Base_Core::AlarmListCtrlException& )
//            {
//                LOG_EXCEPTION_CATCH(SourceInfo, "AlarmListCtrlException", "Could not retrieve the alarm from the alarm store to create a suppression rule");
//                loadError = true;
//            }
//        }

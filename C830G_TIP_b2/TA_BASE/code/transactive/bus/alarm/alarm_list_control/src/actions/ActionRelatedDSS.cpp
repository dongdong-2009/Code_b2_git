 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionRelatedDSS.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Bus
{
    ...
    bool ActionRelatedDSS::isAvailable(IAlarmRetriever& alarmRetriever)
    {
        ...
        if (alarmRetriever.getRightsChecker().canPerformAction( getPlanManagerKey(alarmRetriever),
			TA_Base_Bus::aca_RUN_APPLICATION))
        {
            ...
            try
            {
				...
				std::auto_ptr<TA_Base_Bus::AlarmDataItem> pTemp (pdataTemp);
                alarm = pTemp;
...
//        }
    ...
    unsigned long ActionRelatedDSS::getPlanManagerKey(IAlarmRetriever& alarmRetriever)
    {
        ...
        if (0 == m_planManagerKey)
        {
			...
			TA_Base_Core::IEntityData* pentitydata = 
				alarmRetriever.getDatabaseCache().searchEntityDataList(TA_Base_Core::PlanManagerEntityData::getStaticType());
			...
			if(pentitydata == NULL)
			{
				...
				this->m_planManagerKey = pentitydata->getKey();
			}
            /*try
            {

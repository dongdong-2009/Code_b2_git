 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionSuppressAlarm.h $
 * @author:  Anita Lee
 * @version: $Revision: #7 $
 *
...
namespace TA_Base_Bus
{
    ...
    public:
		...
		std::map<unsigned int, std::string> getSubActionControlIds() const
		{
			...
			subActionIds.insert(std::pair<unsigned int, std::string> (INHIBIT_ALARM_BY_TYPE, "By T&ype") );
			subActionIds.insert(std::pair<unsigned int, std::string> (INHIBIT_ALARM_BY_DESCRIPTION, "By Descripti&on") );
			return subActionIds;
		};

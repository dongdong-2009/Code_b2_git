 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionRelatedInspector.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Bus
{
    ...
    bool ActionRelatedInspector::isAvailable(IAlarmRetriever& alarmRetriever)
    {
        ...
        if (alarmRetriever.getRightsChecker().canPerformAction( getInspectorPanelKey(alarmRetriever),
    ...
    unsigned long ActionRelatedInspector::getInspectorPanelKey(IAlarmRetriever& alarmRetriever)
    {
        ...
        {		
			...
			TA_Base_Core::IEntityData* pentitydata = 
				alarmRetriever.getDatabaseCache().searchEntityDataList(INSPECTOR_PANEL_ENTITY_TYPE_NAME);
			//	unsigned long entitykey;
			if(pentitydata == NULL)
			{
				...
				this->m_inspectorPanelKey = pentitydata->getKey();
			}
            /*try
            {
                TA_Base_Core::IEntityDataList inspectorPanels = 
                    TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfType(INSPECTOR_PANEL_ENTITY_TYPE_NAME);

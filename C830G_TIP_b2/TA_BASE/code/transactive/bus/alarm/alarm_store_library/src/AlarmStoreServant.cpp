...
void AlarmStoreServant::init()
{
	try
	{
		...
		//m_ATSAlarmObserver= new ATSAlarmObserver();
...
AlarmStoreServant::~AlarmStoreServant()
{
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "rightsLibrary.unregisterObserver");
    
	if (m_rightsLibrary !=NULL) m_rightsLibrary->unregisterObserver(*this );
...
void AlarmStoreServant::removeDuty()
{
	...
	//AlarmAccessAdaptor::getInstance().clean();
	AlarmAccessAdaptor::getInstance().deleteAllAlarms();

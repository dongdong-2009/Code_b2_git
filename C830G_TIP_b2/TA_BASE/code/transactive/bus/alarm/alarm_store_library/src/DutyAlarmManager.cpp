...
//		TA_Base_Core::ThreadGuard guard(m_lock);
	...
	long DutyAlarmManager::removeLocSubSystemDutyAlarms(unsigned long ulLocationKey, std::list<unsigned long>& subSystemKeyList)
	{
		TA_Base_Core::ThreadGuard guard(m_lock);
		if (NULL != m_pSharedAlarm)
		{
			return m_pSharedAlarm->removeLocSubSystemDutyAlarms(ulLocationKey, subSystemKeyList);
		}
		return 0;
	}

 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/bus/history/data_recording/src/Datum.cpp $
 * @author:  Jade Welton
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Bus
{
    ...
    {
		size_t nSize = value.length() + 1;
        m_value.m_string = new char[ nSize ];
		memset(m_value.m_string, 0, nSize);
        ...
        if ( &copyMe != this )
        {
            ...
            switch( copyMe.m_type )
            {
                ...
                case STRING_ITEM :
					...
					nSize = strlen( copyMe.m_value.m_string ) + 1;
                    m_value.m_string = new char[ nSize ];
					memset(m_value.m_string, 0, nSize);

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/scheduling/scheduling_manager/src/JobManager.cpp $
  * @author:  San Teo
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    #define CATCH_ALL_EXCEPTIONS(needCleanUp)                                                                           \
        ...
        {                                                                                                               \
            ...
            TA_ASSERT(false, "Agent does not recognise the job type");                                                  \
        }                                                                                                               \
    ...
    void JobManager::receiveSpecialisedMessage(const TA_Base_Core::CommsMessageCorbaDef& message)
    {
        ...
        else if (STRINGS_IDENTICAL == mesgTypeKey.compare(SchedulingCommsJobScheduled.getTypeKey()))
        {
            ...
            SchedulingAgentCorbaDef::JobID id = CORBA::string_dup(data->jobIdentifer);
			std::string jobIDStr(id);
            ...
            if (existingJob == m_jobs.end())
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Got an update but the job hasn't been added to our list.  So adding job %s", id);
            ...
            (STRINGS_IDENTICAL == mesgTypeKey.compare(SchedulingCommsJobUpdated.getTypeKey())))
        {
            ...
            SchedulingAgentCorbaDef::JobID id = CORBA::string_dup(data->identifier);
			std::string jobIDStr(id);
            JobPtrMap::iterator existingJob = m_jobs.find(jobIDStr);
            if (existingJob == m_jobs.end())
            {
    ...
    void JobManager::deleteJob(const SchedulingAgentCorbaDef::JobID& id)
    {
        ...
        try
        {
			JobPtrMap::iterator it = m_jobs.find(jobIDStr); 
			...
			CORBA_CALL( m_scheduler,
			            removeJob,
			            ( CORBA::string_dup(id), m_sessionId.c_str() ) );
        }
    ...
    void JobManager::saveNewJob(Job& job)
    {
        ...
        // SchedulingAgentCorbaDef::JobID id = CORBA::string_dup(job.getId());
		...
		JobPtrMap::iterator it = m_jobs.find(id);
	...
	//TD16352++
	void JobManager::updateEditLock(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId, const bool bLock)
	{
		...
		if(it == m_jobs.end())
		{
			...
			LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "[TD16352]Job id %s submitted for edit lock not found", jobId);
			return;
		}
		...
		try
		{
			CORBA_CALL( m_scheduler,
			            updateEditLock,
			            ( CORBA::string_dup(jobId), m_sessionId.c_str(), bLock ) );
		}
		CATCH_ALL_EXCEPTIONS(false);
	...
	bool JobManager::isJobLockedForEdit(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId)
	{
		...
		{
			//job is not in map so safe to edit
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "[TD16352]Job id %s is not edit locked since it is not in cache", jobId);
			return false;
		}

		try
		{
			CORBA_CALL_RETURN( isJobLocked,
			                   m_scheduler,
			                   isJobLockedForEdit,
			                   ( CORBA::string_dup(jobId), m_sessionId.c_str() ) );

			if(isJobLocked==true)
			{
				LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "[TD16352]Job id %s is edit locked", jobId);
			}
			else
			{
				LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "[TD16352]Job id %s is edit locked", jobId);
    ...
    void JobManager::enableJob(const SchedulingAgentCorbaDef::JobID& id, const bool enable)
    {
        ...
        try
        {
			...
			if (enable && isInhibited && (it->second->getStartDate() != NULL))
			{
                ...
                CORBA_CALL( m_scheduler,
                            updateJob,
                            ( it->second->toCorbaObject(), m_sessionId.c_str(), true, true ) );
			}
			...
			else if (!enable && !isInhibited)
			{
                ...
                CORBA_CALL( m_scheduler,
                            updateJob,
                            ( it->second->toCorbaObject(), m_sessionId.c_str(), true, false ) );
			}
        }
        CATCH_ALL_EXCEPTIONS(false);
    ...
    void JobManager::loadProfileList()
    {
        ...
        for (JobPtrMap::iterator job = m_jobs.begin(); job != m_jobs.end(); job++)
        {
            TA_ASSERT(job->second != NULL, "Pointing to a NULL job");
    ...
    bool JobManager::operatorHasJobProfile(const Job& job)
    {
        ...
        SchedulingAgentCorbaDef::ProfileID profileId = job.getProfileId();

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/scheduling/scheduling_manager/src/SchedulingManagerDoc.cpp $
  * @author:  
  * @version: $Revision: #1 $
  *
...
END_MESSAGE_MAP()
    ...
    void CSchedulingManagerDoc::deleteJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId)
    {
	    FUNCTION_ENTRY( "deleteJob" );
        TA_ASSERT(m_jobManager != NULL, "JobManager is NULL, cannot continue");
        m_jobManager->deleteJob(CORBA::string_dup(jobId));
	    FUNCTION_EXIT;
    }


    void CSchedulingManagerDoc::enableJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId, const bool enable)
    {
	    FUNCTION_ENTRY( "enableJob" );
        TA_ASSERT(m_jobManager != NULL, "JobManager is NULL, cannot continue");
        m_jobManager->enableJob(CORBA::string_dup(jobId), enable);
	    FUNCTION_EXIT;
    }

	//TD16352++
	void CSchedulingManagerDoc::updateEditLock(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId, const bool bLock)
	{
		FUNCTION_ENTRY( "updateEditLock" );
		TA_ASSERT(m_jobManager != NULL, "JobManager is NULL, cannot continue");
		m_jobManager->updateEditLock(CORBA::string_dup(jobId), bLock);
		FUNCTION_EXIT;
	}

	bool CSchedulingManagerDoc::isJobLockedForEdit(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobId)
	{
		FUNCTION_ENTRY( "isJobLockedForEdit" );
		TA_ASSERT(m_jobManager != NULL, "JobManager is NULL, cannot continue");
		FUNCTION_EXIT;
		return m_jobManager->isJobLockedForEdit(CORBA::string_dup(jobId));
	}
	//++TD16352

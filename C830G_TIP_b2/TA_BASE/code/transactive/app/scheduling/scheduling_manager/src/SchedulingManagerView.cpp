...
*/
    ...
    void CSchedulingManagerView::populateItem(int index, Job& job)
    {
        ...
        if (theProfile != profiles.end())
        {
            ...
            if (theProfile->second->requiresLocationToBeSelected())
            {
                ...
                {
                    m_listCtrl.SetItemText(index, Location, theLocation->second->getName().c_str());
                }
            }
        }
		...
		if (!GetDocument()->isProfileFilterOn() || profileFound)
		{
			...
			if (index < 0)
			{
				...
				if (totalItems == MAX_JOB_TO_DISPLAY)
				{
					...
					updateNumJobsStatus();
					LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugDebug, "The list is already showing max number of jobs, not going to display job = %s", job->getId());
					FUNCTION_EXIT;
					return 0;
				}
				...
				updateNumJobsStatus();
				LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugDebug, "Adding job = %s into the view", job->getId());
			}
    ...
    void CSchedulingManagerView::onJobAdd() 
    {
        ...
        TA_ASSERT(GetDocument() != NULL, "Document should not be NULL");
    ...
    void CSchedulingManagerView::onJobDelete() 
    {
        ...
        try
        {
            ...
            for (unsigned int i = 0; i < total; i++)
            {
                try
                {
					...
					if(isLocked==true)
					{
							LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "[TD16352]Displaying transactive message with action job id%s", selections[i]->getId());
							TA_Base_Bus::TransActiveMessage userMsg;
							userMsg << action;
					...
					{
						GetDocument()->deleteJob(CORBA::string_dup(selections[i]->getId()));
						updateNumJobsProcessed(IDS_DELETE_JOBS_STATUS, i, total);
					}
					//++TD16352
                ...
                catch (SchedulingManagerException& ex)
                {
                    ...
                    case SchedulingManagerException::SESSION_PROFILES_NOT_MATCH_JOB_PROFILE: // The session profiles do not include one that is associated with the job
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Unable to delete job %s : %s", selections[i]->getId(), selections[i]->getDescription().c_str());//TD14337 yezh++
                        break;
                    default:
                        throw;
                    }
        ...
        catch (const SchedulingManagerException& ex)
        {
            ...
            default:
                userMsg.showMsgBox(IDS_UE_250001);
                break;
            }
        }
        FUNCTION_EXIT;
	...
	//TD16352++
	bool CSchedulingManagerView::isJobEditLockedForSelection(TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobId)
	{
		...
		try
        {
            isLocked = GetDocument()->isJobLockedForEdit(CORBA::string_dup(jobId));
		}
	...
	void CSchedulingManagerView::updateEditLock(TA_Base_Bus::SchedulingAgentCorbaDef::JobID jobId, bool lock)
	{
		try
        {
			GetDocument()->updateEditLock(CORBA::string_dup(jobId),lock);
		}
		catch (const SchedulingManagerException& ex)
        {
			TA_Base_Bus::TransActiveMessage userMsg;
			...
			switch(ex.getFailType())
            {
				case SchedulingManagerException::JOB_NOT_EXIST:          // Job does not exist
						userMsg << action;
						userMsg.showMsgBox(IDS_UE_250003);
            ...
            for (unsigned int i = 0; i < total; i++)
            {
                try
                {
					...
					else
					{
						...
						GetDocument()->enableJob(CORBA::string_dup(selections[i]->getId()), isEnable);
                ...
                catch (const SchedulingManagerException& ex)
                {
					...
					LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Unable to enable job %d : %s", selections[i]->getId(), selections[i]->getDescription().c_str());//TD14337 yezh++
    ...
    {
        bool isAscend = (currentSort == AutoSortListCtrl::ECurrentSort::ASCENDING);
	...
	bool CSchedulingManagerView::canLoadDLL()
	{
        ...
        try
        {
			...
			else
			{
                ...
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_250005);
			}
        }
        catch (...)
        {

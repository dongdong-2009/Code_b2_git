/**
  ...
  *
  * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/scheduling/scheduling_agent/src/SchedulingAgent.cpp $
  * @author:   Dirk McCormick
  * @version:  $Revision: #3 $
  *
  * Last modification: $DateTime: 2009/01/02 14:44:05 $
...
using TA_Base_Core::StateUpdateMessageSender;
...
namespace TA_Base_App
{
    ...
    typedef TA_Base_Core::ThreadedMemberFunction< TA_Base_App::SchedulingAgent > AgentThreadedMemberFunction;

	const char* SchedulingAgent::RPARAM_WORKSTATIONID = "WorkstationId";


SchedulingAgent::SchedulingAgent(int argc, char* argv[])
    :
    m_genericAgent(NULL)
{
    m_genericAgent = new GenericAgent(argc, argv, this, NULL, AGENT_USER_TYPE_NORMAL);
    TA_ASSERT(m_genericAgent != NULL, "m_genricAgent should never be NULL");

    try
    {
        initialiseSingletons(argc, argv);
    }
...
void SchedulingAgent::initialiseSingletons(int argc, char* argv[])
{
    //
    ...
    {
        TA_THROW( 
            TA_Base_Core::GenericAgentException(
                "WorkstationID RunParam must be set") );
    }

    std::string workstationIDAsString =
        RunParams::getInstance().get(RPARAM_WORKSTATIONID);
    if( ! getStringAsUnsignedLong(workstationIDAsString, workstationID) )
    {
        TA_THROW( 
            TA_Base_Core::GenericAgentException(
                "The workstation id is not a number"));
    }
    ...
    LOG_GENERIC(
        ...
        "Initialising singletons. entityKey: '%d', subsystemKey: '%d', "
    ...
    JobFactory::getInstance().initialise(argc, argv, *entityData);
...
{
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Terminating the Scheduling Agent");

    //
    // If this object is in control mode, disable the controller
    //
...
IEntity* SchedulingAgent::createEntity(TA_Base_Core::IEntityDataPtr entityData)
{
    return NULL;
}
...
{
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Setting Agent to monitor mode");

    //limin++--, failover
    AgentThreadedMemberFunction& thrd = AgentThreadedMemberFunction::alloc( *this );
    thrd.add( &SchedulingAgent::processingMonitorMode );
    thrd.add( &SchedulingAgent::unsubscribeToMessages );
    thrd.add( &SchedulingAgent::registerForMonitorMessages );
    thrd.start();
    //MessageSubscriptionManager::getInstance().unsubscribeToMessages(this);
...
//limin, for failover
void SchedulingAgent::unsubscribeToMessages()
{
    MessageSubscriptionManager::getInstance().unsubscribeToMessages( this );
}
...
{
    LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Setting Agent to control mode");

    //limin++--, failover
    AgentThreadedMemberFunction& thrd = AgentThreadedMemberFunction::alloc( *this );
    thrd.add( &SchedulingAgent::processingControlMode );
    thrd.add( &SchedulingAgent::unsubscribeToMessages );
    thrd.add( &SchedulingAgent::registerForControlMessages );
    thrd.start();
...
//limin++, for failover
/*
void SchedulingAgent::agentSetOperationModeInThread()
{
    //
    // If this object is in control mode,
    // deactivate the controller,
...
void SchedulingAgent::processingControlMode()
{
    FUNCTION_ENTRY( "processingControlMode" );
...
void SchedulingAgent::processingMonitorMode()
{
    FUNCTION_ENTRY( "processingMonitorMode" );
...
void SchedulingAgent::receiveBatchUpdateRequest(
    const TA_Base_Core::StateUpdateMessageCorbaDef& message)
{
    TA_ASSERT(
        std::string(message.messageTypeKey) ==
...
void SchedulingAgent::sendBatchUpdateMessage()
{
    ...
    //
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Sending batch update");

    StateUpdateMessageSender* messageSender =
...
void SchedulingAgent::run()
{
    ...
    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Starting Scheduling Agent");
    ...
    try
    {
        ...
        else
        {
            ...
            message += RunParams::getInstance().get(RPARAM_OPERATIONMODE);
    ...
    if(message != "")
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugFatal, message.c_str());
...
{
    TA_ASSERT(RunParams::getInstance().get(RPARAM_OPERATIONMODE) == RPARAM_CONTROL,
    ...
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Initialising Agent in control mode");
...
{
    TA_ASSERT(RunParams::getInstance().get(RPARAM_OPERATIONMODE) == RPARAM_MONITOR,
              "initialiseAgentMonitorMode() must be called on agent in "
              "monitor mode");

    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Initialising Agent in monitor mode");
    ...
    //
    LOG_GENERIC(
        SourceInfo, DebugUtil::DebugInfo, "Sending batch update request");
...
bool SchedulingAgent::getStringAsUnsignedLong(
    const std::string& numAsString, unsigned long& number)
{

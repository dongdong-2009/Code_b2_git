/**
  ...
  *
  * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/scheduling/scheduling_agent/src/SchedulingAlarmSender.cpp $
	...
	std::string assetName)
{
    ...
    try
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Closing Agent initialisation Failure Alarm");
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Closing Agent Unexpected Termination Alarm");
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Closing Database Failure Alarm");
...
void SchedulingAlarmSender::jobExecutionFailed(
    ...
    const std::string& errorMessage)
{
    ...
    NameValuePair descriptionPair("Description", jobDescription);
    NameValuePair instanceIDPair("InstanceId", minuteStartedString.str());
    NameValuePair errorMessagePair("ErrorMessage", errorMessage);
    ...
    submitNonUniqueAlarm(
        TA_Base_Core::SchedulingAlarms::JobExecutionFailureAlarm,
        params);
}
    ...
    const std::string& reason)
{
    ...
    // [Reason]
    //
    NameValuePair reasonPair("Reason", reason);
    ...
    //
    submitAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingAgentInitialisationFailureAlarm,
        params);
}
    ...
    const std::string& reason)
{
    ...
    //
    submitAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingAgentUnexpectedTerminationAlarm,
        params);
}
    ...
    const std::string& reason)
{
    ...
    //
    submitNonUniqueAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingInvalidDataConfigurationAlarm,
        params);
}


void SchedulingAlarmSender::schedulingMonitorError(
    const std::string& action, const std::string& reason)
{
    ...
    // [Reason]
    //
    NameValuePair actionPair("Action", action);
    NameValuePair reasonPair("Reason", reason);
    ...
    //
    submitNonUniqueAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingMonitorAlarm, params);
}
    ...
    const std::string& action, const std::string& reason)
{
    ...
    //
    submitNonUniqueAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingControllerAlarm, params);
}
...
void SchedulingAlarmSender::databaseFailure(const std::string& reason)
{
    ...
    //
    submitAlarm(
        TA_Base_Core::SchedulingAlarms::SchedulingDatabaseFailureAlarm, params);
}

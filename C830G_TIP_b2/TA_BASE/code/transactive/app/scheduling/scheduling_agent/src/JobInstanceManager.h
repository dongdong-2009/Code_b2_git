...
public:
     ...
     */
    void jobRemoved(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID);
     ...
     */
	bool updateEditLock(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID, std::string strSessionId, bool isLocked);
     ...
     */
	bool isJobLocked(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID, std::string strSessionId);
	//++TD16352

	//CL-21225 don't allowed to remove a job which running now
	bool isJobRunning(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID);
    /**
     ...
     *          NULL otherwise.
     */
    IJob* getJob(const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID) const;
...
private:
    ...
    /**
     *  Removes a job instance without locking the object.
     */
    void removeJobInstanceUnlocked(
        const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID,
        unsigned long minuteStarted);
    ...
    <
        TA_Base_Bus::SchedulingAgentCorbaDef::JobID, unsigned long
    >
    RunningJobsMultiMap;
	*/
    ...
    /**
     *  Gets an iterator pointing to the entry in the running jobs multimap
     *  entry matching the parameters.
     *
     *  @return the iterator if a matching entry is found,
     *          m_runningJobsMultiMap.end() if no matching entry is found.
     */
    RunningJobsMultiMap::iterator findRunningJobInstance(
        const TA_Base_Bus::SchedulingAgentCorbaDef::JobID& jobID,
        unsigned long minuteStarted);


    /**
     *  Gets all scheduled jobs, without locking the object.
     */
    TA_Base_Bus::SchedulingAgentCorbaDef::JobSequence getAllJobsUnlocked() const;

    /**
     *  Gets all job instances without locking the object.
     */
    TA_Base_Bus::SchedulingAgentCorbaDef::JobInstanceSequence
        getJobInstancesUnlocked() const;

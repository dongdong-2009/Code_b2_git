...
/**/
                                                       ...
                                                       std::vector<std::string>& params )
    {
        if (m_currentEntity->getParameterValue(parameterName).empty())
        {
			...
			unsigned long typekey = m_currentEntity->getTypeKey();
			std::string parameterDefaultValue = 
				TA_Base_Core::EntityAccessFactory::getInstance().getParameterDefaultValue(parameterName, typekey);
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "name:%s,typekey:%d,value:%s", parameterName.c_str(), typekey, parameterDefaultValue.c_str());

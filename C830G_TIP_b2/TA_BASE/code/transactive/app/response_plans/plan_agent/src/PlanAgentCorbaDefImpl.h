...
/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanAgentCorbaDefImpl.h $
...
//Replace 'exceptionToString' with 'exc.what.in()', for exceptions with 'what' member variable.
// dahe++
#define TA_THROW_CORBA_EXCEPTION(exc)											\
{																				\
	LOG(SourceInfo,																\
...
namespace TA_Base_App
{
	class PlanAgentUser;
	//Class declaration
    class PlanAgentCorbaDefImpl : public virtual POA_TA_Base_Bus::IPlanAgentCorbaDef,
	...
	public:
		...
		virtual void resumePlanAtStep(const char* session, const TA_Base_Core::ActivePlanId& plan, TA_Base_Core::StepNumber step);
		//++lizettejl
		...
		// TD20368 Yanrong++
		virtual unsigned long runPlanByNameWithoutActionPermittedCheck(const char* session, const char* path, const char* param = NULL);
		// ++Yanrong
		//marvin++
		//TD13527
		virtual unsigned long runPlanByNameWithEntityRestriction( const char* session, const char* path, unsigned long entitykey);
		//++marvin
		//TD13527

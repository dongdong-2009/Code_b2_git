/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepEditStationTisPage.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #2 $
  *
...
void CPlanStepEditStationTisPage::DisplayParameters()
{
    ...
    std::string stationLevel(step->getStationLevelParameter()->getMessageText());
    TA_Base_Core::TISPidList pids(step->getPidListParameter()->getPids());
    TA_ASSERT(!(stationLevel.empty() == false && pids.length() > 0), "Both the station level and PID filters are set");
...
void CPlanStepEditStationTisPage::UpdateParameters()
{
    ...
    else
    {
        ...
        stationParameter->setStation(static_cast<LPCTSTR>(stationName));
    }

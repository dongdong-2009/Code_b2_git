...
TA_Base_Core::ActivePlanDetail* PlanAgentAccess::getAssocAlarmPlanDetail(std::string alarmId)
{
    ...
    PAA_BEGIN_TRY
    {
        ...
        m_planAgentMap.getDefaultPlanAgent().setPlanAgentTimeout(PLAN_AGENT_PLAN_MEDIUM_TIMEOUT);
        // ++TD15049
        m_planAgentMap.getDefaultPlanAgent().getAssocAlarmPlanDetail(alarmId.c_str(), activePlanDetailVar.out());
...
void PlanAgentAccess::startPlan(TA_Base_Core::NodeId planNodeId)
{
    ...
    PAA_BEGIN_TRY
    {
        ...
        activePlanId.param = CORBA::string_dup("");
...
void PlanAgentAccess::resumePlanAtStep(TA_Base_Core::ActivePlanId activePlanId, TA_Base_Core::StepNumber stepNumber)
{
    ...
    {
        m_planAgentMap.getDefaultPlanAgent().setPlanAgentTimeout(PLAN_AGENT_PLAN_LONG_TIMEOUT);
        m_planAgentMap.getPlanAgent(activePlanId.location).resumePlanAtStep(getSessionId().c_str(),
            activePlanId, stepNumber);
    }
    PAA_END_TRY(resumePlan);

...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/trending/trend_viewer/src/HistoricalDataLoader.h $
  * @author:  Craig Grant
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    private:
		...
		//TD17884 
		TA_Base_Core::IData* executeQuery(TA_Base_Core::IDatabase* dbConnection, const std::string& sql, std::vector<std::string>& columnNames);

        //TD17883 - needed to display trend values correctly
        PlotData executeSql(const std::string& sql);		
		PlotData executeSql(const std::string& sql, unsigned long entityKey, time_t startTime, time_t endTime, unsigned long samplePeriod);
		...
		std::string generateSql(DayComponents& dayComponents, unsigned long entityKey, 

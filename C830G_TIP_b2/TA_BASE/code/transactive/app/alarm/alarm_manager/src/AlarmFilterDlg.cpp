/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/app/alarm/alarm_manager/src/AlarmFilterDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2011/05/27 14:28:17 $
...
namespace TA_Base_App
{
	...
	const std::string AlarmFilterDlg::FILTER_ASSET("Asset");
...
/////////////////////////////////////////////////////////////////////////////
	...
	void AlarmFilterDlg::OnTextchangeNamedFilter()
	{
		...
		if(!str.Compare("") || str.Find("DEFAULT")>=0
			|| str.Find("USER")>=0 || str.Find("DEFINED")>=0)
		{
			m_saveButton.EnableWindow( false );
		}
...
// 		bool isDefault = ( fName.Find("DEFAULT") >= 0 ) ? true : false;
		bool isUserORDefindORDefault = ( fName.Find("USER") >= 0 
			|| fName.Find("DEFAULT") >= 0 ||fName.Find("DEFINED") >= 0 ) ? true : false;
		bool isInComboList = m_namedFilter.FindString(-1, fName) >= 0 ? true : false;
		
		if ( isUserORDefindORDefault ) // Can't save a Default or User Defined file
		{
...
/////////////////////////////////////////////////////////////////////////////
	...
	void AlarmFilterDlg::retrieveLocations()
	{
		...
		for (unsigned int i = 0;i < allLocationData.size(); ++i)
        {
			...
			{
				ListItem location(allLocationData[i]->getKey(),allLocationData[i]->getName());
	...
	void AlarmFilterDlg::OnSelchangeNamedFilter() 
	{
		...
		if ( 0 <= selIdx )
		{
			...
			if ( isCurFilter == false )
			{
				...
				{
					LOG_EXCEPTION_CATCH( SourceInfo, "DataException", ex.getWhichData() );
					//begin Li Zhongyan add, just for not display the warning when using the user defined filter
					if(filterName == TA_Base_Core::NamedFilter::UNNAMED)
					{
						LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "filterName == TA_Base_Core::NamedFilter::UNNAMED");
						//.compare(TA_Base_Core::NamedFilter::UNNAMED.c_str()))
					}
					//end Li Zhongyan add

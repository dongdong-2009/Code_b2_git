 ...
 * 
 * Source:    $File: //depot/4669_T01271350/TA_BASE/transactive/app/security/authentication_agent/src/AuthenticationAgent.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #7 $
 *
 * Last modification: $DateTime: 2009/07/17 14:53:05 $
...
const int OVERRIDE_OPERATOR = 1;
const int OVERRIDE_PROFILE = 1;
...
namespace TA_Base_App
{
    ...
    void AuthenticationAgent::processStateUpdate(TA_Base_Bus::SessionInfoCorbaDef* session)
    {
        ...
        if (session->updateType == TA_Base_Bus::TA_Security::SessionStart)
        {
            ...
            ISession* theSession = SessionAccessFactory::getInstance().createSession(
				...
				session->LocationId, m_entityKey);
    ...
    void AuthenticationAgent::processSessionTimeStateUpdate(TA_Base_Bus::SessionInfoCorbaDef* session)
    {
        ...
        if (session->updateType == TA_Base_Bus::TA_Security::SessionStart)
        {
            ...
            for ( iterStart=0; iterStart<session->additionalOperators.length(); iterStart++ )
            {
                sessionInfo.OperatorId.push_back( session->additionalOperators[iterStart] );
            }
            for ( iterStart=0; iterStart<session->additionalProfiles.length(); iterStart++ )
            {
                sessionInfo.ProfileId.push_back( session->additionalProfiles[iterStart] );
            }
			m_authentication->updateSessionTime(sessionInfo, false);
        }
		else if (session->updateType == TA_Base_Bus::TA_Security::ProfileUpdate)
		{
			bool isAlreadyOverride = false;

			std::vector<std::string>::iterator ovrideSesPos;
			ovrideSesPos = std::find(m_overrideSessionVec.begin(), m_overrideSessionVec.end(),sessionInfo.SessionId);
			if (ovrideSesPos != m_overrideSessionVec.end())
			{
				isAlreadyOverride = true;
			}

			// (sessionInfo.OperatorId.size() > 1)&&(sessionInfo.ProfileId.size() > 1)
			if (true == isAlreadyOverride) // Already overrided, so this profile update should be clear override.
			{
				unsigned long opSize = sessionInfo.OperatorId.size();
				unsigned long prSize = sessionInfo.ProfileId.size();
				LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
					"Clear override to Monitor AuthenticationAgent. SessionId: %s, OperatorId size before clear: %ld, ProfileId size before clear: %ld", 
					sessionInfo.SessionId.c_str(), opSize, prSize);

				m_authentication->updateMonitorOverrideInfo(sessionInfo.SessionId,
					sessionInfo.OperatorId.at(BASE_OPERATOR),
					sessionInfo.ProfileId.at(BASE_PROFILE),
					false);
				sessionInfo.OperatorId.pop_back();
				sessionInfo.ProfileId.pop_back();

				if (ovrideSesPos != m_overrideSessionVec.end())
				{
					m_overrideSessionVec.erase(ovrideSesPos);
				}
			}
			else  // Did not override yet, so this time should be add override.
			{
				LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
					"Add override to Monitor AuthenticationAgent. SessionId: %s, Override Operator: %lu, Override Profile: %lu",
					sessionInfo.SessionId.c_str(), sessionInfo.OperatorId.at(OVERRIDE_OPERATOR), sessionInfo.ProfileId.at(OVERRIDE_PROFILE));

				unsigned long iterUpdate;
				for ( iterUpdate=0; iterUpdate<session->additionalOperators.length(); iterUpdate++ )
				{
					sessionInfo.OperatorId.push_back( session->additionalOperators[iterUpdate] );
				}
				for ( iterUpdate=0; iterUpdate<session->additionalProfiles.length(); iterUpdate++ )
				{
					sessionInfo.ProfileId.push_back( session->additionalProfiles[iterUpdate] );
				}

				m_authentication->updateMonitorOverrideInfo(sessionInfo.SessionId,
					sessionInfo.OperatorId.at(OVERRIDE_OPERATOR),
					sessionInfo.ProfileId.at(OVERRIDE_PROFILE),
					true);

				m_overrideSessionVec.push_back(sessionInfo.SessionId);
			}
		}
        else if (session->updateType == TA_Base_Bus::TA_Security::SessionEnd)
        {

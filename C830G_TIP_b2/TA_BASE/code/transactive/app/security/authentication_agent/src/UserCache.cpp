...
namespace TA_Base_App
{
    ...
    UserCache::UserDetails UserCache::getUserDetails(const unsigned long userId)
    {
		...
		LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"UserCache::getUserDetails start.");
        ...
        try
        {
            ...
            {// scope for guarding the operator map
                ...
                {
					IOperator* newOper = NULL;

					LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get operator from database.");
					newOper = TA_Base_Core::OperatorAccessFactory::getInstance().getOperator(userId, true);
					LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get operator from database done.");
					...
					{// scope for guarding the operator map
						//ThreadGuard guard(m_operatorMapLock);
						LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Insert new operator to operator map.");
						m_operatorMap.insert(OperatorMap::value_type(newOper->getKey(), dynamic_cast<IConfigOperator*>(newOper)));
						LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Insert new operator to operator map done.");
					
						LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Try to find operator in operator map.");
						opIter = m_operatorMap.find(userId);
						LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Try to find operator in operator map done.");
				...
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get IConfigOperator from operator map.");
                IConfigOperator* op = opIter->second;
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get IConfigOperator from operator map done.");

		        // Create a UserDetails object

                // store the basic details
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get password from IConfigOperator.");
				user.password = op->getPassword();	
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get password from IConfigOperator done.");
				...
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get profile from IConfigOperator.");
                profiles = (op->getAssociatedProfileKeys());
				LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"Get profile from IConfigOperator done.");
            }
		...
		LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"UserCache::getUserDetails done.");

...
#include "DutyAgent.h"
#include "DutyAgentImpl.h"
#include "IDutyAgentExecutive.h"
#include "DutyManagerSnapshotConverter.h"
#include "TransferableRegionStateConverter.h" 
#include "TransferableSubsystemStateConverter.h"
#include "CorbaDutyStateConverter.h" 
#include "GetDutyConverter.h" 
#include "GetExclusiveDutyConverter.h" 
#include "SubsystemStateSnapshotConverter.h"
#include "SubsystemStateUpdateConverter.h"
#include "AllocateDAC.h"
#include "DegradeDAC.h"
#include "FreeDAC.h"
#include "GetStateDAC.h"
#include "GetSessionStateDAC.h"
#include "GetSubsystemStateDAC.h"
...
namespace TA_Base_App
{
		...
		m_executive(NULL)
    {
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new DutyManagerUpdateSender");
		boost::shared_ptr<DutyManagerUpdateSender> pDutyManagerSend(new DutyManagerUpdateSender());
		m_pDutyManagerSender= pDutyManagerSend;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new DutyManagerUpdateSender");

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new PopupMessageSender");
		boost::shared_ptr<PopupMessageSender> pPopupSender(new PopupMessageSender());
		m_pPopupSender= pPopupSender;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new PopupMessageSender");


		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new DutyAlarmSender");
		boost::shared_ptr<DutyAlarmSender> pAlarmSender(new DutyAlarmSender());
		m_pAlarmSender= pAlarmSender;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new DutyAlarmSender");


		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new DutyAuditMessageSender");
		boost::shared_ptr<DutyAuditMessageSender> pAuditSender(new DutyAuditMessageSender());
		m_pAuditSender= pAuditSender;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new DutyAuditMessageSender");


		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new StateUpdateSender");
		boost::shared_ptr<StateUpdateSender> pStateUpdateSender(new StateUpdateSender());
		m_pStateUpdateSender= pStateUpdateSender;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new StateUpdateSender");


		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new DutyMonitorManager");
		boost::shared_ptr<DutyMonitorManager> pDutyMonitorManager(new DutyMonitorManager());
		m_pDutyMonitorManager= pDutyMonitorManager;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new DutyMonitorManager");


		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before new SubsystemStateUpdateSender");
		boost::shared_ptr<SubsystemStateUpdateSender>  pSubsystemStateUpdateSender(new SubsystemStateUpdateSender());
		m_pSubsystemStateUpdateSender= pSubsystemStateUpdateSender;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"after new SubsystemStateUpdateSender");
	...
	{
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"begin DutyAgentImpl::localStartDutyMonitor");		
		m_pDutyMonitorManager->start();
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::localStartDutyMonitor");
	}
	...
	void DutyAgentImpl::authAllocateDuty(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::authAllocateDuty ");

        FUNCTION_EXIT;
	}
/*
	void DutyAgentImpl::authReallocateDuty(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
        ...
        try
        {
		    ...
		    if( TA_Base_Bus::DutySessionCache::instance()->isOverrideBegin( sessionInfo ) )
		    {
			    TA_Base_Bus::DutyState dutyState(sessionInfo.SessionId, DutyAction::SESSION_OVR_BEGIN);
			    sessionOverrideBegin( dutyState, session, m_feLocSet );
		    ...
		    else
		    {
			    ...
			    const TA_Base_Bus::DASType::DbKey remainingProfile = sessionInfo.ProfileId.front();

			    // get a list of ALL the profiles
			    std::vector<TA_Base_Bus::DASType::DbKey> profiles;
			    TA_Base_Bus::DutySessionCache::instance()->getProfiles( sessionInfo.SessionId, profiles );
			    ...
			    for( it=profiles.begin(); it!=profiles.end(); it++ )
			    {
				    if( remainingProfile != *it )
				    {
					    ...
					    sessionInfo.ProfileId.push_back(*it);

					    TA_Base_Bus::DutyState dutyState(sessionInfo.SessionId, DutyAction::SESSION_OVR_END);
					    sessionOverrideEnd( dutyState, sessionInfo, m_feLocSet );
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::authReallocateDuty ");
		FUNCTION_EXIT;
	}
*/
	...
	void DutyAgentImpl::authFreeDuty(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::authFreeDuty ");
		FUNCTION_EXIT;
	}
		...
		const TA_Base_Bus::DutyAgentTypeCorbaDef::KeySequence& subsystems, unsigned long locationKey)
	{
        ...
        try
        {
		    ...
		    TA_Base_Bus::FrontEndStateChangeSubject::instance()->update( dutyState );
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrDegradeSubsystemsAtLocation");
        }
        catch( TA_Base_Core::TransactiveException& ex )
        {
            LOG( SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "TransactiveException caught at top level, converting to UnhandledException." );
		...
		const TA_Base_Bus::DutyAgentTypeCorbaDef::KeySequence& regions)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrTransferRegionDuty ");
		FUNCTION_EXIT;
	}
		...
		const TA_Base_Bus::DutyAgentTypeCorbaDef::SubsystemTreeSequence& subsystems)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrTransferSubsystemDuty ");
		FUNCTION_EXIT;
	}
	...
	void DutyAgentImpl::mgrTransferAllSubsystemDuty(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrTransferAllSubsystemDuty");
		FUNCTION_EXIT;
	}
	...
	void DutyAgentImpl::mgrDegradeAllSubsystems(const char* session)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrDegradeAllSubsystems");
		FUNCTION_EXIT;
	}
			...
			TA_Base_Bus::DutyAgentTypeCorbaDef::RegionDutySequence_out regDuty)
	{
        ...
        try
        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrGetDutyState");
        }
        catch( TA_Base_Core::TransactiveException& ex )
        {
            LOG( SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "TransactiveException caught at top level, converting to UnhandledException." );
		...
		TA_Base_Bus::DutyAgentTypeCorbaDef::RegionDutySequence_out regDuty)
	{
        ...
        try
        {
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"begin DutyAgentImpl::mgrGetSessionDutyState session %s", session);
			TA_Base_Bus::DutyState dutyState(session,"");
			...
			subDuty = dmsc.getSubsystemDuty();
			regDuty = dmsc.getRegionDuty();
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrGetSessionDutyState");
        }
        catch( TA_Base_Core::TransactiveException& ex )
        {
            LOG( SOURCE_INFO, GENERIC_LOG_TYPE, DEBUG_LOG_LEVEL, "TransactiveException caught at top level, converting to UnhandledException." );
			...
			const TA_Base_Bus::DutyAgentTypeCorbaDef::KeySequence& subKeys, TA_Base_Bus::DutyAgentTypeCorbaDef::Key locationKey)
	{
        ...
        try
        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrGetSubsystemDutyState");
            FUNCTION_EXIT;
		...
		DutyAgentImpl::mgrGetTransferableRegions(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
        ...
        try
        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrGetTransferableRegions");
		    FUNCTION_EXIT;
		...
		const TA_Base_Bus::DutyAgentTypeCorbaDef::KeySequence& regions)
	{
        ...
        try
        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentImpl::mgrGetTransferableSubsystems");
		    FUNCTION_EXIT;
		...
		TA_Base_Bus::DutyAgentTypeCorbaDef::Key& actionGroup)
	{
        ...
        EnsureInControlState();
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"[CL-21875] begin DutyAgentImpl::queryHasDuty ");
        try
        {
			...
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
					"[CL-21875], in DutyAgentImpl::queryHasDuty, getResult()=true, for session=%s, region=%lu, subsystem=%lu, profile=%lu, actionGroup=%lu",
					session.SessionId.in(),region,subsystem,profile,actionGroup);
			} 
			else
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
					"[CL-21875], in DutyAgentImpl::queryHasDuty, getResult()=false, for session=%s, region=%lu, subsystem=%lu, profile=%lu, actionGroup=%lu",
					session.SessionId.in(),region,subsystem,profile,actionGroup);
			}

			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"[CL-21875] end DutyAgentImpl::queryHasDuty ");
		    FUNCTION_EXIT;

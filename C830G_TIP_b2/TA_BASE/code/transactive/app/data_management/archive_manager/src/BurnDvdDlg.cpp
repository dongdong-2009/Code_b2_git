...
namespace TA_Base_App
{
    ...
    void BurnDvdDlg::run()
    {
		...
		try
		{
            ...
            try
            {
				appendStringToLogDisplay("Initializing hardware...");
				if(!iMediaAccess->initialise(false))
				{
                // Add all the files to be burnt to m_neroIsoItemsList.
					...
					appendStringToLogDisplay("Failed to initialise hardware.");
				...
				appendStringToLogDisplay("Initialized Successfully");
				...
				appendStringToLogDisplay("Drive "+ m_chosenDrive + "selected for archiving.");
				...
				appendStringToLogDisplay("Configuring Write Settings...");
				...
				if(!writeSuccessful)
				{
					...
					appendStringToLogDisplay ( std::string( reason ) );
					appendStringToLogDisplay ( "Write Failed" );
					CString error;
					error.FormatMessage( UE050090_WRITE_FAILED, m_volumeLabel.c_str(), reason );
				...
				m_buttonCancel.SetWindowText( "Close" );
				 ...
				 {		
					 ...
					 appendStringToLogDisplay ( "Write Successful");
	...
	void BurnDvdDlg::BurnStatusCallBack (long totalTime, long timeLeft, BurnStatus burnStatus) 
	{
		switch(burnStatus)
		{
		case(BurnStatus::ValidatingMedia):
			m_stageStatic.SetWindowText( _T("Validating Media") );
			break;
		case(BurnStatus::FormattingMedia):
			m_stageStatic.SetWindowText( _T("Formatting Media") );
			break;
		case(BurnStatus::InitialisingHardware):
			m_stageStatic.SetWindowText( _T("Initializing Media") );
			break;
		case(BurnStatus::CalibratingPower):
			m_stageStatic.SetWindowText( _T("Calibrating Power") );
			break;
		...
		case(BurnStatus::WriteFinalization):
			m_stageStatic.SetWindowText( _T("Write Finalization") );
			break;
		case(BurnStatus::WriteCompleted):
			m_stageStatic.SetWindowText( _T("Write Completed") );
			break;
		case(BurnStatus::WriteVerifying):
			m_stageStatic.SetWindowText( _T("Verifying Burned Data") );
			break;

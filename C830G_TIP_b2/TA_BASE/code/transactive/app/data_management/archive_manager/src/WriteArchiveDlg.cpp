/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/data_management/archive_manager/src/WriteArchiveDlg.cpp $
  * @author:  Alan Brims
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    BOOL WriteArchiveDlg::OnInitDialog() 
    {
        ...
        m_fileList.InsertColumn( DATE_COL,  _T("Date"), LVCFMT_CENTER, dateWidth, 0);
        m_fileList.InsertColumn( NAME_COL,  _T("Filename"), LVCFMT_LEFT, m_fileListColNameWidth, 1);
        m_fileList.InsertColumn( SIZE_COL,  _T("Size"), LVCFMT_RIGHT, sizeMbWidth, 2);
    ...
    void WriteArchiveDlg::populateList()
    {
        ...
        catch( TA_Base_Core::ArchiveException& e )
        {
            ...
            userMsg << e.what();
            UINT selectedButton = userMsg.showMsgBox(IDS_UE_050141);
        ...
        if ( fileDetails.empty() )
        {
            ...
            if ( m_isManualArchive )
            {
                ...
                UINT selectedButton = userMsg.showMsgBox(IDS_UW_050013);
            }
            else
            {
                CString files;
                ...
                TA_Base_Bus::TransActiveMessage userMsg;
                userMsg << files;
                UINT selectedButton = userMsg.showMsgBox(IDS_UW_050014);
            }
        ...
        try
        {
			...
			if( !TA_Base_Core::RunParams::getInstance().isSet( WriteArchiveHelper::CD_ARCHIVE.c_str()) )
			{
				numCds << m_cdsMap.size() << NUM_DVDS_MSG;
				m_insertCDDVD.SetWindowText( INSERT_DVD_MSG.c_str() );
			}
			else
			{
				numCds << m_cdsMap.size() << NUM_CDS_MSG;
        ...
        catch( TA_Base_Core::ArchiveException& e )
        {
            ...
            TA_Base_Bus::TransActiveMessage userMsg;
            userMsg << e.what();
            UINT selectedButton = userMsg.showMsgBox(IDS_UE_050142);

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/scada/OpcBridgeLib/src/DataNodeAgentProxy.cpp $
  * @author:  Ripple
  * @version: $Revision: #5 $
  *
			...
			try
			{
				...
				//lock1 block
				{
					...
					for (unsigned int i = 0; i < updateSize && !m_toBeTerminated; ++i)
					{
						...
						LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug,"MarkA--DataNodeAgentProxy %s - received an update for entity %d",m_agentName.c_str(), dataNodeUpdate.entityKey);         

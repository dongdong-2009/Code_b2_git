/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/scada/inspector_panel/src/LimitDetailsListCtrl.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2010/08/03 13:33:05 $
				...
				case THRESHOLD_VALUE_COLUMN:
					{
						...
						// get confirmation from the operator
						std::stringstream ss;
						ss << "change alarm threshold for " << dataPoint->getEntityName();
			...
			try
			{
				...
				if ( true == foundAlarmType )
				{
					...
					if (it != alarmProperties.end())
					{
						...
						if ( (dataPoint->getType() == DPT_ANALOGUE || dataPoint->getType() == DPT_METERED) && false == dataPoint->getUnit().empty() )
						//++caiwie CL-20513
						{

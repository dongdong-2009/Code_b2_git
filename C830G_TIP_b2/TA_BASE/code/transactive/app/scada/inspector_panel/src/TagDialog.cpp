...
namespace TA_Base_App
{
	...
	bool CTagDialog::applyTag()
	{
		...
		if (apply)
		{
				...
				case IDC_RADIO_TAG:
					{
						...
						submitAuditMessage("Tag", "Added", 
											dataNode->getAssetName(), dataNode->getEntityKey());
				...
				case IDC_RADIO_TAG_PTW:
					{
						...
						submitAuditMessage("Tag with PTW \"" + ptwId + "\"", "Added", 
											dataNode->getAssetName(), dataNode->getEntityKey());							
				...
				case IDC_RADIO_NOT_TAGGED:
					{
						...
						if (m_originalTagStatus == TA_Base_Bus::Tagged)
						{
							submitAuditMessage("Tag", "Removed", 
												dataNode->getAssetName(), dataNode->getEntityKey());
						}
						else
						{
							submitAuditMessage("Tag with PTW \""+ m_originalPtwId + "\"", "Removed", 
												dataNode->getAssetName(), dataNode->getEntityKey());
						}
			...
			{
				//specially handle the corba timeout exception
				bool isTimeOutButSuccess = false;

				std::string exStr = TA_Base_Core::CorbaUtil::getInstance().exceptionToString( ____ex );

				std::string message="Error: CORBA SystemException :";
				message += exStr;
				
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
						"applyTag() - %s", message.c_str());

				if (exStr.find("TRANSIENT_CallTimedout") != std::string::npos)
				{
					try
					{
						//if timeout we wait for more 35secs at most and check the result.
						//sometimes tag operation will need more than 5secs execution time 
						int loopCount = 35;
						while (loopCount-- > 0 && !isTimeOutButSuccess)
						{
							//must check the session id, exclude other's operation result
							if (dataNode->getLastStatusChangeSessionId() == sessionId)
							{
								TA_Base_Bus::ETagState newTagState = dataNode->getTaggingState();
								switch (tagRadioButton)
								{
								case IDC_RADIO_TAG:
									isTimeOutButSuccess = (newTagState == TA_Base_Bus::Tagged);
									break; 
								case IDC_RADIO_TAG_PTW:
									isTimeOutButSuccess = (newTagState == TA_Base_Bus::TaggedPtw);
									break;
								case IDC_RADIO_NOT_TAGGED:
									isTimeOutButSuccess = (newTagState == TA_Base_Bus::NotTagged);
									break;
								}
							}
						...
						LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, 
						"applyTag() finished and result :  %d", isTimeOutButSuccess);
					}
					catch(...)
					{
						//do nothing
				...
				if (isTimeOutButSuccess)
				{
					switch (tagRadioButton)
					{
					case IDC_RADIO_TAG:
						submitAuditMessage("Tag", "Added", dataNode->getAssetName(), dataNode->getEntityKey());
				...
				{
					std::string message="Error: CORBA::SystemException when tagging Equipment " + 
	...
	void CTagDialog::submitAuditMessage(std::string tagType, std::string addRemove, 

 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/event/event_viewer/src/CombinedEventFilterDlg.cpp $
...
BOOL CombinedEventFilterDlg::OnInitDialog() 
{
    ...
    setUpSeverities( m_dbAccess.getSeverities() );
...
void CombinedEventFilterDlg::loadListData()
{
    ...
    // Retrieve the subsystems and then populate the subsystems list.
    LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "loadListData");


    std::vector< std::pair<unsigned long,std::string> > subsystems = m_dbAccess.getSubsystems();
    for ( i=0; i<subsystems.size(); i++ )
    {
        ListItem item( subsystems[i].first, subsystems[i].second );
        m_subsystems.push_back( item );
    }
    m_subsystems.sort();
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "getSubsystems");
...
void CombinedEventFilterDlg::OnSubsystemExt() 
{
    ...
    MultipleLoadDlg loadDlg("Select Subsystem...");
    ...
    if( loadDlg.DoModal() == IDOK )
    {
        ...
        m_dbAccess.separateSubsystems( selection, subsystems, physicalSubsystems );
...
void CombinedEventFilterDlg::OnSaveFilter()
{
	...
	/*
    // Get the state of the filter name.
    ENamedFilterState namedFilterState = getNamedFilterState( filterName );
	

    if ( NF_READONLY == namedFilterState ) // Can't save read-only filters
    {
        TA_ASSERT( false, "The save button should not be enabled for read-only filters!" );
    }
    ...
    if ( NF_READWRITE == namedFilterState )
    {
        ...
        {
            // Load the filter.
            std::auto_ptr<NamedFilter> loadedFilter( TA_Base_Core::NamedFilterAccessFactory::getInstance().getNamedFilter( filterName ) );
            loadedFilter->deleteNamedFilter();
        }
        catch(...)
        {
...
void CombinedEventFilterDlg::loadFilter( bool importFromFilter )
{
    ...
    NamedFilter::NumericList subsystems = m_namedFilter.getNumericFilter( CombinedEventFilter::FILTER_APPLICATION_SUBSYSTEM );
    NamedFilter::NumericList physicalSubsystems = m_namedFilter.getNumericFilter( CombinedEventFilter::FILTER_PHYSICAL_SUBSYSTEM );
    subsystems.insert( subsystems.end(), physicalSubsystems.begin(), physicalSubsystems.end() );
    populateEditFromSelection( m_subsystemEdit, subsystems, m_subsystems );

    // And now do it all again for location...
    populateEditFromFilter( m_locationEdit, CombinedEventFilter::FILTER_LOCATION, m_locations );
...
void CombinedEventFilterDlg::GetFilterData()
{
	...
	m_OldFilterData.m_strEventType.Empty();
	...
	GetDlgItem(IDC_EVENT_TYPE)->GetWindowText(m_OldFilterData.m_strEventType);
...
bool CombinedEventFilterDlg::IsFilterDataChanged()
{
	...
	if(m_OldFilterData.m_strOperator != strTemp)
		return true;
	GetDlgItem(IDC_EVENT_TYPE)->GetWindowText(strTemp);
	if(m_OldFilterData.m_strEventType != strTemp)
		return true;

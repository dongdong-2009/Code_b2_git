...
ProcessManager::~ProcessManager()
{
    ...
    try
    {
		...
		//allResourseList.clear();
		//permitedResourceList.clear();
		//preAppProp.erase( preAppProp.begin(), preAppProp.end() );
    }
    catch ( ... )
    {
...
void ProcessManager::applicationHasTerminated( unsigned long processId )
{
    ...
    if (matchingAppIter != m_runningApplications.end())
    {
        ...
        delete matchingAppIter->second;
        matchingAppIter->second = NULL;
...
std::vector<ApplicationProperties> ProcessManager::getApplications()
{
	...
	int tryTimes = 0;
	while (!ResourceGot&& tryTimes < 3)
	{
		tryTimes ++;

		SessionManager::getInstance().isActionsPermitted(TA_Base_Bus::aca_RUN_APPLICATION, allResourseList,tempList);
		if(tempList.size() != 0)
		{

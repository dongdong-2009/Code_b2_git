...
namespace TA_Base_App
{
    ...
    /** 
      ...
      */
	void DbSynchPoint::getConnectionParameters( std::string& schema, std::string& password )
	{
	...
	* 
	*/
	void DbSynchPoint::setStateForNoProcessResolution()
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
		"Set datapoint (pkey = %u) state (stopped, TA_Base_Bus::QUALITY_GOOD_NO_SPECIFIC_REASON).",getDataPoint()->getEntityKey());
	...
	bool DbSynchPoint::resolve()
    {
        ...
        if ( !_Connect() )
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugError,
                "DbServerPoint::resolve() Caught exception while trying to connect to databse server %s schema %s password %s.",m_strServerName.c_str(), m_strUserName.c_str(), m_strPassword.c_str() );
            FUNCTION_EXIT;
            return false;
        }
	...
	int  DbSynchPoint::_GetDbSynStatue()
	{
		...
		try 			
		{
			...
			if (resolve())
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Db Server resolved on %s", m_strServerName.c_str());
				poll(nNewValue);

				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Db Server poll() was OK on %s", m_strServerName.c_str());
			}	
			else
			{

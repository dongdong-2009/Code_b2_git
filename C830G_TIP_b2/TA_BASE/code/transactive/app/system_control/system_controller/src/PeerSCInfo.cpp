...
namespace TA_Base_App
{
    ...
    // Constructor
    //
    PeerSCInfo::PeerSCInfo(const std::string& myHostname, const std::string& peerHostname,
                            int heartbeatSendPeriod)
        : m_myHostname(myHostname), m_peerHostname(peerHostname),
          m_isPeerResolved(false), m_heartbeatSendPeriod(heartbeatSendPeriod),
          m_systemControllerMonitor(NULL), 
		  //m_firstHeartbeatTime(0),
          //m_heartbeatCount(0), 
		  m_isRunning(false), m_port(0)
    {
    }

...
namespace TA_Base_App
{
		...
		: m_groupProcessesAllStarted(false)
    {
        ...
        try
        {
			...
			m_peersMonitor = new PeersMonitor(peerHostnames, m_heartbeatSendPeriod);
			//
            ...
            for (iter = peerHostnames.begin(); iter != peerHostnames.end(); iter++)
            {
                ...
                LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
                    "Will be sending heartbeats to peer %s, heartbeat period: %lu ms", (*iter).c_str(), /*yanrong++:CL-21195*/ m_heartbeatSendPeriod);
                m_peerSCsToPulsate.push_back(new PeerSCInfo(m_myHostname, *iter,
                                                        m_heartbeatSendPeriod));
            ...
            for (iter = groupsToMonitor.begin(); iter != groupsToMonitor.end(); iter++)
            {
                ...
                m_groupMonitors.push_back(new GroupMonitor(*iter, hostsInGroup, m_heartbeatSendPeriod));
    ...
    void SystemControllerMonitor::stopHeartBeating()
    {
        ...
        std::vector<PeerSCInfo*>::iterator it;
        for ( it = m_peerSCsToPulsate.begin(); it != m_peerSCsToPulsate.end(); ++it)
        {
            (*it)->terminate();
        }
        ...
        for ( it = m_groupSCsToPulsate.begin(); it != m_groupSCsToPulsate.end(); ++it)
        {
            (*it)->terminate();
        }
	...
	void SystemControllerMonitor::run()
    {
		...
		//TD15949++
		unsigned int ulHeartBeatSendPeriodSecs = ((m_heartbeatSendPeriod / 1000) * 2); // Heartbeats must be received within double the heartbeat rate.
		//++TD15949
        ...
        while (m_isRunning)
        {
			...
			if(diffTimeMsecs < (m_heartbeatSendPeriod * 2))
			{
	...
	void SystemControllerMonitor::checkForGroupProcesses()
	{
		...
		if (ProcessManager::getInstance().haveAllAgentsStartedAtLeastOnce())
		{
            ...
            for (iter = m_monitorsOfMyGroup.begin(); iter != m_monitorsOfMyGroup.end(); iter++)
            {
                ...
                m_groupSCsToPulsate.push_back(new PeerSCInfo(m_myHostname, *iter, m_heartbeatSendPeriod));

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/app/system_control/transact_screensaver/src/DIB256.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
{
    CDIBitmap::CBmpPalette::CBmpPalette( CDIBitmap* pBmp )
    {
        ASSERT( pBmp );
        ...
        pPal->palNumEntries = (USHORT)cPaletteEntries;
    ...
    BOOL CDIBitmap :: Load( BYTE* pFile, int nFileLength )
    {
        ASSERT( pFile );
        BOOL fReturn = TRUE;
        try {
		    ...
		    memcpy(&fileHeader, pFile, sizeof(fileHeader));
		    nFilePos += sizeof(fileHeader);
            if ( fileHeader.bfType != 0x4D42 )
                throw TEXT("Error:Unexpected file type, not a DIB\n");

            BITMAPINFOHEADER infoHeader;
		    memcpy( &infoHeader, &pFile[nFilePos], sizeof(infoHeader) );
		    nFilePos += sizeof(infoHeader);
		    ...
		    memcpy( ((BYTE*)m_pInfo) + sizeof(BITMAPINFOHEADER), &pFile[nFilePos],
                         cColorTable );
		    nFilePos += cColorTable;
            //
            // Allocate space for the pixel area, and load the pixel
            // info from the file.
            m_pPixels = new BYTE[cPixels];
		    nFilePos = fileHeader.bfOffBits;
		    memcpy( m_pPixels, &pFile[nFilePos], cPixels );
		    CreatePalette();
		    m_bIsPadded = TRUE;
    #ifdef _DEBUG
        } catch( TCHAR * psz ) {
    ...
    BOOL CDIBitmap :: Save( CFile * pFile )
    {
        ...
        DWORD dwImageSize= (DWORD)m_pInfo->bmiHeader.biSize;
	    // Add in palette size
        ...
        bmfHdr.bfOffBits = (DWORD)sizeof(BITMAPFILEHEADER) + (DWORD)m_pInfo->bmiHeader.biSize + wPaletteSize;
        pFile->Write(&bmfHdr, sizeof(BITMAPFILEHEADER));
    
        pFile->Write(m_pInfo, sizeof(BITMAPINFO) + (wColors-1)*sizeof(RGBQUAD));
        pFile->Write(m_pPixels, (dwPadWidth*(DWORD)m_pInfo->bmiHeader.biBitCount*GetHeight())/8 );
    ...
    DWORD CDIBitmap :: LastByte( DWORD dwBitsPerPixel, DWORD dwPixels ) const
    {
        ...
        return ((DWORD)extraBits % 8) ? (DWORD)numBytes+1 : (DWORD)numBytes;
    }

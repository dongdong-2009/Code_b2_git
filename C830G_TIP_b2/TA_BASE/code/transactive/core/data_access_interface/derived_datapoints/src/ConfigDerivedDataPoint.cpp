/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/derived_datapoints/src/ConfigDerivedDataPoint.cpp $
  * @author Karen Graham
  * @version $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    ConfigDerivedDataPoint::ConfigDerivedDataPoint(unsigned long pkey)
        : m_isValidData(false),
          m_isNew(false),
          m_key(pkey),
    ...
    void ConfigDerivedDataPoint::reload()
    {
        ...
        for (InputAssociationList::const_iterator iaIt(inputs.begin()); iaIt != inputs.end(); iaIt++)
        {
            ...
            m_inputAssociations.insert( InputAssociationMap::value_type(iaIt->inputDataPointKey, inputAssoc) );
        }
        ...
        for (DerivedStateList::const_iterator dsIt(states.begin()); dsIt != states.end(); dsIt++)
        {
            ...
            m_derivedStates.insert( StateMap::value_type(dsIt->stateValue, derivedState) );
        ...
        // Finally load all the output associations. Step through each derived state and load the output
        // associations for it

...
namespace TA_Base_Core
{
    ...
    bool ArchiveManagerEntityData::isArchiveManagerAlreadyRunning( std::string& workstationName )
    {
		...
		*/
        ...
        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Schema to check is %s", components[1].c_str() );
        ...
        std::string sql("select TERMINAL from local_sessions where UPPER(program) like '%ARCHIVEMANAGER%' and schemaname = UPPER('");
        ...
        try
        {
            std::auto_ptr< IData > data( databaseConnection->executeQuery( sql, columns ) );
            if ( 0 == data.get() )
            {

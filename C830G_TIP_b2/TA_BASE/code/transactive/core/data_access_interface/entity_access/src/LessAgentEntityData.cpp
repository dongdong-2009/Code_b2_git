/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/entity_access/src/LessAgentEntityData.cpp $
...
namespace TA_Base_Core
{
	...
	unsigned long LessAgentEntityData::getLessOpcServerRateInMs()
	{
		...
		unsigned long tmpRate = EntityHelper::getUnsignedLongData(value);
		if( tmpRate < 500 )
		{
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugWarn, "Invalid rate \
				%lu continuing to use %lu", tmpRate, lessOpcServerRateInMs );
		}
		else
		{

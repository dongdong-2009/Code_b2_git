...
namespace TA_Base_Core
{
    ...
    void EntityHelper::assignDefaultParameterValueData( std::string parameterName, std::string parameterValue)
    {
        FUNCTION_ENTRY("EntityHelper::assignDefaultParameterValueData");
...
#ifdef TDS_DEVELOP
					sql += " values (" + databaseConnection->getNextPkey("entityparametervalue_seq") + ",";
#else
    ...
    void EntityHelper::writeEntityData()
    {
        ...
        {
                TA_THROW( TA_Base_Core::DataConfigurationException("Entity data not fully specified. Entity cannot be written to database"
                                                                ,fieldNames) );
        }
    ...
    void EntityHelper::addNewEntity()
    {
		...
		seq_name << m_location << "_seq";
        formatSQL << "values ( " << databaseConnection->getNextPkey(seq_name.str()) << ",'";
#else
    ...
    void EntityHelper::modifyAgentKeys(unsigned long newAgentKey)
    {
        ...
        databaseConnection->executeProcedure( sqlFunctionCall.str() );
    ...
    void EntityHelper::checkForChildEntities()
    {
        ...
        // want to list them.
        IData* data = databaseConnection->executeQuery(sql.str(),columnNames,11);
        ...
        if (data->getNumRows() > 0)
        {
            ...
            TA_THROW( DataException("This entity cannot be deleted as it has child entities.",
	...
	void EntityHelper::writeEntityStatusData(EEntityStatusType statustype, const std::string& newValue)
	 {		 
		...
		{				
			...
			// execute the query
			IData* existingData = databaseConnection->executeQuery(sql.str(),columnNames);
	...
	double EntityHelper::getRealData ( const std::string & parameterValue )
	{
		...
		std::string::size_type otherDash = parameterValue.find_first_of("-",firstDash+1);
		...
		std::string::size_type otherPoint = parameterValue.find_first_of(".",firstPoint+1);

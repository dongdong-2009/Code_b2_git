/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/entity_access/src/ConfigEntity.cpp $
  * @author Nick Jardine
  * @version $Revision: #1 $
...
namespace TA_Base_Core
{
    ...
    std::map<std::string, std::string> ConfigEntity::loadEnumerationValues(const std::string& validation, const bool isMandatory)
    {
        ...
        else
        {
            ...
            // We are responsible for deleting the returned IData object when we're done with it
            IData* data = databaseConnection->executeQuery(validation,columnNames);

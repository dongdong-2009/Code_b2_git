    ...
    ConfigJobData::getAllJobsOfAnyType(const std::string& jobType)
{
    ...
    try
    {
        ...
        do {
            ...
            // Execute the query
            //
            data = database->executeQuery(sql.str(), columnNames, SimpleDb::MAX_ROWS);
    ...
    {
        for(JobDataAccessFactory::IConfigJobDataVector::iterator it =
                jobs.begin();
            it != jobs.end();
            it ++)
        {
...
void ConfigJobData::createJobInDB()
{
    ...
    //
    std::string description = database->escapeAQSQLString(m_description); // Merge from 4677_ALL_TIP
    std::string jobType     = database->escapeAQSQLString(m_jobType); // Merge from 4677_ALL_TIP
    ...
    std::string parameters =
        database->escapeAQSQLString(getParametersAsString()); // Merge from 4677_ALL_TIP
    ...
    database->executeModification(insertSql.str());
...
void ConfigJobData::getDataFromRow(IData* data, unsigned int rowNum)
{
    ...
    IJobData::ParameterMap parameters = getParametersAsMap(parametersString);

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/alarm_rule/src/AlarmRuleHelper.cpp $
  * @author:  Dirk McCormick
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2010/01/25 17:30:39 $
...
namespace TA_Base_Core
{
    ...
    std::string AlarmRuleHelper::getEntityTypeName(unsigned long entityTypeKey)
    {
        ...
        if(entityTypeKey != 0)
        {
            ...
            //                  sql.str(), columnNames);
			IData* data = DatabaseFactory::getInstance().getDatabase(AlarmRules_Cd,Write)->executeQuery(
                              sql.str(), columnNames);
			// ++Yanrong
    ...
    void AlarmRuleHelper::createNewAlarmRule()
    {
        ...
        if(m_alarmTypeKey == 0)
        {
            getPkeyUsingAllParameters << "alarmtype IS NULL AND ";
        }
        else
        {
        ...
        {
            getPkeyUsingAllParameters << "alarmdescription IS NULL";
        }
        else
        {
        ...
        IData* data = databaseConnection->executeQuery(
                          getPkeyUsingAllParameters.str(),
                          columnNames);
		...
		{
            sql << ", alarmdescription";
		}

        if(m_createdBySessionKey.empty() == false)
        {
            sql << ", created_by_session_key";
			sql << ", created_by_profile_key";
			sql << ", created_by_operator_name";
        }
		...
		if (m_alarmDescription.empty() == false)
		{
            sql << ", '" << m_alarmDescription << "' ";
		}

        if(m_createdBySessionKey.empty() == false)
        {
            sql << ", '" << m_createdBySessionKey << "' ";
			sql << ", " << m_createdByProfileKey << " ";
			sql << ", '" << m_createdByOperatorName << "' ";
        }
    ...
    void AlarmRuleHelper::updateAlarmRule()
    {
        ...
        if(m_alarmDescription.empty() == true)
        {
            getPkeyUsingAllParameters << "alarmdescription IS NULL AND ";
        }
        else
        {
		...
		if (m_alarmDescription.empty() == false)
		{
			sql << ", alarmdescription = '" << m_alarmDescription << "' ";
		}
		sql	<< "WHERE pkey = " << m_pkey;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/plans/src/EventPlanAssocHelper.cpp $
  * @author:  Brad Cleaver
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    std::string EventPlanAssocHelper::retrieveEntityTypeName( unsigned long entityTypeKey )
    {
        ...
        if ( 0 != entityTypeKey )
        {
            ...
            // Execute it and ensure that only one row was returned.
            std::auto_ptr<IData> data( DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd, Read)->executeQuery( sql.str(), columnNames ) );
            if ( 1 == data->getNumRows() )
            {
    ...
    void EventPlanAssocHelper::addNew()
    {
        ...
        // This is documented in the comment of this method.
        // We are responsible for deleting the returned IData object when we're done with it
        data = std::auto_ptr<IData>( databaseConnection->executeQuery( formatSQL.str(), columnNames ) );
        if ( 0 == data->getNumRows() )
        {

    ...
    IArchiveInventory::ArchiveInventoryItem ArchiveInventoryData::getArchiveInventoryItem( unsigned long inventoryItemKey )
    {
        ...
        if ( it == m_inventory.end() )
        {
            ...
            TA_THROW( TA_Base_Core::DataException( error.str().c_str(), 
                ...
                TA_ArchiveManagerDAI::INVENTORY_KEY_COLUMN ) );
        }            
...
//         sql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
//         sql << "', '" << dateFormatStr << "' )";
		sql << "where " << TA_ArchiveManagerDAI::DATA_DATE_COLUMN << " >= ";
        sql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
        sql << "', '" << dateFormatStr << "' ) AND ";
        ...
        std::auto_ptr< IData > data( DatabaseFactory::getInstance().getDatabase(Archive_d, Write)->executeQuery( sql.str(), columnNames ) );
        if ( 0 == data.get() )
        {
            TA_THROW( DataException( "Failed to load the data for the ArchiveInventory.",
    ...
    void ArchiveInventoryData::removeArchiveInventoryItemFromDatabase( unsigned long itemKey )
    {
        ...
        DatabaseFactory::getInstance().getDatabase(Archive_d, Write)->executeModification( sql.str() );

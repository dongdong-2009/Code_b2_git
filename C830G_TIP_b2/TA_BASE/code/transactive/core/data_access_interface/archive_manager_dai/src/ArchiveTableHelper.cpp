...
namespace TA_Base_Core
{
    ...
    bool ArchiveTableHelper::isDateInQueryColumn( const time_t& date )
    {
		...
		selectSql << "select count( * ) from " << databaseConnection->escapeQueryString(m_name) << " ";
        selectSql << "where " << databaseConnection->escapeQueryString(m_queryColumn) << " >= ";
        selectSql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
        selectSql << "', '" << dateFormatStr << "' ) AND ";
		selectSql << databaseConnection->escapeQueryString(m_queryColumn) << " < ";
		selectSql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
        selectSql << "', '" << dateFormatStr << "' ) +1 ";
		// change the sql where clause from: WHERE [col_name] LIKE TO_DATE( '20130922', 'YYYYMMDD' )
		// to :WHERE [col_name] >= TO_DATE( '20130922', 'YYYYMMDD' ) AND [col_name] < TO_DATE( '20130922', 'YYYYMMDD' )+1
        ...
        if ( 1 != data->getNumRows() )     // This is a new ArchiveTable.
        {
            ...
            TA_THROW( DataException( error.str().c_str(), DataException::NOT_UNIQUE, 
                TA_ArchiveManagerDAI::QUERY_COLUMN ) );
        }
    ...
    void ArchiveTableHelper::deleteRestoredDataForDate( const time_t& date )
    {
		...
		deleteSql << "delete from " << databaseConnection->escapeQueryString(m_name) << " where ";
        deleteSql << databaseConnection->escapeQueryString(m_queryColumn) << " >= ";
        deleteSql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
        deleteSql << "', '" << dateFormatStr << "' )  AND ";
		deleteSql << databaseConnection->escapeQueryString(m_queryColumn) << " < ";
		deleteSql << "TO_DATE( '" << TA_ArchiveManagerDai::convertTimeTToStringDate( date );
        deleteSql << "', '" << dateFormatStr << "' ) +1 ";
        deleteSql << "and rownum < " << MAX_ROWS_TO_DELETE_AT_ONCE;
		// ++ TD12474
    ...
    void ArchiveTableHelper::insertNewArchiveTable()
    {
		...
		std::auto_ptr< IData > data( databaseConnection->executeQuery( getKeySql.str(), columnNames ) );
        if ( 0 == data.get() )
        {
        ...
        databaseConnection->executeModification( insertSql.str() );

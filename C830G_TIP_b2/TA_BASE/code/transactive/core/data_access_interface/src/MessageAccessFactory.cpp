 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/MessageAccessFactory.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Core
{
	...
	)
    {
        ...
        {
            //sql += "(select name from operator where pkey = 0)";
            // Fix for TD2576 - removed the above line, and replace with a space
            sql += "' '";
        }
        else
        {
        ...
        const unsigned long maxEvents /*= 0*/)
    {
        ...
        {
            sql << " AND rownum <= " << ((maxEvents == 0) ? MAX_ROWS : maxEvents); 
        }
        ...
        LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"SQL statement is ", sql.str().c_str()); 

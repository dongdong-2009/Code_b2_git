 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/DbConnectionStrings.cpp $
 * @author:  R.Stagg
 * @version: $Revision: #3 $
 *
 * Last modification: $DateTime: 2011/04/12 16:54:51 $
...
//		return dbIter->second;
    ...
    void DbConnectionStrings::ParseFile()
    {
        ...
        do
        {
            ...
            if ( values.size() < 5 ) // min required is datatype,dataaction,dbname,dbuser,dbpass
            {
            ...
            if ( (values.size() -5 ) % 3 != 0 ) // any remaing values must be connection triples
            {
                //throw; // something OR just log it ??
                LOG ( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugDebug,
            ...
            while (values.size())
            {
                if( !values[0].empty() )
				{
					connections.push_back( std::make_pair(values[0], values[0] + ":" + values[1] + ":" + values[2]) );
				}
                values.erase(values.begin());
                values.erase(values.begin());
                values.erase(values.begin());

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/ActionAccessFactory.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
	...
	void ActionAccessFactory::getActionBySql(const std::string& sql, std::vector<IAction*>& actions)
    {
    ...
    IAction* ActionAccessFactory::getAction( const unsigned long actionKey )
    {
		...
		getActionBySql(sql.str(), actions);
...
//limin++ for centralize duty/rights
	...
	void ActionAccessFactory::getNonPhysicalSubsystemActionGroup(const std::vector<unsigned long>& profileList, std::multimap<unsigned long, unsigned long>& groupIdList)
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugTrace, "SQL sentence: %s", sql.str().c_str());
        // Execute the query. The method can throw a DatabaseException.
        // This is documented in the comment of this method.
        // We are responsible for deleting the returned IData object when we're done with it
        IData* data = databaseConnection->executeQuery( sql.str(), columnNames );

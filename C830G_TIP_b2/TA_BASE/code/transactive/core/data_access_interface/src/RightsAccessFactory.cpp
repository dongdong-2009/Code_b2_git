/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/RightsAccessFactory.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    void RightsAccessFactory::getRightBySql(const std::string& sql, std::vector< IRights* >& rights, const bool readWrite)
    {
    ...
    std::vector<IRights*> RightsAccessFactory::getAllRights(const bool readWrite)
    {
		...
		getRightBySql(sql.str(), rights, readWrite);
		...
		unsigned long subsystemStateKey )	
    {
        ...
        if (data == NULL || 0 == data->getNumRows())
        {
            ...
            TA_THROW( DataException( "ActionGroup not found",DataException::NO_VALUE, sql.str().c_str() ) );
        }

...
/**
  ...
  */
	...
	void NamingPort::loadDataFromDb()
	{
        ...
        try
        {
            databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(Naming_Cd, Read);
        }
        catch ( const TA_Base_Core::DatabaseException& )
        {
            // TODO: temporary solution, should remove this catch after the new connection string files deployed to site
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "Cannot find data type Naming_Cd, using Event_Ad for table naming_ports" );
            databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(Event_Ad, Read);
        }
		...
		data = databaseConnection->executeQuery(querySql.str(), columnNames);

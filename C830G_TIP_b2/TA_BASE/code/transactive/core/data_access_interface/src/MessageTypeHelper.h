  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/MessageTypeHelper.h $
  * @author:  Nick Jardine
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
	...
	public:
		 ...
		 */
		MessageTypeHelper(const std::string& key): m_isValidData(false),m_key(key),m_name(""),m_description(""),m_priority(10),
		...
		MessageTypeHelper(const std::string& key,
            ...
            : m_isValidData(true),m_key(key),m_name(name),m_description(description),m_priority(priority),
            m_toBePrinted(toBePrinted),m_toBeLogged(toBeLogged),m_toBePersisted(toBePersisted), m_dateCreated(0),
            m_dateModified(0), m_viewLevel(viewLevel) {};

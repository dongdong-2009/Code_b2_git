/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/SimpleDbData.cpp $
  * @author Nick Jardine
  * @version $Revision: #3 $
...
//  For downloading CPU usage
//     Using DebugUtil::DebugTrace to replace all DebugUtil::FuntionEntry and DebugUtil::FuntionExit 
//  Modified by: LUO HUI RONG
...
namespace TA_Base_Core
{
	...
	timeb SimpleDbData::getTimestampData(const unsigned long rowIndex, const std::string& fieldName)
	{
		...
		// string
		// and get something invalid
		if ((17 != target.size()) || (std::string::npos != numberTest))
		{
			char reasonMessage[256] = {0};
	...
	double SimpleDbData::getRealData(const unsigned long rowIndex, const std::string& fieldName, const double defaultValue)
	{
		...
		size_t validCharacters = target.find_first_not_of("0123456789-.");				// locates any invalid characters in this string
		...
		else if (firstDash != otherDash && std::string::npos != otherDash) // there is more than one dash
		{
			char reasonMessage[256] = {0};
		...
		else if (std::string::npos != firstDash && 0 != firstDash) // the only dash is not in the first location
		{
			char reasonMessage[256] = {0};
		...
		else if (std::string::npos != firstPoint && ((0 == firstPoint) || (std::string::npos != firstDash && 1 == firstPoint))) // the decimal point is in an ivalid location (either the first position, or the second if there is a dash)
		{
			char reasonMessage[256] = {0};

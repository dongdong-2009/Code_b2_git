 ...
 * in any form.
 *
 * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/EventColourHelper.cpp $
 * @author Derrick Liew
 * @version $Revision: #1 $
    ...
    void EventColourHelper::applyChanges()
    {
        ...
        if( !fieldNames.empty() )
        {
            ...
            TA_THROW(TA_Base_Core::DataConfigurationException(
                        "Event colour data not fully specified. "
                        "Event colour cannot be written to database",
                        fieldNames));
        }
        ...
        databaseConnectionWrite->executeModification(sql.str());
        ...
        // This is documented in the comment of this method.
        // We are responsible for deleting the returned IData object when we're done with it
        data = databaseConnectionWrite->executeQuery(sql.str(),columnNames);

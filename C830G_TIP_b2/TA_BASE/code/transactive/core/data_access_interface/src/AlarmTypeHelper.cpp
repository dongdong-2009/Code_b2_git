  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/AlarmTypeHelper.cpp $
  * @author:  Nick Jardine
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2010/01/25 17:02:37 $
	...
	*/
    ...
    void AlarmTypeHelper::reload()
    {
        ...
        sql << "select a.meatyp_id, a.type_name, a.memcon_id, mc.context_name, nvl(a.type_description,'NULL') as type_description, a.measev_id, a.print_alarm, a.system_alarm, a.isvisible, "
			...
			//  [8/27/2009 ++caiwei] CL-18279
            //<< "a.persist_alarm, a.alarm_value, a.normal_value, a.mms_state, TO_CHAR(a.date_modified,'YYYYMMDDHH24MISS'), TO_CHAR(a.date_created,'YYYYMMDDHH24MISS'), a.phsical_alarm from "
    ...
    void AlarmTypeHelper::reloadUsing(const unsigned long row, TA_Base_Core::IData& data)
    {
        ...
        m_contextKey = data.getUnsignedLongData(row, columnNames[2]);
        m_contextName = data.getStringData(row, columnNames[3]);
        
		m_description = data.getStringData(row, columnNames[4]);
		m_description = ("NULL" == m_description) ? "" : m_description;

        m_severityKey = data.getUnsignedLongData(row, columnNames[5]);
        m_toBePrinted = (1 == data.getIntegerData(row, columnNames[6])) ? true : false;
		m_isSystemAlarm = data.getBooleanData(row, columnNames[7]);
		
		// Set the isVisible indicator
		m_isVisible = (data.getIntegerData(row, columnNames[8]) == 0)? false : true;
        m_toBePersisted = (1 == data.getIntegerData(row, columnNames[9])) ? true : false;
		m_alarmValue  = data.getStringData(row, columnNames[10]);
		m_normalValue = data.getStringData(row, columnNames[11]);
		//TD17628 marc_bugfix support mms_state
		m_MmsStateType = (IAlarmData::EMmsState)data.getIntegerData(row, columnNames[12]);
        m_dateModified = data.getDateData(row, columnNames[13], 0);
        m_dateCreated = data.getDateData(row, columnNames[14]);
		m_isPhysicalAlarm = data.getBooleanData(row, columnNames[15]); //20449++

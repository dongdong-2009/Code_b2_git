/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/GlobalParameterHelper.cpp $
  * @author Karen Graham
  * @version $Revision: #1 $
...
namespace TA_Base_Core
{
    ...
    void GlobalParameterHelper::deleteThisGlobalParameter()
    {
        ...
        DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd, Write)->executeModification(sql.str());
    ...
    void GlobalParameterHelper::addNewGlobalParameter()
    {
        ...
        // This is documented in the comment of this method.
        // We are responsible for deleting the returned IData object when we're done with it
        IData* data = DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd, Read)->executeQuery(sql.str(),columnNames);
        ...
        //throw(TA_Base_Core::TransactiveException)
    {
        ...
        if (m_isNew)
        {
            ...
            // execute the query
            IData* data = DatabaseFactory::getInstance().getDatabase(GlobalParameter_Cd, Read)->executeQuery(sql.str(),columnNames);

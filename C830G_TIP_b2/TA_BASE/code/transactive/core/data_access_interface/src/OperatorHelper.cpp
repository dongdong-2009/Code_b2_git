...
namespace TA_Base_Core
{
    ...
    {       
        ...
        // get a connection to the database
        IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(Plans_Cd, Write);
    ...
    void OperatorHelper::modifyExistingOperator()
    {
        ...
        std::string formatSQL = "select PKEY from operator_V where NAME = '"
        ...
        if (1 == data->getNumRows()) // The name was found - need to check if it was for this pkey
        {
            ...
            {   // A name already exists that is the same as the currently specified name
                ...
                throw DataException(reasonMessage.str().c_str(),DataException::NOT_UNIQUE,"name and description");
            }
        ...
        std::ostringstream sql;
        sql << "update OPERATOR_V set NAME = '";

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/RegionAccessFactory.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    void RegionAccessFactory::getRegionBySql(const std::string& sql, std::vector< IRegion* >& regions, const bool readWrite)
    {
    ...
    std::vector<IRegion*> RegionAccessFactory::getAllRegions(const bool readWrite)
    {
		...
		getRegionBySql(sql.str(), regions, readWrite);
	...
	std::vector<IRegion*> RegionAccessFactory::getRegionsByLocation( const unsigned long locationKey )
	{
		...
		std::vector< IRegion* > regions;
		getRegionBySql(sql.str(), regions, true);

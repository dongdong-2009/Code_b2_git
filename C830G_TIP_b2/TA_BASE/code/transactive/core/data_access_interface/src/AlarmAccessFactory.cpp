 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/src/AlarmAccessFactory.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #2 $
 *
...
// 		return alarms[0];
	...
	unsigned int AlarmAccessFactory::getDBActiveAlarmCount()
	{
		...
		try
		{
			...
			{
				TA_ASSERT(false, "There is more than 1 row returned by the count SQL statement");
			}
		}
    ...
    void AlarmAccessFactory::runQueryForAlarms(const std::string& query, std::vector<IAlarmData*>& alarms)
    {
		// get a connection to the database
        IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(Alarm_Sd, Read);
        ...
        IData* data = databaseConnection->executeQuery( query, columnNames);

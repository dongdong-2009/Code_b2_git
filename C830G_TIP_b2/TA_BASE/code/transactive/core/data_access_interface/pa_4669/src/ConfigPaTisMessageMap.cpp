/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/pa_4669/src/ConfigPaTisMessageMap.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
...
namespace TA_Base_Core
{
    ...
    void ConfigPaTisMessageMap::setLocationKey(const unsigned long locationKey)
    {
        FUNCTION_ENTRY("setLocationKey");
        
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
        
        updatePaTisMessageMapChanges(LOCATIONKEY, 
                                    isNew() ? 0 : m_paTisMessageMapHelper->getLocationKey(), 
                                    locationKey);

        m_paTisMessageMapHelper->setLocationKey(locationKey);

        FUNCTION_EXIT;
    }
    

    void ConfigPaTisMessageMap::setId(const unsigned long id)
    {
        ...
        updatePaTisMessageMapChanges(ID, 
                                    isNew() ? 0 : m_paTisMessageMapHelper->getId(), 
                                    id);

        m_paTisMessageMapHelper->setId(id);

        FUNCTION_EXIT;
    }

    
    void ConfigPaTisMessageMap::setLabel(const std::string& label)
    {
        FUNCTION_ENTRY("setLabel");
        
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
    
        updatePaTisMessageMapChanges(LABEL, 
                                    isNew() ? "" : m_paTisMessageMapHelper->getLabel(), 
                                    label);

        m_paTisMessageMapHelper->setLabel(label);

        FUNCTION_EXIT;
    }


    void ConfigPaTisMessageMap::setType(const std::string& type)
    {
        FUNCTION_ENTRY("setType");
        
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
    
        updatePaTisMessageMapChanges(TYPE, 
                                     isNew() ? "" : m_paTisMessageMapHelper->getType(), 
                                     type);

        m_paTisMessageMapHelper->setType(type);
    ...
    unsigned long ConfigPaTisMessageMap::getLocationKey()
    {
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
        return m_paTisMessageMapHelper->getLocationKey();
    }

    unsigned long ConfigPaTisMessageMap::getId()
    {
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
        return m_paTisMessageMapHelper->getId();
    }

    std::string ConfigPaTisMessageMap::getLabel()
    {
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");
        return m_paTisMessageMapHelper->getLabel();
    }    
    
    std::string ConfigPaTisMessageMap::getType()
    {
        TA_ASSERT(m_paTisMessageMapHelper != NULL,"The PaTisMessageMapHelper pointer is null.");

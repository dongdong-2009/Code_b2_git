...
#include "core/data_access_interface/pa_4669/src/ConfigPaTrainDvaMessage.h"
...
namespace TA_Base_Core
{
    ...
    static const std::string LIBRARY_VERSION_COL_NAME           = "LIBRARY_VERSION";
    ...
    unsigned long PaTrainDvaMessageAccessFactory::getLibraryVersion()
    {
        ...
        sql << "select " << LIBRARY_VERSION_COL_NAME << " from (select * from " << PA_TRAIN_DVA_MESSAGE_TABLE_NAME << ") where rownum <= 1";

        // Set up the columnNames vector to be passed to executeQuery()
        std::vector<std::string> columnNames;
        columnNames.push_back(LIBRARY_VERSION_COL_NAME);
        ...
        unsigned long libraryVersion= data->getUnsignedLongData(0, LIBRARY_VERSION_COL_NAME);
    ...
    void PaTrainDvaMessageAccessFactory::setLibraryVersion(unsigned long libraryVersion)
    {
        ...
        formatSQL << "update " << PA_TRAIN_DVA_MESSAGE_TABLE_NAME << " set " << LIBRARY_VERSION_COL_NAME << " = "
                  << libraryVersion << "');";
        formatSQL << "END;";

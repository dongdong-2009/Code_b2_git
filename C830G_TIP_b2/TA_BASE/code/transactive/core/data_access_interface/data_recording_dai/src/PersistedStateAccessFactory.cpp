 ...
 * in any form.
 *
 * Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/data_access_interface/data_recording_dai/src/PersistedStateAccessFactory.cpp $
 * @author:  Jade Welton
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Core
{
    ...
    std::map<unsigned long, PersistedStatePtr> PersistedStateAccessFactory::getPersistedStatesOfTypeWhere( unsigned long typeKey, const std::string& whereSql )
    {
        ...
        if ( ! properties.empty() )
        {
            ...
            TA_Base_Core::IData* data = databaseConnection->executeQuery( sql.str().c_str(), columnNames );
            if ( data != NULL )
            {
                do
                {

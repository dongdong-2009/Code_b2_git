...
* in any form.
*
* Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/database/src/SimpleDbStatusMonitor.cpp $
* @author:  Ripple
* @version: $Revision: #1 $
*
...
namespace TA_Base_Core
{
	...
	SimpleDbStatusMonitor::SimpleDbStatusMonitor(const char* connection, 
		const char* user, 
		const char* passwd,
		const char* hostname)
		: SimpleDb(connection, user, passwd, hostname)
	{
		...
		OCIAccessStatusMonitor::getInstance().createConnection(m_connectionStr, connection, user, passwd, hostname, false );

...
* in any form.
*
* Source:   $File: //depot/4669_Review_Branch/TA_BASE/transactive/core/database/src/OCIConnection.cpp $
* @author:  Ripple
* @version: $Revision: #6 $
*
* Last modification: $DateTime: 2011/06/07 10:08:42 $
...
namespace TA_Base_Core
{
	...
	{
        {
            ...
            for ( std::map<QueryAdapter*, long>::iterator itLoop = m_mapActiveQuery.begin();
                    m_mapActiveQuery.end() != itLoop; ++itLoop )
            {
                try
                {
                    ...
                    {
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "Unexpected NULL pointer of QueryAdapter" );
                        continue;
                    }
                    ...
                    {
                        delete (itLoop->first);
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugTrace, "QueryAdapter: 0x%p is deleted by ConnectionReaper in destructor of OCIConnection", itLoop->first );
                    }
	...
	*/
    ...
    void OCIConnection::exec( const std::string& statement, QueryAdapter*& pQueryAdapter, bool isQuery /*= false*/, int prefetch /*= 10*/)
	{
		FUNCTION_ENTRY("exec");
		...
		try
		{
			query = new OraQuery(); // Wrap it to QueryAdapter
...
//				}
            ...
            else //like the insert command
			{
				...
				query->setCommandText(statement);
        ...
        {
            ThreadGuard guard( m_queryLock );
            pQueryAdapter = new QueryAdapter( query );
            m_mapActiveQuery[pQueryAdapter] = QUERY_EXIST;
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugTrace, "QueryAdapter: 0x%p is created for connection: 0x%p", pQueryAdapter, this );
	...
	void OCIConnection::updateBLOB(const std::string &strSQL, 
		const std::string &colomnName,
...
// 	}
    ...
    void OCIConnection::cleanQuery( QueryAdapter*& pQuery )
    {
        ...
        std::map<QueryAdapter*, long>::iterator itExistQuery = m_mapActiveQuery.find( pQuery );
        if ( m_mapActiveQuery.end() != itExistQuery )
        {
            ...
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugTrace, "QueryAdapter: 0x%p is deleted by SimpleDb", pQuery );
        ...
        else
        {
            ...
            {
                delete pQuery;
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugTrace, "QueryAdapter: 0x%p is deleted by SimpleDb through it has been reaped", pQuery );
            }

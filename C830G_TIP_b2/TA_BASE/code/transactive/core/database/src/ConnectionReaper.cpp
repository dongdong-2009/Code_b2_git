...
* in any form.
*
* Source:   $File: //depot/4669_T01271350/TA_BASE/transactive/core/database/src/ConnectionReaper.cpp $
* @author:  Ripple
* @version: $Revision: #2 $
*
...
* 120 minutes.
	...
	void ConnectionReaper::run()
	{
		while( !m_isTerminating )
		{
            try
            {
			    ...
			    OCIAccess::getInstance().refreshConnections();
			    ...
			    while ( !m_waitingQueue.empty() )
			    {
				    ...
				    std::vector< OCIConnection* >::iterator it = m_monitoredConnections.begin();
				    int i = 1;
	...
	*/
	...
	void ConnectionReaper::terminate()
	{
		...
		// Delete any left over OCIConnection objects from the queue.
		{
			...
			std::vector<OCIConnection*>::iterator it = m_waitingQueue.begin();
	...
	*/
	...
	void ConnectionReaper::reapConnection( OCIConnection* connection )
	{
		if ( !connection )
		{
			return;
	...
	*/
	...
	void ConnectionReaper::deleteUnusedConnections()
	{
		...
		std::vector< OCIConnection* >::iterator it = m_monitoredConnections.begin();
		std::vector< OCIConnection* >::iterator find_it;
		while ( it != m_monitoredConnections.end() )
		{

...
namespace TA_IRS_App
{
    ...
    public:
		...
		/**
         *	This check if the Message is sent previously. If the same message is sent earlier, it will return true
			else it will note the current structure and return false.
         *
         * @param event
         */
		bool isMessageSentPreviously(TA_IRS_Bus::PaTypes::TrainLibraryInfo &pa_info);
		bool isMessageSentPreviously(TA_IRS_Bus::TtisTypes::TisLibraryVersionUpdate &tis_info);
    private:
	   ...
	   /**
		* Bohong++, for Cl-20889. We seperate the initialization steps into 3 funtions, then we can choose what we want 
		* to do as our wish
		*/
		void closeTrainCctv( TA_IRS_Bus::CommonTypes::TrainIdType trainId );
		
		void closeTrainPec( TA_IRS_Bus::CommonTypes::TrainIdType trainId );

		void closeIncommingCall( TA_IRS_Bus::CommonTypes::TrainIdType trainId );
		//++Bohong
		...
		map<TA_IRS_Bus::CommonTypes::TrainIdType,TA_IRS_Bus::PaTypes::TrainLibraryInfo> m_trainidtoPALibraryMap;
		map<TA_IRS_Bus::CommonTypes::TrainIdType,TA_IRS_Bus::TtisTypes::TisLibraryVersionUpdate> m_trainidtoTisLibraryUpdateMap;

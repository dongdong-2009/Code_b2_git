...
namespace TA_IRS_App
{
                ...
                if ( true == commsState->hasPeristedStateChanged() )
                {
                    ...
                    try
                    {
                        TA_IRS_Core::PersistedTrainStateAccessFactory::saveTrainState( commsState->getTrainId(),
																					   ...
																					   commsState->getNextTtisLibraryVersion() ,
																					   commsState->getNextTtisScheduleVersion() ,
                                                         ...
                                                         unsigned long senderEntity )
    {
        ...
        unsigned short ttisLibraryVersion = state->getTtisLibraryVersion();
        unsigned short ttisScheduleVersion = state->getTtisScheduleVersion();
		unsigned short nextTtisLibraryVersion = state->getNextTtisLibraryVersion();
		unsigned short nextTtisScheduleVersion = state->getNextTtisScheduleVersion();
        ...
        if ( ( state->getTtisLibraryVersion() != ttisLibraryVersion ) ||
			 ...
			 ( state->getNextTtisLibraryVersion() != nextTtisLibraryVersion ) ||
			 ( state->getNextTtisScheduleVersion() != nextTtisScheduleVersion) )
        {
			...
			tisLibraryInfo.nextlibraryVersion = state->getNextTtisLibraryVersion();
			tisLibraryInfo.nextscheduleVersion = state->getNextTtisScheduleVersion();
    ...
    void StateUpdateManager::syncGlobalStates()
    {
             ...
             ( TA_Base_Core::ILocation::DPT != m_configuration.getAgentLocationType() ) )
        {
            try
            {
                TA_IRS_Bus::StateUpdateTypes::FullGlobalState fullState = m_depotPeer->getGlobalState();
    ...
    //limin++, CL-20779
    void StateUpdateManager::notifyOperationStateChange( TA_Base_Core::EOperationMode operationState )
    {
        FUNCTION_ENTRY( "notifyOperationStateChange" );

        if ( TA_Base_Core::Control == operationState )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "limin: control mode, unsubscribed train state update message" );

            m_localPeer->removeObserver( this );
        }
        else if ( TA_Base_Core::Monitor == operationState )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "limin: monitor mode, subscribed train state update message" );

            m_localPeer->addObserver( this );
        }
        else
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "limin: unknown mode, unsubscribed train state update message" );

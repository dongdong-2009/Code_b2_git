/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ats/ATSSimulatorLibrary/src/ATSSimCommsObserver.cpp $
...
ATSSimCommsObserver::ATSSimCommsObserver
...
ATSSimCommsObserver::~ATSSimCommsObserver()
{
    FUNCTION_ENTRY("~ATSSimCommsObserver");
    if( m_clientSocket != 0 )
    {
...
/**
  ...
  */
void ATSSimCommsObserver::processReceivedData( std::vector<unsigned char> & pData, TA_Base_Core::ISocket * pSocket )
{
    ...
    while ( keep_processing )
    {
        ...
        if ( ( functionCode == ATSSim_FC_Read ) || ( functionCode == ATSSim_FC_Write ) )
        {
            ...
            if ( ( functionCode == ATSSim_FC_Read ) && ( pData.size() >= 8 ) )
            {
                query = processReceivedRead(iter);
            }
            else if ( functionCode == ATSSim_FC_Write )
            {
                if ( pData.size() > 5 )
                {
...
ModbusQuery * ATSSimCommsObserver::processReceivedRead(std::vector<unsigned char>::iterator & iter)
{
...
ModbusQuery * ATSSimCommsObserver::processReceivedWrite(std::vector<unsigned char>::iterator & iter)
{
...
//writeFailed
//
void ATSSimCommsObserver::writeFailed()
{
...
//connectionLost
//
bool ATSSimCommsObserver::connectionLost()
{
...
//
void ATSSimCommsObserver::connectionEstablished()
{
    m_stillConnected = true;
}


std::string ATSSimCommsObserver::getClientName() const
{
    //

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ats/ATSSimulatorLibrary/cmd/SetTwpCommand.cpp $
...
bool SetTwpCommand::makeQuery(TokenList & args, const std::string & argstr, UserQuery * query, LocationType location, std::ostream & err)
{
    ...
    TableIdentifier tableId = AtsWash;
    //
    // Train Wash Plant table only valid at DEPOT
    if ( location != DEPOT )
    {
        err << "ERROR: Ats Train Wash Plant table only valid at DEPOT" << std::endl;
        return false;
    }
    ...
    while ( argiter != args.end() )
    {
        ...
        if (parameters.size() == 2)
        {
            ...
            if (field != 0xFFFF)
            {
                UserQuery::FieldValueType value = ATSSimUtility::string2bool(valuestr);

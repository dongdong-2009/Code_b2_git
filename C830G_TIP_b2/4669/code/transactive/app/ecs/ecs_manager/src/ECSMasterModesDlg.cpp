/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ecs_manager/src/ECSMasterModesDlg.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
#define WM_ADD_STATUSLINE							(WM_APP+7)
#define WM_SET_LOCATIONS							(WM_APP+8)
#define WM_SET_MASTERMODEDETAILS					(WM_APP+9)

using namespace TA_Base_Core;

namespace TA_IRS_App
{
    const int MINIMUM_DIALOG_WIDTH              = 400;
    const int MINIMUM_DIALOG_HEIGHT             = 350; 
    const int NORMAL_START_HEIGHT               = 392;
    const int EMERGENCY_START_HEIGHT            = 500; 
    const int EMERGENCY_FOR_ZONE_START_HEIGHT   = 385; 
    const int NORMAL_START_WIDTH                = 460; 
    const int EMERGENCY_START_WIDTH             = 1050;
    ...
    void ECSMasterModesDlg::setMasterModeDetails( const MasterModeDetails& masterModeDetails )
    {
		...
		WPARAM wParam = reinterpret_cast<WPARAM>(new MasterModeDetails(masterModeDetails));
		PostMessage(WM_SET_MASTERMODEDETAILS, wParam, 0);
	...
	BEGIN_MESSAGE_MAP( ECSMasterModesDlg, TransActiveDialog )
		...
		ON_MESSAGE(WM_SET_MASTERMODEDETAILS, onSetMasterModeDetails)
    ...
    // OnDestroy
    //
    void ECSMasterModesDlg::OnDestroy() 
    {
	...
	void ECSMasterModesDlg::run()
	{
		onInitialise();
	}

	void ECSMasterModesDlg::terminate()
	{

	}

	LRESULT ECSMasterModesDlg::onAddStatusLine( WPARAM wParam, LPARAM lParam )
	{
	...
	LRESULT ECSMasterModesDlg::onSetLocations( WPARAM wParam, LPARAM lParam )
	{
        ...
        m_masterModesGridCtrl.setLocations( *pLocationNames );
		delete pLocationNames;
		return 0;
	}

	LRESULT ECSMasterModesDlg::onSetMasterModeDetails( WPARAM wParam, LPARAM lParam )
	{
		MasterModeDetails* pMasterModeDetails = reinterpret_cast<MasterModeDetails*>(wParam);
		if(NULL == pMasterModeDetails)
		{
			return 0;
		}
        m_masterModesGridCtrl.populateMasterModeDetails( *pMasterModeDetails );
		delete pMasterModeDetails;
		return 0;
	}

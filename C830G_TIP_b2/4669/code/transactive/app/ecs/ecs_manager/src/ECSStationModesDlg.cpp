/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ecs_manager/src/ECSStationModesDlg.cpp $
  * @author:  Robert Young
  * @version: $Revision: #6 $
  *
  * Last modification: $DateTime: 2009/07/17 12:17:48 $
...
#define WM_ENABLE_LOCATIONSELECTIONCONTROL      (WM_APP+11)
#define WM_SET_COLOURMAP                        (WM_APP+12)
#define WM_SET_INBOUNDLOCATION                  (WM_APP+13)
#define WM_SET_LOCATIONS                        (WM_APP+14)
#define WM_SET_OUTBOUNDLOCATION                 (WM_APP+15)
#define WM_SET_SELECTEDLOCATIONINDEX            (WM_APP+16)
#define WM_SET_STATIONMODEDETAILS               (WM_APP+17)
...
{
const int MINIMUM_DIALOG_WIDTH              = 365;
const int MINIMUM_DIALOG_HEIGHT             = 335;
...
const int OPERATION_WIDTH                   = 450;
const int TRACK_WIDTH                       = 200;
const int CONDITION_WIDTH                   = 460;
...
BEGIN_MESSAGE_MAP( ECSStationModesDlg, TransActiveDialog )
    ...
    ON_MESSAGE( WM_ENABLE_LOCATIONSELECTIONCONTROL, onEnableLocationSelectionControl )
    ON_MESSAGE( WM_SET_COLOURMAP, onSetColourMap )
    ON_MESSAGE( WM_SET_INBOUNDLOCATION, onSetInboundLocation )
    ON_MESSAGE( WM_SET_LOCATIONS, onSetLocations )
    ON_MESSAGE( WM_SET_OUTBOUNDLOCATION, onSetOutboundLocation )
    ON_MESSAGE( WM_SET_SELECTEDLOCATIONINDEX, onSetSelectedLocationIndex )
    ON_MESSAGE( WM_SET_STATIONMODEDETAILS, onSetStationModeDetails )
...
BOOL ECSStationModesDlg::OnInitDialog()
{
    ...
    rp.maxWidth = -1;
    rp.maxHeight = -1;
    rp.minWidth = MINIMUM_DIALOG_WIDTH;
    rp.minHeight = MINIMUM_DIALOG_HEIGHT;
...
void ECSStationModesDlg::setLocations( const LocationNames& locations )
{
    ...
    WPARAM wParam = reinterpret_cast<WPARAM>( new LocationNames( locations ) );
...
void ECSStationModesDlg::setSelectedLocationIndex( unsigned int selectedLocationIndex )
{
    ...
    PostMessage( WM_SET_SELECTEDLOCATIONINDEX, static_cast<WPARAM>( selectedLocationIndex ) );
...
void ECSStationModesDlg::enableLocationSelectionControl( bool doEnableLocationSelectionControl )
{
    ...
    PostMessage( WM_ENABLE_LOCATIONSELECTIONCONTROL, doEnableLocationSelectionControl ? 1 : 0, 0 );
...
void ECSStationModesDlg::setStationModeDetails( const StationModeDetails& stationModeDetails )
{
    ...
    WPARAM wParam = reinterpret_cast<WPARAM>( new StationModeDetails( stationModeDetails ) );
    PostMessage( WM_SET_STATIONMODEDETAILS, wParam, 0 );
...
void  ECSStationModesDlg::setInboundLocation( const std::string& innerboundLocation )
{
    ...
    WPARAM wParam = reinterpret_cast<WPARAM>( new std::string( innerboundLocation ) );
    PostMessage( WM_SET_INBOUNDLOCATION, wParam, 0 );
...
void  ECSStationModesDlg::setOutboundLocation( const std::string& outerboundLocation )
{
    ...
    WPARAM wParam = reinterpret_cast<WPARAM>( new std::string( outerboundLocation ) );
    PostMessage( WM_SET_OUTBOUNDLOCATION, wParam, 0 );
...
{
    PostMessage( WM_SET_COLOURMAP, static_cast<WPARAM>( index ), static_cast<LPARAM>( colour ) );
}
...
// OnDestroy
//
void ECSStationModesDlg::OnDestroy()
{
...
void ECSStationModesDlg::run()
{
    onInitialise();
}

void ECSStationModesDlg::terminate()
{

}

LRESULT ECSStationModesDlg::onAddStatusLine( WPARAM wParam, LPARAM lParam )
{
    ...
    std::string statusText = *pStatusText;
...
LRESULT ECSStationModesDlg::onEnableLocationSelectionControl( WPARAM wParam, LPARAM lParam )
{
    m_locationSelectionCombo.EnableWindow( wParam > 0 ? TRUE : FALSE );
    return 0;
}

LRESULT ECSStationModesDlg::onSetColourMap( WPARAM wParam, LPARAM lParam )
{
    m_locationSelectionCombo.MapIndexToColour( static_cast<unsigned int>( wParam ), static_cast<COLORREF>( lParam ) );
    return 0;
}

LRESULT ECSStationModesDlg::onSetInboundLocation( WPARAM wParam, LPARAM lParam )
{
    std::string* pInnerboundLocation = reinterpret_cast<std::string*>( wParam );
    if( NULL == pInnerboundLocation )
    {
        return 0;
    }
    m_inboundLocationDisplay.SetWindowText( pInnerboundLocation->c_str() );
    delete pInnerboundLocation;
    return 0;
}

LRESULT ECSStationModesDlg::onSetLocations( WPARAM wParam, LPARAM lParam )
{
    ...
    const LocationNames& locations = *pLocationNames;
    //
...
LRESULT ECSStationModesDlg::onSetOutboundLocation( WPARAM wParam, LPARAM lParam )
{
    std::string* pOutboundLocation = reinterpret_cast<std::string*>( wParam );
    if( NULL == pOutboundLocation )
    {
        return 0;
    }
    m_outboundLocationDisplay.SetWindowText( pOutboundLocation->c_str() );
    delete pOutboundLocation;
    return 0;
}

LRESULT ECSStationModesDlg::onSetSelectedLocationIndex( WPARAM wParam, LPARAM lParam )
{
    m_locationSelectionCombo.SetCurSel( static_cast<unsigned int>( wParam ) );
    return 0;
}

LRESULT ECSStationModesDlg::onSetStationModeDetails( WPARAM wParam, LPARAM lParam )
{
    StationModeDetails* pStationModeDetails = reinterpret_cast<StationModeDetails*>( wParam );
    if( NULL == pStationModeDetails )
    {
        return 0;
    }
    m_stationModeSelectionListCtrl.populateStationModeDetails( *pStationModeDetails );
    delete pStationModeDetails;
    return 0;
}

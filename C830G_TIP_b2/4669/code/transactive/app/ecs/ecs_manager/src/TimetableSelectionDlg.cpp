/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_Review_Branch/4669/transactive/app/ecs/ecs_manager/src/TimetableSelectionDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #6 $
  *
  * Last modification: $DateTime: 2011/05/30 15:58:21 $
...
namespace TA_IRS_App
{
	...
	void CTimetableSelectionDlg::OnDestroy()
	{
        TA_Base_Core::RunParams::getInstance().deregisterRunParamUser(this);
		CRightsMgrSingleton::deleteRightsMgr();

        m_currentTimetableDataPoint.reset();
		m_selectTimetableDataPoint.reset();
		m_stationVisualLockDataPoint.reset();
	...
	void CTimetableSelectionDlg::SaveChanges()
	{
		...
		TA_Base_Bus::RightsLibrary *rightsMgr = CRightsMgrSingleton::getRightsMgr();
	...
	{				
        ...
        try
        {
            ...
            else
            {
				...
				m_proxyFactory->createDataPointProxy(dpEntityData, *this, m_currentTimetableDataPoint);
		    }
            ...
            else
            {
                ...
                m_proxyFactory->createDataPointProxy(dpEntityData, *this, m_selectTimetableDataPoint);
            }
            ...
            else
            {
                ...
                m_proxyFactory->createDataPointProxy(dpEntityData, *this, m_stationVisualLockDataPoint);
            }
        }                
        catch (TA_Base_Core::ScadaProxyException &e)
        {
	...
	void CTimetableSelectionDlg::OnAppHelp() 
	{
		TA_Base_Bus::HelpLauncher::getInstance().displayHelp();
	}
    
    LRESULT CTimetableSelectionDlg::OnEntityUpdateMessage(WPARAM wParam, LPARAM lParam)
    {
        
        unsigned long entityKey = lParam; 
        int updateType = wParam;
        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "OnEntityUpdateMessage, entitykey [%u], updateType [%d], currentEntityKey [%u], selectEntityKey[%u].", entityKey, updateType, m_currentTimetableDataPoint->getEntityKey(), m_selectTimetableDataPoint->getEntityKey());
        //ThreadGuard guard(m_mapLock);
        try
        {
        	if (entityKey == m_currentTimetableDataPoint->getEntityKey() &&
                 ( updateType == TA_Base_Bus::ConfigAvailable || updateType == TA_Base_Bus::ValueStateUpdate ))
            {
        ...
        try
        {
            if (entityKey == m_selectTimetableDataPoint->getEntityKey())
            {
                ...
                {   
                    ...
                    //empty combo list
				    m_NewTimetableCombo.ResetContent();
                    ...
                    for (TimetableStateMap::iterator it = m_selectTimetableStateMap.begin();
					     ...
					     ++ i, ++ it)
                    {
                        int nInserted = m_NewTimetableCombo.InsertString(i, it->second.c_str());
                        m_NewTimetableCombo.SetItemData(nInserted, it->first);

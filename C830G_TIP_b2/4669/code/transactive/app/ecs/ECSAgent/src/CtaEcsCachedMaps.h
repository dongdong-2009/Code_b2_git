/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ECSAgent/src/CtaEcsCachedMaps.h $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
...
public:
    ...
    virtual void refreshZoneRecords(const std::vector<unsigned long>& zoneIds);
    ...
    virtual void validateAllInitialisedMaps(); 
   ...
   /**
    ...
    virtual unsigned long getZoneIdFromMasterMode(/*unsigned*/ long masterMode);	//dahe++ for CL-21046

    virtual MasterModePair getMasterModePairFromMasterMode(/*unsigned*/ long masterMode);	//dahe++ for CL-21046

    virtual TA_IRS_Bus::EcStationModeKeys getEcStationModeKeysFromMasterMode(/*unsigned*/ long masterMode);	//dahe++ for CL-21046
...
protected:
    ...
    typedef std::map<void*, bool>
        MemberPointerToBoolMap;

    
    bool isMemberInitialised(void* member)
    {
        return (m_isMemberToInitialised[member]==true);
    }

    void setMemberInitialised(void* member)
    {
        m_isMemberToInitialised[member]=true;
    }
    ...
    void prevalidateSingleZoneRecord(const TA_IRS_Bus::EcZoneRecord& record);

    void prevalidateSingleStationModeRecord(const TA_IRS_Bus::EcStationModeRecord& record);
    ...
    // Used to check initialised status of the above map members
    MemberPointerToBoolMap                      m_isMemberToInitialised;
    ...
    std::string                                 m_localModeSqlToken;
    ...
    typedef std::multimap<EItaEcsCachedMapsItemKey, ItaEcsCachedMapsObserver*> 
        KeyToObserversMap;
    typedef KeyToObserversMap::iterator                                 
        KeyToObserversMapIt;

    typedef KeyToObserversMap::value_type                               
        key2observer;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ECSAgent/src/CtaEcsAgentCompleteState.cpp $
  * @author:  Ripple
  * @version: $Revision: #4 $
  *
...
void CtaEcsAgentCompleteState::processEcsAgentStateUpdateRequest()
{
	...
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugFatal, "Exception thrown while processing event");
	}
...
void CtaEcsAgentCompleteState::clearProposedOverridingStationModes(unsigned long locationKey)
{
    ...
    while (it != m_proposedModeSet.end())
    {
        ...
        {
            m_proposedModeSet.erase(it++); //CL-21326 wzy, Fix ECSAgent Crash sometimes
            m_hasProposedModesChangedSinceCommit = true;
        }
		else
		{   

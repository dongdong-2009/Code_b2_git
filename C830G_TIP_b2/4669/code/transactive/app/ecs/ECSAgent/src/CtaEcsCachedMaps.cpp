/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/ecs/ECSAgent/src/CtaEcsCachedMaps.cpp $
  * @author:  seanl
  * @version: $Revision: #3 $
  *
...
unsigned long CtaEcsCachedMaps::getZoneIdFromMasterMode(/*unsigned*/ long masterMode)	//dahe++ for CL-21046
{
	...
	//dahe++ for CL-21046
	// if masterMode <= 0, skip the following code and throw exception
	if(masterMode > 0)
	{
		// We have to iterate through the map to find the key so we can detect when if the key
		// is absent.  If we juest rely on referencing the map via operator[], we can only tell
...
MasterModePair CtaEcsCachedMaps::getMasterModePairFromMasterMode(/*unsigned*/ long masterMode)	//dahe++ for CL-21046
{
    return getMasterModesFromZoneId(getZoneIdFromMasterMode(masterMode));
}


TA_IRS_Bus::EcStationModeKeys CtaEcsCachedMaps::getEcStationModeKeysFromMasterMode(/*unsigned*/ long masterMode)	//dahe++ for CL-21046
{

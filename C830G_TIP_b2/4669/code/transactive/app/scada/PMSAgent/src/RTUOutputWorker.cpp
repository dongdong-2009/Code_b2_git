...
namespace TA_IRS_App
{
	...
	void RTUOutputWorker::processEvent( TA_Base_Bus::DataPointWriteRequest * newDataPointWriteRequest )
	{
		...
		{// to check write request.
			...
			dp->stopControlTimeOut();
			if (dp->isWriteRequestExpired(newDataPointWriteRequest->getCommandId()))
			{
				LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Write Request[%d] has Expired for %s", 
					newDataPointWriteRequest->getCommandId(),
					newDataPointWriteRequest->getDataPoint()->getDataPointName().c_str() );
				FUNCTION_EXIT;
				return;
			}

			if (false == dp->executeLaunchCondition(
				newDataPointWriteRequest->getSessionID(),
				newDataPointWriteRequest->getEngValue()) )
			{
				LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "failed to Check the Launching Condition for %s", newDataPointWriteRequest->getDataPoint()->getDataPointName().c_str() );
				FUNCTION_EXIT;
				return;
			}
		}

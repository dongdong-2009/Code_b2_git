/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/ats_power_entity_map/src/AtsPowerEntityMapListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the AtsPowerEntityMapDetailView as this will show the details of the item selected in this view.
  */
...
typedef TA_IRS_App::AtsPowerEntityMapDatabaseAccessor::AtsPowerEntityMapRecord AtsPowerEntityMapRecord;

using namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    const int       AtsPowerEntityMapListView::APPLIED_COLUMN						= 0;
	const int       AtsPowerEntityMapListView::MSSZONE_COLUMN						= 1;
    const int       AtsPowerEntityMapListView::LOCATION_COLUMN						= 2;
    const int       AtsPowerEntityMapListView::SUBSECTION_COLUMN					= 3;
	const int       AtsPowerEntityMapListView::DPNAME_COLUMN						= 4;

    const CString   AtsPowerEntityMapListView::APPLIED_COLUMN_NAME					= "Status";
	const CString   AtsPowerEntityMapListView::MSSZONE_COLUMN_NAME					= "MSS Sector";
    const CString   AtsPowerEntityMapListView::LOCATION_COLUMN_NAME					= "Location";
    const CString   AtsPowerEntityMapListView::SUBSECTION_COLUMN_NAME				= "Subsection";
	const CString   AtsPowerEntityMapListView::DPNAME_COLUMN_NAME					= "DataPoint";

    const int       AtsPowerEntityMapListView::APPLIED_COLUMN_WIDTH					= 75;
    const int       AtsPowerEntityMapListView::MSSZONE_COLUMN_WIDTH					= 80;
    const int       AtsPowerEntityMapListView::LOCATION_COLUMN_WIDTH				= 80;
    const int       AtsPowerEntityMapListView::SUBSECTION_COLUMN_WIDTH				= 80;
    const int       AtsPowerEntityMapListView::DPNAME_COLUMN_WIDTH					= 300;

    // These are the possible values for the status of each ATS PSD Entity Map
    const CString AtsPowerEntityMapListView::NEW = "New";
    const CString AtsPowerEntityMapListView::MODIFIED = "Modified";
    const CString AtsPowerEntityMapListView::UPTODATE = "";

    const bool AtsPowerEntityMapListView::ASCENDING = true;
    const bool AtsPowerEntityMapListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(AtsPowerEntityMapListView, CListView)


    AtsPowerEntityMapListView::AtsPowerEntityMapListView()
    ...
    {
        m_componentIcon = LoadIcon(GetModuleHandle(AtsPowerEntityMapComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_ATS) );
    }
...
BEGIN_MESSAGE_MAP(AtsPowerEntityMapListView, AbstractListView)
	//{{AFX_MSG_MAP(AtsPowerEntityMapListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int AtsPowerEntityMapListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void AtsPowerEntityMapListView::OnInitialUpdate() 
    {
        ...
        GetListCtrl().InsertColumn( SUBSECTION_COLUMN,			SUBSECTION_COLUMN_NAME,			LVCFMT_LEFT, SUBSECTION_COLUMN_WIDTH    );
		GetListCtrl().InsertColumn( DPNAME_COLUMN,				DPNAME_COLUMN_NAME,				LVCFMT_LEFT, DPNAME_COLUMN_WIDTH		);
    }


    void AtsPowerEntityMapListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void AtsPowerEntityMapListView::loadAllItems()
    {
        ...
        AtsPowerEntityMapDatabaseAccessor* dbAccessor = dynamic_cast<AtsPowerEntityMapDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long,AtsPowerEntityMapRecord>::iterator iter = m_currentDisplayedItems.begin(); 
             iter != m_currentDisplayedItems.end(); ++iter)
        {
            ...
            GetListCtrl().SetItemText(position, SUBSECTION_COLUMN,		iter->second.subsection);
    ...
    void AtsPowerEntityMapListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        AtsPowerEntityMapRecord details = dbAccessor->getItemDetails( id );
        ...
        if (foundItemIndex != -1)
        {
            ...
            GetListCtrl().SetItemText(foundItemIndex, SUBSECTION_COLUMN,		details.subsection);
        ...
        else
        {
            ...
            GetListCtrl().SetItemText(position, SUBSECTION_COLUMN,		details.subsection);
        ...
        std::map<unsigned long,AtsPowerEntityMapRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,AtsPowerEntityMapRecord>::value_type(id,details) );
        }
    ...
    void AtsPowerEntityMapListView::copyItem()
    {
    ...
    void AtsPowerEntityMapListView::newItem()
    {
...
/*    void AtsPowerEntityMapListView::deleteItem()
    {
    ...
    void AtsPowerEntityMapListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void AtsPowerEntityMapListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(AtsPowerEntityMapComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK AtsPowerEntityMapListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        AtsPowerEntityMapListView* list = reinterpret_cast<AtsPowerEntityMapListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<AtsPowerEntityMapListView*>(list);
        ...
        std::map<unsigned long,AtsPowerEntityMapRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,AtsPowerEntityMapRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
        ...
        switch(list->m_sortColumn)
        {
            ...
            case(SUBSECTION_COLUMN):
                    value1 = atoi((LPCTSTR)item1->second.subsection);
                    value2 = atoi((LPCTSTR)item2->second.subsection);
    ...
    void AtsPowerEntityMapListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // AtsPowerEntityMapListView diagnostics

    #ifdef _DEBUG
    void AtsPowerEntityMapListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void AtsPowerEntityMapListView::Dump(CDumpContext& dc) const
    {

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/src/MessageSender.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
#include "app/administration/configuration/config_plugins/generic_entity/src/StdAfx.h"
#include "app/administration/configuration/config_plugins/src/MessageSender.h"
#include "app/administration/configuration/configuration_editor/src/ConfigurationEditorConstants.h"
...
namespace TA_IRS_App
{
    ...
    MessageSender::~MessageSender()
    {
        ...
        try
        {
            ...
            {
                TA_Core::MessagePublicationManager::getInstance().cleanUpSuppliers();
            }
        }
        catch ( ... )
        {
    ...
    void MessageSender::setUpMessageSenders()
    {
        ...
        using TA_Core::MessagePublicationManager;
        ...
        if( (m_auditMessageSender == NULL) && (!RunParams::getInstance().get(RPARAM_SESSIONID).empty()) )
        {
            ...
            m_auditMessageSender = MessagePublicationManager::getInstance().getAuditMessageSender( TA_Core::Configuration::Context );
        ...
        if( (m_configMessageSender == NULL) && (!RunParams::getInstance().get(RPARAM_SESSIONID).empty()) )
        {
            ...
            m_configMessageSender = MessagePublicationManager::getInstance().getConfigUpdateMessageSender( TA_Core::ConfigUpdate::Context );
    ...
    void MessageSender::sendAuditMessage(const UpdateDetails& updateDetails)
    {
        ...
        TA_Core::DescriptionParameters description;
        description.push_back( new TA_Core::NameValuePair("ConfigurationItemType", RunParams::getInstance().get(RPARAM_COMPONENT.c_str())) );
        description.push_back( new TA_Core::NameValuePair("Item", updateDetails.nameOfChangedItem) );

        std::string details("");
        if (updateDetails.isNewItem)
        {
            // This is a new item so we need to send an ItemAdded audit message.
            m_auditMessageSender->sendAuditMessage(TA_Core::Configuration::ConfigurationItemAdded,
        ...
        if (updateDetails.isDelete)
        {
            ...
            m_auditMessageSender->sendAuditMessage(TA_Core::Configuration::ConfigurationItemDeleted,
							                       m_configurationEditorEntityKey,
							                       description,
							                       details,
                                                   TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID),
        ...
        if (!updateDetails.isNewItem && !updateDetails.isDelete)
        {
            ...
            m_auditMessageSender->sendAuditMessage(TA_Core::Configuration::ConfigurationItemUpdated,
							                       m_configurationEditorEntityKey,
							                       description,
                                                   details,
                                                   TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID),
    ...
    void MessageSender::sendOnlineUpdateMessage(const UpdateDetails& updateDetails)
    {
        ...
        TA_Core::EModificationType configUpdateType;

        if (updateDetails.isNewItem)
        {
            // This is a new item so we need to send config update Create message
            configUpdateType = TA_Core::Create;
        }
        ...
        {
            case(ENTITY):
                m_configMessageSender->sendConfigUpdateMessage(TA_Core::ConfigUpdate::ConfigEntityOwner,
            ...
            case(OPERATOR):
                m_configMessageSender->sendConfigUpdateMessage(TA_Core::ConfigUpdate::ConfigOperator,
                                                               updateDetails.keyOfChangedItem,
                                                               configUpdateType,
                                                               changes);
                break;

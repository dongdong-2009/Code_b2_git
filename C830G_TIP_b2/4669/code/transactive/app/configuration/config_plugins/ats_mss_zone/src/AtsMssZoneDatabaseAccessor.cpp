/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/ats_mss_zone/src/AtsMssZoneDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
{
    AtsMssZoneDatabaseAccessor::AtsMssZoneDatabaseAccessor()
        : m_isLoaded(false)
    {
        m_configItems.clear();
    }
    ...
    void AtsMssZoneDatabaseAccessor::loadItems()
    {
        ...
        if (m_isLoaded)
        {
            ...
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "We have already loaded the Ats MSS Zones and this is not a refresh so we exit");
            FUNCTION_EXIT;
            return;
        }
        ...
        // Now reload all the Ats MSS Zones
        IAtsMssZones items = AtsMssZoneAccessFactory::getInstance().getAllAtsMssZones(true);

        // And insert the Ats MSS Zones into our map. At the same time cast them to IConfigAtsMssZone*
        // instead of IAtsMssZone*.
        for (IAtsMssZonesIt it = items.begin(); it != items.end(); ++it)
        {
            IAtsMssZone* item = *it;
            IConfigAtsMssZone* configItem = dynamic_cast<IConfigAtsMssZone*>(item);
            if( configItem != NULL )
            {
    ...
    std::map<CString,unsigned long> AtsMssZoneDatabaseAccessor::getItemNames(CProgressCtrl& progress)
    {
        TA_ASSERT(false, "This method cannot be called for Ats MSS Zones");
    ...
    std::map<unsigned long,AtsMssZoneDatabaseAccessor::AtsMssZoneRecord> AtsMssZoneDatabaseAccessor::getAllItemDetails(CProgressCtrl& progress)
    {
        FUNCTION_ENTRY("getAllItemDetails");

        std::map<unsigned long,AtsMssZoneRecord> details; 
        ...
        for (ConfigItemsIt it = m_configItems.begin(); it != m_configItems.end(); ++it)
        {
            if (it->second != NULL)
            {
                ...
                record.mssZone = it->second->getMssZone().c_str();
                
                details.insert( std::map<unsigned long, AtsMssZoneRecord>::value_type( it->second->getUniqueIdentifier(), record ) );
                progress.StepIt();
            }
        }
    ...
    AtsMssZoneDatabaseAccessor::AtsMssZoneRecord AtsMssZoneDatabaseAccessor::getItemDetails(unsigned long identifier)
    {
        FUNCTION_ENTRY("getItemDetails");

        // Attempt to find the Ats MSS Zone matching the key passed in.
        ConfigItemsIt matching = m_configItems.find(identifier);
        if (matching == m_configItems.end() )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Could not find the Ats MSS Zone with identifier %lu",identifier);
            FUNCTION_EXIT;
            AtsMssZoneRecord record;
            return record;
        }
        ...
        record.station = station.str().c_str();
        record.mssZone = matching->second->getMssZone().c_str();
    ...
    TA_Base_Core::IConfigItem* AtsMssZoneDatabaseAccessor::getItem(unsigned long key)
    {
        ...
        if (matching == m_configItems.end() )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Could not find the Ats MSS Zone with key %lu",key);
            FUNCTION_EXIT;
            return NULL;
        }
    ...
    void AtsMssZoneDatabaseAccessor::deleteItem(unsigned long key)
    {
        ...
        TA_ASSERT(matching->second != NULL, "Stored Ats MSS Zone was equal to NULL");
    ...
    TA_Base_Core::IConfigItem* AtsMssZoneDatabaseAccessor::newItem()
    {
        FUNCTION_ENTRY("newItem");

        TA_IRS_Core::IConfigAtsMssZone* newAssoc = TA_IRS_Core::AtsMssZoneAccessFactory::getInstance().createAtsMssZone();
    ...
    TA_Base_Core::IConfigItem* AtsMssZoneDatabaseAccessor::copyItem(const unsigned long idOfItemToCopy)
    {
        FUNCTION_ENTRY("copyItem");

        TA_IRS_Core::IConfigAtsMssZone* newAssoc = NULL;
        ...
        TA_ASSERT(matching->second != NULL, "There was a NULL ConfigAtsMssZone stored in the map");

        newAssoc = TA_IRS_Core::AtsMssZoneAccessFactory::getInstance().copyAtsMssZone(matching->second);
    ...
    bool AtsMssZoneDatabaseAccessor::areCurrentChangesPending(std::vector<std::string>& itemsNotApplied)
    {
        ...
        for (ConfigItemsIt it = m_configItems.begin(); it != m_configItems.end();  ++it)
        {
            if( (it->second != NULL) && (it->second->hasChanged() || it->second->isNew()) )
            {
                ...
                // individual change for Ats MSS Zones

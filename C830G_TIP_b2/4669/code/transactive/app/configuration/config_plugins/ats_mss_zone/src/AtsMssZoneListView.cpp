/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/ats_mss_zone/src/AtsMssZoneListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * 
  * This view contains a list of all Ats MSS Zones available to be configured. This view must communicate
  * with the AtsMssZoneDetailView as this will show the details of the item selected in this view.
  */
...
//using TA_IRS_App::AtsMssZoneDatabaseAccessor::AtsMssZoneRecord;
typedef TA_IRS_App::AtsMssZoneDatabaseAccessor::AtsMssZoneRecord AtsMssZoneRecord;

using namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    const int       AtsMssZoneListView::APPLIED_COLUMN                     = 0;
    const int       AtsMssZoneListView::STATION_COLUMN					   = 1;
    const int       AtsMssZoneListView::MSS_ZONE_COLUMN                    = 2;

    const CString   AtsMssZoneListView::APPLIED_COLUMN_NAME                = "Status";
    const CString   AtsMssZoneListView::STATION_COLUMN_NAME				   = "Station";
    const CString   AtsMssZoneListView::MSS_ZONE_COLUMN_NAME               = "MSS Sector";

    const int       AtsMssZoneListView::APPLIED_COLUMN_WIDTH               = 75;
    const int       AtsMssZoneListView::STATION_COLUMN_WIDTH			   = 120;
    const int       AtsMssZoneListView::MSS_ZONE_COLUMN_WIDTH              = 120;

    // These are the possible values for the status of each Ats MSS Zone
    const CString AtsMssZoneListView::NEW = "New";
    const CString AtsMssZoneListView::MODIFIED = "Modified";
    const CString AtsMssZoneListView::UPTODATE = "";

    const bool AtsMssZoneListView::ASCENDING = true;
    const bool AtsMssZoneListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(AtsMssZoneListView, CListView)


    AtsMssZoneListView::AtsMssZoneListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(STATION_COLUMN)  // This is the column to sort on when the list control gets created
    {
        m_componentIcon = LoadIcon(GetModuleHandle(AtsMssZoneComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_ATS) );
    }
...
BEGIN_MESSAGE_MAP(AtsMssZoneListView, AbstractListView)
	//{{AFX_MSG_MAP(AtsMssZoneListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int AtsMssZoneListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void AtsMssZoneListView::OnInitialUpdate() 
    {
        ...
        GetListCtrl().InsertColumn( MSS_ZONE_COLUMN,             MSS_ZONE_COLUMN_NAME,             LVCFMT_LEFT, MSS_ZONE_COLUMN_WIDTH             );
    }


    void AtsMssZoneListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void AtsMssZoneListView::loadAllItems()
    {
        ...
        AtsMssZoneDatabaseAccessor* dbAccessor = dynamic_cast<AtsMssZoneDatabaseAccessor*>(getDatabaseAccessor());
        TA_ASSERT( dbAccessor != NULL, "DB accessor could not be cast to an Ats MSS Zone one");
        ...
        for (std::map<unsigned long,AtsMssZoneRecord>::iterator iter = m_currentDisplayedItems.begin(); 
             iter != m_currentDisplayedItems.end(); ++iter)
        {
            ...
            GetListCtrl().SetItemText(position, MSS_ZONE_COLUMN,             iter->second.mssZone);
    ...
    void AtsMssZoneListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        AtsMssZoneRecord details = dbAccessor->getItemDetails( id );
        ...
        if (foundItemIndex != -1)
        {
            ...
            GetListCtrl().SetItemText(foundItemIndex, MSS_ZONE_COLUMN,             details.mssZone);
        ...
        else
        {
            ...
            GetListCtrl().SetItemText(position, MSS_ZONE_COLUMN,             details.mssZone);
        ...
        std::map<unsigned long,AtsMssZoneRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,AtsMssZoneRecord>::value_type(id,details) );
        }
    ...
    void AtsMssZoneListView::copyItem()
    {
        ...
        if (pos == NULL)
        {
            //UE-030059
			// TD14164 ++
            /*MessageBox("An item must be selected before a copy can be performed", TA_Base_Core::RunParams::getInstance().get(RPARAM_APPNAME).c_str(),MB_OK|MB_ICONEXCLAMATION);*/
			TA_Base_Bus::TransActiveMessage userMsg;
			CString action = "copy";
			userMsg << action;
		...
		for(int i=0; i<selectedCount; i++)
		{
			...
			catch ( const TA_Base_Core::DataException& )
			{
				...
				userMsg << reason;
				UINT selectedButton = userMsg.showMsgBox(IDS_UE_030069);				
				// ++ TD14164
			}
			catch ( const TA_Base_Core::DatabaseException& )
			{
    ...
    void AtsMssZoneListView::newItem()
    {
...
/*    void AtsMssZoneListView::deleteItem()
    {
    ...
    void AtsMssZoneListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void AtsMssZoneListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(AtsMssZoneComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK AtsMssZoneListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        AtsMssZoneListView* list = reinterpret_cast<AtsMssZoneListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<AtsMssZoneListView*>(list);
        ...
        std::map<unsigned long,AtsMssZoneRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,AtsMssZoneRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
    ...
    void AtsMssZoneListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // AtsMssZoneListView diagnostics

    #ifdef _DEBUG
    void AtsMssZoneListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void AtsMssZoneListView::Dump(CDumpContext& dc) const
    {

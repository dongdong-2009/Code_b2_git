/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/pa_station/src/PAStationListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the PaStationDetailView as this will show the details of the item selected in this view.
  */
...
namespace TA_IRS_App
{
    ...
    const CString PaStationListView::NEW = "New";
    const CString PaStationListView::MODIFIED = "Modified";
    const CString PaStationListView::UPTODATE = "";

    const bool PaStationListView::ASCENDING = true;
    const bool PaStationListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(PaStationListView, CListView)


    PaStationListView::PaStationListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PaStationListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(PaStationComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_STATIONS) );
    }
...
BEGIN_MESSAGE_MAP(PaStationListView, AbstractListView)
	//{{AFX_MSG_MAP(PaStationListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int PaStationListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void PaStationListView::OnInitialUpdate() 
    {
    ...
    void PaStationListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void PaStationListView::loadAllItems()
    {
        ...
        PaStationDatabaseAccessor* dbAccessor = dynamic_cast<PaStationDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long, PaStationDatabaseAccessor::PaStationRecord>::iterator iter 
    ...
    void PaStationListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        PaStationDatabaseAccessor::PaStationRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long, PaStationDatabaseAccessor::PaStationRecord>::iterator matching;
        ...
        else
        {
            m_currentDisplayedItems.insert( 
                std::map<unsigned long, PaStationDatabaseAccessor::PaStationRecord>::value_type(id, details) );
        }
    ...
    void PaStationListView::copyItem()
    {
    ...
    void PaStationListView::newItem()
    {
 ...
 /*   void PaStationListView::deleteItem()
    {
    ...
    void PaStationListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void PaStationListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(PaStationComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK PaStationListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        PaStationListView* list = reinterpret_cast<PaStationListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<PaStationListView*>(list);
        ...
        std::map<unsigned long, PaStationDatabaseAccessor::PaStationRecord>::iterator item1 
                                                        = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long, PaStationDatabaseAccessor::PaStationRecord>::iterator item2 
    ...
    void PaStationListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // PaStationListView diagnostics

    #ifdef _DEBUG
    void PaStationListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void PaStationListView::Dump(CDumpContext& dc) const
    {

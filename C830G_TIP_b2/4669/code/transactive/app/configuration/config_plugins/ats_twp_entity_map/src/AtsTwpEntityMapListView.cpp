/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/ats_twp_entity_map/src/AtsTwpEntityMapListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the AtsTwpEntityMapDetailView as this will show the details of the item selected in this view.
  */
...
//using TA_IRS_App::AtsTwpEntityMapDatabaseAccessor::AtsTwpEntityMapRecord;
typedef TA_IRS_App::AtsTwpEntityMapDatabaseAccessor::AtsTwpEntityMapRecord AtsTwpEntityMapRecord;

using namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    const int       AtsTwpEntityMapListView::APPLIED_COLUMN                     = 0;
    const int       AtsTwpEntityMapListView::COMMAND_COLUMN						= 1;
	const int       AtsTwpEntityMapListView::DPNAME_COLUMN						= 2;

    const CString   AtsTwpEntityMapListView::APPLIED_COLUMN_NAME                = "Status";
    const CString   AtsTwpEntityMapListView::COMMAND_COLUMN_NAME				= "Command";
	const CString   AtsTwpEntityMapListView::DPNAME_COLUMN_NAME					= "DataPoint";

    const int       AtsTwpEntityMapListView::APPLIED_COLUMN_WIDTH               = 75;
    const int       AtsTwpEntityMapListView::COMMAND_COLUMN_WIDTH				= 240;
    const int       AtsTwpEntityMapListView::DPNAME_COLUMN_WIDTH				= 300;

    // These are the possible values for the status of each ATS PSD Entity Map
    const CString AtsTwpEntityMapListView::NEW = "New";
    const CString AtsTwpEntityMapListView::MODIFIED = "Modified";
    const CString AtsTwpEntityMapListView::UPTODATE = "";

    const bool AtsTwpEntityMapListView::ASCENDING = true;
    const bool AtsTwpEntityMapListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(AtsTwpEntityMapListView, CListView)


    AtsTwpEntityMapListView::AtsTwpEntityMapListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(COMMAND_COLUMN)  // This is the column to sort on when the list control gets created
    {
        m_componentIcon = LoadIcon(GetModuleHandle(AtsTwpEntityMapComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_ATS) );
    }
...
BEGIN_MESSAGE_MAP(AtsTwpEntityMapListView, AbstractListView)
	//{{AFX_MSG_MAP(AtsTwpEntityMapListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int AtsTwpEntityMapListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void AtsTwpEntityMapListView::OnInitialUpdate() 
    {
        ...
        GetListCtrl().InsertColumn( COMMAND_COLUMN, COMMAND_COLUMN_NAME, LVCFMT_LEFT, COMMAND_COLUMN_WIDTH );
		GetListCtrl().InsertColumn( DPNAME_COLUMN,	DPNAME_COLUMN_NAME,	 LVCFMT_LEFT, DPNAME_COLUMN_WIDTH );
    }


    void AtsTwpEntityMapListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void AtsTwpEntityMapListView::loadAllItems()
    {
        ...
        AtsTwpEntityMapDatabaseAccessor* dbAccessor = dynamic_cast<AtsTwpEntityMapDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long,AtsTwpEntityMapRecord>::iterator iter = m_currentDisplayedItems.begin(); 
             iter != m_currentDisplayedItems.end(); ++iter)
        {
            ...
            GetListCtrl().SetItemText(position, COMMAND_COLUMN,			iter->second.command);
    ...
    void AtsTwpEntityMapListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        AtsTwpEntityMapRecord details = dbAccessor->getItemDetails( id );
        ...
        if (foundItemIndex != -1)
        {
            ...
            GetListCtrl().SetItemText(foundItemIndex, COMMAND_COLUMN,			details.command);
        ...
        else
        {
            ...
            GetListCtrl().SetItemText(position, COMMAND_COLUMN,			details.command);
        ...
        std::map<unsigned long,AtsTwpEntityMapRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,AtsTwpEntityMapRecord>::value_type(id,details) );
        }
    ...
    void AtsTwpEntityMapListView::copyItem()
    {
    ...
    void AtsTwpEntityMapListView::newItem()
    {
...
/*    void AtsTwpEntityMapListView::deleteItem()
    {
    ...
    void AtsTwpEntityMapListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void AtsTwpEntityMapListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(AtsTwpEntityMapComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK AtsTwpEntityMapListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        AtsTwpEntityMapListView* list = reinterpret_cast<AtsTwpEntityMapListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<AtsTwpEntityMapListView*>(list);
        ...
        std::map<unsigned long,AtsTwpEntityMapRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,AtsTwpEntityMapRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
        ...
        switch(list->m_sortColumn)
        {
            ...
            case(COMMAND_COLUMN):
                    string1 = item1->second.command;
                    string2 = item2->second.command;
                    returnVal = string1.CompareNoCase(string2);
                    break;
    ...
    void AtsTwpEntityMapListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // AtsTwpEntityMapListView diagnostics

    #ifdef _DEBUG
    void AtsTwpEntityMapListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void AtsTwpEntityMapListView::Dump(CDumpContext& dc) const
    {

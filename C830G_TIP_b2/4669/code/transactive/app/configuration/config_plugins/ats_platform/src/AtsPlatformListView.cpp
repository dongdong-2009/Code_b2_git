/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/ats_platform/src/AtsPlatformListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * 
  * This view contains a list of all Ats Platforms available to be configured. This view must communicate
  * with the AtsPlatformDetailView as this will show the details of the item selected in this view.
  */
...
//using TA_IRS_App::AtsPlatformDatabaseAccessor::AtsPlatformRecord;
typedef TA_IRS_App::AtsPlatformDatabaseAccessor::AtsPlatformRecord AtsPlatformRecord;

using namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    const int       AtsPlatformListView::APPLIED_COLUMN                     = 0;	
    const int       AtsPlatformListView::PLATFORMID_COLUMN                  = 1;
    const int       AtsPlatformListView::STATION_COLUMN					    = 2;
	const int       AtsPlatformListView::PLATFORMNAME_COLUMN                = 3;
    const int       AtsPlatformListView::TISPLATFORMID_COLUMN               = 4;

    const CString   AtsPlatformListView::APPLIED_COLUMN_NAME                = "Status";
	const CString   AtsPlatformListView::PLATFORMID_COLUMN_NAME             = "Platform Id";
    const CString   AtsPlatformListView::STATION_COLUMN_NAME				= "Station";
	const CString   AtsPlatformListView::PLATFORMNAME_COLUMN_NAME           = "Platform Name";
    const CString   AtsPlatformListView::TISPLATFORMID_COLUMN_NAME          = "Tis Platform Id";


    const int       AtsPlatformListView::APPLIED_COLUMN_WIDTH               = 75;
	const int       AtsPlatformListView::PLATFORMID_COLUMN_WIDTH            = 75;
    const int       AtsPlatformListView::STATION_COLUMN_WIDTH			    = 120;
	const int       AtsPlatformListView::PLATFORMNAME_COLUMN_WIDTH		    = 180;
    const int       AtsPlatformListView::TISPLATFORMID_COLUMN_WIDTH         = 90;
    

    // These are the possible values for the status of each Ats Platform
    const CString AtsPlatformListView::NEW = "New";
    const CString AtsPlatformListView::MODIFIED = "Modified";
    const CString AtsPlatformListView::UPTODATE = "";

    const bool AtsPlatformListView::ASCENDING = true;
    const bool AtsPlatformListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(AtsPlatformListView, CListView)


    AtsPlatformListView::AtsPlatformListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PLATFORMID_COLUMN)  // This is the column to sort on when the list control gets created
    {
        m_componentIcon = LoadIcon(GetModuleHandle(AtsPlatformComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_ATS) );
    }
...
BEGIN_MESSAGE_MAP(AtsPlatformListView, AbstractListView)
	//{{AFX_MSG_MAP(AtsPlatformListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int AtsPlatformListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void AtsPlatformListView::OnInitialUpdate() 
    {
        ...
        GetListCtrl().InsertColumn( PLATFORMID_COLUMN,           PLATFORMID_COLUMN_NAME,           LVCFMT_LEFT, PLATFORMID_COLUMN_WIDTH           );
        GetListCtrl().InsertColumn( STATION_COLUMN,				 STATION_COLUMN_NAME,              LVCFMT_LEFT, STATION_COLUMN_WIDTH			  );
        GetListCtrl().InsertColumn( PLATFORMNAME_COLUMN,         PLATFORMNAME_COLUMN_NAME,         LVCFMT_LEFT, PLATFORMNAME_COLUMN_WIDTH         );
        GetListCtrl().InsertColumn( TISPLATFORMID_COLUMN,        TISPLATFORMID_COLUMN_NAME,        LVCFMT_LEFT, TISPLATFORMID_COLUMN_WIDTH        );
    }


    void AtsPlatformListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void AtsPlatformListView::loadAllItems()
    {
        ...
        AtsPlatformDatabaseAccessor* dbAccessor = dynamic_cast<AtsPlatformDatabaseAccessor*>(getDatabaseAccessor());
        TA_ASSERT( dbAccessor != NULL, "DB accessor could not be cast to an Ats Platform one");
        ...
        for (std::map<unsigned long,AtsPlatformRecord>::iterator iter = m_currentDisplayedItems.begin(); 
             iter != m_currentDisplayedItems.end(); ++iter)
        {
			...
			GetListCtrl().SetItemText(position, PLATFORMID_COLUMN,           iter->second.platformId);
            GetListCtrl().SetItemText(position, STATION_COLUMN,              iter->second.station);	
			GetListCtrl().SetItemText(position, PLATFORMNAME_COLUMN,         iter->second.platformName);
            GetListCtrl().SetItemText(position, TISPLATFORMID_COLUMN,        iter->second.tisPlatformId);
    ...
    void AtsPlatformListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        AtsPlatformRecord details = dbAccessor->getItemDetails( id );
        ...
        if (foundItemIndex != -1)
        {
            ...
            GetListCtrl().SetItemText(foundItemIndex, PLATFORMID_COLUMN,           details.platformId);
            GetListCtrl().SetItemText(foundItemIndex, STATION_COLUMN,			   details.station);
            GetListCtrl().SetItemText(foundItemIndex, PLATFORMNAME_COLUMN,         details.platformName);
            GetListCtrl().SetItemText(foundItemIndex, TISPLATFORMID_COLUMN,        details.tisPlatformId);
        }
        else
        {
            // Add the new details
            ...
            GetListCtrl().SetItemText(position, PLATFORMID_COLUMN,           details.platformId);
            GetListCtrl().SetItemText(position, STATION_COLUMN,				 details.station);
            GetListCtrl().SetItemText(position, PLATFORMNAME_COLUMN,         details.platformName);
            GetListCtrl().SetItemText(position, TISPLATFORMID_COLUMN,        details.tisPlatformId);
        ...
        std::map<unsigned long,AtsPlatformRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,AtsPlatformRecord>::value_type(id,details) );
        }
    ...
    void AtsPlatformListView::copyItem()
    {
    ...
    void AtsPlatformListView::newItem()
    {
...
/*    void AtsPlatformListView::deleteItem()
    {
    ...
    void AtsPlatformListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void AtsPlatformListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(AtsPlatformComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK AtsPlatformListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        AtsPlatformListView* list = reinterpret_cast<AtsPlatformListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<AtsPlatformListView*>(list);
        ...
        std::map<unsigned long,AtsPlatformRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,AtsPlatformRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
        ...
        switch(list->m_sortColumn)
        {
			...
			case(PLATFORMID_COLUMN):
                    value1 = item1->second.platformIdValue;
                    value2 = item2->second.platformIdValue;
			...
			case(PLATFORMNAME_COLUMN):
                    string1 = item1->second.platformName;
                    string2 = item2->second.platformName;
                    returnVal = string1.CompareNoCase(string2);
                    break;

            case(TISPLATFORMID_COLUMN):
                    value1 = item1->second.tisPlatformIdValue;
                    value2 = item2->second.tisPlatformIdValue;
                    returnVal = value1 - value2;
                    break;
        }
    ...
    void AtsPlatformListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // AtsPlatformListView diagnostics

    #ifdef _DEBUG
    void AtsPlatformListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void AtsPlatformListView::Dump(CDumpContext& dc) const
    {

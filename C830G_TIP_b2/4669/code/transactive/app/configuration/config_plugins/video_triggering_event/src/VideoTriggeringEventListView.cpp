/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/configuration/config_plugins/video_triggering_event/src/VideoTriggeringEventListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the VideoTriggeringEventDetailView as this will show the details of the item selected in this view.
  */
...
namespace TA_IRS_App
{
    ...
    const CString VideoTriggeringEventListView::NEW = "New";
    const CString VideoTriggeringEventListView::MODIFIED = "Modified";
    const CString VideoTriggeringEventListView::UPTODATE = "";

    const bool VideoTriggeringEventListView::ASCENDING = true;
    const bool VideoTriggeringEventListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(VideoTriggeringEventListView, CListView)


    VideoTriggeringEventListView::VideoTriggeringEventListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(VideoTriggeringEventListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(VideoTriggeringEventComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_STATIONS) );
    }
...
BEGIN_MESSAGE_MAP(VideoTriggeringEventListView, AbstractListView)
	//{{AFX_MSG_MAP(VideoTriggeringEventListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int VideoTriggeringEventListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void VideoTriggeringEventListView::OnInitialUpdate() 
    {
    ...
    void VideoTriggeringEventListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void VideoTriggeringEventListView::loadAllItems()
    {
        ...
        VideoTriggeringEventDatabaseAccessor* dbAccessor = dynamic_cast<VideoTriggeringEventDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long, VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord>::iterator iter 
    ...
    void VideoTriggeringEventListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long, VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord>::iterator matching;
        ...
        else
        {
            m_currentDisplayedItems.insert( 
                std::map<unsigned long, VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord>::value_type(id, details) );
        }
    ...
    void VideoTriggeringEventListView::copyItem()
    {
    ...
    void VideoTriggeringEventListView::newItem()
    {
    ...
    /*
    void VideoTriggeringEventListView::deleteItem()
    {
    ...
    */
    ...
    void VideoTriggeringEventListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void VideoTriggeringEventListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(VideoTriggeringEventComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK VideoTriggeringEventListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        VideoTriggeringEventListView* list = reinterpret_cast<VideoTriggeringEventListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<VideoTriggeringEventListView*>(list);
        ...
        std::map<unsigned long, VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord>::iterator item1 
                                                        = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long, VideoTriggeringEventDatabaseAccessor::VideoTriggeringEventRecord>::iterator item2 
    ...
    void VideoTriggeringEventListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // VideoTriggeringEventListView diagnostics

    #ifdef _DEBUG
    void VideoTriggeringEventListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void VideoTriggeringEventListView::Dump(CDumpContext& dc) const
    {

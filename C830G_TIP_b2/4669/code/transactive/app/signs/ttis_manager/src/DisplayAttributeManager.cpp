/**
  ...
  *
  * Source:    $File: //depot/4669_T01271350/4669/transactive/app/signs/ttis_manager/src/DisplayAttributeManager.cpp $
  * @author:   Adam Radics
  * @version:  $Revision: #3 $
  *
...
namespace TA_IRS_App
{
    ...
    */
    ...
    void DisplayAttributeManager::dateTimeChanged(CallbackDateTimeCtrl* control)
    {
        ...
        {
		    CTimeSpan incTime(0,0,1,0);
            startTime+=incTime;
            m_startTime->SetTime(&startTime);
        }

		if (startTime.GetHour() < 3)
		{
			if(endTime < startTime || endTime.GetHour() >= 3)
			{
				endTime = startTime;
		...
		if (startTime.GetHour() >= 3)
		{
			if(!(endTime > startTime) && !(0 <= endTime.GetHour() && endTime.GetHour() <= 2))
			{
				endTime = startTime;
				m_endTime->SetTime(&endTime);
			}			

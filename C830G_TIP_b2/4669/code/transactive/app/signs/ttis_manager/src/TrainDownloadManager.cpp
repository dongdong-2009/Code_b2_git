/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/signs/ttis_manager/src/TrainDownloadManager.cpp $
  * @author:  Adam Radics
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    TrainDownloadManager::TrainDownloadManager(CWnd* window)
        : m_libraryDownloads(window),
          ...
          m_scheduleDownloads(window),
          m_scheduleUpgrades(window)
    {
    ...
    void TrainDownloadManager::startScheduleDownload(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList, bool retry)
    {
        FUNCTION_ENTRY( "startScheduleDownload" );

        m_scheduleDownloads.startDownload(trainList, retry);

        FUNCTION_EXIT;
    }


    void TrainDownloadManager::startScheduleUpgrade(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList, bool retry)
    {
        FUNCTION_ENTRY( "startScheduleUpgrade" );

        m_scheduleUpgrades.startDownload(trainList, retry);

        FUNCTION_EXIT;
    }


    void TrainDownloadManager::stopLibraryDownload(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList)
    {
        FUNCTION_ENTRY( "stopLibraryDownload" );

        m_libraryDownloads.stopRetrying(trainList);

        FUNCTION_EXIT;
    }


    void TrainDownloadManager::stopLibraryUpgrade(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList)
    {
        FUNCTION_ENTRY( "stopLibraryUpgrade" );

        m_libraryUpgrades.stopRetrying(trainList);

        FUNCTION_EXIT;
    }


    void TrainDownloadManager::stopScheduleDownload(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList)
    {
        FUNCTION_ENTRY( "stopScheduleDownload" );

        m_scheduleDownloads.stopRetrying(trainList);

        FUNCTION_EXIT;
    }


    void TrainDownloadManager::stopScheduleUpgrade(const TA_IRS_Bus::CommonTypes::TrainIdList& trainList)
    {
        FUNCTION_ENTRY( "stopScheduleUpgrade" );

        m_scheduleUpgrades.stopRetrying(trainList);
    ...
    void TrainDownloadManager::start()
    {
        ...
        m_scheduleDownloads.start();
        m_scheduleUpgrades.start();
    ...
    void TrainDownloadManager::terminateAndWait()
    {
        ...
        m_scheduleDownloads.terminateAndWait();
        m_scheduleUpgrades.terminateAndWait();
    ...
    void TrainDownloadManager::processDownloadChange(const TA_Base_Core::TrainDownloadStatus& downloadChange)
    {
        ...
        case TA_Base_Core::ScheduleDownloadStart:
            m_scheduleDownloads.processDownloadStart(downloadChange.trainNumber);
            break;
        case TA_Base_Core::ScheduleDownloadFinish:
            m_scheduleDownloads.processDownloadFinish(downloadChange.trainNumber,
                                                      downloadChange.success,
                                                      std::string(downloadChange.errorDetails));
            break;
        ...
        case TA_Base_Core::ScheduleUpgrade:
            m_scheduleUpgrades.processDownloadFinish(downloadChange.trainNumber,
                                                     downloadChange.success,
                                                     std::string(downloadChange.errorDetails));
            break;
        }

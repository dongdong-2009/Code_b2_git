...
#include "app/cctv/video_switch_agent/protocols/src/IPDStageModeInterface.h"  //++tianxiang Merge ECP++
...
#include "core/alarm/IDL/src/AlarmMessageCorbaDef.h" // TD15853
...
namespace TA_IRS_App
{
    ...
    void SwitchManager::acknowledgeTrainCameraAlarm( unsigned long locationId, unsigned long trainId, const TA_Base_Bus::SwitchManagerCorbaDef::TrainCameraSequence& cameraIds, unsigned long consoleId )
	{
        ...
        {
            throw TA_Base_Bus::VideoSwitchAgentException("No cameras to add to alarm stack");
        }
    ...
    void SwitchManager::closeAlarm(const TA_Base_Core::AlarmDetailCorbaDef& pAlarmDetail)
	{
		...
		if(m_switchAlarmInterface != NULL)
		{
			...
			{
				throw TA_Base_Bus::VideoSwitchAgentException("error closing alarm in switch");
			}
    ...
    void SwitchManager::closeAlarmWithEvent( unsigned long stationId, unsigned long eventId )
	{
		...
		{
			throw TA_Base_Bus::VideoSwitchAgentException("No Interface found to close alarm");
		}
	}
...
//++tianxiang Merge ECP++
    ...
    SwitchManager::SwitchManager()
        : m_switchAlarmInterface(NULL),
          m_stageModeInterface(NULL) //++tianxiang Merge ECP++
    {

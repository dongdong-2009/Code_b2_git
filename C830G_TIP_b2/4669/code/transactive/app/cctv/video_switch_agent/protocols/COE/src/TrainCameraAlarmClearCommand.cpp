/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/app/cctv/video_switch_agent/protocols/COE/src/TrainCameraAlarmClearCommand.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    //
    // TrainCameraAlarmClearCommand (IAgentRequestCommand intended constructor)
    //
    TrainCameraAlarmClearCommand::TrainCameraAlarmClearCommand( unsigned long stageId, unsigned long monitorId )
        : AbstractCOERequestCommand()
    {
        ...
        commandPacket[ TA_COE::COMMAND_BYTE ] = TA_COE::TRAIN_ALARM_CLEAR_COMMAND;

		commandPacket[ TA_COE::FIRST_DATA_BYTE ] = static_cast<unsigned short>(stageId) % TA_COE::BYTE_HEX_CEILING;
		commandPacket[ TA_COE::FIRST_DATA_BYTE +1 ] = static_cast<unsigned short>(monitorId) % TA_COE::BYTE_HEX_CEILING;
    ...
    // processResponse
    //
    bool TrainCameraAlarmClearCommand::processResponse( const std::vector< unsigned char >& data )
    {
        FUNCTION_ENTRY( "processResponse" );
		//theres not alot we can do except make sure the response is valid
        if ( processGenericResponsePacketAttributes( data,  TA_COE::TRAIN_ALARM_CLEAR_COMMAND, TA_COE::PACKET_HEADER_LENGTH,

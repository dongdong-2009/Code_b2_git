...
namespace TA_IRS_App
{
        ...
        : AbstractCOERequestCommand(), m_expectedResponseCmd( 0x00 ), m_dwellTime( dwellTime ), m_videoInputs( videoInputs ), m_sequence( sequence )
    {
        ...
        if ( false == (seqAddr >> sequenceID && seqAddr.eof()) )
        {
            TA_THROW( TA_Base_Core::VideoSwitchAgentInternalException( 
                ...
                "Invalid value set to Sequence Address" ) );            
        }
        ...
        {
            packetLength = 4 + ( 3 * m_videoInputs.size() );
        }
        else
        {
            packetLength = 3;
        ...
        for ( ; videoInputsIt != m_videoInputs.end(); videoInputsIt++ )
        {
			...
			unsigned short camId = camAddr.second % 0x0201;
			msg.push_back( camId % 0x0100); //LSB
			msg.push_back( camId / 0x0100); //MSB
        }

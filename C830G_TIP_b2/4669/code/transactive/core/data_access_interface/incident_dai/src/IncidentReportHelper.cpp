/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/core/data_access_interface/incident_dai/src/IncidentReportHelper.cpp $
  * @author:  Grace Koh
  * @version: $Revision: #1 $
  *
...
const int IncidentReportHelper::MAX_INCIDENTCOUNTER = 99999;
...
*/
    ...
    IData* data = database->executeQuery(sql.str(),columnNames);
    if (data == NULL)
    {
...
void IncidentReportHelper::close()
{
	...
	if (m_closeDate == 0)
	{
		...
		try
		{
			...
			string closeString = getUpdateDateSql(closeDate);
...
//Bug 652 (TD12313)
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			...
			m_reportId.c_str(), m_alarmId.c_str(), sql.str().c_str());
...
void IncidentReportHelper::getForeignKeyValues(TA_Base_Core::IDatabase* database, std::string& locationId1, std::string& locationId2)
{
	...
	std::string IncidentReportHelper::getUpdateDateSql(time_t datetime)
	{
	...
	std::string IncidentReportHelper::retrieveReportID()
	{
		...
		try
		{
			IData* data = m_dbConnection->executeQuery(sql.str(), columnNames);
			if (data == NULL)
			{
				TA_THROW( IncidentException("Execute query error", IncidentException::DATABASE_ERROR) );
			}
			...
			char reportIdBuffer[20] = {0}; // maxim // increase the buffer to allow automatic NULL termination during sprintf
	...
	void IncidentReportHelper::updateReportIDCounter()
	{
		...
		m_dbConnection->executeModification(sql.str());
	}

...
//                                                TA_Base_Core::DataException::INVALID_VALUE, name ) );
    ...
    void TrainRadioGroup::applyChanges()
    {
        ...
        if ( true == m_isNew )
        {
            ...
            pkeyColumn.push_back( fieldName );
            
            TA_Base_Core::IData* data = databaseConnection->executeQuery( sql.str(), pkeyColumn );
            
            m_pkey = data->getUnsignedLongData( 0, fieldName );

            sql.clear(); //BUG : Calling clear method won't clear the stringbuffer in the ostrinstream
			sql.str(""); //SOLUTION : Call str method with and empty string.

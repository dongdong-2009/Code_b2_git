/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/core/data_access_interface/ecs/src/ECSStationModeAccessFactory.cpp $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2008/12/31 11:34:10 $
...
namespace TA_IRS_Core
{
    ...
    IECSStationModes ECSStationModeAccessFactory::getECSStationModesByMasterModeKey( unsigned long key, const bool readWrite )
	{
		...
		TA_Base_Core::IData* data = databaseConnection->executeQuery(query.str(), columnNames);
    ...
    std::vector< std::pair<unsigned long, std::string> > ECSStationModeAccessFactory::getOrderedLocations()
	{
		...
		std::ostringstream sql;
		sql << "select distinct l.pkey, l.order_id, l.name "
		...
		do
        {
            ...
            {   
				...
				station.second = data->getStringData( i, "NAME" );

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4669_T01271350/4669/transactive/bus/banner_pages/call_page_library/src/CallEntryManager.cpp $
  * @author:  Dirk McCormick
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2010/05/11 10:16:48 $
...
#include "bus/banner_pages/call_page_library/src/SoundEntity.h"
...
namespace TA_IRS_Bus
{
    ...
    void CallEntryManager::refreshSoundPriority(const std::string& category, int priority)
    {
        ...
        try
        {
			// TA_Base_Bus::SoundManagerProxy soundManager(0); // limin- CL-21240
            // soundManager.setPriority(category, priority);   // limin-- CL-21240

            // limin++ CL-21240
            boost::shared_ptr< SoundEntity > entity ( new SoundEntity );
            entity->setPrority( priority ) ;
            entity->setCatergory ( category ) ;
            m_soundThread.insert( entity );
            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "refreshSoundPriority(): set priority successfully with priority %d", priority);
            // ++limin CL-21240
        }

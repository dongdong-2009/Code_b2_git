/**
  ...
  * in any form.
  *
  * Source: $File: //depot/4669_T01271350/4669/transactive/bus/pa/cachedmaps/src/PaDvaPrivateVersionCachedMap.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
...
{
    PaDvaPrivateVersionCachedMap::PaDvaPrivateVersionCachedMap()
    :
    GenericCachedMap<DvaPrivateVersionRecord, TA_Base_Core::IPaDvaMessagePrivateVersion>
    ...
    TA_Base_Core::IPaDvaMessagePrivateVersion* PaDvaPrivateVersionCachedMap::getRecordFromDatabaseUsingKey(unsigned long pKey, bool readWrite)
        //throw(TA_Base_Core::DataException, TA_Base_Core::DatabaseException)
    {
        return TA_Base_Core::PaDvaMessagePrivateVersionAccessFactory::getInstance().
                                getPaDvaMessagePrivateVersion(pKey, readWrite);
    }


    void PaDvaPrivateVersionCachedMap::validateRecordData(const DvaPrivateVersionRecord& dvaVersion)
    {
    ...
    DvaPrivateVersionRecord PaDvaPrivateVersionCachedMap::createRecordFromInterface(TA_Base_Core::IPaDvaMessagePrivateVersion& item)
         //throw (TA_Base_Core::InvalidPasConfigurationException)
    {
        DvaPrivateVersionRecord result;
    
        result.m_pKey = item.getKey();
        result.m_locationKey = item.getLocationKey();
    
        result.m_privateAdhoc1 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC1);
        result.m_privateAdhoc2 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC2);
        result.m_privateAdhoc3 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC3);
        result.m_privateAdhoc4 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC4);
        result.m_privateAdhoc5 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC5);
        result.m_privateAdhoc6 = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC6);
        result.m_privatePreRecorded = item.getVersionData(TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_PRERECORDED);
    
        return result;
    }


    void PaDvaPrivateVersionCachedMap::refreshKeyToRecordMap(unsigned long locationKey) 
            //throw(TA_Base_Core::DataException, TA_Base_Core::DatabaseException, TA_Base_Core::InvalidPasConfigurationException)
    {
        ...
        TA_Base_Core::IPaDvaMessagePrivateVersions dvaVersions;

        if (locationKey == -1)
        {
            // Obtain read only array of *all* DVA Message Versions
            dvaVersions = TA_Base_Core::PaDvaMessagePrivateVersionAccessFactory::getInstance().
                                            getAllPaDvaMessagePrivateVersions(false);
        }
        else
        {
            // Obtain read only array of location specific DVA Message Versions
            dvaVersions.push_back(TA_Base_Core::PaDvaMessagePrivateVersionAccessFactory::getInstance().
                                            getPaDvaMessagePrivateVersionByLocationKey(locationKey, false));
        }    
    ...
    DvaPrivateVersionRecord PaDvaPrivateVersionCachedMap::getRecordFromLocation(unsigned long locationKey)
        //throw(TA_Base_Core::CachedMappingNotFoundException)
    {
        ...
        DvaPrivateVersionRecords matchingRecords = getFirstMatchingRecord(filter);
        ...
        if (matchingRecords.size() == 0)
        {
            ...
            msg << "Failed to locate DvaPrivateVersionRecord with locationKey=" 
                << locationKey << " in KeyToPaDvaPrivateVersionRecordMap";
    ...
    unsigned long PaDvaPrivateVersionCachedMap::getKeyFromLocation(unsigned long locationKey)
        //throw(TA_Base_Core::CachedMappingNotFoundException)
    {
    ...
    void PaDvaPrivateVersionCachedMap::setRecord( DvaPrivateVersionRecord& record, 
        ...
        //throw(TA_Base_Core::DataException, TA_Base_Core::DatabaseException, TA_Base_Core::DataConfigurationException)
    {
        ...
        {
			TA_Base_Core::IConfigPaDvaMessagePrivateVersion* configPaDvaMessageVersion = NULL;

            if (isExistingRecord)
            {
                LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, 
                   "Writing modified PaDvaPrivateVersionRecord for pKey=%d", 
                   record.m_pKey);

                TA_Base_Core::IPaDvaMessagePrivateVersion* paDvaMessageVersion = 
                    TA_Base_Core::PaDvaMessagePrivateVersionAccessFactory::getInstance().getPaDvaMessagePrivateVersion(record.m_pKey, true);
                configPaDvaMessageVersion = dynamic_cast<TA_Base_Core::IConfigPaDvaMessagePrivateVersion*>(paDvaMessageVersion);
            ...
            {
                LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, 
                   "Writing new PaDvaPrivateVersionRecord");

                configPaDvaMessageVersion = TA_Base_Core::PaDvaMessagePrivateVersionAccessFactory::getInstance().createPaDvaMessagePrivateVersion();
            }
            ...
            try
            {
                ...
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc1, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC1);
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc2, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC2);
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc3, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC3);
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc4, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC4);
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc5, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC5);
                configPaDvaMessageVersion->setVersionData(record.m_privateAdhoc6, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_ADHOC6);
                configPaDvaMessageVersion->setVersionData(record.m_privatePreRecorded, TA_Base_Core::IPaDvaMessagePrivateVersion::PRIVATE_PRERECORDED);
    ...
    // ExceptionChecked
    void PaDvaPrivateVersionCachedMap::submitConfigUpdate( unsigned long recordKey, 
                                                    ...
                                                    const std::vector<std::string>& changes)
    {
        ...
        {
            std::ostringstream msg;
            msg << "Failed to submit the PaDvaPrivateVersionCachedMap Config Update Message";

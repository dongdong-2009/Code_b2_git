...
namespace TA_IRS_Bus
{
    ...
    void CSearchPage::OnDestroy()
    {
        ...
        try
        {
			...
			m_callbackHelper.getTrainSelectorGUI().removeCallbackWindow( this );
		}
        catch ( ... )
        {
                                                 ...
                                                 bool bClearItems /* = true */ )
    {
        ...
        try
        {
			...
			m_listSubscriberName.push_back(subscriberName);
            int iRet;
            //TD17287++
    ...
    void CSearchPage::OnDisplayGroupDetails()
    {
        ...
        try
        {
            ...
            for ( unsigned int i = 0; i < groupMembers->length(); i++ )
            {
                ...
                {
					m_listSubscriberName.push_back(groupMembers[i].subscriberName.in());
                    iRet = m_SearchList.InsertItem( i, groupMembers[i].subscriberName );
                    m_SearchList.SetItemText( iRet, 1, groupMembers[i].TSI );
                    m_SearchList.SetItemText( iRet, 2, groupMembers[i].locationName );
                    m_SearchList.SetItemData( iRet, TYPE_INDIVIDUAL );
    ...
    void CSearchPage::processTrainNotification( TA_IRS_Bus::MFCTrainSelectorGUI::TrainListUpdate* trainListUpdate )
    {
        ...
        bool isLocInCtrl= m_callbackHelper.getTrainSelectorGUI().isLocationInControl( trainListUpdate->trainDetails.currentLocation, trainListUpdate->trainDetails.communicatingLocation );//limin++, ECP-338 CL-20150
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     "Notification from TrainListComponent, trainLocation = %d, trainLoc = %d, isLocInCtrl = %s, trainCommunicationLocation=%d",
                     trainListUpdate->trainDetails.currentLocation, trainListUpdate->trainDetails.currentLocation,
                     isLocInCtrl ? "TRUE" : "FALSE",
                     trainListUpdate->trainDetails.communicatingLocation );
        ...
        else if ( TA_Base_Bus::IRadioCorbaDef::INDIVIDUAL_SUBSCRIBER == details.type )
        {
            ...
            else
            {
                ...
                if ( iter != m_stationLocationMap.end() )
                {
                    ...
                    // [[Add start by HuangQi for TD18774 on 2009-07-10
                    // Sort one station children nodes
                    if (NULL != iter->second)
                    {
                        m_DirectoryTree.SortChildren( iter->second );
                    }
                    // HuangQi add end on 2009-07-10]]
                }
                else
                {
                    ...
                    // [[Add start by HuangQi for TD18774 on 2009-07-10
                    // Sort station/others children nodes
                    if (NULL != m_hTreeItemStation)
                    {
                        m_DirectoryTree.SortChildren( m_hTreeItemStation );
                    }
                    // Sort one station children nodes
                    if (NULL != localStation)
                    {
                        m_DirectoryTree.SortChildren( localStation );
                    }
                    // HuangQi add end on 2009-07-10]]
    ...
    bool CSearchPage::doesItemExistInSearchList( CString name )
    {
		...
		if( m_listSubscriberName.end()!= std::find(m_listSubscriberName.begin(),m_listSubscriberName.end(),name))
			return true;

        /*for ( int i = 0; i < m_SearchList.GetItemCount(); i++ )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "check item exists: %s , name of item: %s.",

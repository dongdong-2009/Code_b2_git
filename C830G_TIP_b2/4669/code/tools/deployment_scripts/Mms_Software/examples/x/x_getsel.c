...
ExampleGetByType( )
{
   ...
   */
   for ( i = EVEN; i <= ODD; i++ ) {
      ...
      /*
      **    Set the selection filter to select only those messages
      **    which have the same type as we are looking for.
      */
      sel_filter[0]  = i;
      ...
      while ( dmq_status == PAMS__SUCCESS ) {
         /*  Initialize the message area to hold the message  */
         (void)memset((void*)&msg, '\0', MAX_MESSAGE_SIZE );

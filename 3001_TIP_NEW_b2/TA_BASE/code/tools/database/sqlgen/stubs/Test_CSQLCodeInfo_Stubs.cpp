 ...
 */
#include "src\SQLCodeInfo.h"

/** User stub definition for function: int CSQLCodeInfo::analyzeData(VrtLinesConT &) */
int (::CSQLCodeInfo::CppTest_Stub_analyzeData) (::VrtLinesConT & vrtAllLines) 
{
	if (CppTest_IsCurrentTestCase("test__err4_GeneralSQLFiles_1"))
	{
		return -1;	
	}

	return analyzeData( vrtAllLines);
}

/** User stub definition for function: int CSQLCodeInfo::checkOracleAndMysqlSqlNum(void) */
int (::CSQLCodeInfo::CppTest_Stub_checkOracleAndMysqlSqlNum) (void) 
{
	if (CppTest_IsCurrentTestCase("test__err5_GeneralSQLFiles_1"))
	{
		return -1;	
	}

	return checkOracleAndMysqlSqlNum();
}

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/naming/src/NamedObject.h $
...
{
	template <class CorbaDef, class CorbaDef_ptr, class CorbaDef_var> class NamedObject
	...
	public:
		/** 
		  ...
		  */
		NamedObject() 
			: m_timeoutPeriod( 0 ), m_varInvalid( true )
		{
		...
		/**
		  ...
		  */
		virtual void assignObject( CORBA::Object_ptr newObject )
		{
			// Gaurd the corbaDefVar
			ThreadGuard guard( m_varGuard );
...
#define CORBA_CALL_RETURN( result, namedObject, corbaInterface, parameters )                                                                                            \
    {                                                                                                                                                                   \
        try                                                                                                                                                             \
        {                                                                                                                                                               \
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "CORBA CALL - BEGIN: %s = %s->%s%s", #result, #namedObject, #corbaInterface, #parameters);    \
            result = namedObject->corbaInterface parameters;                                                                                                            \
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "CORBA CALL - END: %s = %s->%s%s", #result, #namedObject, #corbaInterface, #parameters);    \
        }                                                                                                                                                               \
        catch ( TA_Base_Core::OperationModeException& e )                                                                                                               \
        {                                                                                                                                                               \
            LOG_EXCEPTION_CATCH( SourceInfo, "TA_Base_Core::OperationModeException", TA_Base_Core::CorbaUtil::getInstance().exceptionToString( e ).c_str() );           \
            namedObject.clear();                                                                                                                                        \
            result = namedObject->corbaInterface parameters;                                                                                                            \
        }                                                                                                                                                               \
...
#define CORBA_CALL( namedObject, corbaInterface, parameters )                                                                                                           \
    {                                                                                                                                                                   \
        try                                                                                                                                                             \
        {                                                                                                                                                               \
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "CORBA CALL - BEGIN: %s->%s%s", #namedObject, #corbaInterface, #parameters);                  \
            namedObject->corbaInterface parameters;                                                                                                                     \
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "CORBA CALL - END: %s->%s%s", #namedObject, #corbaInterface, #parameters);                  \
        }                                                                                                                                                               \
        catch ( TA_Base_Core::OperationModeException& e )                                                                                                               \
        {                                                                                                                                                               \
            LOG_EXCEPTION_CATCH( SourceInfo, "TA_Base_Core::OperationModeException", TA_Base_Core::CorbaUtil::getInstance().exceptionToString( e ).c_str() );           \

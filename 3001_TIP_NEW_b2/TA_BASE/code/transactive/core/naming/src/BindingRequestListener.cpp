/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/naming/src/BindingRequestListener.cpp $
  * @author:  Cameron Rochester
  * @version: $Revision: #7 $
  *
  * Last modification: $DateTime: 2014/08/06 10:02:03 $
...
namespace TA_Base_Core
{
    ...
    CORBA::Object_ptr BindingRequestListener::getObject( const char* objectName )
    {
        ...
        if ( ! m_inControlMode )
        {
            LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugWarn,
                "getObject called while agent is not in control mode" );

            // Throw exception
            throw TA_Base_Core::OperationModeException( "getObject called while agent is not in control mode" );
        }
        ...
        if ( it != m_bindingsMap.end() )
        {
            ...
            if ( CORBA::is_nil( it->second) )
            {
                ...
                // Throw exception
                std::string errMsg( "Object reference invalid for " );
                errMsg += objectName;

                throw TA_Base_Core::NamedObjectRepositoryCorbaDef::NameNotFound( errMsg.c_str() );
            }
            ...
            //Bohong++ for more infomation when resolve
            {
                ...
                CORBA::String_var objToString( (CorbaUtil::getInstance().getOrb()).object_to_string(CORBA::Object::_duplicate( it->second )) );
        ...
        else
        {
            ...
            std::string errMsg( "Object reference not found for " );
            errMsg += objectName;
    ...
    void BindingRequestListener::receiveSpecialisedMessage( const TA_Base_Core::BindingRequestMessageCorbaDef& message )
    {
        ...
        LOG3( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
            "Agent received BindingRequest message for NamedObjectRepository: messageType=%s, nameContext=%s, name=%s",
            message.messageTypeKey.in(), message.nameContext.in(), message.name.in() );
        ...
        { // cl-21128 gongzong++
            ...
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "<gz_log><09/12/1> receiveSpecialisedMessage()" );
        } // ++gongzong
    ...
    void BindingRequestListener::processMessage(TA_Base_Core::IWorkItem* item, const TA_Base_Core::BindingRequestMessageCorbaDef& message )
    {
        ...
        static const unsigned long DEFAULT_RESOLVE_TIMEOUT_MS = 2000; // 2 seconds
        ...
        if ( m_agentName.compare( message.name ) == 0 )
        {
            try
            {
                ...
                if ( !CORBA::is_nil(originator) )
                {
                    ...
                    {   //Bohong++ for more infomation when resolve
                        //CORBA::ORB orb = CorbaUtil::getInstance().getOrb();
                        CORBA::String_var objToString( (CorbaUtil::getInstance().getOrb()).object_to_string(m_myObjRef) );
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "ObjectToString: %s", objToString.in() );
                    }   //++Bohong
    ...
    void BindingRequestListener::poll()
    {
        ...
        {
            LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugWarn,
                "poll called while agent is not in control mode, throwing OperationModeException" );

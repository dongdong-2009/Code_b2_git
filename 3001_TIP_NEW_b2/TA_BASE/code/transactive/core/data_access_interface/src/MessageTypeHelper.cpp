  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/src/MessageTypeHelper.cpp $
  * @author:  Nick Jardine
  * @version: $Revision: #6 $
  *
...
namespace TA_Base_Core
{
	...
	void MessageTypeHelper::getColumnNames(std::vector<std::string>& columnNames)
	{
		...
		columnNames.push_back("ISENABLEREALTIMEEVNT");
	}
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(ME_MESSAGE_TYPE_SELECT_10055, m_key);
	...
	void MessageTypeHelper::reloadUsing(const unsigned long row, TA_Base_Core::IData& data)
    {
        ...
        m_priority = data.getIntegerData(row, columnNames[3]);
        unsigned int printFlag = data.getIntegerData(row, columnNames[4]);
        m_toBePrinted = (1 == printFlag); 
        unsigned int logFlag = data.getIntegerData(row, columnNames[5]);
        m_toBeLogged = (1 == logFlag);
        unsigned int persistFlag = data.getIntegerData(row, columnNames[6]);
        m_toBePersisted = (1 == persistFlag);
        m_viewLevel = data.getUnsignedLongData(row, columnNames[7]);
        
        m_dateModified = data.getDateData(row, columnNames[8], 0);
        m_dateCreated = data.getDateData(row, columnNames[9]);

		m_actionID = data.getUnsignedLongData(row, columnNames[10]);
		unsigned int isRealTimeEvent = data.getIntegerData(row, columnNames[11]);

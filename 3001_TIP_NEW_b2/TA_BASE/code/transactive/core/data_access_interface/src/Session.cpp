/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/src/Session.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #9 $
  *
...
namespace TA_Base_Core
{
	...
	void Session::resetHasEnded()
	{
		LOG( SourceInfo, DebugUtil::FunctionEntry, "Session::resetHasEnded");
		...
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, TA_SESSION_UPDATE_11205,getLocalDatabaseName(),m_sessionId);
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(TA_SESSION_UPDATE_11202,userOverrideId,profileOverrideId, m_sessionId);
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, TA_SESSION_UPDATE_11202,getLocalDatabaseName(),userOverrideId,profileOverrideId, m_sessionId);
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(TA_SESSION_UPDATE_11203, m_sessionId);
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, TA_SESSION_UPDATE_11203,getLocalDatabaseName(), m_sessionId);
...
//		 std::string strSql  = databaseConnection->prepareSQLStatement(TA_SESSION_UPDATE_11204,displayOnly, m_sessionId);
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, TA_SESSION_UPDATE_11204,getLocalDatabaseName(),displayOnly, m_sessionId);

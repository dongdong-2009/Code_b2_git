/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/src/SubsystemHelper.cpp $
...
//		strSql  = databaseConnection->prepareSQLStatement(SUBSYSTEM_UPDATE_18151, 
		...
		databaseConnection->prepareSQLStatement(strSql, SUBSYSTEM_UPDATE_18151,
			databaseConnection->escapeQueryString(m_name), m_isExclusive, m_key);
...
//		strSql  = databaseConnection->prepareSQLStatement(SUBSYSTEM_INSERT_18351, 
		...
		databaseConnection->prepareSQLStatement(strSql, SUBSYSTEM_INSERT_18351,
			...
			databaseConnection->escapeInsertString(m_name), m_isExclusive);
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(SUBSYSTEM_SELECT_18057, m_key);
	...
	void SubsystemHelper::reloadUsing(const unsigned long row, TA_Base_Core::IData& data)
    {
		...
		try
		{
			...
			m_isPhysical	= data.getBooleanData( row, columnNames[2]);
			m_isExclusive	= data.getBooleanData( row, columnNames[3]);

			m_dateModified = data.getDateData(row, columnNames[4] );
			m_dateCreated	= data.getDateData( row, columnNames[5]);
			m_systemKey = data.getUnsignedLongData( row, columnNames[6]);

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/system_controller/src/ConfigSystemController.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
...
namespace TA_Base_Core
{
    ...
    ItemChanges ConfigSystemController::getAllItemChanges()
    {
        ...
        for (ItemChanges::iterator iter = m_systemControllerChanges.begin(); iter != m_systemControllerChanges.end(); ++iter)
        {
            ...
            if( -1 == iter->first.find_first_not_of("0123456789") )
            {
    ...
    void ConfigSystemController::writeMonitoredGroupChanges()
    {
        ...
        if (!setDifference2.str().empty())
        {
            ...
            int startPos = 0;
            int commaPos = setDifference2.str().find(",",startPos);
            while( commaPos != -1)
            {

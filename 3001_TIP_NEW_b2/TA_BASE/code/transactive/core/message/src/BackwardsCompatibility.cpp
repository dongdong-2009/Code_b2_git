/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/message/src/BackwardsCompatibility.cpp $
  * @author:  Cameron Rochester
  * @version: $Revision: #8 $
  *
  * Last modification: $DateTime: 2016/01/20 17:22:09 $
...
namespace TA_Base_Core
{
                                                              ...
                                                              const FilterData* filterableData /*= NULL*/ )
    {
        ...
        if ( context.getChannelName() != "GeneralComms" )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "pass in wrong message type: %s", context.getContextName().c_str() );
		}
                                                              ...
                                                              const FilterData* filterableData /*= NULL*/ )
    {
        ...
        if ( messageType.getContextObj().getChannelName() != "GeneralComms" )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "pass in wrong message type: %s", (messageType.getContextObj().getContextName() + "::" + messageType.getTypeKey()).c_str() );
        }
                                                                       ...
                                                                       const FilterData* filterableData /*= NULL*/ )
    {
        ...
        if ( context.getChannelName() != "BroadcastComms" )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "pass in wrong message type: %s", context.getContextName().c_str() );
        }
                                                                       ...
                                                                       const FilterData* filterableData /*= NULL*/ )
    {
        ...
        if ( messageType.getContextObj().getChannelName() != "BroadcastComms" )
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "pass in wrong message type: %s", (messageType.getContextObj().getContextName() + "::" + messageType.getTypeKey()).c_str() );
        }
                                                                     ...
                                                                     unsigned long locationKey )
    {
        ...
        subscriber->m_subscriptions.addSubscription( &gGetSubscriptionManager(), context.getChannelName(), sf, locationKey, subscriber );
    }

...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/message/src/StructuredEventConsumer.h $
...
{
    class SortableBatchingQueue : public BatchingQueue<CosNotification::StructuredEvent>
    {
        typedef std::deque<CosNotification::StructuredEvent*> Queue_t;

        enum EDispatchResult{ DISPATCH_FAILED = 0, DISPATCH_SUCCESS = 1, DISPATCH_OUT_OF_ORDER = 2 };

        struct StructuredEventCompareBySequenceNumberWrap
        {
        public:
            
            StructuredEventCompareBySequenceNumberWrap( CosNotification::StructuredEvent* msg, unsigned long seq_num, unsigned long queue_index )
                : m_msg( msg ),
                  m_seq_num( seq_num ),
            ...
            inline bool operator < ( const StructuredEventCompareBySequenceNumberWrap& rhs ) const 
            {
                return ( m_seq_num < rhs.m_seq_num ) ||
                       ( m_seq_num == rhs.m_seq_num && m_queue_index < rhs.m_queue_index );
            }
            
        public:
            
            CosNotification::StructuredEvent* m_msg;
            unsigned long m_seq_num;
            unsigned long m_queue_index;
        };

    public:

        SortableBatchingQueue();

    protected:

        virtual void run();
        void pushObjects(/*takes*/ const std::vector<CosNotification::StructuredEvent*>& objs);

    private:
        
        EDispatchResult dispatchBatchInOrder();
        void sortMessageQueue( std::deque< CosNotification::StructuredEvent* >& messages );
        bool isOutOfOrder( const std::vector<CosNotification::StructuredEvent*>& events );
        unsigned long getQueueFromCache();
        void getAppenderIdAndSeqNum( const CosNotification::StructuredEvent& event, const char*& evtAppenderId, unsigned long& evtSeqNum );

    private:
        
        Queue_t m_queueCache;
        TA_Base_Core::ReEntrantThreadLockable m_queueCacheLock;

        const unsigned long WAIT_MESSAGE_TIME_MS;
    };
	...
	public:
		...
		StructuredEventConsumer(/*takes*/ IAdminProvider* adminProvider, bool handleOutOfOrder = false );

  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/message/src/TA_CosUtility.cpp $
...
namespace TA_Base_Core
{
                                         ...
                                         const FilterData* filterableData )
    {
        ...
        if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_ENTITYNAME))
        {
            ...
            event.header.variable_header[index].value <<= TA_Base_Core::RunParams::getInstance().get(RPARAM_ENTITYNAME).c_str();
        }
        // ++yanrong
    ...
    {            
        ...
        for (FilterData::const_iterator it = filterableData->begin(); it != filterableData->end(); )
        {
            ...
            if ( (*it)->value.find_first_of( "\\'" ) != std::string::npos ) // two special characters: \'
            {

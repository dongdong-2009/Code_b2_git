/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/message/src/TA_CosNotifyChannel.cpp $
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void CosNotifyChannelConsumer::push_structured_events(const CosNotification::EventBatch& eventBatch)
    {
        ...
        // Copy the events from the Corba sequence into a std::vector so
        // they can be passed to our IEventHandler.
        std::vector<const CosNotification::StructuredEvent*> vecBatch;
        vecBatch.reserve(eventBatch.length());

        for ( unsigned int i = 0; i < eventBatch.length(); i++ )
        {
            LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugMsgPub,
                "CRECEIVE EVT %d: %s", i, gStringFromEventHeader(eventBatch[i]).c_str() );

            vecBatch.push_back(&eventBatch[i]);
        }
        ...
        if ( m_eventHandler )
        {
            TA_Base_Core::gLogSpecificMessage(eventBatch[0], "CosNotifyChannelConsumer::push_structured_events()");
            m_eventHandler->onEventsReceived(vecBatch);
        }
    ...
    bool CosNotifyChannelConsumer::createAndSetProxySupplier()
    {
        ...
        try
        {
			...
			CosNotifyChannelAdmin::SequenceProxyPushSupplier_ptr proxySupplier = 

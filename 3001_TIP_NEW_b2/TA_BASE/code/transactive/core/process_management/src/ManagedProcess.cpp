/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/process_management/src/ManagedProcess.cpp $
  * @author:  Ripple
  * @version: $Revision: #6 $
  *
  * Last modification: $DateTime: 2015/07/15 17:18:29 $
...
namespace TA_Base_Core
{
    ...
    : m_ProcessManager(IProcessManagerCorbaDef::_nil()), m_terminationFromManager(false), 
      m_monitoringThread(NULL), m_managedProcessUser(user), m_pollListener(pollListener)
    {
        ...
        try
        {
			...
			bool fTestNSANoDB = RunParams::getInstance().isSet("TestNSANoDb");
			if (fTestNSANoDB)
			{
				hasRegistered = true;
			}
        ...
        catch( ... )
        {
            LOG( SourceInfo, DebugUtil::ExceptionCatch,"Unknown", 
                 "Perform necessary clean up before re-throwing the exception" );
            if( hasRegistered )
            {
                    notifyTerminating(TA_Base_Core::InitError,
                                  "Exception raised while initialising ManagedProcess");
            }
            deactivateServant();
            throw;
        }
    ...
    void ManagedProcess::setParam(const TA_Base_Core::RunParam& param)
    {
        ...
        {
            TA_Base_Core::CorbaUtil::getInstance().setCorbaParam(param.value);
        }
        else
        {
    ...
    TA_Base_Core::RunParamSeq* ManagedProcess::getParams()
    {
        ...
        {
            (*seq)[i].name = CORBA::string_dup(vec[i].name.c_str());
            (*seq)[i].value = CORBA::string_dup(vec[i].value.c_str());
        }
    ...
    {
        LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugFatal,
            "Process Manager not responding, shutting down");
        ...
        {
            // avoid second call
            IManagedProcessUser* tmp = m_managedProcessUser;
            m_managedProcessUser = NULL;
            tmp->onTerminate();
        }
    }
    ...
    void ManagedProcess::cleanup()
    {
        LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugDebug,
             "Tell any registered managed process users that we are terminating");
    ...
    void ManagedProcess::locateProcessManager()
    {
        ...
        {
            unsigned long port =
                strtoul(RunParams::getInstance().get(RPARAM_MGRPORT).c_str(), NULL, 0);
            if (port == 0)
            {
    ...
    void ManagedProcess::retrieveSystemParameters()
    {
        try
        {
            ...
            TA_Base_Core::RunParamSeq_var p = 
				m_ProcessManager->getParams(m_managedProcessID, "");

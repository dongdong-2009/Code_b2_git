#if !defined(__C_SERVER_ENQ_WORK_HH__)
#define __C_SERVER_ENQ_WORK_HH__
...
class CSrvEnqWorker : public Thread, public IPropagateHandler
{
private:
...
public:
	CSrvEnqWorker(CSrvComWorker* pComWorker,PAQWorkInfo pDeqEnqInfo, unsigned long ID);
	~CSrvEnqWorker();
...
private:
	...
	int _EnqToDBEx(ENQREQEX* pRcvFrame, ulong64 uLLastMsgSID);
	...
	int _TurnBigListToSmallList(lstDBMsgConT& lstINDBMsg, lstDBMsgsConT& lstOUTDBMsgs, int nINEachLstNum);
...
private: 
	...
	TA_AQ_Lib::EnqueueMode	m_nEnqMode;//EnqueueMode {ENQ_AQ_API = 1,ENQ_SQL_PRC = 2};
...
#endif//__C_SERVER_ENQ_WORK_HH__

...
bool CCommandHandler::submitAlarm(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail, bool bUpdateList/* = true*/)
{
...
bool CCommandHandler::acknowledgeAlarm(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail)
{
	TA_Base_Bus::ThreadGuard guard(m_ThreadLock);
	bool bResult = false;
	...
	if (NULL != g_pAtsInterface && NULL != m_pProfileInfo)
	{
		...
		if (pAlarmDetail->state == I_ATS_ISCS_ALARM::AlarmReturned)
		{	
			pRequest->mgsType = CLOSE_ALARM;
			pAlarmDetail->state = I_ATS_ISCS_ALARM::AlarmClosed;
		}
		else
		{
			pRequest->mgsType = UPDATE_ALARM;
			pAlarmDetail->state = I_ATS_ISCS_ALARM::AlarmAcknowledged;
		}
...
bool CCommandHandler::closeAlarm(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail)
{
	TA_Base_Bus::ThreadGuard guard(m_ThreadLock);
	bool bResult = false;
...
bool CCommandHandler::normalizeAlarm(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail)
{
	TA_Base_Bus::ThreadGuard guard(m_ThreadLock);
	bool bResult = false;
	...
	if (NULL != g_pAtsInterface && NULL != m_pProfileInfo)
	{
		...
		pAlarmDetail->closeTime = time(0);
		if (pAlarmDetail->state == I_ATS_ISCS_ALARM::AlarmAcknowledged)
		{			
			pAlarmDetail->state = I_ATS_ISCS_ALARM::AlarmClosed;
			pRequest->mgsType = CLOSE_ALARM;
		}
		...
		{
			pAlarmDetail->state = I_ATS_ISCS_ALARM::AlarmReturned;
			pRequest->mgsType = UPDATE_ALARM;
		}
		pRequest->pAlarmDetail = pAlarmDetail;
		m_requestQue.push(pRequest);	
...
bool CCommandHandler::updateMMSState(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail)
{
	TA_Base_Bus::ThreadGuard guard(m_ThreadLock);
	bool bResult = false;
...
void CCommandHandler::setProfileInfo(I_ATS_ISCS_ALARM::ProfileInfo profileInfo)
{
	TA_Base_Bus::ThreadGuard guard(m_ThreadLock);
	...
	// Set new profile
	m_pProfileInfo = new I_ATS_ISCS_ALARM::ProfileInfo();
...
void CCommandHandler::getProfileInfo(I_ATS_ISCS_ALARM::ProfileInfo& profileInfo)
{
	if (NULL != m_pProfileInfo)
	{
...
void CCommandHandler::notifyObserver(UPDATE_TYPE tUpdate, I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetail)
{
	switch (tUpdate)
	{
	case NEW_ALARM:
	...
	while (!m_bTerminate)
	{
		...
		try
		{
			...
			if (pMgs != NULL && g_pAtsInterface != NULL)
			{
				...
				case UPDATE_ALARM:
				case CLOSE_ALARM:
					g_pAtsInterface->submitATSAlarm(*(pMgs->pAlarmDetail), *m_pProfileInfo);
					if (pMgs->bUpdateList)
					{

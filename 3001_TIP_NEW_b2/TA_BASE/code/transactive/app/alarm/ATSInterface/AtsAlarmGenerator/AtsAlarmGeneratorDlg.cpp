...
void CAtsAlarmGeneratorDlg::Initialize(void)
{
	...
	// Alarm State
	for (int nIdx=0; nIdx<= (int)I_ATS_ISCS_ALARM::AlarmAcknowledged; nIdx++)
	{
		CString strAlarmState(_formatState((I_ATS_ISCS_ALARM::AlarmState)nIdx).c_str());
	...
	I_ATS_ISCS_ALARM::ProfileInfo profileInfo;
...
//	m_pThreadFunctionExe->executeFunction(alarmList, ACKNOWLEDGE);
	...
	{
		if (alarmList[nIdx]->state == I_ATS_ISCS_ALARM::AlarmOpen || alarmList[nIdx]->state == I_ATS_ISCS_ALARM::AlarmReturned)
		{
...
{
	// TODO: Add your control notification handler code here
	I_ATS_ISCS_ALARM::AlarmDetail * pAlarm = NULL;
...
std::string CAtsAlarmGeneratorDlg::_formatState(I_ATS_ISCS_ALARM::AlarmState state)
{
	CString strState;
	switch (state)
	{
...
void CAtsAlarmGeneratorDlg::OnBnClickedBtnNormalize()
{
	...
	for(int nIdx = 0; nIdx < (int)alarmList.size(); nIdx++)
	{
		if (alarmList[nIdx]->state == I_ATS_ISCS_ALARM::AlarmOpen || alarmList[nIdx]->state == I_ATS_ISCS_ALARM::AlarmAcknowledged)
		{

...
public:
	...
	void removeAlarmFromAutoClose(I_ATS_ISCS_ALARM::AlarmDetail* pAlarmDetails);
...
// 	virtual bool removeAllAlarms();
	...
	I_ATS_ISCS_ALARM::AlarmDetail* getAlarmDetails(std::string& strAlarmID);
...
private: // functions
	...
	std::string _formatState(I_ATS_ISCS_ALARM::AlarmState state);
	std::string _formatMMSState(I_ATS_ISCS_ALARM::MmsStateType mmsStateType);

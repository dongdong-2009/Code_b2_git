/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/scada/inspector_panel/src/MeterListCtrl.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2012/06/18 16:50:26 $
	...
	ICellManager::CellType CMeterListCtrl::GetCellType(CEditableListCtrl* listCtrl, int item, int subItem)
	{
		...
		if (NULL != m_equipment && item >= 0)	
		{
			...
			if ( ( subItem >= 0 ) && ( subItem == MeterAdjustColumn ) )
			{
				...
				try
				{
					double doubleValue = dataPoint->getValueAsDouble();
				}
				catch (...)
				{
		...
		if (item >= 0)	
		{
			...
			if ( ( subItem >= 0 ) && ( subItem == MeterAdjustColumn ) )
			{
				...
				try
				{
					...
					statusLine << "Adjusted From " << dataPoint->getValueAsString();
					...
					if ( valueStream >> correction )
     				{
						...
						if ( false == dataPoint->getUnit().empty() )
						{
								 ...
								 ( adjustedValue > dataPoint->getLimit ( false ) ) )	// high eng limit
							{
								...
								statusLine << " - Valid range for Adjusted Value is from ";

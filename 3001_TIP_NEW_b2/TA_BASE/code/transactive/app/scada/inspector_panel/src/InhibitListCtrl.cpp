/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/scada/inspector_panel/src/InhibitListCtrl.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #3 $
  *
...
namespace TA_Base_App
{
					...
					if ((strValue == "Yes") != prevInhibitState)
					{
						...
						if (inhibitOverrideConfirm(IDS_SCAN_INHIBIT, dataPoint, (strValue == "Yes")))
						{
							...
							try
							{
								...
								submitAuditMessage("Scan Inhibit", (strValue == "Yes")?"ADD":"REMOVE", 
				...
				case ALARM_INHIBIT_COLUMN:
				{
					...
					if ((strValue == "Yes") != prevInhibitState) 
					{
						...
						if (inhibitOverrideConfirm(IDS_ALARM_INHIBIT, dataPoint, (strValue == "Yes")))
						{
							...
							try
							{
                                // TD12137: Log an inhibit requested event BEFORE attempting to set the datapoint's alarm inhibition state.
								submitAuditMessage("Alarm Inhibit", (strValue == "Yes")?"ADD":"REMOVE", 
													dataPoint->getEntityName(),
													dataPoint->getEntityKey());
				...
				case CONTROL_INHIBIT_COLUMN:
				{
					...
					if ((strValue == "Yes") != dataPoint->isControlInhibited())
					{
						...
						if (inhibitOverrideConfirm(IDS_CONTROL_INHIBIT, dataPoint, (strValue == "Yes")))
						{
							...
							try
							{
								// TD12137: Log an inhibit requested event BEFORE attempting to set the datapoint's control inhibition state.
								submitAuditMessage("Control Inhibit", (strValue == "Yes")?"ADD":"REMOVE", 
													dataPoint->getEntityName(),
													dataPoint->getEntityKey());
	...
	void CInhibitListCtrl::displayItem(int nItem)
	{
		...
		if (dataPoint->isWriteable() == false)
		{
			...
			try
			{
				...
				TA_Base_Bus::BooleanTag dnCombineMMS = dataNode->getCombinedInhibitMms(); 
				if (dnCombineMMS.quality == TA_Base_Bus::QUALITY_GOOD_NO_SPECIFIC_REASON)
				{
					isChecked |= dnCombineMMS.value;
				}
	...
	void CInhibitListCtrl::submitAuditMessage(std::string inhibitType, std::string addRemove, 
											  std::string dataPoint, unsigned long entityKey)
	{

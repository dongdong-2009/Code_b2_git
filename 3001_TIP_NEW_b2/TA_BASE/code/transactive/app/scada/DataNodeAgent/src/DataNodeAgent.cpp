/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/scada/DataNodeAgent/src/DataNodeAgent.cpp $
  * @author:	Darren Sampson
  * @version:	$Revision: #4 $
  *
  * Last modification:	$DateTime: 2016/12/27 18:15:35 $
...
namespace TA_Base_App
{
	...
	m_operatorMode(TA_Base_Core::NotApplicable),
    m_genericAgent( NULL ),
	...
	{
		// get a handle to the ScadaProxyFactory without starting it's threads
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Agent %s started...", m_agentName.c_str());
	}


	DataNodeAgent::~DataNodeAgent()
	{
        // Remove SingletonTimerUtil singleton
		...
		// set the proxies to standby mode
	...
	void DataNodeAgent::agentSetStandby()
	{
		if (m_operatorMode != TA_Base_Core::Standby)
		{
			...
			// set the proxies to monitor mode
			m_scadaProxyFactory->setOperationMode(m_operatorMode);

			// if bulk interface is set up
			if ( NULL != m_bulkPolledInterface )
			{
				// stop receiving remote requests
				m_bulkPolledInterface->setToStandby();
			}

			if (NULL != m_datanodeAccessInterface) 
			{
				m_datanodeAccessInterface->setToStandby();
			}
		}
	}
    ...
    void DataNodeAgent::agentSetControlInThread()
    {
		...
		// check the runtime parameter FailoverWait, default=1 millisecond
		unsigned long sleepSecs = 1;
		if ( true == TA_Base_Core::RunParams::getInstance().isSet ( "FailoverWait" ) )
		{
			unsigned long waitInMs = atol ( TA_Base_Core::RunParams::getInstance().get( "FailoverWait" ).c_str() );
			sleepSecs = waitInMs > 0 ? waitInMs : sleepSecs;
		}
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "DataNodeAgent has FailoverWait in milliseconds = %u", sleepSecs);
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Agent %s now in control mode.", m_agentName.c_str());
			...
			if ( updateEvent.getModifications() == TA_Base_Core::Create )
			{
                ...
                if (entityData->getType() == TA_Base_Core::DataNodeEntityData::getStaticType())
                {
                    ...
                    try
                    {
                        ...
                        dn->changeOperationMode( m_operatorMode );
                    }
					catch(TA_Base_Core::DatabaseException& dbe)
					{
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "Error creating DataNode %s (%ul)",

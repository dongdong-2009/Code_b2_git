/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/data_management/archive_manager/src/BurnDvdDlg.cpp $
...
namespace
{
	...
	const CString CaptionWriteComplete = "Write Complete";
}

namespace TA_Base_App
{
    
    BurnDvdDlg::BurnDvdDlg( const std::string& volumeLabel, const std::string& cdLabel, 
        const WriteArchiveHelper::FileDetailsVector& files, CWnd* pParent /*=NULL*/)
        : CDialog(BurnDvdDlg::IDD, pParent),
          m_volumeLabel( volumeLabel ),
          m_cdLabel( cdLabel ),
          m_files( files )
    {
        //{{AFX_DATA_INIT(BurnDvdDlg)
        m_csStage = _T("");
        m_csLog = _T("");
        //}}AFX_DATA_INIT
    }
    ...
    void BurnDvdDlg::DoDataExchange(CDataExchange* pDX)
    {
        CDialog::DoDataExchange(pDX);
        //{{AFX_DATA_MAP(BurnDvdDlg)
    ...
    BEGIN_MESSAGE_MAP(BurnDvdDlg, CDialog)
        //{{AFX_MSG_MAP(BurnDvdDlg)
	    ON_WM_DESTROY()
	//}}AFX_MSG_MAP
    ...
    void BurnDvdDlg::run()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                for( WriteArchiveHelper::FileDetailsVector::const_iterator it = m_files.begin(); it != m_files.end(); ++it )
                {
					...
					addFile( (*it).nameWithPath, strFile );
                }
                ...
                UINT selectedButton = userMsg.showMsgBox(IDS_UI_050046,CaptionWriteComplete);
        ...
        {
            LOG_EXCEPTION_CATCH( SourceInfo, "Unknown Exception", "Unknown exception caught in BurnDvdDlg::run()" ); 
        }
    }

    
    void BurnDvdDlg::terminate()
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // BurnDvdDlg message handlers
    
    BOOL BurnDvdDlg::OnInitDialog() 
    {
    ...
    LRESULT BurnDvdDlg::OnCloseDlg(WPARAM, LPARAM lparam)
    {
        // If the user has cancelled - don't go any further.
...
//*********************************** NERO
    ...
    {	
        ...
        {
            CString reason;
            reason.FormatMessage( UE050090_REASON_NO_DRIVES, "DVD-RW" );
            CString error;
            error.FormatMessage( UE050090_WRITE_FAILED, m_volumeLabel.c_str(), reason );
            TA_THROW( TA_Base_Core::ArchiveException( std::string( error ) ) );
        }
        ...
        appendStringToLogDisplay( "Drive " + chosenDrive + ": selected for archiving.");
    }


    void BurnDvdDlg::neroApiFree()
    {
    ...
    void BurnDvdDlg::doBurn()
    {
        appendStringToLogDisplay("Preparing to write DVD...");
...
//
        ...
        {
            m_stageStatic.SetWindowText( "DVD Write Successful" );
        }
        else
        {
            m_stageStatic.SetWindowText( "DVD Write Failed" );
            CString error;
            error.FormatMessage( UE050090_WRITE_FAILED, m_volumeLabel.c_str(), reason );
            TA_THROW( TA_Base_Core::ArchiveException( std::string( error ) ) );
        }
    ...
    void NERO_CALLBACK_ATTR BurnDvdDlg::neroAddLogLine(void *pUserData, NERO_TEXT_TYPE type, const char *text)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process Nero add log line callback." );
        }
        else
        {
    ...
    void NERO_CALLBACK_ATTR BurnDvdDlg::neroSetPhaseCallback(void *pUserData, const char *text)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process Nero set phase callback." );
            return;
        }
        else
        {
    ...
    void NERO_CALLBACK_ATTR BurnDvdDlg::neroSetMajorPhaseCallback(void *pUserData, NERO_MAJOR_PHASE phase,void *reserved)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process Nero set major phase callback." );
            return;
        }
        else
        {
    ...
    NeroUserDlgInOut NERO_CALLBACK_ATTR BurnDvdDlg::neroUserDialog(void *pUserData, NeroUserDlgInOut type, void *data)
    {
        switch (type)
        {
            ...
            case DLG_FILESEL_IMAGE:
                {
                    ...
                    {
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                            "Failed to cast void* to BurnDvdDlg*. Can't process Nero user dialog callback." );
                    }
                    else
                    {
    ...
    BOOL NERO_CALLBACK_ATTR BurnDvdDlg::neroIdleCallback(void *pUserData)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process BurnDvdDlg UI messages." );
    ...
    BOOL NERO_CALLBACK_ATTR BurnDvdDlg::neroProgressCallback(void *pUserData, DWORD dwProgressInPercent)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process Nero progress callback." );
            return false;  // Assume we're not aborting.
        }
    ...
    BOOL NERO_CALLBACK_ATTR BurnDvdDlg::neroAbortedCallback(void *pUserData)
    {
        TA_Base_App::BurnDvdDlg* dlg = static_cast< TA_Base_App::BurnDvdDlg* >( pUserData );
        if ( dlg == 0 )
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, 
                "Failed to cast void* to BurnDvdDlg*. Can't process Nero aborted callback." );
            return false;  // Assume we're not aborting.
        }
        if ( StateManager::STOPPING == dlg->m_stateManager.getState() )
        {
    ...
    void BurnDvdDlg::addFile( const std::string& fileNameWithPath, const std::string& fileName, bool isDirectory )
    {
    ...
    void BurnDvdDlg::appendStringToLogDisplay( const std::string& sAdd )
    {
	...
	std::string BurnDvdDlg::formatStringHelper( const std::string& strSrc)
	{

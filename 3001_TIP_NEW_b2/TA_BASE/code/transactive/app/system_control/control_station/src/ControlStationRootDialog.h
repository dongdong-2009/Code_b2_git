/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/ControlStationRootDialog.h $
  * @author:  Karen Graham
  * @version: $Revision: #7 $
  *
...
// structure for display related information (wchar_t)
struct DisplayInformationEx
{
	CStringW display;     // The name of the display
	CStringW asset;       // The identifier located on the display that is centred on the the screen
	long position;       // The position on the screen that this display is displayed on 
	// (the display will take up the entire screen area where this point is located)
};
...
protected:
	...
	afx_msg LRESULT onDisplayChangeDisplayModeDialogue(WPARAM, LPARAM);
	...
	afx_msg LRESULT onForceLogoutLostNetwork(WPARAM,LPARAM);
...
private:
	...
	static const CString PROFILE_DISPLAY_ONLY ;

	// Message that is sent to GraphWorx to tell it to change the colour of the 
	// operator/profile panel to indicate the system is in normal display mode.
	static const CString PROFILE_DISPLAY_NORMAL ;
	...
	bool m_fHasInitDialog; //whether call onInitDialog
...
class EventWorkItem: public TA_Base_Core::IWorkItem
{
public:
	EventWorkItem(ControlStationRootDialog* pDialog);
	virtual ~EventWorkItem();

	//operations specific to IWorkItem
	virtual void executeWorkItem();
private:
	ControlStationRootDialog* m_pDialog;

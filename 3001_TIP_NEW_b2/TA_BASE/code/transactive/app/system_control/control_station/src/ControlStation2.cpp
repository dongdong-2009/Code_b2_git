/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/ControlStation2.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #8 $
  *
  * Last modification: $DateTime: 2013/04/04 16:16:01 $
...
STDMETHODIMP CControlStation2::getLocation(BSTR* pRet)
{
	...
	CString strResult = SessionManager::getInstance().getSessionDetails().getLocationDisplayName().c_str();
...
STDMETHODIMP CControlStation2::changeDisplayMode()
{
	...
	ControlStationRootDialog::getInstance().PostMessage(WM_DISPLAY_CHANGE_DISPLAY_MODE_DIALOG, 0, 0);
...
STDMETHODIMP CControlStation2::setRunParam(BSTR paramName, BSTR paramValue)
{
	...
	_bstr_t name(paramName);
	_bstr_t value_(paramValue);

	TA_Base_Core::RunParams::getInstance().set(name,value_);
	
	//DebugLevel or debug-level,  EDebugLevel
	//TODO: debugUtil.getInstance().setLevel
	LPSTR pszParamName= OLE2A (paramName);

	std::string szParamName(pszParamName);

	std::remove(szParamName.begin(), szParamName.end(), '-');
	boost::to_upper(szParamName);
	if(szParamName.compare("DEBUGLEVEL") == 0)
	{
		DebugUtil::getInstance().setLevel(DebugUtil::getInstance().getDebugLevelFromString(value_));
	}
...
STDMETHODIMP CControlStation2::entityCommand(EntityMessageType entityType, BSTR location, BSTR sourceSchematicID, BSTR message)
{
	...
	if(strResult == locationStr)
	{
        ...
        delete commsMessageSender;
        commsMessageSender = NULL;
	}
...
STDMETHODIMP CControlStation2::getDisplayMode(BSTR* pRet)
{
	...
	BOOL displayOnlyMode = SessionManager::getInstance().getSessionDetails().getDisplayMode();
	CString strResult;
	if(displayOnlyMode)
	strResult = "DisplayOnly";
	else
	strResult = "DisplayNormal";

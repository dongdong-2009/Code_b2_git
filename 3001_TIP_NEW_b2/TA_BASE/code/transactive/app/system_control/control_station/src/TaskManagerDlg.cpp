/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/TaskManagerDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2012/12/13 16:12:35 $
	...
	else
	{
		...
		else
		{
            if (m_apps != NULL)
            {
                ...
                TA_Base_Core::RunParamSeq_var tmpParams = iter->second->getParameters();
                if ( NULL != tmpParams->get_buffer() )
                {
...
void CTaskManagerDlg::OnEndlabeleditListParameters(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pDispInfo->item.pszText != NULL)
	{
		...
		{
            TA_Base_Core::RunParamSeq_var tmpParams = new TA_Base_Core::RunParamSeq;
            if (tmpParams->get_buffer() == NULL)
            {
                *pResult = 0;
                return;
            }

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/SessionManager.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #32 $
  *
...
void SessionManager::cleanUp()
{
    ...
    ATSProfileHandler::clearProfile();
...
void SessionManager::login(const ProfileLoginDlg& loginDlg)
{
	...
	// [2011-11-09] if locked out, throw out
	LOG_GENERIC(SourceInfo,DebugUtil::DebugInfo,"[2011-11-11] SessionManager::login");
	...
	LOG_GENERIC(SourceInfo,DebugUtil::DebugInfo,"[2011-11-11] SessionManager::getLatestLockedTime done %d, %s",isLockedOut,cLockedTime.Format( "%Y%m%d %H:%M:%S" ));
	...
	LOG_GENERIC(SourceInfo,DebugUtil::DebugInfo,"[2011-11-11] SessionManager::first time login");
	...
	LOG_GENERIC(SourceInfo,DebugUtil::DebugInfo,"[2011-11-11] SessionManager::login done");
						  ...
						  bool displayOnly)
{
	...
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"The loginDate is %d ; the modifiedTimespan is :%d",loginDate.GetYear(),modifiedTimeSpan.GetDays() >= password_life_time);
	...
	if(TA_Base_Core::RunParams::getInstance().isSet("PasswordExpireNotificationSwitch"))
	{
		...
		LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"The passwordEditNotification status is %s", passwordEditNotification.c_str());
	...
	if(turnPasswordExpireNotificationOn)
	{
		if((loginDate.GetYear() == 1970) || (modifiedTimeSpan.GetDays() >= password_life_time) )
		{
			...
			{
				errorId = IDS_UE_620024;
				CString tmp = theOperator->getName().c_str();
				userMsg << tmp;

			}
			else if (modifiedTimeSpan.GetDays() >= password_life_time )
			{
				errorId = IDS_UE_620025;
			}
			userMsg.showMsgBox(errorId);
			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"[2011-11-16] PopUpChangePasswordDialog");

			bool isSuccessfulChange;

			do{
				LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"[2011-11-16] PopUpChangePasswordDialog => inWhileLoop");
				isSuccessfulChange = PopUpChangePasswordDialog();
			} while( !isSuccessfulChange);

			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"[2011-11-16] PopUpChangePasswordDialog Done");
		}
		else if((modifiedTimeSpan.GetDays() < password_life_time) && (modifiedTimeSpan.GetDays()>=(password_life_time - password_warn_time)))
		{
			TransActiveMessage userMsg;
			unsigned long errorId;
			errorId = IDS_UE_620026;
			std::ostringstream outputMessage;
			outputMessage<< ((LONGLONG)password_life_time - modifiedTimeSpan.GetDays());
			CString tmp = outputMessage.str().c_str();
			userMsg<<tmp;
			int resp = userMsg.showMsgBox(errorId);
			if(resp == IDOK)
			{
				LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"[2011-11-16] PopUpChangePasswordDialog => inWhileLoop");
    ...
    try
    { 
		LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] m_graphWorxManager->launchGraphWorx()");
        m_graphWorxManager->launchGraphWorx();
		LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] m_graphWorxManager->launchGraphWorx() Done");
    }
    catch( const GraphWorxException& )
    {
	...
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] Load Banner()");
    loadBanners();
    LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] Load Banner() Done");
    LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Load TADllHost()");
    loadDllHost();
    LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Load TADllHost() Done");
    // Submit an audit with the name of the console on which this application is running
    unsigned long entityKey = 0;
    try
    {
  ...
  /*  sender->sendAuditMessage( TA_Base_Core::ControlStationAudit::ControlStationLogin, // Message Type
	...
	sender->sendAuditMessage(TA_Base_Core::ControlStationAudit::ControlStationLogin, // Message Type
		...
		m_sessionDetails.getSessionInfo().operatorId,
		m_sessionDetails.getOperatorName(),
		m_sessionDetails.getSessionInfo().profileId,
...
void SessionManager::changePassword(const CPasswordDlg& passwordDlg)
{
    ...
    catch (const AuthenticationSecurityException& ex)
    {
		...
		else if(ex.getReason() == AuthenticationSecurityException::PASSWORD_USEDBEFORE)
		{
			...
			// We shouldn't really get this since we've checked the length of the password in CPasswordDlg, but
			// if for some reason it wasn't caught there, 
			TA_THROW(SessionException(SessionException::PASSWORD_USEDBEFORE, 
...
void SessionManager::logout()
{
    ...
    std::string consoleName = m_sessionDetails.getConsoleName();
    std::string userName = m_sessionDetails.getOperatorName();
	unsigned long consoleId = m_sessionDetails.getConsoleId();
	unsigned long operatorId = m_sessionDetails.getSessionInfo().operatorId;
	unsigned long profileId = m_sessionDetails.getSessionInfo().profileId;
   ...
   /* sender->sendAuditMessage( TA_Base_Core::ControlStationAudit::ControlStationLogout, // Message Type
                             ...
                             "" );                                          // Parent event ID - null because there's no parent event*/
	std::string value = "";
	sender->sendAuditMessage(
		TA_Base_Core::ControlStationAudit::ControlStationLogout,
		entityKey,
		0,
		0,
		0,
...
void SessionManager::loadBanners()
{
	...
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] loadBanners()");
    try
    {
        ...
        try
        {
			...
			*/
			...
			for (unsigned int i = 0; i < numBanners; i++)
            {
				...
				if (TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DISABLE_AUTOLAUNCH_RADIOMGR))
				{
					commandLine << " --";
					commandLine << RPARAM_DISABLE_AUTOLAUNCH_RADIOMGR;
				}
	...
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "[2011-11-09] loadBanners() leaving...");
...
bool SessionManager::PopUpChangePasswordDialog(){
	...
	*/
	...
	do
	{
		...
		{
			//BeginWaitCursor(); // display the hourglass cursor
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"SessionManager::PopUpChangePasswordDialog():changepassword");

			changePassword(m_passwordDialog);
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"SessionManager::PopUpChangePasswordDialog():changepassword successfully");

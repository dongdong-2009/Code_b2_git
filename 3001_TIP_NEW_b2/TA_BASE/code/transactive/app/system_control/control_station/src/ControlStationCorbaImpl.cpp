/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/ControlStationCorbaImpl.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #5 $
  *
...
void ControlStationCorbaImpl::launchDisplay(const char* display, const char* asset, long leftPosition)
{
    ...
    DisplayInformationEx* info = new struct DisplayInformationEx;

    if (info != NULL)
    {
        //info->display = CString(display);
        //info->asset = CString(asset);
        info->display = TA_Base_Core::CodeConverter::UTF8ToUnicode(display).c_str();
        info->asset = TA_Base_Core::CodeConverter::UTF8ToUnicode(asset).c_str();
...
/*
bool ControlStationCorbaImpl::isDisplayOnlyMode()
{ 
	LOG( SourceInfo, DebugUtil::FunctionEntry, "isDisplayOnlyMode");



	return SessionManager::getInstance().isDisplayOnly();

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/control_station/src/ControlStationRootDialog.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #26 $
  *
  * Last modification: $DateTime: 2015/03/19 11:26:38 $
...
#include "app\system_control\control_station\src\ATSProfileHandler.h"
...
const CString ControlStationRootDialog::PROFILE_DISPLAY_ONLY = "DisplayOnly";
const CString ControlStationRootDialog::PROFILE_DISPLAY_NORMAL = "DisplayNormal";
...
namespace
{
    ...
    public:
        ...
        BSTRProxy(const CString& str)
            : m_bstr( str.AllocSysString() )
        {
        }

		BSTRProxy(const CStringW& str)
            : m_bstr( str.AllocSysString() )
        {
        }
      ...
      CDialog(ControlStationRootDialog::IDD, NULL)
{
	...
	automatic_logout_time = SessionManager::getInstance().automatic_logout_time;
}
...
BEGIN_MESSAGE_MAP(ControlStationRootDialog, CDialog)
	...
	ON_MESSAGE(WM_DISPLAY_CHANGE_DISPLAY_MODE_DIALOG, onDisplayChangeDisplayModeDialogue)
	...
	ON_MESSAGE(WM_FORCE_LOGOUT_LOSTNETWORD, onForceLogoutLostNetwork)
...
BOOL ControlStationRootDialog::OnInitDialog() 
{
	...
	if(timerSwitch.compare("OFF") == 0 || timerSwitch.compare("off")==0 ||timerSwitch.compare("Off")==0)
	{
		m_shouldStartTimer = false;
		LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Timer is turned off");
	}
	else
	{
		m_shouldStartTimer = true;
		LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Timer is turned on");
	}
...
LRESULT ControlStationRootDialog::onDisplayLoginDialogue(WPARAM,LPARAM)
{
    ...
    do
    {
                ...
                case TA_Base_App::TA_ControlStation::DENIED:
						 {
							 ...
							 std::auto_ptr<TA_Base_Core::IOperator> iOperator(TA_Base_Core::OperatorAccessFactory::getInstance().getOperator(sessionInfo.operatorId, false));
        ...
        catch ( SessionException& exception)
		{
			...
			case SessionException::AUTHENTICATION_FAILED:
				{
					...
					std::auto_ptr<TA_Base_Core::IOperatorPassword> operatorPassword( TA_Base_Core::OperatorPasswordAccessFactory::getInstance().getOperatorPassword(theOperator->getKey(),false,false));
					...
					if(isLockedOut>=  SessionManager::getInstance().failed_login_attempts && timeSpan.GetTotalMinutes() <=  SessionManager::getInstance().password_lock_time)
					{
						errorId = IDS_UE_620022;
						MaxLoginTimes.Format("%d", SessionManager::getInstance().failed_login_attempts);
						ReuseTime.Format("%d",SessionManager::getInstance().password_lock_time);
						userMsg<< MaxLoginTimes;
						userMsg<< ReuseTime;
			...
			case SessionException::AUTHENTICATION_AGENT_FAILED:
				...
				break;
			case SessionException::USER_LOCKED: 
				{
					TA_Base_Core::IOperator* theOperator = NULL;
					TA_Base_Core::IProfile*  theProfile  = NULL;
					TA_Base_Core::ILocation*  theLocation  = NULL;
				...
				break;
			//REL-193-100 "A Control Station error has occurred. Contact the system administrator"
			case SessionException::GENERAL_ERROR:
				errorId = IDS_UE_020043;
				break;
			default:
	...
	if(m_shouldStartTimer == TRUE &&timerFlag == FALSE)
	{
		SetTimer(123123,60000,0);
		LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,  "onDisplayLoginDialogue()::Timer is successfully set up!" );
		timerFlag = TRUE;
	}
...
LRESULT ControlStationRootDialog::onDisplayChangeProfileDialogue(WPARAM,LPARAM)
{
    ...
    {
        m_comControlStation->Fire_shortMessage(BSTRProxy(MENU_SUSPEND_MESG));
    }
    ...
    do
    {
		...
		try
		{
			...
			loginDetailsChanged();
			ATSProfileHandler::updateProfile();
	...
	if (m_comControlStation != NULL)
	{
		m_comControlStation->Fire_shortMessage(BSTRProxy(MENU_RESUME_MESG));
	}
...
LRESULT ControlStationRootDialog::onDisplayChangeDisplayModeDialogue(WPARAM,LPARAM)
{
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Change Display Mode");
	// TD 8214 check if window is already active
	if (m_loginDialog.m_hWnd != NULL)
	{
		// already open
	...
	CString actionName;
	TA_VERIFY(actionName.LoadString(IDS_CHANGE_DISPLAY_MODE)!=0, "Unable to load IDS_CHANGE_DISPLAY_MODE");
	TransActiveMessage userMsg;
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Change Display Mode:TransActiveMessage");
	...
	LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Change Display Mode:m_comControlStation");
	// We got rights to do this, so continue
	...
	do
	{
		...
		if (response == IDCANCEL)
		{
			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "User has cancelled the change display mode");
			break;
		}
		...
		try
		{
			...
			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Change Display Mode:try");
			BeginWaitCursor(); // display the hourglass cursor
			SessionManager::getInstance().changeProfile(m_loginDialog);
			loginDetailsChanged();
			ATSProfileHandler::updateProfile();
			...
			LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Change Display Mode:try Done");
		}
		catch ( const SessionException& exception)
		{
			userMsg << actionName;
	...
	LOG( SourceInfo, DebugUtil::FunctionExit, "onDisplayChangeDisplayModeDialogue");
	return S_OK;
}
...
LRESULT ControlStationRootDialog::onForceLogoutLostNetwork(WPARAM,LPARAM)
{
	static bool isShowing = false;
	LOG( SourceInfo, DebugUtil::FunctionEntry, "onForceLogoutLostNetwork");
	...
	CString sessionType;
	TA_VERIFY(sessionType.LoadString(IDS_LOGIN)!=0, "Lost network");
	TransActiveMessage userMsg;
	userMsg << sessionType;
	userMsg.showMsgBox(IDS_UE_020052);
	...
	logout();
	isShowing = false;
	LOG( SourceInfo, DebugUtil::FunctionExit, "onForceLogoutLostNetwork");
	return S_OK;
}
...
LRESULT ControlStationRootDialog::onLaunchDisplay(WPARAM display, LPARAM )
{
    ...
    DisplayInformationEx* info = reinterpret_cast<DisplayInformationEx*>(display);
    ...
    if (m_comControlStation != NULL)
    {
        ...
        m_comControlStation->Fire_loadDisplayEx(BSTRProxy(info->display), BSTRProxy(info->asset), info->position);
        m_comControlStation->Fire_loadDisplay(BSTRProxy(info->display), info->position);
        delete reinterpret_cast<DisplayInformationEx*>(info);
    }
...
LRESULT ControlStationRootDialog::onSystemControllerStateChange(WPARAM isOnline, LPARAM group)
{
    ...
    if(m_comControlStation != NULL)
    {
        if(isOnline)
        {
            ...
            m_comControlStation->Fire_shortMessage(BSTRProxy(SYSTEM_NORMAL_MESG));
        }
        else
        {
			if( m_soundInterface != NULL)
            ...
            m_comControlStation->Fire_shortMessage(BSTRProxy(SYSTEM_DEGRADED_MESG));
        }
    }
	...
	if (1 == nSeqFlag)
	{
		...
		DutyChangeNoteDlg* pNoteDlg = new DutyChangeNoteDlg();
				if (pNoteDlg->DoModal() == IDCANCEL)
				{
					...
					//[2011-11-10] m_comControlStation->Fire_dutyChanged();
...
{    
    ...
    if (m_comControlStation != NULL)
    {
        ...
        m_comControlStation->Fire_gxMessage(BSTRProxy(pGxMsg->targetSchematicID), BSTRProxy(pGxMsg->sourceEntityName), BSTRProxy(pGxMsg->message));
    }
...
void ControlStationRootDialog::loginDetailsChanged()
{
    ...
    if (m_comControlStation != NULL)
    {
		...
		BOOL displayOnly =  SessionManager::getInstance().getSessionDetails().getDisplayMode();
											  
		if (displayOnly)
        {
			m_comControlStation->Fire_loginDetailsChanged(BSTRProxy(operatorName), BSTRProxy(profileName), BSTRProxy(PROFILE_DISPLAY_ONLY));
        }
		else
        {
			m_comControlStation->Fire_loginDetailsChanged(BSTRProxy(operatorName), BSTRProxy(profileName), BSTRProxy(PROFILE_DISPLAY_NORMAL));
        }
	...
	//SessionManager::getInstance().getSessionDetails().getOperatorName()
	...
	{
		SetTimer(123123,60000,0);
		LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,  "Timer is successfully set up!" );
		timerFlag = TRUE;
	}
...
void ControlStationRootDialog::OnTimer(UINT nIDEvent) 
{
	...
	{
		LOG_GENERIC(SourceInfo, DebugUtil::DebugError,"Failed to get the time of last input event!");
		return;
	}
	...
	if(noActionTimePeriod>= automatic_logout_time*60000)
	{	
		if(SessionManager::getInstance().getLoginStatus()==TRUE)
		{
			KillTimer(123123);
			timerFlag = false;
			std::ostringstream outputMessage;
			outputMessage<<"No action in "<<automatic_logout_time<<" minutes!";
			//AfxMessageBox(_T(outputMessage.str().c_str()));
			std::string msg = outputMessage.str();
			LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "auto logout due to %s",msg.c_str() );

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/system_controller/src/ProcessManager.cpp $
...
using TA_Base_Core::IManagedProcessData;
using TA_Base_Core::ProcessData;
using TA_Base_Core::ServerEntityData;
using TA_Base_Core::IEntityData;
using TA_Base_Core::IProcess;
...
namespace TA_Base_App
{
    ...
    void ProcessManager::setupDbMonitors(int& pollPeriod)
    {
		...
		{
			std::string DbConnectionInfo = RunParams::getInstance().get(RPARAM_MONITOREDDATABASE);
			...
			if (!connectInfo[4].empty())					
				m_monitoredDbName = (connectInfo[1] + "@" + connectInfo[4]);
			else
				m_monitoredDbName = connectInfo[1];
		}
		...
		{
			std::string DbConnectionInfo = RunParams::getInstance().get(RPARAM_CENTRALDATABASE);
			m_moniteredDatabases.insert( std::make_pair( ( DbConnectionInfo ), (TA_Base_Bus::SimpleDbStatusChecker *)0 ) );
			...
			if (!connectInfo[4].empty())					
				m_centralDbName = (connectInfo[1] + "@" + connectInfo[4]);
			else
				m_centralDbName = connectInfo[1];
		}
    ...
    // getAllProcesses
    //
    void ProcessManager::getAllProcesses( std::vector<TA_Base_Core::ProcessData>& processDataList, bool isLockFirst /* = true */ )
    {
        FUNCTION_ENTRY( "getAllProcesses" );

        processDataList.clear();
        processDataList.resize( m_processes.size() );

        int i = 0;
        for ( EntityToProcessMap::iterator iter = m_processes.begin(); iter != m_processes.end(); ++iter, ++i )
        {
           iter->second->getProcessData( processDataList[ i ], isLockFirst );
        }
    ...
    TA_Base_Core::AgentStatusEnum ProcessManager::getAgentStatusSummary()
    {
        ...
        std::map<unsigned long, TA_Base_Core::EOperationMode> operationModeMap;

        for ( EntityToProcessMap::iterator it = m_processes.begin(); it != m_processes.end(); ++it )
        {
            ManagedProcessInfo* process = dynamic_cast<ManagedProcessInfo*>( it->second );
            ...
            if ( TA_Base_Core::Standby == process->getNormalOperationMode() )
            {
                continue;
            }

            if ( true == process->isServiceProcess() )
            {
                if ( process->getOperationMode() != process->getNormalOperationMode() )
                {
                    return TA_Base_Core::Abnormal;
                }

                continue;
            }

            TA_Base_Core::EProcessStatus status = process->getProcessStatus();

            if ( status == TA_Base_Core::Unstarted ||
                 status == TA_Base_Core::Startup ||
                 status == TA_Base_Core::Terminating ||
                 status == TA_Base_Core::NotRunning ||
                 status == TA_Base_Core::Stopped )
            {
                return TA_Base_Core::AgentFault;
            }

            unsigned long locationKey = process->getLocationKey();
            TA_Base_Core::EOperationMode operationMode = process->getOperationMode();

            std::map<unsigned long, TA_Base_Core::EOperationMode>::iterator findIt = operationModeMap.find( locationKey );

            if ( findIt == operationModeMap.end() )
            {
                operationModeMap.insert( std::make_pair( locationKey, operationMode ) );
                continue;
            }

            if ( findIt->second != operationMode )
            {
                return TA_Base_Core::Abnormal;
            }
        }
...
//         }
    ...
    void ProcessManager::notifyProcessData(const TA_Base_Core::ProcessData& peerProcessData)
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Received data about process %s",
                    entity.c_str());
        LOGMORE(SourceInfo, "Time started: %lu", peerProcessData.started);
        LOGMORE(SourceInfo, "Host: %s", peerHost.c_str());
        LOGMORE(SourceInfo, "State: %d", peerProcessData.state);
        LOGMORE(SourceInfo, "Requested State: %d", peerProcessData.requestedState);
        LOGMORE(SourceInfo, "Operation mode: %d", peerProcessData.operationMode);
        LOGMORE(SourceInfo, "Restarts: %d", peerProcessData.restarts);
        LOGMORE(SourceInfo, "Debug level: %s", (const char*)peerProcessData.debugLevel);
        ...
        if (m_processes.find(entity) != m_processes.end())
        {
            ...
            if (localProcess->isAPeer(peerHost) && localProcess->isRunning())
            {
                    ...
                    peerProcessData.operationMode == TA_Base_Core::Control)
                {
					...
					handleTwoAgentsInControl(localProcess);
				}
...
// 			// ++Yanrong
	...
	//
	void ProcessManager::handleTwoAgentsInControl(ManagedProcessInfo* agent)
	{
			...
			agent->getNormalOperationMode() == TA_Base_Core::Control)
		{
			...
			LOG_GENERIC(SourceInfo, DebugUtil::DebugError,
				"Both %s processes in Control. Telling peer to switch modes.",
		...
		else
		{
			...
			LOG_GENERIC(SourceInfo, DebugUtil::DebugError,
				"Both %s processes in Control. Switching to Monitor",
				agent->getEntityName().c_str());
...
// 			}
    ...
    void ProcessManager::registerWithPeer(const std::string& host)
    {
        ...
        std::ostringstream peerProcessManagerString;
        peerProcessManagerString << "corbaloc::" << host << ":"
                                 << m_port << "/PeerProcessManager";

        // Attempt to resolve an IProcessSubjectCorbaDef object
        try
        {        
            obj = CorbaUtil::getInstance().stringToObject(peerProcessManagerString.str());
        ...
        TA_Base_Core::IProcessSubjectCorbaDef_var peer;
        {
            ...
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugError,
                    "%s's PeerProcessManager is nil", host.c_str());
                return;
            }
            else
            {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,
            "Registering with peer %s", host.c_str());
        ...
        try
        {
			...
			TA_Base_Core::IProcessObserverCorbaDef_var observer // gongzong++ CL-21303
			= peer->registerProcessObserver(m_myHostname.c_str(), this->_this());
...
// 					}
    ...
    TA_Base_Core::RunParams::ParamVector ProcessManager::getCommonParams()
    {
		...
		{
			std::vector<std::string> connectInfo;
			SimpleDb::ParseConnectStr( itr->first, connectInfo);
			std::string dbName (RPARAM_DBPREFIX);
			//dbName += connectInfo[1];
			if (!connectInfo[4].empty())					
				dbName += (connectInfo[1] + "@" + connectInfo[4]);
	...
	void ProcessManager::initRunParams()
	{
		...
		std::vector<std::string> connectInfo;
        SimpleDb::ParseConnectStr(dbConnectionStr, connectInfo);
		m_controlDbName = connectInfo[1];
    ...
    void ProcessManager::notifyPeers(const std::string& entity)
    {
        ...
        {
            // Process is in map
            TA_Base_Core::ProcessData data;
            iter->second->getProcessData( data );
    ...
    std::vector< std::pair< std::string, std::string> > ProcessManager::getDatabaseStatus() const
    {
        ...
        const std::string DBPREFIX = RPARAM_DBPREFIX;

        for ( MoniteredDbs::const_iterator it = m_moniteredDatabases.begin(); it != m_moniteredDatabases.end(); ++it )
        {
            // LIMINTODO: refactor
            std::vector<std::string> connectInfo;
            SimpleDb::ParseConnectStr(it->first, connectInfo);
            std::string dbRunParam = DBPREFIX;
			if (!connectInfo[4].empty())					
				dbRunParam += (connectInfo[1] + "@" + connectInfo[4]);
			else
				dbRunParam += connectInfo[1];

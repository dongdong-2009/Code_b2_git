/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/system_controller/src/SystemControllerMonitor.cpp $
  * @author:  Justin Ebedes
  * @version: $Revision: #9 $
  * Last modification: $DateTime: 2017/03/03 10:42:07 $
...
namespace TA_Base_App
{
	...
	void SystemControllerMonitor::run()
    {
		...
		struct timeb beforeTime;          // Time before checking for failures
        struct timeb afterTime;           // Time after checking for failures
		...
		//TD15949++
		unsigned int ulHeartBeatSendPeriodSecs = ((m_heartbeatSendPeriod / 1000) * m_heartbeatSendNumber); // Heartbeats must be received within double the heartbeat rate.
		//++TD15949
        ...
        while (m_isRunning)
        {
            // Get the time before checking for failures.
        	ftime( &beforeTime );
            ...
            // Get the time after checking for failures.
		    ftime( &afterTime );

            // Sleep for the right amount of time - careful not to overflow...
            diffTimeMsecs = afterTime.time - beforeTime.time;
            diffTimeMsecs *= 1000;
            diffTimeMsecs += (afterTime.millitm - beforeTime.millitm);
            // change sleep interval to 1 poll interval instead of 2 poll intervals
            // sleepTimeMsecs = (m_heartbeatSendPeriod * 2) - diffTimeMsecs;
            ...
            LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "Would be sleeping for %ld milliseconds (%ld.%u - %ld.%u)", 
                sleepTimeMsecs, afterTime.time, afterTime.millitm, beforeTime.time, beforeTime.millitm);
			...
			if ( diffTimeMsecs < ( m_heartbeatSendPeriod * m_heartbeatSendNumber ) )
			{
				//Processing time falls within the heartbeat interval

				//sleep(m_heartbeatSendPeriod * 2);
			...
			else
			{
				...
				LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn, "Checking for failed groups took longer than %d seconds(heat beat send timeout). Actual processing time took %d seconds",ulHeartBeatSendPeriodSecs, sleepTimeMsecs);
...
//         m_hasGroupFailed = currentFailedGroups;
    ...
    void SystemControllerMonitor::raisePeerFailedAlarm(const std::string& hostname)
    {
        ...
        try
        {
            ...
            boost::shared_ptr<TA_Base_Core::IEntityData> alarmEntity( TA_Base_Core::EntityAccessFactory::getInstance().getEntity( makeAlarmEntityName( hostname ) ) );
            ...
            TA_Base_Core::AlarmHelperManager::getInstance().getAlarmHelper().submitAlarm(
                    ...
                    alarmEntity->getKey(), alarmEntity->getTypeKey(), dp, 
                    alarmEntity->getName(), alarmEntity->getLocation(), 
    ...
    void SystemControllerMonitor::raiseGroupFailedAlarm(const std::string& groupName)
    {
        ...
        try
        {
	        ...
	        dp.push_back(&name);
            boost::shared_ptr<TA_Base_Core::IEntityData> alarmEntity( TA_Base_Core::EntityAccessFactory::getInstance().getEntity( makeAlarmEntityName( groupName ) ) );

            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
				"raise GroupCommunicationsFailure alarm for group fail: key:%d, typekey:%d, location:%d, subsystem:%d",
				alarmEntity->getKey(), alarmEntity->getTypeKey(), alarmEntity->getLocation(), alarmEntity->getSubsystem());
    ...
    void SystemControllerMonitor::closePeerFailedAlarm(const std::string& hostname)
    {
        ...
        {
            boost::shared_ptr<TA_Base_Core::IEntityData> alarmEntity( TA_Base_Core::EntityAccessFactory::getInstance().getEntity( makeAlarmEntityName( hostname ) ) );
            TA_Base_Core::AlarmHelperManager::getInstance().getAlarmHelper().closeAlarmAtLocation( TA_Base_Core::SystemAlarms::GroupCommunicationsFailure, alarmEntity->getKey(), alarmEntity->getLocation() );
        }
        catch(TA_Base_Core::DataException& ex)
        {
            LOG_EXCEPTION_CATCH(SourceInfo, "DataException", ex.what());
    ...
    void SystemControllerMonitor::closeExistingAlarms()
    {
        FUNCTION_ENTRY("closeExistingAlarms");

        struct CloseExistingAlarmsThread : TA_Base_Core::Thread
        {
            CloseExistingAlarmsThread( SystemControllerMonitor& monitor )
                : m_scm( monitor )
            {
                start();
            }

            ~CloseExistingAlarmsThread()
            {
                terminateAndWait();
            }
            ...
            virtual void run()
            {
                sleep( m_scm.m_heartbeatSendPeriod * m_scm.m_heartbeatSendNumber + 1000 );

                for ( unsigned int i = 0; i != m_scm.m_groupMonitors.size(); ++i )
                {
                    if ( false == m_scm.m_hasGroupFailed[i] )
                    {
                        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
                            "Closing existing alarms for group: %s.", m_scm.m_groupMonitors[i]->getGroupName().c_str()); 
                        m_scm.closeGroupFailedAlarm( m_scm.m_groupMonitors[i]->getGroupName() );
                    }
                }
            }

            virtual void terminate() {}
            SystemControllerMonitor& m_scm;
        };

        if ( m_groupMonitors.size() )
        {
            static CloseExistingAlarmsThread theThread( *this );
        }

        FUNCTION_EXIT;
    }

    std::string SystemControllerMonitor::makeAlarmEntityName( const std::string& groupOrPeerName )
    {
        return "Alarm." + m_myHostname + "_" + groupOrPeerName;
    }

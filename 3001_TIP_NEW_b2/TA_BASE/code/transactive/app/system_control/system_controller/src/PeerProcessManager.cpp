/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/system_controller/src/PeerProcessManager.cpp $
  * @author:  Justin Ebedes
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2013/10/29 18:01:42 $
...
namespace TA_Base_App
{
	...
	{
		{
			...
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
						"Peer %s registered for process updates", host.c_str()),
			LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Number of observers is now %d",
						m_observers.size());
        ...
        ProcessManager::getInstance().getAllProcesses( processes );
         
        for (std::vector<TA_Base_Core::ProcessData>::iterator iter = processes.begin();
    ...
    void PeerProcessManager::sendProcessUpdate(const TA_Base_Core::ProcessData& processData)
    {
...

		{
			...
			for (std::map<std::string, TA_Base_Core::IProcessObserverCorbaDef_ptr>::iterator iter = observerMap.begin(); 
				iter != observerMap.end(); iter++)
			{
				LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, 
					"Sending data about process %s to peer %s",
							(const char*)processData.ProcessId, iter->first.c_str());
				LOGMORE(SourceInfo, "Time started: %lu", processData.started);
				LOGMORE(SourceInfo, "Host: %s", (const char*)processData.host);
				LOGMORE(SourceInfo, "State: %d", processData.state);
				LOGMORE(SourceInfo, "Requested State: %d", processData.requestedState);
				LOGMORE(SourceInfo, "Operation mode: %d", processData.operationMode);
				LOGMORE(SourceInfo, "Restarts: %d", processData.restarts);
				LOGMORE(SourceInfo, "Debug level: %s", (const char*)processData.debugLevel);
        ...
        for (std::vector<std::string>::iterator failIter = failedObservers.begin();
            failIter != failedObservers.end(); failIter++)
        {

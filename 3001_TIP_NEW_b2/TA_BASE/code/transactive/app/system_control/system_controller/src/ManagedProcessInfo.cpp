/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/system_controller/src/ManagedProcessInfo.cpp $
  * @author:  Justin Ebedes
  * @version: $Revision: #11 $
  *
  * Last modification: $DateTime: 2014/08/14 10:34:27 $
...
// 	}
	...
	void ManagedProcessInfo::setPeerState(std::string hostName, EPeerState state)
	{
        ...
        {
            // A peer is in the RunningMonitor state.
            m_peerState = RunningInMonitor;
        }
        else if (state == StartingInMonitor)
        {
            // A peer is starting up in Monitor.
        ...
        {
            // A peer is in the RunningStandby state.
            m_peerState = RunningInStandby;
        }
        else if (state == StartingInStandby)
        {
            // A peer is starting up in Standby.
			...
			if (stateReportedSuccessfully)
			{
				if (getProcessStatus() == TA_Base_Core::RunningStandby && true == m_isStandbyProcess)
				{
					if(true == m_isGroupOffline)
					{
						...
						if( m_shouldAutoFailback && getNormalOperationMode() == TA_Base_Core::Control)
						{
							// We've restarted in Standby, and we're configured to autofailback.

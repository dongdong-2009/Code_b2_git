/**
  ...
  * 
  * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/system_control/system_status_agent/src/pingDriver/PingableDataPointDriver.cpp $
  * @author:   Chris DeWolfe
  * @version:  $Revision: #4 $
  *
  * Last modification: $DateTime: 2013/10/11 13:52:51 $
...
namespace TA_Base_App
{
    ...
    void PingableDataPointDriver::doPostPollUpdate()
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::doPostPollUpdate - begin." );

        size_t i = 0;
        for ( IterToDp iter = m_dataPoints.begin(); iter != m_dataPoints.end(); ++iter, ++i )
        {
            PingablePoint* pPoint = dynamic_cast<PingablePoint*>((*iter));
            ...
            if ( ! ( policy->pointIsPingable() ) )
            {
                ...
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                    "PingableDataPointDriver::doPostPollUpdate - Setting ping point to false: index=%d, %s.", i, getLogString( pPoint ).c_str() );
            }
            else //if the we can ping the point...
            {
                ...
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                    "PingableDataPointDriver::doPostPollUpdate - Setting ping point to true: index=%d, %s.", i, getLogString( pPoint ).c_str() );
            }
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::doPostPollUpdate - end." );
    ...
    void PingableDataPointDriver::pollPoints()
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - begin." );
        ...
        while ( responsesReceived < expectedResponses )
        {
            ...
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                "PingableDataPointDriver::pollPoints - waitForOneIO() ing. data point index=%d, responses received=%d, retries=%d, total data points=%d.",
                dataPointIndex, responsesReceived, numRetries, expectedResponses );

            if ( TA_Base_Core::SocketSet<TA_Base_Core::IDatagramSocket>::waitForOneIO( &m_socket,
                                                                                       true,
                                                                                       numRetries < PING_RETRIES_PER_CYCLE,
                                                                                       canRead,
                                                                                       canWrite,
                                                                                       200 ) )
            {
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                    "PingableDataPointDriver::pollPoints - waitForOneIO() succeed. canRead=%d, canWrite=%d.", canRead, canWrite );
                ...
                if (canWrite)
                {
                    ...
                    while ( !triedToSendOne )
                    {
                        ...
                        if ( !pPoint->hasBeenProcessed() )
                        {
                            ...
                            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                "PingableDataPointDriver::pollPoints - ECHO send() ing. m_pid=%d, index=%d, %s.", m_pid, dataPointIndex, getLogString(pPoint).c_str() );
                            ...
                            if ( !m_socket.send( ina, datagram ) )
                            {
                                ...
                                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                    "PingableDataPointDriver::pollPoints - ECHO send() failed. m_pid=%d, index=%d, %s.", m_pid, dataPointIndex, getLogString(pPoint).c_str() );
                            ...
                            {
                                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                    "PingableDataPointDriver::pollPoints - ECHO send() secceed. m_pid=%d, index=%d, %s.", m_pid, dataPointIndex, getLogString(pPoint).c_str() );
                            }
                ...
                if (canRead)
                {
                    ...
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - ECHO receive() ing." );
                        ...
                        if ( TA_Base_Core::ICMPMessageCodec::getInstance()->decodeECHO( datagram, id, index ) )
                        {
                            ...
                            if ( id == m_pid && index < NUM_ENTITIES )
                            {
                                ...
                                if ( !pPoint->hasBeenProcessed() )
                                {
                                    ...
                                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                        "PingableDataPointDriver::pollPoints - ECHO receive() succeed. m_pid=%d, index=%d, %s.", m_pid, index, getLogString(pPoint).c_str() );
                                ...
                                {
                                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                        "PingableDataPointDriver::pollPoints - ECHO receive() succeed, but duplicate. m_pid=%d, index=%d, %s.", m_pid, index, getLogString(pPoint).c_str() );
                                }
                            }
                            else
                            {
                                ...
                                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                                    "PingableDataPointDriver::pollPoints - ECHO receive() succeed, but decodeECHO() got bad id=%u or bad seq=%u.", id, index);
                            }
                        }
                        else
                        {
                            // this isn't necessarily bad. It could be someone else's packet.
                            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - ECHO receive() succeed, but decodeECHO() got bad checksum.");
                        }
                    }
                    else
                    {
                        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - ECHO receive() failed.");
                    }
                } //end : if(canRead)
            ...
            else //if socket is not ready to either read or write...
            {
                ...
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                    "PingableDataPointDriver::pollPoints - waitForOneIO() failed. current=%f, sentTime=%f, maxTime=%f, current-sentTime=%f.",
                    current, sentTime, maxTime, current - sentTime );

                //if m_pingTimeoutSeconds has elapsed sence all were sent...
                if ( current - sentTime >= maxTime )
                {
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - timed out.");
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - total failed datapoints=%d.", NUM_ENTITIES - responsesReceived );
            ...
            for ( unsigned int i = 0; i < NUM_ENTITIES; i++ )
            {
                ...
                if ( !pPoint->hasBeenProcessed() )
                {
                    ...
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                        "PingableDataPointDriver::pollPoints - failed datapoint details: m_pid=%d, index=%d, %s.", m_pid, i, getLogString(pPoint).c_str() );
                }
            }
        }

        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "PingableDataPointDriver::pollPoints - end." );
    ...
    const std::string& PingableDataPointDriver::getLogString( PingablePoint* pPoint )
    {
        if ( pPoint != NULL && pPoint->getDataPoint() != NULL )
        {
            static std::map<unsigned long, std::string> pointStringMap; // single thread, no lock

            unsigned long entityKey = pPoint->getDataPoint()->getEntityKey();
            std::map<unsigned long, std::string>::iterator findIt = pointStringMap.find( entityKey );

            if ( findIt == pointStringMap.end() )
            {
                std::stringstream strm;
                strm
                    << "key=" << entityKey << ", "
                    << "name=" << pPoint->getDataPoint()->getDataPointName() << ", "
                    << "hostname=" << pPoint->getInternetAddressRef().getHostName() << ", "
                    << "ip address=" << inet_ntoa( pPoint->getInternetAddressRef().getIPAddress() );

                findIt = pointStringMap.insert( std::make_pair( entityKey, strm.str() ) ).first;
            }

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/configuration/configuration_editor/src/ConfigurationEditorSelectionBar.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2012/02/29 18:44:33 $
...
namespace TA_Base_App
{
		  ...
		  m_parentId(parentId)
    {
        ...
        m_wndSelectionBar.Create(WS_CHILD|WS_VISIBLE, CRect(0,0,0,0), &parentSplitter,parentId,
		...
		m_wndSelectionBar.AddFolder("loading...", -1);//add one temp folder
        FUNCTION_EXIT;
    ...
    void ConfigurationEditorSelectionBar::populateSelectionBar()
    {
             ...
             ++iter)
        {
            ...
            HICON tempIcon = iter->componentInterface->getIcon();
            ...
            std::map<CString, int>::iterator matchingFolder = foldersAndIds.find( iter->category );
            ...
            if ( matchingFolder == foldersAndIds.end() )
            {
                ...
                matchingFolderNumber = m_wndSelectionBar.AddFolder(iter->category,-1);
                foldersAndIds.insert( std::map<CString, int>::value_type( iter->category, matchingFolderNumber ) );
            }
            else
            {
            ...
            int position = m_wndSelectionBar.InsertItem(matchingFolderNumber,-1,iter->component,iconPosition,reinterpret_cast<DWORD>(&(*iter)));

            matchingFolderNumber--; // limin++ components are wrongly disabled, because the temp folder before and will be removed later.
            m_wndSelectionBar.DisableItem(position, matchingFolderNumber, !iter->isAccessible);

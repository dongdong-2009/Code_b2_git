/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/configuration/configuration_editor/src/MainFrm.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #10 $
  *
  * Last modification: $DateTime: 2014/01/08 17:58:57 $
...
//#ifdef TDS_DEVELOP
	//for TDS CE
	const std::string FILENAME_PREFIX = "Config_Update";
	const std::string LOGDIR_MFT = "c:\\transactive\\logs\\";
	const std::string LOGDIR_SERVER = "./patch/";
//#endif
...
BEGIN_MESSAGE_MAP(CMainFrame, TransActiveFrame)
	...
	//}}AFX_MSG_MAP
//#ifdef TDS_DEVELOP
	ON_COMMAND(ID_TDSLOG_NEW, onTDSLogNew)
//#endif
...
END_MESSAGE_MAP()
    ...
    CMainFrame::CMainFrame() : m_toolBar(NULL),
						       ...
						       m_selectionBar(NULL)
    {
        ...
        properties.minWidth = 600;
        properties.minHeight = 600;
        setResizingProperties(properties);	

//#ifdef TDS_DEVELOP
		time(&m_clickTime);
		time(&m_lastClickTime);
		if (isWorkingOnTdsMode())
			TA_Base_Core::RunParams::getInstance().set(RPARAM_TDSCE, "");

		m_bInitComeplete = false;
//#endif
...
/*
        ...
        //
        m_rightsChecker = new RightsChecker();
    ...
    long CMainFrame::onOutbarNotify(WPARAM wParam, LPARAM lParam)
    {
        ...
        switch (wParam)
	    {
		    case NM_FOLDERCHANGE:
				...
				/*
//#ifdef TDS_DEVELOP
				//Now care for folder change for TDS
				if ((TA_Base_Core::RunParams::getInstance().isSet(RPARAM_TDSCE)) && (m_bInitComeplete==true))
				{
					{
						if(lParam==6)
						{
							m_toolBar->showButton(ID_TDSLOG_NEW,false);
							LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Show TDS button");
						} 
						else
						{
							m_toolBar->showButton(ID_TDSLOG_NEW,true);
							LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Hide TDS button");
						}
					}
				}
//#endif
    ...
    long CMainFrame::onSessionUpdate(WPARAM wParam, LPARAM lParam)
    {
        ...
        TA_Base_Bus::TransActiveMessage userMsg;
        UINT selectedButton = userMsg.showMsgBox(IDS_UW_010005);
    ...
    bool CMainFrame::canReloadOrShutdown()
    {
        ...
        //UW-030017
        TA_Base_Bus::TransActiveMessage userMsg;
        userMsg << itemsNotApplied;
        UINT selectedButton = userMsg.showMsgBox(IDS_UW_030017);
...
//#ifdef TDS_DEVELOP
	void CMainFrame::onTDSLogNew()
	{
		if (TA_Base_Core::RunParams::getInstance().isSet(RPARAM_TDSCE))
		{
			m_lastClickTime = m_clickTime;
			time(&m_clickTime);

			try
			{
				std::string fileNameOracle = WriteTDSLog(0);			// Write oracle sql script to file
                std::string fileNameMysql = WriteTDSLog(1);   // Write mysql sql script to file

/*				if(UploadSQLFile(fileName) == true)
				{
					AfxMessageBox("SQL file created successfully!");
				}
				else
				{
					AfxMessageBox("SQL file upload failed!");
				}
*/				eraseTDSLOG_FromDB();
				std::string fullOracleFileName = LOGDIR_MFT + fileNameOracle;
				std::string fullMysqlFileName = LOGDIR_MFT + fileNameMysql;
				std::string str = "SQL file created successfully on: " + fullOracleFileName + "," + fullMysqlFileName;
				AfxMessageBox(str.c_str());

/*				if( remove( fullFileName.c_str() ) == -1 )
					LOG_GENERIC(SourceInfo, DebugUtil::DebugError,"Could not delete %s", fullFileName.c_str());
				else
					LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo,"Deleted %s", fullFileName.c_str());
*/
			}
			catch (TA_Base_Core::TransactiveException* e)
			{
				AfxMessageBox("Cannot create sql script file");
...
//#ifdef TDS_DEVELOP
		m_bInitComeplete = true;
//#endif
		FUNCTION_EXIT;
	}

//#ifdef TDS_DEVELOP
	//get filename only, add by hongzhi
	const std::string CMainFrame::GetFileName(const std::string& suffix)
	{
		// construct full name
		std::ostringstream fileName;
		fileName << "";

		struct tm from = *localtime(&m_lastClickTime);
		struct tm to = *localtime(&m_clickTime);
		fileName << FILENAME_PREFIX
			<< "_from_"
			<< (1900 + from.tm_year) << "-" << (1 + from.tm_mon) << "-" << from.tm_mday << "-" << from.tm_hour << "-" << from.tm_min << "-" << from.tm_sec
			<< "_to_"
			<< (1900 + to.tm_year) << "-" << (1 + to.tm_mon) << "-" << to.tm_mday << "-" << to.tm_hour << "-" << to.tm_min << "-" << to.tm_sec
            << suffix
			<< ".sql";
	...
	std::string CMainFrame::WriteTDSLog(int dbType)
	{
		std::string fileName = GetFileName(0 == dbType ? "_oracle": "_mysql");
		std::string fullFileName = LOGDIR_MFT + "\\" + fileName;
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Write TDS to sql to log file: %s",fullFileName);

		// first create root directory.
		m_outFile.open(fullFileName.c_str(),std::ios::out|std::ios::app);
		if (m_outFile.fail())
		{
			LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "TdsSQLLoggerWritter::WriteTDSLog: error opening file! - The reason is: %s",
				strerror( errno ));
			m_outFile.clear();
		}
		else
		{
			std::string str;		// read from TDS_SQLLOG table
			str += "set define off;\n";
			try
			{
                std::string sqlColName = (0 == dbType ? "LOG_CONTENT" : "LOG_CONTENT_2");
                std::vector<unsigned char> sqlContent;
                ...
                databaseConnection->readBLOB(sqlColName, "TDS_SQLLOG", "", sqlContent);
                str.append(sqlContent.begin(), sqlContent.end());
			}
			catch (...)
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,"Catch Exception when read TDS_SQLLOG Table");
				return "";
			}
			m_outFile << str;
		}
	...
	void CMainFrame::eraseTDSLOG_FromDB()
	{
		try
		{
			TA_Base_Core::SQLStatement sql;
			TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::OnlineUpdatable_Cd, TA_Base_Core::Read);
			databaseConnection->prepareSQLStatement(sql, TDS_SQLLOG_UPDATE_81003);
			databaseConnection->executeModification(sql);
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Clear TDS SQLLOG in database");
		}
		catch (...)
		{
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Catch Exception when clear sql logs for TDS_SQLLOG Table");
		}
	}

	//*
	bool CMainFrame::isWorkingOnTdsMode()
	{
		try
		{
			...
			TA_Base_Core::SQLStatement sql;
			TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::OnlineUpdatable_Cd, TA_Base_Core::Read);
			databaseConnection->prepareSQLStatement(sql, TDS_MODE_SELECT_80001);
			std::string tableCount( "TABLE_COUNT" );
			std::vector<std::string> tdsRecord;
			tdsRecord.push_back(tableCount);
			
			// Execute the query. The method can throw a DatabaseException.
			// This is documented in the comment of this method.
			// We are responsible for deleting the returned IData object when we're done with it
			TA_Base_Core::IData* data = databaseConnection->executeQuery(sql, tdsRecord);
            if(data->getNumRows() == 0)
            {
                return FALSE;
            }
			unsigned long nRecordCount = data->getUnsignedLongData(0, tableCount);
		...
		{
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Catch Exception when read TDS_MODE Table");
		}
		return FALSE;
	}
//#endif

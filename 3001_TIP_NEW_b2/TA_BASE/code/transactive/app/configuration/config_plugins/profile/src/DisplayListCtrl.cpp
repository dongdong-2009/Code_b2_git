/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/configuration/config_plugins/profile/src/DisplayListCtrl.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2012/12/27 08:48:09 $
...
namespace TA_Base_App
{
    ...
    const CString DisplayListCtrl::LOCATION_AVAILABLE_COLUMN_NAME = "Available";
    const int     DisplayListCtrl::LOCATION_AVAILABLE_COLUMN_WIDTH = 80;
    const int     DisplayListCtrl::LOCATION_AVAILABLE_COLUMN = 0;
    const CString DisplayListCtrl::LOCATION_NAME_COLUMN_NAME = "Location";
    ...
    const CString     DisplayListCtrl::DISPLAY1_COLUMN_NAME = "Display 1";
    const CString     DisplayListCtrl::DISPLAY2_COLUMN_NAME = "Display 2";
    const CString     DisplayListCtrl::DISPLAY3_COLUMN_NAME = "Display 3";
    ...
    const CString     DisplayListCtrl::LEFT_BANNER_COLUMN_NAME = "LeftBannerDefaultPage"; 
    const CString     DisplayListCtrl::CENTRE_BANNER_COLUMN_NAME = "CentreBannerDefaultPage";
    const CString     DisplayListCtrl::RIGHT_BANNER_COLUMN_NAME = "RightBannerDefaultPage";
...
END_MESSAGE_MAP()
    ...
    void DisplayListCtrl::setupList()
    {
        ...
        setUpKnownParameter(tempNames, LPCSTR(DISPLAY1_COLUMN_NAME));
        setUpKnownParameter(tempNames, LPCSTR(DISPLAY2_COLUMN_NAME));
        setUpKnownParameter(tempNames, LPCSTR(DISPLAY3_COLUMN_NAME));
        setUpKnownParameter(tempNames, LPCSTR(LEFT_BANNER_COLUMN_NAME));
        setUpKnownParameter(tempNames, LPCSTR(CENTRE_BANNER_COLUMN_NAME));
        setUpKnownParameter(tempNames, LPCSTR(RIGHT_BANNER_COLUMN_NAME));
        ...
        std::vector<std::string>::iterator iter = m_parameterNames.begin();
        int count = LOCATION_NAME_COLUMN + 1;
        for(; iter != m_parameterNames.end(); ++iter)
        {
            InsertColumn(count, iter->c_str(),  LVCFMT_LEFT, DISPLAY_COLUMN_WIDTH);
            ++count;
        }
    ...
    void DisplayListCtrl::populateControl(TA_Base_Core::IConfigProfile* currentProfile)
    {
        ...
        try
        {
            ...
            for (std::vector<ILocation*>::iterator iter = locations.begin(); iter != locations.end(); ++iter)
            {
                ...
                SetItemText(position,LOCATION_NAME_COLUMN,(*iter)->getDisplayName().c_str());
    ...
    bool DisplayListCtrl::UpdateCellValue(CEditableListCtrl* listCtrl, int item, int subItem, CString value)
    {
        ...
        try
        {
            if(hdi.pszText == DISPLAY1_COLUMN_NAME || hdi.pszText == DISPLAY2_COLUMN_NAME || hdi.pszText == DISPLAY3_COLUMN_NAME)
            {
				...
				str << displayKey;
				m_currentProfile->setParameterValue(locationKey,hdi.pszText,str.str());
            }
            else
            {
                m_currentProfile->setParameterValue(locationKey,hdi.pszText,std::string(value));
            }

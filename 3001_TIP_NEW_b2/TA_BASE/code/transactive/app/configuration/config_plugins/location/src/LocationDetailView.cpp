/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/configuration/config_plugins/location/src/LocationDetailView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2012/12/18 10:54:44 $
...
namespace TA_Base_App
{
	...
	DDX_Control(pDX, IDC_EDIT_DISPLAYNAME, m_displayName);
	//}}AFX_DATA_MAP
    }
...
BEGIN_MESSAGE_MAP(LocationDetailView, AbstractDetailView)
	...
	ON_EN_KILLFOCUS(IDC_EDIT_DISPLAYNAME, onKillfocusEditDisplayName)
	ON_BN_CLICKED(IDC_CHECK_DISPLAYONLY, OnDisplayOnlyClicked)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
    ...
    void LocationDetailView::OnInitialUpdate() 
    {
		...
		m_displayName.SetLimitText(DISPLAY_NAME_FIELD_LENGTH);
    ...
    void LocationDetailView::showItemDetails(bool isShown)
    {
		...
		GetDlgItem(IDC_STATIC_DISPLAYNAME)->ShowWindow(isShown);
		GetDlgItem(IDC_DISPLAYNAME_BITMAP)->ShowWindow(isShown);
		GetDlgItem(IDC_EDIT_DISPLAYNAME)->ShowWindow(isShown);
    ...
    void LocationDetailView::populateItemDetails(TA_Base_Core::IConfigItem* currentItem)
    {
        ...
        try
        {
			...
			m_displayName.SetWindowText( m_currentLocation->getDisplayName().c_str() );
            ...
            if( m_currentLocation->isNew() )
            {
				...
				((CButton*)GetDlgItem(IDC_CHECK_DISPLAYONLY))->SetCheck(m_displayOnly ? 1 : 0);
            }
            else
            {
				...
				if(m_currentLocation->isDisplayOnly()){
					((CButton*)GetDlgItem(IDC_CHECK_DISPLAYONLY))->SetCheck(1);
					m_displayOnly = true;
				}
				else
				{
					((CButton*)GetDlgItem(IDC_CHECK_DISPLAYONLY))->SetCheck(0);
					m_displayOnly = false;
				}
                // ++ TD12474
            }
	...
	void LocationDetailView::onKillfocusEditDisplayName()
	{
		FUNCTION_ENTRY("onKillfocusEditDisplayName");
		...
		if (m_currentLocation != NULL )
		{
			...
			{
				CString displayName;
				m_displayName.GetWindowText(displayName);
				m_currentLocation->setDisplayName( std::string(displayName) );
			}
			catch ( const TA_Base_Core::DatabaseException& )
			{
				LOG_EXCEPTION_CATCH(SourceInfo, "DatabaseException", "Could not set the data");
	...
	void LocationDetailView::OnDisplayOnlyClicked()
	{
		FUNCTION_ENTRY("OnDisplayOnlyClicked");
		 ...
		 if (m_currentLocation != NULL )
		 {
			 try
			 {
				 ...
				 m_displayOnly = !m_displayOnly;
				 m_currentLocation->setDisplayOnly(m_displayOnly);
			 }
			 catch ( const TA_Base_Core::DatabaseException& )
			 {
				 LOG_EXCEPTION_CATCH(SourceInfo, "DatabaseException", "Could not set the data");

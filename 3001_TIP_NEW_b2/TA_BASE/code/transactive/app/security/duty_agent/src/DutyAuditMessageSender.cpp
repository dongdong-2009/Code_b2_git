/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/duty_agent/src/DutyAuditMessageSender.cpp $
  * @author:  Gregg Kirkpatrick
  * @version: $Revision: #12 $
  *
  * Last modification: $DateTime: 2013/01/11 15:30:19 $
...
namespace 
{
	...
	// otherwise an empty string. 'session' can be NULL. 
	std::string getLocationDisplayNameFromSession(TA_Base_Bus::SessionInfo& sessionInfo)
	{
		...
		{ 
			if(sessionInfo.OperatorId.size()> 0 )
			{
				std::string result = TA_Base_Bus::DutyDbCache::instance()->getLocationDisplayName(sessionInfo.LocationId);
				return result;
			}
			else
				return "";
...
*/
        try
        {
		    ...
		    if( DutyAction::REQUEST_REGION_DUTY_TRANSFER == state.getInitiatingAction() )
		    {
                ...
                {
                    std::string regionName = getRegionDisplayNameFromKey( regionsIter->first );

 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/duty_agent/src/DutyManagerSnapshotConverter.cpp $
...
namespace TA_Base_App
{
	...
	{
		m_subsystemState->length( m_subsystemState->length() + count );
	}
		...
		const TA_Base_Bus::SubsystemState::State& addState, const TA_Base_Bus::SubsystemState::State& remState )
	{
       ...
       // m_subsystemState->length(insertPosition+1);
        m_subsystemState[m_subsystemStateIndex].regionKey    = m_region;
        m_subsystemState[m_subsystemStateIndex].subsystemKey = subsystem;
        m_subsystemState[m_subsystemStateIndex].prevState    = remState;
        m_subsystemState[m_subsystemStateIndex].currState    = addState;

		m_subsystemStateIndex++;
	}


	void DutyManagerSnapshotConverter::setRegionAddCount( unsigned long count )
	{
		m_regionDuty->length( m_regionDuty->length() + count );
	}
	...
	{
		m_subsystemDuty->length( m_subsystemDuty->length() + count );
	}

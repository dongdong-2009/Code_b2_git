 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/duty_agent/src/DutyAgentImpl.cpp $
...
namespace TA_Base_App
{
		...
		m_pSubsystemStateUpdateSender(new SubsystemStateUpdateSender())
    {
		...
		if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DUTY_CENTER))
		{
			m_pDutyCenter = boost::shared_ptr<DutyCenter>( new DutyCenter(m_taskManager));
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"create DutyCenter");
		}
...
*/
	...
	void DutyAgentImpl::mgrTransferAllSubsystemDuty(const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
        ...
        try
        {
			...
			try
			{
				if( TA_Base_Bus::IDutyPolicy::PT_CENTRAL == TA_Base_Bus::DutyDbCache::instance()->getProfileType(sessionInfo.ProfileId[0]))
				{
		    		TA_Base_Bus::DutyDbCache::instance()->getAllRegionKeys(allRegions);
				}
				else
				{
					allRegions = m_localLocSet;
					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"session %s not central profile, assign local locaion set",sessionInfo.SessionId.c_str());
				}
			}
			catch(TA_Base_Core::TransactiveException&ex)
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,"catch TransactiveException:%s",ex.what());
			}
			catch(...)
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,"catch UnknownException");
			}
	...
	void DutyAgentImpl::peerGetState(TA_Base_Bus::DutyAgentTypeCorbaDef::DaDutySequence_out dutyState)
	{
        ...
        try
        {
		    ...
		    getDutyState( state, this->getPeerQueryLocationSet(), false );
	...
	void DutyAgentImpl::peerGetSessionState(const char* session,TA_Base_Bus::DutyAgentTypeCorbaDef::DaDutySequence_out dutyState)
	{
        ...
        try
        {
			...
			getSessionDutyState( state, session,this->getPeerQueryLocationSet(), false );
		...
		const TA_Base_Bus::DutyAgentTypeCorbaDef::KeySequence& subsystems)
	{
		...
		try
		{
			...
			getSubsystemDutyState( state, subSet, this->getPeerQueryLocationSet(), false );
		...
		const TA_Base_Bus::IAuthenticationAgentCorbaDef::SessionInfoCorbaDef& session)
	{
        ...
        try
        {
		   ...
		   // hasExclusiveDuty( state, session, m_localLocSet, false );
		    hasExclusiveDuty( state, session, this->getPeerQueryLocationSet(), false );
	...
	TA_Base_Bus::DASType::LocationSet	DutyAgentImpl::getPeerQueryLocationSet()
	{
		TA_Base_Bus::DASType::LocationSet rtnLocationSet = m_localLocSet;
		if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DUTY_CENTER) && m_pDutyCenter.get()!=NULL)
		{
			TA_Base_Bus::DASType::LocationSet tmpLocationSet = m_pDutyCenter->getOffLineStations();
			rtnLocationSet.insert(tmpLocationSet.begin(),tmpLocationSet.end());	
		}
		return rtnLocationSet;
	}


	void DutyAgentImpl::testNotifyStationOnLine(DutyAgentTypeCorbaDef::Key locationKey)
	{
		this->stationOnLine(locationKey);
	}


	void DutyAgentImpl::testNotifyStationOffLine(DutyAgentTypeCorbaDef::Key locationKey)
	{
		this->stationOffLine(locationKey);
	}


	void DutyAgentImpl::stationOnLine(unsigned long locaitonKey)
	{
		if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DUTY_CENTER) && m_pDutyCenter.get()!=NULL)
		{
			m_pDutyCenter->stationOnLine(locaitonKey);	
		}
	}
	
	void DutyAgentImpl::stationOffLine(unsigned long locationKey)
	{
		if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DUTY_CENTER) && m_pDutyCenter.get()!=NULL)
		{
			m_pDutyCenter->stationOffLine(locationKey);
		}
	}

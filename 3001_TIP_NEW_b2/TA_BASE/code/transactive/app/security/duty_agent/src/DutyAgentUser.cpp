 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/duty_agent/src/DutyAgentUser.cpp $
...
namespace TA_Base_App
{
		...
		m_bulkUpdateComplete(0)
    {
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"begin DutyAgentUser");

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"begin create GenericAgent");

		m_genericAgent = new TA_Base_Bus::GenericAgent(argc,argv,this,NULL,AGENT_USER_TYPE_NORMAL);

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end create GenericAgent");
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end DutyAgentUser");
	...
	void DutyAgentUser::notifyGroupOffline( const std::string& group )
	{
		...
		try
		{
			...
			{
				std::map<std::string,TA_Base_Bus::DASType::DbKey>::iterator itr = m_locationNameMap.find(group);
				if(itr != m_locationNameMap.end())
				{
					LOG( SOURCE_INFO, GENERIC_LOG_TYPE, INFO_LOG_LEVEL, "notify location %d offline", itr->second);
					this->m_dutyAgentImpl->testNotifyStationOffLine(itr->second);
				}
				else
				{
		...
		try
		{
			if(TA_Base_Core::RunParams::getInstance().isSet(RPARAM_DUTY_CENTER) )
			{
				...
				if(itr != m_locationNameMap.end())
				{
					...
					this->m_dutyAgentImpl->testNotifyStationOnLine(itr->second);
				}
				else
				{
					LOG( SOURCE_INFO, GENERIC_LOG_TYPE, ERROR_LOG_LEVEL, "Cant find location key for group: %s", group.c_str() );
	...
	void DutyAgentUser::initLocationNameMap()
	{
		...
		try
		{
			...
			for(int index=0; index < locationVector.size();++index)
			{
				...
				//only save station
				if(NULL != pLocation && pLocation->getLocationType() == TA_Base_Core::ILocation::STATION)
				{
    ...
    void DutyAgentUser::run()
    {
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"begin new DutyAgentImpl");
		m_dutyAgentImpl = new DutyAgentImpl( getAgentEntityName());
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"end new DutyAgentImpl");
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"before set executive");
		m_dutyAgentImpl->setExecutive( newExec );

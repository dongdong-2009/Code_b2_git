 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/rights_agent/src/RightsAgentImpl.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #5 $
 *
	...
	TA_Base_Bus::IRightsAgentCorbaDef::EDecisionModule& decisionModule)
{
	...
	void RightsAgentImpl::stationOnLine( unsigned long locaitonKey )
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Station %lu is online.", locaitonKey);

		m_rightsFactory.stationOnLine( locaitonKey );
	}

	void RightsAgentImpl::stationOffLine( unsigned long locationKey )
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Station %lu is offline.", locationKey);

		m_rightsFactory.stationOffLine( locationKey );
	}
}

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/DutyManager/src/RegionSelectionDlg.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2013/11/21 18:28:50 $
...
BOOL RegionSelectionDlg::OnInitDialog() 
{
    ...
    try
    {
        ...
        for (RegionKeyList::const_iterator regionIt = regionKeys.begin(); regionIt != regionKeys.end(); regionIt++)
        {
            try
            {
				// Local Profile should only show its own location rights
				if(m_dataCache.isCentralProfile(m_sessionCache.getSessionInfo(m_sessionCache.getCurrentSessionId()).ProfileId[0]) == false)
				{
					if ( *regionIt != m_sessionCache.getSessionInfo(m_sessionCache.getCurrentSessionId()).LocationId )
					{
						continue;
					}
				}

                CString regionName = m_dataCache.getRegionDisplayName(*regionIt).c_str();

				if (0 != regionName.CompareNoCase("All Locations"))
				{
					int item(m_assignedRegionList.InsertItem(m_assignedRegionList.GetItemCount(), _T(regionName)));

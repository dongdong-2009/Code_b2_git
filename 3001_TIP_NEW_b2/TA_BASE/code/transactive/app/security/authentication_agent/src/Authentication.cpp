 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/authentication_agent/src/Authentication.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #24 $
 * Last modification: $DateTime: 2015/05/11 16:42:12 $
...
namespace
{
	...
	const unsigned short RESPONSE_TIME( 30 );	// CL-20822++
};
...
namespace TA_Base_App
{
		...
		TA_Base_Core::ConfigUpdate::ConfigOperator))
	{
		...
		LOG_GENERIC(SourceInfo,DebugUtil::DebugInfo,"Read Security Settings from SE_SECURITY_SETTING table...");
		readSecuritySetting();
		FUNCTION_EXIT;
	}
	...
	{	
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Authentication::checkWorkstationExist");
		try
		{
		...
		catch(TA_Base_Core::DataException& ex)
		{
			...
			{
				TA_THROW(TA_Base_Core::AuthenticationSecurityException("The workstation is not known to the system", TA_Base_Core::AuthenticationSecurityException::INVALID_WORKSTATION) );
			}
			else
			{
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Authentication::checkWorkstationExist => Leaving.....");
	}
		...
		unsigned long workstationId, const std::string & password,bool displayOnly)
	{
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-11-04] Authentication::requestSession");

		TA_Base_Bus::GenericAgent::ensureControlMode();

		//TD18707 xinsong++
		//the locationId should be same as the agent id
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-11-04] Authentication::ensureIsLocalLocationId");
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-11-04] Authentication:: m_userCache.getUserDetails(userId)");
		...
		try
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-11-04] Authentication::checkUserSecurity");
			profileName = checkUserSecurity(user,profileId,locationId,password);
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-11-04] Authentication::requestSession done");
		}
		...
		try
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] End Old Session");
			endOldDeadSessions(user,profileId,workstationId);
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] End Old Session Done");
		}
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Read session from sessionAccessFactory");
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Read session from sessionAccessFactory done");
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] addSession");
		m_sessionRegister.addSession(session, true);
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] addSession Done");
		...
		try
		{
			...
			// yanrong: notify the rights agent to update session cache
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
				"[2011-11-04] m_rightsAgent csessionUpdate" );
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "[2011-12-19]session.isDisplayOnly %lu",session.isDisplayOnly);
		...
		catch(TA_Base_Core::TransactiveException& ex)
		{
			...
			try
			{
				...
				CORBA_CALL( m_dutyAgent, authFreeDuty, ( TA_Base_Bus::AuthenticationLibrary::convertCSessionToCORBASession(session) ) );
			}
			catch(TA_Base_Core::TransactiveException& ex)
			{
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"[2011-11-04] sendStateUpdateMessage and sendSessionNotification" );
	...
	void Authentication::endSession(const std::string & sessionId)
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::endSession");
		TA_Base_Bus::GenericAgent::ensureControlMode();
		...
		try
		{
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] try");
		...
		try
		{
			...
			( sessionId.c_str() ) );*/
			// yanrong: notify the rights agent to update session cache
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::sessionUpdate");
			IAuthenticationAgentCorbaDef::SessionInfoCorbaDef corbaSession =
				AuthenticationLibrary::convertCSessionToCORBASession(session);
			CORBA_CALL( m_rightsAgent,
				sessionUpdate,
				( corbaSession,true) );
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::done");
		...
		std::auto_ptr<TA_Base_Core::ISession> endedSession(TA_Base_Core::SessionAccessFactory::getInstance().createEndedSession(sessionId));

		//m_sessionQueue.push(endedSession);
		m_sessionQueueProcessor.writeNewSession(endedSession.release());
		// Send a message to the distributed agents to remove this session

		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::sendSessionNotification");
		sendSessionNotification(session,SessionEnd);

		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::sendStateUpdateMessage");
		sendStateUpdateMessage(session, SessionEnd);

		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::sendStateUpdateMessage Done");
		...
		if( 0 == RunParams::getInstance().get(RPARAM_OPERATIONMODE).compare(RPARAM_CONTROL) )
		{
			...
			sendSessionStateAuditMessage(session.SessionId,session.UserId,getUserName(session.UserId), session.WorkstationId,getWorkstationName(session.WorkstationId),
				session.ProfileId[BASE_PROFILE],getProfileName(session.ProfileId[BASE_PROFILE]),TA_Base_Core::AuthenticationAgentAudit::SessionEnded);
		}
		else
		{
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-09] Authentication::endSession Done");
	...
	void Authentication::changePassword(const std::string & sessionId, const std::string & oldPassword, const std::string & newPassword)
	{
		...
		{
			TA_THROW(TA_Base_Core::AuthenticationSecurityException("The password does not match", TA_Base_Core::AuthenticationSecurityException::INVALID_PASSWORD) );
		}
		...
		{
			TA_THROW(TA_Base_Core::AuthenticationSecurityException("The new password is either too short or too long", TA_Base_Core::AuthenticationSecurityException::INVALID_PASSWORD_LENGTH) );
		}
		...
		try{
			...
			if(isNewPasswordUsedBefore(user.userId,newPassword))
			{
				TA_THROW(TA_Base_Core::AuthenticationSecurityException("The new password has been used before", TA_Base_Core::AuthenticationSecurityException::PASSWORD_USEDBEFORE) );
			}
			m_userCache.changeUserPassword(session.OperatorId[BASE_OPERATOR],newPassword);
			/*
			std::auto_ptr<TA_Base_Core::IOperatorPassword> operatorPassword( TA_Base_Core::OperatorPasswordAccessFactory::getInstance().getOperatorPassword(session.OperatorId[BASE_OPERATOR],true,false));
			operatorPassword->changePassword(newPassword);
		...
		unsigned long locationId, const std::string & password,bool displayOnly)
	{
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-12-29] Authentication::changeProfile");
		TA_Base_Bus::GenericAgent::ensureControlMode();
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-12-28] SessionInfo newSession");
		...
		if (profileId == session.ProfileId[TA_Base_Bus::Authentication::BASE_PROFILE] && locationId == session.LocationId )
		{
			...
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-12-28] SessionAccessFactory::getInstance().done");
		}
		else
		{
			// Need to ensure there are no sessions in the register for the same user/profile/workstation combination.
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,"[2011-12-28] rights agent sessionupdate");
		try
		{
			// yanrong: notify the rights agent to update session cache
			IAuthenticationAgentCorbaDef::SessionInfoCorbaDef corbaSession =
		...
		//send change profile comm message to RightsAgent
		//sendChangeProfileCommMessage(newSession,SessionStart);
	...
	void Authentication::getProfiles(const std::string & sessionId, std::vector<unsigned long> & vtProfileId )
	{
		...
		TA_Base_Bus::GenericAgent::ensureControlMode();
		//LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-12-29] Authentication::getProfiles");
		vtProfileId.clear();
		...
		{
			// Requ
			//LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-12-29] m_sessionRegister.getSession");
			TA_Base_Bus::SessionInfo session = m_sessionRegister.getSession(sessionId);
			//LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"[2011-12-29] m_sessionRegister.getSession done");
		...
		catch (TA_Base_Core::AuthenticationSecurityException&)
		{
			LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugWarn,
				"AuthenticationSecurityException, session <%s> hasn't profiles", sessionId.c_str());
	...
	void Authentication::updateOverrideSessionTime(const TA_Base_Bus::SessionInfo & session, bool beginOverride)
	{
		if ( beginOverride )
		{
			if(session.ProfileId.size() > 1)
			{

				for(int i =1;i < session.ProfileId.size();i++)
				{
			...
			//TD15069 azenitha++
			LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
				"m_sessionRegister.addOverride() called (%s)", session.SessionId.c_str());
			//TD15069 azenitha++
		}
		else
		{
			...
			//TD15069 azenitha++
			LOG1(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
				"m_sessionRegister.clearOverrides() called (%s)", session.SessionId.c_str());
			//TD15069 azenitha++
		}
	}
	...
	void Authentication::startObjects()
	{
		...
		catch(...)
		{
			...
			TA_THROW(TA_Base_Core::AuthenticationAgentException ("Error retrieving database information for the Duty Agent."));
		}
		...
		{
			TA_THROW(TA_Base_Core::AuthenticationAgentException ("Error running the dead session checker thread."));
		}
		...
		unsigned long locationId,const std::string& password)
	{
		...
		if(!TA_Base_Core::OperatorAccessFactory::getInstance().getOperator(user.userId,false)->isSystemOperator())
		{
			...
			{
				// when password is miss match, update the IS_PASSWORD_LOCKOUT 
				// once the IS_PASSWORD_LOCKOUT is equal to the maxmum_failed_times, then update the LATEST_LOCKED_TIME to current system time
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "[2011-11-10] Authentication::checkUserSecurity!=>Wrong Password, db password: %s, passed password:%s", user.password.c_str(), password.c_str());
				// Bring in the exception to be used

				//[2011-11-10] show user lockout message
				updateLockedTimes(user.userId,false);	

				TA_THROW(AuthenticationSecurityException("The password does not match", AuthenticationSecurityException::INVALID_PASSWORD) );
			}
		}
		...
		if (iter == user.profileLocationMap.end() )
		{
			...
			TA_THROW(AuthenticationSecurityException("The profile is not valid for the user", AuthenticationSecurityException::PROFILE_NOT_PERMITTED) );
		}
		else
		{
			...
			{
				//login successfully, update the IS_PASSWORD_LOCKOUT 
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "[2011-11-10] Authentication::checkUserSecurity!=>Right Password");
				updateLockedTimes(user.userId,true);
			...
			else
			{
				...
				TA_THROW(AuthenticationSecurityException("The location is not valid for the profile", AuthenticationSecurityException::LOCATION_NOT_PERMITTED) );
			}
		}
		...
		unsigned long workstationId)
	{
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Authentication::endOldDeadSessions");
		std::vector<TA_Base_Bus::SessionInfo> sessions;
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] m_sessionRegister.getRegister");
		m_sessionRegister.getRegister(sessions);
		// ly++ CL-20784

		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] m_sessionRegister.appendGhostSession");
		m_sessionRegister.appendGhostSession(sessions);
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] m_sessionRegister.appendGhostSession done");
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"[2011-11-08] Authentication::endOldDeadSessions leaving");
		FUNCTION_EXIT;
	}
	...
	void Authentication::sendSessionStateAuditMessage(const std::string& sessionId,unsigned long operatorKey,std::string operatorName,
		unsigned long sessionLocationKey,const std::string& consoleName, unsigned long profileID,const std::string& profileName,
	...
	void Authentication::sendChangePasswordAuditMessage(const std::string& sessionId)
	{
		...
		else
		{
			...
			TA_THROW(TA_Base_Core::AuthenticationAgentException ("Unexpected sending of PasswordChanged (System is in Monitor Mode)"));;
		}
		//TD15069 ++azenitha
	}
	...
	void Authentication::updateLockedTimes(unsigned long key, bool rightPassword)
	{
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "[2011-11-11] updateLockedTimes:");

		try{
			std::auto_ptr<TA_Base_Core::IOperatorPassword> theSession( TA_Base_Core::OperatorPasswordAccessFactory::getInstance().getOperatorPassword(key,false,rightPassword) );
			theSession->applyChanges();
		}
		catch(DataException&)
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to updateLockedTimes") );
		}
		catch(DatabaseException&)
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to updateLockedTimes") );
		...
		{
			TA_THROW(AuthenticationAgentException("An unknown error occured while attempting to updateLockedTimes") );
		}
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "[2011-11-11] updateLockedTimes:");
		/*
		//When password is miss match, if LATEST_LOCKED_TIME is under maxmum_failed_time,update the LATEST_LOCKED_TIME to LATEST_LOCKED_TIME+1
		if(wrongPassword)
		{

		std::ostringstream sql;
		sql<<"SELECT  IS_PASSWORD_LOCKOUT FROM OPERATOR_PASSWORD WHERE PKEY = "<<key;

		TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Sessions_Ad,TA_Base_Core::Read);
		std::vector<std::string> columnNames;
		columnNames.push_back("IS_PASSWORD_LOCKOUT");
		TA_Base_Core::IData* data;

		try
		{
		data = databaseConnection->executeQuery(sql.str(),columnNames);
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "data = databaseConnection->executeQuery");
		if(data->getNumRows() ==1)
		{

		int locked_times = data->getIntegerData(0,"IS_PASSWORD_LOCKOUT");

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "retrieve LATEST_LOCKED_TIME and IS_PASSWORD_LOCKOUT");
		std::ostringstream  sqlUpdate;
		if(locked_times < failed_login_attempts)
		{
		locked_times++;
		sqlUpdate<<"Update OPERATOR_PASSWORD SET IS_PASSWORD_LOCKOUT = "<<locked_times;
		}
		else 
		{
		sqlUpdate<<"Update OPERATOR_PASSWORD SET IS_PASSWORD_LOCKOUT = "<<failed_login_attempts;
		}
		...
		struct tm* temp;
		temp = localtime(&currentSystemTime);

		sqlUpdate<<", LATEST_LOCKED_TIME = TO_DATE('"
		<<temp->tm_year+1900<<"-"<<temp->tm_mon+1<<"-"<<temp->tm_mday
		<<" "
		<<temp->tm_hour<<":"<<temp->tm_min<<":"<<temp->tm_sec
		<<"','YYYY-MM-DD HH24:MI:SS')";
		}
		sqlUpdate<< " where pkey = "<<key;
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "SessionManager::updatePasswordChangeDate:%s",sqlUpdate.str().c_str());
		databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Sessions_Ad,TA_Base_Core::Write);
		databaseConnection -> executeModification(sqlUpdate.str());
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "SessionManager::updatePasswordChangeDate:%s",sqlUpdate.str().c_str());
		...
		{	
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError, "Failed to fetch the specific entry:sqlUpdate ");
		...
		delete data;
		data=NULL;
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DatabaseException", "Throw a DatabaseException during execute selectSQL.");
		}
		catch( const TA_Base_Core::DataException& )
		{
		delete data;
		data=NULL;
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DataException", "Throw a DataException during execute selectSQL.");
		}
		}
		// when successfully login, update the IS_PASSWORD_LOCKOUT to 0 and LATEST_LOCKED_TIME to null and update the latest LATEST_LOGIN_DATE to current system time
		else
		{
		try
		{
		...
		std::ostringstream sql;
		sql<<"Update OPERATOR_PASSWORD SET IS_PASSWORD_LOCKOUT = 0 , LATEST_LOCKED_TIME = NULL, LATEST_LOGIN_DATE = TO_DATE('"
		<<temp->tm_year+1900<<"-"<<temp->tm_mon+1<<"-"<<temp->tm_mday
		<<" "
		<<temp->tm_hour<<":"<<temp->tm_min<<":"<<temp->tm_sec
		<<"','YYYY-MM-DD HH24:MI:SS') where pkey = "<<key;

		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo, "%s",sql.str().c_str());
		TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Sessions_Ad,TA_Base_Core::Write);
		databaseConnection -> executeModification(sql.str());

		}
		catch( const TA_Base_Core::DatabaseException& )
		{
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DatabaseException", "Throw a DatabaseException during execute updateSQL.");
		}
		catch( const TA_Base_Core::DataException& )
		{
		LOG( SourceInfo, DebugUtil::ExceptionCatch, "DataException", "Throw a DataException during execute updateSQL.");
		}
	...
	bool Authentication::isNewPasswordUsedBefore(unsigned long pkey, const std::string& password)
	{
		LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"Authentication::isNewPasswordUsedBefore");
		LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,"CURRENT_PASSWD_INDICATOR:");
		try{
			std::auto_ptr<TA_Base_Core::IOperatorPassword> theSession( TA_Base_Core::OperatorPasswordAccessFactory::getInstance().getOperatorPassword(pkey,false,false) );
			std::string password0 = theSession->getPassword0();
			std::string password1 = theSession->getPassword1();
			std::string password2 = theSession->getPassword2();

			if((password0.compare(password)==0) || (password1.compare(password)==0) || (password2.compare(password)==0))
			{
				return true;
			}
		...
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to isNewPasswordUsedBefore") );
		}
		catch(DatabaseException&)
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to isNewPasswordUsedBefore") );
		...
		{
			TA_THROW(AuthenticationAgentException("An unknown error occured while attempting to isNewPasswordUsedBefore") );
		}
		return false;
		/*
		std::ostringstream sql;
		sql<<"SELECT PASSWORD_0,PASSWORD_1,PASSWORD_2 from OPERATOR_PASSWORD WHERE PKEY = "<<pkey;
		TA_Base_Core::IDatabase* databaseConnection=TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::OfflineSecurity_Cd,TA_Base_Core::Read);
		std::vector<std::string> columnNames;
		columnNames.push_back("PASSWORD_0");
		columnNames.push_back("PASSWORD_1");
		columnNames.push_back("PASSWORD_2");
		TA_Base_Core::IData* data=databaseConnection->executeQuery(sql.str(),columnNames);
		std::string password_0=data->getStringData(0,"PASSWORD_0");
		std::string password_1=data->getStringData(0,"PASSWORD_1");
		std::string password_2=data->getStringData(0,"PASSWORD_2");
		delete data;
		data=NULL;
		if((password_0.compare(password)==0) || (password_1.compare(password)==0) || (password_2.compare(password)==0))
		{
		return 1;
		}
		return 0;*/
	...
	void Authentication::readSecuritySetting()
	{
		try{
			automatic_logout_time = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getAutomaticLogoutTime();
			failed_login_attempts = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getFailedLoginAttemtps() ;
			password_reuse_max = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getPasswordReuseMax() ;
			password_warn_time = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getPasswordWarnTime() ;
			password_reuse_time = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getPasswordReuseTime();
			password_life_time = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getPasswordLifeTime();
			password_lock_time = TA_Base_Core::SecuritySettingAccessFactory::getInstance().getPasswordLockTime();
		}
		catch(DataException&)
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to readSecuritySetting") );
		}
		catch(DatabaseException&)
		{
			TA_THROW(AuthenticationAgentException("A database error occured while attempting to readSecuritySetting") );
		...
		{
			TA_THROW(AuthenticationAgentException("An unknown error occured while attempting to readSecuritySetting") );
		}
	}

 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/authentication_agent/src/Authentication.h $
 * @author:   Nick Jardine
 * @version:  $Revision: #6 $
 *
 * Last modification: $DateTime: 2014/08/13 18:15:41 $
...
namespace TA_Base_App
{
    ...
    public:
         ...
         */
        std::string changeProfile(const std::string & sessionId, unsigned long profileId, unsigned long locationId, const std::string & password, bool displayOnly);
		...
		void updateOverrideSessionTime(const TA_Base_Bus::SessionInfo & session, bool beginoverride);
    ...
    private:
		...
		void sendSessionStateAuditMessage(const std::string& sessionId,unsigned long operatorKey,std::string operatorName,
		...
		bool isNewPasswordUsedBefore( unsigned long userId, const std::string& password);

		void updateLockedTimes(unsigned long key, bool rightPassword);
	private:

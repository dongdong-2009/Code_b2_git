 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/security/authentication_agent/src/SessionRegister.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #15 $
 *
 * Last modification: $DateTime: 2014/08/15 15:44:59 $
...
{
    SessionRegister::SessionRegister( bool isControl, unsigned long entityKey ) /*chenlei++ CL-21183*/: m_entityKey( entityKey )
    {
        FUNCTION_ENTRY("SessionRegister::SessionRegister() constructor");
		//m_operatorOverrideKey = -1;
		//m_seprofOverrideKey = -1;
        ...
        if( isControl )
        {
			...
			try
			{
				//[2011-11-09] ownedSessions = TA_Base_Core::SessionAccessFactory::getInstance().getOwnedSessions( entityKey );
    ...
    void SessionRegister::removeSession( const std::string& sessionId )
    {
        ...
        if ( m_sessionRegister.end() == registerIter )
        {
            ...
            TA_THROW( AuthenticationSecurityException( exMsg.str().c_str(), AuthenticationSecurityException::INVALID_SESSION ) );
        }
    ...
    TA_Base_Bus::SessionInfo SessionRegister::getSession(const std::string& sessionId)
    {
			...
			*/
			...
			else
			{
				...
				if ( getOwnedSessionInfoFromDb(sessionId, tmpSession,hasEnded) )
				{
					...
					TA_Assert( iter != m_sessionRegister.end() );
					*/	
				//if the session loaded from database is not ended ,add this session to ghostsesesion
				if(!hasEnded)
				{
					{
						TA_Base_Core::ThreadGuard guardB( m_activeTimesAccessLock );
...

					{
						...
						LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, 
							"Not ended session from database: %s", 
	...
	void SessionRegister::loadOvrrideInfomation( const std::string &sessionId,TA_Base_Bus::SessionInfo& session )
	{
		...
		/*
		IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Sessions_Ad,TA_Base_Core::Read);
				...
				char selectSql[256];
				sprintf(selectSql,"select OPERATOR_OVERRIDE_KEY, SEPROF_OVERRIDE_ID from TA_SESSION where PKEY = '%s'",
					sessionId.c_str());
		
				std::string operatorOverrideColumn = "OPERATOR_OVERRIDE_KEY";
				std::string seprofOverrideColumn = "SEPROF_OVERRIDE_ID";
		
				std::vector<std::string> columnNames;
				columnNames.push_back(operatorOverrideColumn);
				columnNames.push_back(seprofOverrideColumn);
		
				IData *data = databaseConnection->executeQuery(selectSql,columnNames);
				if ( NULL == data || 0 == data->getNumRows() )
				{
					...
					sprintf(reasonMessage, "No session found for pkey = %s", sessionId.c_str() );			
				...
				else if ( 1 < data->getNumRows() )
				{
					...
					char reasonMessage[256];
					sprintf(reasonMessage,"More than one session found for pkey = %s",sessionId.c_str());
					TA_THROW(DataException(reasonMessage,DataException::NOT_UNIQUE,""));
				}
		
				m_operatorOverrideKey = data->getIntegerData(0,operatorOverrideColumn);
				m_seprofOverrideKey = data->getIntegerData(0,seprofOverrideColumn);

...
// in any form.
//
// Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_manager/src/MessageBox.cpp $
// @author:  Bart Golab
// @version: $Revision: #7 $
//
...
void MessageBox::error(const unsigned long actionId, const CString& actionParam, const TransactiveException& exception)
{
	...
	if (actionId == IDS_DELETE_PLAN || actionId == IDS_DELETE_CATEGORY)
	{
...
void MessageBox::displayPlanStepDetailError(const CString& actionName, const PlanStepDetailException& exception)
{
    ...
    switch (exception.getDetailType())
    {
        ...
        // ++lizettejl
        case PSDT_TIME:
            resourceId = IDS_STEP_PARAMETER_TIME_SPAN;
            break;
        case PSDT_PROFILE:

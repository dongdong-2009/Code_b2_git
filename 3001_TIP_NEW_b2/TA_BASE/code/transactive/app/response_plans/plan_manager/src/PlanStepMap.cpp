/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepMap.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #8 $
  *
...
//++lizettejl
        ...
        case TA_Base_Core::PLAN_CHECK_TIMER_STEP:
            FUNCTION_EXIT;
            return new PlanCheckTimerStep(m_parent, stepId);
#endif

#if defined (STEPTYPE_PLAN_SET_TIMER)
        case TA_Base_Core::PLAN_SET_TIMER_STEP:
            FUNCTION_EXIT;
            return new PlanSetTimerStep(m_parent, stepId);
#endif

#if defined (STEPTYPE_EXECUTE_SCRIPT)
        case TA_Base_Core::EXECUTE_SCRIPT_STEP:
            FUNCTION_EXIT;
            return new ExecuteScriptStep(m_parent, stepId);
#endif

#if defined (STEPTYPE_DATA_POINT_TIME_CHECK)
        case TA_Base_Core::DATA_POINT_TIME_CHECK_STEP:
            FUNCTION_EXIT;
            return new DataPointTimeCheckStep(m_parent, stepId);
#endif

#if defined (STEPTYPE_POPUP_MFT_MESSAGE)
        case TA_Base_Core::POPUP_MFT_MESSAGE_STEP:
            FUNCTION_EXIT;
            return new PopupMFTMessageStep(m_parent, stepId);
#endif

#if defined (STEPTYPE_SHUTDOWN_SYSTEM)
        case TA_Base_Core::SHUTDOWN_SYSTEM_STEP:
            FUNCTION_EXIT;
            return new ShutdownSystemStep(m_parent, stepId);
#endif

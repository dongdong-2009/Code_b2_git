/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_manager/src/BackgroundLaunchSchematicStep.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #4 $
  *
                                               ...
                                               PlanStepParameterList &stepParameters)
{
    ...
    TA_Base_Core::IProfile *profile = ProfileConfigAccess::instance()->getProfileByKey(parameterSet.bgLaunchSchematic().profile);
    if (NULL == profile)
    {
        bValidProfile = false;
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugFatal, "Invalid profile key = %lu", parameterSet.bgLaunchSchematic().profile);
    }

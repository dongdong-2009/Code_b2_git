/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanDataWriter.cpp $
        ...
        for (int m = 0; m < numberOfSteps; m++)
        {
            ...
            case TA_Base_Core::STATION_TRAVELLER_INFO_SYSTEM_STEP:
                {
                    ...
                    std::string strStation = refParams.destination.station.in();

                    if (strStation.empty() && !refParams.destination.allStations)
                    {
            ...
            case TA_Base_Core::TRAIN_TRAVELLER_INFO_SYSTEM_STEP:
                ...
                case TA_Base_Core::PLAN_CHECK_TIMER_STEP:
                    {
                        const TA_Base_Core::PlanCheckTimerParameters& refParams = stepParameters->planCheckTimer();
                ...
                case TA_Base_Core::EXECUTE_SCRIPT_STEP:
                    {
                        const TA_Base_Core::ExecuteScriptParameters& refParams = stepParameters->executeScript();
                        std::string strCommand = refParams.commands.in();

                        if ( strCommand.empty() )
                        {
                            message << "Step " << m+1 << " - Parameter 'Command line' : Command line is not valid";
                            TA_THROW(TA_Base_Core::PlanStepDetailException(message.str(), TA_Base_Core::PSDT_APPLICATION, TA_Base_Core::PSDE_INVALID, m+1 ));
                        }
                    }
                    break;
                ...
                case TA_Base_Core::DATA_POINT_TIME_CHECK_STEP:
                    {
                        const TA_Base_Core::DataPointTimeCheckParameters& refParams = stepParameters->dataPointTimeCheck();
                        ...
                        if ( refParams.timeSpan <= 0 )
                        {
                            message << "Step " << m+1 << " - Parameter 'Time Span' : Specified time span is not valid";
                            TA_THROW(TA_Base_Core::PlanStepDetailException(message.str(), TA_Base_Core::PSDT_TIME, TA_Base_Core::PSDE_INVALID, m+1 ));
                        }
...
////-------------
		...
		{	//Add Step into step_details
            ...
            case TA_Base_Core::STATION_TRAVELLER_INFO_SYSTEM_STEP:
            {
                ...
                ostrstrm.str("");
                ostrstrm << refParams.destination.station.in() << SEPARATOR
                         << refParams.destination.allStations;
                valueVector.push_back(ostrstrm.str());

                // Station level
                valueVector.push_back(refParams.stationLevel.in());
                ...
                while (pidIdx < refParams.stationPids.length())
                {
                    ostrstrm.str("");

                    for (unsigned long i(0);
                         i < PIDS_PER_ROW && pidIdx < refParams.stationPids.length();
                         i++, pidIdx++)
                    {
                        ostrstrm << refParams.stationPids[pidIdx].in() << SEPARATOR;
                    }
			 ...
			 case TA_Base_Core::STATION_PUBLIC_ADDRESS_STEP:
            {
                ...
                // "station,all_stations"
                ostrstrm.str("");
                ostrstrm << refParams.station.station.in() << SEPARATOR
            ...
            case TA_Base_Core::PLAN_SET_TIMER_STEP:
            {
                const TA_Base_Core::PlanSetTimerParameters& refParams = stepParameters->planSetTimer();

                ostrstrm.str("");
                ostrstrm << refParams.time;
                valueVector.push_back( ostrstrm.str() );
            ...
            {
                const TA_Base_Core::PopupMFTMessageParameters& refParams = stepParameters->popupMFTMessage();
                ...
                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.pendingStep);
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << refParams.operatorProfileNumber;
                valueVector.push_back(ostrstrm.str());

                ////////////////////////////////////////////////////////////////////////////
                ostrstrm.str("");
                ostrstrm << refParams.messageBoxType;
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.abortStep);
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.ignoreStep);
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.retryStep);
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.okStep);
                valueVector.push_back(ostrstrm.str());

                ostrstrm.str("");
                ostrstrm << STEP_NUMBER_PREFIX << getStepNumber(plan, refParams.cancelStep);
                valueVector.push_back(ostrstrm.str());
            ...
            case TA_Base_Core::SHUTDOWN_SYSTEM_STEP:
            {
                const TA_Base_Core::ShutdownSystemParameters& refParams = stepParameters->shutdownSystem();

                ostrstrm.str("");
                ostrstrm << refParams.notifyPeer;
                valueVector.push_back(ostrstrm.str());

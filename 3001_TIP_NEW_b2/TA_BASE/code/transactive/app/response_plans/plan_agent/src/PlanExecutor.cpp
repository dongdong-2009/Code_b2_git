/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanExecutor.cpp $
...
// 			}
    ...
    void PlanExecutorImpl::processPopupMftMessageStepResponse( const TA_Base_Core::CurrentPlanStepDetail& step, const int status )
    {
#if defined ( STEPTYPE_POPUP_MFT_MESSAGE )
        ActivePlanPopupMftMessageStepMessage* popupMftMessageStepMessage = new ActivePlanPopupMftMessageStepMessage(step, status);
        sendMessage( step.activePlan, popupMftMessageStepMessage );

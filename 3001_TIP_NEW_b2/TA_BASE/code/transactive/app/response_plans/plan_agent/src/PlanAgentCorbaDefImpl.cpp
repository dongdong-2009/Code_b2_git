...
*  in any form.
*
*  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanAgentCorbaDefImpl.cpp $
...
namespace TA_Base_App
{
    ...
    void PlanAgentCorbaDefImpl::processPopupMftMessageStepResponse( const TA_Base_Core::CurrentPlanStepDetail& currentStepDetail, TA_Base_Core::EPlanStepCompletionStatus status )
    {
        FUNCTION_ENTRY("processPopupMftMessageStepResponse");

        checkOperationMode();

        try
        {
            PlanExecutor::instance()->processPopupMftMessageStepResponse( currentStepDetail, status );
        }
        catch (const TA_Base_Core::PlanStepDetailException& e)
        {
            FUNCTION_EXIT;

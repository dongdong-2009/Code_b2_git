/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/response_plans/plan_agent/src/ActiveLaunchSchematicStep.cpp $
  *  @author:  Bart Golab
  *  @version: $Revision: #9 $
  *
...
namespace TA_Base_App
{
	...
	ActiveLaunchSchematicStep::ActiveLaunchSchematicStep( const TA_Base_Core::StepDetail& stepDetail, 
        const TA_Base_Core::LaunchSchematicParameters& params, IActivePlanFlow& flowControl ) : 
    ActiveStep(stepDetail, flowControl, TA_Base_Bus::IPlanManagerCorbaDef::LAUNCHSCHEMATIC_RESPONSE_TIMEOUT),
    m_params(params)
	{
	}

	TA_Base_Core::EActiveStepState ActiveLaunchSchematicStep::executeImpl( void )
	{
		...
		try
		{
			...
			if ( planManager != NULL )
			{
				...
				else
                {
                    ...
                    {   
                        std::string message = "ActiveLaunchSchematicStep: Exception caught IPlanManagerCorbaDef::InteractiveStepExecuteError. ";
                        message += isee.reason.in();
			            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, message.c_str() );
			            m_remark = RemarkSchematicCanNotLaunch;
			            stepState = TA_Base_Core::FAILED_ACTIVE_STEP_STATE;                            
                    }
                }
			}

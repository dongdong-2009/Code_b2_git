/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/app/reporting/crystal_reports_viewer/src/crutil.cpp $
  * @author:  Ripple
  * @version: $Revision: #6 $
  *
  * Last modification: $DateTime: 2012/11/29 14:19:54 $
...
namespace TA_Base_App
{
	...
	: TransActiveWinApp(new TA_Reporting::crutilGui(), "Report Manager")
	{
	}
	#else
	CCrutilApp::CCrutilApp()
					...
					if (pszReportName)
					{
						...
						if (objArgv.parameterExists( PRINT_INCIDENT_LABEL ))
						{
                            ...
                            std::vector<std::string> reportidstr = objArgv.getParameterValue(INCIDENT_REPORT_ID_LABEL);
								...
								{
									AfxMessageBox( "Incorrect name of Incident report id lable.", MB_OK | MB_ICONERROR );
									LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Incorrect name of Incident report id lable.");
								}
   						}

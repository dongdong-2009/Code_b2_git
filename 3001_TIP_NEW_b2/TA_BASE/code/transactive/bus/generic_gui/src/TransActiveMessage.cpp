...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/generic_gui/src/TransActiveMessage.cpp $
  * @author:  Brett Zimmerman
  * @version: $Revision: #5 $
  *
  * Last modification: $DateTime: 2013/10/24 16:11:53 $
...
std::vector<HWND> TransActiveMessage::s_popupHWnd;
...
UINT TransActiveMessage::doMsgBox(LPCTSTR pszCaption, LPCTSTR pszMessage, UINT uiMsgBoxType, UINT uiHelpID, 
									BOOL bAuditEvent)
{
...
bool TransActiveMessage::logMessage(LPCTSTR pszCaption, LPCTSTR pszMessage, UINT uiResult, BOOL bAuditEvent)
{
	...
	sMessage.Format(_T("Message Box Caption %s, Message %s, Result %s\n"), pszCaption, pszMessage, sResult); //SL
...
// 		return (TRUE == sMessage.LoadString(m_hResourceDll, uiMessageID, (WORD)m_lcidCurrentLang));
		return LoadString(sMessage, uiMessageID);
#endif
	...
	catch(CMemoryException* e)
	{
        ...
        TA_THROW(TA_Base_Core::MessageResourceException(std::string(errorMessage), TA_Base_Core::MessageResourceException::GENERAL_ERROR,uiMessageID));
	}
}
...
void TransActiveMessage::closeAllMessages()
{
    ...
    for (unsigned int i = 0; i < s_popupHWnd.size(); i++)
    {
        if (::IsWindow(s_popupHWnd[i]))
        {
            ::PostMessage(s_popupHWnd[i], WM_CLOSE, 0, 0);
        }
    }
...
HWND TransActiveMessage::determineParentWindow()
{
    ...
    if ( NULL == hParentWnd )
    {
        ...
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Using main window as parent.");
        }
    }
    else
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Using active window as parent.");
    }

    // Couldn't get main window or window is minimised so get desktop window.
    if ((hParentWnd == NULL) || ::IsIconic(hParentWnd))
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Can't get the parent window or window is minimised, so using the desktop window");

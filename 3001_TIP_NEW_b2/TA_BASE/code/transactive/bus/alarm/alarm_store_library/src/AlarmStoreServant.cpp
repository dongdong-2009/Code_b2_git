/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/alarm/alarm_store_library/src/AlarmStoreServant.cpp $
  * @author:  Ripple
  * @version: $Revision: #11 $
  *
  * Last modification: $DateTime: 2017/12/12 15:47:26 $
...
const std::string PARAM_ATS_EXP_ENABLE = "AtsExportEnable";
const std::string PARAM_ATS_EXP_PATH = "AtsExportPath";
const std::string PARAM_ATS_EXP_INTERVAL = "AtsTimeInterval";

const std::string DEFAULT_ATS_EXPPATH = "C:\\transactive\\AtsAlarms\\";
const unsigned long DEFAULT_ATS_INTERVAL = 5000;
...
void AlarmStoreServant::init()
{
	try
	{
		...
		bool exporEnabled = false;
		std::string exportPath = DEFAULT_ATS_EXPPATH;
		unsigned long exportInterval = DEFAULT_ATS_INTERVAL;
		if (TA_Base_Core::RunParams::getInstance().isSet(PARAM_ATS_EXP_ENABLE.c_str()))
		{
			exporEnabled = true;
			if (TA_Base_Core::RunParams::getInstance().isSet(PARAM_ATS_EXP_PATH.c_str()))
			{
				exportPath = TA_Base_Core::RunParams::getInstance().isSet(PARAM_ATS_EXP_PATH.c_str());
			}
			if (TA_Base_Core::RunParams::getInstance().isSet(PARAM_ATS_EXP_INTERVAL.c_str()))
			{
				exportInterval = atol(TA_Base_Core::RunParams::getInstance().get(PARAM_ATS_EXP_INTERVAL.c_str()).c_str());
			}
		}
		
		m_ATSAlarmObserver= new ATSAlarmObserver(exporEnabled, exportPath, exportInterval);

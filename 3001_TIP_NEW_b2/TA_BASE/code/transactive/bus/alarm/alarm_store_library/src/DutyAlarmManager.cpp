...
// 		TA_Base_Core::ThreadGuard guard(m_lock);
	...
	unsigned long DutyAlarmManager::removeLocationAlarms(unsigned long locationKey)
	{
		if ( NULL != m_pSharedAlarm )
		{
			return m_pSharedAlarm->removeLocationAlarms(locationKey );
		}
		return false;
...
//		TA_Base_Core::ThreadGuard guard(m_lock);
	...
	long DutyAlarmManager::removeLocSubSystemDutyAlarms(unsigned long ulLocationKey, std::list<unsigned long>& subSystemKeyList, bool isATSAlarms/*=false*/)
	{
		TA_Base_Core::ThreadGuard guard(m_lock);
		if (NULL != m_pSharedAlarm)
		{
			return m_pSharedAlarm->removeLocSubSystemDutyAlarms(ulLocationKey, subSystemKeyList, isATSAlarms);
		}
		return 0;
	}

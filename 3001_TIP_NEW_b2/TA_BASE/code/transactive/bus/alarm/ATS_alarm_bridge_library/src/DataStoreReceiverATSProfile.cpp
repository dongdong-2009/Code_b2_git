...
namespace TA_Base_Bus
{
   ...
   boost::shared_ptr<ATSProfileInfo> DataStoreReceiverATSProfile::getProfile()
   {
	   std::vector<boost::shared_ptr<ATSProfileInfo>  >*  datas= getData();
	   boost::shared_ptr<ATSProfileInfo>  returnProfile (new ATSProfileInfo());
	   if  ( datas != NULL)
	   {
		   std::vector<boost::shared_ptr<ATSProfileInfo>  >::iterator iter=datas->begin();
	...
	// 	
	// 	CommandCacheManager* DataStoreReceiverATSProfile::getDataManager()
	// 	{

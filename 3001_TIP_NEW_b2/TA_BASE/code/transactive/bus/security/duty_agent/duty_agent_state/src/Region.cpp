/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/duty_agent/duty_agent_state/src/Region.cpp $
  *  @author:  Gregg Kirkpatrick
  *  @version: $Revision: #4 $
  *
  *  Last modification: $DateTime: 2013/11/14 10:20:12 $
	...
	void Region::transfer(DutyState& state, const DutyPrimitive& session, const DASType::SubsystemSet& subsystems, bool apply)
	{
		...
		//if( m_dutyHelper.hasDuty(session))
		{
			...
			if(apply)
			{
				if(!m_dutyHelper.hasDuty(session))
				{
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "transfer subsystem:session:%s, have no region %u duty",
						session.m_session.c_str(),m_regionKey);

					m_dutyHelper.add(session);
					m_dutyHelper.apply();

					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "transfer subsystem:add session:%s, profile:%u to region duty",
						session.m_session.c_str(),session.m_profile);

					//state.add(m_regionKey, session);
					state.processRegionPolicyResult ( m_regionKey, session, session, IDutyPolicy::PR_ADD );
				}

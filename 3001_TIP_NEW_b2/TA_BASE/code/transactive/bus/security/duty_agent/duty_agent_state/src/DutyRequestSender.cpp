/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/duty_agent/duty_agent_state/src/DutyRequestSender.cpp $
...
namespace TA_Base_Bus
{
	...
	void DutyRequestSender::sendRequests()
	{
		...
		for( it=m_outstandingRequests.begin(); it!=m_outstandingRequests.end(); it++ )
		{
			if ( true == DutySessionCache::instance()->isSystemOperatorSession( (std::string)it->second.targetSession ) )
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
					"Session %s is a system agent session.", it->second.targetSession.in() );

				TA_Base_Bus::DutyResponseSubject::instance()->update( it->first, IDutyAgentRespCorbaDef::DR_ACCEPT );
			}
			else
			{
				CORBA::Any commsData;

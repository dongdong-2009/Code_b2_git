/**
  ...
  *  Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/duty_agent/duty_agent_state/src/TransferableSubsystemQuery.cpp $
...
namespace TA_Base_Bus
{
	...
	void TransferableSubsystemQuery::execute(DutyState& state, const DASType::DbKey& region, const DASType::DutySet& dutySet)
	{
		...
		for( prfIt = profiles.begin(); prfIt != profiles.end(); prfIt++ )
		{
			...
			{
				m_regionDuty.insert(dp);
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "TD15346: Operator have no duty for this region, but force him to have the duty temporarily");
			}

 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/rights_agent/xacml_factory/src/XacmlContextFactory.cpp $
 * @author:   Gregg Kirkpatrick
 * @version:  $Revision: #9 $
 *
 * Last modification: $DateTime: 2014/09/03 14:28:52 $
...
namespace TA_Base_Bus
{
		...
		unsigned long subject, unsigned long resource, unsigned long action )
	{
		...
		//check display only mode
		if(isDisplayOnly(session)){
			getActionAttributes( ctx, action ); //thread safe, actionlock

			if( isPhysicalResource( resource ) ) //threadsafe,resourceLock
			{
				ctx->addResourceAttribute( RightsConst::RESOURCE_TYPE, RightsConst::DISPLAY_ONLY_LOGICAL );
				getSubjectAttributesFromDutyAgent(ctx, session, subject, resource); //thread safe, resourceLock, loc-sub-duty lock, actionGroupLock
			}
			else
			{
				ctx->addResourceAttribute( RightsConst::RESOURCE_TYPE, RightsConst::DISPLAY_ONLY_LOGICAL );

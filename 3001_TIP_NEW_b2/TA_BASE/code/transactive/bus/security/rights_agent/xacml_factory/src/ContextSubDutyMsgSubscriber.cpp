 ...
 * 
 * Source:    $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/security/rights_agent/xacml_factory/src/ContextSubDutyMsgSubscriber.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #4 $
 *
 * Last modification: $DateTime: 2015/08/04 09:58:17 $
...
// 					TA_Base_Bus::RightsAgentInfo::m_subsystem,TA_Base_Bus::RightsAgentInfo::m_location,NULL);
		...
		else if ( 0 == TA_Base_Core::DutyAgentBroadcastComms::StationOnLineOffLineNotification.getTypeKey().compare(message.messageTypeKey) )
		{
			...
			if ( 0 != (message.messageState >>= data) )
			{
				...
				{
					unsigned long occLocKey = TA_Base_Core::LocationAccessFactory::getInstance().getOccLocationKey();
					if (m_locationKey != occLocKey
						&& locationKey != occLocKey)
					{
						// No need to do reset cache.
						LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
							"receive StationOnLineOffLineNotification, but current location is not occ, and the location need to reset is another location(not occ), so ignore reset cahce.");
    ...
    {
		TA_Base_Core::ThreadGuard dutyMapGuard(m_Lock);
	...
	void ContextSubDutyMsgSubscriber::resetLocationCache( unsigned long locationKey )
	{
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"resetLocationCache step 1, function begin, lock map.");

		RightsType::LocSubDutyMap::iterator iter;
		iter = m_locSubDutyMap.begin();
		
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"resetLocationCache step 2, init iterator.");

		RightsType::LocSubDutyMap::const_iterator iterEnd = m_locSubDutyMap.end();
		
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"resetLocationCache step 3, get map end element.");

		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"resetLocationCache step 4, Location-subystem Duty Map have size %d before reset location %d cache.", m_locSubDutyMap.size(), locationKey);

		unsigned long iLoop = 1;
		while ( iter != iterEnd )
		{
			if ( iter->first.m_location == locationKey )
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
					"resetLocationCache step 5.1, inside erase loop begin, loop %lu, location key %lu.", iLoop, locationKey);
					
				ACE_Write_Guard<ACE_RW_Mutex> guard(m_locSubDutyLock);
				m_locSubDutyMap.erase( iter );
				
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
					"resetLocationCache step 5.2, inside erase loop end, loop %lu, location key %lu.", iLoop, locationKey);
			}
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"resetLocationCache step 6, Location-subystem Duty Map have size %d after reset location %d cache.", m_locSubDutyMap.size(), locationKey);
	}

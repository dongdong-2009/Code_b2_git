/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/scada/common_library/src/MmsScadaUtility.cpp $
...
namespace TA_Base_Bus
{
	...
	ScadaUtilities::MaintenanceData* MmsScadaUtility::getMmmMaintData(TA_Base_Core::IEntityData* entityData)
	{
		...
		if (dpEntityData != NULL)
		{
				...
				(mmsPeriodTime >= 0 && mmsPeriodTime <= 23))
			{
				...
				pMaintData->value = 0;
				pMaintData->timeStamp = TA_Base_Core::AlarmConstants::defaultTime;
	...
	void MmsScadaUtility::getMmsMaintDataValue (ScadaUtilities::MaintenanceData& data)
	{
		try
		{
			...
			if (dpState.get()!= NULL)
			{
				data.timeStamp = dpState->timeStamp;

/**
  ...
  * in any form.
  *
  * Source : $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/scada/datapoint_library/src/SafetyOutputDataPoint.cpp $
  * @author  HoaVu
  * @version $Revision: #5 $
		...
		if ( true == foundDerivedState )
		{
			...
			if ( true == isActionPermitted ( sessionID, TA_Base_Bus::aca_DATA_POINT_WRITE ) )
			{
				...
				if ( false == m_ControlInhibitedState )
				{
					...
					if ( true == m_selectedDerivedState->setOutputEntities ( sessionID.c_str(), auditEventLoggingEnabled, trigDp ) )
					{

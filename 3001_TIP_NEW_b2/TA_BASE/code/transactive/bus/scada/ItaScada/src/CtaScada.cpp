/**
  ...
  * in any form.
  *
  * Source : $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/scada/ItaScada/src/CtaScada.cpp $
  * @author  Sean Liew
  * @version $Revision: #5 $
...
namespace TA_Base_Bus
{
    ...
    CtaScada::~CtaScada()
	{
		...
		m_internalDataPointEntityKeyMap.clear();
		std::map < std::string, TA_Base_Bus::DataPointProxySmartPtr * >	tempMap;
		tempMap.clear();
		{
			// can't delete DataPointProxySmartPtr* from map directly, maybe dead lock. while update processing.
			TA_Base_Core::ThreadGuard guard( m_externalDataPointEntityNameMapLock );
			m_externalDataPointEntityNameMap.swap(tempMap);
		}
		// clean up the proxies
		std::map < std::string, TA_Base_Bus::DataPointProxySmartPtr * >::iterator itr1;
		for ( itr1 =	tempMap.begin();
			  itr1 != tempMap.end();
			  itr1++ )
		{
			delete (*itr1).second;
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					dataPointProxy = new DataPointProxySmartPtr();

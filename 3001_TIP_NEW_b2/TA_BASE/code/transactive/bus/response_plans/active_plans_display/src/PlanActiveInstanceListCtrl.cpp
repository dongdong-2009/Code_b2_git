/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/response_plans/active_plans_display/src/PlanActiveInstanceListCtrl.cpp $
...
void CPlanActiveInstanceListCtrl::InitialiseActivePlanList()
{
	if (!m_initialised) // want to do this once only
	{
		...
		// Instance Column
		InsertColumn(INSTANCE_COLUMN_INDEX, _T(INSTANCE_COLUMN_LABEL), LVCFMT_RIGHT, 72);
		// Location Column
		InsertColumn(LOCATION_COLUMN_INDEX, _T(LOCATION_COLUMN_LABEL), LVCFMT_LEFT, 75);
		// Owner Column 
		InsertColumn(OWNER_COLUMN_INDEX, _T(OWNER_COLUMN_LABEL), LVCFMT_LEFT, 150);
		// Activation Column 
		InsertColumn(ACTIVATION_COLUMN_INDEX, _T(ACTIVATION_COLUMN_LABEL), LVCFMT_LEFT, 145);
		// State Column

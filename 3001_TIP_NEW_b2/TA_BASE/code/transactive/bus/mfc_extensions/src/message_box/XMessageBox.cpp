/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/mfc_extensions/src/message_box/XMessageBox.cpp $
  * @author:  Ripple
  * @version: $Revision: #8 $
  *
  * Last modification: $DateTime: 2015/05/08 17:29:11 $
...
// in CMbDialogTemplate remains consistent with your changes.
...
public:
	CMbDialogItem(enum controltype cType);	// default constructor will fill in default values
	CMbDialogItem() {};						// default constructor, not to be called directly
...
class CMbDialogTemplate
{
public:
	CMbDialogTemplate(LPCTSTR lpszMessage, UINT nStyle);
	virtual ~CMbDialogTemplate();
	int Display(LPCTSTR lpszCaption);
	void AddItem(CMbDialogItem::controltype cType,
				 UINT nID, 
				 CRect* prect = NULL, 
				 LPCTSTR pszCaption = NULL);

protected:
	CMbDialogItem* m_pDlgItemArray[MAXITEMS];
};
...
static CMutex       gMutex;             // support multiple thread
XMessageBoxProxy*   XMessageBoxProxy::g_hwnd;
CMutex              XMessageBoxProxy::g_mutex;
DWORD               XMessageBoxProxy::g_thread_id;
				...
				UINT nHelpId /* = 0 */)
{
	...
	CMbDialogTemplate dlg(lpszMessage, nStyle);
...
{
#ifdef _DEBUG
    HINSTANCE hModInst = GetModuleHandle("TA_MFCExtensions_d.dll");
#else
    HINSTANCE hModInst = GetModuleHandle("TA_MFCExtensions.dll");
#endif

	if (::LoadString(hModInst, IDS_MB_OK, szOK, MAXBUTTONSTRING) == 0)
		_tcscpy(szOK, _T("OK"));
	szOK[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_CANCEL, szCancel, MAXBUTTONSTRING) == 0)
		_tcscpy(szCancel, _T("Cancel"));
	szCancel[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_IGNORE, szIgnore, MAXBUTTONSTRING) == 0)
		_tcscpy(szIgnore, _T("&Ignore"));
	szIgnore[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_RETRY, szRetry, MAXBUTTONSTRING) == 0)
		_tcscpy(szRetry, _T("&Retry"));
	szRetry[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_ABORT, szAbort, MAXBUTTONSTRING) == 0)
		_tcscpy(szAbort, _T("&Abort"));
	szAbort[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_HELP, szHelp, MAXBUTTONSTRING) == 0)
		_tcscpy(szHelp, _T("&Help"));
	szHelp[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_YES, szYes, MAXBUTTONSTRING) == 0)
		_tcscpy(szYes, _T("&Yes"));
	szYes[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_NO, szNo, MAXBUTTONSTRING) == 0)
		_tcscpy(szNo, _T("&No"));
	szNo[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_CONTINUE, szContinue, MAXBUTTONSTRING) == 0)
		_tcscpy(szContinue, _T("&Continue"));
	szContinue[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_DONOTASKAGAIN, szDoNotAskAgain, MAXBUTTONSTRING) == 0)
		_tcscpy(szDoNotAskAgain, _T("Don't ask me again"));
	szDoNotAskAgain[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_DONOTTELLAGAIN, szDoNotTellAgain, MAXBUTTONSTRING) == 0)
		_tcscpy(szDoNotTellAgain, _T("Don't tell me again"));
	szDoNotTellAgain[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_YESTOALL, szYesToAll, MAXBUTTONSTRING) == 0)
		_tcscpy(szYesToAll, _T("Yes to &All"));
	szYesToAll[MAXBUTTONSTRING-1] = _T('\0');

	if (::LoadString(hModInst, IDS_MB_NOTOALL, szNoToAll, MAXBUTTONSTRING) == 0)
		_tcscpy(szNoToAll, _T("No to A&ll"));
							...
							LPARAM)
{
	...
	switch (message)
	{
		...
		{
            gMutex.Unlock();
            XMessageBoxProxy::insideXMessageBox( hwnd );
...
///////////////////////////////////////////////////////////////////////////////
// CMbDialogTemplate class

///////////////////////////////////////////////////////////////////////////////
// CMbDialogTemplate ctor
CMbDialogTemplate::CMbDialogTemplate(LPCTSTR lpszMessage, UINT nStyle)
{
	UINT nID = 1001;
	...
	if (nStyle & MB_ICONMASK)
	{
		...
		if (lpIcon)
		{
			...
			AddItem(CMbDialogItem::STATICTEXT, 1000, &iconrect, _T(""));
		}
	}

	AddItem(CMbDialogItem::STATICTEXT, nID++, &msgrect, lpszMessage);
	...
	switch (nStyle & MB_TYPEMASK)
	{
		case MB_OK:
			...
			AddItem(CMbDialogItem::BUTTON, IDOK, &rect, szOK);
			gbOkButton = TRUE;
			break;
		...
		case MB_OKCANCEL:
			...
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDCANCEL, &rect, szCancel);
			gbCancelButton = TRUE;
			break;

		case MB_YESNO:
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDYES, &rect, szYes);

			if (nStyle & MB_YESTOALL)
			{
				x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
				rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
				AddItem(CMbDialogItem::BUTTON, IDYESTOALL, &rect, szYesToAll);
			}

			x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDNO, &rect, szNo);

			if (nStyle & MB_NOTOALL)
			{
				x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
				rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
				AddItem(CMbDialogItem::BUTTON, IDNOTOALL, &rect, szNoToAll);
			}
			break;
		...
		case MB_ABORTRETRYIGNORE:
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDABORT, &rect, szAbort);

			x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDRETRY, &rect, szRetry);

			x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDIGNORE, &rect, szIgnore);
			break;
		...
		case MB_CONTINUEABORT:
			rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
			AddItem(CMbDialogItem::BUTTON, IDCONTINUE, &rect, szContinue);
	...
	if (nStyle & MB_HELP)
	{
		x += MSGBOXEX_BUTTONWIDTH + MSGBOXEX_BUTTONSPACING;
		rect.SetRect(x, y, x + MSGBOXEX_BUTTONWIDTH, y + MSGBOXEX_BUTTONHEIGHT);
		AddItem(CMbDialogItem::BUTTON, IDEXHELP, &rect, szHelp);
	}
	...
	if (nStyle & MB_DONOTASKAGAIN)
	{
		...
		AddItem(CMbDialogItem::CHECKBOX, IDDONOTASKAGAIN, &rect, szDoNotAskAgain);
	...
	else if (nStyle & MB_DONOTTELLAGAIN)
	{
		...
		AddItem(CMbDialogItem::CHECKBOX, IDDONOTASKAGAIN, &rect, szDoNotTellAgain);
		buttonrow.bottom = y + MSGBOXEX_DONOTASKAGAIN_HEIGHT;
		mbrect.bottom = buttonrow.bottom + MSGBOXEX_SPACING;
		if (mbrect.Width() < (x + w))
			mbrect.right = mbrect.left + x + w;
...
///////////////////////////////////////////////////////////////////////////////
// CMbDialogTemplate dtor
CMbDialogTemplate::~CMbDialogTemplate()
{
...
///////////////////////////////////////////////////////////////////////////////
// CMbDialogTemplate::AddItem
void CMbDialogTemplate::AddItem(CMbDialogItem::controltype cType, UINT nID, CRect* prect, LPCTSTR pszCaption)
{
	ASSERT(m_pDlgItemArray[gdlgTempl.cdit] == NULL);

	CMbDialogItem::controltype ct = cType;
	if (ct == CMbDialogItem::CHECKBOX)
	{
		ct = CMbDialogItem::BUTTON;
	}
	m_pDlgItemArray[gdlgTempl.cdit] = new CMbDialogItem(ct);
...
///////////////////////////////////////////////////////////////////////////////
// CMbDialogTemplate::Display
int CMbDialogTemplate::Display(LPCTSTR lpszCaption)
{
...
///////////////////////////////////////////////////////////////////////////////
// CMbDialogItem ctor
CMbDialogItem::CMbDialogItem(enum CMbDialogItem::controltype ctrlType)
{
	m_controltype = ctrlType;
}

///////////////////////////////////////////////////////////////////////////////
// CMbDialogItem::AddItem
void CMbDialogItem::AddItem(enum controltype ctrltype, UINT nID, CRect* prect, LPCTSTR lpszCaption)
{
	short hidbu = HIWORD(GetDialogBaseUnits());
	short lodbu = LOWORD(GetDialogBaseUnits());

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/bus/mathematical_evaluation/src/Tokeniser.cpp $
  * @author:  Ross Tucker
  * @version: $Revision: #8 $
  *
  * Last modification: $DateTime: 2017/12/22 17:26:33 $
    ...
    void Tokeniser::parse( const std::string& p_expression)
    {
        ...
        while( i < p_expression.length() )
        {
            ...
            else
            {
                ...
                {
                    std::stringstream ss;
                    ss << "Invalid expression: " << p_expression;
                    LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError,"%s", ss.str().c_str());

...
void SimpleBrowser::Write(LPCTSTR string)
{
	if (_Browser != NULL) {
		...
		MSHTML::IHTMLDocument2 *document = GetDocument2();
...
MSHTML::IHTMLDocument* SimpleBrowser::GetDocument()
{
	MSHTML::IHTMLDocument *document = NULL;
	...
	if (_Browser != NULL) {
		...
		if (SUCCEEDED(hr) && (document_dispatch != NULL)) {
			...
			hr = document_dispatch->QueryInterface(IID_IHTMLDocument, (void **)&document);
...
MSHTML::IHTMLDocument2* SimpleBrowser::GetDocument2()
{
...
MSHTML::IHTMLDocument3 *SimpleBrowser::GetDocument3()
{
	MSHTML::IHTMLDocument3 *document = NULL;
	...
	if (_Browser != NULL) 
	{
		...
		{
			CLSID clsid;
			CLSIDFromString(_T("{3050F485-98B5-11CF-BB82-00AA00BDCE0B}"), &clsid);

			hr = document_dispatch->QueryInterface(clsid, (void **)&document);
			document_dispatch->Release();
		}	
	}
...
BOOL SimpleBrowser::LoadFromResource(UINT uiResourceID)
{
	...
	if (GetModuleFileName(hInstance, lpszModule, _MAX_PATH))
		LoadFromResource(lpszModule, uiResourceID);
	else
		bRetVal = FALSE;
...
void SimpleBrowser::LoadFromResource(LPCTSTR lpszModule, UINT uiResourceID)
{
	CString strResourceURL;
	strResourceURL.Format(_T("res://%s/%ld"), lpszModule, uiResourceID);
	_Browser->Navigate(strResourceURL.GetBuffer(0), 0, 0,0,0);
}
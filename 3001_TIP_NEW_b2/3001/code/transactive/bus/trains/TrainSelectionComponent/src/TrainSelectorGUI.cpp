/**
  ...
  *
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/bus/trains/TrainSelectionComponent/src/TrainSelectorGUI.cpp $
  * @author:   Adam Radics
  * @version:  $Revision: #7 $
  *
  * Last modification: $DateTime: 2018/01/12 13:48:54 $
...
namespace TA_IRS_Bus
{
		...
		m_isInRadioFallback(false)
    {
        ...
        LocationCache::getInstance().registerObserver(this); //for: onIsSelectable --> LocationCache
    ...
    TrainSelectorGUI::~TrainSelectorGUI()
    {
        ...
        LocationCache::getInstance().deregisterObserver(this);
										   ...
										   unsigned long trainCommunicationLocation)
    {
       ...
       bool r= LocationCache::getInstance().isInControlOfLocation( trainLocation,trainCommunicationLocation );
	   LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "[selectable=%d][trainId: %d] [location=%d][communication location=%d]",
		   r,trainId, trainLocation,  trainCommunicationLocation );
	   return r;
    }
    ...
    void TrainSelectorGUI::onLocationCacheChange()
    {
        
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "onLocationCacheChange");
		if (m_inboundList)
        {
            m_inboundList->Invalidate();
        }

        if (m_outboundList)
        {
            m_outboundList->Invalidate();
        }
    }

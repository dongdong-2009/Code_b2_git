/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/3001/transactive/bus/trains/TrainSelectionComponent/src/TrainListCtrl.cpp $
  * @author Adam Radics
  * @version $Revision: #9 $
...
#define FOREGROUND_TRAIN_OA1_LOST 255
#define FOREGROUND_TRAIN_NORMAL 0
...
namespace TA_IRS_Bus
{
	...
	TrainListCtrl::TrainListCtrl(std::string name)
		...
		m_name(name)
	{
		...
		m_locationKey= atol(TA_Base_Core::RunParams::getInstance().get(RPARAM_LOCATIONKEY).c_str());
    ...
    LRESULT TrainListCtrl::onHideScrollBars(UINT wParam, LONG lParam)
    {
    ...
    void TrainListCtrl::addToList(const TrainInformationTypes::TrainDetails& td)
    {
        ...
        int itemIndex=InsertItem(nItem, getLocationString(*itemDetails) );
		...
		//DTL-2183,set background to red if lost OA1 heartbeat with the train
		if (td.communicationValid==false || td.communicatingLocation !=m_locationKey  ) 
		{			
			setItemColour((DWORD)itemDetails,FOREGROUND_TRAIN_OA1_LOST,I_ITEMDATA,CT_FOREGROUND);
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s, update train %d: Tcolour= Red, OA=%d,CLOC=%d,LOC=%d",
				m_name.c_str(),td.trainId, td.communicationValid,td.communicatingLocation,m_locationKey);
		}
		else
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s, Add train %d: Tcolour= black, ",m_name.c_str(), td.trainId);
    ...
    void TrainListCtrl::deleteFromList(const TrainInformationTypes::TrainDetails& td)
    {
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s, deleteFromList %d: ",m_name.c_str(), td.trainId);
    ...
    void TrainListCtrl::updateList(const TrainInformationTypes::TrainDetails& td)
    {
		...
		else
		{
			...
			*itemDetails = td;
			...
			{
				setItemColour((DWORD)itemDetails,FOREGROUND_TRAIN_NORMAL,I_ITEMDATA,CT_FOREGROUND);
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s,update train %d: Tcolour= Black, ",m_name.c_str(), td.trainId);
			}
	...
	void TrainListCtrl::deleteAllTrains()
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s, deleteAllTrains",m_name.c_str());

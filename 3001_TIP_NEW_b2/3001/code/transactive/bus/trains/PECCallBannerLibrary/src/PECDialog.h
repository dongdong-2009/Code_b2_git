/**
  ...
  * @version: $Revision: #9 $
  *
  * Last modification: $Date: 2017/08/07 $
...
namespace TA_IRS_Bus
{
	...
	const DWORD PEC_UPDATE_THREAD_STATUS = WM_USER + 3;
	const DWORD PEC_CALL_TERMINATE_MSG = WM_USER + 4;
	const DWORD PEC_ENABLE_WINDOW = WM_USER + 5;
	const DWORD PEC_HIDE_DLG = WM_USER + 6;
	const DWORD PEC_SHOW_INFO_MSG = WM_USER + 7;
	const DWORD PEC_SHOW_ERR_MSG = WM_USER + 8;
	const DWORD PEC_SHOW_DLG = WM_USER + 9;
	const DWORD PEC_ENABLE_BTNS = WM_USER + 10;
	const DWORD PEC_DISABLE_BTNS = WM_USER + 11;
	const DWORD PEC_SET_FORE_WINDOW = WM_USER + 12;
	const DWORD PEC_CHANGE_OVER_FAIL_MSG = WM_USER + 13;
	const DWORD PEC_DISABLE_WINDOW = WM_USER + 14;
    ...
    public:
        ...
        void setStatusMpuChangeover();
		 ...
		 /**
         * Tells the operator that the PEC cannot be established
         */
    ...
    protected:
		...
		afx_msg LRESULT onShowCallTerminateMsg( WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT doWindowEnable( WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT doWindowDisable( WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onHideDialog( WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onShowInformationMessage( WPARAM wParam, LPARAM lParam );
        afx_msg LRESULT onShowErrorMessage( WPARAM wParam, LPARAM lParam );
        afx_msg LRESULT onShowDialog( WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onDisableAllButtons(WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onEnableAllButtons(WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onSetForeWindow(WPARAM wParam, LPARAM lParam );
		afx_msg LRESULT onShowChangeOverFailMsg(WPARAM wParam, LPARAM lParam );
		...
		afx_msg LRESULT updateThreadStatus(WPARAM wParam, LPARAM lParam);
    ...
    private:
        ...
        */
		...
		static const std::string PECDialog::STATUS_CONNECTION_ERROR;

		/**
		 * The window message box caption for PEC dialog message
		 */
		static const std::string PEC_MESSAGE_BOX_CAPTION;
		...
		static HHOOK m_hhookCBTProc;
		
		// this function manage to check the coupled train from PEC source.
		void setTvssButtonVisibility ();
		
	private:
		
		bool m_bWasHoldReset;

		//this thread is to handle CORBA call, thus not to hang the whole dialog
		TA_Base_Core::ThreadPoolManager*	m_threadPoolManager;

		BOOL m_bIsBackgroundThreadBusy;

    };

	typedef boost::shared_ptr< PECCallWorkItem > PECCallWorkItemPtr;

...
namespace TA_IRS_Bus
{
    ...
    TrainStateLock::~TrainStateLock()
    {
        ...
        if ( dataChanged == true )
        {
			...
			// add log for troubleshooting of sending message
		    LOG4(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"notifyStateChanged CoreResource [trainId: %d], ResCategroy=%d, SyncReq=%d, Peris=%d", 
				m_state->getTrainId(), (int)m_state->getResourceCategory(), (int)m_state->isSynchronisationRequired(), (int)m_state->hasPeristedStateChanged());

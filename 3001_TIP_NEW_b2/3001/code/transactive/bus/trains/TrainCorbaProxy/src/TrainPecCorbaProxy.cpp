...
namespace TA_IRS_Bus
{
										...
										const std::string& sessionId, PecTypes::EPecTrainSource pecTrainSource )
    {
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"answerPec: TrainId=%d, car=%d, pec=%d, sessioId=%s, pecTrainSource=%d",
			static_cast<int> (trainId),
			static_cast<ProtocolPecTypes::EPecCarNumber> ( car ),
			static_cast<ProtocolPecTypes::EPecNumber> ( pec ),
			sessionId.c_str(),
			static_cast<PecTypes::EPecTrainSource> ( pecTrainSource ));
        ...
        CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, answerPec, ( trainId,
																			  ...
																			  sessionId.c_str(), PecTypesConversion::convertToCorba( pecTrainSource ) ) ) );//limin
									   ...
									   bool bCallTerminated)
    {
		...
		if (!bCallTerminated)
		{
			...
			CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, resetPec, ( trainId,
																				  ...
																				  PecTypesConversion::convertToCorba( pec ),
																				  sessionId.c_str(),
																				  PecTypesConversion::convertToCorba( pecTrainSource )) ) );//limin
		}
		else
		{
			CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, resetTerminatedPec, ( trainId,
									 PecTypesConversion::convertToCorba( car ),
									 PecTypesConversion::convertToCorba( pec ),
									 sessionId.c_str(),
									 PecTypesConversion::convertToCorba( pecTrainSource )) ) );//limin
										  ...
										  bool bCallTerminated)
    {
		...
		if (!bCallTerminated)
		{
			CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, resetAllPec, ( trainId, sessionId.c_str(), 
									PecTypesConversion::convertToCorba( pecTrainSource )) ) );//limin
		}
		else
		{
			CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, resetAllPecAtPecTermination, ( trainId, sessionId.c_str(), 
									PecTypesConversion::convertToCorba( pecTrainSource )) ) );//limin
		}
    ...
    void TrainPecCorbaProxy::holdPec( CommonTypes::TrainIdType trainId,
                                      ProtocolPecTypes::EPecCarNumber car,
                                      ProtocolPecTypes::EPecNumber pec,
									  const std::string& sessionId, PecTypes::EPecTrainSource pecTrainSource )
    {
        ...
        // Do parameter conversion using the appropriate convertToCorba methods
        // call the corba method, catching and wrapping any CORBA exceptions in C++ exceptions
        CORBA_CATCH( CORBA_CALL( m_pecCorbaObject->flexibleTimeoutNamedObject, holdPec, ( trainId,
                                                                                          PecTypesConversion::convertToCorba( car ),
                                                                                          PecTypesConversion::convertToCorba( pec ),
                                                                                          sessionId.c_str(),
																						  PecTypesConversion::convertToCorba( pecTrainSource ) ) ) );//limin

...
namespace TA_IRS_Bus
{
    ...
    void TrainStateUpdateCorbaProxy::receiveSpecialisedMessage( const TA_Base_Core::StateUpdateMessageCorbaDef& payload )
    {
        ...
        if ( payload.stateUpdateInfo >>= agentOperationMode )
        {
            ...
            std::set< unsigned long >::iterator ite = cppType.controlledLocations.begin();

			//Fixed crash if controlledLocations is empty
			//Should not process Agent operation mode if controlledLocations is empty
			if ( ite == cppType.controlledLocations.end() )
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"Will not process operation mode due to empty Controlled Locations..");
				return;
			}

            locations << *( ite++ );
            for ( ; ite != cppType.controlledLocations.end(); ++ite )
            {
                locations << ", " << *ite;
            }

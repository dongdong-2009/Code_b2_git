...
namespace TA_IRS_Bus
{
    ...
    void TimsCommand::setLong( unsigned short position, unsigned long value )
    {
        FUNCTION_ENTRY( "setWord" );

        unsigned char lowestByte = static_cast< unsigned char >( value & 0x000000FF ); // retrieve the lowest byte
        unsigned char lowerByte = static_cast< unsigned char >( (value & 0x0000FF00) >> 8 ); // retrieve the lower byte
        unsigned char higherByte = static_cast< unsigned char >( (value & 0x00FF0000) >> 16 ); // retrieve the higher byte
        unsigned char highestByte = static_cast< unsigned char >( (value & 0xFF000000) >> 24 ); // retrieve the highest byte

        // little endian, so the lower byte goes first.
        setByte( position, lowestByte );
        setByte( position + 1, lowerByte );
        setByte( position + 2, higherByte );
        setByte( position + 3, highestByte );

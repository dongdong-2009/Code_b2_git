/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/cctv/video_display_manager/src/SequenceEditorDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #5 $
  *
...
namespace
{
    static const int MAX_SEQ_NAME_CHARACTERS = 100;
...
namespace TA_IRS_App
{
    ...
    void SequenceEditorDlg::enableSaveButton()
    {
        ...
        m_saveButton.EnableWindow(
            ...
            (m_existingDwellTime != newDwellTime && m_dwellSpinCtrlPreviousValue != newDwellTime && (!m_sequenceVideoSources.empty())) ||
            m_changed );
    ...
    BOOL SequenceEditorDlg::OnInitDialog()
    {
        ...
        m_sequenceDwellTime.GetWindowText(m_dwellSpinCtrlPreviousValue);
        ...
        if (m_sequence->getReadOnly())
        {
			...
			//if(TA_Base_Core::RunParams::getInstance().isSet("DisableSequenceDialog"))
			{
				m_cameraTreeCtrl.EnableWindow(FALSE);
				m_sequencesListBox.EnableWindow(FALSE);
				m_saveButton.EnableWindow(FALSE);
				m_sequenceDescription.EnableWindow(FALSE);
				m_spinControl.EnableWindow(FALSE);
			}
        }
        else
        {
            ...
            if (ret == IDOK)
            {
                ...
                try
                {
                    ...
                    if ( ( m_sequenceVideoSources.size() > 0 ) && ( dwellTime == 0 ) )
                    {
						...
						AfxMessageBox("Error encounter when performing the requested operation" );
                        FUNCTION_EXIT;
                        return;
                    }
        ...
        else
        {
            ...
            try
            {
                if ( isSourcesModified() || ( m_existingDwellTime != newDwellTime ) )
                {
                    ...
                    m_dwellSpinCtrlPreviousValue = newDwellTime;
                    m_existingDwellTime = newDwellTime;
                    m_changed = FALSE;
                }
                ...
                if ( newDescription != m_sequence->getSequenceDescription().c_str() )
                {
                    SwitchAgentCommunicator::getInstance().setSequenceDescription( m_sequence, newDescription.GetBuffer(0) );
                }

                // Submit audit message, cause agent may rise twice at same time
                try
                {
                    std::auto_ptr<TA_Base_Core::AuditMessageSender> auditMsgSender
                        (TA_Base_Core::MessagePublicationManager::getInstance().getAuditMessageSender(TA_Base_Core::VisualAudit::Context));
                    TA_Base_Core::DescriptionParameters params;
            		TA_Base_Core::NameValuePair blankParam("Blank", " ");
                    TA_Base_Core::EntityHelper helper(m_sequence->getKey(), TA_Base_Core::Sequence::getStaticType());
            		TA_Base_Core::NameValuePair sequenceID("SequenceID", helper.getAddress());
        			params.clear();
        			params.push_back(&blankParam);
        			params.push_back(&sequenceID);
                    std::string sessionId = TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID);
                    auditMsgSender->sendAuditMessage(TA_Base_Core::VisualAudit::VisualEditSequence,
                        m_sequence->getKey(),
                        params,
                        "",
                        sessionId,
    ...
    void SequenceEditorDlg::OnSeUpButton()
    {
        ...
        // no random access iterator for lists
        for (int i = 0; i < (curSel-1); i++) {theIterator++;}
    ...
    LRESULT SequenceEditorDlg::onSequenceUpdated(WPARAM wParam, LPARAM lParam)
    {
        ...
        if (input == m_sequence)
        {
            ...
            // if the seqeunce is empty - the dwell time is 0.
            // set it to the default dwell time
            // Comment this, only new sequence need a default dwell time
            /*
            if (dwellTime == 0)
            {
            ...
            if ( !same || dwellTimeNeedsUpdate )
            {
                     ...
                     ((currentTime - m_lastUpdatedTime) > UPDATE_THRESHOLD_TIME) )
                {
                   ...
                   // UINT selectedButton = userMsg.showMsgBox(IDS_UI_090072);
					//AfxMessageBox( "Error encounter when performing the requested operation");
			        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Sequence config change too frequently");

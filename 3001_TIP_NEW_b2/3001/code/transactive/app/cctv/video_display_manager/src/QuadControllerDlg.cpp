/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/cctv/video_display_manager/src/QuadControllerDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2014/03/06 14:02:42 $
...
namespace TA_IRS_App
{
    ...
    int QuadControllerDlg::DoModal(Monitor* monitor)
    {
        FUNCTION_ENTRY( "DoModal" );

        TA_ASSERT(monitor != NULL, "NULL quad object given to quad controller dialog");
    ...
    BOOL QuadControllerDlg::OnInitDialog()
    {
        ...
        TA_ASSERT(m_monitor != NULL, "Cannot load a quad controller dialog with a NULL quad");
		...
		windowText += m_monitor->getName().c_str();
    ...
    void QuadControllerDlg::loadExistingSegmentAssociations()
    {
        ...
        TA_ASSERT(m_monitor != NULL, "Cannot load existing quad associations when m_quad is NULL");
        //m_segmentAssociations = m_quad->getCameras();
		m_segmentAssociations = m_monitor->getQuadCameras();
    ...
    void QuadControllerDlg::registerLabels()
    {
        ...
        for( int i = my_first_monitor; i <= my_last_monitor; i++ )
        {
            ...
            pLabel->ModifyStyle(0, SS_CENTER | SS_WORDELLIPSIS);
            pLabel->SetFontName("Microsoft Sans Serif");
    ...
    void QuadControllerDlg::paintHilitedControl()
    {
        ...
        for ( theIterator = m_monitors.begin(); theIterator != m_monitors.end(); theIterator++ )
        {
            ...
            {
                ( *theIterator )->SetBitmap( ::LoadBitmap( AfxGetInstanceHandle(),MAKEINTRESOURCE( IDB_VD_WALL_H ) ) );
    ...
    void QuadControllerDlg::paintStandardControl()
    {
        ...
        for ( theIterator = m_monitors.begin(); theIterator != m_monitors.end(); theIterator++ )
        {
            ...
            {
                ( *theIterator )->SetBitmap( ::LoadBitmap( AfxGetInstanceHandle(), MAKEINTRESOURCE( IDB_VD_WALL ) ) );
    ...
    void QuadControllerDlg::adjustApplyButton()
    {
       ...
       // std::vector<Camera*> cameras = m_quad->getCameras();
		TA_ASSERT(m_monitor != NULL, "Cannot test against a NULL monitor");
	    std::vector<Camera*> cameras = m_monitor->getQuadCameras();
        for (int i = 0; i < 4; i++)
        {

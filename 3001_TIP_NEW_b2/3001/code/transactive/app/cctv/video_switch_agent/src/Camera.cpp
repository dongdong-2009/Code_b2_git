/**
  ...
  *
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/cctv/video_switch_agent/src/Camera.cpp $
...
namespace TA_IRS_App
{
    ...
    TA_Base_Bus::CameraCorbaDef::SupportedCameraFeatures Camera::getSupportedFeatures()
    {
        ...
        TA_Base_Bus::CameraCorbaDef::SupportedCameraFeatures features;
    ...
    TA_Base_Bus::CameraCorbaDef::CameraState Camera::getState()
    {
        ...
        try
        {
            // If the Camera is focus or iris controllable and it supports the given features,
            // get the focus and iris state.
            ...
            // If the Camera is movement controllable and it supports presets, get the active preset.

            if ( 0 != m_movementControl.get() )
            {
                if ( m_movementControl->isFeatureSupported( TA_Visual::Presets ) )
                {
                    TA_Base_Bus::MovementControlCorbaDef::CameraPreset* preset = m_movementControl->getActivePreset();
                    if ( 0 != preset )
                    {
            ...
            if ( 0 != m_pdCamera.get() )
            {
                state.isInAlarmState = m_pdCamera->isInAlarmState();
            }
        }
    ...
    TA_Base_Bus::MovementControlCorbaDef_ptr Camera::getMovementControl()
    {
        ...
        if( 0 == m_movementControl.get() )
        {
            throw TA_Base_Bus::VideoSwitchAgentException( "Camera does not support movement control" );
        }
    ...
    TA_Base_Bus::FocusIrisControlCorbaDef_ptr Camera::getFocusIrisControl()
    {
        ...
        if( 0 == m_focusIrisControl.get() )
        {
            throw TA_Base_Bus::VideoSwitchAgentException( "Camera does not support focus or iris control" );
        }
    ...
    TA_Base_Bus::WashWipeControlCorbaDef_ptr Camera::getWashWipeControl()
    {
        ...
        if( 0 == m_washWipeControl.get() )
        {
            throw TA_Base_Bus::VideoSwitchAgentException( "Camera does not support wash or wipe control" );
        }
    ...
    void Camera::createControls()
    {
        ...
        try
        {
            ...
            if ( PDInterfaceFactory::isMovementControlSupported() )
            {
            ...
            if ( PDInterfaceFactory::isFocusIrisControlSupported() )
            {
            ...
            if ( PDInterfaceFactory::isWashWipeControlSupported() )
            {
                // Check that this Camera supports focus and iris control.

...
//limin++, failover
	...
	bool ObservedSocket::connect()
	{
		...
		//ACE_UINT32 lAddr= m_remoteAddress.get_ip_address();
		//memcpy(chAddr,(unsigned char*)&lAddr,sizeof(chAddr));
		//LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Ip Address :%d.%d.%d.%d,Port: %u",chAddr[0],chAddr[1],chAddr[2],chAddr[3], m_remoteAddress.get_port_number());
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Ip Address %s:%u",m_remoteAddress.get_host_addr(), m_remoteAddress.get_port_number());
		...
		//	throw("Failed to TCP conncection");//TA_IRS_App::TcpMethodTimeoutException());
		}
		else
		{
			m_isConnected = true;

...
boost::shared_ptr<AtsITcpEvent> TcpTransactionManager::waitForEvent(unsigned long transactionID)
{
	FUNCTION_ENTRY("waitForEvent");

	// Wait on our semaphore - will block until event or timeout
	LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, 
  	     "Waiting for trainId %lu",transactionID);
m_waitTimeout=100;// ONLY FOR TESTING
	...
	LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, 
  	     "Wait for trainId %x complete.",transactionID);

	// Get the passed event
	boost::shared_ptr<AtsITcpEvent> event = waitListGetEvent(transactionID);
...
* @param    event
*/
bool TcpTransactionManager::receivedEvent(const AtsEvent& event, unsigned long transactionID )
{
	FUNCTION_ENTRY("receivedEvent");

	// Make a copy of the event to pass to the waiting thread
	boost::shared_ptr<AtsEvent> event_copy(new AtsEvent(event));
		   ...
		   if (it->elapseMilliSecs>m_waitTimeout)
		   {
			   ...
			   it->waitSemaphore->post();
				LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, 
  					 "Wait for train Id %x timed out.",it->transactionID);
		   }
		}
	}
...
bool TcpTransactionManager::waitListSignalTransaction(unsigned long transactionID,boost::shared_ptr<AtsITcpEvent> event)
{
...
boost::shared_ptr<AtsITcpEvent> TcpTransactionManager::waitListGetEvent(unsigned long transactionID)
{
	FUNCTION_ENTRY("waitListGetEvent");
	std::vector<TransactionEntry>::iterator it;
	boost::shared_ptr<AtsITcpEvent>    event;

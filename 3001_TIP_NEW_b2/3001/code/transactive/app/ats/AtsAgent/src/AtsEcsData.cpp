/**
  ...
  * 
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/ats/AtsAgent/src/AtsEcsData.cpp $
  * @author:   Darren Sampson
  * @version:  $Revision: #4 $
  *
  * Last modification: $DateTime: 2013/04/30 14:19:19 $
...
namespace TA_IRS_App
{
	...
	bool AtsEcsData::inThisSector(unsigned long stationId)
	{
		...
		return true ; //c955 no this concept already
								  ...
								  unsigned long	headCarAbscissaOnFb)
	{
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
			"Track Circuit Number = %d, Track Circuit Offset = %d, Arm Id = %d",
			fixedBlocksBranchId,fbAbscissaInTheBranch,headCarAbscissaOnFb);

		try
		{
			stationZoneId = (unsigned long)m_stationZones.getZoneId(fixedBlocksBranchId, fbAbscissaInTheBranch, headCarAbscissaOnFb);
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"stationZoneId = %d",stationZoneId);
		...
		try
		{
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"ecsZoneId = %d",ecsZoneId);
		}
		catch (TA_Base_Core::DatabaseException& e)
		{
	...
	void AtsEcsData::updateExternalTemperatureInCelsius(double externalTemperature)
	{
		if (std::abs(externalTemperature - m_atsEcsMessage.oa1Data.maxExternalTemperatureInCelsius)>0.01)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateExternalTemperatureInCelsius() - train %d temperature, was %.2f now %.2f", 
...
// 				= new TA_IRS_Bus::IAtsEcsCorbaDef::AtsEcsMessageCorbaDef(buildAtsEcsMessage());
			...
			LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
					"sendUpdates() - Info: AtsEcsMessage sent - train=%d, entity=%d, location=%d, localisation=%s, ecsZoneId=%d,"
					"stationZoneId=%d, serviceNumber=%d, scheduleNumber=%d, oa1=%s, emergency=%s,"
					"stallTimeInSeconds=%d, maxExternalTempInCelsius=%.2f",
					...
					m_atsEcsMessage.localisation.stationZoneId,
					m_atsEcsMessage.localisation.serviceNumber,
					m_atsEcsMessage.localisation.scheduleNumber,
					(m_atsEcsMessage.oa1Data.valid?"VALID":"INVALID"),
					(m_atsEcsMessage.oa1Data.isInEmergency?"YES":"NO"),
					m_atsEcsMessage.localisation.stallTimeInSeconds,
					m_atsEcsMessage.oa1Data.maxExternalTemperatureInCelsius);

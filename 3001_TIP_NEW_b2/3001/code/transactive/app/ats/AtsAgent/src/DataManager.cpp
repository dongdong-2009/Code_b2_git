/**
  ...
  * 
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/ats/AtsAgent/src/DataManager.cpp $
  * @author:   Ross Tucker
  * @version:  $Revision: #6 $
  * @see:      9999-Q01-02-0016-GenericAgent_Usage_Guidelines.doc
  *
  * Last modification: $DateTime: 2018/03/08 11:04:00 $
  * Last mofified by:  $Author: jiayi.zhang $
  *
...
namespace TA_IRS_App
{
	  ...
	  m_secondaryServer("")
	{
		...
		//Get AtsServer for AtsMethod.
		getAtsServer();
		m_atsServer = new AtsTcpServer(m_primaryServer,m_secondaryServer);
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
			"\n\t\t\tLocation Type: %d",m_locationType);
		...
		m_watchDogData = new WatchDogData(m_locationType,
										  ...
										  m_entityData->getConnectionFailedTimeout());
		...
		for (physicalTrainNumber = 1; physicalTrainNumber <= 150; physicalTrainNumber++) //ICD B.3.1.2
		{
			...
			AtsCctvData* cctvData = new AtsCctvData(physicalTrainNumber, m_locationKey, *m_stationZones);
		...
		{
		//	m_twpData = new AtsTwpData(entityKey, m_locationKey, *m_dataPointHelper);//14/02/2011 21

		//	TA_ASSERT(m_twpData != NULL, "Fatal: TWP Data can not be created");  //14/02/2011 21
		}
...
//14/02/2011 21
		...
		//	m_dpControlStateProcessor = new DataPointControlStateProcessor(); //14/02/2011 21


		//	m_dpControlStateProcessor->InitialisePsdData(psdData);////14/02/2011 21


		//	m_dpControlStateProcessor->start(); ////14/02/2011 21
											...
											const TA_IRS_Bus::IAtsTrainCorbaDef::OctetSequence& oa1)
	{
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, 
				"sendAtcTrainStatusReportToAts() Train ID: %d",physicalTrainNumber);
		...
		try
		{
			...
			trainData.startAtsServer(m_atsServer);
			trainData.updateAtsMethodOa1Table(oa1);
												     ...
												     const TA_IRS_Bus::IAtsTrainCorbaDef::OctetSequence& atcTrainStatus)
	{
		...
		try
		{
			...
			trainData.startAtsServer(m_atsServer);
			trainData.updateAtsMethodTrainStatusReport(atcTrainStatus);
	...
	void DataManager::run()
    {
        ...
        while (m_keepRunning)
        {
			...
			else
			{
				...
				{
					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"(0=AgentNormal,1=AgentRadioFallback,2=AgentOccFailure,3=AgentIscsDegraded) AgentState = %d",m_operationModeManager.getAgentState());
			...
			// Train
			{
				...
				for (trainIt = m_trainDataMap.begin(); trainIt != m_trainDataMap.end(); trainIt++)
				{
					if ((*trainIt).second != NULL)
					{
						...
						{
							LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Occ AddTrainUpdate...");
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Thread:keepRunning...., m_pasDataMap = %d",m_pasDataMap.size());
			if (m_locationType != TA_Base_Core::AtsAgentEntityData::Depot)
			{
				PasDataMapIt pasIt;
           ...
           /* if (m_locationType == TA_Base_Core::AtsAgentEntityData::Depot)// && m_operationModeManager.getAgentState() == OperationModeManager::AgentNormal)
			...
			if (m_occUpdateProcessor != NULL &&
				m_locationType == TA_Base_Core::AtsAgentEntityData::Occ)
			{
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"sendOccUpdates to stations...");
	...
	{		
		// Lucky: Need to Initialize AtsPasData for the Station to update the data from the Platform Table
		TA_IRS_Core::IAtsPlatforms platforms;
	...
	bool DataManager::ensureNormalMode()
	{
		...
		try
		{
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Resynchronising with the OCC Agent...");
			...
			//Lucky: Only ECS and Train data have OCC Updates.
			/*
	...
	void DataManager::updateControlModeStatus(bool inControl)
	{
		...
		//Newly added.
		/*if ( m_atsTcpServer != NULL)
			m_atsTcpServer->setToControlMode();*/
	...
	void DataManager::getAtsServer()
	{
		//get Primary Server
		std::string primaryServer = m_entityData->getAtsServer1IpAddress();
		//get the Primary Port
		std::string primaryPort = "49502";
		//get Secondary Server
		std::string secondaryServer = m_entityData->getAtsServer2IpAddress();
		//get Secondary Port
		std::string secondaryPort = "49502";
		//append primaryServer with ":"
		primaryServer.append(":");
		//then append the primaryPort
		primaryServer.append(primaryPort);
		//append secondaryServer with ":"
		secondaryServer.append(":");
		//then append the secondaryPort
		secondaryServer.append(secondaryPort);
		
		m_primaryServer = primaryServer;

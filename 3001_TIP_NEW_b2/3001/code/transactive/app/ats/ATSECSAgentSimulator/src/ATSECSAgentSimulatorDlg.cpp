/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ats/ATSECSAgentSimulator/src/ATSECSAgentSimulatorDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #3 $
  *
...
ATSECSAgentSimulatorDlg::ATSECSAgentSimulatorDlg(TA_Base_Bus::IGUIAccess& genericGUICallback, CWnd* pParent /*=NULL*/)
  ...
  m_randomGenerator(NULL)
{
    ...
    try
    {
        m_zoneIds = TA_IRS_Core::ECSZoneAccessFactory::getInstance().getAllECSZones();
    }
...
BOOL ATSECSAgentSimulatorDlg::OnInitDialog() 
{
    ...
    char zoneId[20];
    std::vector<TA_IRS_Core::IECSZone*>::iterator it = m_zoneIds.begin();
    for ( ; it != m_zoneIds.end() ; ++it)
    {
        sprintf(zoneId, "%ld", (*it)->getECSZoneId());

        int nextIndex=m_ECSZoneLst.InsertItem(m_ECSZoneLst.GetItemCount(), zoneId); 
	    m_ECSZoneLst.SetItemText(nextIndex,1,(*it)->getTrack().c_str());
	    m_ECSZoneLst.SetItemText(nextIndex,2,(*it)->getECSLocation().c_str());
        m_ECSZoneLst.SetItemData(nextIndex,static_cast<DWORD>((*it)->getECSZoneId()));
    }
...
void ATSECSAgentSimulatorDlg::OnResetBtn() 
{
    ...
    {
        // Send packet to ECS Agents
        ECSAgentUpdater::getInstance()->sendUpdate( (*it)->getECSZoneId(), 0, 0, 0 );
        updateList( (*it)->getECSZoneId(), 0, 0, 0 );
    }
    m_stallTimeTxt.SetWindowText("0");
    m_temperatureTxt.SetWindowText("0");

/**
  ...
  * 
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/maintenance_management/mms_agent/src/MmsApiWrapper.cpp $
...
namespace TA_IRS_App
{
	...
	void MmsApiWrapper::sendMessageNoRetry(const std::string& message, MmsApiWrapper::messageType type)
	{
		...
		try //try and send the message
		{
			...
			int result = MmsApi::getInstance().put_Q("ISCS", "MMS", const_cast<char*>(msgCopy.c_str()), const_cast<char*>(typeStr.c_str()), msgCopy.length() );
	...
	void MmsApiWrapper::exitQ()
	{
		...
		for(int i = 0; i < NUM_TRIES; i++)
		{
			...
			catch(MmsConnectionException& ex)
			{
				...
				{
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
						"exitQNoRetry() throw %d times. sendMessage() is now going to throw", NUM_TRIES);
					throw ex;
				}
			}
		}
	...
	void MmsApiWrapper::exitQNoRetry()
	{
		try
		{
			...
			if(1 == MmsApi::getInstance().exit_Q())
			{
				return;
			}

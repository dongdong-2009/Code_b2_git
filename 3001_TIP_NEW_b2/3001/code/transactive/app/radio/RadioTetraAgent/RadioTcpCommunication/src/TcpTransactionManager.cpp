...
*/
unsigned short TcpTransactionManager::getNewTransactionID()
{
...
boost::shared_ptr<ITcpEvent> TcpTransactionManager::waitForEvent(unsigned short transactionID)
{
	...
	if ( m_waitTimeout > 0 )
	{
		bool bWait = waitListGetSemaphore(transactionID)->timedWait( m_waitTimeout );

		if (bWait)
		{
			LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, 
				"Transaction %lu event recieve",transactionID);
		}
		else
		{
			LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, 
				"Transaction %lu expire",transactionID);
		}
	...
	// Remove the transaction - if no m
 	...
 	{
 		LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, 
 			"Deleting transaction %lu",transactionID);
...
* @param    event
*/
bool TcpTransactionManager::receivedEvent(const RadioEvent& event, unsigned short transactionID )
{
	FUNCTION_ENTRY("receivedEvent");

	// Make a copy of the event to pass to the waiting thread
	boost::shared_ptr<RadioEvent> event_copy(new RadioEvent(event));
...
void TcpTransactionManager::waitListNewTransaction(unsigned short transactionID)
{
...

	{
		...
		LOG_GENERIC (SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
					 "Adding TransId : %d, WaitListTransaction Buffer Count : %d",
					  transactionID,m_waitList.size());
...
bool TcpTransactionManager::waitListSignalTransaction(unsigned short transactionID,boost::shared_ptr<ITcpEvent> event)
{
...
TA_Base_Core::TimedWaitSemaphore* TcpTransactionManager::waitListGetSemaphore(unsigned short transactionID)
{
...
unsigned short TcpTransactionManager::waitListGetElapsedTime(unsigned short transactionID)
{
...
boost::shared_ptr<ITcpEvent> TcpTransactionManager::waitListGetEvent(unsigned short transactionID)
{
...
void TcpTransactionManager::waitListDeleteTransaction(unsigned short transactionID)
{
	...
	
	{
		...
		for (it=m_waitList.begin();it!=m_waitList.end(); ++it)
		{
			if (it->transactionID==transactionID)
			{
				...
				LOG_GENERIC (SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
							 "Erasing TransId : %d, WaitListTransation Size : %d",
							 transactionID,m_waitList.size());

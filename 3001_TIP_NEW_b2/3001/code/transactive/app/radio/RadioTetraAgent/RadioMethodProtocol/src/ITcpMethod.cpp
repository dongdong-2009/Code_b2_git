...
* 
*/
ITcpMethod::ITcpMethod(unsigned long methodLength,unsigned long methodStxStartPosition)
		    : m_messageBuffer(0),
			  m_messageSize(methodLength)
{
	...
	//if(methodHeaderLength > 0)
		//setLong(methodStxStartPosition,m_messageSize);
}
...
void ITcpMethod::setParameterString(unsigned int position,std::string value)
{
	for (int index=0; index<value.length();index++)
	{
		setByte(position+index ,value[index]);
	}
}

 ...
 * in any form.
 *
 * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui_icdv4/src/RadioSelectedEntity/SelectedEntityOperations/CEntityOperationsAreaDlg.cpp $
...
CEntityOperationsAreaDlg::~CEntityOperationsAreaDlg()
{
	...
	The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_VOLUME_CHANGED);
...
BEGIN_MESSAGE_MAP(CEntityOperationsAreaDlg, CDialog)
	...
	ON_MESSAGE(WM_VOLUME_CHANGED, OnNotifiedVolumeChanged)
...
BOOL CEntityOperationsAreaDlg::OnInitDialog() 
{
	...
	The_RadioOperations::instance()->m_observerHelper.addChildObserver(this,WM_VOLUME_CHANGED);
...
LRESULT CEntityOperationsAreaDlg::OnNotifiedVolumeChanged(WPARAM pVolumeChangeData, LPARAM volume)
{
	//Prototype from: RadioOperations::onVolumeChangedEvent(radioResource,volume)
	RadioResource* radioResource = (RadioResource*) pVolumeChangeData;

	if (radioResource->id == m_RadioResource.id)
	{
...
LRESULT CEntityOperationsAreaDlg::OnNotifiedSelectSpeakerSelected(WPARAM pSelectSpeakerData, LPARAM unused)
{
	RadioResource* radioResource = (RadioResource*)pSelectSpeakerData;

	if (radioResource->id == m_RadioResource.id)
	{
		//m_selectedSpeakerButton must be "CheckBox Style";
		m_selectedSpeakerButton.SetCheck(1);

		m_staticText.SetWindowText("Selected Volume");

//TD18785
		The_RadioSystemStatus::instance()->addSelectSpeakerResource(*radioResource);
//TD18785
	}
...
LRESULT CEntityOperationsAreaDlg::OnNotifiedSelectSpeakerUnselected(WPARAM pSelectSpeakerData, LPARAM unused)
{
	RadioResource* radioResource = (RadioResource*)pSelectSpeakerData;
	...
	if (radioResource->id == m_RadioResource.id)
	{
		...
		m_staticText.SetWindowText("Unselected Volume");

//TD18785
		The_RadioSystemStatus::instance()->deleteSelectSpeakerResource(*radioResource);
//TD18785

	}
	else if (!The_RadioSystemStatus::instance()->getResource_isSelected(m_RadioResource)) // Transactive-managed resource
	{
		m_selectedSpeakerButton.SetCheck(0);
...
//TD18785
		The_RadioSystemStatus::instance()->deleteSelectSpeakerResource(m_RadioResource);
//TD18785

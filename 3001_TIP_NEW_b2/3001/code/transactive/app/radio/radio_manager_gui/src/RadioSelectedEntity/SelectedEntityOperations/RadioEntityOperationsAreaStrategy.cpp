/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui/src/RadioSelectedEntity/SelectedEntityOperations/RadioEntityOperationsAreaStrategy.cpp $
  * @author:  F. Stuart
  * @version: $Revision: #2 $
  *
...
void RadioEntityOperationsAreaStrategy::answer()
{
	...
	unsigned long lDriverCall = The_RadioOperations::instance()->isDriverCall(m_CEntityOperationsAreaDlg->m_RadioResource.id);

	if (NOTDRIVERCALL == lDriverCall)
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"Doing private call from radio mft agent");
		The_RadioOperations::instance()->answerPrivateCall(m_CEntityOperationsAreaDlg->m_RadioResource);
	}
	else
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"Doing private call from train agent");
		The_RadioOperations::instance()->makeDriverCall(m_CEntityOperationsAreaDlg->m_RadioResource);
	}

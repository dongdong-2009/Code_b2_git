/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui/src/TabCtrlEx.cpp $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
...
CTabCtrlEx::CTabCtrlEx() : m_installed(FALSE)
{
}
...
void CTabCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpd)
{
	...
	HBRUSH h;
	h = CreateSolidBrush(RGB(250,250,250));
	...
	if ( lpd->itemState & ODS_SELECTED ) {
		...
		CRect rect1( lpd->rcItem );
		FillRect( lpd->hDC, rect1, (HBRUSH)(CTLCOLOR_DLG + 1) );
		CDC hDC1;
		CPen grayPen( PS_SOLID, 1, GetSysColor(COLOR_3DSHADOW) );
		CPen orangePen (PS_SOLID, 4, RGB(255,165,70));
		CPen darkerOPen (PS_SOLID, 1, RGB(255,127,36));

		hDC1.Attach( lpd->hDC );
		CPen* pOldPen = hDC1.SelectObject( &grayPen );
		int x1,x2,y1,y2 = 0;
		x1 = rect1.TopLeft().x;
		y1 = rect1.TopLeft().y;
		...
		hDC1.MoveTo(x1,y1);
		hDC1.LineTo(x1 + rect1.Width()-1, y1);
		...
		hDC1.MoveTo(x2 - rect1.Width(), y2);
		hDC1.LineTo(x2-1,y2);
		hDC1.SelectObject(&orangePen);
		/*hDC1.MoveTo(x2 - rect1.Width(), y2);
		hDC1.LineTo(x2-1,y2);*/
		hDC1.MoveTo(x1+1,y1+1);
		hDC1.LineTo(x1+1,y2-1);
		hDC1.SelectObject(&darkerOPen);
		hDC1.MoveTo(x1,y1+1);
		hDC1.LineTo(x1,y2-1);
		hDC1.Detach();
		...
		hDC1.Attach( lpd->hDC );
		CPen* pOldPen = hDC1.SelectObject( &whitePen );
		int x1,x2,y1,y2 = 0;
		x1 = rect1.TopLeft().x;
		y1 = rect1.TopLeft().y;
		x2 = rect1.BottomRight().x;
    ...
    {
        drawStyle = DT_SINGLELINE | DT_LEFT | DT_BOTTOM;
    }
    else
    {
        drawStyle = DT_SINGLELINE | DT_LEFT | DT_VCENTER;
    }
	...
	if ( bIsEnabled ) {
		...
		// Draw the text
		DrawText( lpd->hDC, buf, -1, rect, drawStyle );
	}
	// Draw enabled
	else {
		// Set the text color for our hilight (remember the original color)
		...
		// Draw the text
		DrawText( lpd->hDC, buf, -1, rect + CPoint(1,1), drawStyle );

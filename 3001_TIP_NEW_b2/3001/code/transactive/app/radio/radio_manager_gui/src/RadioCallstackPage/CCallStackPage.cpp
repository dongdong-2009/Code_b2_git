 ...
 * in any form.
 *
 * Source:   $File: //depot/4677_TIP/4677/transactive/app/radio/radio_manager_gui/src/RadioCallstackPage/CCallStackPage.cpp $
 * @author:  Glen Kidd
 * @version: $Revision: #2 $
 *
...
CCallStackPage::~CCallStackPage()
{
	...
	The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_RESOURCE_REMOVE);
	The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_PRIVATE_CALL_REJECTED);
	// TD18166
	//The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_RESOURCE_EMERGENCY);
	//The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_RESOURCE_EMERGENCY_ACKNOWLEDGED);
	//The_RadioOperations::instance()->m_observerHelper.removeChildObserver(this,WM_RESOURCE_EMERGENCY_ENDED);
...
BEGIN_MESSAGE_MAP(CCallStackPage, CPropertyPage)
	...
	ON_MESSAGE(WM_PRIVATE_CALL_REJECTED,onNotifiedPrivateCallRejected)
	...
	ON_MESSAGE(WM_RESOURCE_REMOVE, onNotifiedResourceRemove)
...
BOOL CCallStackPage::OnInitDialog() 
{
 ...
 //       SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
	...
	The_RadioOperations::instance()->m_observerHelper.addChildObserver(this,WM_RESOURCE_REMOVE);
	The_RadioOperations::instance()->m_observerHelper.addChildObserver(this,WM_PRIVATE_CALL_REJECTED);
	...
	if ( pos != NULL  )
	{
		...
		if (m_RadioResource.end() != iter) {
			if ((*iter).type == RR_RADIO) {
				...
				r.type = RR_TRAIN;
				r.alias = (*iter).alias;
				The_RadioOperations::instance()->entitySelectionChange(r, false);
			}
		}
	}
...
afx_msg LRESULT CCallStackPage::onNotifiedPrivateCallRejected(WPARAM wParam, LPARAM lParam)
{
	FUNCTION_ENTRY("onNotifiedPrivateCallRejected");

	if ( wParam != 0)
	{
		RadioResource* resource = (RadioResource*)wParam;
		std::string msg="Call has been rejected by resource " +  resource->alias;
		::MessageBox(this->m_hWnd,msg.c_str(), "Warning Message", MB_OK|MB_ICONINFORMATION);
	}
...
void CCallStackPage::addCall(RadioResource& resource)
{
	...
	m_RadioCallStackListCtrl.SetItemData(nIndex,resource.ssi);
...
LRESULT CCallStackPage::onNotifiedDbUpdate(WPARAM pOldResource, LPARAM pNewResource)
{
	...
	LV_FINDINFO findinfo;
	findinfo.flags = LVFI_PARAM;
	findinfo.lParam = oldRes->ssi;

	int nIndex = m_RadioCallStackListCtrl.FindItem(&findinfo);

	if ( nIndex != -1)
	{
		m_RadioCallStackListCtrl.SetItemText(nIndex,0, newRes->alias.c_str());
...
LRESULT CCallStackPage::onNotifiedDbDelete(WPARAM pOldResource, LPARAM lParam)
{
...
LRESULT CCallStackPage::onNotifiedResourceRemove(WPARAM pResource, LPARAM lParam)
{
	FUNCTION_ENTRY("onNotifiedDbDelete");
	RadioResource* res = (RadioResource*) pResource;
	removeCall(*res);
	FUNCTION_EXIT;
	return 0;
}
		
void CCallStackPage::OnPaint() 
{
  	CPaintDC dc(this);

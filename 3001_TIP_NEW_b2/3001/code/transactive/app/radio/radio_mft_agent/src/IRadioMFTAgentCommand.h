...
/**
		...
		virtual void connectPatchGroup(RadioResource& resource,CommandState state=START,unsigned int grpNo=0) =0;
		virtual void assignPatchMember(RadioResource& resource) =0;
		virtual void releasePatchGroup(RadioResource& resource) =0;
		virtual void deassignPatchMember(RadioResource& resource) =0;
		virtual void deletePatchMember(RadioResource& resource) = 0;
		virtual void addNextPatchMember(RadioResource& resource, ResourceIdentifier iscsGrpId, unsigned int mccGrpId) = 0;

		virtual void connectMselGroup(RadioResource& resource,CommandState state=START,unsigned int grpNo=0) =0;
		virtual void assignMselMember(RadioResource& resource) =0;
		virtual void releaseMselGroup(RadioResource& resource,bool bSolicited) =0;
		virtual void deassignMselMember(RadioResource& resource) =0;
		virtual void addNextMselMember(RadioResource& resource, ResourceIdentifier iscsGrpId, unsigned int mccGrpId) = 0;
		...
		virtual void makePrivateCall(RadioResource& resource,CommandState state=START) =0;
		virtual void answerPrivateCall(RadioResource& resource,CommandState state=START) =0;
		...
		virtual void restartRemoteApiConnection() = 0;

		virtual void beginRefresh() = 0;

		virtual void updateDbGrant(bool grant = true) = 0;

		virtual void broadcastTrain(RadioResource& resource, CommandState state=START) = 0;

/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP/3001/transactive/app/trains/TrainBorne_PA/src/TrainDvaStatusDlg.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
...
void TrainDvaStatusDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_DVA_MSG_PRIORITY, m_dvaMsgPrioriry);
    //}}AFX_DATA_MAP
...
BOOL TrainDvaStatusDlg::OnInitDialog()
{
	...
	HICON icon = SetIcon(m_hIcon, TRUE);
...
void TrainDvaStatusDlg::fillOutBroadcastConfiguration()
{
    ...
    try
    {
		...
		m_dvaMsgPrioriry = trainDvaConfig.isEmergency ? "Emergency" : "Normal";

        const TA_Base_Bus::IPAAgentCorbaDef::TrainCyclicParams& period = trainDvaConfig.stCyclicParams;

        if (0 == period.durationInSecs && 0 == period.periodInSecs)
        {
        ...
        else
        {
            ...
            int periodTime = period.durationInSecs;
            int durationTime = period.periodInSecs;
...
void TrainDvaStatusDlg::OnClose()
{
...
void TrainDvaStatusDlg::removeAnnouncement()
{

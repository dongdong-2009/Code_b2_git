...
//        if ( true == OperationModeManager::isRunningControl() )		hongzhi
    ...
    void TrainAgentStatusManager::setUpInitialStates()
    {
		...
		//Lucky : Initialization of the Trains should be based on the TR_TRAIN Table in the Database.
			...
			persistedTrainStates.end() != stateIter; ++stateIter )
		{
			try
			{
				...
				TrainCommsStatePtr trainCommsState( new TrainCommsState( stateIter->first ) );
				TrainAlarmStatePtr trainAlarmState( new TrainAlarmState( stateIter->first ) );
				TrainCctvStatePtr trainCctvState( new TrainCctvState( stateIter->first ) );
				TrainPaStatePtr trainPaState( new TrainPaState( stateIter->first ) );
				TrainCallStatePtr trainCallState( new TrainCallState( stateIter->first ) );
				TrainPecStatePtr trainPecState( new TrainPecState( stateIter->first ) );
				TrainTisStatePtr trainTisState( new TrainTisState( stateIter->first ) );
    ...
    void TrainAgentStatusManager::loadPersistedStates()
    {
              ...
              persistedTrainStates.end() != stateIter; ++stateIter )
        {
            try
            {
                ...
                trainCommsState->initTrainTsi( TA_IRS_Bus::ProtocolCommonTypes::MpuBackup,
                                              stateIter->second->getSecondaryTsi(),
                                              timestamp,
                                              false );

                trainCommsState->initTrainTsi( TA_IRS_Bus::ProtocolCommonTypes::MpuMain,

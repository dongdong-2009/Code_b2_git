...
namespace TA_IRS_App
{
                                                  ...
                                                  TA_IRS_Bus::CommonTypes::TrainIdType trainId )
    {
		...
		//Commented : Event Value must be empty or blank based on DTL-1283
        // if isAtsValid
        /*if ( atsValid )
        {
		...
		//Event Value must be empty or blank based on DTL-1283
        descriptionParameters.insert( TA_Base_Core::NameValuePairs::value_type( "Location", " " ) );

        FUNCTION_EXIT;
    }

	void AuditSender::auditPecHold( const std::string& sessionId, TA_IRS_Bus::CommonTypes::TrainIdType trainId, time_t timeStamp, TA_IRS_Bus::ProtocolPecTypes::EPecCarNumber carNumber, TA_IRS_Bus::ProtocolPecTypes::EPecNumber pecNumber )
	{
		FUNCTION_ENTRY( "auditPecHold" );
		...
		std::stringstream _strFormat;
		_strFormat << static_cast< unsigned int >( pecNumber )
			       << " CAR "
				   << static_cast< unsigned int >( carNumber );

		descriptionParameters.insert( TA_Base_Core::NameValuePairs::value_type( "PECID", _strFormat.str()) );
		descriptionParameters.insert( TA_Base_Core::NameValuePairs::value_type( "TrainID", trainIdString.str()) );

		//auditEvent( TA_Base_Core::TrainAudit::TrainPECHoldCall, sessionId, descriptionParameters, trainId, timeStamp );
		...
		// audit the message
		m_taAuditing.submitAdvancedAuditMessage( TA_Base_Core::TrainAudit::TrainPECHoldCall,
			m_configuration.getEntityKey(),
			descriptionParameters,
			"",
			sessionId,

...
namespace TA_IRS_App
{
			...
			m_bProcessDutyHandover(false)
    {
		...
		TrainLibDownloadSequenceProc * trainLibDownload = new TrainLibDownloadSequenceProc ();;
		
		m_endDownloadCallBack = (IEndDownloadCallBack *)trainLibDownload;
		m_libDownloadSeq = (ITrainLibDownloadSequence *)trainLibDownload;
        ...
        if ( false == m_rightsChecker.canDisplayTtisFreeText( sessionId ) )
        {
           // TA_THROW( TA_IRS_Bus::TrainAccessDeniedException( "Can not display TTIS Free Text message" ) );
			 LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     "DVA announcement for trains No Rightb for displayFreeTextMessage: %s",
					 sessionId.c_str() );
        }
                                       ...
                                       const std::string& sessionId )
    {
        ...
        if ( false == m_rightsChecker.canResetTtisMessage( sessionId ) )
        {
          //  TA_THROW( TA_IRS_Bus::TrainAccessDeniedException( "Can not reset TTIS message" ) );
			 LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     "DVA announcement for trains No Rightb for canResetTtisMessage: %s",
					 sessionId.c_str ()  );
        }
                                          ...
                                          const std::string& sessionId )
    {
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
			"SessionID: %s"
			"Train size: %d",sessionId.c_str(), trains.size() );
   ...
   //         TA_THROW( TA_IRS_Bus::TrainAccessDeniedException( "Can not download TTIS library" ) );
			 LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     "DVA announcement for trains No Rightb for canDownloadTtisLibrary: %s",
					 sessionId.c_str ()  );
        }
		
		unsigned char origin = m_configuration.getOperatorOrigin( sessionId );

		TrainLibStackInfo * libStackInfo = new TrainLibStackInfo ();
		
 		libStackInfo->libaryData.insert (libStackInfo->libaryData.begin(),
 										  libraryData.begin(),
 										  libraryData.end ());
		
	
		libStackInfo->callback = this;
		libStackInfo->lTrainversion = libraryVersion;
		libStackInfo->strSessionId = sessionId;
		libStackInfo->ucOrigin = origin; 

		m_libDownloadSeq->processSequenceDownload(trains,libStackInfo);
    

        FUNCTION_EXIT;
    }

	void TimsTisManager::cancelLibraryDownload( const TA_IRS_Bus::CommonTypes::TrainIdList& trains, 
												const std::string& sessionId )
	{
		FUNCTION_ENTRY( "cancelLibraryDownload" );
		...
		for ( TA_IRS_Bus::CommonTypes::TrainIdList::const_iterator trainIter = trains.begin();
			trains.end() != trainIter; ++trainIter )
		{
			// create a new CancelLibraryDownloadTask
			// store it in an IWorkItemPtr and then queue it using m_threadPool
			TA_Base_Core::IWorkItemPtr newTask(
				new CancelLibraryDownloadTask( *this,
				*trainIter,
				sessionId ) );
                                           ...
                                           const std::string& sessionId )
    {
      ...
      //      TA_THROW( TA_IRS_Bus::TrainAccessDeniedException( "Can not download TTIS schedule" ) );
              LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     "DVA announcement for trains No Rightb for canDownloadTtisSchedule: %s",
					 sessionId.c_str ()  );
        }
    ...
    void TimsTisManager::handleTrainTimeout( TA_IRS_Bus::CommonTypes::TrainIdType trainId )
    {
        ...
        unsigned long currentDownloadVersion = 0;
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                         "TIS 2 Timeout [trainId: %d][currentDownloadType: %s][currentDownloadVersion: %u][transferTsi: %s][sessionId: %s]",
			...
			m_endDownloadCallBack->processEndDownload(trainId);
        }
                                                  ...
                                                  const std::string& details )
    {
        ...
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                     ...
                     "Transfer complete for [trainId: %d][currentDownloadType: %s][currentDownloadVersion: %u]"
        ...
        if ( TA_IRS_Bus::TtisTypes::TtisNone != currentDownloadType )
        {
            ...
            else
            {
				...
				if (m_bProcessDutyHandover)
				{
					bool bCanDownload = m_rightsChecker.canDownloadTtisLibrary(trainId,sessionId);

					// if true, cancellation is due to user mft cancellation
					// if false, cancellation is dut to the for cancellation since this location 
					// doesnt have any rights or duty on this location
					if (bCanDownload)
					{
						update.commandStatus  = TA_IRS_Bus::TtisTypes::TtisFailed;
					}
					...
					{
						update.commandStatus  = TA_IRS_Bus::TtisTypes::TtisDutyHandover;
					}
					
					m_bProcessDutyHandover = false;

					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
								"Train %d has no control on this location", trainId);
				}
				...
				LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
					"Success is false, informing callback to process next train on queue");
                                          ...
                                          const std::string& sessionId )
    {
		...
		{	
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"No duty to perform related action : Train : %d",trainId);
			TA_IRS_Bus::TtisTypes::TisLibraryUpdate update;
			update.trainId = trainId;
			update.version = libraryVersion;
			update.updateType = TA_IRS_Bus::TtisTypes::LibraryDownloadComplete;
			update.commandStatus = TA_IRS_Bus::TtisTypes::TtisDutyHandover;
			update.downloadResult = TA_IRS_Bus::TtisTypes::TransferFailed;
	...
	void TimsTisManager::cancelLibraryDownload( TA_IRS_Bus::CommonTypes::TrainIdType trainId, 
												const std::string& sessionId )
	{
		FUNCTION_ENTRY( "cancelLibraryDownload" );

		{
			...
			if ( TA_IRS_Bus::TtisTypes::TtisNone != currentDownloadType )
			{
				LOG_GENERIC (SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Cancelling Library Download");
				m_radioAgent.cancelDataCall(trainId,sessionId);
			}
                                          ...
                                          const std::string& operationDescription )
    {
        ...
        catch ( TA_IRS_Bus::TrainAccessDeniedException& e )
        {
            ...
            // set the error status
            libraryUpdate.commandStatus = TA_IRS_Bus::TtisTypes::TtisDutyHandover;
            success = false;
        }
        catch ( TA_IRS_Bus::TrainTransmissionFailureException& e )
        {
        ...
        catch ( TA_IRS_Bus::TrainTimeoutException& e )
        {
			...
			libraryUpdate.downloadResult = TA_IRS_Bus::TtisTypes::TimedOut;
            success = false;
        }
        catch ( TA_IRS_Bus::TransactionCancelledException& e )
        {
        ...
        if ( true == success )
        {
            try
            {
                ...
                LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
							 ...
							 "transferTsi : %s",
                             data.size(), trainId, transferTsi.c_str() );
		...
		{
			// failure result, informing queue processor that 
			// download already halted this time, so allowing WorkItem to
			// process the next train on queue
                                         ...
                                         const std::string& operationDescription )
    {
        ...
        try
        {
            ...
            if ( commandReceived )
            {
                // get the result of the command
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
					"Received Response : %s",commandReceived->getLogString().c_str());
            ...
            else if ( mpuChangeover )
            {
                ...
                libraryUpdate.commandStatus = TA_IRS_Bus::TtisTypes::MpuChangeoverError;
				libraryUpdate.downloadResult = TA_IRS_Bus::TtisTypes::MpuChangeover;
            }
            else
            {
                // if no valid events were returned,
...
//
    ...
    void TimsTisManager::processEndOfTtisDownloadEvent( TA_IRS_Bus::EndOfTtisDownloadEventPtr event )
    {
        ...
        unsigned long currentDownloadVersion;
        std::string sessionId;
...

        {
            ...
            switch ( downloadResult )
            {
				...
				case TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadCancelled:       //Lucky
				case TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadCancelledByOCC:  //Lucky
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                         "TTIS download complete [TrainId: %d][Type: %s][Version: %u][Status: %d]",
                         trainId,
            ...
            if ( TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadRepetition != downloadResult )
            {
				...
				else if ( (TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadCancelled      == downloadResult) ||
					      (TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadCancelledByOCC == downloadResult) )
				{
					update.commandStatus  = TA_IRS_Bus::TtisTypes::TtisFailed;
					update.downloadResult = TA_IRS_Bus::TtisTypes::Cancelled;
				}

                // otherwise if the result was failed
				else if (TA_IRS_Bus::EndOfTtisDownloadEvent::TtisDownloadFailed)
				{
					update.commandStatus = TA_IRS_Bus::TtisTypes::TtisEndDwnloadFailed;
		...
		//Notify RadioTetraAgent for the Status, 0 = Success, 1 = Failed, 3 = Cancelled, 4 = Cancelled by OCC
		m_radioAgent.notifyEndOfTtisDownload(event->getTrainId(),downloadResult);

		// call callback to process next train on queue
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
					"End of Ttis Download Recieved, informing processor to process next queue");
		m_endDownloadCallBack->processEndDownload(trainId);
    ...
    void TimsTisManager::processChangeoverStatusEvent( TA_IRS_Bus::ChangeoverStatusEventPtr event )
    {
		...
		LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"ChangeOver Event Recieve TTIS");
...

        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,
						"processChangeoverStatusEvent - [DownloadType : %d]",currentDownloadType);
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
                         "TTIS download complete due to MPU changeover [TrainId: %d][Type: %s][Version: %lu]",
                         trainId,
                         TA_IRS_Bus::TtisTypes::ttisDownloadTypeToString( currentDownloadType ).c_str(),
                         currentDownloadVersion );
		...
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"No Current Download");
		}
	...
	void TimsTisManager::processReleaseOfCtrl()
	{
		...
		TA_IRS_Bus::TrainIdToStateMap::iterator stateIter = tisStates.begin();
		
			for (;stateIter != tisStates.end(); stateIter++)
			{
				TrainTisStatePtr tisStatePtr = boost::dynamic_pointer_cast< TrainTisState >( stateIter->second);
				unsigned long lTrainId = stateIter->first;
				std::string strSessionId = tisStatePtr->getSessionId();
				
				
				bool bCanDownload = m_rightsChecker.canDownloadTtisLibrary(lTrainId,strSessionId);

				if (!bCanDownload)
				{
					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"Cancelling Data Call for Train :%d, Duty Lost", lTrainId);
					m_bProcessDutyHandover = true;
					this->cancelLibraryDownload(lTrainId,strSessionId);
				}
			}
	}
}

...
namespace TA_IRS_App
{
    ...
    void TimsStatusManager::processOa1Timeout( TA_IRS_Bus::CommonTypes::TrainIdType trainId )
    {
        ...
        if ( true == m_operationModeManager.isInControlOfTrain( trainId, true, false ) )
        {
            ...
            if ( true == trainValid )
            {
                ...
                {
                    alarmState->setOa1AlarmId( m_alarms.raiseOa1AtcTimerAlarm( trainId, "CriticalAlarm", timestamp ) );
                }
				//send change mode to train borne DTL-2012				//
				// TBS right drop to station
				// Train stop at station and sleep
				// ISCS can receive ATS information, but it cannot receive train HB message from train borne. 
				// Train turn on radio and send wake up message 72 to default location OCC or depot
				// since train location is at station and OCC no right for this location, iscs can not answer the wake up command from train
				...
				if ( TA_Base_Core::ILocation::STATION == m_configuration.getAgentLocationType() )
				{
					...
					LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
						"DTL-2012,send change mode for train ID=%d",
						static_cast<int>(trainId));
				}
    ...
    void TimsStatusManager::processEvent( TA_IRS_Bus::TrainEventPtr event )
    {
...

        {
            ...
            commsState->setActiveTrainTsi( timsEvent->getTsi(),
                                     timsEvent->getTime(),
                                     setTsiAsPrimary );
        ...
        {
			//The NonDutyTrainTSI broadcast message has been removed. 
			//There is no need to inform other TrainAgents for the NonDutyTSI state updates. 
			//This will just caused the trainAgent to send the Change area command to the wrong
			//position of the Train.
			//Train at the station should only process and handle its Change Area/Change mode
			//command that has the control of its location and likewise if the Train is at Depot.
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"TrainID: %d has no controlled for this location.",
				       static_cast<int>(trainId));
        }
                                                  ...
                                                  TrainCommsStatePtr commsState )
    {
...

        {
            ...
            // remove the messages for library info (pa, ttis, ttis version)
		...
		//Set default status12 = 0 which is used for Mainline
		unsigned char status12 = 0;

        if ( TA_Base_Core::ILocation::DPT == m_configuration.getAgentLocationType() )
        {
			//If location type is DEPOT, read the Database parameter for Train Status12 information.
			//Values : 1=TSFB,2=GDCC
            status12 = m_configuration.getTrainStatus12();
        }
                                             ...
                                             TrainCommsStatePtr commsState )
    {
		...
		// unit test
		// Dennis Abriol
		// comment out the function call m_atsAgent.sendAtcTrainStatusReportToAts ()
		// The call is executed with try catch block already doing the said call.
		// This should be also at the try catch block.
                                                          ...
                                                          TrainCommsStatePtr commsState )
    {
...

        {
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugInfo,"processChangeoverStatusEvent, set Oa1Validity to true .");

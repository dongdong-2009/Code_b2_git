...
namespace TA_IRS_App
{
																...
																time_t timestamp, TA_IRS_Bus::PecTypes::EPecTrainSource pecTrainSource )
    {
		...
		newPoint.pecTrainSource = pecTrainSource;
																				 ...
																				 TA_IRS_Bus::PecTypes::EPecTrainSource pecTrainSource )
    {
        ...
        try
        {
			...
			answeredPec.pecTrainSource = pecTrainSource;
																	  ...
																	  TA_IRS_Bus::PecTypes::EPecTrainSource pecTrainSource )
    {
        ...
        givenPoint.car = car;
        givenPoint.pec = pec;
		givenPoint.pecTrainSource = pecTrainSource;
	...
	TA_IRS_Bus::PecTypes::ActivePecPointList TrainPecState::pecHold( TA_IRS_Bus::ProtocolPecTypes::EPecCarNumber car,
																	 TA_IRS_Bus::ProtocolPecTypes::EPecNumber pec,
																	 time_t timestamp,
																	 TA_IRS_Bus::PecTypes::EPecTrainSource pecTrainSource )
	{
		FUNCTION_ENTRY( "pecHold" );
		...
		TA_IRS_Bus::PecTypes::ActivePecPointList holdPec;

		if ( m_activePecPoints.end() != findIter )
		{
			// take a copy of the PEC
			holdPec.insert( *findIter );
			...
			if ( TA_IRS_Bus::PecTypes::PecSourceOperator == holdPec.begin()->source )
			{
				clearCall();
				resetSessionId();
				resetAutoContinueCount();
    ...
    TA_IRS_Bus::PecTypes::ActivePecPointList TrainPecState::invalidateAllPec( time_t timestamp )
    {
              ...
              m_activePecPoints.end() != pointIter; ++pointIter )
        {
			...
			//DTL-2093: Set PEC to invalid status, otherwise
			//it will send message to call banner to change Pec Status  
			//from "Invalid" to "In-Progress", but the operator has no
			//right for Pec Call actually

/**
  * The source code in this file is the property of 
  * Ripple Systems and is not for redistribution
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/pa_zone/src/PAZoneDatabaseAccessor.h $
  * @author:  Karen Graham
  * @version: $Revision: #7 $
  *
  * Last modification: $DateTime: 2014/06/06 10:11:45 $
  * Last modified by:  $Author: huangjian $
  * 
  * This class uses the Data Access Interface to retrieve PA Zone information and returns it to
  * the caller in a variety of formats. It abstracts some of the database access from the views.
  * 
  */


#if !defined(PaZoneDatabaseAccessor_40D09158_85E4_464f_81D1_E7B9225238D9__INCLUDED_)
#define PaZoneDatabaseAccessor_40D09158_85E4_464f_81D1_E7B9225238D9__INCLUDED_

#include <map>
#include <vector>
#include <string>

#include "app/configuration/config_plugins/config_plugin_helper/src/IDatabaseAccessor.h"

#include "core/data_access_interface/pa_4669/src/IPaTisZoneMap.h"
#include "core/data_access_interface/entity_access/src/IEntityData.h"
#include "core/data_access_interface/src/ILocation.h"
#include "core/data_access_interface/src/LocationAccessFactory.h"

#include "app/configuration/config_plugins/config_plugin_helper/src/ItemKeyToNameMap.h"
#include "core/data_access_interface/src/PrimitiveWrapper.h"

// Forward declarations
namespace TA_Base_Core
{
    class IConfigItem;
}

namespace TA_Base_Core
{
    class IConfigPaZone;
}

namespace TA_IRS_App
{
    class PaZoneDatabaseAccessor : public TA_Base_App::IDatabaseAccessor
    {

    public:

        typedef std::map<unsigned long, std::string>    LocationKeyToNameMap;
        typedef LocationKeyToNameMap::iterator          LocationKeyToNameMapIt;

        typedef std::map<unsigned long, std::string>    EntityKeyToNameMap;
        typedef EntityKeyToNameMap::iterator            EntityKeyToNameMapIt;

        struct PaZoneRecord
        {
            CString status;             // This will be the current applied status of the item (ie new, modified etc)
                                        // It will be set elsewhere so we don't worry about it
            CString key;                
            CString location;          
            CString id;          
            CString label;         
            
            //CString equipmentEntityName;        
            CString stnEquipmentEntityName;
            CString occEquipmentEntityName;
        };
        
        struct PaTisZoneMapsRecord
        {
            bool hasChanged;

            TA_Base_Core::IPaTisZoneMaps paZisZoneMaps;
        };

        /**
         * Constructor
         */
        PaZoneDatabaseAccessor();


        /**
         * Destructor
         */
        virtual ~PaZoneDatabaseAccessor();


        /**
         * loadItems
         *
         * This will retrieve all the PA Zone records from the database and store them in m_PaZones.
         *
         * @exception DataException - This will be thrown if there is an error with the PA Zone
         *                            data and it cannot be retrieved for some reason.
         * @exception DatabaseException - This will be thrown if there is an error communicating with the
         *                                database.
         */
        void loadItems();

        
        /**
         * getItemNames
         *
         * This method cannot be used by PA Zones and will just assert
         */
        std::map<CString,unsigned long> getItemNames(CProgressCtrl& progress);
        

        /**
         * getAllItemDetails
         *
         * This retrieves the items' details in a display format.
         * Precondition: loadItems() must be called before this method
         * 
         * @param CProgressCtrl - This is used to show how long the loading is taking
         *
         * @return map<unsigned long, PaZoneRecord> - This is all the PA Zones keyed
         *                                                    on their unique identifier
         */
        std::map<unsigned long,PaZoneRecord> getAllItemDetails(CProgressCtrl& progress);
        
        
        /**
         * getItemDetails
         *
         * This retrieves the item's details in a display format. The items is specified by passing
         * in its unique identifier.
         * Precondition: loadItems() must be called before this method
         * 
         * @param unsigned long - The identifier of the item to retrieve details for
         *
         * @return PaZoneRecord - This is all the PA Zone details
         */
        PaZoneRecord getItemDetails(unsigned long identifier);
 
        
        /**
         * getItem
         *
         * This returns a database object for the item specified. The item is specified by
         * passing in its unique identifier..
         * Precondition: loadItems() must be called before this method
         *
         * @param unsigned long - The identifier of the item to retrieve
         *
         * @return IConfigItem* - The database object for this item. This will be NULL if one
         *                            is not found.
         */
        virtual TA_Base_Core::IConfigItem* getItem(unsigned long identifier);


        /**
         * deleteItem
         *
         * This will delete the specified item from the database and it will then delete the
         * IConfigItem object held.
         *
         * @param unsigned long - This is the unique identifier for the item to delete
         *
         * @exception DatabaseException A DatabaseException will be thrown if there is an error
         *                              deleting the item
         */
        virtual void deleteItem(unsigned long id);


        /**
         * newItem
         *
         * This creates a new item. This item will be added to the internal list
         * of current items but will not yet be applied to the database.
         *
         * @return IConfigItem* - The newly created item ready for its data to be populated
         */
        virtual TA_Base_Core::IConfigItem* newItem();


        /**
         * copyItem
         *
         * This creates a new item with the same data in it as the item passed in.
         * This item will be added to the internal list of current items but will not yet be
         * applied to the database.
         *
         * @param unsigned long - The unique identifier of the item to copy
         * 
         * @return IConfigItem* - The newly created item ready for its data to be populated
         *
         * @exception DatabaseException - This is thrown if data could not be retrieved for the item
         *                                that is being copied.
         */
        virtual TA_Base_Core::IConfigItem* copyItem(const unsigned long idOfItemToCopy);

        
        /**
         * areCurrentChangesPending
         *
         * This is called to determine if there are any current changes that have not been applied.
         * If this returns true then it will populate the vector of strings passed in with all items
         * not applied.
         * 
         * @param vector<string>& - This is an OUT parameter and should be passed in empty. It will
         *                          be populated with names for all data not yet applied
         * 
         * @return bool - This will return true if there are items that have had changes which have  not
         *                been applied.
         */
         bool areCurrentChangesPending(std::vector<std::string>& itemsNotApplied);

        
         /**
          * invalidate
          *
          * This will set m_isLoaded to false indicating that next time load is called all data should
          * be refreshed from the database.
          */
         void invalidateData()
         {
             m_isLoaded = false;
         }


        /**
         * getAllLocations
         *
         * This returns all location name and keys to the caller.  Ensure that loadItems() is called first
         * otherwise no data will be returned.
         *
         * @return map<unsigned long, string> - This returns all the location keys and names. This is all
         *                                      information needed to update data and to display to the user
         */
        LocationKeyToNameMap getAllLocations()
        {
            return m_locationNameCache.getInternalMap();
        }


        /**
         * getAllEntities
         *
         * This returns all *datanode entity* name and keys to the caller.  Ensure that loadItems() is called first
         * otherwise no data will be returned.
         *
         * @return map<unsigned long, string> - This returns all the location keys and names. This is all
         *                                      information needed to update data and to display to the user
         */
        EntityKeyToNameMap getAllEntities()
        {
            return m_paEntityNameCache.getInternalMap();
        }

        /**
         * getAllEntitiesAtLocation
         *
         * Same as getAllEntities, except filters out all those not at the specified input location
         * @param locationKey the location to filter by
         * @param fetchPaEntitiesOnly set to true to only return PA zone entities (have a special format)
         */
        EntityKeyToNameMap getAllEntitiesAtLocation(unsigned long locationKey, bool fetchPaEntitiesOnly = false);

        std::string getLocationName(unsigned long locationKey);
        std::string getPaEntityName(unsigned long entityKey);
        std::string getTisEntityName(unsigned long entityKey);
        unsigned long getPaEntityKey(const std::string& entityName);
        unsigned long getTisEntityKey(const std::string& entityName);
        unsigned long getLocationKey(const std::string& locationName);

        /// @return the location key associated with the input entity, -1 if input is invalid
        unsigned long getEntityLocation(unsigned long entityKey);
        
        /**
         * isFormattedAsPaZoneEntity
         *
         * @return true if the input string format corresponds to that of a PA Zone entity
         * @param entityName
         * 
         */
        bool isFormattedAsPaZoneEntity(const std::string entityName);
        
        /**
         * getOccLocationKey
         *
         * @return the location key of the OCC station
         */
        unsigned long getOccLocationKey() const;

		/**
         * isOccLocationKey
         *
         * @return true if the input 'locationKey' defined the OCC location
         *
         */
        bool isOccLocationKey(unsigned long locationKey);

        PaTisZoneMapsRecord* getPaTisZoneMapsRecordByPaZoneKey(unsigned long zoneKey);

    protected:

        void refreshKeyToNameMaps();
   
    private:
        /**
         * Copy Constructor
         *
         * Hidden as a private method so it is not used.
         */
        PaZoneDatabaseAccessor( const PaZoneDatabaseAccessor& thePaZoneDatabaseAccessor){ };


        /**
         * Operator =
         *
         * Hidden as a private method so it is not used.
         */
        PaZoneDatabaseAccessor& operator=(const PaZoneDatabaseAccessor& pm);

        /**
         * getRecordFrom
         *
         * Converts data from IConfigPaZone into PaZoneRecord structure
         */
        PaZoneRecord getRecordFrom(TA_Base_Core::IConfigPaZone& item);

        typedef std::map<unsigned long, TA_Base_Core::IConfigPaZone*>    ConfigItems;
        typedef ConfigItems::iterator                                       ConfigItemsIt;


        ConfigItems             m_configItems;      // This is all the PA Zones loaded from the database
        
        typedef std::map<unsigned long, PaTisZoneMapsRecord> ConfigPaTisZoneMapsRecordMaps;
        typedef ConfigPaTisZoneMapsRecordMaps::iterator ConfigPaTisZoneMapsRecordMapsIt;

        ConfigPaTisZoneMapsRecordMaps m_configPaTisZoneMapsRecordMaps;

        bool m_isLoaded;                // This indicates if we have loaded the PA Zones before or not.
                                        // If we have never loaded the PA Zones then when we are told
                                        // to refresh we shouldn't load them.

        ItemKeyToNameMap<TA_Base_Core::ILocation> m_locationNameCache;
        ItemKeyToNameMap<TA_Base_Core::IEntityData> m_paEntityNameCache;
        ItemKeyToNameMap<TA_Base_Core::IEntityData> m_tisEntityNameCache;
        std::map<unsigned long, unsigned long> m_entityKeyToLocationMap;
		std::map<unsigned long, TA_Base_Core::ILocation::ELocationType> m_locationTypeMap;

        mutable TA_Base_Core::PrimitiveWrapper<unsigned long> m_occLocationKey;
    };
}

#endif // !defined(PaZoneDatabaseAccessor_40D09158_85E4_464f_81D1_E7B9225238D9__INCLUDED_)

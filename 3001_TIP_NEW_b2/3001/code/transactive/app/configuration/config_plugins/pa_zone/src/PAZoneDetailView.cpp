/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/pa_zone/src/PAZoneDetailView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #13 $
  *
...
namespace TA_IRS_App
{
    ...
    PaZoneDetailView::PaZoneDetailView() : 
        ...
        m_configPaTisZoneMaps(NULL),
        m_paTisZoneMapsRecord(NULL),
...
END_MESSAGE_MAP()
    ...
    void PaZoneDetailView::populateItemDetails(TA_Base_Core::IConfigItem* currentItem)
    {
        ...
        TA_Base_Core::IConfigPaZone* tmpPaZone = m_configItem;
        m_configItem = dynamic_cast<TA_Base_Core::IConfigPaZone*>(currentItem);

        if (m_configItem == NULL)
        { 
            m_configItem = tmpPaZone;
            showItemDetails(false);
            FUNCTION_EXIT;
            return;
        }
        m_paTisZoneMapsRecord = m_databaseAccessor->getPaTisZoneMapsRecordByPaZoneKey(m_configItem->getKey());
        
        m_configPaTisZoneMaps = &(m_paTisZoneMapsRecord->paZisZoneMaps);
    ...
    {        
        ...
        std::list<std::string> entities = getAssociatedEquipmentEntities();
        
        for (std::list<std::string>::iterator itr = entities.begin(); itr != entities.end(); itr ++)
        {
            unsigned long entityKey = m_databaseAccessor->getTisEntityKey(itr->c_str());
            bool validAssociation = false;

            // For each item in list, confirm it is a valid entity
            if (entityKey >= 0)
            {
                if (( m_databaseAccessor->getOccLocationKey() == m_configItem->getLocationKey() ) || 
                    ( m_configItem->getLocationKey() == m_databaseAccessor->getEntityLocation(entityKey)))
                {
    ...
    void PaZoneDetailView::updateAssociations(EAssocUpdate update)
    {
        ...
        case EDIT_EXISTING: 
            {
                ...
                {                       
                    ...
                    setAssociatedEquipmentEntity(dlg.getSelectedName().GetBuffer(0), true);
                }
            }                    
            break;
        case REMOVE_EXISTING:
            if (selectedItem.GetLength() > 0)
            {
                ...
                setAssociatedEquipmentEntity(selectedItem.GetBuffer(0), false);
            }
            break;
        };
        ...
        if (EDIT_EXISTING == update)
        {
            ...
            if (entities != getAssociatedEquipmentEntities())
            {
        ...
        if (entities != getAssociatedEquipmentEntities())
        {
            ...
            updateListViewItemStatusForPaTisZoneMaps();
            refreshButtonAvailability();
    ...
    void PaZoneDetailView::stripExistingAssociationsFromMap(TA_Base_App::ChangeDialog::ItemsForSelection& items, CString forceInclusion)
    {
        ...
        for (int i = 0; i < m_associationListCtrl.GetItemCount(); i ++)
        {
        ...
        for (size_t i = 0; i < (int) keysToErase.size(); i ++)
        {
    ...
    void PaZoneDetailView::refreshButtonAvailability()
    {
        ...
        // We have an old item
        if (m_configItem->hasChanged() || m_paTisZoneMapsRecord->hasChanged)
        {
            GetDlgItem(IDC_BUTTON_RESET)->EnableWindow(true);
            GetDlgItem(IDC_BUTTON_APPLY)->EnableWindow(true);
        }
    ...
    void PaZoneDetailView::onKillfocusEditStnEquipmentEntityName() 
    {
        ...
        unsigned long entityKey = m_databaseAccessor->getPaEntityKey((LPCTSTR)entityString);
    ...
    void PaZoneDetailView::onKillfocusEditOccEquipmentEntityName() 
    {
        ...
        if (entityKey != -1)
        {
			...
			if (!m_databaseAccessor->isOccLocationKey(m_databaseAccessor->getEntityLocation(entityKey)))
            {
                // Entity taken from a different location, this is invalid
                m_occEquipmentEntityNameEdt.setTextType( TA_Base_Bus::TextColourEdit::INVALID );
                return;
    ...
    {   
        ...
        std::string strNewEntityName = dlg.getSelectedName().operator LPCTSTR();

        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "User selected a new location which is %s", strNewEntityName.c_str());

        try
        {
            TA_ASSERT(m_configItem !=  NULL, "m_configItem is NULL.");

            if (currentEntity != strNewEntityName.c_str())
            {
                // Now update the entity type the details view is showing
                m_stnEquipmentEntityNameEdt.SetWindowText( strNewEntityName.c_str() );

                unsigned long entityKey = m_databaseAccessor->getPaEntityKey(strNewEntityName);
                // If a valid entity is returned, confirm it is for the correct location
                if (entityKey >= 0)
                {
                    if ((m_configItem->getLocationKey() != m_databaseAccessor->getEntityLocation(entityKey)) || 
                        !m_databaseAccessor->isFormattedAsPaZoneEntity( strNewEntityName ))
                    {
                        // Entity taken from a different location, this is invalid
                        m_stnEquipmentEntityNameEdt.setTextType( TA_Base_Bus::TextColourEdit::INVALID );
                        return;
                ...
                m_configItem->setStnEntityName( strNewEntityName );
    ...
    {   
        ...
        try
        {
            ...
            {
                // Now update the entity type the details view is showing
                m_occEquipmentEntityNameEdt.SetWindowText( strNewEntityName.c_str() );
                ...
                // If this is a different status datapoint then set it in our database object.
                m_configItem->setOccEntityName( strNewEntityName );
    ...
    void PaZoneDetailView::onButtonReset() 
    {
        ...
        for (TA_Base_Core::IPaTisZoneMapsIt it = m_paTisZoneMapsToBeRemoved.begin(); it != m_paTisZoneMapsToBeRemoved.end(); ++it)
        {
            (*m_configPaTisZoneMaps).push_back(*it);
        }

        m_paTisZoneMapsToBeRemoved.clear();

        for (TA_Base_Core::IPaTisZoneMapsIt it = (*m_configPaTisZoneMaps).begin(); it != (*m_configPaTisZoneMaps).end(); ++it)
        {
            TA_Base_Core::IConfigPaTisZoneMap* cfgPaTisZoneMap = dynamic_cast<TA_Base_Core::IConfigPaTisZoneMap*>(*it);
            if (NULL != cfgPaTisZoneMap)
            {
                if (cfgPaTisZoneMap->isNew())
                {
                    m_paTisZoneMapsToBeRemoved.push_back(*it);
                }
                else
                {
                    resetButtonPressed(cfgPaTisZoneMap);
                }
            }
        }
        

        for (TA_Base_Core::IPaTisZoneMapsIt itemToRemove = m_paTisZoneMapsToBeRemoved.begin(); itemToRemove != m_paTisZoneMapsToBeRemoved.end(); ++itemToRemove)
        {
            for(TA_Base_Core::IPaTisZoneMapsIt it = (*m_configPaTisZoneMaps).begin(); it != (*m_configPaTisZoneMaps).end(); ++it)
            {
                if ((*it)->getKey() == (*itemToRemove)->getKey())
                {
                    (*m_configPaTisZoneMaps).erase(it);
                    break;
                }
            }
            delete *itemToRemove;
        ...
        m_paTisZoneMapsToBeRemoved.clear();
        m_paTisZoneMapsRecord->hasChanged = false;
    ...
    void PaZoneDetailView::onButtonApply() 
    {
        ...
        TA_Base_Core::IPaTisZoneMaps tmpZoneMaps = *m_configPaTisZoneMaps;
        for(TA_Base_Core::IPaTisZoneMapsIt it = tmpZoneMaps.begin(); it != tmpZoneMaps.end(); ++it)
        {
            applyButtonPressed(dynamic_cast<TA_Base_Core::IConfigPaTisZoneMap*>(*it));
        }

        for(TA_Base_Core::IPaTisZoneMapsIt it = m_paTisZoneMapsToBeRemoved.begin(); it != m_paTisZoneMapsToBeRemoved.end(); ++it)
        {
            TA_Base_Core::IConfigPaTisZoneMap* configPaTisZoneMap = dynamic_cast<TA_Base_Core::IConfigPaTisZoneMap*>(*it);
            if (!configPaTisZoneMap->isNew())
            {
                configPaTisZoneMap->deleteThisObject();
            }
            delete *it;
    ...
    std::list<std::string> PaZoneDetailView::getAssociatedEquipmentEntities()
    {
        std::list<std::string> entities;
        for (TA_Base_Core::IPaTisZoneMapsIt it = (*m_configPaTisZoneMaps).begin(); it != (*m_configPaTisZoneMaps).end(); ++it)
        {
            entities.push_back((*it)->getTisZoneEntity());
        }
        return entities;
    }

    void PaZoneDetailView::setAssociatedEquipmentEntity( const std::string& entityName, bool isAssociated )
    {
        bool bFind = false;
        TA_Base_Core::IPaTisZoneMapsIt matchingPaTisZoneMap = (*m_configPaTisZoneMaps).begin();
        for(; matchingPaTisZoneMap != (*m_configPaTisZoneMaps).end(); ++matchingPaTisZoneMap)
        {
            if (entityName == (*matchingPaTisZoneMap)->getTisZoneEntity())
            {
                bFind = true;
                break;
            }
        ...
        if (isAssociated)
        {
            ...
            if ( !bFind )
            {
                bool bFindInToBeRemoved = false;
                for(TA_Base_Core::IPaTisZoneMapsIt it = m_paTisZoneMapsToBeRemoved.begin(); it != m_paTisZoneMapsToBeRemoved.end(); ++it)
                {
                    if (entityName == (*it)->getTisZoneEntity())
                    {
                        bFindInToBeRemoved = true;
                        (*m_configPaTisZoneMaps).erase(it);
                        (*m_configPaTisZoneMaps).push_back(*it);
                        break;
                    }
                ...
                if ( !bFindInToBeRemoved )
                {
                    TA_Base_Core::IConfigPaTisZoneMap* newPaTisZoneMap = TA_Base_Core::PaTisZoneMappingAccessFactory::getInstance().createPaTisZoneMapping();
                    newPaTisZoneMap->setTisZoneEntity(entityName);
                    newPaTisZoneMap->setPaZoneKey(m_configItem->getKey());
                    (*m_configPaTisZoneMaps).push_back(newPaTisZoneMap);
                }

                m_paTisZoneMapsRecord->hasChanged = true;
            }
        ...
        else
        {
            ...
            if ( bFind )
            {
                m_paTisZoneMapsToBeRemoved.push_back(*matchingPaTisZoneMap);
                // This is definitely in the list so we must remove it
                (*m_configPaTisZoneMaps).erase(matchingPaTisZoneMap);
                m_paTisZoneMapsRecord->hasChanged = true;
            }
    ...
    void PaZoneDetailView::updateListViewItemStatusForPaTisZoneMaps()
    {
        if (m_paTisZoneMapsRecord->hasChanged)
        {
            m_listView->itemStatusUpdate(m_configItem->getUniqueIdentifier(), m_listView->MODIFIED);
        }
    }

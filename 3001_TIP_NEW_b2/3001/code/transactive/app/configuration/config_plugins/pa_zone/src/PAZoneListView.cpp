/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/pa_zone/src/PAZoneListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2013/04/22 15:38:09 $
  * Last modified by:  $Author: huangjian $
  * 
  * This view contains a list of all PA Zones available to be configured. This view must communicate
  * with the PaZoneDetailView as this will show the details of the item selected in this view.
  */
...
// using TA_IRS_App::PaZoneDatabaseAccessor::PaZoneRecord;
typedef TA_IRS_App::PaZoneDatabaseAccessor::PaZoneRecord PaZoneRecord;
...
namespace TA_IRS_App
{
    ...
    const CString PaZoneListView::NEW = "New";
    const CString PaZoneListView::MODIFIED = "Modified";
    const CString PaZoneListView::UPTODATE = "";

    const bool PaZoneListView::ASCENDING = true;
    const bool PaZoneListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(PaZoneListView, CListView)


    PaZoneListView::PaZoneListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PaZoneListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(PaZoneComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_ZONES) );
    }
...
BEGIN_MESSAGE_MAP(PaZoneListView, AbstractListView)
	//{{AFX_MSG_MAP(PaZoneListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int PaZoneListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void PaZoneListView::OnInitialUpdate() 
    {
    ...
    void PaZoneListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void PaZoneListView::loadAllItems()
    {
        ...
        PaZoneDatabaseAccessor* dbAccessor = dynamic_cast<PaZoneDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long,PaZoneRecord>::iterator iter = m_currentDisplayedItems.begin(); 
    ...
    void PaZoneListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        PaZoneRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long,PaZoneRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,PaZoneRecord>::value_type(id,details) );
        }
 ...
 /*   void PaZoneListView::deleteItem()
    {
    ...
    void PaZoneListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void PaZoneListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(PaZoneComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK PaZoneListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        PaZoneListView* list = reinterpret_cast<PaZoneListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<PaZoneListView*>(list);
        ...
        std::map<unsigned long,PaZoneRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,PaZoneRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
    ...
    void PaZoneListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // PaZoneListView diagnostics

    #ifdef _DEBUG
    void PaZoneListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void PaZoneListView::Dump(CDumpContext& dc) const
    {
	    AbstractListView::Dump(dc);
    }
    #endif //_DEBUG
    void PaZoneListView::itemStatusUpdate( unsigned long id, const CString& status )
    {
        LVFINDINFO info;
        info.flags = LVFI_PARAM;
        info.lParam = id;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/ats_fixed_block_map/src/AtsFixedBlockMapDetailView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
...
namespace TA_IRS_App
{
	...
	DDX_Control(pDX, IDC_EDIT_ARM_ID, m_armIdEdit);
	//}}AFX_DATA_MAP
    }
...
BEGIN_MESSAGE_MAP(AtsFixedBlockMapDetailView, AbstractDetailView)
	...
	ON_EN_KILLFOCUS(IDC_EDIT_ARM_ID, OnKillfocusEditArmId)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
    ...
    void AtsFixedBlockMapDetailView::showItemDetails(bool isShown)
    {
		...
		m_armIdEdit.ShowWindow(isShown);
		...
		GetDlgItem(IDC_ARM_ID_STATIC)->ShowWindow(isShown);
		...
		GetDlgItem(IDC_ARM_ID_BITMAP)->ShowWindow(isShown);
    }
    ...
    void AtsFixedBlockMapDetailView::populateItemDetails(TA_Base_Core::IConfigItem* currentItem)
    {
        ...
        try
        {
			...
			//Arm ID
			m_armIdEdit.SetWindowText( details.armId );
			m_armIdEdit.setTextType( TA_Base_Bus::TextColourEdit::VALID );
			m_armIdEdit.EnableWindow(true);
	...
	void AtsFixedBlockMapDetailView::OnKillfocusEditArmId()
	{
		FUNCTION_ENTRY("OnKillfocusEditArmId");
		TA_ASSERT(m_databaseAccessor != NULL, "Database accessor is NULL");
		...
		CString armIdCString;
		m_armIdEdit.GetWindowText(armIdCString);
		std::string armIdString = armIdCString;
		std::istringstream armIdStream; 
		armIdStream.str(armIdString);
		unsigned long armId;
		if (!(armIdStream >> armId))
		{
			// Failed - clear out
			m_armIdEdit.setTextType( TA_Base_Bus::TextColourEdit::INVALID );
		...
		{
			// If the branchId has not changed then there is no point doing an update
			if (m_configItem->getArmId() == armId)
			{
				m_armIdEdit.setTextType( TA_Base_Bus::TextColourEdit::VALID );
				FUNCTION_EXIT;
				return;

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/ats_fixed_block_map/src/AtsFixedBlockMapListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
...
namespace TA_IRS_App
{
	...
	const int		AtsFixedBlockMapListView::ARM_ID_COLUMN                      = 7;
	...
	const CString   AtsFixedBlockMapListView::ARM_ID_COLUMN_NAME				 = "Arm ID";
	...
	const int       AtsFixedBlockMapListView::ARM_ID_COLUMN_WIDTH                = 70;
...
END_MESSAGE_MAP()
    ...
    void AtsFixedBlockMapListView::OnInitialUpdate() 
    {
		...
		GetListCtrl().InsertColumn( ARM_ID_COLUMN,               ARM_ID_COLUMN_NAME,               LVCFMT_LEFT, ARM_ID_COLUMN_WIDTH               );
	}
    ...
    void AtsFixedBlockMapListView::loadAllItems()
    {
             ...
             iter != m_currentDisplayedItems.end(); ++iter)
        {
			...
			GetListCtrl().SetItemText(position, ARM_ID_COLUMN,               iter->second.armId);
    ...
    void AtsFixedBlockMapListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        if (foundItemIndex != -1)
        {
			...
			GetListCtrl().SetItemText(foundItemIndex, ARM_ID_COLUMN,             details.armId);
        ...
        else
        {
			...
			GetListCtrl().SetItemText(position, ARM_ID_COLUMN,               details.armId);
...
/*    void AtsFixedBlockMapListView::deleteItem()
    ...
    int CALLBACK AtsFixedBlockMapListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        ...
        switch(list->m_sortColumn)
        {
			...
			case (ARM_ID_COLUMN):
					string1 = item1->second.armId;
					string2 = item2->second.armId;
					returnVal = string1.CompareNoCase(string2);

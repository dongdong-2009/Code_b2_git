/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/pa_station_dva_message/src/PAStationDvaMessageListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2012/12/05 10:50:15 $
  * Last modified by:  $Author: huangjian $
  * 
  * This view contains a list of all PA Zones available to be configured. This view must communicate
  * with the PaStationDvaMessageDetailView as this will show the details of the item selected in this view.
  */
...
namespace TA_IRS_App
{
    ...
    const CString PaStationDvaMessageListView::NEW = "New";
    const CString PaStationDvaMessageListView::MODIFIED = "Modified";
    const CString PaStationDvaMessageListView::UPTODATE = "";

    const bool PaStationDvaMessageListView::ASCENDING = true;
    const bool PaStationDvaMessageListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(PaStationDvaMessageListView, CListView)


    PaStationDvaMessageListView::PaStationDvaMessageListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PaStationDvaMessageListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(PaStationDvaMessageComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_STATION_DVA_MESSAGES) );
    }
...
BEGIN_MESSAGE_MAP(PaStationDvaMessageListView, AbstractListView)
	//{{AFX_MSG_MAP(PaStationDvaMessageListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int PaStationDvaMessageListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void PaStationDvaMessageListView::OnInitialUpdate() 
    {
    ...
    void PaStationDvaMessageListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void PaStationDvaMessageListView::loadAllItems()
    {
        ...
        PaStationDvaMessageDatabaseAccessor* dbAccessor = dynamic_cast<PaStationDvaMessageDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long, PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord>::iterator iter 
    ...
    void PaStationDvaMessageListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord details = dbAccessor->getItemDetails( id );

        // If the item is found then update it with the new details
        int foundItemIndex = GetListCtrl().FindItem(&info);
        if (foundItemIndex != -1)
        {
            TA_Base_Core::IConfigPaDvaMessage* dvaMessage = dynamic_cast<TA_Base_Core::IConfigPaDvaMessage*>(dbAccessor->getItem(id));
            TA_Base_Core::IConfigPaTisMessageMap* paTisMessageMap = dbAccessor->getPaTisMessageMapByPadmesId(dvaMessage->getKey());
            ...
            if (NULL != paTisMessageMap)
            {
                if (paTisMessageMap->isNew() || paTisMessageMap->hasChanged() )
                {
                    details.status = MODIFIED;
                }
                else
        ...
        std::map<unsigned long, PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord>::iterator matching;
        ...
        else
        {
            m_currentDisplayedItems.insert( 
                std::map<unsigned long, PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord>::value_type(id, details) );
        }
    ...
    void PaStationDvaMessageListView::copyItem()
    {
    ...
    void PaStationDvaMessageListView::newItem()
    {
 ...
 /*   void PaStationDvaMessageListView::deleteItem()
    {
    ...
    void PaStationDvaMessageListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void PaStationDvaMessageListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(PaStationDvaMessageComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK PaStationDvaMessageListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        PaStationDvaMessageListView* list = reinterpret_cast<PaStationDvaMessageListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<PaStationDvaMessageListView*>(list);
        ...
        std::map<unsigned long, PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord>::iterator item1 
                                                        = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long, PaStationDvaMessageDatabaseAccessor::PaStationDvaMessageRecord>::iterator item2 
    ...
    void PaStationDvaMessageListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // PaStationDvaMessageListView diagnostics

    #ifdef _DEBUG
    void PaStationDvaMessageListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void PaStationDvaMessageListView::Dump(CDumpContext& dc) const
    {

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/pa_station_dva_message/src/PAStationDvaMessageDatabaseAccessor.h $
  * @author:  Karen Graham
  * @version: $Revision: #6 $
  *
...
namespace TA_IRS_App
{
    ...
    public:
         ...
         */
        static CString getTypeString(const unsigned short type);        
         ...
         *
         */
        std::vector<TA_Base_Core::TisMessage> getAllTisMessages();
         ...
         */
        TA_Base_Core::TisMessage getTisMessage(TA_Base_Core::IConfigPaTisMessageMap& configItem);
        TA_Base_Core::TisMessage getTisMessage(unsigned long key) const;

        TA_Base_Core::IConfigPaTisMessageMap* getPaTisMessageMapByPadmesId(unsigned long padmesId);
        TA_Base_Core::IConfigPaTisMessageMap* createPaTisMessageMap(unsigned long padmesId);

        void removePaTisMap(unsigned long padmesId);
    ...
    private:
         ...
         */
        PaStationDvaMessageRecord getRecordFrom(TA_Base_Core::IConfigPaDvaMessage& item);
 

        typedef std::map<unsigned long, TA_Base_Core::IConfigPaTisMessageMap*>    ConfigPaTisMapItems;
        typedef ConfigPaTisMapItems::iterator                                       ConfigPaTisMapItem;

        ConfigPaTisMapItems             m_configPaTisMapItems;      // This is all the PA Tis Message Map loaded from the database
        

        typedef std::map<unsigned long, TA_Base_Core::IConfigPaDvaMessage*> ConfigPaDvaMessages;
        typedef ConfigPaDvaMessages::iterator                                       ConfigPaDvaMessagesIt;

        ConfigPaDvaMessages             m_configPaDvaMessages;      // This is all the PA Dva Message Map loaded from the database
        

        typedef std::map<unsigned long, TA_Base_Core::TisMessage> TisMessages;

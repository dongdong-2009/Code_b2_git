/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/ecs_zone/src/ECSZoneListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
  ...
  * 
  * This view contains a list of all ECS Zones available to be configured. This view must communicate
  * with the ECSZoneDetailView as this will show the details of the item selected in this view.
  */
...
typedef TA_IRS_App::ECSZoneDatabaseAccessor::ECSZoneRecord ECSZoneRecord;

using  namespace TA_Base_App::TA_Configuration;

namespace TA_IRS_App
{
    const int       ECSZoneListView::APPLIED_COLUMN                     = 0;
    const int       ECSZoneListView::ECS_ZONE_ID_COLUMN                 = 1;
    const int       ECSZoneListView::ECS_LOCATION_COLUMN                = 2;
    const int       ECSZoneListView::TRACK_COLUMN                       = 3;
    const int       ECSZoneListView::OCC_EQUIPMENT_COLUMN               = 4;
    const int       ECSZoneListView::STN_EQUIPMENT_COLUMN               = 5;
	//TD17826 - marc_bugfix_start
	//Support IS_IGNORING_CONGESTION field	
	const int		ECSZoneListView::IGNORE_CONGESTION_COLUMN           = 6;
	//TD17826 - marc_bugfix_end

    const CString   ECSZoneListView::APPLIED_COLUMN_NAME                = "Status";
    const CString   ECSZoneListView::ECS_ZONE_ID_COLUMN_NAME            = "Zone ID";
    const CString   ECSZoneListView::ECS_LOCATION_COLUMN_NAME           = "ECS Location";
    const CString   ECSZoneListView::TRACK_COLUMN_NAME                  = "Track";
    const CString   ECSZoneListView::OCC_EQUIPMENT_COLUMN_NAME          = "OCC Equipment";
    const CString   ECSZoneListView::STN_EQUIPMENT_COLUMN_NAME          = "Station Equipment";
	//TD17826 - marc_bugfix_start
	const CString   ECSZoneListView::IGNORE_CONGESTION_COLUMN_NAME      = "Ignore Congestion";
	//TD17826 - marc_bugfix_end

    const int       ECSZoneListView::APPLIED_COLUMN_WIDTH               = 75;
    const int       ECSZoneListView::ECS_ZONE_ID_COLUMN_WIDTH           = 80;
    const int       ECSZoneListView::ECS_LOCATION_COLUMN_WIDTH          = 80;
    const int       ECSZoneListView::TRACK_COLUMN_WIDTH                 = 160;
    const int       ECSZoneListView::OCC_EQUIPMENT_COLUMN_WIDTH         = 300;
    const int       ECSZoneListView::STN_EQUIPMENT_COLUMN_WIDTH         = 300;
	//TD17826 - marc_bugfix_start
	const int       ECSZoneListView::IGNORE_CONGESTION_COLUMN_WIDTH      = 75;
	//TD17826 - marc_bugfix_end

    // These are the possible values for the status of each ECS Zone
    const CString ECSZoneListView::NEW = "New";
    const CString ECSZoneListView::MODIFIED = "Modified";
    const CString ECSZoneListView::UPTODATE = "";

    const bool ECSZoneListView::ASCENDING = true;
    const bool ECSZoneListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(ECSZoneListView, CListView)


    ECSZoneListView::ECSZoneListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(ECS_ZONE_ID_COLUMN)  // This is the column to sort on when the list control gets created
    {
        m_componentIcon = LoadIcon(GetModuleHandle(ECSZoneComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_ECS_ZONES) );
    }
...
BEGIN_MESSAGE_MAP(ECSZoneListView, AbstractListView)
	//{{AFX_MSG_MAP(ECSZoneListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int ECSZoneListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void ECSZoneListView::OnInitialUpdate() 
    {
        ...
        GetListCtrl().InsertColumn( ECS_ZONE_ID_COLUMN,          ECS_ZONE_ID_COLUMN_NAME,          LVCFMT_LEFT, ECS_ZONE_ID_COLUMN_WIDTH          );
        GetListCtrl().InsertColumn( ECS_LOCATION_COLUMN,         ECS_LOCATION_COLUMN_NAME,         LVCFMT_LEFT, ECS_LOCATION_COLUMN_WIDTH         );
        GetListCtrl().InsertColumn( TRACK_COLUMN,                TRACK_COLUMN_NAME,                LVCFMT_LEFT, TRACK_COLUMN_WIDTH                );
        GetListCtrl().InsertColumn( OCC_EQUIPMENT_COLUMN,        OCC_EQUIPMENT_COLUMN_NAME,        LVCFMT_LEFT, OCC_EQUIPMENT_COLUMN_WIDTH        );
        GetListCtrl().InsertColumn( STN_EQUIPMENT_COLUMN,        STN_EQUIPMENT_COLUMN_NAME,        LVCFMT_LEFT, STN_EQUIPMENT_COLUMN_WIDTH        );
		//TD17826 - marc_bugfix_start
		GetListCtrl().InsertColumn( IGNORE_CONGESTION_COLUMN,    IGNORE_CONGESTION_COLUMN_NAME,    LVCFMT_LEFT, IGNORE_CONGESTION_COLUMN_WIDTH    );
		//TD17826 - marc_bugfix_end
    }


    void ECSZoneListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void ECSZoneListView::loadAllItems()
    {
        ...
        ECSZoneDatabaseAccessor* dbAccessor = dynamic_cast<ECSZoneDatabaseAccessor*>(getDatabaseAccessor());
        TA_ASSERT( dbAccessor != NULL, "DB accessor could not be cast to an ECS Zone one");
        ...
        for (std::map<unsigned long,ECSZoneRecord>::iterator iter = m_currentDisplayedItems.begin(); 
             iter != m_currentDisplayedItems.end(); ++iter)
        {
            ...
            GetListCtrl().SetItemText(position, ECS_ZONE_ID_COLUMN,          iter->second.ecsZoneId);
            GetListCtrl().SetItemText(position, ECS_LOCATION_COLUMN,         iter->second.ecsLocation);
            GetListCtrl().SetItemText(position, TRACK_COLUMN,                iter->second.track);
            GetListCtrl().SetItemText(position, OCC_EQUIPMENT_COLUMN,        iter->second.occEquipment);
            GetListCtrl().SetItemText(position, STN_EQUIPMENT_COLUMN,        iter->second.stnEquipment);
			//TD17826 - marc_bugfix_start
			GetListCtrl().SetItemText(position, IGNORE_CONGESTION_COLUMN,    iter->second.bIgnoringCongestion ? "Yes" : "No");
			//TD17826 - marc_bugfix_end
    ...
    void ECSZoneListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        ECSZoneRecord details = dbAccessor->getItemDetails( id );
        ...
        if (foundItemIndex != -1)
        {
            ...
            GetListCtrl().SetItemText(foundItemIndex, ECS_ZONE_ID_COLUMN,          details.ecsZoneId);
            GetListCtrl().SetItemText(foundItemIndex, ECS_LOCATION_COLUMN,         details.ecsLocation);
            GetListCtrl().SetItemText(foundItemIndex, TRACK_COLUMN,                details.track);
            GetListCtrl().SetItemText(foundItemIndex, OCC_EQUIPMENT_COLUMN,        details.occEquipment);
            GetListCtrl().SetItemText(foundItemIndex, STN_EQUIPMENT_COLUMN,        details.stnEquipment);
			//TD17826 - marc_bugfix_start
			GetListCtrl().SetItemText(foundItemIndex, IGNORE_CONGESTION_COLUMN,    details.bIgnoringCongestion ? "Yes" : "No");
			//TD17826 - marc_bugfix_end
        ...
        else
        {
            ...
            GetListCtrl().SetItemText(position, ECS_ZONE_ID_COLUMN,          details.ecsZoneId);
            GetListCtrl().SetItemText(position, ECS_LOCATION_COLUMN,         details.ecsLocation);
            GetListCtrl().SetItemText(position, TRACK_COLUMN,                details.track);
            GetListCtrl().SetItemText(position, OCC_EQUIPMENT_COLUMN,        details.occEquipment);
            GetListCtrl().SetItemText(position, STN_EQUIPMENT_COLUMN,        details.stnEquipment);
			//TD17826 - marc_bugfix_start
			GetListCtrl().SetItemText(position, IGNORE_CONGESTION_COLUMN,    details.bIgnoringCongestion ? "Yes" : "No");
			//TD17826 - marc_bugfix_end
        ...
        std::map<unsigned long,ECSZoneRecord>::iterator matching;
        ...
        {
            m_currentDisplayedItems.insert( std::map<unsigned long,ECSZoneRecord>::value_type(id,details) );
        }
    ...
    void ECSZoneListView::copyItem()
    {
		...
		for(int i=0; i<selectedCount; i++)
		{
			...
			{
				LOG_EXCEPTION_CATCH( SourceInfo, "UnknownException","Could not copy the item");
				TA_Base_Bus::TransActiveMessage userMsg;
				CString reason = "of an error";
				userMsg << reason;
				userMsg.showMsgBox(IDS_UE_030069);
			}
		}
    ...
    void ECSZoneListView::newItem()
    {
		...
		{
			LOG_EXCEPTION_CATCH( SourceInfo, "UnknownException","Could not create a new item");
			//UE-030060
			// TD14164 ++
			/*MessageBox("The item could not be created because of a database error.",RunParams::getInstance().get(RPARAM_APPNAME).c_str(),MB_OK|MB_ICONSTOP);*/
			TA_Base_Bus::TransActiveMessage userMsg;
...
/*    void ECSZoneListView::deleteItem()
    {
    ...
    void ECSZoneListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void ECSZoneListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(ECSZoneComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK ECSZoneListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        ECSZoneListView* list = reinterpret_cast<ECSZoneListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<ECSZoneListView*>(list);
        ...
        std::map<unsigned long,ECSZoneRecord>::iterator item1 = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long,ECSZoneRecord>::iterator item2 = list->m_currentDisplayedItems.find( index2 );
        ...
        switch(list->m_sortColumn)
        {
            ...
            case(ECS_ZONE_ID_COLUMN):
                    value1 = atoi((LPCTSTR)item1->second.ecsZoneId);
                    value2 = atoi((LPCTSTR)item2->second.ecsZoneId);
                    returnVal = value1 - value2 ;
                    break;

            case(ECS_LOCATION_COLUMN):
                    string1 = item1->second.ecsLocation;
                    string2 = item2->second.ecsLocation;
                    returnVal = string1.CompareNoCase(string2);
                    break;
            ...
            case(OCC_EQUIPMENT_COLUMN):
                    string1 = item1->second.occEquipment;
                    string2 = item2->second.occEquipment;
                    returnVal = string1.CompareNoCase(string2);
                    break;

            case(STN_EQUIPMENT_COLUMN):
                    string1 = item1->second.stnEquipment;
                    string2 = item2->second.stnEquipment;
                    returnVal = string1.CompareNoCase(string2);
                    break;
			//TD17826 - marc_bugfix_start
			case(IGNORE_CONGESTION_COLUMN):
					if (item1->second.bIgnoringCongestion == item2->second.bIgnoringCongestion)
					{
						returnVal = 0;
					}
					else if (item1->second.bIgnoringCongestion == false)
					{
						returnVal = -1;
					}
					else 
    ...
    void ECSZoneListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // ECSZoneListView diagnostics

    #ifdef _DEBUG
    void ECSZoneListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void ECSZoneListView::Dump(CDumpContext& dc) const
    {

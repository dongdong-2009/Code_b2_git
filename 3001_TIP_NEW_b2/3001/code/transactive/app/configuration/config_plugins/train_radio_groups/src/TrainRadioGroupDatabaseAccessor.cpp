...
namespace TA_IRS_App
{
    ...
    std::map<CString, unsigned long> TrainRadioGroupDatabaseAccessor::getItemNames( CProgressCtrl& progress )
    {
              ...
              iter != m_trainRadioGroups.end(); ++iter )
        {
            ...
            names.insert( std::map<CString, unsigned long>::value_type( strName.c_str(), unKey ) );
            progress.StepIt();
        }

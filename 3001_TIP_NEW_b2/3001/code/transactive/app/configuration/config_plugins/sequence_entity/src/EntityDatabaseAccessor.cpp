/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/configuration/config_plugins/sequence_entity/src/EntityDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #4 $
  *
...
namespace TA_Base_App
{
    ...
    std::map<CString,unsigned long> EntityDatabaseAccessor::getItemNames(CProgressCtrl& progress)
    {
             ...
             ++iter)
        {
            if (iter->second != NULL)
            {	
				names.insert(std::make_pair(iter->second->getName().c_str(), iter->second->getUniqueIdentifier()));
                progress.StepIt();
            }
        }
        FUNCTION_EXIT;
        ...
        for (std::vector<ILocation*>::iterator iter = locations.begin(); iter != locations.end(); ++iter)
        {
            if (*iter != NULL)
            {
                try
                {
                    ...
                    std::string locationName = (*iter)->getName().c_str();
					namesAndKeys.insert(std::make_pair(key, locationName));
                }
                catch ( const TA_Base_Core::DataException& )
                {

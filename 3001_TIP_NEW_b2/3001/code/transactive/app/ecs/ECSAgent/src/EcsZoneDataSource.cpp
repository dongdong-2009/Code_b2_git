/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ECSAgent/src/EcsZoneDataSource.cpp $
  * @author:  seanl
  * @version: $Revision: #10 $
  *
    ...
    // throws TrainIdNotFoundException
{
    ...
    for (ZoneIdToTrainStatesMapIt it = m_trainStatesMap.begin(); it != m_trainStatesMap.end(); ++it)
    {
		...
		if (NULL == (it->second))
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError,"trainState is NULL" );
			TA_THROW(TA_Base_Core::TransactiveException("trainState is NULL"));
		}
	...
	if (NULL == trainStatePtr)
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Unable to allocate memory." );
		TA_THROW(TA_Base_Core::TransactiveException("Unable to allocate memory."));
	}
	// ++marvin
    ...
    bool updateAffectedDataPoints)
{
	...
	try
	{
		it = getRecordOfTrainId(trainId);
	}
	catch ( ... )
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError,"trainState of train id(%d) is NULL",trainId );
		LOG(SourceInfo, TA_Base_Core::DebugUtil::ExceptionCatch, "Unknown", "trainState is NULL");
		return;
	}
	...
	try
	{
		...
		if (!trainStatePtr->m_isLocalisationValid)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Localisation validity bit are invalid. ATS Data is not used.");	
			return;
		}
		...
		trainStatePtr->m_isStalled = (EcsHelpers::getStallTimeInSeconds(packet) >= m_theConfig.getThresholdStallTimeInSecs());
		...
		if (!trainStatePtr->m_isHot)
		{
			// Not in hot state yet
			trainStatePtr->m_isHot = (EcsHelpers::getMaxExternalTemperatureInCelsius(packet) >= m_theConfig.getThresholdTemperature());

			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
				"EcsZoneDataSource::processAtsPacket: packet temp=%f, thresholdTemp=%lu, m_isHot=%d", 
				EcsHelpers::getMaxExternalTemperatureInCelsius(packet), m_theConfig.getThresholdTemperature(), trainStatePtr->m_isHot);
		}
		else 
		{
			// Consider deadband if it's in hot state
			if ( EcsHelpers::getMaxExternalTemperatureInCelsius(packet) < m_theConfig.getTemperatureDeadbandLowerLimit())
			{
			...
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
				"EcsZoneDataSource::processAtsPacket: packet temp=%f, deadBandLowerLimitTemp=%lu, m_isHot=%d", 
				EcsHelpers::getMaxExternalTemperatureInCelsius(packet), m_theConfig.getTemperatureDeadbandLowerLimit(), trainStatePtr->m_isHot);
		}
		...
		*/
        ...
        trainStatePtr->m_isStalledCongested = false;
        trainStatePtr->m_isHotCongested = false;
		...
		if (false == trainStatePtr->m_isIgnoringCongestion)
		{
			...
			{
				trainStatePtr->m_isStalledCongested |= trainStatePtr->m_isStalled;
			}

			if (true == m_theConfig.getCongestionUsesTemperature())
			{
				trainStatePtr->m_isHotCongested |= trainStatePtr->m_isHot;
			}

			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
				"EcsZoneDataSource::processAtsPacket: useStallTime=%d useTemperature=%d m_isHotCongested=%d m_isStalledCongested=%d",
				m_theConfig.getCongestionUsesStallTime(), m_theConfig.getCongestionUsesTemperature(),
				trainStatePtr->m_isHotCongested, trainStatePtr->m_isStalledCongested);

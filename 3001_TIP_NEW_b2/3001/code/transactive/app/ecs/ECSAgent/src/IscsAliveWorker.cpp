/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ECSAgent/src/IscsAliveWorker.cpp $
  * @author:  Ripple
  * @version: $Revision: #8 $
  *
...
void IscsAliveWorker::initProxy()
{
    ...
    TA_Base_Core::DataPointEntityData* rtu1DPEntityData = NULL;
    ...
    try
    {
        ...
        rtu1DPEntityData = dynamic_cast<TA_Base_Core::DataPointEntityData*>(rtu1EntityData);
    }
    ...
    {
        LOG_EXCEPTION_CATCH( SourceInfo, "UnknownException", "Exception caught when EntityAccessFactory::getEntity" );
    }

    TA_Base_Core::DataPointEntityData* rtu2DPEntityData = NULL;
    ...
    try
    {
        ...
        rtu2DPEntityData = dynamic_cast<TA_Base_Core::DataPointEntityData*>(rtu2EntityData);
    }
    catch ( const std::exception& expWhat )
    {
        LOG_EXCEPTION_CATCH( SourceInfo, typeid(expWhat).name(), expWhat.what() );
    ...
    {
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "[DataError] RTU1[%s] is not defined as datapoint entity", rtu1PollingStatusEntityName.c_str() );
    }

    if ( NULL != rtu2DPEntityData )
    {
    ...
    {
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "[DataError] RTU2[%s] is not defined as datapoint entity", rtu2PollingStatusEntityName.c_str() );
    }
...
{
	unsigned long rtu1Quality = 0u;
    unsigned long rtu2Quality = 0u;
    unsigned long rtu1Value	= 0u;
    unsigned long rtu2Value = 0u;

    try
    {
        rtu1Quality = m_rtu1proxy->getStatus();
        rtu1Value = m_rtu1proxy->getValue().getEnum();
    }
    catch (const TA_Base_Core::ScadaProxyException& expScada)
    {
        LOG_EXCEPTION_CATCH(SourceInfo, "TA_Base_Core::ScadaProxyException", expScada.what() );
    }
    catch (...)
    {
        LOG_EXCEPTION_CATCH(SourceInfo, "Unknown", "Caught unknown exception when getStatus or getValue" );
    }

    try
    {
        rtu2Quality = m_rtu2proxy->getStatus();
        rtu2Value	= m_rtu2proxy->getValue().getEnum();
    }
    catch (const TA_Base_Core::ScadaProxyException& expScada)
    {
        LOG_EXCEPTION_CATCH(SourceInfo, "TA_Base_Core::ScadaProxyException", expScada.what() );

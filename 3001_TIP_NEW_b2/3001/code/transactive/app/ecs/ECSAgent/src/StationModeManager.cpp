...
/**
  ...
  * in any form.
  *
  * Source : $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ECSAgent/src/StationModeManager.cpp $
  * @author  A. Parker
  * @version $Revision: #9 $
...
const unsigned short STATION_MODE_ERROR_MSG_MAX_LEN = 200;
  ...
  m_rightsMgr(NULL)		// CL17274
{
	...
	if ( NULL == m_rightsMgr)
	{
		...
		TA_THROW(TA_Base_Core::TransactiveException("Rights library not created."));
	}
	// ++CL17274
...
void StationModeManager::executeStationMode(CORBA::Long stationMode, CORBA::Boolean canOverrideStationModeInProgress, const char* sessionId)
{
	...
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "No rights to perform station mode %lu to station %lu", stationMode, m_theConfig.getAgentLocationKey());
            ...
            (!canOverrideStationModeInProgress) )
    {
        ...
        // DTL-467 huang jian, should never use sprintf instead of snprintf
		char  buffer[STATION_MODE_ERROR_MSG_MAX_LEN];
        memset(buffer, 0, STATION_MODE_ERROR_MSG_MAX_LEN);
        ACE_OS::snprintf(buffer, STATION_MODE_ERROR_MSG_MAX_LEN, TA_IRS_Bus::ECS_ERROR_MSG_SM_IN_PROGRESS_P, stationModeState.toStationMode);
    ...
    try
    {
        m_stationModeExecutionWorker.execute(m_theDatapoints.getCurrentStationMode(),
            stationMode, TA_IRS_Bus::ECS_NONE_OR_INVALID, sessionId);
    }
...
void StationModeManager::executeStationModeForMasterMode(CORBA::Long stationMode, CORBA::Long forMasterMode, const char* sessionId)
{
	...
	if ( !checkPermission( sessionId ) )
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "CL-17274 No rights to perform station mode %lu fro master mode %lu to station %lu", stationMode, forMasterMode, m_theConfig.getAgentLocationKey());
		throw TA_Base_Core::AccessDeniedException("No rights to perform station mode");
	}

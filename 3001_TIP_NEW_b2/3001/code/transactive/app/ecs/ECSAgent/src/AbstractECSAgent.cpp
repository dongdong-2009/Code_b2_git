/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ECSAgent/src/AbstractECSAgent.cpp $
  * @author:  seanl
  * @version: $Revision: #7 $
  *
    ...
    // throw(TA_Base_Core::InvalidECSConfigurationException, TA_Core::DatabaseException, TA_Core::DataException)
{
	...
	if (NULL == m_genericAgent)
	{
		...
		TA_THROW( TA_Base_Core::TransactiveException("m_genericAgent is NULL" ));
	}
   ...
   // throw(TA_Base_Core::InvalidECSConfigurationException)
{
    ...
    m_theSummary.getItaEcsCachedMapsInitialiser().validateAllInitializedMaps();
    ...
    // throw(TA_Base_Core::InvalidECSConfigurationException)
{
	...
	if ( TA_Base_Core::DataPointEntityData::getStaticType() == entityDataType )
	{
		...
		theEntity = createDataPointEntity ( EntityData );
	}
...
void AbstractECSAgent::agentSetControl()
{
	...
	// chenlei++
	// 1. For the first launching(the if block), it's not necessary for us to use multiple thread
	//	  cos the flag m_isOnStartup will be set to FAULSE quickly, this maybe bring some problem
	// 2. In the else block, the following part moves into a function call name failoverSpecific()
	//    and a new thread is up to perform that function so as to reduce the failover time.   
	if (true == m_isOnStartup)
	{
...
//chenlei++
// perform necessary failover procedure in this function
void AbstractECSAgent::failoverSpecific()
{
	FUNCTION_ENTRY("failoverSpecific");
	m_theSummary.setControl(m_isOnStartup);
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "FAILOVER LOG: summary set control finished.");

	m_ecsZoneDataSource->setControl(m_isOnStartup);
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "FAILOVER LOG: ecsZoneDataSource set control finished.");
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "FAILOVER LOG: summary alarm closing finished.");
	FUNCTION_EXIT;

...
/**
  ...
  * in any form.
  *
  * Source : $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ECSAgent/src/MasterModeManager.cpp $
  * @author  A. Parker
  * @version $Revision: #7 $
...
const unsigned short ECS_ERROR_MSG_MAX_LEN = 128; //DTL-467

MasterModeManager::MasterModeManager( ItaEcsCachedMaps& theMaps,
    ItaEcsCachedConfig& theConfig, ItaEcsAuditing& theAuditing, ItaEcsAlarms& theAlarms,
    ItaEcsAgentCompleteState& theCompleteState, ItaEcsDatapoints& theDatapoints) : 
//throw (TA_Core::GenericAgentException)
m_masterModeExecutionWorker( theMaps, theConfig, theAuditing, theAlarms, theCompleteState, theDatapoints, *this ), 
...
void MasterModeManager::processEmergencyUpdate(unsigned long zoneId, bool value)
{
	...
	MasterModePair masterModePair;

    try
    {
        masterModePair = m_theMaps.getMasterModesFromZoneId(zoneId);
    }
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
						"MasterModeManager::processEmergencyUpdate - barrier end");
}
										  ...
										  const char* sessionId)
{
    ...
    {
        // DTL-467
        char msg[ECS_ERROR_MSG_MAX_LEN];
        memset(msg, 0, ECS_ERROR_MSG_MAX_LEN);
        ACE_OS::snprintf( msg, ECS_ERROR_MSG_MAX_LEN, TA_IRS_Bus::ECS_ERROR_MSG_MM_ACTIVE_P, m_theDatapoints.getCurrentMasterMode() );
        ...
        throw(TA_IRS_Bus::IMasterEcsAgentCorbaDef::MasterModeActive(m_theDatapoints.getCurrentMasterMode(), msg));
    }
    ...
    if (false==canOverrideMasterModeInProgress && 1==m_theDatapoints.getMasterModeInProgress())
    {
        ...
        char msg[ECS_ERROR_MSG_MAX_LEN];
        memset(msg, 0, ECS_ERROR_MSG_MAX_LEN);
        ACE_OS::snprintf(msg, ECS_ERROR_MSG_MAX_LEN, TA_IRS_Bus::ECS_ERROR_MSG_MM_IN_PROGRESS_P, masterModeState.toMasterMode);
        ...
        throw(TA_IRS_Bus::IMasterEcsAgentCorbaDef::MasterModeInProgress (masterModeState.toMasterMode, msg));
    }
...
void MasterModeManager::selectMasterMode(CORBA::Long masterMode, const char* sessionId)
{
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Selected Master mode = %lu; Current Master mode = %lu, Configured Normal mode = %lu", 
			masterMode, m_theDatapoints.getCurrentMasterMode(), m_theConfig.getNormalMasterMode());

	///
	//check m_theDatapoints.getEmergencyApplied(zoneId) for ALL THE ZONES
	 
	bool found = m_theMaps.checkIfEmergencyMasterMode(masterMode);
    ...
    if (found == true)
    {
        ...
        {
			std::ostringstream msg;
			msg <<"Can not select Master mode as total zones in emergency = "<<zonesInEmergency.size();
			msg <<" : Ecs zones " << EcsHelpers::toCommaSeparatedList(zonesInEmergency)<< "in emergency. ";
			msg <<" And current emergency Master mode = "<<m_theDatapoints.getCurrentMasterMode();
			m_theAuditing.sendEcsMasterModeResult(
				m_theDatapoints.getCurrentMasterMode(),
				masterMode, 
				msg.str().c_str(),
			...
			if(1 == m_theDatapoints.getMasterModeInProgress())
			{
				TA_IRS_Bus::IMasterEcsAgentCorbaDef::EmergencyMasterModeInProgress exception;
				exception.what = msg.str().c_str();
				throw(exception);	
			}
			else
			{
				...
				TA_IRS_Bus::IMasterEcsAgentCorbaDef::EmergencyMasterModeActive exception;
				exception.what = msg.str().c_str();
				throw(exception);	
			}	
		}
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "MasterModeManager::selectMasterMode - No zones in emergency.");
		}
	}
	///
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "MasterModeManager::selectMasterMode - Proceeding for Selected Master mode ");
    ...
    // Set selection ModeSymbols to either TA_IRS_Bus::ECS_NONE_AND_GUI_OPEN or corresponding to the masterMode
    // Don't touch last applied ModeSymbols 

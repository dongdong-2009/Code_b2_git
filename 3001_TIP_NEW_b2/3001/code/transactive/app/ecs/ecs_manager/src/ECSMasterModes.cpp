/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ecs_manager/src/ECSMasterModes.cpp $
  * @author:  Robert Young
  * @version: $Revision: #9 $
  *
...
namespace TA_IRS_App
{
    ...
    void ECSMasterModes::masterModeSelectionChanged( int masterModeIndex )
    {
        ...
        try
        {
            ...
            {
				LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Calling selectMasterMode for current selection = %d",
					 m_currentMasterModeDetails[masterModeIndex].masterMode);
				CORBA_CALL( (*m_masterECSAgent),
                    selectMasterMode, ( m_currentMasterModeDetails[masterModeIndex].masterMode, m_sessionId.c_str() ) );

                std::ostringstream status;
                status << "Master mode  " << m_currentMasterModeDetails[masterModeIndex].masterMode << " is currently selected. ";
                m_ecsMasterModesView.addStatusLine(status.str());
            }
        }
		catch( const TA_Base_Core::BadParameterException& ex )
		{
			...
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::BadParameterException", TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex).c_str(), "Failed to select Master Mode:%u", 
				m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 
		...
		catch( const TA_Base_Core::AccessDeniedException& ex )
		{
			...
			m_ecsMasterModesView.addStatusLine(status.str());
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::AccessDeniedException", TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex).c_str(), "Failed to select Master Mode:%u", 
				m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 

		}
		catch( const TA_IRS_Bus::IMasterEcsAgentCorbaDef::EmergencyMasterModeInProgress& ex )
		{
			m_ecsMasterModesView.enableApplyMasterModeControl( false );
			std::ostringstream status;
			status << "Unable to Select ECS Master Mode ";
			status << ex.masterMode;
			status << TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex);
			m_ecsMasterModesView.addStatusLine(status.str());

			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::EmergencyMasterModeInProgress", TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex).c_str(), "Current Master mode:%u so failed to select Master Mode:%u", 
				ex.masterMode, m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 
		}
		catch( const TA_IRS_Bus::IMasterEcsAgentCorbaDef::EmergencyMasterModeActive& ex )
		{
			m_ecsMasterModesView.enableApplyMasterModeControl( false );
			std::ostringstream status;
			status << "Unable to Select ECS Master Mode ";
			...
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::EmergencyMasterModeActive", TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex).c_str(), "Current Master mode:%u so failed to select Master Mode:%u", 
				ex.masterMode, m_currentMasterModeDetails[ masterModeIndex ].masterMode );
		}
 		catch( const CORBA::Exception& ex )
		{
			...
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::Exception", TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex).c_str(), "Failed to select Master Mode:%u", 
				m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 
		}
		catch (const TA_Base_Core::ObjectResolutionException& )
		{
			...
			m_ecsMasterModesView.addStatusLine(ostr.str());
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::ObjectResolutionException:Failed to select Master Mode:%u", 
				m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 
		...
		catch( ... )
		{
			...
			m_ecsMasterModesView.addStatusLine(ostr.str());
			LOG( SourceInfo, TA_Base_Core::DebugUtil::ExceptionDetails, 
				"TA_Core::Unknown_Exception: Failed to select Master Mode:%u", 
				m_currentMasterModeDetails[ masterModeIndex ].masterMode ); 
		} 
		...
		if ( selectedButton == IDOK)
        {
			...
			while(tryAgain)
			{
				...
				catch( const TA_IRS_Bus::IMasterEcsAgentCorbaDef::StationsInLocalMode& ex )
				{
					...
					userMsg << param;
					UINT selectedButton = userMsg.showMsgBox(IDS_UW_520014);
					// ++TD16884

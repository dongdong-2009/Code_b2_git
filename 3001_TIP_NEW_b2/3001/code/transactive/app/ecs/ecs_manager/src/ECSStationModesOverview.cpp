/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/ecs/ecs_manager/src/ECSStationModesOverview.cpp $
  * @author:  Robert Young
  * @version: $Revision: #14 $
  *
...
namespace TA_IRS_App
{
	...
	void ECSStationModesOverview::confirmApplyProposedStationMode(std::map<int, TA_IRS_App::EProposedStationModeType> allProposedIndexAndType)
	{
        ...
        if ( !vecEmergencyLocs.empty() )
        {
            std::stringstream stmStations;
            std::vector<int>::iterator itLoop = vecEmergencyLocs.begin();
            stmStations << m_locationNames[*itLoop];

            for ( ++itLoop; vecEmergencyLocs.end() != itLoop; ++itLoop )
            {
                stmStations << ", " << m_locationNames[*itLoop];
            }

            TA_Base_Bus::TransActiveMessage userMsg;
            userMsg << stmStations.str();
            userMsg.showMsgBox( IDS_UE_520017 );
        }
	}
    ...
    void ECSStationModesOverview::applyAllProposedCongestionStationModes()
    {
		...
		if ( userMsg.showMsgBox(IDS_UW_520011) == IDOK )
        {
            ...
            if ( !vecEmergencyLocs.empty() )
            {
                ...
                TA_Base_Bus::TransActiveMessage useError;
                useError << stmStations.str();
                useError.showMsgBox( IDS_UE_520017 );
            }

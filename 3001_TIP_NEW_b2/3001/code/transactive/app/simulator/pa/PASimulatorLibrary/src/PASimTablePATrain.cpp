/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/simulator/pa/PASimulatorLibrary/src/PASimTablePATrain.cpp $
...
#include "app/PA/PASimulatorLibrary/src/PASimTablePATrain.h"
...
PASimTablePATrain::PASimTablePATrain(PASimTableQueryProcessor * parent, LocationType loc)
: PASimTable(PATrain,parent, loc)
, m_tableData(0)
{
    switch (loc)
    {
    ...
    {
        m_tableData = new PASimTablePATrainRecord[m_numberOfRecords];
    }
    else
    {
    ...
    for ( unsigned int i=0 ; i<m_numberOfRecords ; i++ )
    {
        ...
        m_tableData[i].month = 0; //PASimUtility::randomWord(1,12);
        m_tableData[i].day = 0; //PASimUtility::randomWord(1,28);
        m_tableData[i].hour = 0; //PASimUtility::randomWord(0,23);
        m_tableData[i].minutes = 0; //PASimUtility::randomWord(0,59);
        m_tableData[i].seconds = 0; //PASimUtility::randomWord(0,59);
    }
}


PASimTablePATrain::~PASimTablePATrain()
{
    delete [] m_tableData;
}

unsigned short PASimTablePATrain::getTableSize() const
{
...
UserResponse * PASimTablePATrain::processUserRead(UserQuery * qry)
{
    ...
    if (qry->getType() == Show)
    {
        ...
        if (qry->getOutputFormat() == f_tabular)
        {
            ...
            // all these fields are irrelevant until otherwise specified
            PASimUtility::bool2string( (m_tableData[record].trainDataValidity==Relevant), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);
            ...
            PASimUtility::bool2string( (m_tableData[record].holdStatus!=0), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);
            ...
            PASimUtility::bool2string( (m_tableData[record].trainReadinessMode!=0), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);

            PASimUtility::bool2string( (m_tableData[record].stalledStatus!=0), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);

            PASimUtility::bool2string( (m_tableData[record].evacuationStatus!=0), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);

            PASimUtility::bool2string( (m_tableData[record].ebStatus!=0), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);

            // OA1 Data
            PASimUtility::bool2string( (m_tableData[record].oa1DataValidity==Relevant), boolstr, PASimUtility::booleanOutputYesNo );
            sprintf(buffStr, "|                            | %s     |\n", boolstr.c_str() );
            response->m_responseData.append(buffStr);
        ...
        {
            response->m_responseData.append("ERROR: Hex format of PATrain table not supported\n");
        }
...
UserResponse * PASimTablePATrain::processUserWrite(UserQuery * qry)
{
    if (qry->getType() == Set)
    {
        UserResponse * response = new UserResponse(qry);
...
void PASimTablePATrain::toStream(std::ostream& os) const
{
}


void PASimTablePATrain::fromStream(const std::ostream& os)
{
}


void PASimTablePATrain::toNetwork(ByteVector & n)
{
    int idx;
    ...
    {
                                                            //  @Begin
        PA_TABLE_PUT(Word,physicalTrainNumber);            //  0
        PA_TABLE_PUT(Word,atcTrainNumber);                 //  2

        // Train Data
        PA_TABLE_PUT_SPARE(Byte);                          //  4 spare bits    
        PA_TABLE_PUT(Byte,trainDataValidity);              //  5
        PA_TABLE_PUT(Word,serviceNumber);                  //  6
        PA_TABLE_PUT(Word,scheduleNumber);                 //  8
        PA_TABLE_PUT(Word,branchNumber);                   //  10
        PA_TABLE_PUT(Word,fbAbscissa);                     //  12
        PA_TABLE_PUT(Word,headCarAbscissa);                //  14
        PA_TABLE_PUT_SPARE(Byte);                          //  16  spare bits
        PA_TABLE_PUT(Byte,headCabDirection);               //  17
        PA_TABLE_PUT(Byte,talkativeStatus);                //  18
        PA_TABLE_PUT(Byte,holdStatus);                     //  19
        PA_TABLE_PUT(Word,platformAreaId);                 //  20
        PA_TABLE_PUT(Byte,trainReadinessMode);             //  22
        PA_TABLE_PUT(Byte,stalledStatus);                  //  23
        PA_TABLE_PUT(Byte,evacuationStatus);               //  24
        PA_TABLE_PUT(Byte,ebStatus);                       //  25

        // OA1 Data
        // On-board communication
        PA_TABLE_PUT_SPARE(Byte); // spare bits            //  26
        PA_TABLE_PUT(Byte,oa1DataValidity);                //  27
        PA_TABLE_PUT(Byte,tetraMobileStatus);              //  28
        for ( idx=0 ; idx<3 ; idx++ )                       //  29 30 31
        { PA_TABLE_PUT(Byte,pecuStatusCar[idx]); }
//         PA_TABLE_PUT(Byte,pecuStatusCar1);
//         PA_TABLE_PUT(Byte,pecuStatusCar2);
//         PA_TABLE_PUT(Byte,pecuStatusCar3);
        PA_TABLE_PUT(Byte,cctvCameraStatus);               //  32
        PA_TABLE_PUT(Byte,cctvSystemEquipmentStatus);      //  33
        PA_TABLE_PUT_SPARE(Byte); // spare bits            //  34
        PA_TABLE_PUT(Byte,trainPAStatus);                  //  35
        // Fire detection data
        // Car1
        PA_TABLE_PUT(Byte,underSeatFireDetectionCar1);     //  36
        PA_TABLE_PUT(Byte,underFrameFireDetectionCar1);    //  37
        PA_TABLE_PUT(Byte,internalSmokeDetectionCar1);     //  38
        PA_TABLE_PUT(Byte,externalSmokeDetectionCar1);     //  39
        // Car2
        PA_TABLE_PUT(Byte,underSeatFireDetectionCar2);     //  40
        PA_TABLE_PUT(Byte,underFrameFireDetectionCar2);    //  41
        PA_TABLE_PUT(Byte,internalSmokeDetectionCar2);     //  42
        PA_TABLE_PUT(Byte,externalSmokeDetectionCar2);     //  43
        // Car3
        PA_TABLE_PUT(Byte,underSeatFireDetectionCar3);     //  44
        PA_TABLE_PUT(Byte,underFrameFireDetectionCar3);    //  45
        PA_TABLE_PUT(Byte,internalSmokeDetectionCar3);     //  46
        PA_TABLE_PUT(Byte,externalSmokeDetectionCar3);     //  47
        //
        PA_TABLE_PUT_SPARE(Byte); // spare bits            //  48
        PA_TABLE_PUT(Byte,externalTemperature);            //  49
        for ( idx=0 ; idx<3 ; idx++ )
        { PA_TABLE_PUT(Byte,extinguisherStatusCar[idx]); } //  50 51 52
        PA_TABLE_PUT_SPARE(Byte); // spare bits            //  53
        //
        for ( idx=0 ; idx<3 ; idx++ )
        { PA_TABLE_PUT(Word,ehsStatusCar[idx]); }          //  54 56 58
        PA_TABLE_PUT(Byte,detrainmentDoorStatusCar1);      //  60
        PA_TABLE_PUT(Byte,detrainmentDoorStatusCar3);      //  61

        // OA2 Data
        PA_TABLE_PUT_SPARE(Byte); // spare bits            //  62
        PA_TABLE_PUT(Byte,oa2DataValidity);                //  63
        PA_TABLE_PUT(Byte,driverConsoleCoverStatusCar1);   //  64
        PA_TABLE_PUT(Byte,driverConsoleCoverStatusCar3);   //  65

        for ( idx=0 ; idx<3 ; idx++ )                       //  66 68  Car1
        {                                                   //  70 72  Car2
            PA_TABLE_PUT(Word,trainDoorStatus[idx][0]);    //  74 76  Car3
            PA_TABLE_PUT(Word,trainDoorStatus[idx][1]);
        }

        for ( idx=0 ; idx<3 ; idx++ )
        { PA_TABLE_PUT(Byte,emergencyHandleCoverStatusCar[idx]); } //  78 79 80

        for ( idx=0 ; idx<3 ; idx++ )
        { PA_TABLE_PUT(Byte,ttisStatusCar[idx]); }         //  81 82 83

        PA_TABLE_PUT(Word,year);                           //  84
        PA_TABLE_PUT(Word,month);                          //  86
        PA_TABLE_PUT(Word,day);                            //  88
        PA_TABLE_PUT(Word,hour);                           //  90
        PA_TABLE_PUT(Word,minutes);                        //  92
        PA_TABLE_PUT(Word,seconds);                        //  94
    }
}


void PASimTablePATrain::fromNetwork(const ByteVector & s)
{
    throw std::logic_error("PA Train Table should not be put to vector fromNetwork");
}


void PASimTablePATrain::getByteVector(ByteVector & bv) const
{
}

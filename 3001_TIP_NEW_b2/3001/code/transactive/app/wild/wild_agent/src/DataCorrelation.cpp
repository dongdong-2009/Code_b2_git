 ...
 * in any form.
 *
 * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/wild/wild_agent/src/DataCorrelation.cpp $
 * @author:  <Andy Siow>
 * @version: $Revision: #2 $
 *
...
namespace TA_IRS_App
{
	...
	void DataCorrelation::correlateData(const std::string& dataPointValue)
	{
		...
		WILDDetails* wildDetails = extractWILDData(dataPointValue); //This returns NULL if fails
	...
	void DataCorrelation::parseWILDData(const WILDDetails* wildDetails)
	{
		...
		while(currentTrainInfo == NULL && static_cast<unsigned long>(timer.elapsedTime()) < WILDConfiguration::getInstance()->getCorrelationThresholdTimeoutSecs())
		{
		...
		if(currentTrainInfo != NULL) // Matching ATS record
		{
			...
			if(currentTrainInfo->isRevenue && doesTrainHaveAnyWheelAlarms(wheelAxleAlarms))
			{
				...
				LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog,TA_Base_Core::DebugUtil::DebugDebug,
					...
					int wheelNum = 0; 
					std::vector<bool>::iterator  iter= wheelAxleAlarms.begin();
					for ( iter; iter != wheelAxleAlarms.end(); iter ++)
					{ 
						if (*iter)
						{
							LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog,TA_Base_Core::DebugUtil::DebugDebug,
					...
					updateWheelDataPoints(wildDetails, currentTrainInfo);
			if (isCriticalAlarm)
			{
				submitCriticalAlarm(TrainName);
			}
	...
	DataCorrelation::WILDDetails* DataCorrelation::extractWILDData(const std::string& dataPointValue)
	{
		...
		try
		{
			...
			{ 
				unsigned int bit = static_cast<unsigned char>(pow((float)2, (float)(i-1)));

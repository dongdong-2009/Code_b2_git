/**
  ...
  *
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/signs/tis_agent/src/STISCommunicationsHandler.cpp $
  * @author:   Robin Ashcroft
  * @version:  $Revision: #5 $
  *
  * Last modification: $DateTime: 2015/06/15 16:07:51 $
...
namespace TA_IRS_App
{
	...
	void STISCommunicationsHandler::processReceivedData( std::vector<unsigned char>& pData, TA_Base_Core::ISocket* socket)
	{
		...
		if(false == pData.empty())
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Message format is not correct");
			pData.clear();
		}
		FUNCTION_EXIT;
	}
	...
	void STISCommunicationsHandler::sendMessage( const std::vector<unsigned char>& data )
	{
		...
		char* packet = data.empty() ? 0 : (char*) &(data[0]);
	...
	void CCTCommunicationsHandler::connectionEstablished()
	{	
		FUNCTION_ENTRY("connectionEstablished");

		m_connected = true;

		LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Connection established with CCT server" );
	...
	bool CCTCommunicationsHandler::connectionLost()
	{
		FUNCTION_ENTRY("connectionLost");

		m_connected = false;

		LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Lost connection to CCT server" );
	...
	void CCTCommunicationsHandler::sendMessage( const std::vector<unsigned char>& data )
	{
		FUNCTION_ENTRY("sendMessage");

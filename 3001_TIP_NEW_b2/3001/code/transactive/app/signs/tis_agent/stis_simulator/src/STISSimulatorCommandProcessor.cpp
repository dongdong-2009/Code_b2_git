/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/signs/tis_agent/stis_simulator/src/STISSimulatorCommandProcessor.cpp $
...
namespace TA_IRS_App
{
	...
	std::string STISSimulatorCommandProcessor::commands[] =
	{
        "sendPacket",
		"status",
        "updatepid",
	...
	//TD9161 add two commands
	std::string STISSimulatorCommandProcessor::commandUsage[] =
	{
        "sendPacket",
		"status",
		...
		"currentttislibrary <library verison>"
	};
	...
	bool STISSimulatorCommandProcessor::execute(const std::string &name, const std::string &params)
	{
		...
		else if( name == "currentttislibrary")
		{
			m_parentSimulator->currentTtisLibrary( params );
			return true;
		}

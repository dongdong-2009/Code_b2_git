...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/signs/tis_agent/stis_simulator/src/STISSimulator.cpp $
...
namespace TA_IRS_App
{
  ...
  /*      if( 0 != m_commandProcessor )
	...
	bool STISSimulator::newTtisLibrary( const std::string& params )
	{
		...
		for( iter = m_commsObserverVector.begin() ; iter != m_commsObserverVector.end() ; iter++ )
		{
			(*iter)->updateLibraryVersion( "TTIS", strtoul(libraryVersionStr.c_str(), NULL, 10) );
		}

		return true;
	}

	bool STISSimulator::currentTtisLibrary( const std::string& params )
	{
		int tmpIndex = params.find(" ",0);
		std::string libraryVersionStr = params.substr( 0, tmpIndex );
		...
		for( iter = m_commsObserverVector.begin() ; iter != m_commsObserverVector.end() ; iter++ )
		{
			(*iter)->updateCurrentTTISLibraryVersion( strtoul(libraryVersionStr.c_str(), NULL, 10) );
		}
	...
	bool STISSimulator::currentStisLibrary( const std::string& params )
	{
		...
		for( iter = m_commsObserverVector.begin() ; iter != m_commsObserverVector.end() ; iter++ )
		{
			(*iter)->updateCurrentSTISLibraryVersion( strtoul(libraryVersionStr.c_str(), NULL, 10) );
		}

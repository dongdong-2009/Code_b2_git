/**
  ...
  *
  * Source:    $File: //depot/3001_TIP_NEW/3001/transactive/app/signs/ttis_manager/src/DisplayRequestManager.cpp $
  * @author:   Adam Radics
  * @version:  $Revision: #2 $
  *
...
namespace TA_IRS_App
{
                                           ...
                                           const TA_Base_Core::PredefinedMessage& selectedMessage )
    {
        ...
        LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo,
                     ...
                     selectedMessage.message.c_str(), startTime, endTime );
                                          ...
                                          std::string& endTime )
    {
        ...
        // start and end time
        char startTimeString[5];
        char endTimeString[5];
        ...
        if ( timeType == DisplayAttributeManager::MESSAGE_CYCLIC )
        {
            ...
            if ( ( ( startTime.GetHour() == 3 ) && ( startTime.GetMinute() == 0 ) ) ||  // start time is 3am OR
                 ( ( endTime.GetHour() == 3 ) && ( endTime.GetMinute() == 0 ) && ( endTime.GetSecond() > 0 ) ) ||  // end time is 3am OR
                 ( ( startTime.GetHour() < 3 ) && ( ( endTime.GetHour() == 3 ) && ( endTime.GetMinute() > 0 ) ) ) ||  // start time is less than 3am and end time is after 3 am OR
                 ( ( startTime.GetHour() < 3 ) && ( endTime.GetHour() > 3 ) ) ) // start time is before 3am and end time is after 3am
            {

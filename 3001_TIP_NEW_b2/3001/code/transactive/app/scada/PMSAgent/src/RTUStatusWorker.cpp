/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3001_TIP_NEW/3001/transactive/app/scada/PMSAgent/src/RTUStatusWorker.cpp $
  * @author:	HoaVu
  * @version:	$Revision: #3 $
  *
...
namespace TA_IRS_App
{
	...
	RTUStatusWorker::RTUStatusWorker( unsigned long statusOffsetAddress,
	...
	void RTUStatusWorker::processRTUDataBlock ( const TA_Base_Bus::DataBlock< WORD > & addressBlock, const bool isCommsOK )
	{
	...
	void RTUStatusWorker::processRTUStatus( const TA_Base_Bus::DataBlock< WORD > & addressBlock )
	{
	...
	//TD10586
	bool RTUStatusWorker::getIsDownloadRequired() const
	{
	...
	bool RTUStatusWorker::getIsCommsValidAtWord(int index) const
	{
	...
	bool RTUStatusWorker::getIsCommsWithStationSystemsValid ( int systemID ) const
	{
	...
	int RTUStatusWorker::getSWCWeightage() const
	{
        return m_isCommsOK ? m_SWCWeightage : 0;
	}

	bool RTUStatusWorker::getIsCommsWithStatusWordNumber( int wordIndex ) const
	{
		// if station system id equil to rtu's slave id, just use communication status as valid indicator

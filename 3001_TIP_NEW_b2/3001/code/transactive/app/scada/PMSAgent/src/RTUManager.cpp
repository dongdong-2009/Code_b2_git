/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3001_TIP_NEW/3001/transactive/app/scada/PMSAgent/src/RTUManager.cpp $
  * @author:	HoaVu
  * @version:	$Revision: #7 $
  *
  * Last modification:	$DateTime: 2015/11/16 18:40:13 $
...
namespace TA_IRS_App
{
    ...
    RTUManager::~RTUManager()
    {
		...
		{
			RTUDpProcessor* dpProcessor = m_rtuDpProcessors[i];
			dpProcessor->terminate();
			delete dpProcessor;
		}
		m_rtuDpProcessors.clear();
    ...
    TA_Base_Bus::IEntity* RTUManager::createRTUEntity ( TA_Base_Core::IEntityDataPtr EntityData )
    {
        ...
        //TD10586
		{
			TA_Base_Core::ThreadGuard guard(m_rtuAddLock);
	...
	void RTUManager::setOperationMode(TA_Base_Core::EOperationMode mode)
	{
		unsigned long i;
		for (i = 0; i < m_rtus.size(); ++ i)
		{
			m_rtus[i]->setOperationMode(mode);
		}

		for (i = 0; i < m_rtuDpProcessors.size(); ++i)
		{
			m_rtuDpProcessors[i]->setOperationMode(mode);
		}
	...
	void RTUManager::terminate()
	{
		...
		m_rtuHeartBeatWorker.setToMonitor();
	}
    ...
    void RTUManager::processRTUDataBlock(RTU& rtu, const TA_Base_Bus::DataBlock< WORD > & addressBlock)
    {
		...
		// if this not real primaryRTU, just ignored the data
		if(false == rtu.getIsPrimaryRTU() && true == rtu.getIsCommsOK())
		{
			LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "RTU [%s] is not primary rtu, datablock is ingored", 
				rtu.getName().c_str() );
			return;
        ...
        {
            int statusWordNumber = m_rtuDpProcessors[i]->getStatusWordNumber();
              	
            // only process data as good quality when this rtu is primary one and station system is rtu internal system
            if (rtu.getIsCommsWithStationSystemsValid(statusWordNumber))
            {
	...
	std::vector<RTU*> RTUManager::getRTUEntities()
	{
		TA_Base_Core::ThreadGuard guard(m_rtuAddLock);
		std::vector<RTU*> retRtus(m_rtus);
		return retRtus;
	}
}

/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/3001/transactive/app/pa/PAManager/src/PaRightsManager.cpp $
  * @author Sean Liew
  * @version $Revision: #5 $
...
bool PaRightsManager::debugConfirmRightsExist()
{
    try
    {
        ...
        for (int i = 0; i < PA_RIGHT_MAX; i ++)
        {
            ...
            {
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Multiple local rights mapped to same global right" );
				return false;
			}
        }
    }
    ...
    {
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Local right not identified by map" );
		return false;
    }
...
bool PaRightsManager::isBroadcastInProgress(const std::string& broadcastId, PAAgentNamedObject& agentObject) const
{
    try
    {
        ...
        CORBA_CALL_RETURN( progressReport, agentObject, getProgressReport, ( broadcastId.c_str() ) );
...
{
    return CachedConfig::getInstance()->getMftLocKey() == CachedConfig::getInstance()->getAgentLocKey();
}
...
void PaRightsManager::registerRightsChangeListener(IRightsChangeListener& listener)
{
    ...
    m_rightsChangeListeners.insert(&listener);
}
                                                                ...
                                                                PAAgentNamedObject& agentObject) const
{
    try
    {
        TA_Base_Bus::IPAAgentCorbaDef::BroadcastConfig_var data;
        CORBA_CALL_RETURN( data, agentObject, getBroadcastConfig, ( broadcastId.c_str() ) );

/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/3001/transactive/app/pa/PAManager/src/HMISelectionListener.cpp $
  * @author Jade Lee
  * @version $Revision: #8 $
...
void HmiSelectionListener::processRunParamChange(const std::string& name, const std::string& value)
{
    ...
    // Parameter name is case matched
    if(RunParamListener::SELECT_TAG.compare( name ) == 0)
    {
        selectionUpdated |= onZoneSelectionChange(value, SELECT);
    }
    else if(RunParamListener::DESELECT_TAG.compare( name ) == 0)
    {
        selectionUpdated |= onZoneSelectionChange(value, DESELECT);
    }
    else if (RunParamListener::UPDATE_SELECTION_TAG.compare( name ) == 0)
    {
...
void HmiSelectionListener::refreshSelectionUsingZones(const std::set<std::string>& zoneNames, EOperation operation)
{
    ...
    bool bAtOcc = CachedConfig::getInstance()->isSchematicAtOcc();

/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/3001/transactive/app/pa/PAManager/src/DvaStatusPage.cpp $
  * @author Jade Lee
  * @version $Revision: #6 $
...
bool DvaStatusPage::tryUpdateStatusNameOnClose()
{
    ...
    if (askUserIfWantToForceClose)
    {
        ...
        if (IDYES == userMsg.showMsgBox(IDS_UW_610008))
        {
...
bool DvaStatusPage::displayDuplicateNameOnCloseWindowWarning()
{
    ...
    if (IDCANCEL == warningDialog.DoModal())
    {
...
void DvaStatusPage::refreshCurrentStatusName()
{
	...
	{
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "getStatusModelPointer NULL" );
        TA_THROW(TA_Base_Core::TransactiveException("getStatusModelPointer NULL"));
	}

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/app/incidents/incident_manager/src/StationReportDialog.cpp $
  * @author:  Grace Koh
  * @version: $Revision: #7 $
  *
  * Last modification: $DateTime: 2014/07/01 18:24:31 $
...
namespace TA_IRS_App
{
	...
	void StationReportDialog::onSave() 
	{
		...
		else
		{
			...
			catch(TA_Base_Core::DataConfigurationException &e)
			{					
				LOG( SourceInfo, DebugUtil::ExceptionCatch, "DataConfigurationException","");
				CString actionName, reason;
				actionName = "save";
				reason = e.what();
				std::vector<std::string> missing = e.getMissingFields();
	...
	long StationReportDialog::OnRefreshDisplay( WPARAM wParam, LPARAM lParam )
	{
		...
		//Shiva++
		//DTL-1174 - 7 digit number display
		//(GetDlgItem(IDC_EDIT_ID))->SetWindowText(m_report.getDisplayReportId().c_str());
	...
	void StationReportDialog::enableTimeControls(bool bEnable)
	{
		...
		if ( bEnable )
		{
			...
			{ 
				...
				m_DatePickerControl.GetRange(&startDate,&endDate);
				m_DatePickerControl.SetTime(&endDate);
				m_DateTime = endDate;			
			}

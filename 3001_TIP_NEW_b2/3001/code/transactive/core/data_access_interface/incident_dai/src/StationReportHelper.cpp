/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/core/data_access_interface/incident_dai/src/StationReportHelper.cpp $
  * @author:  Grace Koh
  * @version: $Revision: #6 $
  *
  * Last modification: $DateTime: 2014/06/23 18:43:33 $
...
namespace TA_IRS_Core
{
    ...
    void StationReportHelper::writeStationReportData()
	{
		...
		if ( !fieldNames.empty() ) // Need to throw an exception
		{
			...
			TA_THROW(TA_Base_Core::DataConfigurationException("StationReport data not fully specified. Data cannot be written to database. Missing data =",fieldNames));
		}
	...
	void StationReportHelper::reload()
	{
		...
		sql << "select " 
			...
			<< IN_STATION_ID_COL << " = '" 
			<< m_reportId << "'";*/
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(),IN_STATION_REPORT_Oracle_SELECT_54051,m_reportId);
//		std::string strSql  = databaseConnection->prepareSQLStatement(IN_STATION_REPORT_SELECT_54051,m_reportId);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, IN_STATION_REPORT_SELECT_54051,m_reportId);
	...
	/*	std::ostringstream sql;    
		...
		sql << "');";
		sql << "END;";*/
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), IN_STATION_REPORT_Oracle_DELETE_54451, m_reportId);
//		std::string strSql  = databaseConnection->prepareSQLStatement(IN_STATION_REPORT_DELETE_54451, getLocalDatabaseName(), m_reportId);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, IN_STATION_REPORT_DELETE_54451, getLocalDatabaseName(), m_reportId);
	...
	//Bug 652 (TD12313)
		...
		sql << "');";
		sql << "END;";*/
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), IN_STATION_REPORT_Oracle_INSERT_54351,getLocalDatabaseName(),
//		std::string strSql  = databaseConnection->prepareSQLStatement(IN_STATION_REPORT_INSERT_54351,getLocalDatabaseName(),
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, IN_STATION_REPORT_INSERT_54351,getLocalDatabaseName(),
			m_reportId,databaseConnection->escapeAQSQLString(m_submittedBy),databaseConnection->escapeAQSQLString(m_ep),incidentDateString,
			databaseConnection->escapeAQSQLString(type),databaseConnection->escapeAQSQLString(m_type.other),databaseConnection->escapeAQSQLString(location),
			databaseConnection->escapeAQSQLString(m_location.other),databaseConnection->escapeAQSQLString(cause),databaseConnection->escapeAQSQLString(m_cause.other),
			databaseConnection->escapeAQSQLString(condition),databaseConnection->escapeAQSQLString(m_condition.other),databaseConnection->escapeAQSQLString(m_action),
			databaseConnection->escapeAQSQLString(m_staff),databaseConnection->escapeAQSQLString(m_description),m_createLocationId,m_creatorProfile);
...
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), IN_STATION_REPORT_Oracle_UPDATE_54201,getLocalDatabaseName(),
//		std::string strSql  = databaseConnection->prepareSQLStatement(IN_STATION_REPORT_UPDATE_54201,getLocalDatabaseName(),

		//
		TA_Base_Core::SQLTypeAdapter sqlParamCloseDate;
		getUpdateDateSql(sqlParamCloseDate, m_incidentClosedDate, true);

		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, IN_STATION_REPORT_UPDATE_54201,getLocalDatabaseName(),
			databaseConnection->escapeAQSQLString(m_submittedBy),databaseConnection->escapeAQSQLString(m_ep),convertIncidentDate2Tmstruct(),
			databaseConnection->escapeAQSQLString(convertType2BitString()),databaseConnection->escapeAQSQLString(m_type.other),
			databaseConnection->escapeAQSQLString(convertLocation2BitString()),databaseConnection->escapeAQSQLString(m_location.other),
			databaseConnection->escapeAQSQLString(convertCause2BitString()),databaseConnection->escapeAQSQLString(m_cause.other),
			databaseConnection->escapeAQSQLString(convertCondition2BitString()),databaseConnection->escapeAQSQLString(m_condition.other),
			databaseConnection->escapeAQSQLString(m_action),databaseConnection->escapeAQSQLString(m_staff),
			databaseConnection->escapeAQSQLString(m_description),sqlParamCloseDate,m_reportId);
	...
	void StationReportHelper::getUpdateDateSql(TA_Base_Core::SQLTypeAdapter& sqlParam, time_t datetime, bool bIsAQ)
	{
		...
		sqlParam.add(TA_Base_Core::enumDateFormat_3, TA_Base_Core::enumFuncFormat_2, oStream.str(), bIsAQ);
		FUNCTION_EXIT;		
	}

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_TIP_NEW/3001/transactive/core/data_access_interface/ecs/src/ECSMasterModeHelper.cpp $
  * @author:  Nick Jardine
  * @version: $Revision: #8 $
  *
...
namespace TA_IRS_Core
{
    ...
    void ECSMasterModeHelper::invalidate()
    {
		...
		if (m_isNew)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Attempted to call invalidate() on a new ECSMasterMode" );
			return;
		}
    ...
    void ECSMasterModeHelper::reload()
    {
		...
		if (0 == m_key)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "This ECSMasterMode does not yet exist in the database. Therefore it cannot be reloaded." );
			return;
		}
  ...
  //          << KEY_COL << " = " << m_key;
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), EC_MASTER_MODE_Oracle_SELECT_50553, m_key);
//		std::string strSql  = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_SELECT_50553, m_key);
    ...
    {       
        ...
        {
            TA_THROW(TA_Base_Core::DataConfigurationException("ECSMasterMode data not fully specified. ECSMasterMode cannot be written to database", fieldNames));
        }
    ...
    void ECSMasterModeHelper::deleteECSMasterMode(bool cascade)
    {
		...
		if (m_isNew)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "This ECSMasterMode does not yet exist in the database, and therefore cannot be deleted" );
			return;
		}
        ...
        if (!cascade)
        {
           ...
           /* sql.str("");
			...
			//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_HELPER_STD_SELECT_51001, m_key);
//			strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_HELPER_SELECT_51001, m_key);
		...
		databaseConnection->prepareSQLStatement(strSql, EC_MASTER_MODE_HELPER_SELECT_51001, m_key);
            ...
            if (0 != data->getNumRows())
            {
                ...
                TA_THROW(TA_Base_Core::DataException("A reference to this EC_MASTER_MODE exists in the EC_MASTER_MODE_HELPER table", TA_Base_Core::DataException::CANNOT_DELETE,"EC_MASTER_MODE_HELPER"));
            }
       ...
       /* sql.str("");
		...
		//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_STD_DELETE_50901, m_key);
//		strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_DELETE_50901, m_key);
    ...
    void ECSMasterModeHelper::updateStationModeAssociations()
    {
       ...
       /* std::ostringstream sql;
        sql << "select " << STATIONMODEKEY_COL 
            ...
            << MASTERMODEKEY_COL << " = " << m_key;*/
		//std::string strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_HELPER_STD_SELECT_51002, m_key);
//		std::string strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_HELPER_SELECT_51002, m_key);
		TA_Base_Core::SQLStatement strSql;
        ...
        if (!databaseStationModesToRemove.empty())
        {
         ...
         /*   sql << "delete EC_MASTER_MODE_HELPER where " 
           ...
           /* sql << ") and "
			...
			//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_HELPER_STD_DELETE_51401,sql.str(), m_key);
//			strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_HELPER_DELETE_51401, sql.str(), m_key);
        ...
        if (!localStationModesToAdd.empty())
        {
                 ...
                 iterToAdd != localStationModesToAdd.end(); ++iterToAdd)
            {
               ...
               /* std::ostringstream sql;
				...
				//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_HELPER_Oracle_INSERT_51351, m_key, *iterToAdd);
//				strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_HELPER_SELECT_51351, m_key, *iterToAdd);
    ...
    void ECSMasterModeHelper::modifyExistingECSMasterMode() throw(TA_Base_Core::TransactiveException)
    {
		...
		/*std::string strSql = defPrepareSQLStatement(nDbType,EC_MASTER_MODE_Oracle_UPDATE_50701,m_masterMode,
			...
			databaseConnection->escapeInsertString(m_dispDir),databaseConnection->escapeInsertString(m_verificationMsg), m_key);*/
//		std::string strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_UPDATE_50701, m_masterMode,
		TA_Base_Core::SQLStatement strSql;
       ...
       /* sql << "select TO_CHAR(DATE_CREATED,'YYYYMMDDHH24MISS'), "
		...
		//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_Oracle_SELECT_50554, m_key);
//		strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_SELECT_50554, m_key);
    ...
    void ECSMasterModeHelper::addNewECSMasterMode() throw(TA_Base_Core::TransactiveException)
    {
       ...
       /* sql << "','"
		...
		/*std::string strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_Oracle_INSERT_50851, m_masterMode,databaseConnection->escapeInsertString(m_condition),
			sql.str(),databaseConnection->escapeInsertString(m_evacDir),databaseConnection->escapeInsertString(m_airDir),databaseConnection->escapeInsertString(m_dispDir),
			databaseConnection->escapeInsertString(m_verificationMsg));*/
//		std::string strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_INSERT_50851, m_masterMode,databaseConnection->escapeInsertString(m_condition),
		TA_Base_Core::SQLStatement strSql;
       ...
       /* sql.str("");
        sql << "select " 
            ...
            << "from EC_MASTER_MODE where "
            << MASTERMODE_COL << " = " << m_masterMode;*/
		//strSql = defPrepareSQLStatement(nDbType, EC_MASTER_MODE_Oracle_SELECT_50555, m_masterMode);
//		strSql = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_SELECT_50555, m_masterMode);
...
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(),EC_MASTER_MODE_STD_SELECT_50503, m_masterMode, m_key);
//		std::string strSql  = databaseConnection->prepareSQLStatement(EC_MASTER_MODE_SELECT_50503, m_masterMode, m_key);
		TA_Base_Core::SQLStatement strSql;
    ...
    void ECSMasterModeHelper::validateZoneIdExistence() throw(TA_Base_Core::TransactiveException)
    {
        ...
        TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Ecs_Cd, TA_Base_Core::Read);
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), EC_ZONE_STD_SELECT_51501,m_ECSZoneId);
//		std::string strSql  = databaseConnection->prepareSQLStatement(EC_ZONE_SELECT_51501,m_ECSZoneId);
		TA_Base_Core::SQLStatement strSql;
    ...
    void ECSMasterModeHelper::validateStationModeKeyExistence(unsigned long stationModeKey) throw(TA_Base_Core::TransactiveException)
    {
       ...
       /* std::ostringstream sql;
        ...
        TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Ecs_Cd, TA_Base_Core::Read);
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), EC_STATION_MODE_STD_SELECT_52001, stationModeKey);
//		std::string strSql  = databaseConnection->prepareSQLStatement(EC_STATION_MODE_SELECT_52001, stationModeKey);
		TA_Base_Core::SQLStatement strSql;

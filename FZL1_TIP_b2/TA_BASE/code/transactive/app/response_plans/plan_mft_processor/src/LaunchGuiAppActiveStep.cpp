/**
  ...
  *
  * Source:   $File: //depot/TA_Common_V1_TIP/transactive/app/response_plans/plan_mft_processor/src/LaunchGuiAppActiveStep.cpp $
  * @author:  Huang Jian
  * @version: $Revision: #4 $
  *
  * Last modification: $DateTime: 2018/03/01 10:57:13 $
...
void LaunchGuiAppActiveStep::launch()
{
    ...
    try
    {
		...
		// Move the x position from maximum x position to manageable position
		int resGap = 40;
		if (parameters.pos.left > appBounds.right - resGap)
		{
			appBounds.left += appBounds.right - resGap;
		}
		else
		{
            ...
            parameters.pos.height != static_cast<unsigned long>(-1))
        {
			...
			long bottom = appBounds.top + parameters.pos.height;
			if (bottom <= appBounds.bottom )
			{
				appBounds.bottom = bottom;
			}
        }
        else
        {

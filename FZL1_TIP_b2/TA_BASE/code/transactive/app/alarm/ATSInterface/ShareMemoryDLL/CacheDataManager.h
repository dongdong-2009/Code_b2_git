...
namespace TA_Base_Bus
{
	...
	public:	
		...
		{
		  initProcessLock();
		  TA_Base_Bus::ProcessGuard wGuard(*m_RW_Process_Mutex);
		  openOrCreateMem();
		 
		}
	    virtual void processEvent(CacheDataList&  batch)
		{
			TA_Base_Bus::ProcessGuard guard(*m_RW_Process_Mutex);
 			updateData (batch);
		}        
	...
	private:
		IProcessLockable*    m_RW_Process_Mutex;
	...
	void	CacheDataManager<CacheDataKeyType, CacheDataT>::initProcessLock()
	{
		...
		{			
			...
			{
				m_RW_Process_Mutex=(IProcessLockable*) (new NonReEntrantProcessLockable(m_RW_Process_Mutex_Name.c_str()));
			}
		}		 
		catch(...)
		{

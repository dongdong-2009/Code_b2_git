...
//		AlarmCache::getInstance()->setCallbackAgent(this);
    ...
    void AlarmAgent::getSession()
    {
		...
		if (m_authLib==NULL) m_authLib=new TA_Base_Bus::AuthenticationLibrary();
        ...
        while(!resolvedAuthenticationAgent)
        {
            try
            {
				...
				//change m_authLib to class member, due to the requestsession will be terminated after 30 minutes and cannot trigger plan
                std::string session = m_authLib->requestSession(

...
namespace TA_Base_App
{
	...
	void DutyManagerBridge::dataChanged(const TA_Base_Bus::DataUpdate& dataUpdate)
	{
		...
		if (actionMap.empty())
		{
			actionMap.insert(std::make_pair(TA_Base_Core::Create, std::string("增加")));
			actionMap.insert(std::make_pair(TA_Base_Core::Update, std::string("修改")));
			actionMap.insert(std::make_pair(TA_Base_Core::Delete, std::string("删除")));
		}
		
		if (itemMap.empty())
		{
			itemMap.insert(std::make_pair(TA_Base_Core::OPERATOR, std::string("操作员")));
			itemMap.insert(std::make_pair(TA_Base_Core::PROFILE, std::string("角色")));
			itemMap.insert(std::make_pair(TA_Base_Core::ACTION_GROUP, std::string("行为组")));
			itemMap.insert(std::make_pair(TA_Base_Core::LOCATION, std::string("位置")));
			itemMap.insert(std::make_pair(TA_Base_Core::REGION, std::string("区域")));
			itemMap.insert(std::make_pair(TA_Base_Core::SUBSYSTEM, std::string("子系统")));
		}

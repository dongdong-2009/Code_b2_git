...
namespace TA_Base_App
{
	...
	//1.0.0.4 add input/output datapoint information compare with 1.0.0.3
	const std::string OPCDataCacheFile::OPC_DATA_CACHE_VERSION	 = "1.0.0.4";
	...
	void DataPointSaveData::convertToMe(const DataPointCacheData& obj)
	{
		...
		agentKey  			= obj.agentKey;
		locationKey 		= obj.locationKey;
		isOutputDataPoint 	= obj.isOutputDataPoint;
	...
	bool OPCDataCacheFile::readDataPoint(std::ifstream& fs, DataPointCacheDataList& dpList)
	{
		...
		while (i < listSize)
		{
			...
			while(j < readNum)
			{
				dpList.push_back(DataPointCacheData(pObj[j].entityKey,
													pObj[j].agentKey,
													pObj[j].locationKey,
													pObj[j].isOutputDataPoint,

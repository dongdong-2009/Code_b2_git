/**
  ...
  * any form.
  *
  * Source:		$File: //depot/FLZ1_TIP/TA_BASE/transactive/app/scada/PEnergyAgent/src/PEnergyAgent.cpp $
  * @author:	Liu Yong
  * @version:	$Revision: #1 $
  *
  ...
  * Description:
  *
  *	PEnergyAgent implementes the GenericAgent interface.  PEnergyAgent monitors the
  * the entities associated with its process, such as the DataPoint entities.
  *
  */
#include <sstream>
...
namespace TA_Base_App
{
	...
	PEnergyAgent::PEnergyAgent (int argc, char* argv[])
	:
	m_PEnergyAgentPrimaryKey (0),
	...
	{
		FUNCTION_ENTRY("PEnergyAgent::PEnergyAgent");
		m_datapoints.clear();
		m_dataNodes.clear();
        ...
        // force datapoints to load config data which will be used used by createPEnergyWorkers
        ...
        createPEnergyWorkers();
		FUNCTION_EXIT;//("PEnergyAgent::PEnergyAgent");
	}


	PEnergyAgent::~PEnergyAgent()
	{
		FUNCTION_ENTRY("PEnergyAgent::~PEnergyAgent");
	...
	void PEnergyAgent::loadAgentParamValues()
	{
		FUNCTION_ENTRY("loadAgentParamValues");
		// get configuration data for this PEnergyAgent
		TA_Base_Core::IEntityDataPtr entityData = m_genericAgent->getAgentEntityConfiguration();

		// dynamic cast the entity data to PEnergyAgentEntityData
		TA_Base_Core::PEnergyAgentEntityDataPtr pEnergyAgentEntityData;
		pEnergyAgentEntityData = 
			boost::dynamic_pointer_cast<TA_Base_Core::PEnergyAgentEntityData>(entityData);

		// get the primary key of this PEnergyAgent
		m_PEnergyAgentPrimaryKey = pEnergyAgentEntityData->getKey();

		// get the agent asset name
		m_agentAssetName = pEnergyAgentEntityData->getAssetName();

		// get indication if the PEnergyUnit switching functionality is disabled
        //m_isRTUSwitchingDisabled = PEnergyAgentEntityData->getIsRTUSwitchingDisabled();
        //m_analogueDpUpdateInterval = PEnergyAgentEntityData->getAnalogueDataPointUpdateInterval();

		// get the configured station name
		m_stationName = pEnergyAgentEntityData->getStationName();
		...
		//m_proxyDpMonitoringPeriodSecs = pEnergyAgentEntityData->getProxyDpHealthMonitoringPeriodSecs();
	...
	void PEnergyAgent::startProcessors()
	{
	...
	void PEnergyAgent::stopProcessors()
	{
		FUNCTION_ENTRY("stopProcessors");

		m_PEnergyManager.stopProcess();
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "stop--finish to stop PEnergyManager");
	...
	void PEnergyAgent::createPEnergyWorkers()
	{
		FUNCTION_ENTRY("createPEnergyWorkers");
		// if does not have the agent's pkey
		if (0 == m_PEnergyAgentPrimaryKey)
		{
			loadAgentParamValues();
            
            m_PEnergyManager.setAgentAssetName(m_agentAssetName);
            m_PEnergyManager.setStationName(m_stationName);
        }

        // create PEnergyUnit workers
        //m_PEnergyManager.createPEnergyWorkers();

        // add datapoint to PEnergy manager except virtual datapoint
        for (std::map< unsigned long, TA_Base_Bus::DataPoint * >::iterator itrDp = m_datapoints.begin(); itrDp != m_datapoints.end(); ++itrDp)
        {
            ...
            {
                m_PEnergyManager.addDataPoint(itrDp->second);
            }
        }
		FUNCTION_EXIT;//("createPEnergyWorkers");
    }

	TA_Base_Bus::IEntity* PEnergyAgent::createEntity(TA_Base_Core::IEntityDataPtr EntityData)
	{
		//
		...
		if (TA_Base_Core::PEnergyUnitEntityData::getStaticType() == entityDataType)
		{
			// create and return the PEnergyUnit entity
			return m_PEnergyManager.createWorkers(EntityData);
		}
	...
	bool PEnergyAgent::createAllEntities( TA_Base_Core::IEntityDataPtr agentEntityData,
	...
	void PEnergyAgent::entityCreationComplete()
	{
	...
	void PEnergyAgent::agentTerminate()
	{
	...
	void PEnergyAgent::agentSetMonitor()
	{
		FUNCTION_ENTRY("agentSetMonitor");
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "PEnergyAgent is being set to monitor...");
        ...
        // set PEnergy manager to monitor mode
        m_PEnergyManager.setToMonitor();
        
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "PEnergyManager is set to monitor.");
		...
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "PEnergyAgent for Station %s has been started up in MONITOR mode",
            m_stationName.c_str());
		FUNCTION_EXIT;//("agentSetMonitor");
	}

    void PEnergyAgent::agentSetControl()
    {
		FUNCTION_ENTRY("agentSetControl");
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "PEnergyAgent is being set to control...");
        ...
        m_PEnergyManager.setToControl();
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "PEnergyManager is set to control.");

        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "PEnergyAgent for Station %s has been started up in CONTROL mode",
            m_stationName.c_str());
		FUNCTION_EXIT;//("agentSetControl");
    }
    

    void PEnergyAgent::processOnlineUpdate(const TA_Base_Core::ConfigUpdateDetails& updateEvent)
	{
		FUNCTION_ENTRY("processOnlineUpdate");
		unsigned long updateEventPKey = updateEvent.getKey();

		// if the update event is for PEnergyAgent
		if (m_PEnergyAgentPrimaryKey == updateEventPKey)
		{
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Received config update for PEnergyAgent...");

			// update PEnergyAgent configuration
			updatePEnergyAgentConfiguration(updateEvent);
			return;
		}

		// if the update event is for PEnergyUnit
		if (m_PEnergyManager.updatePEnergyUnitConfiguration(updateEvent))
		{
	...
	void PEnergyAgent::startPEnergyAgent()
	{
		FUNCTION_ENTRY("startPEnergyAgent");
		...
		FUNCTION_EXIT;//("startPEnergyAgent");
	}


	void PEnergyAgent::submitAuditMessage (const TA_Base_Core::DescriptionParameters & description,
										const TA_Base_Core::MessageType & messageType)
	{
		...
		try
		{
			...
			m_auditMessageSender->sendAuditMessage(messageType,				// message type
												   m_PEnergyAgentPrimaryKey,		// entity key
	...
	void PEnergyAgent::registerForControlMessages()
	{
	...
	void PEnergyAgent::registerForMonitorMessages()
	{
	...
	void PEnergyAgent::updatePEnergyAgentConfiguration (const TA_Base_Core::ConfigUpdateDetails & updateEvent)
	{
		FUNCTION_ENTRY("updatePEnergyAgentConfiguration");
		std::string msg ("");

		switch (updateEvent.getModifications())
		{
			// only if the PEnergyAgent has been updated in the database
			case TA_Base_Core::Update:
			{
				...
				{
					// get configuration data for this PEnergy Agent
					TA_Base_Core::IEntityDataPtr entityData = m_genericAgent->getAgentEntityConfiguration();

					// dynamic cast the entity data to PMSAgentEntityData
					TA_Base_Core::PMSAgentEntityDataPtr PEnergyAgentEntityData;
					PEnergyAgentEntityData = 
					...
					PEnergyAgentEntityData->invalidate();
		...
		FUNCTION_EXIT;//("updatePEnergyAgentConfiguration");
	}


    void PEnergyAgent::updateDataPointConfiguration (const TA_Base_Core::ConfigUpdateDetails & updateEvent)
	{
		...
		if (dpIter != m_datapoints.end())
		{
            ...
            case TA_Base_Core::Delete:
                m_PEnergyManager.removeDataPoint(dp);
                break;
                
            case TA_Base_Core::Create:
                m_PEnergyManager.addDataPoint(dp);
                break;
			...
			for (unsigned int i = 0; i < configChanges->size(); i++)
			{
				...
				{
					//
					// update the PEnergyUnit's with the data point whose update priority
					// has been changed
					//
					m_PEnergyManager.removeDataPoint (dp);
					m_PEnergyManager.addDataPoint (dp);
	...
	void PEnergyAgent::notifyGroupOffline(const std::string& group)
	{
		// nothing to do at this stage
	...
	void PEnergyAgent::notifyGroupOnline(const std::string& group)
	{
		// nothing to do at this stage
	...
	TA_Base_Bus::IEntity* PEnergyAgent::createDataPointEntity (TA_Base_Core::IEntityDataPtr EntityData)
	{
        ...
        TA_ASSERT(NULL != dataPointEntityData.get(), "dataPointEntityData is NULL in PEnergyAgent::createDataPointEntity");
	...
	TA_Base_Bus::DataPoint * PEnergyAgent::getDataPoint (unsigned long entityKey)
	{
    ...
    void PEnergyAgent::getAllDataPoints (std::map < unsigned long, TA_Base_Bus::DataPoint *>& dataPointList )
	{
    ...
    TA_Base_Bus::DataNode * PEnergyAgent::getDataNode( unsigned long entityKey )
	{
	...
	void PEnergyAgent::getAllDataNodes (std::map < unsigned long, TA_Base_Bus::DataNode *>& dataNodeList )
    {
	...
	TA_Base_Bus::DataPointFactory* PEnergyAgent::getDataPointFactory()
	{
    ...
    void PEnergyAgent::registerForStateUpdates()
    {
		FUNCTION_ENTRY("registerForStateUpdates");
		FUNCTION_EXIT;//("registerForStateUpdates");
   }
	
    void PEnergyAgent::receiveSpecialisedMessage(const TA_Base_Core::StateUpdateMessageCorbaDef& message)
    {

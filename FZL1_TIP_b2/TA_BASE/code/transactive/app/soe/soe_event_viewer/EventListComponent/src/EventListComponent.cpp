 ...
 * in any form.
 *
 * Source:   $File: //depot/3002_TIP/TA_BASE/transactive/app/soe/event_viewer/EventListComponent/src/EventListComponent.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #4 $
 *
 * Last modification: $DateTime: 2009/12/04 13:52:44 $
...
#include "app/soe/soe_event_viewer/EventListComponent/src/ActionThread.h"
...
: m_filter(NULL), m_eventLoader(NULL), m_refreshPeriodMs(0), m_elapsedTimeMs(0), m_currentSortColumn(-1),
...
EventListComponent::~EventListComponent()
{
    if (m_eventLoader != NULL)
    {
        m_eventLoader->terminateAndWait();
        delete m_eventLoader;
        m_eventLoader = NULL;
    }
...
void EventListComponent::setFilter(Filter* filter)
{
    ...
    // Create the loader thread if it hasn't been already
    if (m_eventLoader == NULL)
    {
        m_eventLoader = new EventLoader(m_filter);
        m_eventLoader->start();
    ...
    {
        m_eventLoader->changeFilter(m_filter);
    }
...
void EventListComponent::OnGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    if (pItem->mask & LVIF_TEXT) //valid text buffer?
    {
		...
		// isprint() in replaceNonPrintableCharactersWithSpace() cannot accept chinese characters, e.g. negative value
		//text = replaceNonPrintableCharactersWithSpace(m_filter->getItemData(iItemIndx,headerName));
		//std::auto_ptr<char> pstr = Encoder::UTF8ToAnsi( text.c_str() );
		//text = pstr.get();

// 		if( strcmp( headerName, "值") == 0 )
// 		{
			// translate certain values to chinese
// 			if( text.compare("ALARM") == 0 )
// 			{
// 				text = "警报";
// 			}
// 			else if( text.compare("NORMAL") == 0 )
// 			{
// 				text = "正常";
// 			}
// 			else if( text.compare("LOCKED") == 0 )
// 			{
// 				text = "被锁住";
// 			}
// 			else if( text.compare("RELEASED") == 0 )
// 			{
// 				text = "已释放";
// 			}
// 			else if( text.compare("HIGH") == 0 )
// 			{
// 				text = "高";
// 			}
// 			else if( text.compare("SUCCESSFUL") == 0 )
// 			{
// 				text = "成功";
// 			}				
...
void EventListComponent::loadEvents(bool enableRefresh)
{
    m_eventLoader->setLoad(enableRefresh);
}

void EventListComponent::reloadEvents()
{
	m_eventLoader->reLoad();
}

void EventListComponent::refreshEvents()
{   
    m_eventLoader->setRefresh();
}

void EventListComponent::setRefreshPeriod(const unsigned long refreshTimeMs)
{
    m_eventLoader->setRefreshPeriod(refreshTimeMs);
}
...
void EventListComponent::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// stop user from sorting while loading
	if( dynamic_cast<CombinedEventFilter*>(m_filter)->isLoading() )
	{
		return;
	}
...
void EventListComponent::shutdownList()
{
    if (m_eventLoader != NULL)
    {
        // AL : 6/2/04
        // Added for TD1308: Interrupt the current load midway so shutdown
        // can be executed immediately.
        if(m_eventLoader->isCurrentlyLoading())
        {
            m_eventLoader->stopCurrentLoad();
        }

        while (m_eventLoader->isCurrentlyActive())
        {
            // pump message que
	        MSG msg;
...
void EventListComponent::print()
{
	...
	CString header = "事件查看器 : " + CTime::GetCurrentTime().Format(dateFormatStr.c_str());
	// ++ TD12474
...
void EventListComponent::export(const CString& exportLocation)
{
...
bool EventListComponent::isActive()
{
    if (NULL != m_eventLoader)
    {
        return m_eventLoader->isCurrentlyActive();
    }
    else
    {
        return false;
...
{
    // Disable refreshing - a refresh during sorting could be bad.
    unsigned long period = m_eventLoader->setRefreshPeriod(0);

    // Check to se that a refresh isn't currently underway either.
    if ( m_eventLoader->isCurrentlyLoading() )
    {
        TA_Base_Bus::TransActiveMessage userMsg;
        UINT selectedButton = userMsg.showMsgBox(IDS_UI_040019);
        m_eventLoader->setRefreshPeriod(period);
        return;
    }
	...
	// sort
	//li Zhongyan make change
	if( m_filter->getCurrentPage() )
	{
		DisplayItem::setSortInfo( columnName, m_sortAscending );
		if( m_currentSortColumn == columnId || ( m_currentSortColumn == -1 && columnId == 0 ))
		{
			m_filter->reverse(columnName, m_sortAscending );
		}
		else{
			dynamic_cast<CombinedEventFilter*>(m_filter)->initPageMap();
	...
	else
	{
		...
		else
		{
			...
			else{ //If it use the first column sort, it will reload
				dynamic_cast<CombinedEventFilter*>(m_filter)->initPageMap();
				//m_eventLoader->setLoad(false);
			}
		}
		//dynamic_cast<CombinedEventFilter*>(m_filter)->adjustNextPageTime();
	}

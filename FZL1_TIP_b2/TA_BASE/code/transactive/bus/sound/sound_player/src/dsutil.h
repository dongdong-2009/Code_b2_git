/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/TA_Common_V1_TIP/transactive/bus/sound/sound_player/src/dsutil.h $
  * @author:  Ripple
  * @version: $Revision: #2 $
  *
  * Last modification: $DateTime: 2015/03/16 10:55:01 $
...
namespace TA_Base_Bus
{
		...
		//-----------------------------------------------------------------------------
		// container type for device name and its corresponding GUID
		//-----------------------------------------------------------------------------
		typedef std::map<std::string, LPGUID> T_DEVICE_GUID;
        ...
        private:
            ...
            // True if initialisation failed, and false otherwise. This is used
            // to determine if a message box should be displayed to the user (which
            // should only happen the first time an initialisation error occurs).
            bool m_initFailed;
			T_DEVICE_GUID m_objDeviceList;
		};

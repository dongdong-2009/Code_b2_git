	...
	MessageDetail* XmlUtility::xmlToDetail(const std::string& xmlData)
	{
		...
		if (pNode->hasAttributes()) // should always be true
		{
			...
			for(unsigned int i = 0; i < attributes->length(); i++)
			{
				...
				{
					pRetVal->topicName = attribute->nodeValue();
				}
		...
		while (pNode)
		{
			...
			boost::shared_ptr<Attribute> pChild( new Attribute);
			pChild->name = pNode->nodeName();
			pRetVal->attribValue.insert(T_AttribValueMap::value_type(pChild->name, pChild));

			NodeList* childnodes=pNode->childNodes();
			if (!pNode->hasChildNodes())
			{
				pChild->value = pNode->nodeValue();
			}
			else if (childnodes->length() == 1 && pNode->firstChild()->nodeType() != Poco::XML::Node::ELEMENT_NODE)
			{

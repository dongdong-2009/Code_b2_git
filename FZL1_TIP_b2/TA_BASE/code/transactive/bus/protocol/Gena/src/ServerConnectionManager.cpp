...
namespace TA_Base_Bus
{
	...
	*/
	ServerConnectionManager::ServerConnectionManager(const std::vector< GenaClientSourcePtr >& sourceServers, GENAConnectionFactory* pConnectionMgr)
	{
		...
		for (itr; itr!=sourceServers.end(); itr++)
		{
			...
			LOG4(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Creating Connection for URL: %s, TimeOut: %d, MaxRetry: %d, DpAddress %s", tempConfig->serverURL.c_str(), tempConfig->timeout, tempConfig->maxRetry, tempConfig->dpAddress.c_str());
			objServer.pConnection = pConnectionMgr->createConnection(tempConfig->serverURL, tempConfig->timeout, tempConfig->maxRetry, tempConfig->dpAddress);
			objServer.state = GENAServer_Unknown;
			objServer.dpAddress = tempConfig->dpAddress;
	...
	*/
	...
	bool ServerConnectionManager::isActiveServerConnected()
	{
		TA_Base_Core::ThreadGuard guard(m_resolveLock);
		if (m_activeConn.get() != 0) 
		{
			 return m_activeConn->isConnected();
		}
		return false;
	}
	...
	*/
	GENARequestStatus ServerConnectionManager::sendRequest(GENARequestPtr request, GenaResponse& objResponse)
	{
		...
		{	
			...
			{
				itr->second.state = GENAServer_Unknown;
			}
		}
	...
	*/
	void ServerConnectionManager::setActiveServer(std::string serverURI)
	{
		...
		{	 
			...
			m_servers[serverURI].state = GENAServer_Active;
			...
			for (iter; iter!=m_servers.end(); iter++)
			{
				...
				{
					iter->second.state=GENAServer_Passive;
				}
			}		 
		}

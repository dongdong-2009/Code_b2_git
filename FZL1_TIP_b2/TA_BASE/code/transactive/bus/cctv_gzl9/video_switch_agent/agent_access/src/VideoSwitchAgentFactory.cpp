/**
  ...
  *
  * Source:    $File: //depot/GZL6_TIP/TA_BASE/transactive/bus/cctv_gzl9/video_switch_agent/agent_access/src/VideoSwitchAgentFactory.cpp $
  * @author:   Katherine Thomson
  * @version:  $Revision: #2 $
  *
  * Last modification: $DateTime: 2014/06/24 17:25:08 $
...
namespace TA_Base_Bus_GZL9
{
    ...
    VideoSwitchAgentFactory::VideoOutputGroupVector VideoSwitchAgentFactory::getVideoOutputGroups()
    {
        ...
        // Get the VideoOutputGroups from the agent.

        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::VideoOutputGroupSequence_var videoOutputGroupSeq = 
            m_switchManager->getVideoOutputGroups();

        // Convert each group into a NamedObject and then put it in a vector.

        VideoOutputGroupVector videoOutputGroups;
        for ( unsigned int i = 0; i < videoOutputGroupSeq->length(); i++ )
        {
            // Get the Corba name of the VideoOutputGroup.

            VideoOutputGroupNamedObject* videoOutputGroupNamedObject = 0;
            try
            {
                String_var name = videoOutputGroupSeq[ i ]->getName();
                videoOutputGroupNamedObject = new VideoOutputGroupNamedObject( m_agentName, name.in() );
                if ( 0 == videoOutputGroupNamedObject )
                {
                    throw TA_Base_Bus_GZL9::VideoSwitchAgentException( 
                        "Failed to create new VideoOutputGroupNamedObject." );
                }

                try
                {
                    videoOutputGroupNamedObject->assignObject( videoOutputGroupSeq[ i ] );
                }//TD18706,jianghp, handle the operation mode
				catch (...)
                {
                    (*videoOutputGroupNamedObject).clear();
                }
            }
            catch( const TA_Base_Bus_GZL9::VideoSwitchAgentException& )
            {
                delete videoOutputGroupNamedObject;
                videoOutputGroupNamedObject = 0;

                VideoOutputGroupVector::iterator it = videoOutputGroups.begin();
                for( ; it != videoOutputGroups.end(); ++it )
                {
                    delete *it;
                    *it = 0;
                }
            ...
            videoOutputGroups.push_back( videoOutputGroupNamedObject );
        }
    ...
    VideoSwitchAgentFactory::VideoOutputGroupNamedObject* VideoSwitchAgentFactory::getVideoOutputGroup( unsigned long entityKey )
    {
        ...
        // Get the VideoOutputGroup from the agent.

        TA_Base_Bus_GZL9::VideoOutputGroupCorbaDef_var videoOutputGroup;
		CORBA_CALL_RETURN( videoOutputGroup,  m_switchManager, getVideoOutputGroup, ( entityKey ) );//limin
        // Get the Corba name of the VideoOutputGroup and create a named obj ref.

        VideoOutputGroupNamedObject* videoOutputGroupNamedObject = 0;
        try
        {
            String_var name = videoOutputGroup->getName();
            videoOutputGroupNamedObject = new VideoOutputGroupNamedObject( m_agentName, name.in() );
            if ( 0 == videoOutputGroupNamedObject )
            {
                throw TA_Base_Bus_GZL9::VideoSwitchAgentException( 
            ...
            try
            {
                videoOutputGroupNamedObject->assignObject( videoOutputGroup );
			}
			catch (...) 
			{
				videoOutputGroupNamedObject->clear();
    ...
    VideoSwitchAgentFactory::VideoOutputVector VideoSwitchAgentFactory::getVideoOutputsForGroup( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::VideoOutputGroupCorbaDef_var videoOutputGroup;
		CORBA_CALL_RETURN( videoOutputGroup,  m_switchManager, getVideoOutputGroup, ( entityKey ) );//limin
        // Get the VideoOutputs for the group from the agent.

        TA_Base_Bus_GZL9::VideoOutputGroupCorbaDef::VideoOutputSequence_var videoOutputSeq = 
            videoOutputGroup->getVideoOutputList();
    ...
    VideoSwitchAgentFactory::VideoMonitorVector VideoSwitchAgentFactory::getVideoMonitors()
    {
		...
		testAgentConnection();
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::VideoMonitorSequence_var videoMonitorSeq;
    ...
    VideoSwitchAgentFactory::VideoMonitorNamedObject* VideoSwitchAgentFactory::getVideoMonitorNamedObject( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::VideoMonitorCorbaDef_var videoMonitor;
    ...
    VideoSwitchAgentFactory::VideoMonitorVector VideoSwitchAgentFactory::getConsoleMonitors()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::VideoMonitorSequence_var consoleMonitorSeq;
    ...
    VideoSwitchAgentFactory::VideoMonitorVector VideoSwitchAgentFactory::getLocalConsoleMonitors()
    {
		...
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "VideoSwitchAgentFactory::getLocalConsoleMonitors(). consoleKey:(%ld)",consoleKey);
        // Get the VideoMonitors from the agent.
    ...
    VideoSwitchAgentFactory::RecordingUnitVector VideoSwitchAgentFactory::getRecordingUnits()
    {
        ...
        // Get the RecordingUnits from the agent.

        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::RecordingUnitSequence_var recordingUnitSeq;
		CORBA_CALL_RETURN( recordingUnitSeq,  m_switchManager, getRecordingUnits, () );//limin
        // Convert each group into a NamedObject and then put it in a vector.

        RecordingUnitVector recordingUnits;
        for ( unsigned int i = 0; i < recordingUnitSeq->length(); i++ )
        {
            // Get the Corba name of the RecordingUnit.

            RecordingUnitNamedObject* recordingUnitNamedObject = 0;
            try
            {
                String_var name = recordingUnitSeq[ i ]->getName();
                recordingUnitNamedObject = new RecordingUnitNamedObject( m_agentName, name.in() );
                if ( 0 == recordingUnitNamedObject )
                {
                    throw TA_Base_Bus_GZL9::VideoSwitchAgentException( 
                        "Failed to create new RecordingUnitNamedObject." );
                }
                try
                {
                    recordingUnitNamedObject->assignObject( recordingUnitSeq[ i ] );
		        }
		        catch (...) 
		        {
			        recordingUnitNamedObject->clear();
		        }
            }
            catch( const TA_Base_Bus_GZL9::VideoSwitchAgentException& )
            {
                delete recordingUnitNamedObject;
                recordingUnitNamedObject = 0;

                RecordingUnitVector::iterator it = recordingUnits.begin();
                for( ; it != recordingUnits.end(); ++it )
                {
                    delete *it;
                    *it = 0;
                }
            ...
            recordingUnits.push_back( recordingUnitNamedObject );
        }
    ...
    VideoSwitchAgentFactory::RecordingUnitVector VideoSwitchAgentFactory::getLocalConsoleRecordingUnits()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::RecordingUnitSequence_var consoleRecordingUnitSeq;
		CORBA_CALL_RETURN( consoleRecordingUnitSeq, m_switchManager, getConsoleRecordingUnits, ( consoleKey ) );//limin
        // Convert each group into a NamedObject and then put it in a vector.

        RecordingUnitVector consoleRecordingUnits;
        for ( unsigned int i = 0; i < consoleRecordingUnitSeq->length(); i++ )
        {
            // Get the Corba name of the RecordingUnit.

            RecordingUnitNamedObject* consoleRecordingUnitNamedObject = 0;
            try
            {
                String_var name = consoleRecordingUnitSeq[ i ]->getName();
                consoleRecordingUnitNamedObject = new RecordingUnitNamedObject( m_agentName, name.in() );
                if ( 0 == consoleRecordingUnitNamedObject )
                {
                    throw TA_Base_Bus_GZL9::VideoSwitchAgentException( 
                        "Failed to create new RecordingUnitNamedObject." );
                }
                try
                {
                    consoleRecordingUnitNamedObject->assignObject( consoleRecordingUnitSeq[ i ] );
		        }
		        catch (...) 
		        {
			        consoleRecordingUnitNamedObject->clear();
		        }
            }
            catch( const TA_Base_Bus_GZL9::VideoSwitchAgentException& )
            {
                delete consoleRecordingUnitNamedObject;
                consoleRecordingUnitNamedObject = 0;

                RecordingUnitVector::iterator it = consoleRecordingUnits.begin();
                for( ; it != consoleRecordingUnits.end(); ++it )
                {
                    delete *it;
                    *it = 0;
                }
            ...
            consoleRecordingUnits.push_back( consoleRecordingUnitNamedObject );
        }
        return consoleRecordingUnits;
    }
    ...
    VideoSwitchAgentFactory::RecordingUnitNamedObject* VideoSwitchAgentFactory::getRecordingUnit( unsigned long entityKey )
    {
        ...
        // Get the RecordingUnit from the agent.

        TA_Base_Bus_GZL9::RecordingUnitCorbaDef_var recordingUnit;
		CORBA_CALL_RETURN( recordingUnit, m_switchManager, getRecordingUnit, ( entityKey ) );//limin
        // Get the Corba name of the RecordingUnit and create a named obj ref.

        RecordingUnitNamedObject* recordingUnitNamedObject = 0;
        try
        {
            String_var name = recordingUnit->getName();
            recordingUnitNamedObject = new RecordingUnitNamedObject( m_agentName, name.in() );
            if ( 0 == recordingUnitNamedObject )
            {
                throw TA_Base_Bus_GZL9::VideoSwitchAgentException( 
            ...
            try
            {
                recordingUnitNamedObject->assignObject( recordingUnit );
		    }
		    catch (...) 
		    {
			    recordingUnitNamedObject->clear();
    ...
    VideoSwitchAgentFactory::CameraVector VideoSwitchAgentFactory::getCameras()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::CameraSequence_var cameraSeq;
		try
		{
    ...
    VideoSwitchAgentFactory::CameraVector VideoSwitchAgentFactory::getPTZCameras()
    {
        ...
        for ( unsigned int i = 0; i < cameraSeq->length(); i++ )
        {
			...
			unsigned int tPosition = cameraName.find(PTZ_CAMERA, 0);
    ...
    VideoSwitchAgentFactory::CameraNamedObject* VideoSwitchAgentFactory::getCamera( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::CameraCorbaDef_var camera;
    ...
    VideoSwitchAgentFactory::QuadVector VideoSwitchAgentFactory::getQuads()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::QuadSequence_var quadSeq;
  ...
  /*  VideoSwitchAgentFactory::BVSStageVector VideoSwitchAgentFactory::getBVSStages()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::BVSStageSequence_var bvsStageSeq;
...
*/
    ...
    VideoSwitchAgentFactory::QuadNamedObject* VideoSwitchAgentFactory::getQuad( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::QuadCorbaDef_var quad;
    ...
    VideoSwitchAgentFactory::SequenceVector VideoSwitchAgentFactory::getSequences()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::SequenceSequence_var sequenceSeq;
    ...
    VideoSwitchAgentFactory::SequenceNamedObject* VideoSwitchAgentFactory::getSequence( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::SequenceCorbaDef_var sequence;
    ...
    VideoSwitchAgentFactory::VideoOutputVector VideoSwitchAgentFactory::getVideoOutputs()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::VideoOutputSequence_var videoOutputSeq;
    ...
    VideoSwitchAgentFactory::VideoOutputNamedObject* VideoSwitchAgentFactory::getVideoOutput( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::VideoOutputCorbaDef_var videoOutput;
    ...
    VideoSwitchAgentFactory::VideoInputVector VideoSwitchAgentFactory::getVideoInputs()
    {
        ...
        TA_Base_Bus_GZL9::SwitchManagerCorbaDef::VideoInputSequence_var videoInputSeq;
    ...
    VideoSwitchAgentFactory::VideoInputNamedObject* VideoSwitchAgentFactory::getVideoInput( unsigned long entityKey )
    {
        ...
        TA_Base_Bus_GZL9::VideoInputCorbaDef_var videoInput;
    ...
    unsigned long VideoSwitchAgentFactory::getLocalConsoleKey()
    {
        ...
        throw TA_Base_Bus_GZL9::VideoSwitchAgentException( "The key of this console could not be determined." );
        }
        catch( const TA_Base_Core::DataException& e )
        {
            delete console;
	...
	bool VideoSwitchAgentFactory::testAgentConnection()
	{
		...
		{
			LOG_EXCEPTION_CATCH( SourceInfo,"testAgentConnection Error ",e.what());
			return false;
		}
		catch(...)
		{

...
namespace TA_Base_Bus
{
    ...
    void SteRTUManager::dispatchDataBlock(RTU& rtu, const TA_Base_Bus::DataBlock< WORD > & addressBlock)
    {
        ...
        for (unsigned long i = 0; i < m_rtuDpProcessors.size(); ++ i)
        {
            ...
            if ((true == m_foundMultiplePrimaryRTU && false == m_wrongStatus) ||
                (true == m_foundNoPrimaryRTU && false == m_wrongStatus))
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "No need handle the block...");
                continue; // will not dispatch the data block;
            }
            else if (steRTU->getIsCommsWithStationSystemsValid(steProcessor->getStatusWordNumber(),steProcessor->getSlaveID()) &&
                     false == m_foundMultiplePrimaryRTU && 
                     false == m_wrongStatus)
            {
    ...
    void SteRTUManager::resetRTUManagerStatus()
    {
        m_wrongStatus = false;
        m_foundWrongStatusTime = 0;
        m_foundNoPrimaryRTU = false;
    ...
    void SteRTUManager::switchPrimaryRTUstoStandby()
    {
        ...
        if (true == m_foundMultiplePrimaryRTU)   
        {
            ...
            {
                m_wrongStatus = true;
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "into wrong status.");
            }
        } 

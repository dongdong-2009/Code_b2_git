...
namespace TA_Base_Bus
{
    ...
    public:
        ...
        // for some special todo when changed to monitor mode
        virtual void resetRTUManagerStatus() = 0;
		...
		**/
        ...
        virtual bool hasKnownAllRTUStatus();
	...
	protected:
        ...
        // it indicates RTU manager has known all rtus status,
        // if some rtus has not try to communicate with equipment, the
        // status should be false. If all rtus have finihsed the trying,
        // the status should be true
        std::map<unsigned long, bool> m_rtuHasKnownStatus;
        TA_Base_Core::ReEntrantThreadLockable m_rtuHasKnownStatusLock;

/**
  ...
  * @version $Revision: #3 $
  *
  * Last modification : $DateTime: 2018/04/03 14:28:18 $
...
namespace TA_Base_Bus
{
	...
	void AlarmAckSubscribeWorkItem::executeWorkItem()
	{
        ...
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "AlarmAckSubscribeWorkItem processSubscribe unknow error, entitykey [%d], alarm ID [%s], value string [%s]", m_subscribeRequest->m_entityKey, m_subscribeRequest->m_alarmId.c_str(), m_subscribeRequest->m_valueString.c_str());
        }
		m_sync->itemEnd();

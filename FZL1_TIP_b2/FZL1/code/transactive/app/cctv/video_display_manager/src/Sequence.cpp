/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/cctv/video_display_manager/src/Sequence.cpp $
  * @author:  Adam Radics
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    Sequence::Sequence( unsigned long entityKey,
      ...
      : VideoInput(entityKey, name, description, group , locationId ),
		m_address(addr),
        m_dwellTime(0),
        m_cameras(),
        m_readOnly(false),
	...
	std::string Sequence::getAddress()
	{
	...
	void Sequence::setAddress(std::string addr)
	{
	...
	//
	// saveSequenceDescription
	//
	bool Sequence::saveSequenceDescription(const char* newDescription)
	{
		if ( getReadOnly() )
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Sequence configuration is read only");
			return false;
		}

		try
		{
			TA_Base_Core::Sequence* entityData = new TA_Base_Core::Sequence(this->m_entityKey);
			entityData->setSequenceDescription(newDescription);
			delete entityData;
		}

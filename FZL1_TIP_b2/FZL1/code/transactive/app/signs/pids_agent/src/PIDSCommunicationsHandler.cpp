/**
  ...
  *
  * Source:    $File: //depot/3002_TIP/3002/transactive/app/signs/pids_agent/src/PIDSCommunicationsHandler.cpp $
...
/*
	void PIDSCommunicationsHandler::connectionEstablished()
	{	
		FUNCTION_ENTRY("connectionEstablished");

		m_connected = true;

		LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Connection established with PIDS server" );

		PIDSManager::getInstance()->closeAlarm( TA_Base_Core::PIDSAlarms::PIDSServerCommFailure );

		FUNCTION_EXIT;
	}

	bool PIDSCommunicationsHandler::connectionLost()
	{
		FUNCTION_ENTRY("connectionLost");

		m_connected = false;

		LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Lost connection to PIDS server" );

		TA_Base_Core::DescriptionParameters dp;

		TA_Base_Core::NameValuePair nvp("Location", PIDSManager::getInstance()->getLocationName() );

		dp.push_back( &nvp );

		PIDSManager::getInstance()->submitAlarm( TA_Base_Core::PIDSAlarms::PIDSServerCommFailure, dp );
...
//		if( PIDSManager::getInstance()->getLocationType() != TA_Base_Core::ILocation::OCC)
//		{
//			PIDSManager::getInstance()->setBadDataPointQuality();
//		}
...
*/
	short PIDSCommunicationsHandler::sendMessage( PIDSMessage* data, bool isSigMsg/* = false*/, WORD startAddress/*= 1*/)
	{
		...
		if ( 0 == m_pModbusHelper && !isSigMsg)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "No modbus helper yet, wait for config update" );
			return -1;
		}
		else if( 0 == m_pModbusHelperSignalling && isSigMsg)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "No signalling modbus helper yet, wait for config update" );
			return -1;
		}
		...
		try
		{
			...
			{
				m_pModbusHelperSignalling->SendData( dbwData, startAddress);
			}			
		}
...
//			throw(e);			
	...
	PIDSCommunicationsHandler::PIDSCommunicationsHandler() 
		: m_pModbusHelper( static_cast<TA_Base_Bus::IModbusHelper*>(0) ),
		  m_pModbusHelperSignalling( static_cast<TA_Base_Bus::IModbusHelper*>(0) )
	{
	...
	void PIDSCommunicationsHandler::initialise(unsigned long agentLocation)
	{
		...
		{
			// boost smart pointer
			m_pModbusHelperSignalling = TA_Base_Bus::ModbusHelperFactory::createHelper(
				agentLocation, TA_Base_Bus::ModbusHelperFactory::SIGNALLING );
		}
		catch ( TA_Base_Bus::NotFoundFEPConfigException& expConfigError )
		{
			LOG_EXCEPTION_CATCH( SourceInfo, "TA_Base_Bus::NotFoundFEPConfigException", expConfigError.what() );			
		...
		{
			LOG_EXCEPTION_CATCH( SourceInfo, "Unkonw exception", "Failed to create signalling modbus helper instance" );
		}
		FUNCTION_EXIT;
	}

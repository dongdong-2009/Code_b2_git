/**
  ...
  *
  * Source:    $File: //depot/3002_TIP/3002/transactive/app/signs/pids_agent/src/PIDSManager.cpp $
...
//#include "app/signs/pids_agent/src/PIDSMessageDataDefinitionsHelper.h"
...
#include "app/signs/pids_agent/src/PIDSCircleMessageProcessor.h"
...
namespace TA_IRS_App
{
    ...
    PIDSManager::PIDSManager() :
        ...
        m_encodingConvertBufLen(2048),
        m_encodingConvertBuf(new char[m_encodingConvertBufLen]),
        m_encoding("gb18030")
    {
        ...
        {
            encoding =
                TA_Base_Core::GlobalParameterAccessFactory::getInstance().getGlobalParameterValueFromNameAndSubsystem("CharacterSet", 0);
        }
        catch (...)
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Failed get value of CharacterSet parameter");
        }
        if(!encoding.empty())
        {
            m_encoding = encoding;
    ...
    void PIDSManager::submitSigRequest(const ::TA_Base_Bus::IPIDSManagerCorbaDef::SigData& data)
    {
		...
		PIDSCommunicationsHandler::getInstance()->sendMessage( pidsSigRequest.get(), true, PIDSSigDisplayRequest::SIGINFO_FEP_REGISTER_ADDRESS );
        FUNCTION_EXIT;
    }
            ...
            ::CORBA::Boolean isCyclic )
    {
        ...
        dump1 << "station,gateway,downtrack,uptrack,concourse:";
        for ( int i = 0; i < destinationList.length(); i++ )
        {
            dump1 << destinationList[i].station << " " << destinationList[i].gateway << " " << destinationList[i].downTrack << " " << destinationList[i].upTrack << " " << destinationList[i].concourse << ", ";
        }
		if(messageContent.length() % 2 != 0)
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "The length of message content is invalid");
            return 0;
        }
		LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "%s", dump1.str().c_str() );
		std::ostringstream dump2;
		// Log the unicode content value
        dump2 << "content=";
		dump2 << std::hex << std::uppercase;
        for ( int i = 0; i < messageContent.length(); i += 2 )
        {
			// Message content pass from PIDSManager, LSB is forward
			dump2 << std::setw(2) << std::setfill('0') << (unsigned short)messageContent[i+1]
			<< std::setw(2) << std::setfill('0') << (unsigned short)messageContent[i] << " ";
        }
		dump2 << std::dec << std::nouppercase;
        dump2 << ", messageType=" << msgType << ", showType=" << showType << ", trainNo=" << trainNo;
        //dump << ", startTime=" << startTime.hour << ":" << startTime.minute << ":" << startTime.second;
        //dump << ", endTime=" << endTime.hour << ":" << endTime.minute << ":" << endTime.second;
        //dump << ", isCyclic=" << isCyclic;
        LOGLARGESTRING( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, dump2.str().c_str() );
        ...
        short nRet = 0;
		//FZL1 have no cyclic message
//        if ( !isCyclic )
        {
            LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Sending adhoc display request message" );
            nRet = PIDSCommunicationsHandler::getInstance()->sendMessage( displayRequest );
            //LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Adhoc message sent successfully." );
...
/*
        ...
        {
            LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Add message to circle list" );
            PIDSCircleMessageProcessor::getInstance()->addCirleMessage( displayRequest, startTime, endTime );
            PIDSCircleMessageProcessor::getInstance()->addClearMessage( destinationList , isTrain );
        }
*/        return nRet;
        FUNCTION_EXIT;
    }
    ...
    ::CORBA::Short PIDSManager::submitAdhocDisplayRequestWithMulityByteMsgContent( const ::TA_Base_Bus::IPIDSManagerCorbaDef::PidsDestinationList& destinationList,
            ...
            ::CORBA::Boolean isCyclic )
    {
        TA_Base_Core::CodeConverter converter(m_encoding.c_str(), TA_UTF16LE);
        int inlen = messageContent.length();
        int outlen = m_encodingConvertBufLen;
        converter.convert((const char*)messageContent.NP_data(), inlen, m_encodingConvertBuf, outlen);
        ::TA_Base_Bus::IPIDSManagerCorbaDef::PidsMessage newContent;
        if(outlen < 0)
        {
            LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError,
                "Convert message encoding %s failed", m_encoding.c_str());
            return 0;
        }
        newContent.length(outlen);
        for(int i = 0; i < outlen; ++i)
        {
            newContent[i] = m_encodingConvertBuf[i];
        }
		::CORBA::Long trainNo = -1;		//This interface is for StationPid PlanStep, it don't need train function
        return submitAdhocDisplayRequest(destinationList, newContent, msgType, showType,
            trainNo, startTime, endTime, interval, isCyclic);
    }
            ...
            ::CORBA::Long trainNo )
    {
        ...
        dump << "station,gateway,downtrack,uptrack,concourse:";
        for ( int i = 0; i < destinationList.length(); i++ )
        {
            dump << destinationList[i].station << " " << destinationList[i].gateway << " " << destinationList[i].downTrack << " " << destinationList[i].upTrack << " " << destinationList[i].concourse << ", ";
        }

...
const CString PIPE("|");
static const unsigned int MAX_PIDS_MESSAGE_NUMBER = 30;
...
//        TA_THROW(PidAgentException(message));	// temp//test zhou yuan - for testing only
	...
	std::vector<TA_IRS_Core::PidMessage> StationPidUpdater::getPidMessages()
	{
		...
		if(pidMsgs.size() > MAX_PIDS_MESSAGE_NUMBER)
		{
			pidMsgs.erase(pidMsgs.begin()+MAX_PIDS_MESSAGE_NUMBER, pidMsgs.end());
		}
		FUNCTION_EXIT;
		return pidMsgs;
	}

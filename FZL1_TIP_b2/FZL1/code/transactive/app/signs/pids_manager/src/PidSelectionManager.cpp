...
const std::string PIDSelectionManager::GATEWAY = "GT01";
...
std::map<unsigned long, std::string> PIDSelectionManager::m_stationIdLocationDisplayNameMap;
...
PIDSelectionManager::PIDSelectionManager(): m_isSelectTrain( false )
{
	...
	try
	{
			...
			++it)
		{
			...
			m_stationIdLocationDisplayNameMap.insert(std::pair<unsigned long, std::string>(it->first, location->getDisplayName()));
			delete location;
		}
...
void PIDSelectionManager::selectStationOneTrack( std::string strStationName, std::string strTrack )
{
	...
	tmpDest.gateway = false;

	if ( strTrack == CONC )
		tmpDest.concourse = true;
	else if ( strTrack == UPTRACK )
        tmpDest.upTrack = true;
    ...
    for ( int i = 0; i < m_pidsList.length(); ++i )
    {
        if ( m_pidsList[i].station == tmpDest.station )
        {
            ...
            m_pidsList[i].gateway |= tmpDest.gateway;
            return;
        }
    }
    m_pidsList.length( m_pidsList.length() + 1 );
...
void PIDSelectionManager::deselectStationOneTrack( std::string strStationName, std::string strTrack )
{
    ...
    for ( int i = 0; i < m_pidsList.length(); ++i )
    {
        ...
        else
        {
			...
			m_pidsList[i].downTrack ^= tmpDest.downTrack;
			m_pidsList[i].gateway ^= tmpDest.gateway;
            if ( m_pidsList[i].gateway || m_pidsList[i].downTrack || m_pidsList[i].upTrack || m_pidsList[i].concourse )
            {
                tmpList.length( tmpList.length() + 1 );
                tmpList[tmpList.length() - 1] = m_pidsList[i];
            }
...
std::string PIDSelectionManager::getPidsString(int trainNo)
{
	std::stringstream ss;
	for( unsigned long i = 0; i < m_pidsList.length(); i++ )
	{
		int stationId = m_pidsList[i].station;
		std::string locationDisplayName = m_stationIdLocationDisplayNameMap[stationId];
		ss << locationDisplayName;
		if(m_pidsList[i].concourse == 1)
		{
			ss << " 站厅";
		}
		if(m_pidsList[i].upTrack == 1)
		{
			ss << " 上行轨道";
		}
		if(m_pidsList[i].downTrack == 1)
		{
			ss << " 下行轨道";
		}
		if(m_pidsList[i].gateway == 1)
		{
			ss << " 出入口";
		}
		if (i < m_pidsList.length() - 1)
		{
			ss << ", ";
		}
	}

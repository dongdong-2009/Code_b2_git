/**
  ...
  * in any form.
  *
  * Source:  $File: //depot/3001_TIP_NEW/3001/transactive/app/maintenance_management/MmsController/src/MmsControllerModel.cpp $
  * @author Jade Lee
  * @version $Revision: #2 $
  *
...
namespace TA_IRS_App
{
    ...
    TA_Base_Bus::DataNodeProxySmartPtr * MmsControllerModel::getDatanodeProxy(const std::string equipmentName, bool waitForInit)
    {
        ...
        {
            LOG(SourceInfo, TA_Base_Core::DebugUtil::ExceptionCatch, "Unknown", "getDatanodeProxy");
            TA_ASSERT(FALSE, "getDatanodeProxy can not return a datanode proxy");
        }
		return NULL;
    }
    ...
    void MmsControllerModel::sendGlobalInhibitAuditMessage(const EInhibitResult& result)
    {
        try
        {
            ...
            case INHIBIT_FAILED:
                ...
                break;
            default:
                TA_ASSERT(FALSE, "sendGlobalInhibitAuditMessage: Unrecognized inhibit result");
            }
        }
        catch (...)
        {
    ...
    {
        TA_ASSERT(!subsystemName.empty(), "No subsystem is specified");
        try
        {
	...
	{
		TA_ASSERT(!nonPhysicalsystemName.empty(), "No non-physical system is specified");
        try
        {
            ...
            case SYSTEM_ALARM_INHIBIT_FAILED:
                ...
                break;
            default:
                TA_ASSERT(FALSE, "sendNonPhysicalSubsystemInhibitAuditMessage: Unrecognized inhibit result");
            }
        }
        catch (...)
        {

...
{
	const CString MSG_DLG_TITLE = "故障申报单";
	...
	{//TD18095, jianghp, to fix the performance of showing manager application
		...
		{
			reportFailureAndTerminate("EAM Agent通讯失败", "EAM Agent通讯失败");
			return;
		}
	...
	void CJobRequestMgrDlg::OnSubmit()
	{
		...
		try
		{
			...
			else
			{
				...
				if (ret==false)
				{
					std::string displayStr = "EAM Agent通讯失败.";
	...
	LRESULT CJobRequestMgrDlg::OnLaunchConditionsChange(WPARAM wParam, LPARAM lParam)
	{
		...
		try
		{
			switch(m_launchConditions.launchedFrom)
			{
				...
				case INVALID_PARAMS:
				{
					reportFailureAndTerminate("无法获取相应的数据", "无法获取相应的数据");
					break;
				}
				default:
				{

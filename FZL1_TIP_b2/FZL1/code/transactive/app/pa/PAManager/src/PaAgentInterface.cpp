...
int PaAgentInterface::startDVABroadcast( std::string& strBroadcasstId, std::vector<unsigned long> vecZoneKeys, unsigned long ulDVAMsgKey, int nCylicTimes, time_t tmStartTimeInDay )
{
    if ( NULL == s_pPaAgentNamedObject )
    {
    ...
    try
    {
        if (( nCylicTimes > 0u ) || ( tmStartTimeInDay > 0u ))
        {
            ...
            CORBA_CALL_RETURN( strBroadcasstId, (*s_pPaAgentNamedObject), broadcastStationScheduleDva,
                (messages, zones, 0, false, false, "", stSchedule, CachedConfig::getInstance()->getSessionId().c_str()) );
        }
        else
        {
            CORBA_CALL_RETURN( strBroadcasstId, (*s_pPaAgentNamedObject), broadcastStationDva,
                (messages, zones, 0, false, false, "", CachedConfig::getInstance()->getSessionId().c_str()) );
        }
    }

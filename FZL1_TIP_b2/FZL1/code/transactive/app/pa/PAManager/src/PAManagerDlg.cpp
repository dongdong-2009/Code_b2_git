...
m_pScheduleBroadcastDlg( new ScheduleBroadcastDlg() ),
...
PAManagerDlg::~PAManagerDlg()
{
    ...
    if ( NULL != m_pScheduleBroadcastDlg )
    {
        delete m_pScheduleBroadcastDlg;
        m_pScheduleBroadcastDlg = NULL;
    }
...
BOOL PAManagerDlg::OnInitDialog()
{
    ...
    if ( NULL == m_hUnknow )
    {
        m_hUnknow = static_cast<HICON>(::LoadImage( ::AfxGetInstanceHandle(), 
            MAKEINTRESOURCE(IDB_UNKNOW), IMAGE_ICON, BACKUP_KEY_ICON_SIZE, BACKUP_KEY_ICON_SIZE, 0 ));
    }
    ...
    m_picBackupStatus.SetIcon( m_hUnknow );
...
void PAManagerDlg::OnBnClickedBtnBroadcastStart()
{
    ...
    int nRet = PaAgentInterface::startDVABroadcast( strBroadcastID, vecZones, ulDVAMsgKey, nRepeatTimes, tmStartTimeInDay );
...
void PAManagerDlg::OnClickedCheckScheduleManager()
{
    ...
    long lDiffer = stScheduleRect.right - stScheduleRect.left;
...
void PAManagerDlg::changeIconPosition(bool isRestore)
{
    CWnd* pWnd=GetDlgItem(IDC_FREQ_USED_MESSAGE );
    parallelMoveIcon( pWnd, 30, 254, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_A);
    parallelMoveIcon( pWnd, 10, 50, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_B);
    parallelMoveIcon( pWnd, 10, 50, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_C);
    parallelMoveIcon( pWnd, 10, 50, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_D);
    parallelMoveIcon( pWnd, 10, 50, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_E);
    parallelMoveIcon( pWnd, 10, 50, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_F);
    parallelMoveIcon( pWnd, 60, 100, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_G);
    parallelMoveIcon( pWnd, 60, 100, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_H);
    parallelMoveIcon( pWnd, 60, 100, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_I);
    parallelMoveIcon( pWnd, 60, 100, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_J);
    parallelMoveIcon( pWnd, 60, 100, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_K);
    parallelMoveIcon( pWnd, 110, 150, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_L);
    parallelMoveIcon( pWnd, 110, 150, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_M);
    parallelMoveIcon( pWnd, 110, 150, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_N);
    parallelMoveIcon( pWnd, 110, 150, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_O);
    parallelMoveIcon( pWnd, 110, 150, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_P);
    parallelMoveIcon( pWnd, 160, 200, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_Q);
    parallelMoveIcon( pWnd, 160, 200, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_R);
    parallelMoveIcon( pWnd, 160, 200, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_S);
    parallelMoveIcon( pWnd, 160, 200, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_T);
    parallelMoveIcon( pWnd, 160, 200, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_U);
    parallelMoveIcon( pWnd, 210, 254, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_V);
    parallelMoveIcon( pWnd, 210, 254, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_W);
    parallelMoveIcon( pWnd, 210, 254, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_X);
    parallelMoveIcon( pWnd, 210, 254, isRestore );
    pWnd=GetDlgItem(IDC_BTN_BROADCAST_Y);
    parallelMoveIcon( pWnd, 210, 254, isRestore );

    pWnd=GetDlgItem(IDC_BROADCAST_ZONE);
    parallelMoveIcon( pWnd, 30, 65, isRestore );
    parallelMoveIcon( &m_ckDVABroadcastMode, 35, 45, isRestore );
    parallelMoveIcon( &m_ckLiveBroadcastMode, 35, 45, isRestore );
    parallelMoveIcon( &m_ckBackgroundMusicMode, 35, 45, isRestore );
    pWnd=GetDlgItem(IDC_BACKUP_KEY);
    parallelMoveIcon( pWnd, 35, 45, isRestore );
    parallelMoveIcon( &m_picBackupStatus, 35, 45, isRestore );

    pWnd=GetDlgItem(IDC_BROADCAST_ZONE_GROUP);
    parallelMoveIcon( pWnd, 30, 65, isRestore );
    parallelMoveIcon( &m_lbSeledZones, 35, 45, isRestore );
    parallelMoveIcon( &m_cbGroupNameList, 35, 45, isRestore );
    parallelMoveIcon( &m_btnSaveZoneGroup, 35, 40, isRestore );
    parallelMoveIcon( &m_btnDelZoneGroup, 40, 45, isRestore );

    pWnd = GetDlgItem( IDC_ZONE_AREA_MARKER );
    parallelMoveIcon( pWnd, 75, 180, isRestore );
    GwxDisplayWrapper::getInstance().changeSchematicSize(pWnd->GetSafeHwnd(), GetSafeHwnd());

    pWnd = GetDlgItem( IDC_PRERECORD_MESSAGE );
    parallelMoveIcon( pWnd, 200, 254, isRestore );
    parallelMoveIcon( &m_tabDvaMsg, 200, 254, isRestore );

    pWnd = GetDlgItem( IDC_CYCLIC_BROADCAST );
    parallelMoveIcon( pWnd, 200, 210, isRestore );
    parallelMoveIcon( &m_ckCycleBroadcast, 200, 200, isRestore );
    pWnd = GetDlgItem( IDC_CYCLIC );
    parallelMoveIcon( pWnd, 205, 205, isRestore );
    parallelMoveIcon( &m_edCycleTimes, 205, 210, isRestore );
    pWnd = GetDlgItem( IDC_TIMES );
    parallelMoveIcon( pWnd, 210, 210, isRestore );

    pWnd = GetDlgItem( IDC_SCHEDULED_IN_DAY );
    parallelMoveIcon( pWnd, 210, 240, isRestore );
    parallelMoveIcon( &m_ckScheduleInDay, 210, 210, isRestore );
    parallelMoveIcon( &m_dtStartTimeInDay, 220, 240, isRestore );
    parallelMoveIcon( &m_ckSchedulemanager, 240, 240, isRestore );

    pWnd = GetDlgItem( IDC_TIMETABLE_BROADCAST );
    parallelMoveIcon( pWnd, 240, 254, isRestore );
    parallelMoveIcon( &m_btnSchedule, 240, 254, isRestore );

    pWnd = GetDlgItem( IDC_BROADCAST );
    parallelMoveIcon( pWnd, 200, 254, isRestore );
    pWnd = GetDlgItem(IDC_BROADCAST_CONTENT);
    parallelMoveIcon( pWnd, 230, 230, isRestore );
    parallelMoveIcon( &m_edBroadcastRecordName, 230, 254, isRestore );
    parallelMoveIcon( &m_btnDVABroadcastStart, 230, 240, isRestore );
    parallelMoveIcon( &m_btnDVABroadcastStop, 240, 254, isRestore );
    parallelMoveIcon( &m_btnMonitorStart, 230, 240, isRestore );
    parallelMoveIcon( &m_btnMonitorStop, 240, 254, isRestore );
}

void PAManagerDlg::parallelMoveIcon(CWnd* pWnd, long lLeftDiff, long lRightDiff, bool isRestore )
{
    if ((NULL != pWnd) && (::IsWindow(pWnd->GetSafeHwnd())))
    {
        ...
        if (isRestore)
        {
            stRect.left = stRect.left + lLeftDiff;
            stRect.right = stRect.right + lRightDiff;
        }
        else
        {
            stRect.left = stRect.left - lLeftDiff;
        ...
        pWnd->MoveWindow(&stRect);
    }
    else
    {
        LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Windows point is NULL!" );
    }
}
    ...
    const TA_Base_Bus::IPAAgentCorbaDef::MessageKeySeq& messages, const std::string& sessionId )
{
    ...
    NameValuePair messageLabelsParam ( "MsgLabels", formatDvaMsgNamesForAudit( messages ));
...
void PasHelpers::submitPaBroadcastStationMusicRequestAuditMessage( const TA_Base_Bus::IPAAgentCorbaDef::EMusicType& musicType, const std::string& sessionId )
{
    ...
    if(TA_Base_Bus::IPAAgentCorbaDef::LocalMusic == musicType || TA_Base_Bus::IPAAgentCorbaDef::OccMusic == musicType )
	{
		// send the message
	...
	{
		// send the message
		submitAuditMessage( TA_Base_Core::PAAgentAudit::PaStopBroadcastStationMusicRequest, parameters, sessionId );
	}
    ...
    unsigned long ulMsgKey, unsigned short usMsgType, const std::string& sessionId )
{
    ...
    NameValuePair messageIdParam ( "MessageId", formatDvaMsgIdForAudit(ulMsgKey));
    parameters.push_back ( &messageIdParam );
	
    NameValuePair messageLabelsParam( "MsgLabel", formatDvaMsgNameForAudit(ulMsgKey));
    parameters.push_back ( &messageLabelsParam );
    ...
    const TA_Base_Bus::IPAAgentCorbaDef::MessageKeySeq& messages, const std::string& sessionId )
{
    ...
    // build the parameters
    DescriptionParameters parameters;
    NameValuePair messageLabelsParam ( "BroadcastId", formatDvaMsgNamesForAudit( messages ));
    parameters.push_back ( &messageLabelsParam );
    ...
    unsigned short newType, unsigned long messageId, const std::string& sessionId )
{
    ...
    NameValuePair messageLabelsParam ( "MessageId", formatDvaMsgNameForAudit( messageId ));
    parameters.push_back ( &messageLabelsParam );
...
void PasHelpers::submitPaStopBroadcastStationDvaRequestAuditMessage( const TA_Base_Bus::IPAAgentCorbaDef::ZoneCoverage& zones, const std::string& sessionId )
{
	FUNCTION_ENTRY("submitPaStopBroadcastStationDvaRequestAuditMessage");
	...
	// send the message
	submitAuditMessage( TA_Base_Core::PAAgentAudit::PaStopBroadcastStationDvaRequest, parameters, sessionId );

    FUNCTION_EXIT;
}

void PasHelpers::submitPaStopBroadcastStationLiveRequestAuditMessage( const TA_Base_Bus::IPAAgentCorbaDef::ZoneCoverage& zones, const std::string& sessionId )
{
	FUNCTION_ENTRY("submitPaStopBroadcastStationLiveRequestAuditMessage");
	...
	// send the message
	submitAuditMessage( TA_Base_Core::PAAgentAudit::PaStopBroadcastStationLiveRequest, parameters, sessionId );

    FUNCTION_EXIT;
}

void PasHelpers::submitPaBroadcastMonitoringRequestAuditMessage( const TA_Base_Bus::IPAAgentCorbaDef::ZoneCoverage& zones, const std::string& sessionId )
{
	FUNCTION_ENTRY("submitPaBroadcastMonitoringRequestAuditMessage");
	...
	// send the message
	submitAuditMessage( TA_Base_Core::PAAgentAudit::PaBroadcastMonitoringRequest, parameters, sessionId );

    FUNCTION_EXIT;
}

void PasHelpers::submitPaStopBroadcastMonitoringRequestAuditMessage( const std::string& sessionId )
{
	FUNCTION_ENTRY("submitPaStopBroadcastMonitoringRequestAuditMessage");
	...
	// send the message
	submitAuditMessage( TA_Base_Core::PAAgentAudit::PaStopBroadcastMonitoringRequest, parameters, sessionId );
...
void PasHelpers::sendScheduleBroadcastsUpdate( const TA_Base_Bus::IPAAgentCorbaDef::ScheduleBroadcastsUpdateData& updateData )
{
    FUNCTION_ENTRY( "sendCurrentBroadcastsUpdate" );
...
void PasHelpers::sendPAAgentRemovedBroadcastUpdate( unsigned long ulScheduleKey )
{
    ...
    data <<= static_cast<CORBA::ULong>(ulScheduleKey);
...
void PasHelpers::sendPAAgentNewBroadcastUpdate(const TA_Base_Bus::IPAAgentCorbaDef::TriggeredBroadcastConfig& updateData)
{
    FUNCTION_ENTRY( "sendPAAgentNewBroadcastUpdate" );

    LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Sync Sig broadcast to monitor agent: %s", updateData.strBroadcastId._ptr);
...
void PasHelpers::corbaConfigToStdConfig( const TA_Base_Bus::IPAAgentCorbaDef::StationDvaConfig& refCorbaConfig, StdStationDvaConfig& stToConverted )
{
	...
	stToConverted.ulOriginalStartTime = static_cast<unsigned long>(refCorbaConfig.bCyclic ? refCorbaConfig.stCyclicParams.startTime : currentTime);
	stToConverted.ulOriginalEndTime = static_cast<unsigned long>(refCorbaConfig.bCyclic ? refCorbaConfig.stCyclicParams.stopTime : (currentTime + SECONDS_FOR_ONE_MINUTE));
...
void PasHelpers::corbaConfigToStdConfig( const TA_Base_Bus::IPAAgentCorbaDef::StationDvaConfig& refCorbaConfig, ScheduleDvaConfig& stToConverted )
{
    ...
    for ( unsigned int unMessageLoop = 0; unMessageLoop < refCorbaConfig.messages.length(); ++unMessageLoop )
	{
        stToConverted.vecMsgKeys.push_back( refCorbaConfig.messages[unMessageLoop] );
	}
...
unsigned long PasHelpers::getPaPriorityIdBySessionId( const std::string& strSessionId )
{
    FUNCTION_ENTRY( "getPaPriorityIdBySessionId" );

    unsigned long ulPriorityGroupId = TA_Base_Core::DEFAULT_LOWEST_PA_PRIORITY_ID;

    try
    {
        //ulPriorityGroupId = TA_Base_Core::PaPriorityGroupAccessFactory::getInstance().getPaPriorityGroupIdBySessionId( strSessionId );
        std::auto_ptr<TA_Base_Core::IConsole> ptrConsole( TA_Base_Core::ConsoleAccessFactory::getInstance().getConsoleFromSession( strSessionId ) );
        ulPriorityGroupId = ptrConsole->getPagingConsoleSourceId();
    }
    ...
    FUNCTION_EXIT;
    return ulPriorityGroupId;
}

void PasHelpers::getStationZoneIdsByZoneKeys( StationIdToZoneIdsMap& mapStaionZoneIds, const Keys& keys )
{
    FUNCTION_ENTRY( "getStationZoneIdsByZoneKeys" );

    for ( Keys::const_iterator itLoop = keys.begin(); keys.end() != itLoop; ++itLoop )
    {
        ...
        if ((PA_STATION_ID_MIN <= stationId) && (PA_STATION_ID_MAX >= stationId) &&
            (PA_ZONE_ID_MIN <= zoneId) && (PA_ZONE_ID_MAX >= zoneId))
        {
            mapStaionZoneIds[stationId].push_back( zoneId );
        }
    }
...
void PasHelpers::getDvaMsgIdsByDvaMsgKeys( std::vector<unsigned long>& vecDvaMsgIds, const Keys& keys )
{
    FUNCTION_ENTRY( "getDvaMsgIdsByDvaMsgKeys" );

    for ( Keys::const_iterator itLoop = keys.begin(); keys.end() != itLoop; ++itLoop )
    {
        unsigned long ulDvaMsgId = TA_IRS_Bus::CachedMaps::getInstance()->getStationDvaMessageIdFromKey( *itLoop );
        if ( (PA_DVA_MSG_ID_MIN <= ulDvaMsgId) && (PA_DVA_MSG_ID_MAX >= ulDvaMsgId) )
        {
            vecDvaMsgIds.push_back( ulDvaMsgId );
        }
    }

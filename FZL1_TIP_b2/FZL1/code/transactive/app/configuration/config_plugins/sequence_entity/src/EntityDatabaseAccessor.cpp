/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/configuration/config_plugins/sequence_entity/src/EntityDatabaseAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    std::multimap<CString,unsigned long> EntityDatabaseAccessor::getItemNames(CProgressCtrl& progress)
    {
             ...
             ++iter)
        {
            ...
            {	
				...
				{
					m_subStringItemNamesNumber.insert(std::multimap<CString,unsigned long>::value_type(subStringItemNames,1));
				}
				else
				{
                    m_subStringItemNamesNumber.find(subStringItemNames)->second++;
    ...
    void EntityDatabaseAccessor::deleteItem(unsigned long key)
    {
        ...
        m_subStringItemNamesNumber.find(tempName)->second--;
    ...
    TA_Base_Core::IConfigItem* EntityDatabaseAccessor::newItem(const std::string& type)
    {
		...
		//tempName = tempName.GetBufferSetLength(3);
		//std::multimap<CString,unsigned long>[tempName] = (std::multimap<CString,unsigned long>.find(tempName))->second+1;
        return newEntity;
	...
	void EntityDatabaseAccessor::increaseLocationCount(const CString& locName,const CString& prevText)
	{
		if((locName.CompareNoCase(prevText)!=0)&&prevText.Compare("")==0)
            ...
            m_subStringItemNamesNumber.find(locName)->second++;
		 if ((locName.CompareNoCase(prevText)!=0)&&(prevText.Compare("")!=0))
		 {
             //m_subStringItemNamesNumber[prevText] = (m_subStringItemNamesNumber.find(prevText))->second-1;
             //m_subStringItemNamesNumber[locName] = (m_subStringItemNamesNumber.find(locName))->second+1;
             m_subStringItemNamesNumber.find(prevText)->second--;
             m_subStringItemNamesNumber.find(locName)->second++;
		 }

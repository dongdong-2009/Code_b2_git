/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3002_TIP/3002/transactive/app/configuration/config_plugins/pa_fas_triggered_broadcast/src/PAFasTriggeredBroadcastListView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
  ...
  * with the PaFasTriggeredBroadcastDetailView as this will show the details of the item selected in this view.
  */
...
namespace TA_IRS_App
{
    ...
    const CString PaFasTriggeredBroadcastListView::NEW = "新建的";
    const CString PaFasTriggeredBroadcastListView::MODIFIED = "已修改";
    const CString PaFasTriggeredBroadcastListView::UPTODATE = "";

    const bool PaFasTriggeredBroadcastListView::ASCENDING = true;
    const bool PaFasTriggeredBroadcastListView::DESCENDING = false;

    IMPLEMENT_DYNCREATE(PaFasTriggeredBroadcastListView, CListView)


    PaFasTriggeredBroadcastListView::PaFasTriggeredBroadcastListView()
    :     m_sortOrder(DESCENDING),        // Set to descending so the first time we sort it switches to ASCENDING
          m_sortColumn(PaFasTriggeredBroadcastListColumns::getDefaultSortColumn())
    {
        m_componentIcon = LoadIcon(GetModuleHandle(PaFasTriggeredBroadcastComponent::DLL_NAME), MAKEINTRESOURCE(IDI_CONFIG_PA_FAS_TRIGGERED_BROADCASTS) );
    }
...
BEGIN_MESSAGE_MAP(PaFasTriggeredBroadcastListView, AbstractListView)
	//{{AFX_MSG_MAP(PaFasTriggeredBroadcastListView)
	ON_WM_CREATE()
    ON_WM_SIZE()
...
END_MESSAGE_MAP()
    ...
    int PaFasTriggeredBroadcastListView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
    {
    ...
    void PaFasTriggeredBroadcastListView::OnInitialUpdate() 
    {
    ...
    void PaFasTriggeredBroadcastListView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    void PaFasTriggeredBroadcastListView::loadAllItems()
    {
        ...
        PaFasTriggeredBroadcastDatabaseAccessor* dbAccessor = dynamic_cast<PaFasTriggeredBroadcastDatabaseAccessor*>(getDatabaseAccessor());
        ...
        for (std::map<unsigned long, PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord>::iterator iter 
    ...
    void PaFasTriggeredBroadcastListView::itemDetailsChanged(unsigned long id, const CString& name)
    {
        ...
        PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord details = dbAccessor->getItemDetails( id );
        ...
        std::map<unsigned long, PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord>::iterator matching;
        ...
        else
        {
            m_currentDisplayedItems.insert( 
                std::map<unsigned long, PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord>::value_type(id, details) );
        }
    ...
    void PaFasTriggeredBroadcastListView::copyItem()
    {
    ...
    void PaFasTriggeredBroadcastListView::newItem()
    {
 ...
 /*   void PaFasTriggeredBroadcastListView::deleteItem()
    {
    ...
    void PaFasTriggeredBroadcastListView::onColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
    {
	...
	void PaFasTriggeredBroadcastListView::updateHeaderSort(const unsigned int column)
	{
        ...
        headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(PaFasTriggeredBroadcastComponent::DLL_NAME), MAKEINTRESOURCE(resourceToLoad), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    ...
    int CALLBACK PaFasTriggeredBroadcastListView::compareItems(LPARAM index1, LPARAM index2, LPARAM listCtrl)
    {
        PaFasTriggeredBroadcastListView* list = reinterpret_cast<PaFasTriggeredBroadcastListView*>(listCtrl);
        // Double check it has converted ok
        list = dynamic_cast<PaFasTriggeredBroadcastListView*>(list);
        ...
        std::map<unsigned long, PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord>::iterator item1 
                                                        = list->m_currentDisplayedItems.find( index1 );
        std::map<unsigned long, PaFasTriggeredBroadcastDatabaseAccessor::CfgPaFasTriggeredBroadcastRecord>::iterator item2 
    ...
    void PaFasTriggeredBroadcastListView::OnDraw(CDC* pDC)
    {
    ...
    /////////////////////////////////////////////////////////////////////////////
    // PaFasTriggeredBroadcastListView diagnostics

    #ifdef _DEBUG
    void PaFasTriggeredBroadcastListView::AssertValid() const
    {
	    AbstractListView::AssertValid();
    }

    void PaFasTriggeredBroadcastListView::Dump(CDumpContext& dc) const
    {

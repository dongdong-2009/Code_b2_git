/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3002_TIP/3002/transactive/app/configuration/config_plugins/pa_fas_triggered_broadcast/src/PAFasTriggeredBroadcastFrameworkView.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
    ...
    const int PaFasTriggeredBroadcastFrameworkView::TOP_PANE      = 0;
    const int PaFasTriggeredBroadcastFrameworkView::BOTTOM_PANE   = 1;
    const int PaFasTriggeredBroadcastFrameworkView::ZERO_COLUMN   = 0;

    const int PaFasTriggeredBroadcastFrameworkView::TOP_PANE_SIZE = 200;

    IMPLEMENT_DYNCREATE(PaFasTriggeredBroadcastFrameworkView, CView)


    PaFasTriggeredBroadcastFrameworkView::PaFasTriggeredBroadcastFrameworkView()
    {
    }
...
BEGIN_MESSAGE_MAP(PaFasTriggeredBroadcastFrameworkView, AbstractFrameworkView)
	//{{AFX_MSG_MAP(PaFasTriggeredBroadcastFrameworkView)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

    
    BOOL PaFasTriggeredBroadcastFrameworkView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
    {
    ...
    void PaFasTriggeredBroadcastFrameworkView::OnSize(UINT nType, int cx, int cy) 
    {
    ...
    CRuntimeClass* PaFasTriggeredBroadcastFrameworkView::getRuntimeClassForDetailView()
    {
        return RUNTIME_CLASS(PaFasTriggeredBroadcastDetailView);
    }


    CRuntimeClass* PaFasTriggeredBroadcastFrameworkView::getRuntimeClassForListView()
    {
        return RUNTIME_CLASS(PaFasTriggeredBroadcastListView);
    }

    
    /////////////////////////////////////////////////////////////////////////////
    // PaFasTriggeredBroadcastFrameworkView diagnostics

    #ifdef _DEBUG
    void PaFasTriggeredBroadcastFrameworkView::AssertValid() const
    {
	    AbstractFrameworkView::AssertValid();
    }

    void PaFasTriggeredBroadcastFrameworkView::Dump(CDumpContext& dc) const
    {

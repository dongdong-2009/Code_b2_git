/**
  ...
  * any form.
  *
  * Source:		$File: //depot/FZL1_TIP/FZL1/transactive/app/bas/BASAgent/src/BasModeEquipment.cpp $
  * @author:	Huang Jian
  * @version:	$Revision: #6 $
  *
  * Last modification:	$DateTime: 2016/01/28 11:42:38 $
...
namespace TA_IRS_App
{
    ...
    m_modeValueInit(false)
	{
		...
		m_modeCtrlPt = BasCachedConfig::getInstance().getModeCtrlDPFromNode(m_nodeName);
        ...
        if (NULL != m_modeCtrlPt)
        {
            DataPointUpdateProcessor::instance()->registerDatapointUpdate(m_modeCtrlPt->getEntityKey(), this);
        }
	...
	void BasModeEquipment::executeStationMode(unsigned long modeValue, bool canOverrideStationModeInProgress, const std::string& sessionID)
	{
        ...
        {
            nAutoManStatus = m_modeAutoManual->getCurrentValue().getBoolean() ? eControlHMI : eControlAuto;
        }

		if ( nAutoManStatus != eControlHMI )
		{
		...
		try
		{
			m_modeCtrlPt->WriteDataPointValue(sessionID, dpValue, 0, "");
		}
    ...
    void BasModeEquipment::processDataPointControlStateUpdate( const unsigned long dpEntityKey, const int nControlState, const TA_Base_Bus::DataPointControl& controlState )
    {
        ...
        else if (dpEntityKey == m_modeCtrlPt->getEntityKey())
        {
            ...
            BasEventSubmitter::instance()->submitBasSystemModeExecutionEvent( basMode->getDescription(), nControlState, basMode->getName(), m_modeCtrlPt->getLastOperatorSession(), dpEntityKey, m_modeCtrlPt->getAssetName());
        }
    }

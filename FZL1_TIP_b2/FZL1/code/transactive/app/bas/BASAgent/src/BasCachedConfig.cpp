/**
  ...
  * any form.
  *
  * Source:		$File: //depot/FZL1_TIP/FZL1/transactive/app/bas/BASAgent/src/BasCachedConfig.cpp $
  * @author:	Huang Jian
  * @version:	$Revision: #3 $
  *
  * Last modification:	$DateTime: 2015/01/27 13:48:56 $
...
namespace TA_IRS_App
{
	...
	TA_Base_Bus::DataPoint* BasCachedConfig::getDatapointFromName(const std::string& ptName)
	{
		...
		//TA_ASSERT(!m_dataPoints.empty(), "Points Map should not be empty");
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Get Datapint %s", ptName.c_str());
		...
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugFatal, "Mode function related point [%s] are not found!!", ptName.c_str());
		//TA_ASSERT(false, "The point can not be found");
		return NULL;
	}
	...
	unsigned long BasCachedConfig::isStationFireMonitorValid()
	{
    ...
    std::string BasCachedConfig::getLocDispName()
    {

...
void BasManagerSubscriber::stopSubscriber()
{
    ...
    for (std::map<unsigned long, FireRecoverEquipment_SmartPtr>::iterator it = m_fireRecoverEquipment.begin(); 
...
void BasManagerSubscriber::updateModeStatus(unsigned long locationKey, unsigned long modeKey, unsigned long subsystemKey, 
											EquipmentValueType valueType, double value, int status)
{

...
Equipment::Equipment(unsigned long locationKey, BasManagerSubscriber* basSubscriberPtr, TA_IRS_Core::SubSystem_SmartPtr subsystem, std::string equipmentName)
:m_locationKey(locationKey), m_basSubscriberPtr(basSubscriberPtr),m_subsystem(subsystem), m_equipmentName(equipmentName)
{
...
void Equipment::processEntityUpdateEvent(unsigned long entityKey, TA_Base_Bus::ScadaEntityUpdateType updateType)
{
	...
	BAS_TRY
	{
		...
		{
            if ( !isModeValid( static_cast<unsigned long>(newValue) ) )
            {
		...
		else if (EVT_CurrentExcuteModeStatus == valueType)
		{
            ...
            if ( isModeValid( static_cast<unsigned long>(modeValue) ) )
            {
...
unsigned long Equipment::getEquipmentValue(EquipmentValueType type)
{
    ...
    case EVT_CurrentExcuteModeBit:
        ...
        else if ( (TA_Base_Bus::DPT_DERIVED == m_dpCurrentExcuteModeBit->getType() ) || (TA_Base_Bus::DPT_ENUM == m_dpCurrentExcuteModeBit->getType()) )
        {
    ...
    case EVT_CurrentExcuteModeStatus:
        ...
        else if ( (TA_Base_Bus::DPT_DERIVED == m_dpCurrentExcuteModeStatus->getType()) || (TA_Base_Bus::DPT_ENUM == m_dpCurrentExcuteModeStatus->getType())  )
        {

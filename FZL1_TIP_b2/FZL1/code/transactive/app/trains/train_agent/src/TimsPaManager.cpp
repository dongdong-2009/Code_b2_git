...
namespace TA_IRS_App
{
                                               ...
                                               const std::string& sessionId )
    {
        ...
        // if there is no group ID given
        if ( true == tsi.empty() )
        {
            // collect all train TSIs
            TA_IRS_Bus::CallTypes::ItsiList trainItsiList;
            ...
            // if there is more than 1 item in the ItsiList
            if ( trainItsiList.size() > 1 )
            {
                // a DGNA must be used
                tsi = m_radioAgent.createDynamicGroup( trainItsiList, sessionId );
            }

            // otherwise, a single call can be made
            else
            {
                tsi = trainItsiList[0];
                groupCall = false;
        ...
        // if a group call is to be made
        if ( true == groupCall )
        {
            // make the group PA call
            callDetails = m_radioAgent.makeGroupPaCall( tsi, sessionId );
        }

        // otherwise the call is a single call
        else
        {
            // make the single PA call
            callDetails = m_radioAgent.makeSinglePaCall( tsi, sessionId );
        }
                                             ...
                                             const std::string& sessionId )
    {
        ...
        try
        {
            // the call for the live announcement needs to be retreived
            TA_IRS_Bus::StateUpdateTypes::RadioCallDetails callDetails =
                m_statusManager.getCallDetailsForLiveAnnouncement( announcementId );

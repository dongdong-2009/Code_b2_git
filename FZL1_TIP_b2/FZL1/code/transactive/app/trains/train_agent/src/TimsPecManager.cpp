...
namespace TA_IRS_App
{
          ...
          m_cctvManager( cctvManager )
    {
        ...
        m_transactionManager.registerForTrainEvents( TA_IRS_Bus::PecReadyForConversationEvent::getStaticType(), this );
                                    ...
                                    const std::string& sessionId )
    {
...

        {
            ...
            {
                TA_THROW( TA_IRS_Bus::PecException( "PEC is in use by another operator",
            ...
            pecState->pecAnswered( car, pec, time( NULL ), sessionId, 0, 0 );
            ...
            while ( false == answered )
            {
                try
                {
...

                    {
                        ...
                        // send the command using the TimerPEC2 from m_configuration, but set the number of retries to 0, the retry logic must be implemented here
                    ...
                    // subtract the elapsed time from the timerPEC2 and use this to start the timer
                    time_t timeOut = m_configuration.getTimerPEC2() - elapsedTime;
                    ...
                    {//limin++, TD20554
                        ...
                        /*limin--, TD20554
                        // using m_transactionManager.startTrainTimer start the timer (for TrainPecResource)
                        m_transactionManager.startTrainTimer( trainId,
                                                              TA_IRS_Bus::CommonTypes::TrainPecResource,
                                                              timeOut );

                        // the timer is running, now the call can be made to the train
                        --limin*/
                        ...
                        TA_IRS_Bus::StateUpdateTypes::RadioCallDetails callDetails = m_radioAgent.makePecCall( activeTsi,
                                                                                                               sessionId );
                ...
                catch ( TA_IRS_Bus::TrainTimerException& e )
                {
                    ...
                    endRadioCall( trainId, car, pec, sessionId );
        ...
        catch ( ... )
        {
...

            {
                ...
                pecState->pecActivated( car, pec, time( NULL ) );
        ...
        */
        ...
        if ( true == answeredByOperator )
        {
            ...
            try
            {
                ...
                m_radioAgent.endCall( callId, sessionId );
            }
             ...
             ( true == checkPecValidity( trainId ) ) )
        {
            ...
            try
            {
                ...
                TA_IRS_Bus::PecResetEventPtr resetCommand = command->getPecResetResponse();
    ...
    */
			...
			TA_IRS_Bus::ProtocolPecTypes::Request == rqsOrAck )
        {
            ...
            TA_IRS_Bus::PecResetCommandPtr command(
                new TA_IRS_Bus::PecResetCommand( trainId,
                                                 ...
                                                 car,
                                                 pec,
												 TA_IRS_Bus::ProtocolPecTypes::Acknowledge,
                                                 false ) );
    ...
    void TimsPecManager::processPecSelectedByDriverEvent( TA_IRS_Bus::PecSelectedByDriverEventPtr event )
    {
        ...
        if ( 0 != answeredCallId )
        {
            ...
            {
                // end the call using m_radioAgent
                m_radioAgent.endCall( answeredCallId, answeredSessionId );
            }
            // catch and log any exceptions
            catch ( TA_IRS_Bus::TrainRadioException& e )
            {

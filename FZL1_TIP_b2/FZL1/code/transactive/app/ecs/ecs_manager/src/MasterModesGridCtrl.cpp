/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/MasterModesGridCtrl.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
// using TA_IRS_App::TA_ECSManagerCommonDefs::MasterModeDetails;
// using TA_IRS_App::TA_ECSManagerCommonDefs::ECurrentStationModeType;
// using TA_IRS_App::TA_ECSManagerCommonDefs::EModeProposalState;

typedef TA_IRS_App::TA_ECSManagerCommonDefs::LocationNames LocationNames;
typedef TA_IRS_App::TA_ECSManagerCommonDefs::MasterModeDetails MasterModeDetails;
typedef TA_IRS_App::TA_ECSManagerCommonDefs::ECurrentStationModeType ECurrentStationModeType;
typedef TA_IRS_App::TA_ECSManagerCommonDefs::EModeProposalState EModeProposalState;

//using TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_MODE_COLORREFS;
// using TA_IRS_App::TA_ECSManagerCommonDefs::CURRENTLY_PROPOSED_COLORREF;
// using TA_IRS_App::TA_ECSManagerCommonDefs::CURRENTLY_OVERRIDING_PROPOSED_COLORREF;
...
// using TA_IRS_App::TA_ECSManagerCommonDefs::FLASH_INTERVAL;
// using TA_IRS_App::TA_ECSManagerCommonDefs::FLASH_TIMER_ID;
// using TA_IRS_App::ECSManagerConfigurationAccess;
// using TA_IRS_App::ECSManagerConfigurationAccess::NORMAL_MASTER_MODES;

#define CURRENTLY_PROPOSED_COLORREF TA_IRS_App::TA_ECSManagerCommonDefs::CURRENTLY_PROPOSED_COLORREF
#define CURRENTLY_OVERRIDING_PROPOSED_COLORREF TA_IRS_App::TA_ECSManagerCommonDefs::CURRENTLY_OVERRIDING_PROPOSED_COLORREF
...
#define FLASH_INTERVAL TA_IRS_App::TA_ECSManagerCommonDefs::FLASH_INTERVAL
#define FLASH_TIMER_ID TA_IRS_App::TA_ECSManagerCommonDefs::FLASH_TIMER_ID
typedef TA_IRS_App::ECSManagerConfigurationAccess ECSManagerConfigurationAccess;
...
namespace TA_IRS_App
{
    ...
    const int INITIAL_LOCATION_COLUMN_WIDTH = 41;
    ...
    void MasterModesGridCtrl::setLocations( const LocationNames& locationNames )
    {
        ...
        TA_ASSERT( ( false == locationNames.empty() ), "Attempt to initialise Master Modes Grid with empty list of locations." );

		static int numLocations = locationNames.size();
    ...
    void MasterModesGridCtrl::populateMasterModeDetails( const MasterModeDetails& masterModeDetails )
    {
        ...
        int insertIndex;
        int associatedModeIndex;
        StationModesAsStrings stationModesAsStr;
        StationModesAsStrings::const_iterator stationModesAsStrIt;
        MasterModeDetails::const_iterator masterModeDetailsIt = masterModeDetails.begin();
        while ( masterModeDetailsIt != masterModeDetails.end() )
        {
            ...
            {	
                ...
                SetItemText( insertIndex, 3, _T( ( *masterModeDetailsIt ).zoneLocation.c_str() ) );
            ...
            stationModesAsStr = ( *masterModeDetailsIt ).associatedStationModesAsStrings;
            stationModesAsStrIt = stationModesAsStr.begin();
    
            TA_ASSERT( ( ( GetColumnCount() - GetFixedColumnCount() ) == static_cast< int >( stationModesAsStr.size() ) ), 
                "Number of Associated Station Modes do not correspond with number of locations." );

            for ( associatedModeIndex = GetFixedColumnCount(); associatedModeIndex < GetColumnCount(); associatedModeIndex++ )
            {	
                               ...
                               ECurrentStationModeType currentStationModeType )
    {
		...
		//if (currentStationModeType == TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_MODE_FAILED)
...
// 						TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(stationModeType) );
                    ...
                    {
                        EModeProposalState proposalState = static_cast< EModeProposalState >
                            ( GetItemData( ( *cellsReqFlashIt ).first, ( *cellsReqFlashIt ).second ) );
                        if ( CURRENTLY_PROPOSED == proposalState )
                        {
                            for ( int col = 0; col < GetColumnCount(); col++ )
                            {
   ...
   void MasterModesGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
    ...
    void MasterModesGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
    {
        ...
        if (m_MouseMode == MOUSE_OVER_COL_DIVIDE) 
        {
            ...
            TA_ASSERT( IsValid(resizingCell), "IsValidCell( GetCellFromPt() ) failed while mouse poition flagged as being over Column divide." );
            CPoint start;
 ...
 /*   CCellID MasterModesGridCtrl::SetFocusCell(CCellID cell)
    ...
    void MasterModesGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
    {
        ...
        if ( (m_MouseMode == MOUSE_SIZING_COL) && (m_LeftClickDownPoint != point) )
        {
            ...
            TA_ASSERT( ( colBeingResized < static_cast< int >( m_isCenteredTextVector.size() ) ),
                "Column being resized has no corresponding entry in m_isCenteredTextVector." );
        }
	...
	void MasterModesGridCtrl::setCurrentStationModeLock(int locationIndex, bool isStationLocked)
	{
		...
		{	// Set the Lock Station Colour.
			SetItemBkColour( cellCoord.first, cellCoord.second, 
				TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_LOCK) );
		}

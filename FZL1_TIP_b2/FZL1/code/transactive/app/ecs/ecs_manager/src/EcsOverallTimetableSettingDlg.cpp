/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/EcsOverallTimetableSettingDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
#define WM_ENABLE_CLOSE_Button (WM_APP+99)

#define CURRENT_TIMETABLE_DP	"TIM-Current"
#define SELECT_TIMETABLE_DP		"TIM-Select"
...
namespace TA_IRS_App
{
		...
		: TransActiveDialog( genericGUICallback, COverallEcsTimetableSettingDlg::IDD, pParent )
	{
        ...
        properties.minWidth		= 421;
        properties.minHeight	= 332;
	...
	ON_MESSAGE(WM_ENABLE_CLOSE_Button,onEnableCloseButton)
...
//                                    CString strStation = m_OverallECSTimetableList.GetItemText(row, 0);
//                                    CString status("Received current timetable from location ");
//                                    status += strStation;
        ...
        if (updateType == TA_Base_Bus::ControlStateUpdate)
        {
            try
            {
                ...
                for (itSelect = m_selectTimetableDataPoints.begin(); itSelect != m_selectTimetableDataPoints.end(); ++ itSelect)
                {
                    ...
                    if ((*dp)->getEntityKey() == entityKey/* && 
                        (*dp)->getLastStatusChangeSessionId() == TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID)*/)
                    {
...
//		SetTimer(TIMER_ID, TIMER_INTERVAL, NULL);
	...
	DWORD WINAPI COverallEcsTimetableSettingDlg::ThreadFunc( LPVOID lpThreadParameter )
	{
		...
		CMC->PostMessage(WM_ENABLE_CLOSE_Button, wParam, 0 );
		return TRUE;
	}
...
//		LoadStations();
	...
	void COverallEcsTimetableSettingDlg::initialiseDataPointsProxy()
	{
		...
		/*for (iter = orderedLocations.begin(); iter != orderedLocations.end(); ++iter)
		{
			try
			{
				TA_Base_Core::IEntityData* cuurentityData = curentities[entityCount];
                ...
                TA_Base_Bus::DataPointProxySmartPtr currentTimetableDataPoint = m_proxyFactory->createDataPointProxy(currdpEntityData, *this);
                m_currentTimetableDataPoints[(*iter).key] = currentTimetableDataPoint;                    
                LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Current timetable datapoint proxy [%u] created", cuurentityData->getKey());
				
				TA_Base_Core::IEntityData* selentityData = selentities[entityCount]; 						
 				boost::shared_ptr<TA_Base_Core::DataPointEntityData> seldpEntityData( dynamic_cast<TA_Base_Core::DataPointEntityData*>(selentityData) );
                TA_Base_Bus::DataPointProxySmartPtr selectTimetableDataPoint = m_proxyFactory->createDataPointProxy(seldpEntityData, *this);
                m_selectTimetableDataPoints[(*iter).key] = selectTimetableDataPoint;
 
                LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Select timetable datapoint proxy [%u] created", selentityData->getKey());

				TA_Base_Core::IEntityData* staentityData = staentities[entityCount];

				boost::shared_ptr<TA_Base_Core::DataPointEntityData> stadpEntityData( dynamic_cast<TA_Base_Core::DataPointEntityData*>(staentityData) );
             
                TA_Base_Bus::DataPointProxySmartPtr stationVisualLockDataPoint = m_proxyFactory->createDataPointProxy(stadpEntityData, *this);
                m_stationVisualLockDataPoints[(*iter).key] = stationVisualLockDataPoint;

                LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Staion Visual datapoint proxy [%u] created", staentityData->getKey());
			}
			catch (TA_Base_Core::ScadaProxyException &e)
             {
                LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "ScadaProxyException caught: %s", e.what());

             CString status("Load timetable of location ");
             status += (*iter).name.c_str();
               status += " failed!";
                WriteStatusLine(status);
		...
		/*for (iter = orderedLocations.begin(); iter != orderedLocations.end(); ++iter)
		{
			try
            {
                //create current timetable datapoint proxy
                ...
                TA_Base_Core::IEntityDataList entities = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocationWithNameLikeToken
                    ( TA_Base_Core::DataPointEntityData::getStaticType(), (*iter).key, sqlToken);
            
                if ( entities.size() != 1 )
                {
                    CString status("Failed! DataPoint for current timetable is not defined in location ");
                    status += (*iter).name.c_str();
                    WriteStatusLine(status);
                ...
                else
                {
                    ...
                    TA_Base_Bus::DataPointProxySmartPtr currentTimetableDataPoint = m_proxyFactory->createDataPointProxy(dpEntityData, *this);
                    m_currentTimetableDataPoints[(*iter).key] = currentTimetableDataPoint;
                    
                    LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Current timetable datapoint proxy [%u] created", entityData->getKey());
                }
                
                //create select timetable datapoint proxy
                ...
                if ( entities.size() != 1 )
                {
                    CString status("Failed! DataPoint for select timetable is not defined in location ");
                    status += (*iter).name.c_str();
                    WriteStatusLine(status);
                    LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Invalid number of datapoints for %s", sqlToken.c_str() );  //TD14337 yezh++      
                }
                else
                {
                    ...
                    TA_Base_Bus::DataPointProxySmartPtr selectTimetableDataPoint = m_proxyFactory->createDataPointProxy(dpEntityData, *this);
                    m_selectTimetableDataPoints[(*iter).key] = selectTimetableDataPoint;

                    LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Select timetable datapoint proxy [%u] created", entityData->getKey());
                }
                ...
                if ( entities.size() != 1 )
                {
                    CString status("Failed! DataPoint for station visual lock is not defined in location ");
                    status += (*iter).name.c_str();
                    WriteStatusLine(status);
                    LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Invalid number of datapoints for %s", sqlToken.c_str() );  //TD14337 yezh++      
                }
                else
                {
                    ...
                    TA_Base_Bus::DataPointProxySmartPtr stationVisualLockDataPoint = m_proxyFactory->createDataPointProxy(dpEntityData, *this);
                    m_stationVisualLockDataPoints[(*iter).key] = stationVisualLockDataPoint;
                ...
                {	
                    ...
                    if (itSelect != m_selectTimetableDataPoints.end())
                    {
                        ...
                        else
                        {
                            ...
                            LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "The select datapoint derived states are not well configed for this station");
                        }
                    }
                    else
                    {
						...
						statusString += strStation;
						WriteStatusLine(statusString.c_str());
                        LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "The select datapoint is not found for this station");
                    }
                }
            }
            catch (TA_Base_Core::ScadaProxyException &e)
	...
	void COverallEcsTimetableSettingDlg::OnSize(UINT nType, int cx, int cy)
	{
		...
		if(m_areOffsetsSetup && nType != SIZE_MAXHIDE && nType != SIZE_MINIMIZED)
        {
            ...
            {
                LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Child windows were not constructed yet.");
                return;
            }
		...
		std::map<unsigned long, TA_Base_Bus::DataPointProxySmartPtr*>& resultMap )
	{
		...
		{	
			...
			if (std::find(currentLocations.begin(), currentLocations.end(), location) != currentLocations.end())
			{
				...
				{
					LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Create DataPoint Proxy: ScadaProxyException caught: %s", e.what());
				}
				catch (...)
				{
					LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Create DataPoint Proxy: UnknownException caught");
				}

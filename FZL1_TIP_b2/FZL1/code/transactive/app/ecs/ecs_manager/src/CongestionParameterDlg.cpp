...
namespace TA_IRS_App
{
	...
	void CCongestionParameterDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
	{
		if (m_isInitialised)
		{
            if(&m_Temp_Slider == dynamic_cast<CSliderCtrl*>(pScrollBar))
            {
            ...
            else if(&m_Temp_DeadBand_Slider == dynamic_cast<CSliderCtrl*>(pScrollBar))
            {
	...
	{	
        try
        {
			...
			bool bUsesStallTime, bUsesTemperature;
			int	nTemperature,nTempDeadBand, nStallTime;
			std::string value;
			...
			{			
				...
				CString s;
				s.Format("%d", nTemperature);
				m_TempLabel.SetWindowText(s);
	...
	{		
		...
		{	
			...
			m_outParams.bUsesStallTime = true;
		...
		{
			maxTrainStallTime.Format("0");
			m_outParams.bUsesStallTime = false;
			m_outParams.nStallTime = 0;
		...
		{	
			...
			m_outParams.bUsesTrainTemp = true;
		...
		{
			str.Format("%d", MIN_TRAIN_TEMP);
			maxTemperatureStr = str;
			str.Format("%d", MIN_TRAIN_DEADBAND);
			temperatureDeadbandStr = str;
			m_outParams.bUsesTrainTemp = false;
			m_outParams.nTrainTemp = MIN_TRAIN_TEMP;
            m_outParams.nTempDeadBand = MIN_TRAIN_DEADBAND;

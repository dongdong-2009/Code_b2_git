/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/StationModesOverviewGridCtrl.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
using TA_IRS_App::TA_ECSManagerCommonDefs::ECurrentStationAutoModeType;
...
namespace TA_IRS_App
{
    ...
    void StationModesOverviewGridCtrl::setLocations( const LocationNames& locationNames )
    {
        ...
        TA_ASSERT( static_cast< int >( locationNames.size() ) < NUM_COLUMNS,
            "StationModesOverviewGridCtrl::setLocations called with vector of location names greater than number of columns for displaying them." );
    ...
    //
    StationModesOverviewGridCtrl::LocationIndexAndProposedType StationModesOverviewGridCtrl::getProposedStationModeFromCellCoordinates( int row, int col )
    {
	...
	void StationModesOverviewGridCtrl::setCurrentStationAutoModeControlState( int locationIndex, TA_IRS_App::TA_ECSManagerCommonDefs::ECurrentStationAutoModeType autoModeType)
	{
...
// 								TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(stationModeType));
    ...
    BOOL StationModesOverviewGridCtrl::DrawCell( CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk )
    {
		...
		if(nRow == CURRENT_STATION_AUTO_MODE_ROW && nCol <=m_numLoadedLocations)
		{
			...
			if(stationAutoModeType == TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_OFF)
			{
				pDC->DrawFrameControl(rect,DFC_BUTTON,DFCS_BUTTONPUSH);			
				SetItemText(nRow,nCol,"");
			}
			else if(stationAutoModeType == TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_ON)					
			{
	...
	void StationModesOverviewGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
	{
		...
		if (clickDownCell.col == -1)
		{
			...
			for( int iCt =1; iCt <= m_numLoadedLocations; iCt++ )
			{
				...
				SetItemData( CURRENT_STATION_AUTO_MODE_ROW, iCt, static_cast<ECurrentStationAutoModeType>(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_OFF ) );	
			}
		...
		if( clickDownCell.row == CURRENT_STATION_AUTO_MODE_ROW && clickDownCell.col != CONTENT_COLUMN )	//CL17276
		{
			...
			if(stationAutoModeType == TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_OFF)
			{
				stationAutoModeType = TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_ON;
			...
			{
				stationAutoModeType = TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_OFF;
			...
			else if (nFlags & MK_CONTROL)
			{
				...
				for (POSITION pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; )
				{
					...
					if (cell.row == CURRENT_PROPOSED_NORMAL_ROW)
					{
						...
						{
							m_selectedrowProposedTypes.push_back(std::make_pair((cell.col - NUM_FIXED_COLUMNS), TA_ECSManagerCommonDefs::PROPOSED_FOR_NORMAL));
						}
					}
					else if (cell.row == CURRENT_PROPOSED_CONGESTION_ROW)
					{
						...
						{
							m_selectedrowProposedTypes.push_back(std::make_pair((cell.col - NUM_FIXED_COLUMNS), TA_ECSManagerCommonDefs::PROPOSED_FOR_CONGESTION));
						}
					}
				}
	...
	void StationModesOverviewGridCtrl::OnFixedColumnClick(CCellID& cell)
	{
		...
		if ( cell.col == 0 && cell.row == CURRENT_STATION_AUTO_MODE_ROW )
		{
			...
			else
			{
				...
				for( int iCt =1; iCt <= m_numLoadedLocations; iCt++ )
				{
					m_selectedrowAutoMode.insert( std::make_pair( ( iCt - NUM_FIXED_COLUMNS ),  TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_ON ) );	
					SetItemData( CURRENT_STATION_AUTO_MODE_ROW, iCt, static_cast<ECurrentStationAutoModeType>(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_AUTO_MODE_ON ) );	
				}
				Invalidate();
...
// 				}
				if (cell.row == CURRENT_PROPOSED_NORMAL_ROW)
				{
					...
					if (FALSE == normalState.IsEmpty())
					{
						m_selectedrowProposedTypes.push_back(std::make_pair((iCt - NUM_FIXED_COLUMNS), TA_ECSManagerCommonDefs::PROPOSED_FOR_NORMAL));
					}
				...
				else if (cell.row == CURRENT_PROPOSED_CONGESTION_ROW)
				{
					...
					if (FALSE == congestionState.IsEmpty())
					{
						m_selectedrowProposedTypes.push_back(std::make_pair((iCt - NUM_FIXED_COLUMNS), TA_ECSManagerCommonDefs::PROPOSED_FOR_CONGESTION));
					}

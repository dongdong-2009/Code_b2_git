/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/ECSStationModes.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
// using TA_IRS_App::TA_ECSManagerCommonDefs::StationModeDetails;
...
typedef TA_IRS_App::TA_ECSManagerCommonDefs::StationModeDetails StationModeDetails;
...
namespace TA_IRS_App
{
	...
	//++zn
	//zn++ - CL-17285 ITEM1
	m_curStationModeIndexApplying(-1),
	//++zn
    ...
    //
    void ECSStationModes::stationModeStatusUpdated( unsigned long locationKey, bool isValid, int value )
    {
        ...
        if ( ( m_currentLocationSelectionIndex > -1 ) && ( m_currentLocationSelectionIndex < static_cast< int >( m_locationKeys.size() ) ) )
        {
            if ( locationKey == m_locationKeys[ m_currentLocationSelectionIndex ] )
            {
                ...
                else
                {
					...
					//zn++ - CL-17285 ITEM1
					if (m_curStationModeIndexApplying != -1)	// means an applied station mode is processing
					{
						//should check whether the apply mode is in propose or overriding propose, 
						//if all no, means manually override, then make it steady
						bool foundProposed(false), foundOverrideProposed(false);
						int appliedMode = m_currentStationModeDetails[ m_curStationModeIndexApplying ].stationMode;
						
						CurrentProposedModeIt itr = std::find(m_currentProposed.begin(), m_currentProposed.end(), appliedMode);
						...
						{
							if(m_currentOverridingStationModes[i].mode == appliedMode)
							{
								foundOverrideProposed = true;
								break;
							}
						}
						
						if (!foundProposed && !foundOverrideProposed)
						{
							LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "[17285] will set %d to override", m_curStationModeIndexApplying);
							m_ecsStationModesView.setCurrentAppliedMode(m_curStationModeIndexApplying, true);
						}
						else
						{
							m_ecsStationModesView.setCurrentAppliedMode(m_curStationModeIndexApplying, false);
						}
					}
					//++zn
                }
	...
	void ECSStationModes::stationVisualLockUpdated( unsigned long locationKey, bool isValid, int value )
	{
        ...
        if ( ( m_currentLocationSelectionIndex > -1 ) && ( m_currentLocationSelectionIndex < static_cast< int >( m_locationKeys.size() ) ) )
        {
			...
			if(index != -1)
			{
				...
				if(isStationLocked)
				{
					colour = TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_LOCK);
				}
				else
				{
				...
				{		
					if ( isValid )
					{
						...
						if ( ( matchesCurrentStationModeDetail ) && !isProposedModeInOverridingMap(value))
						{
							...
							//zn++ - CL-17285 ITEM1
							if((stationModeDetailsIt - m_currentStationModeDetails.begin()) == m_curStationModeIndexApplying)
							{
								m_curStationModeIndexApplying = -1;
								m_ecsStationModesView.setCurrentAppliedMode(-1, false);
							}
							//++zn
					}
				}
...
//			}
    ...
    void ECSStationModes::applyStationMode( int stationModeSelectionIndex )
    {
        ...
        TA_ASSERT( ( ( stationModeSelectionIndex < static_cast< int >( m_currentStationModeDetails.size() ) ) && ( stationModeSelectionIndex > -1 ) ),
		...
		// ++ TD14164 
        {
			...
			while(tryAgain)
			{
				try
				{
					...
					//clean last applied steady first
					if (m_curStationModeIndexApplying != -1)
					{
						m_ecsStationModesView.setCurrentAppliedMode(m_curStationModeIndexApplying, false);
					}
					m_curStationModeIndexApplying = -1;
					...
					//zn++ CL-17285 ITEM1
					m_curStationModeIndexApplying = stationModeSelectionIndex;
					//++zn
				}
				...
				//zn++ CL-17285 ITEM1 - this change is no need base on current proxy lib
    ...
    void ECSStationModes::locationSelectionChanged( int locationIndex )
    {
		...
		//zn++ - CL-17285 ITEM1: clear the steady selection
		if (m_curStationModeIndexApplying != -1)
		{
			m_ecsStationModesView.setCurrentAppliedMode(m_curStationModeIndexApplying, false);
		}
    ...
    void ECSStationModes::loadDetailsForLocation()
    {
        ...
        if ( ( m_currentLocationSelectionIndex > -1 ) && ( m_currentLocationSelectionIndex < static_cast< int >( m_locationKeys.size() ) ) )
        {
            ...
            {
                m_currentStationModeDetails.push_back( m_pMappedECSDataCache->getStationModeDetailFromKey
                    ( ( *stationModeKeysIt ) ) );
                stationModeKeysIt++;
            }

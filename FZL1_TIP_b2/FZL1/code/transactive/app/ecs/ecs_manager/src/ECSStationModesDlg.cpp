/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/ECSStationModesDlg.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_App
{
	...
	//caiwei++
	/*
    const int OPERATION_WIDTH                   = 180;
    const int TRACK_WIDTH                       = 60;
    const int CONDITION_WIDTH                   = 170;
	*/
    ...
    const COLORREF NORMAL_COLOUR                = ::GetSysColor( COLOR_BTNFACE );
	...
	m_isInProgressBlinking(false)
    {
        ...
        properties.minWidth = MINIMUM_DIALOG_WIDTH;
        properties.minHeight = MINIMUM_DIALOG_HEIGHT;
...
// 		// ++CL17285
	...
	LRESULT ECSStationModesDlg::onSetCurrentStationModeLock( WPARAM wParam, LPARAM lParam )
	{
		...
		if(isStationLocked)
		{
			COLORREF colour = TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_LOCK);
	...
	LRESULT ECSStationModesDlg::onSetInboundStationLock( WPARAM wParam, LPARAM lParam )
	{
		...
		if(isStationLocked)
		{
			m_inboundLocationDisplay.setBackgroundColour(TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_LOCK));
		}
		else // FALSE
		{
	...
	LRESULT ECSStationModesDlg::onSetOutboundStationLock( WPARAM wParam, LPARAM lParam )
	{
		...
		if(isStationLocked)
		{
			m_outboundLocationDisplay.setBackgroundColour(TA_IRS_App::ECSColourAccessor::getInstance().returnColourGivenStationModeType(TA_IRS_App::TA_ECSManagerCommonDefs::CURRENT_STATION_LOCK));
		}
		else // FALSE
		{

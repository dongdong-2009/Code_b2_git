/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/ecs/ecs_manager/src/StationModesDataPointManager.cpp $
  * @author:  Robert Young
  * @version: $Revision: #1 $
  *
...
const UINT WM_ENABLE_CLOSE = RegisterWindowMessage("Enable_Close_Button");
...
class TA_Base_Bus::ScadaProxyFactory;
...
namespace TA_IRS_App
{
	...
	void StationModesDataPointManager::deregisterStationModeStatusObserver( IStationModeStatusObserver* observer )
	{
		...
		if( std::binary_search( m_registeredStationModeStatusObservers.begin(), m_registeredStationModeStatusObservers.end(),  observer ) )
        {		
		    StationModeStatusObservers::iterator stationModeStatusObserversIt
			    =  std::lower_bound( m_registeredStationModeStatusObservers.begin(), m_registeredStationModeStatusObservers.end(),  observer );	
		    m_registeredStationModeStatusObservers.erase( stationModeStatusObserversIt );
        }
        else
        {
    ...
    //
	void StationModesDataPointManager::refreshDataPointProxies( const TA_ECSManagerCommonDefs::LocationKeys& locationKeys, 
			bool doStatusAndControl, bool doClearAllExisting /*=true*/ )
	{
		...
		{
			// Reduce manager launching time - Jiang shengguang++
			//populateStatusAndControlDataPointProxies( sortedLocationKeys );
			if(m_LocationKeys.size()>0)
			{
				m_LocationKeys.clear();
			}
			m_LocationKeys = sortedLocationKeys;
			s_thread_hd = CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)ThreadFunction,this,0,&s_thread_id);
			CloseHandle(s_thread_hd);
			// ++Jiang shengguang
	...
	void StationModesDataPointManager::populateStatusAndControlDataPointProxies( const LocationKeys& locationKeys )
	{
		...
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "unknow exception catched." );
		}
	...
	// Reduce manager launching time - Jiang shengguang++
	DWORD WINAPI StationModesDataPointManager::ThreadFunction(LPVOID lpThreadParameter)
	{
		StationModesDataPointManager* SMDPM = (StationModesDataPointManager*) lpThreadParameter;
		SMDPM->populateStatusAndControlDataPointProxies(SMDPM->m_LocationKeys);
		WPARAM wParam = static_cast< WPARAM >( TRUE );
		PostMessage( HWND_BROADCAST, WM_ENABLE_CLOSE, wParam, 0 );
		return TRUE;
	}
	// ++ Jiang shengguang
		...
		EStationModeDataPointType stationModeDataPointType )
	{
		...
		TA_ASSERT( ( ( stationModeDataPointType > -1 ) && ( stationModeDataPointType < NUMBER_OF_STATIONMODE_DATAPOINT_TYPES ) ),
			"StationModesDataPointManager::populateProxiesFromDataPointEntityData called with invalid EStationModeDataPointType" );
			 ...
			 ++DataPointEntityDataIt)
		{
			...
			if ( std::binary_search(locationKeys.begin(), locationKeys.end(), locationKey) )
			{
				...
				m_locationKeyToStationModeDataPointKeySet[locationKey][stationModeDataPointType] = entityKey;
			}	
		}
	...
	void StationModesDataPointManager::populateProxiesForAutoModeControl(const TA_ECSManagerCommonDefs::LocationKeys& locationKeys)
	{
    ...
    //
	void StationModesDataPointManager::cleanUpSpecifiedDataPointProxies( const TA_ECSManagerCommonDefs::LocationKeys& locationKeys, 
						bool doStatusAndControl)
	{
		...
		for( locationKeyToStationModeDataPointKeySetIt = m_locationKeyToStationModeDataPointKeySet.begin();
			 ...
			 ++locationKeyToStationModeDataPointKeySetIt)
		{
			if ( std::binary_search(locationKeys.begin(), locationKeys.end(),
					(*locationKeyToStationModeDataPointKeySetIt).first) &&
				 doStatusAndControl)
			{
...
// 				TA_ECSManagerCommonDefs::ECurrentStationAutoModeType currentStationAutoModeType)
// 	{

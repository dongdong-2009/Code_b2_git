/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/3001_Review_Branch_DBSync/3001/transactive/core/data_access_interface/telephone/telephone_direct_lines_dai/src/DatabaseQueryHelper.cpp $
  * @author:  Jade Lee
  * @version: $Revision: #2 $
  *
...
bool DatabaseQueryHelper::checkIfExistLinksToTableRow(
	...
	const std::string& checkTablePrimaryKeyValue,
                std::string& info)
{
	FUNCTION_ENTRY("checkIfExistLinksToTableRow");
...
//	std::string strSql  = databaseConnection->prepareSQLStatement(SC_OPC_TAG_ALIAS_SELECT_33056, tableName);
		TA_Base_Core::SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, SC_OPC_TAG_ALIAS_SELECT_33056, tableName);
...
bool DatabaseQueryHelper::isWholeWordPresent(std::string input, std::string word)
{
    ...
    FUNCTION_ENTRY("isWholeWordPresent");
...
{        
    FUNCTION_ENTRY("appendWhereClause");
...
{
	FUNCTION_ENTRY("createBasicSelectStatement");

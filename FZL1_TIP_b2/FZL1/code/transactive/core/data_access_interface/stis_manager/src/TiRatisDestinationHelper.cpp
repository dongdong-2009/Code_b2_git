/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/core/data_access_interface/stis_manager/src/TiRatisDestinationHelper.cpp $
  * @author:  chunzhong
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_Core
{
    ...
    void TiRatisDestinationHelper::deleteTiRatisDestination(bool cascade)
    {
		...
		/*TI_RATIS_DEST_DELETE_55901=delete from ti_ratis_dest where TIRDES_ID = %s*/

		TA_Base_Core::SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_DELETE_55901, m_key.getValue());
    ...
    void TiRatisDestinationHelper::modifyExistingTiRatisDestination() //throw(TA_Base_Core::TransactiveException)
    {
		...
		/*TI_RATIS_DEST_UPDATE_55651=update ti_ratis_dest set DESTINATION = '%s' where TIRDES_ID = %s*/

		TA_Base_Core::SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_UPDATE_55651, 
			(databaseConnection->escapeInsertString(m_destination)).c_str(), m_key.getValue());
    ...
    void TiRatisDestinationHelper::addNewTiRatisDestination() //throw(TA_Base_Core::TransactiveException)
    {
        ...
        sql << "insert into " << TI_RATIS_DESTINATION_TABLE << " (" 
			...
			<< m_destination <<  "')";*/
		/*TI_RATIS_DEST_INSERT_55801=insert into ti_ratis_dest (TIRDES_ID,DESTINATION) values (%s, '%s')*/

		TA_Base_Core::SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_INSERT_55801, m_key.getValue(), m_destination);
        //databaseConnection->executeModification(sql.str());
		databaseConnection->executeModification(sql);        
    ...
    void TiRatisDestinationHelper::validateKeyExistence() //throw(TA_Base_Core::TransactiveException)
    {
        ...
        sql << "select " << KEY_COL
            ...
            << " where " << KEY_COL << " = " << m_key.getValue();*/
		/*TI_RATIS_DEST_SELECT_55501=select TIRDES_ID from ti_ratis_dest where TIRDES_ID = %s*/
		TA_Base_Core::SQLStatement sql;
		...
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_SELECT_55501, m_key.getValue());
    ...
    void TiRatisDestinationHelper::validateDestinationAbsence()
    {
		...
		/*TI_RATIS_DEST_SELECT_55502=select TIRDES_ID from ti_ratis_dest where DESTINATION = '%s'*/
        std::vector<std::string> columnNames;
        columnNames.push_back(KEY_COL);
		...
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_SELECT_55502, m_destination);
    ...
    bool TiRatisDestinationHelper::isKeyDuplicate(unsigned long keyToUse)
    {
		...
		TA_Base_Core::SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_DEST_SELECT_55501, keyToUse);

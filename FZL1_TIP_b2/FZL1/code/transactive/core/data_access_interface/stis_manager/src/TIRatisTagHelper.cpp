/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/core/data_access_interface/stis_manager/src/TIRatisTagHelper.cpp $
  * @author:  chunzhong
  * @version: $Revision: #1 $
  *
...
namespace TA_IRS_Core
{
    ...
    void TiRatisTagHelper::deleteTiRatisTag(bool cascade)
    {
		...
		/*TI_RATIS_TAG_DELETE_56401=delete from ti_ratis_tag where TIRTAG_ID = %s*/
		TA_Base_Core::SQLStatement sql;

		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_DELETE_56401, m_key.getValue());
    ...
    void TiRatisTagHelper::modifyExistingTiRatisTag() //throw(TA_Base_Core::TransactiveException)
    {
		...
		/*
		TI_RATIS_TAG_UPDATE_56151=update ti_ratis_tag set TAG_GROUP = '%s',DETAIL = '%s' where TIRTAG_ID = %s
		*/
		TA_Base_Core::SQLStatement sql;

		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_UPDATE_56151, 
			(databaseConnection->escapeInsertString(m_tagGroup)).c_str(), 
			(databaseConnection->escapeInsertString(m_detail)).c_str(),
			m_key.getValue());
    ...
    void TiRatisTagHelper::addNewTiRatisTag() //throw(TA_Base_Core::TransactiveException)
    {
        ...
        sql << "insert into " << TI_RATIS_TAG_TABLE_NAME << " (" 
			...
			<< m_tagGroup << "','"
            << m_detail << "')";*/
		/*TI_RATIS_TAG_INSERT_56301=insert into ti_ratis_tag (TIRTAG_ID,TAG_GROUP,DETAIL) values (%s, '%s', '%s')*/
		TA_Base_Core::SQLStatement sql;

		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_INSERT_56301, 
			m_key.getValue(), m_tagGroup, m_detail);
    ...
    void TiRatisTagHelper::validateKeyExistence() //throw(TA_Base_Core::TransactiveException)
    {
		...
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_SELECT_56001, m_key.getValue());
    ...
    void TiRatisTagHelper::validateKeyAbsence() //throw(TA_Base_Core::TransactiveException)
    {
		...
		databaseConnection->prepareSQLStatement(sql, ME_ALARM_TYPE_SELECT_1552, m_key.getValue());
    ...
    void TiRatisTagHelper::validateGroupDetailPairUniqueness()  //throw(TA_Base_Core::TransactiveException)
    {
		...
		/*TI_RATIS_TAG_SELECT_56002=select TIRTAG_ID from ti_ratis_tag where TAG_GROUP = '%s' and DETAIL = '%s'*/
		TA_Base_Core::SQLStatement sql;
        ...
        TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::TisRatis_Sd, TA_Base_Core::Read);        
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_SELECT_56002, m_tagGroup, m_detail);
    ...
    bool TiRatisTagHelper::isKeyDuplicate(unsigned long keyToUse)
    {
        ...
        TA_Base_Core::IDatabase* databaseConnection = TA_Base_Core::DatabaseFactory::getInstance().getDatabase(TA_Base_Core::TisRatis_Sd, TA_Base_Core::Read);        
		databaseConnection->prepareSQLStatement(sql, TI_RATIS_TAG_SELECT_56001, keyToUse);

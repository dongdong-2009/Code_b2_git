...
namespace TA_IRS_Bus
{
    ...
    BOOL LocationSelectionDialog::OnInitDialog()
    {
        ...
        LocationCache::LocationInfoMap locaInfoMap = LocationCache::getInstance().getAllLocations();
        ...
        LocationCache::LocationInfoMap locationByOrder;

        LocationCache::LocationInfoMap::iterator iter;
        for ( iter = locaInfoMap.begin(); iter != locaInfoMap.end(); ++iter )
        {
            // filter out the list to only station locations.
            if ( iter->second.type == TA_Base_Core::ILocation::STATION )
            {

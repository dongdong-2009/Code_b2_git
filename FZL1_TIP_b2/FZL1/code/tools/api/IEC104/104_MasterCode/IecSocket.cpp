...
int CIecSocket::Connect(const std::string& hostAddr, int hostPort)
{
	...
	int n=0;
	if ( (n = connect(mSocket, (struct sockaddr*) &serverAddr,sizeof(serverAddr)))<0)
	{
	...
	tval.tv_sec = 0; 
	tval.tv_usec = 800000; 
	if ((n = select(mSocket+1, &rset, &wset, NULL,&tval)) == 0)
	{ 
		this->Close(false); 
		errno =-1;//ETIMEOUT; 
		return (-1); 
...
int CIecSocket::Select(SelectType selectType)
{
	...
	for (int i = 0; i < mSelectRetries; i++ )
	{
		...
		if (status > 0) 
		{
			...
			{
				LogDebug("select status > 0 on socket %d, but not FD_ISSET", mSocket);
			}
		}

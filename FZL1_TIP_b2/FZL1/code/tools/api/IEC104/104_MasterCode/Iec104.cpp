...
int  CIec104::LinkCheckCommand()
{
	...
	{
		//LogThread::instance()->printStringWithTime("t3 time out\n");
...
int CIec104::AppCheckCommand()
{
	SIec104Data *p104;	
	 ...
	 {
	 	if((p104->T4Cnt <=0) && (p104->NextCmd==IEC104_CMD_NON))
...
int CIec104::AppCheckCmdFromAgent()
{
	 ...
	 CCommand cmd;
	 float sfa;b_ui16 nva;b_i16 sva;
	 int ret;
	...
	switch(cmd.getTi())
	{
		...
		case IEC104_C_SE_TC://set point command, short floating-point number with time tag CP56Time2a
			...
			//if(AppTxSetValue(cmd._iRtuAddr, cmd._iInfoAddr,cmd._arg,cmd._bCmdType,cmd._bSeType,cmd.tmFlag)==1)
			//	return 0;
			break;
		...
		case IEC104_P_NC_NA://parameter activation
			...
			ret = AppTxAsduType_113(cmd._iRtuAddr, cmd._arg[0], cmd._arg[1]);
			break;
		case IEC104_F_SC_NA:
...
/*
* get rtu parameters value
*/
		temp_string = config_file.get_value("APP_PARA", "RtuAddr");
		if(!temp_string) {
			return b_false;
		}
		pPara->btRtuAddr=atoi(temp_string);

   
		temp_string = config_file.get_value("APP_PARA", "CallDataFlag");
		if(!temp_string) {
			return b_false;
		}

        temp_string = config_file.get_value("APP_PARA", "CallKwhFlag");
		if(!temp_string) {
			return b_false;
		}   
	...
	switch( pApdu[2])
	{
		...
		case 0x0b://start con
			if(m_pIec104Data->LinkCmd= IEC104_LK_CMD_SENDFLAG | IEC104_LK_CMD_STARTDT)
			{
				...
				m_pChannel->setHostStatus(HOST_STATUS_MASTER);
				//m_pIec104Data->NextCmd = IEC104_CMD_NON;
				stopTimer(1);
				return 0;
			}
			return -1;
...
int CIec104::LinkRxFrameI(SIec104Data *p, b_ui8* pApdu)
{
	...
	if( (p->RxSeq - p->LastAckSeq ) >= p->LinkPara->W ) 
		LinkTxFrameS();
...
void CIec104::AppRxAsdu(const b_ui8 *pBuf, b_ui8 iSize)
{
	...
	int ret=0;
	std::string tmpStr;
	PrintInfo("  asdu>>",(b_ui8 *)pBuf,iSize);
	if(iSize<2)
	{
		
		tmpStr="wrong asdu size";
		IEC104_API::getInstance()->logRecord(LOG_LEVEL_ERROR,tmpStr);
		return;
	}
	...
	case IEC104_C_IC_NA: //100
		AppRxAllDataConf(pBuf, iSize );
		break;
	case IEC104_C_CI_NA: //101
		AppRxAllKwhConf( pBuf, iSize );
		break;
	case IEC104_C_RD_NA:	//102 
		//AppRxRdCmdConf( iLoopCnt,pBuf, iSize );
		break;
	case IEC104_C_CS_NA:	//103
		AppRxTimeConf( pBuf, iSize );
		break;
	case IEC104_C_RP_NA:	//105
		AppRxRPConf( pBuf, iSize );
		break;
	case IEC104_C_TS_TA:	//107
		AppRxTsConf( pBuf, iSize );
		break;
	case IEC104_C_DC_TA: 
	case IEC104_C_DC_NA:
	case IEC104_C_SC_NA:
	...
	case IEC104_C_RC_TA:
	case IEC104_C_RC_NA:
		AppRxYkConf( pBuf, iSize );//control command confirmation
		break;
	case IEC104_C_SE_NA:
	case IEC104_C_SE_NB:
	case IEC104_C_SE_NC:
	...
	case IEC104_C_SE_TB:
	case IEC104_C_SE_TC:	
		AppRxSvConf(pBuf, iSize);//set point command confirmation
		break;
	case IEC104_M_SP_NA:    
...
//103 extension
	...
	case IEC104_E_148:
		...
		break;
	default:
		tmpStr="unknown asdu type,not handled\n";
		IEC104_API::getInstance()->logRecord(LOG_LEVEL_ERROR,tmpStr);
		break;
	}
...
void CIec104::AppRxAllDataConf(const b_ui8 *asdu, b_ui8 size)
{
	SIec104Data *p104;
	p104 = m_pIec104Data;

	switch( p104->AppState )
	{
	case IEC104_APP_STATE_WAITALLDATACONF:
		if( (asdu[2]&0x3f) == IEC104_COT_ACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_WAITALLDATA;
		}
		else if( (asdu[2]&0x3f) == IEC104_COT_DEACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
		}
		break;
	case IEC104_APP_STATE_WAITALLDATA://wait for interrogation data uploaded
		if( (asdu[2]&0x3f) == IEC104_COT_ACT_TERM )
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
			p104->NextCmd =	IEC104_CMD_CALLALLKWH;
		}
	...
	case IEC104_APP_STATE_IDLE:
		if( (asdu[2]&0x3f) == IEC104_COT_TRI_SPONT )
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
			p104->NextCmd =	IEC104_CMD_CALLALLKWH;
		}
...
void CIec104::AppRxAllKwhConf(const b_ui8 *asdu, b_ui8 size)
{
	SIec104Data *p104;
	p104 = m_pIec104Data ;
	...
	case IEC104_APP_STATE_WAITALLKWHCONF:
		if( (asdu[2]&0x3f) == IEC104_COT_ACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_WAITALLKWH;
		}
		else if( (asdu[2]&0x3f) == IEC104_COT_DEACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
		}
		else if ( (asdu[2]&0x3f) == IEC104_COT_ACT_TERM )// follow are test
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
	
		}
		break;
	case IEC104_APP_STATE_WAITFRZKWHCONF:
		if ( (asdu[2]&0x3f) == IEC104_COT_ACT_TERM )// follow are test
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
...
void CIec104::AppRxTimeConf(const b_ui8 *apdu, b_ui8 size)
{
	time_t	ltime, tiack;
	struct tm ttm,*ttr;
	int 	iRecvMs,iStart;
	...
	p104 = m_pIec104Data;	
	iStart = p104->AppPara.AsduAddrLen + p104->AppPara.CotLen + 
		p104->AppPara.InfAddrLen +2;
	
	if( (apdu[2]&0x3f) == IEC104_COT_ACT_CON )
	{
		if( p104->AppState == IEC104_APP_STATE_WAITTIMECONF )
		{
			time( &ltime );

			ttr = gmtime(&ltime);
			
			iRecvMs = (apdu[iStart+1]*256+apdu[iStart])%1000;
			ttm.tm_sec = (apdu[iStart+1]*256+apdu[iStart])/1000;
			ttm.tm_min = apdu[iStart+2]&0x3f;
			ttm.tm_hour = apdu[iStart+3]&0x1f;
			ttm.tm_mday = apdu[iStart+4]&0x1f;
			ttm.tm_mon = (apdu[iStart+5]&0x0f)-1;
			ttm.tm_year = (apdu[iStart+6]&0x7f)+100;
			ttm.tm_isdst = 0;
			tiack = mktime( &ttm );
...
void CIec104::AppRxRPConf(const b_ui8 *pBuf, b_ui8 iSize)
{
	SIec104Data *p104;
	p104 = m_pIec104Data;
	switch( p104->AppState )
	{
	case IEC104_APP_STATE_WAITRPCONF:
		if( (pBuf[2]&0x3f) == IEC104_COT_ACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_WAITRPEND;
		}
		else if( (pBuf[2]&0x3f) == IEC104_COT_DEACT_CON )
		{
			p104->AppState = IEC104_APP_STATE_IDLE;
		}
		break;
...
void CIec104::AppRxTsConf(const b_ui8 *apdu, b_ui8 size)
{
	time_t	ti, tiack;
	struct	tm	ttm;
	int 	iRecvMs,iStart;
	...
	p104 = m_pIec104Data;	
	iStart = p104->AppPara.AsduAddrLen + p104->AppPara.CotLen + 
		p104->AppPara.InfAddrLen +4;
	
	if( (apdu[2]&0x3f) == IEC104_COT_ACT_CON )
	{
		if( p104->AppState == IEC104_APP_STATE_WAITTESTCONF )
		{
			time( &ti );
...
void CIec104::AppRxYkConf(const b_ui8 *apdu, b_ui8 size)
{
	struct SResult ykResult;
	SIec104Data *p104;
	b_bool bRet = b_false;
	int		i,iInf,iInfAddrLen;
	int		iRtuAddrLen=0,iRtuAddrIndex=0,iRtuAddr=0,ykno=0;
	b_ui8  bYkType=0,bTi=0,bVal=0,bCot;
	b_bool tmflag=b_false;
	...
	iRtuAddr = getRtuAddr(apdu);
	for (i=iInfAddrLen; i>0 ; i--)
	{
		ykno = ykno*256 + apdu[iInf+i-1];
	}
	bTi = apdu[0];
	bCot = apdu[2]&0x3f;
	if(bTi <=47) 
	...
	else    
	{
		...
		tmflag=b_true;
		SFepTime timeStamp;
		GetTmStmp(&apdu[iInf + iInfAddrLen+1], &timeStamp);
	}
	switch(bTi)
	{
		case  IEC104_C_SC_NA://
	...
	ykResult.iRtu = iRtuAddr;
	ykResult.iOffset = ykno;

	
	switch(bTi)
	{
		case IEC104_C_SC_NA:
			GetHandler()->handleSingleCommand(iRtuAddr,ykno, bCot, apdu[9]);
			break;
		default:
			GetHandler()->handle_yk(iRtuAddr, ykno, bVal,bYkType, ykResult.iResult,tmflag,0);
	}
...
float CIec104::AppYcDecode(const b_ui8 *buf)
{

	b_i16 iYcCode = 0;
	float f= 32768.0;
	memcpy(&iYcCode,buf,2);
	return iYcCode/f;
}

float CIec104::AppYcDecodeSF(const b_ui8 *pBuf)
{
...
int CIec104::AppKwhDecode(const b_ui8 *buf)
{

	int kwhvalue =0;
	memcpy(&kwhvalue,buf,4);
	return kwhvalue;
}
...
int CIec104::AppTxAsduType_100(int rtuAddr, b_ui8 bCot, b_ui8 btQoi)
{
	...
	{
		p104->AppState = IEC104_APP_STATE_WAITALLDATACONF; 
		resetTimer(4);
		return 1;
	}
	else 
...
int CIec104::LinkTxFrameI(const b_ui8 *pAsdu, b_ui8 size, b_ui8 frmtype)
{
	...
	if( p104->LinkState == IEC104_LK_STATE_STARTED)
	{
		...
		p104->LastAckSeq = p104->RxSeq;
		stopTimer(2);
		if( (p104->TxSeq - p104->AckSeq ) >= p104->LinkPara->K )
		{
...
int CIec104::AppTxAsduType_103(int rtuAddr)
{
	...
	{
		p104->AppState = IEC104_APP_STATE_WAITTIMECONF;
		m_bRecvFinish = 0;		
		return 1;
	}
	else return -1;
...
int CIec104::AppTxAsduType_107(int rtuAddr, int infoAddr, b_ui16 bTsc )
{
	...
	CIec104_Asdu asdu(p104->AppPara,IEC104_C_TS_TA,1,IEC104_COT_ACT,rtuAddr,0,btBuf,9);	
	if( LinkTxFrameI( asdu.m_pBuf , asdu.m_iLen, IEC104_FRMTYPE_SEND_WC ) == 1)
	{
		p104->AppState = IEC104_APP_STATE_WAITTESTCONF; 
		return 1;
	}
...
int CIec104::AppTxAsduType_105(int rtuAddr,b_ui8  btQrp)
{
	...
	if( LinkTxFrameI( asdu.m_pBuf , asdu.m_iLen, IEC104_FRMTYPE_SEND_WC ) == 1)
	{
		p104->AppState = IEC104_APP_STATE_WAITRPCONF; 
		return 1;
	}
	return -1;	
}
...
int CIec104::AppTxAsduType_46(int rtuAddr,int infoAddr ,b_ui8 bCot,b_ui8 bDco)
{
	...
	if(  LinkTxFrameI(  asdu.m_pBuf , asdu.m_iLen, IEC104_FRMTYPE_SEND_WC ) == 1)
	{
		m_pIec104Data->AppState = IEC104_APP_STATE_WAITYKCONF;
		m_pIec104Data->CtrlCmd = bCmd;
		return 1;
	}
	return -1;
...
*/
int  CIec104::AppTxAsduType_113(int rtuAddr,b_ui8 bCot,b_ui8 bQpa)
{
	SIec104Data *p104;
	p104 = m_pIec104Data;
	CIec104_Asdu asdu(p104->AppPara,IEC104_P_NC_NA,1,bCot,rtuAddr,0,&bQpa,1);
	if( LinkTxFrameI(  asdu.m_pBuf , asdu.m_iLen, IEC104_FRMTYPE_SEND_WC ) == 1)
	{
		return 1;
	}	
 ...
 int CIec104::AppTxAsduType_48(b_ui16 rtuAddr,b_ui32 infoAddr ,b_ui8 bCot, b_ui16 Nva,b_ui8 bQos)
{
	...
	if(  LinkTxFrameI(  asdu.m_pBuf , asdu.m_iLen, IEC104_FRMTYPE_SEND_WC ) == 1)
	{
		m_pIec104Data->AppState = IEC104_APP_STATE_WAITSVCONF;
		m_pIec104Data->CtrlCmd = bCmd;
		return 1;
	}
	return -1;
...
void CIec104::AppRxSvConf(const b_ui8 *apdu, b_ui8 size)//confirm of setting fix value
{
    struct SResult svResult;

	SIec104Data *p104;
	b_bool bRet = b_false;
	int		i,iInf,iInfAddrLen;
	int		iRtuAddrLen=0,iRtuAddrIndex=0,iRtuAddr=0;
	b_ui8     bTi=0,bVal[4],bLen=0,bSvType=0;
	b_bool tmflag=b_false;
	p104 = m_pIec104Data;
	...
	int svno = 0;//get point number
	for (i=iInfAddrLen; i>0 ; i--)
	{
		svno = svno*256 + apdu[iInf+i-1];
	}
	SFepTime timeStamp;
	bTi = apdu[0];
	switch(bTi)
	{
		case IEC104_C_SE_TA:
			tmflag=b_true;
			GetTmStmp(&apdu[iInf + iInfAddrLen+3], &timeStamp);
		case IEC104_C_SE_NA:
			bSvType = 0;
			memcpy(bVal,&apdu[9],2);
			bLen = 2;
			break;
		case IEC104_C_SE_TB:
			tmflag=b_true;
		...
		case IEC104_C_SE_TC:
			tmflag=b_true;
			GetTmStmp(&apdu[iInf + iInfAddrLen+5], &timeStamp);
		case IEC104_C_SE_NC:		
			bSvType = 2;
			memcpy(bVal,&apdu[9],4);
			bLen = 4;
			break;
		default:;
	...
	svResult.iRtu = iRtuAddr;
	svResult.iOffset = svno;

	switch( p104->AppState )
	{
	case IEC104_APP_STATE_WAITSVCONF://comfirm for set fix value command
		if( (apdu[2]&0x3f) == IEC104_COT_DEACT_CON ) //COT=9 deactivation confirmation
		{				
			p104->AppState = IEC104_APP_STATE_WAITSVFINISH;
			m_bRecvFinish = 1;
			svResult.iResult = 5;//FOR TEST
			bRet =b_true;
			//	return;
		}
		else if( (apdu[2]&0x3f) == IEC104_COT_ACT_CON ) //coT=7 activation confirmation
		{
			if( (apdu[2]&0x40) == 0x40 )	//NACK conf P/N=1
			{
				p104->AppState = IEC104_APP_STATE_IDLE;
				m_bRecvFinish = 1;
			...
			{	
				m_bRecvFinish = 1;	
				if( p104->CtrlCmd == IEC104_CTRL_SELECT )
				{
					p104->AppState = IEC104_APP_STATE_IDLE;
					m_bRecvFinish = 1;
					svResult.iResult = 1;                 //THIS IS FOR TEST

					
					bRet =b_true;
				}
				else if( p104->CtrlCmd == IEC104_CTRL_EXECUTE )
				{
					p104->AppState = IEC104_APP_STATE_WAITSVFINISH;
					m_bRecvFinish = 1;
					svResult.iResult = 3;                 //this is for test
					bRet =b_true;
				}
			}
		}
		else if((apdu[2]&0x3f) == IEC104_COT_ACT_TERM)
		{
			
			if( p104->CtrlCmd == IEC104_CTRL_EXECUTE )
			{
				p104->AppState = IEC104_APP_STATE_IDLE;
				m_bRecvFinish = 1;
				svResult.iResult = 3;//FOR TEST
				bRet =b_true;
			}
			if(p104->CtrlCmd == IEC104_CTRL_CANCEL)
			{
				m_bRecvFinish = 1;
				svResult.iResult = 5;//FOR TEST
				bRet =b_true;
		...
		break;//other cause of transfer
	case IEC104_APP_STATE_WAITSVFINISH:
		if( (apdu[2]&0x3f) == IEC104_COT_ACT_TERM ) //iCOT=10
		{			
			p104->AppState = IEC104_APP_STATE_IDLE;
			m_bRecvFinish = 1;			
		}
	...
	if(bRet == b_true)
	{
		...
		GetHandler()->handle_sv(svResult.iRtu, svResult.iOffset,bVal,bLen,bSvType,svResult.iResult,tmflag);
	}
}
...
void CIec104::resetTimer(int timer)
{
	...
	switch( timer)
	{
		...
		default:
			char str[125];
			sprintf(str,"undefine timer %d when start timer",timer);
...
void CIec104::stopTimer(int timer)
{
	...
	switch( timer)
	{
		...
		default:
			char str[100];
			sprintf(str,"undefine timer %d when stop timer",timer);
			std::string logStr=str;
			IEC104_API::getInstance()->logRecord(LOG_LEVEL_ERROR, logStr);
	}
}
...
void CIec104::AppRxEndOfInitialization( const b_ui8 *pAsdu, b_ui8 iSize)
{
	...
	if(btCot!=IEC104_COT_INIT)
		m_handler->logRecord("bad cot-%02x for EndOfInitialization-70, correct is 4\n",btCot);
	if(btSeqflag!=1)
		m_handler->logRecord("bad seq-%02x for EndOfInitialization-70, correct is 1\n",btSeqflag);
	m_handler->logRecord("recv EndOfInitialization-70, COI is %02x\n",bCoi);
}

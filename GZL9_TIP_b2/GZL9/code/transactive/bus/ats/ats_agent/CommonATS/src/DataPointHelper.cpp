/**
  ...
  * 
  * Source:    $File: //depot/4669_Review_Branch/4669/transactive/app/ats/AtsAgent/src/DataPointHelper.cpp $
...
{
	DataPointHelper::DataPointHelper(TA_Base_Core::AtsAgentEntityDataPtr entityData, IPowerStatusUpdate* powerUpdate)
	...
	{
		//only need to process valueStateUpdate,ignore others, e.g. AlarmSummaryUpdate.
		...
		DataPointDetail& dpDetail = getDataPointDetail(entityKey);
		bool bStatus=false,bDpValue =false;

		try
		{
			//Datapoint Quality states contains  BAD or GOOD Quality.
			//Good Quality Datapoint has two types :
			// 1. QUALITY_GOOD_NO_SPECIFIC_REASON
			// 2. QUALITY_GOOD_LOCAL_OVERRIDE

			//The condition below must check on both GOOD Quality States of the Datapoint
			if ( TA_Base_Bus::QUALITY_GOOD_NO_SPECIFIC_REASON == dpDetail.dataPointProxy->getStatus() ||
				TA_Base_Bus::QUALITY_GOOD_LOCAL_OVERRIDE == dpDetail.dataPointProxy->getStatus() )
			{
				bStatus=true;
				if ( dpDetail.dataPointProxy->getValueAsBool() )
				{
					bDpValue=true;
			...
			LOG3(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"PowerStatus update:pkey:%ld,quality:%d,dpValue:%d",entityKey,bStatus,bDpValue);
			m_powerUpdate->powerStatusUpdated(entityKey,bStatus,bDpValue);
		}
		catch ( TA_Base_Core::ScadaProxyException& expProxy )
		{
			LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugError,"ScadaProxyException: %s.Entitykey:%ld",expProxy.what(),entityKey );
		}
		catch (...)
		{
			LOG1( SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Unknown exception.EntityKey:%ld",entityKey);
		}
	}

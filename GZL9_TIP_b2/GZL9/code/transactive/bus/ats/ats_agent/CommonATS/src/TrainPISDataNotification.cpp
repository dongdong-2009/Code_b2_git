...
namespace TA_IRS_Bus
{
		...
		: DataNotification(paData)
	{
		...
		for (iterPis; iterPis != pisInfoVec->end(); iterPis++)
		{
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
				"PlatformID :%s, 1st Train Arrival Time : %s",
				trainPisInfo.PlatformId.c_str(), trainPisInfo.Train1st_ArrivalTime.c_str());
	...
	void TrainPISDataNotification::updatePISDatapointQuality(TA_Base_Core::EDataPointQuality quality)
	{
		FUNCTION_ENTRY("updatePISDatapointQuality");

		//Update Train Data Datapoint Quality
		TA_IRS_Bus::DataCache::getInstance()->updatePISInfoDatapointQuality(quality);

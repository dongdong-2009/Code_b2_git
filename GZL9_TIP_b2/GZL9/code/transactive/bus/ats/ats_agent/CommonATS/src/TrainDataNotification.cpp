		...
		: DataNotification(trainData)
	{
		...
		for (iterTrain; iterTrain != trainDataVec->end(); iterTrain++)
		{
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,
				"Train ID:%s, Train Congestion:%d, Train Service Number:%s, Train Direction:%d, Train Track Location:%s, Train Drive Mode:%d",
				trainInfo.TrainID.c_str(),trainInfo.TrainCongestion,trainInfo.TrainServiceNumber.c_str(),
				static_cast<int>(trainInfo.genaTrainDirection),trainInfo.TrainTrackLocation.c_str(), trainInfo.TrainDriveMode);

/**
  ...
  * 
  * Source:    $File: //depot/4669_Review_Branch/4669/transactive/bus/ats/ats_agent/CommonATS/src/AtsTrainData.cpp $
...
#include "bus/ats/ats_agent/CommonATS/src/AtsTrainData.h"
...
namespace TA_IRS_Bus
{
	...
	AtsTrainData::AtsTrainData(	std::string physicalTrainNumber, unsigned long entityKey, 
	...
	{
		m_atsTrainMessage.physicalTrainNumber = CORBA::string_dup(physicalTrainNumber.c_str());
		...
		physicalTrainService << physicalTrainNumber << "/_";
	...
	void AtsTrainData::importAtsTrainMessage(const TA_IRS_Bus::IAtsTrainCorbaDef::AtsTrainMessageCorbaDef& trainMessage)
	{
		...
		if (trainMessage.localisation.localisationValid)
		{
			...
			updateDirection(trainMessage.localisation.upDirection);
	...
	{	
		if (!m_atsTrainMessage.trainValid)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "setTrainValidity() - train %s validity, was invalid now valid", 
				m_atsTrainMessage.physicalTrainNumber.in());
	...
	{	
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateTrainValidity() - train %s validity, was %s now %s", 
				m_atsTrainMessage.physicalTrainNumber.in(), (m_atsTrainMessage.trainValid?"valid":"invalid"), 
				(valid?"valid":"invalid"));
	...
	void AtsTrainData::checkIfTrainNotImported()
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "checkIfTrainNotImported() - not updated, train %s validity, was valid now invalid", 
				m_atsTrainMessage.physicalTrainNumber.in());
	...
	{	
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateLocalisationValidity() - train %s validity, was %s now %s", 
				m_atsTrainMessage.physicalTrainNumber.in(), 
	...
	void AtsTrainData::updateServiceNumber(TA_IRS_Bus::ServiceNumberType serviceNumber)
	{
		...
		"updateServiceNumber serviceNumber = %ul", serviceNumber);
		if (serviceNumber != m_serviceNumber)
		{	
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateServiceNumber() - train %s serviceNumber, was %d now %d", 
				m_atsTrainMessage.physicalTrainNumber.in(), m_serviceNumber, serviceNumber);

			m_serviceNumber = serviceNumber;
			std::ostringstream physicalTrainService;
			physicalTrainService << m_atsTrainMessage.physicalTrainNumber.in() << "/";
								   ...
								   unsigned long	headCarAbscissaOnFb)
	{
		...
		catch (TA_Base_Core::DatabaseException& e)
		{
			if (!m_atsTrainMessage.localisation.localisationValid || m_atsTrainMessage.localisation.currentLocation != 0)
			{
				...
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
							"updateZoneId() - train %s - %s", 
							m_atsTrainMessage.physicalTrainNumber.in(), text.c_str());
			}
			stationZoneId = 0;
		}
		...
		updateZoneId(stationZoneId, cctvZoneId, track);
	}
									...
									const TA_IRS_Bus::ETrack track)
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateZoneId() - train %s currentLocation, was %d now %d", 
				m_atsTrainMessage.physicalTrainNumber.in(), 
	...
	void AtsTrainData::updateMute(bool mute)
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateMute() - train %s mute, was %s now %s", 
				m_atsTrainMessage.physicalTrainNumber.in(), 
	...
	void AtsTrainData::updateOa1Table(const TA_IRS_Bus::IAtsTrainCorbaDef::OctetSequence& oa1)
	{
		...
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
					"sendOa1TableToAts() - OA1 Table received for train %s - incorrect size",
					m_atsTrainMessage.physicalTrainNumber.in());
	...
	void AtsTrainData::sendUpdates()
	{
		if (isUpdated())
		{
			...
			LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
				"sendUpdates() - Info: AtsTrainMessage sent - train=%s, entity=%d, location=%d, train=%s, localisation=%s, trainlocation=%d, lastlocation=%d",
				m_atsTrainMessage.physicalTrainNumber.in(), m_entityKey, m_locationKey,
	...
	void AtsTrainData::updateDirection(const TA_IRS_Bus::EDirection direction)
	{
		if (direction != m_atsTrainMessage.localisation.upDirection)
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
                "updateDirection() - train %s upDirection, was %d now %d", 
				m_atsTrainMessage.physicalTrainNumber.in(), 
				m_atsTrainMessage.localisation.upDirection, direction);

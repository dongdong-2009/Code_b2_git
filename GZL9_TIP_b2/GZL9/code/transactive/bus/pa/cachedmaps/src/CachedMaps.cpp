/**
  ...
  * in any form.
  *
  * Source: $File: //depot/GZL6_TIP/3002/transactive/bus/pa/cachedmaps/src/CachedMaps.cpp $
  * @author Sean Liew
  * @version $Revision: #1 $
...
{
    const char* const PA_STN_ZONE_STATUS_TOKEN = "diiPAZONEB-Status";
    const char* const PA_OCC_ZONE_STATUS_TOKEN = "eiiPAZONEB-Status";
    ...
    CachedMaps::CachedMaps() : 
       ...
       m_paFasTriggeredBroadcastCachedMap(new PaFasTriggeredBroadcastCachedMap()),
    ...
    CachedMaps::~CachedMaps()
    {
        ...
        try
        {
            ...
            delete m_paFasTriggeredBroadcastCachedMap;
            m_paFasTriggeredBroadcastCachedMap = 0;
        ...
        //throw(TA_Base_Core::InvalidPasConfigurationException)
    {
        ...
        for (it = pasDataPoints.begin(); it!=pasDataPoints.end(); ++it)
        {
            ...
            else
            {
                if ( bOcc )
                {
                    if ( dpName.find(PA_OCC_ZONE_STATUS_TOKEN) != std::string::npos )
                    {
                        bUnique = true;
                ...
                {
                    if ( dpName.find(PA_STN_ZONE_STATUS_TOKEN) != std::string::npos )
                    {
                        bUnique = true;
                        theZoneMap = &m_zoneIdToStatusDataPointMap;
                    }

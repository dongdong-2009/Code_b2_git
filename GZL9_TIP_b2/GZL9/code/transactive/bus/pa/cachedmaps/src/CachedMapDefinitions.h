/**
  ...
  * in any form.
  *
  * Source: $File: //depot/GZL6_TIP/3002/transactive/bus/pa/cachedmaps/src/CachedMapDefinitions.h $
  * @author Jade Lee
  * @version $Revision: #1 $
...
namespace TA_IRS_Bus
{
    ...
    struct PaFasTriggeredBroadcastRecord
    {
        PaFasTriggeredBroadcastRecord()
            : m_pKey(TA_Base_Core::DatabaseKey::getInvalidKey()),
            m_ulAlarmEntityKey(0),
            m_locationKey(TA_Base_Core::DatabaseKey::getInvalidKey()),
            m_paCyclicTimes(0),
            m_paZoneGroupKey(TA_Base_Core::DatabaseKey::getInvalidKey()),
            m_paDvaMessageKey(TA_Base_Core::DatabaseKey::getInvalidKey()),
        ...
        unsigned long m_pKey;
        unsigned long m_ulAlarmEntityKey;
        unsigned long m_locationKey;
        unsigned long m_paCyclicTimes;
        unsigned long m_paZoneGroupKey;
        unsigned long m_paDvaMessageKey;
        bool          m_isEnabled;

        bool operator==(const PaFasTriggeredBroadcastRecord& s) const
        {
            return (m_pKey== s.m_pKey &&
                m_ulAlarmEntityKey== s.m_ulAlarmEntityKey &&
                m_locationKey== s.m_locationKey &&
                m_paCyclicTimes== s.m_paCyclicTimes &&
                m_paZoneGroupKey== s.m_paZoneGroupKey &&
                m_paDvaMessageKey== s.m_paDvaMessageKey &&
                m_isEnabled== s.m_isEnabled);
        }
    ...
    typedef std::list<PaFasTriggeredBroadcastRecord> PaFasTriggeredBroadcastRecords;
    typedef PaFasTriggeredBroadcastRecords::iterator PaFasTriggeredBroadcastRecordsIt;

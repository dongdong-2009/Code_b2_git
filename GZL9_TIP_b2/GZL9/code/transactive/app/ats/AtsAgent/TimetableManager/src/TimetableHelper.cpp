...
namespace TA_IRS_App{
	...
	const std::string PRACTICAL_FILE_PREFIX = "M09_ACT_TT_";
	const std::string PLANNED_FILE_PREFIX = "M09_PLA_TT_";
	const std::string TIMETABLE_FILE_EXTENSION = ".csv";
	
	//column number counted from 0
	const short COL_NO_TRAIN_ID = 1;
	const short COL_NO_SERVICE_ID = 3;
	const short COL_NO_PLATFORM_ID = 5;
	const short COL_NO_ARRIVAL_TM_NO = 6;
	const short COL_NO_DEST_TM_NO = 7;
	const short COL_NO_DEST_ID = 10;
	...
	void TimetableHelper::getPlannedTrainDataInfo(std::string Data,std::list<PlannedTrainTimetableInfoPtr>& PlannedDataList )
	{
		...
		try{
			...
			PlannedData->strServiceID = Elem.at(COL_NO_SERVICE_ID);

			PlannedData->strPlatformID =  Elem.at(COL_NO_PLATFORM_ID);

			std::string arr_time = Elem.at(COL_NO_ARRIVAL_TM_NO);
			std::string dept_time = Elem.at(COL_NO_DEST_TM_NO);
			...
			PlannedData->strDestinationID = Elem.at(COL_NO_DEST_ID).c_str();
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Add timetable data: %s, total Planned List size %d",
				Data.c_str(), PlannedDataList.size());
	...
	void TimetableHelper::getPracticalTrainDataInfo(std::string Data, std::list<PracticalTrainTimetableInfoPtr> &PracticalDataList)
	{
		...
		try{
			...
			PracticalData->strServiceID = Elem.at(COL_NO_SERVICE_ID);

			PracticalData->strPlatformID =  Elem.at(COL_NO_PLATFORM_ID);
			...
			PracticalData->strDestinationID = Elem.at(COL_NO_DEST_ID).c_str();
			PracticalData->strTrainID = Elem.at(COL_NO_TRAIN_ID).c_str();
			...
			LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Add timetable data: %s, total practical List size %d",
				Data.c_str(), PracticalDataList.size());
		}
		catch (TA_Base_Core::DatabaseException& e)
		{
			// Log and re-throw the exception

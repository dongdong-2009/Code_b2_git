	...
	void ModeStatusManager::updateModeStatus(unsigned long modeKey, int status)
	{
		...
		if (false != bNeedUpdate)
		{
			...
			BAS_TRY
			{
	            ...
	            if (NULL != mode.get())
	            {
	                SubSystemStatusInstance::instance()->updateSubsystemMode(mode->getLocationKey(), 
																			mode->getSubsystemKey(),
																			modeKey, 
																			ModeSet::modeType_CurrentMode);
	                ...
	                else
	                {
	                    ...
	                    SubSystemStatusInstance::instance()->updateSubsystemModeExecuteForClean(mode->getLocationKey(), 
																								mode->getSubsystemKey(), 
																								ModeSet::modeType_RunningMode);
	                }
	                ...
	                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Mode status update, ModeKey:%d ModeName:%s Subsystem:%s  Status:%s ",
	                    modeKey, 
						mode->getName().c_str(), 
						mode->getSubsystemName().c_str(), 
						ModeRunningStatusConfigManager::getModeRunningStatusByType(status).c_str());
	            }
	            else
	            {
	                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, "can not found the mode now, mode value = %lu", modeKey);

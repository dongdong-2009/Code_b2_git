...
namespace TA_IRS_App
{
	...
	BEGIN_MESSAGE_MAP(CBASManagerDlg, TA_Base_Bus::TransActiveDialog)
		...
		ON_MESSAGE(Msg_UpdateModeSatus, &CBASManagerDlg::OnUpdateModeStatus)
	...
	void CBASManagerDlg::setupListCtrls()
	{
		...
		m_lstSubsytems.InsertColumn(ReportListCtrl::COLUMN_INDEX_REPORT_TARGETMODE, _T("下发模式"), LVCFMT_LEFT, 100);
		m_lstSubsytems.InsertColumn(ReportListCtrl::COLUMN_INDEX_REPORT_CURRENTMODE, _T("当前模式"), LVCFMT_LEFT, 80);
		m_lstSubsytems.InsertColumn(ReportListCtrl::COLUMN_INDEX_REPORT_AUTOMANUAL, _T("手/自动状态"), LVCFMT_LEFT, 100);
			...
			else
			{
				...
				for (std::vector<ILocation_SmartPtr>::iterator it = locations.begin(); it != locations.end(); it++)
				{
					if ((*it)->getLocationType() == TA_Base_Core::ILocation::STATION || 
						(*it)->getLocationType() == TA_Base_Core::ILocation::DPT)
					{
	...
	LRESULT CBASManagerDlg::OnUpdateModeStatus(WPARAM wParam, LPARAM lParam)
	{
	...
	void CBASManagerDlg::OnBnClickedBtnExecuteMode()
	{
        ...
        std::stringstream ss;
        ss << "请确认是否在" << BasManagerDataHelperInstance::instance()->getLocationDispNameByKey(locationKey)
            << "执行 " << modePtr->getName() << " " << modePtr->getTypeName()
            << " " << modePtr->getDescription();
        int confirmResult = MessageBox(_T(ss.str().c_str()), _T("模式执行确认"), MB_YESNO | MB_TOPMOST);
        if (IDYES != confirmResult)
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "Cancle mode execute.");
            FUNCTION_EXIT;
            return;
        }
		...
		catch(const TA_IRS_Bus::IStationBasAgentCorbaDef::StationInLocalMode& ex)
		{
			...
			strExceptionString.Format("%s 当前为自动状态, 模式只能在手动状态下执行",getSubsystemNameByKey(locationKey,subsystemKey).c_str());		
		}
	...
	{
	    if (false == BasManagerDataHelperInstance::instance()->getIsManagerInFireMode())
	    {
	        return;
	    }
	...
	void CBASManagerDlg::OnBnClickedBtnAutomanuCfg()
	{
		...
		if (NULL != pos)
		{
			...
			BAS_TRY
			{
				...
				currentValue = m_basManagerSubscriber.getSubsystemAutoCfgValue(locationKey, subsystemKey);

				// SET the otherwise status
				newValue = (currentValue == eControlManual) ? eControlAuto : eControlManual;
				m_basManagerSubscriber.setSubsystemAutoCfgValue(locationKey, subsystemKey, newValue);

				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "set the auto or manually config oldValue[%f] newValue[%f]", currentValue, newValue);
			}
			BAS_CATCH_UNDEPLOY_EXCEPTION;
			...
			{
				CString strErrorMsg;
				strErrorMsg.Format("设置手/自动失败");
				MessageBox(strErrorMsg , MessageBox_Caption, MB_OK | MB_ICONERROR);
			}
			else
			{
				CString strErrorMsg;
				strErrorMsg.Format("设置手/自动成功，由[%s]设置为[%s]", BASManagerUtility::autoCFGValueToString(currentValue).c_str(), BASManagerUtility::autoCFGValueToString(newValue).c_str());

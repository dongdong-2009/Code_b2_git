...
namespace
{
    ...
    static const int MAX_VIDEO_SOURCES = 40;
...
namespace TA_IRS_App
{
    ...
    int SequenceEditorDlg::DoModal(Sequence* sequence, unsigned long stationID)
    {
        ...
        std::list<Camera*>::iterator it = sequenceVideoSources.begin();
        for(; it != sequenceVideoSources.end(); it++)
        {
            Camera* cam = dynamic_cast<Camera*>(*it);
            if(cam->getLocationId() == stationID || stationID == 0 || stationID == 1)
            {
                m_sequenceVideoSources.push_back(cam);
            }
        }
        ...
        {
			m_existingDwellTime.Format("%d", CCTVConfiguration::getinstance().getDefaultDwellTime());
        }
        // pop up the dialog
            ...
            if(ret == IDOK)
            {
                ...
                try
                {
                    ...
                    if((m_sequenceVideoSources.size() == 0) && (dwellTime > 0))
                    {
						...
						newDwellTime.Format("%d", CCTVConfiguration::getinstance().getDefaultDwellTime());
                        m_existingDwellTime = newDwellTime;
                        m_sequenceDwellTime.SetWindowText(m_existingDwellTime);
						m_spinControl.SetPos(CCTVConfiguration::getinstance().getDefaultDwellTime());
                    }
                    if((m_sequenceVideoSources.size() > 0) && (dwellTime == 0))
                    {
                    ...
                    SwitchAgentCommunicator::getInstance().setSequenceConfig(selectedSequence, dwellTime, m_sequenceVideoSources, false);
        ...
        else
        {
            ...
            try
            {
                if(isSourcesModified() || (m_existingDwellTime != newDwellTime))
                {
                    ...
                    // Save the sequence
                    SwitchAgentCommunicator::getInstance().setSequenceConfig(m_sequence, dwellTime, m_sequenceVideoSources, false);
    ...
    void SequenceEditorDlg::loadTree()
    {
        ...
        {
            // insert the item
            Camera* cam = dynamic_cast<Camera*>(it->second);
    ...
    LRESULT SequenceEditorDlg::onSequenceUpdated(WPARAM wParam, LPARAM lParam)
    {
        ...
        if(input == m_sequence)
        {
            ...
            if(dwellTime == 0)
            {
				dwellTime = CCTVConfiguration::getinstance().getDefaultDwellTime();
            }

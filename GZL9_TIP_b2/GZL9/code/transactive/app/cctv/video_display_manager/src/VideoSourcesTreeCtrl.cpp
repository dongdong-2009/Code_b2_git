/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL6_TIP/3002/transactive/app/cctv/video_display_manager/src/VideoSourcesTreeCtrl.cpp $
  * @author:  Robert Young
  * @version: $Revision: #2 $
  *
...
namespace TA_IRS_App
{
	...
	const char* FONT_FACE_NAME= "Arial Bold"; 
         ...
         m_pDetailsEdit(NULL)
    {
		...
		m_fntList.CreatePointFont( FONT_SIZE * 10,FONT_FACE_NAME);
    ...
    void VideoSourcesTreeCtrl::insertVideoSources( std::vector<VideoInput*> sources )
    {
		...
		m_videoSourcesGroups.erase(m_videoSourcesGroups.begin(),m_videoSourcesGroups.end() );
        // insert each item
    ...
    void VideoSourcesTreeCtrl::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult)
    {
        ...
        if ( FALSE == m_isSequenceEditorTree )
        {
            ...
            if ( FALSE == CTreeCtrl::ItemHasChildren(CTreeCtrl::GetSelectedItem()) )
            {
                ...
                //   m_presenter->setDragAdjust(adjust);
                   // ( (VideoDisplayManagerDlg*)(AfxGetApp()->m_pMainWnd) )->
                  //    m_presenter->setDragImage(dragImage);
					  m_presenter->beginDrage ( adjust ,dragImage );
                    m_isDragging = TRUE;
    ...
    void VideoSourcesTreeCtrl::handleSelectionChanged()
    {
        ...
        if ( (NULL != selectedItem)  )
        {
            ...
            if (itemData != NULL)
            {
				...
				m_presenter->SelectSequence ( input->getKey() );
    ...
    void VideoSourcesTreeCtrl::OnMouseMove(UINT nFlags, CPoint point)
    {
        ...
        if ( (nItem==NULL) || (nItem!=m_nCurItem) )
        {
           ...
           if (nItem!=NULL && !IsRectCompletelyVisible(rcText))
           {
              ...
              m_wndTip.SetWindowPos(NULL, rc.left+10, rc.top - 16, sText.GetLength()*16, //rc.Width(),//-60,

...
* @note szServerAdd的取值为1、在服务器端取127.0.0.0, 127.0.0.1或VM服务器地址；在客户端取VM服务器地址
...
* @param [IN]   pstUserLogIDInfo            用户登录ID信息标识
* @param [IN]   szOrgCode                   组织编码
* @param [IN]   pstQueryCondition           查询信息条件(可为NULL;支持的查询条件包括:#NAME_TYPE[编码器名称]; #CODE_TYPE[编码器编码]; #RES_TYPE[资源类型]; #PHY_DEV_IP[编码器IP地址])
* @param [IN]   pstQueryPageInfo            请求分页信息
* @param [OUT]  pstRspPageInfo              响应分页信息
...
* @param [IN]   pstUserLogIDInfo            用户登录ID信息标识
* @param [IN]   szOrgCode                   组织编码
* @param [IN]   pstQueryCondition           通用查询条件(可为NULL;支持的查询条件包括:#NAME_TYPE[解码器名称]; #CODE_TYPE[解码器编码]; #PHY_DEV_IP[解码器IP地址])
* @param [IN]   pstQueryPageInfo            请求分页信息
* @param [OUT]  pstRspPageInfo              响应分页信息
...
* @param [IN]   pstUserLogIDInfo    用户登录ID信息标识
* @param [IN]   szOrgCode           组织编码
* @param [IN]   pstQueryCondition   通用查询条件(可为NULL;支持的查询条件包括:#NAME_TYPE[存储设备名称] #CODE_TYPE[编码] #PHY_DEV_IP[IP地址])
* @param [IN]   pstQueryPageInfo    请求分页信息
* @param [OUT]  pstRspPageInfo      响应分页信息
...
* @param [IN]   pstUserLogIDInfo    用户登录ID信息标识
* @param [IN]   szOrgCode           组织编码
* @param [IN]   pstQueryCondition   通用查询条件(可为NULL;支持的查询条件包括:#CODE_TYPE[VX500编码]; #NAME_TYPE(VX500名称) #PHY_DEV_IP[VX500地址])
* @param [IN]   pstQueryPageInfo    请求分页信息
* @param [OUT]  pstRspPageInfo      响应分页信息
...
* - #ERR_COMMON_SUCCEED
* -     返回操作结果码：见结果码文件sdk_err.h
* @note 1、当pstSDKPtzCtrlCommand中的云台操作参数1和参数2含义为纵向或横向速度时,
*       对于超过最大速度的值按对应最大速度值处理
* @attention 本接口支持跨域操作
*/
...
* @note 无
*/
IMOS_EXPORT ULONG STDCALL IMOS_QueryScreenOutMode
(
    IN  USER_LOGIN_ID_INFO_S         *pstUserLogIDInfo,
    IN  CHAR                         szDcCode[IMOS_CODE_LEN],
    IN  ULONG                        ulOutChannelType,
...
***************************************************************************************/
/** @defgroup groupCSwitchRes 轮切资源管理

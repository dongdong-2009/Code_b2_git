...
typedef struct tagResItemV2
{
    ...
    /** 资源属性信息，对于资源类型是摄像机时，1代表EC的摄像机，2代表DVR的摄像机，其他资源类型该字段暂未使用 */
...
typedef struct tagECInfo
{
    ...
    /** EC通道数量:
        ...
        HIC5421-H(#IMOS_DT_HIC5421)/HIC5401-H(#IMOS_DT_HIC5401): 1
...
/**
 * @struct tagQueryEventRecordConditionInfo
 * @brief 事件记录的查询条件
 * @attention
 */
typedef struct tagQueryEventRecordConditionInfo
{
    /** 查询条件是否包含设备名称 */
    BOOL_T  bUseDeviceName;

    /** 设备名称 */
    CHAR  szDeviceName[IMOS_NAME_LEN];

    /** 查询条件是否包含设备编码 */
    BOOL_T  bUseDeviceCode;

    /** 设备编码 */
    CHAR  szDeviceCode[IMOS_DEVICE_CODE_LEN];

    /** 查询条件是否包含事件类型 */
    BOOL_T  bUseEventType;

    /** 事件类型,枚举值为AlARM_TYPE_E */
    ULONG ulEventType;

    /** 查询条件是否包含事件级别 */
    BOOL_T  bUseEventSecurity;

    /** 事件级别,枚举值为#ALARM_SEVERITY_LEVEL_E */
    ULONG ulEventSecurity;

    /** 查询条件是否包含确认标记 */
    BOOL_T  bUseAckedFlag;
...
typedef struct tagEventTypeInfo
{
    ...
    /** 维护经验 */
    CHAR    szEvnetExperience[IMOS_STRING_LEN_256];
}EVENT_TYPE_INFO_S;
...
typedef struct tagEventType
{
    ...
    /** 事件关联类型信息 */
    EVENT_TYPE_REL_S   stEventTypeRelInfo;
...
typedef struct tagECRBasicInfo
{
    ...
    /** ECR设备型号, 取值为#IMOS_DEVICE_TYPE_E中的#IMOS_DT_ECR3308_HF, #IMOS_DT_ECR3316_HF */
    ULONG   ulECRDevType;

...
* @param [IN] lUserParam            用户设置参数，用户在调用IMOS_SetParseVideoDataCB函数时指定的用户参数。
...
* @param [IN] lUserParam            用户设置参数，用户在调用IMOS_SetParseAudioDataCB函数时指定的用户参数
* @param [IN] lReserved             保留参数
* @return 无。
* @note
...
* @param [IN] lUserParam           用户设置参数，用户在调用IMOS_SetDecodeVideoDataCB函数时指定的用户参数。
* @param [IN] lReserved            保留参数。
* @return 无。
* @note
...
* @param [IN] lUserParam            用户设置参数，用户在调用IMOS_SetDecodeAudioDataCB函数时指定的用户参数。
* @param [IN] lReserved             保留参数。
* @return 无。
* @note
...
/**
* 设置音频数据源。\n
* @param [IN] ulWaveInSource     音频数据源，包括音频采集设备#XP_WAVEIN_SOURCE_DEVICE和音频文件#XP_WAVEIN_SOURCE_FILE两种音频数据源。
...
* @note
* - 1、如果播放器在NAT内网，必须将接收媒体流的协议类型设置为XP_PROTOCOL_TCP协议；
...
* @note
* 1、不调用该接口时默认为全帧下载
* 2、在IMOS_OpenDownloadCallback和IMOS_StartDownloadCallback之间调用
* 3、在IMOS_OpenDownload和IMOS_StartDownload之间调用
* 4、在IMOS_OpenDownloadEx和IMOS_StartDownloadEx之间调用
*/
IMOS_EXPORT ULONG STDCALL IMOS_SetExtractFrameDownload(IN const USER_LOGIN_ID_INFO_S *pstUserLoginIDInfo,
                                                       IN const CHAR *pcChannelCode);
...
* - 2、最大同时支持启动32路下载通道；
* - 3、目前仅支持TCP协议下载媒体流；
* - 4、打开媒体流下载后，需调用#IMOS_StartDownload接口开始下载，停止下载时调用#IMOS_StopDownload接口；
* - 5、下载录像文件的命名规则为：摄像机编码-通道号-S开始时间E结束时间，其中时间的格式为“YYYYMMDDHHMMSSMS”
* -    如：Cam0001-00-S20100316094658286E20100316094806084；
* - 6、启动下载后，在下载过程中，播放器通过#IMOS_SetRunMsgCB设置的回调函数上报消息
...
* -         #ERR_XP_FAIL_TO_GET_PORT_RES         获得下载通道资源失败
* @note
* - 1、该接口与IMOS_OpenDownload和IMOS_StartDownload接口匹配；
* - 2、该接口函数仅支持Windows。
*/
...
* -         #ERR_XP_FAIL_TO_START_DOWNLOAD       开始下载失败
* @note
* - 1、用户调用IMOS_OpenDownloadCallback接口打开媒体流下载后，再调用该接口开始下载，停止下载时调用IMOS_StopDownloadCallback接口；
* - 2、该接口函数仅支持Windows。
*/
...
* -         #ERR_XP_FAIL_TO_GET_PORT_RES         获得下载通道资源失败
* @note
* - 1、该接口与IMOS_OpenDownloadCallback和IMOS_StartDownloadCallback接口匹配调用；
* - 2、该接口函数仅支持Windows。
*/

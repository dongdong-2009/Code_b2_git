/**
  ...
  *
  * Source:    $File: //depot/GZL6_TIP/3002/transactive/app/cctv/video_switch_agent/src/VideoSwitchAgent.cpp $
...
{
	const unsigned long OCC_LOCATION_ID = 1;
	//
    // VideoSwitchAgent
    //
    ...
    {
		LOG_FUNCTION_ENTRY(SourceInfo,"createAllEntities");
        ...
        iter != entityDataList.end(); ++iter )
        {
			...
			LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
				"Insert Entity into createEntities:Key[%ld],Name[%s]",(*iter)->getKey(),(*iter)->getName().c_str() );
        }

		// For OCC Agent - loading all camera and sequence of the all locations.
		unsigned long m_locationID = agentEntityData->getLocation();
		
		if(m_locationID == OCC_LOCATION_ID)
		{
				std::vector<TA_Base_Core::ILocation*> locations = TA_Base_Core::LocationAccessFactory::getInstance().getAllLocations();
				std::vector<TA_Base_Core::ILocation*>::iterator itloc = locations.begin();
				std::vector<unsigned long> locationKeys;
				
				for(; itloc!=locations.end();itloc++)
				{
					TA_Base_Core::ILocation* loc = dynamic_cast<TA_Base_Core::ILocation*> (*itloc);
					unsigned long locationKey = loc->getKey();	
					if(locationKey != 1)
					{
						
						TA_Base_Core::IEntityDataList cameraEntities = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation( TA_Base_Core::Camera::getStaticType(), locationKey);
						TA_Base_Core::IEntityDataList sequenceEntities = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation( TA_Base_Core::Sequence::getStaticType(), locationKey);
						TA_Base_Core::IEntityDataList monitorEntities = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation(TA_Base_Core::VideoMonitor::getStaticType(), locationKey);

						TA_Base_Core::IEntityDataList::iterator it;	
						LOG1( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
							"get camera entities at location:[%ld]",locationKey );				
						for (it = cameraEntities.begin(); it != cameraEntities.end(); it++)
						{

							TA_Base_Core::IEntityDataPtr entityData = TA_Base_Core::SharedIEntityDataList::value_type(*it);
							createdEntities.push_back( 
							TA_Base_Bus::IEntityList::value_type( (*it)->getKey(), createEntity(entityData)) );
							LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
								"Insert Entity into createEntities:Key[%ld],Name[%s]",(*it)->getKey(),(*it)->getName().c_str() );
						}
						LOG1( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
							"get camera entities at location:[%ld]",locationKey );
		...
		LOG_FUNCTION_EXIT(SourceInfo,"createAllEntities");
        return true;
    }
    ...
    void VideoSwitchAgent::entityCreationComplete()
    {
		...
		LOG_FUNCTION_ENTRY(SourceInfo,"entityCreationComplete");
        TA_Base_Core::DataNodeAccessFactory::getInstance()->destroyDataNodeHierarchyMap();
		LOG_FUNCTION_EXIT(SourceInfo,"entityCreationComplete");
    }
        ...
        TA_Base_Core::IEntityDataPtr entityData )
    {
		...
		else if(entityDataType == TA_Base_Core::DataPointEntityData::getStaticType())
		{
			...
			try
			{
				...
				dp = m_dataPointFactory->createDataPoint(dpEntityData);
				theEntity = dp;
				m_keyToDp[dp->getEntityKey()] = dp;
			}
			catch (...)
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Error creating DataNode %s (%ul)",
...
//		TA_Base_Bus::DataPoint* dataPoint = IMOSSwitchManager::getInstance().getDataPointFromKey(updateEvent.getKey());
//		if(dataPoint!=0)
//		{
...
//		}
	...
	void VideoSwitchAgent::beforeGetDescendantsOfAgent( TA_Base_Core::IEntityDataPtr agentEntity )
	{
		TA_Base_Bus::IGenericAgentUser::beforeGetDescendantsOfAgent(agentEntity);
		try
		{
			gLocalLocation = TA_Base_Core::LocationAccessFactory::getInstance().getLocationByKey(agentEntity->getLocation());
		}
		catch (...)
		{
		}
		TA_ASSERT(gLocalLocation != NULL, "Can't get local location")
		initPDInterfaceFactory();
	}

...
struct SIec104Data
{
	...
	int		AckSeq;	// Ackknowlodge sequence
	...
	int			LastAckSeq;			// the current not acknowledged I Frame Sequence
...
public:
	...
	int  AppTxAsduType_113(int rtuAddr,b_ui8 bCot,b_ui8 bQpa);
...
//------------------------follows are used to file transfer---------------------------//
	...
	void AppRxKwhFrame( const b_ui8 *pAsdu, b_ui8 iSize );

	void AppRxYkConf( const b_ui8 *apdu, b_ui8 size );
	
	void AppRxEndOfInitialization( const b_ui8 *pAsdu, b_ui8 iSize);

	void AppRxTsConf(const b_ui8* apdu, b_ui8 size);//107

	void AppRxRPConf(const b_ui8* pBuf, b_ui8 iSize);//105

	void AppRxTimeConf(const b_ui8  *apdu, b_ui8 size);//103

	void AppRxAllKwhConf(const b_ui8  *asdu, b_ui8 size );//101

	void AppRxAllDataConf(const b_ui8  *asdu, b_ui8 size );//100

	void AppRxAsdu(const  b_ui8* pBuf, b_ui8 iSize);

	int	 AppKwhDecode(const b_ui8 *buf );


	b_bool GetCurTm(SFepTime &  t);
	b_bool GetCurTm( char * pBuf);
	b_bool GetTmStmp(const b_ui8 *pBuf,SFepTime *pTmStmp);
	float AppYcDecodeSF(const b_ui8 *pBuf);
	float  AppYcDecode(const b_ui8 * buf);

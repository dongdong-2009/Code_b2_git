/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3002_TIP/3002/transactive/app/scada/EMSAgent/src/EMSAgent.cpp $
  * @author:	HoaVu
  * @version:	$Revision: #8 $
  *
  ...
  * Description:
  *
  *	EMSAgent implementes the GenericAgent interface.  EMSAgent monitors and controls the
...
{
	EMSAgent::EMSAgent (int argc, char* argv[])
	:
	m_EMSAgentPrimaryKey (0),
	...
	m_hasOtherBusiness(false)
	{
		...
		std::string val = TA_Base_Core::RunParams::getInstance().get("OtherBusiness");	
 		if (val == "YES")
 		{
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "EMSAgent start business module");
		}
	...
	std::vector<unsigned long> EMSAgent::parseAddresses(std::string strAddressValues)
	{
	...
	void EMSAgent::loadAgentParamValues()
	{
        // if m_EMSAgentPrimaryKey != 0, means the loadAgentParamValues() has been called, no need multip calls.
        if (0 < m_EMSAgentPrimaryKey)
            return;
		...
		// dynamic cast the entity data to EMSAgentEntityData
		TA_Base_Core::EMSAgentEntityDataPtr EMSAgentEntityData;
		EMSAgentEntityData = 
			boost::dynamic_pointer_cast<TA_Base_Core::EMSAgentEntityData>(entityData);

		// get the primary key of this EMSAgent
		m_EMSAgentPrimaryKey = entityData->getKey();
        
        
        
		// get the agent asset name
		m_agentAssetName = EMSAgentEntityData->getAssetName();

		// get agent specific params
		m_bOverlapModbusRequest = EMSAgentEntityData->getOverlapModbusRequest();
		m_bStandardModbusLengthLimit = EMSAgentEntityData->getStandardModbusLengthLimit();
        m_iMaxModbusLength = EMSAgentEntityData->getMaxModbusLength();
		m_iIntervalBetweenModbusTransaction = EMSAgentEntityData->getIntervalBetweenModbusTransaction();

		// get the configured station name
		m_stationName = EMSAgentEntityData->getStationName();
		m_iPollTimeout = EMSAgentEntityData->getPollTimeout(); 

		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"loadAgentParamValues finished. Set - "
				"m_EMSAgentPrimaryKey = %d \n"
				"m_agentAssetName = %s \n"
				...
				"m_stationName = %s \n"
				"m_iPollTimeout = %d \n"
			, m_EMSAgentPrimaryKey, m_agentAssetName.c_str(), m_bOverlapModbusRequest, m_bStandardModbusLengthLimit, m_iMaxModbusLength, m_iIntervalBetweenModbusTransaction, m_stationName.c_str(), m_iPollTimeout);
		...
		m_polling_di_start_address = parseAddresses(EMSAgentEntityData->getPollingDiStartAddress());
		m_polling_di_end_address = parseAddresses(EMSAgentEntityData->getPollingDiEndAddress());
		m_polling_di_timeout = EMSAgentEntityData->getPollingDiTimeOut();
		m_polling_di_type = EMSAgentEntityData->getPollingDiType();

		m_polling_ai_start_address = parseAddresses(EMSAgentEntityData->getPollingAiStartAddress());
		m_polling_ai_end_address = parseAddresses(EMSAgentEntityData->getPollingAiEndAddress());
		m_polling_ai_timeout = EMSAgentEntityData->getPollingAiTimeOut();
		m_polling_ai_type = EMSAgentEntityData->getPollingAiType();

		m_polling_mi_start_address = parseAddresses(EMSAgentEntityData->getPollingMiStartAddress());
		m_polling_mi_end_address = parseAddresses(EMSAgentEntityData->getPollingMiEndAddress());
		m_polling_mi_timeout = EMSAgentEntityData->getPollingMiTimeOut();
		m_polling_mi_type = EMSAgentEntityData->getPollingMiType();

		m_dpprocessors_start_ranges = parseAddresses(EMSAgentEntityData->getDPProcessorsStartAddresses());
		m_dpprocessors_end_ranges = parseAddresses(EMSAgentEntityData->getDPProcessorsEndAddresses());
		m_analogueDpUpdateInterval = EMSAgentEntityData->getAnalogueDpUpdateInterval();
...
LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
	...
	void EMSAgent::startProcessors()
	{
	}

	void EMSAgent::stopProcessors()
	{
	}


	void EMSAgent::createEMSWorkers()
	{
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,"createEMSWorkers() called.");
	...
	TA_Base_Bus::IEntity* EMSAgent::createEntity(TA_Base_Core::IEntityDataPtr EntityData)
	{
		//
	...
	bool EMSAgent::createAllEntities( TA_Base_Core::IEntityDataPtr agentEntityData,
	...
	void EMSAgent::agentSetMonitor()
	{
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "EMSAgent is being set to monitor...");
		...
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "EMSAgent for Station %s has been started up in MONITOR mode",
            m_stationName.c_str());
	}

    void EMSAgent::agentSetControl()
    {
        LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "EMSAgent is being set to control...");
       ...
       RTUManagerMap::iterator pos;
		...
		LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "EMSAgent for Station %s has been started up in CONTROL mode",
        m_stationName.c_str());
    }
    

    void EMSAgent::processOnlineUpdate(const TA_Base_Core::ConfigUpdateDetails& updateEvent)
	{
		unsigned long updateEventPKey = updateEvent.getKey();

		// if the update event is for EMSAgent
		if (m_EMSAgentPrimaryKey == updateEventPKey)
		{
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "Received config update for EMSAgent...");

			// update EMSAgent configuration
			updateEMSAgentConfiguration(updateEvent);
			return;
		}
	...
	void EMSAgent::startEMSAgent()
	{
		LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "startEMSAgent in..");		
		//
	...
	void EMSAgent::submitAuditMessage (const TA_Base_Core::DescriptionParameters & description,
										const TA_Base_Core::MessageType & messageType)
	{
		try
		{
			...
			m_auditMessageSender->sendAuditMessage(messageType,				// message type
												   m_EMSAgentPrimaryKey,		// entity key
	...
	void EMSAgent::registerForControlMessages()
	{
		// nothing to do at this stage
	}


	void EMSAgent::registerForMonitorMessages()
	{
	}



	void EMSAgent::updateEMSAgentConfiguration (const TA_Base_Core::ConfigUpdateDetails & updateEvent)
	{
		std::string msg ("");

		switch (updateEvent.getModifications())
		{
			// only if the EMSAgent has been updated in the database
			case TA_Base_Core::Update:
			{
				...
				if ((0 != changes) && (false == changes->empty()))
				{
					...
					EMSAgentEntityData->invalidate();

					for (unsigned int i = 0; i < changes->size(); i++)
					{
						// RTU switching disabled
						if (TA_Base_Core::EMSAgentEntityData::OVERLAPREQUEST == (*changes)[i])
						{
                            m_bOverlapModbusRequest = EMSAgentEntityData->getOverlapModbusRequest();
							RTUManagerMap::iterator pos;
        					for(pos = m_rtuManagers.begin(); pos != m_rtuManagers.end(); ++pos)
						...
						else if (TA_Base_Core::EMSAgentEntityData::INTERVAL_BETWN_MODBUSTRX == (*changes)[i])
						{
                            m_iIntervalBetweenModbusTransaction = EMSAgentEntityData->getIntervalBetweenModbusTransaction();
							RTUManagerMap::iterator pos;
        					for(pos = m_rtuManagers.begin(); pos != m_rtuManagers.end(); ++pos)
                        ...
                        else if ( TA_Base_Core::EMSAgentEntityData::MAX_MODBUS_LEN == (*changes)[i] )
                        {
                            m_iMaxModbusLength = EMSAgentEntityData->getMaxModbusLength();
							RTUManagerMap::iterator pos;
        					for(pos = m_rtuManagers.begin(); pos != m_rtuManagers.end(); ++pos)
                        ...
                        else if ( TA_Base_Core::EMSAgentEntityData::STD_MODBUS_LEN_LMT == (*changes)[i] )
                        {
                            m_bStandardModbusLengthLimit = EMSAgentEntityData->getStandardModbusLengthLimit();
							RTUManagerMap::iterator pos;
        					for(pos = m_rtuManagers.begin(); pos != m_rtuManagers.end(); ++pos)
						...
						{
							LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Configuration update changes is not for EMSAgentEntityData");
						}
					}
				}
    ...
    void EMSAgent::updateDataPointConfiguration (const TA_Base_Core::ConfigUpdateDetails & updateEvent)
	{
		unsigned long updateEventPKey = updateEvent.getKey();
	...
	void EMSAgent::notifyGroupOffline(const std::string& group)
	{
		// nothing to do at this stage
	}


	void EMSAgent::notifyGroupOnline(const std::string& group)
	{
		// nothing to do at this stage
	}


	TA_Base_Bus::IEntity* EMSAgent::createDataPointEntity (TA_Base_Core::IEntityDataPtr EntityData)
	{
        ...
        TA_ASSERT(NULL != dataPointEntityData.get(), "dataPointEntityData is NULL in EMSAgent::createDataPointEntity");
	...
	TA_Base_Bus::DataPoint * EMSAgent::getDataPoint (unsigned long entityKey)
	{
    ...
    void EMSAgent::getAllDataPoints (std::map < unsigned long, TA_Base_Bus::DataPoint *>& dataPointList )
	{
		// stop multi threads accessing the list of DataPoint objects at the same time
		TA_Base_Core::ThreadGuard guard ( m_dataPointListtLock );
    ...
    TA_Base_Bus::DataNode * EMSAgent::getDataNode( unsigned long entityKey )
	{
	...
	void EMSAgent::getAllDataNodes (std::map < unsigned long, TA_Base_Bus::DataNode *>& dataNodeList )
    {
        TA_Base_Core::ThreadGuard guard ( m_dataNodeListLock );
	...
	TA_Base_Bus::DataPointFactory* EMSAgent::getDataPointFactory()
	{
    ...
    void EMSAgent::registerForStateUpdates()
    {
    }

	
    void EMSAgent::receiveSpecialisedMessage(const TA_Base_Core::StateUpdateMessageCorbaDef& message)
    {
    }
}

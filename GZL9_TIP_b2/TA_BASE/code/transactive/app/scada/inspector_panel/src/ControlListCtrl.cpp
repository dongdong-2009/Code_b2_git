...
#include "bus/scada/wf_global_inhibit/src/WFErrorManager.h"
...
namespace TA_Base_App
{
	...
	CButtonControlListCtrl::~CButtonControlListCtrl()
	{
        ...
        // destory WF button
		std::map<unsigned long,CControlListCellButton*>::iterator wfItr;
		for(wfItr = m_wfButtonMap.begin(); wfItr != m_wfButtonMap.end(); ++wfItr)
		{
			wfItr->second->DestroyWindow();
			delete wfItr->second;
			wfItr->second = NULL;
		}
	...
	void CButtonControlListCtrl::processButtonClick(int item, CString stringValue, CButton* pButton)
	{
        FUNCTION_ENTRY("processButtonClick");
        TA_ASSERT(pButton != NULL, "CButtonControlListCtrl::processButtonClick() - Button pointer is null.");
		...
		for (std::map<unsigned long, CControlListCellButton*>::iterator it = m_buttonMap.begin(); it != m_buttonMap.end(); it++)
		{
			if (pButton == it->second)
			{
				// Because non-pscada button doesn't care item, so cant use function getButton. just check the button pointer exists or not.
		...
		if (isControlButton)
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Click control button.");
            processControlButtonClick(item, stringValue, pButton);
        }
        else if (getWfButton(item) == pButton && m_equipment->isEnableWf())
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Click WF button.");
            processWfButtonClick(item, stringValue, pButton);
        }
		else
		{
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Invalid button. %d, %d", item, pButton);
			TA_ASSERT(false, "CButtonControlListCtrl::processButtonClick() - Invalid button pointer.");
		}
        FUNCTION_EXIT;
	}
	...
	void CButtonControlListCtrl::setupList()
	{
		...
		if (m_equipment->isEnableWf() && m_equipment->getPScadaFlag())
		{
			this->InsertColumn(WF_COLUMN,		"五防校验");
			this->SetColumnWidth(WF_COLUMN,	92);
			this->SetColumnWidth(NEW_VALUE_COLUMN, 92);
			this->SetColumnWidth(EQUIPMENT_ATTRIBUTES_COLUMN, 120);
		...
		for (unsigned int i=0;i<vectorIndex.size();i++)
		{
			...
			try
			{
				...
				if ((*dataPoints[iPoint])->getPScadaFlag())
				{
                    ...
                    // Using for WF, WF buttons
                    if ( m_equipment->isEnableWf() )
                    {
                        CRect cellSafety;
                        GetSubItemRect( nItem, WF_COLUMN, LVIR_LABEL, cellSafety );
                        CControlListCellButton *wfButton = getWfButton(nItem);
                        if ( wfButton == NULL )
                        {
							wfButton = new CControlListCellButton( this, cellSafety, nItem, strControlValue + "校验", this ); // we need to keep the control value same as control button text
							if ( (*dataPoints[iPoint])->getRelatedWfDataPoint() != 0 )
							{
								wfButton->setValue( strControlValue );
							}
							else
							{
								wfButton->setValue("");
								wfButton->SetWindowText("五防校验");
							}
							wfButton->EnableWindow(false);
                            m_wfButtonMap.insert( std::map<unsigned long,CControlListCellButton*> ::value_type(nItem, wfButton) );
							LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug, "Get WF button string %s", wfButton->getValue());
                        }
                    }
				}
				else
					...
					if (bSetValue)
					{
						...
						for (int nItem=0; nItem < GetItemCount(); nItem++)
						{
							if (this->GetItemData(nItem) == iPoint)
							{
                                ...
                                // Using for WF only datapoints has getRelatedWfDataPoint then can be updated
								if (m_equipment->isEnableWf() && (*dataPoints[iPoint])->getRelatedWfDataPoint() != 0)
								{
									setWfButtonText( nItem, strValue, strValue + "校验" );
								}
		...
		try
		{
			...
			if ( dataPoint->isWriteable() && !dataPoint->isControlInhibited() )
			{
                ...
                if (m_equipment->isEnableWf()
                    ...
                    && dataPoint->getRelatedWfDataPoint() != 0
                    && getWfButton(nItem) != NULL)
                {
                   LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "WF verification before sending execute");
                   processWfCommand(nItem, strValue.c_str(), getButton(nItem), CONTROL_EXECUTE);
                }
                else
                {
                   LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "Send execute command");
                   processPscadaControlCommand(nItem, dataPoint, strValue, CONTROL_EXECUTE);
                }
							}
							else
							{
								pButton->setConfirmDialogHandle(NULL);
                processPscadaControlCommand(nItem, dataPoint, strValue, CONTROL_CANCEL);
							}
		...
		if (it != m_setValueActionAllowed.end())
		{
			if (dataPoint->getPScadaFlag())
			{
				if (it->second)
				{
                    ...
                    // for WF button  using
                    if ( m_equipment->isEnableWf() && dataPoint->getRelatedWfDataPoint() )
                    {
                        LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, "refresh WF button");                        
						EnableWfButton(nItem, true);
                    }
				}
				else
				{
					EnableButton(nItem,false);
                    EnableWfButton( nItem, false );
				}
			}
			else
			{
	...
	void CButtonControlListCtrl::OnDrawCellButtons()
	{
		...
		if ( m_equipment->isEnableWf() )
		{
			std::map<unsigned long,CControlListCellButton*>::iterator wfIt = m_wfButtonMap.begin();			
			for (;wfIt != m_wfButtonMap.end();wfIt++)
			{
				if ( m_equipment->getChildDataPoint(wfIt->first)->getPScadaFlag() )
				{
					int nItem = wfIt->second->getItem();
					CRect cellRect;
					GetSubItemRect(nItem, WF_COLUMN, LVIR_LABEL, cellRect);
					CRect   buttonRect; 
					wfIt->second->GetWindowRect(&buttonRect);
					wfIt->second->MoveWindow(cellRect);					
				}					
			}
		} 
	}
    ...
    // click event from control button
    void CButtonControlListCtrl::processControlButtonClick(int item, CString stringValue, CButton* pButton)
    {
        FUNCTION_ENTRY("processControlButtonClick()");
        UINT iPoint = GetItemData(item);
        TA_Base_Bus::DataPointProxySmartPtr &dataPoint = m_equipment->getChildDataPoint(iPoint);
        if (m_equipment->isEnableWf() && dataPoint->getPScadaFlag() && dataPoint->getRelatedWfDataPoint() != 0 && getWfButton(item) != NULL)
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Send WF command.");
            processWfCommand(item, stringValue, pButton, CONTROL_SELECT);
        }            
        else
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Send control command.");
            processControlCommand(item, stringValue, pButton);
        }
                    
        FUNCTION_EXIT;
    }
    
	// click event from WF button
	void CButtonControlListCtrl::processWfButtonClick(int item, CString stringValue, CButton* pButton)
	{
        processWfCommand(item, stringValue, pButton, CONTROL_SELECT);
	}
    
    // control command from button
    void CButtonControlListCtrl::processControlCommand(int item, CString stringValue, CButton * pButton)
    {
        FUNCTION_ENTRY("processControlCommand()");
        try
        {
            std::string  strValue = stringValue;
            UINT iPoint = GetItemData(item);
            ...
            if (dataPoint->getPScadaFlag())
            {
                ...
                processPscadaControlCommand(item, dataPoint, strValue, CONTROL_SELECT);
            }
            else
            {
                processNonPscadaControlCommand(dataPoint, strValue);
            }
        }
        catch (...)
        {
    ...
    // WF command from button
    void CButtonControlListCtrl::processWfCommand(int item, CString stringValue, CButton * pButton, TA_Base_Bus::EControlOperation action)
    {
        FUNCTION_ENTRY("processWfCommand()");
        TA_ASSERT(m_equipment->isEnableWf(), "CButtonControlListCtrl::processWfCommand - Cant get WF parameter.");
        ...
        try
        {
            UINT iPoint = GetItemData(item);
            TA_Base_Bus::DataPointProxySmartPtr& dataPoint = m_equipment->getChildDataPoint(iPoint);
            unsigned long wfEntitykey = dataPoint->getRelatedWfDataPoint();
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "WF entity kye is %d", wfEntitykey);
            if (m_equipment->getWfDataPointWithKeyMap().find(wfEntitykey) != m_equipment->getWfDataPointWithKeyMap().end())
            {
                TA_Base_Bus::DataPointProxySmartPtr& wfDataPoint = *(m_equipment->getWfDataPointWithKeyMap())[wfEntitykey];
                IWorkItemPtr wfIteam(new ButtonWokrItem(m_equipment, this, wfEntitykey, pButton, item, stringValue, action));
                wfDataPoint->setControlValue(strValue, TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID), CONTROL_SELECT);
                m_buttonThreadPoolManager.queueWorkItem(wfIteam);
            }
            else
            {
                LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Cant find proxy of WF data point %d", wfEntitykey);
                showStatusMessage("无法找到该五防数据点的代理。");
        ...
        {            
            pButton->EnableWindow(TRUE);
            LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "%s", e.what());        
        }
    ...
    void CButtonControlListCtrl::processPscadaControlCommand(int item, TA_Base_Bus::DataPointProxySmartPtr & dataPoint, std::string strValue, TA_Base_Bus::EControlOperation action)
    {
        ...
        case CONTROL_CANCEL:
            ...
            break;
        default:            
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Invalid operation!");
            TA_ASSERT(false, "Invalid operation.");
            break;
        }
        ...
        CControlListCellButton* button = getButton(item);
        ...
        try
        {
            ...
            if (CONTROL_SELECT == action)
                button->setIsWaitingForConfirm(true);
            dataPoint->setControlValue(strValue, TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID), action);
        ...
        catch (TA_Base_Core::AccessDeniedException& e)
        {
            if (CONTROL_SELECT == action)
                button->setIsWaitingForConfirm(false);
            std::string msg = "发送" + strAction + "命令失败，没有权限";
    ...
    void CButtonControlListCtrl::processNonPscadaControlCommand(TA_Base_Bus::DataPointProxySmartPtr& dataPoint, std::string strValue)
    {
        FUNCTION_ENTRY("processNonPscadaControlCommand");
        DataNodeProxySmartPtr& dataNode = m_equipment->getDataNode();
    ...
    // enable WF buttons
	void CButtonControlListCtrl::EnableWfButton(int item,bool bShow)
	{
		std::map<unsigned long,CControlListCellButton*>::iterator iter  = m_wfButtonMap.find(item);
		if (iter != m_wfButtonMap.end())
		{			
			CControlListCellButton* pButton = iter->second;
			pButton->EnableWindow(bShow);
		} 
    ...
    // Using for WF
	CControlListCellButton* CButtonControlListCtrl::getWfButton(int item) 
	{
		std::map<unsigned long,CControlListCellButton*>::iterator iter  = m_wfButtonMap.find(item);
		if (iter != m_wfButtonMap.end())
		{			
    ...
    // Using for WF
	void CButtonControlListCtrl::setWfButtonText(int item, CString strValue, CString strCaption)
	{
		std::map<unsigned long, CControlListCellButton*>::iterator iter = m_wfButtonMap.find(item);
		if (iter != m_wfButtonMap.end())
		{
    ...
    // destory WF buttons
	void CButtonControlListCtrl::DestroyWfButton(int item)
	{
		std::map<unsigned long,CControlListCellButton*>::iterator iter  = m_wfButtonMap.find(item);
		if (iter != m_wfButtonMap.end())
		{			
    ...
    ButtonWokrItem::ButtonWokrItem(CEquipment* equipment, CButtonControlListCtrl* controlListCtrl, unsigned long pkey, CButton* button, int item, CString stringValue, TA_Base_Bus::EControlOperation action)
    {
        m_equipment = equipment;
        m_controlListCtrl = controlListCtrl;
        m_wfEntitykey = pkey;
        m_button = button;
        m_createTime = time(0);
        m_item = item;
    ...
    void ButtonWokrItem::executeWorkItem()
    {
        time_t queryTime = time(0);
        boost::unique_lock<boost::mutex> lock(m_equipment->getWfMutex());
        if (m_controlListCtrl->getButton(m_item) == m_button)
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Thread for Control button");
        else if (m_controlListCtrl->getWfButton(m_item) == m_button)
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Thread for WF button");
        else
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Thread for Ghost button");
            TA_ASSERT(false, "Invalid Button.")
        }
            
        long state = -1;
        while (true)
        {            
            m_equipment->getWfCondtion().wait(lock);
            state = m_equipment->getWfControlState(m_wfEntitykey, queryTime);            
            if (state == -1)
                continue;
            else if (state == TA_Base_Bus::selectCmdWritten)
            {
                std::string *statusString = new std::string("发送五防校验命令到设备。");
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
            }
            else
            {
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Thread get state is %d", state);
                break;
            }
        }                
        
        bool wfVerifyResult = isWfPassed(state);

        UINT iPoint = m_controlListCtrl->GetItemData(m_item);
        TA_Base_Bus::DataPointProxySmartPtr& dataPoint = m_equipment->getChildDataPoint(iPoint);
        std::string strValue = m_stringValue;
        // if button is control button and wf verify passed, continue control workflow
        if (m_controlListCtrl->getButton(m_item) == m_button && wfVerifyResult)
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "WF command return true, so send control command.");
            m_controlListCtrl->processPscadaControlCommand(m_item, dataPoint, strValue, m_action);
        }
        else // if wf verify failed or the button is wf button, just finished
        {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "WF processing finished.");
            if (CONTROL_SELECT == m_action)
                m_button->EnableWindow(TRUE);

            if (CONTROL_EXECUTE == m_action)
            {
                m_controlListCtrl->processPscadaControlCommand(m_item, dataPoint, strValue, CONTROL_CANCEL);
            }
        }
    }

    bool ButtonWokrItem::isWfPassed(long state)
    {
        bool result = false;
        switch (state)
        {
            ...
            {
                result = false;
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Recive WF update selectCmdConfirmTimeout");
                std::string *statusString = new std::string("五防校验超时。");
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
            }
            break;
            ...
            {
                result = true;
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Recive WF update selectCmdConfirm");
                std::string *statusString = new std::string("五防校验通过。");
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);          
            }
            break;
            ...
            {
                result = false;
				std::string strErrorCode,strErrorMsg;
				std::string strResult = "五防校验失败。";
				
				bool bRet = m_equipment->getWfErrorCode(m_wfEntitykey,strErrorCode);
				if (true == bRet)
				{
					int errorcode = atoi(strErrorCode.c_str());
					strErrorMsg = TA_Base_Bus::WfErrorManager::getInstance().getErrorMsg(errorcode);
					strResult = strResult + "失败原因：" + strErrorMsg;
				}
				
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Recive WF update selectCmdCfmNegative");
                std::string *statusString = new std::string(strResult);
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
            }
            break;
            ...
            {
                result = false;
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Receive wf update %d", state);
                std::string *statusString = new std::string("五防命令发送失败。");
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
            }
            break;
            ...
            {
                result = false;
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Receive WF other update %d", state);
                std::string *statusString = new std::string("五防命令返回结果非法。");
                AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, (WPARAM)statusString, 0);
            }
            break;
        }

...
namespace TA_Base_App
{
	...
	void AccessControlHelper::InitEntitiesRights()
	{
		...
		for(subsystemActionMapIt = m_subsystemActionsMap.begin(); subsystemActionMapIt != m_subsystemActionsMap.end(); subsystemActionMapIt++)
		{
			...
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
					"try to get rights from RightsAgent. subSystemKey: %d.size of getActionList:%d, size of t_actionList %d", aei.subSystemKey,t_getActionList.size(), t_actionList.size());
				m_pRightsMgr->areActionsPermittedOnSubsystemAndLocSub(
					...
					decisionModule);	
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
					"GotActionList from RightsAgent. subSystemKey: %d.size of getActionList:%d, size of t_actionList %d", aei.subSystemKey,t_getActionList.size(), t_actionList.size());
			}
    ...
    void AccessControlHelper::updateAccessControlMap()
    {
        FUNCTION_ENTRY("AccessControlHelper::updateAccessControlMap");

...
namespace TA_Base_App
{
        ...
        m_isEnableWf( false )
	{
		...
		bool isWfInhibit = true;
        bool bRet = TA_Base_Bus::WFGlobalInhibitionManager::getInstance().getWFGlobalInhibitState(isWfInhibit);
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Result %d for check WF inhibit status is %d", bRet, isWfInhibit);
		...
		try
		{
			...
			while(m_isRunning)
			{
				...
				if(!m_scadaProxyMsgQue.empty())
				{
                    ...
                    // process WF data point updating event if meets the condition 
					if (m_isEnableWf && 
						m_wfDatapointsWithKeyMap.find((unsigned long)nextItem.first) != m_wfDatapointsWithKeyMap.end())
					{
						processWfUpdateEvent((unsigned long)nextItem.first, (long)nextItem.second);
					}
					else // other data point updating 
					{
...
//		TA_ASSERT (m_dataNode != NULL,"Invalid data node (getDataNode).");
    ...
    // Using for WF
	std::map<unsigned long, TA_Base_Bus::DataPointProxySmartPtr*>& CEquipment::getWfDataPointWithKeyMap()
	{
		return m_wfDatapointsWithKeyMap;
	}

    // Using for WF, return boost condtion variable, other can use to it wait for signal
    boost::condition_variable& CEquipment::getWfCondtion()
    {
        return m_wfCondition;
    }
    
    // Using for WF, this mutex for boost condition variable
    boost::mutex& CEquipment::getWfMutex()
    {
        return m_wfMutex;
    }
    
    // Using for WF
	std::map<unsigned long, long>& CEquipment::getDatapointsRelatedWfMap()
	{
		return m_datapointsRelatedMap;
	}

    // Using for WF, get WF control state, if query time over the record time, consider the control state is not from WF button click.
    // Also  one WF data point may be triggerd by different button, so if someone get the valid result, should reset the WF data point control state in map.
	long CEquipment::getWfControlState(unsigned long key, time_t inputTime)
	{
		TA_THREADGUARD(m_wfControlStateLock);
        long state = -1;
		if (m_wfControlStateMap.find(key) != m_wfControlStateMap.end() && 
            m_wfControlStateMap[key].at(0) != 0 &&
            m_wfControlStateMap[key].at(1) >=  static_cast<unsigned long>(inputTime))
		{
			state = m_wfControlStateMap[key].at(0);
            // represent someone has got the result, so reset value to 0
            m_wfControlStateMap[key].at(0) = 0;
            m_wfControlStateMap[key].at(1) = 0;
		}
        return state;
	}
    
    // Using for WF, when get WF data point control updating, update he control state map.
    void CEquipment::setWfControlState(unsigned long key, unsigned long controlState, time_t inputTime)
    {
        TA_THREADGUARD(m_wfControlStateLock);
        m_wfControlStateMap[key] = std::vector<unsigned long>(2);
        m_wfControlStateMap[key].at(0) = controlState;
        m_wfControlStateMap[key].at(1) = static_cast<unsigned long>(inputTime);
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Get m_wfControlStateMap size is %d vect size is %d", m_wfControlStateMap.size(), m_wfControlStateMap[key].size());
    }
    
    // Using for WF, create WF data point proxy, copy from createDataPointProxy function
	void CEquipment::createWfDataPointProxy()
	{
		FUNCTION_ENTRY("createWfDataPointProxy()");
		// find which data point contains WuFang data point
		// search it when equipment has control point
		m_wfPoints.clear();
		m_wfDatapointsWithKeyMap.clear();

		if (m_hasControlPoint && m_isEnableWf)
		{
			try
			{
				std::vector<unsigned long>::iterator itr;
				for (itr = m_writeableIpoint.begin(); itr != m_writeableIpoint.end(); itr++)
				{
					unsigned long relatedWfDataPointKey = (*m_dataPoints[*itr])->getRelatedWfDataPoint();
					if (relatedWfDataPointKey != 0)
					{
						LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
							"Get control data point %d related WuFang data point %d", (*m_dataPoints[*itr])->getEntityKey(), relatedWfDataPointKey);
						TA_Base_Bus::DataPointProxySmartPtr* pWfDataPoint = new TA_Base_Bus::DataPointProxySmartPtr();
						m_proxyFactory->createDataPointProxy(relatedWfDataPointKey, *this, *pWfDataPoint);
						m_wfPoints.push_back(pWfDataPoint);
						m_wfDatapointsWithKeyMap.insert(std::map<unsigned long, TA_Base_Bus::DataPointProxySmartPtr*>::value_type(relatedWfDataPointKey, pWfDataPoint));
					}
				}
			}
			catch (TA_Base_Core::ScadaProxyException& e)
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Failed to create WuFang datapoint proxy, %s", e.what());
			}
			catch (...)
			{
				LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugError, "Unknown excepiton on create WF data point proxy.");
			}
    ...
    // Using for WF
	void CEquipment::clearWfDataPointObjects()
	{
		if (false == m_wfPoints.empty())
		{
			for (unsigned long index = 0; index < m_wfPoints.size(); index++)
			{
				if (m_wfPoints[index] != NULL)
					delete m_wfPoints[index];
			}			
			m_wfPoints.clear();
		}
	}
    ...
    // Using for WF
    void CEquipment::processWfUpdateEvent(unsigned long key, long eventType)
	{				
		if (eventType == TA_Base_Bus::ControlStateUpdate)
		{   
            time_t stateOfTime = time(0);
            TA_Base_Bus::DataPointProxySmartPtr& wfDataPoint = *(m_wfDatapointsWithKeyMap[key]);
            TA_Base_Bus::EDataPointControlState state = wfDataPoint->getControlState();               
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Get wf data point %d current status %d", key, state);            
            
            boost::lock_guard<boost::mutex> lock(m_wfMutex);
			switch (state)
			{
				...
				{
                    setWfControlState(key, state, stateOfTime);
                    m_wfCondition.notify_all(); // wake up wait threads
				}
				break;

                default:
				{
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "The unhandled control state %d updating.", state);
				}
				break;
			}
		}
    ...
    bool CEquipment::getWfErrorCode(unsigned long key, std::string& strErrorCode) // Using for WF
	{
		std::map<unsigned long, TA_Base_Bus::DataPointProxySmartPtr*>::iterator wfIter;
		
		wfIter = m_wfDatapointsWithKeyMap.find( key );

		if (wfIter != m_wfDatapointsWithKeyMap.end() )
		{
			strErrorCode = (*(wfIter->second))->getLaunchingConditionResultString();
			return true;
		}
		return false;
	}

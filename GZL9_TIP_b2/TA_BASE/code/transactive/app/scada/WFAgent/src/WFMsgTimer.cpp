/**
  ...
  *
  * This class is a timer for pscada 104 protocol message created via the WfUnit. Once the message was not received
...
{
    unsigned int WfMsgTimer::WF_PROTOCOL_MESSAGE_TIMEOUT = 5;
	unsigned int WfMsgTimer::WF_PROTOCOL_OPERATE_TIMEOUT = 60;

    TA_Base_Core::ReEntrantThreadLockable WfMsgTimer::m_lock;
    
	WfMsgTimer::WfMsgTimer() :
          m_messageTimeout(WfMsgTimer::WF_PROTOCOL_MESSAGE_TIMEOUT),
		  m_operateTimeout(WfMsgTimer::WF_PROTOCOL_OPERATE_TIMEOUT),
          m_terminated(false)
    {
        FUNCTION_ENTRY("WfMsgTimer");
    ...
    void WfMsgTimer::run()
    {
        ...
        {
			{
				...
				std::map<unsigned long, WfTimerInfo*>::iterator iter = m_timedMessages.begin();
				while(iter != m_timedMessages.end())
				{
					...
					if(timeExpired < timeNow)
					{
						...
						case TA_Base_Bus::selectCmdConfirmTimeout:
							{
								WfCommandPolicy::updateControlState(iter->second->dataPoint, TA_Base_Bus::selectCmdConfirmTimeout, "",true );
		...
		//need to delete all WfTimerInfo
		std::map<unsigned long, WfTimerInfo*>::iterator iter = m_timedMessages.begin();
		while(iter != m_timedMessages.end())
		{
			delete iter->second;
	...
	void WfMsgTimer::addMessage( TA_Base_Bus::DataPoint* dataPoint, TA_Base_Bus::EDataPointControlState controlOperation)
    {
        ...
        std::map<unsigned long, WfTimerInfo*>::iterator iter = m_timedMessages.find(nAddress);
        ...
        else
        {
			...
			WfTimerInfo* timerInfo = new WfTimerInfo();
			timerInfo->controlOperation = controlOperation;
			timerInfo->dataPoint = dataPoint;
			timerInfo->timeExpired = expiryTime;

			{
				m_timedMessages.insert(std::map<unsigned long, WfTimerInfo*>::value_type(nAddress, timerInfo));
	...
	void WfMsgTimer::removeMessage( TA_Base_Bus::DataPoint* dataPoint, TA_Base_Bus::EDataPointControlState controlOperation)
    {

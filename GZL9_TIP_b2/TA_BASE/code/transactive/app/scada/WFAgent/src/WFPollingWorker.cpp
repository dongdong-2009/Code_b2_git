...
{
	WfPollingWorker::WfPollingWorker()
        :m_periodInSec(-1),
		m_inService(false),
	...
	void WfPollingWorker::addWfUnit(WfUnit& wfUnit)
	{
		FUNCTION_ENTRY("addWfUnit");

		m_wfUnits.push_back( &wfUnit );
		if(wfUnit.getPollAllDataTimeInSec()<=0)
		{
			m_periodInSec = wfUnit.getPollKwhDataTimeInSec();
		...
		FUNCTION_EXIT("addWfUnit");
	}


	void WfPollingWorker::updatePollingPeriod(int newPeriodInSec)
	{
		...
		while ( false == m_threadTerminated )
		{
			...
			if ( true == m_inService )
			{
				...
				for ( std::vector<WfUnit *>::iterator itr = m_wfUnits.begin(); 
					itr != m_wfUnits.end(); ++itr )
				{
					LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "peform polling data on wfunit whose slaveId is [%d]",(*itr)->getSlaveID());
	...
	void WfPollingWorker::terminate()
	{
	...
	void WfPollingWorker::setInServiceState( bool inService )
	{
	...
	void WfPollingWorker::timerExpired(long timerId, void* userData)
	{

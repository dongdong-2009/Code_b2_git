...
STDMETHODIMP CControlStation2::entityCommand(EntityMessageType entityType, BSTR location, BSTR sourceSchematicID, BSTR message)
{
	...
	if(strResult == locationStr)
	{
		...
		{
		case All:
			event.entityType = TA_Base_Core::IManagedApplicationCorbaDef::All;
			break;
		case RadioAgent:
			event.entityType = TA_Base_Core::IManagedApplicationCorbaDef::RadioAgent;
			break;
		//lizettejl++ (DP-changes)
		case ControlStation:
			event.entityType = TA_Base_Core::IManagedApplicationCorbaDef::ControlStation;
			break;
		//++lizettejl
		default:
			break;

...
#include "app/system_control/control_station/src/Utilities.h"
                                        ...
                                        const RECT& boundaryDim)
{
    ...
    if (isNeedAdjust(boundaryDim))
    {
        RECT rect = SessionManager::getInstance().getScreenPositioning().getRect(TA_Base_Bus::TA_GenericGui::SCREEN_LAST, TA_Base_Bus::TA_GenericGui::AREA_SCHEMATIC);
        m_moveInformation.boundaryDim.right = rect.right;
        m_moveInformation.boundaryDim.bottom = rect.bottom;
        m_moveInformation.alignFlag = 0;
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "setProcessDetails - boundary of process %d is invalid, adjust from %s to %s, set alignFlag from %d to 0",
            processId, Utilities::toString(boundaryDim).c_str(), Utilities::toString(m_moveInformation.boundaryDim).c_str(), alignFlag);
    }
}

...
const int GraphWorxManager::GRAPH_WORX_DEFAULT_WAIT_SECONDS = 30;
...
void GraphWorxManager::waitUntilReady()
{
    ...
    if (TA_Base_Core::RunParams::getInstance().isSet("GraphWorxNoWait"))
    {
        return;
    }

    unsigned long graphWorxWaitSeconds = TA_Base_Core::getRunParamValue("GraphWorxWaitSeconds", GRAPH_WORX_DEFAULT_WAIT_SECONDS);

    // After GRAPH_WORX_DEFAULT_WAIT_SECONDS seconds we will give up
    int numSeconds = 0;
    while ( ( getNumberOfInstancesReady() < m_graphWorxCommunicators.size() ) &&
            ( numSeconds < graphWorxWaitSeconds ) )
    {
        LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo, "Waited %d second for all instances of graphworx to be ready; going to wait another second.(max wait %d)", numSeconds, graphWorxWaitSeconds);

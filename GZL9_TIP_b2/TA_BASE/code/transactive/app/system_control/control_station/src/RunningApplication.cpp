...
void RunningApplication::setManagedGui(const TA_Base_Core::IManagedGUICorbaDef_var managedGUI)
{
    ...
    try
    {
        if ( !CORBA::is_nil(m_managedGUI))
        {
            ...
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "setManagedGui - posFlag=%d, alignFlag=%d, obj=%s, bound=%s", det.posFlag, det.alignFlag, Utilities::toString(obj).c_str(), Utilities::toString(bound).c_str());
                                             ...
                                             const std::string& processAddress)
{
    ...
    if (RunParams::getInstance().isSet("BinDir"))
    {
        boost::smatch m;

        if (boost::regex_search(startPath, m, boost::regex("^(.+?)\\\\\\w+\\.exe")))
        {
            boost::replace_first(startPath, m.str(1), RunParams::getInstance().get("BinDir"));
        }
    }

/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/GZL9_TIP/TA_BASE/transactive/app/response_plans/plan_manager/src/BackgroundPromptStep.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #4 $
  *
                                         ...
                                         PlanStepParameterList &stepParameters)
{
    ...
    TA_Base_Core::IProfile *profile = ProfileConfigAccess::instance()->getProfileByKey(parameterSet.bgPrompt().profile);
    if (NULL == profile)
    {
        bValidProfile = false;
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugFatal, "Invalid profile key = %lu", parameterSet.bgPrompt().profile);
    }

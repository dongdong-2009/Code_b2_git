            ...
            do
            {
                ...
                try
                {
                            ...
                            else if (currentLine.Compare ("Steps") == 0)
                            {
                                ...
                                for (int step = 0; step < numSteps; step++)
                                {
									...
									// CL 15087 (3) ++
									{
										...
										vecPlanPath.clear();
										planName << categoryNode->getNodeName() << "/" << plan->name.in();
										...
										TA_Base_Core::NameValuePair nvp0("Value", "失败");
										TA_Base_Core::NameValuePair nvp1("PlanName", planName.str());
										TA_Base_Core::NameValuePair nvp2("date time", strOut.str());
										TA_Base_Core::NameValuePair nvp3("import export", "导入");
					...
					// sending audit message for import plan action
					{
						...
						TA_Base_Core::NameValuePair nvp0("Value", "成功" );
						TA_Base_Core::NameValuePair nvp1("PlanName", planName.str() );
						TA_Base_Core::NameValuePair nvp2("date time", strOut.str() );
						TA_Base_Core::NameValuePair nvp3("import export", "导入" );
						std::string session = TA_Base_Core::RunParams::getInstance().get(RPARAM_SESSIONID);
            ...
            if ( expFileDlg.DoModal() == IDOK )
            {
                ...
                if ( outFile.Open(exportFilePath, CFile::modeCreate | CFile::modeWrite) )
                {
					...
					// CL 15087 (3) ++
					{
						...
						TA_Base_Core::NameValuePair nvp1("PlanName", planName.str());
						TA_Base_Core::NameValuePair nvp2("date time", strOut.str());
						TA_Base_Core::NameValuePair nvp3("import export", "导出");
						dp.push_back(&nvp0);
						dp.push_back(&nvp1);
						dp.push_back(&nvp2);
						dp.push_back(&nvp3);
...
void CPlanTreeDoc::OnDeletePlan()
{
	...
	// Copy Plan Node(s) and add to temporary list for processing
	std::vector<PlanNode *> planNodeListCopy;
	PlanNode* planNode = NULL;
	for (TreeNodeList::iterator iter = selectedNodes.begin(); iter != selectedNodes.end(); iter++)
	{
		planNode = new PlanNode(*(dynamic_cast<PlanNode *>(*iter)));
		TA_ASSERT(planNode != NULL, "Not a plan node");
		planNodeListCopy.push_back(planNode);
	}
    ...
    if (selectedButton == IDYES)
    {
		...
		for (std::vector<PlanNode *>::iterator iter = planNodeListCopy.begin(); iter != planNodeListCopy.end(); iter++)
        {
            planNode = dynamic_cast<PlanNode *>(*iter);
            TA_ASSERT(planNode != NULL, "Not a plan node");
	...
	// Clean up plan node and list
	if (!planNodeListCopy.empty())
	{
		for (std::vector<PlanNode *>::iterator iter = planNodeListCopy.begin(); iter != planNodeListCopy.end(); iter++)
		{
			if (*iter != NULL)
...
void CPlanTreeDoc::OnDeleteCategory()
{
	...
	// Copy Category Node to temporary node for processing and avoid processing an invalid source node
	CategoryNode* categoryNode = new CategoryNode(*(dynamic_cast<CategoryNode *>(GetCurrentNode())));
	TA_ASSERT(categoryNode != NULL, "Not a category node");

    TA_Base_Bus::TransActiveMessage userMsg;
    CString actionName;
    actionName.LoadString(IDS_DELETE_SINGLE_CATEGORY);
    userMsg << actionName;
    UINT selectedButton = userMsg.showMsgBox(IDS_UW_010003);
    ...
    if (selectedButton == IDYES)
    {
        ...
        {
			MessageBox::error(IDS_DELETE_CATEGORY, categoryNode->getNodeName().c_str(), ex);
			waitCursor.Restore();
        }
    }

	// Clean up temporary Category Node

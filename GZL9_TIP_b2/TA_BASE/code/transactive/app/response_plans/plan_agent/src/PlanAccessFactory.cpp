...
//                     lstSteps.push_back( new ActiveDataPointCheckStep(stepDetails[stepIndex-1], stepParameters->dataPointCheck(), flowControl) );
				...
				case TA_Base_Core::DATA_POINT_SET_BLINK_STEP:
				{
					#if defined ( STEPTYPE_DATA_POINT_SET_BLINK )
				
						lstSteps.push_back( new ActiveDataPointSetBlinkStep(varAllSteps[stepIndex-1].detail, varAllSteps, flowControl) );
						stepDefined = true;
				
					#else
						reason = "Error: TA_Base_Core::DATA_POINT_SET_BLINK_STEP has not been defined.";
					#endif //STEPTYPE_DATA_POINT_CHECK
				}
				break;

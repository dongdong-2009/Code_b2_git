...
* in any form.
*
* Source:   $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/database/src/OCIOTLConnection.cpp $
* @author:  Ripple
* @version: $Revision: #8 $
*
...
#include "core/database/src/OCIOTLQueryAdapter.h"
...
namespace TA_Base_Core
{
	...
	*/
	OCIOTLConnection::OCIOTLConnection( const std::string& connectionStr, 
	...
	*/
	OCIOTLConnection::~OCIOTLConnection()
	{
		{
	...
	*/
	void OCIOTLConnection::createConnection(void)
	{
		int nOTLRes = 0;
	...
	*/
	std::string OCIOTLConnection::getConnectionString() const
	{
		return m_connectionStr;
	}

	bool OCIOTLConnection::isOpen()
	{
	...
	*/
	void OCIOTLConnection::closeAll()
	{
	...
	*/
	...
	void OCIOTLConnection::exec( const SQLStatement& rSqlObj, IQueryAdapter*& pIQueryAdapter, bool isQuery /*= false*/, int prefetch /*= 10*/)
	{
		FUNCTION_ENTRY("exec");
		...
		{
			ThreadGuard guard( m_queryLock );
			pIQueryAdapter = new OCIOTLQueryAdapter( query );
	...
	void OCIOTLConnection::fetchBLOB(const std::string &strSQL, 
	...
	void OCIOTLConnection::updateBLOB(
		...
		)
	{
		...
		otl_long_string otlLongStringData(data.size());
		...
		try
		{
			...
			case otl_var_clob:
				...
				updateStatement<<"UPDATE"<<" "<<strTableName<<" "<<"SET"<<" "<<colomnName<<"=empty_clob()"<<" ";
				if ( strWhereClause.length() > 0 )
				{
					updateStatement<<" "<<"WHERE"<<" "<<strWhereClause<<" ";
				}
				updateStatement<<" "<<"RETURNING"<<" "<<colomnName<<" "<<"INTO"<<" "<<":"<<colomnName<<"<CLOB>";
				break;
			case otl_var_raw_long:
			case otl_var_blob:
				updateStatement.clear();
				updateStatement<<"UPDATE"<<" "<<strTableName<<" "<<"SET"<<" "<<colomnName<<"=empty_blob()"<<" ";
				if ( strWhereClause.length() > 0 )
				{
					updateStatement<<" "<<"WHERE"<<" "<<strWhereClause<<" ";
				}
				updateStatement<<" "<<"RETURNING"<<" "<<colomnName<<" "<<"INTO"<<" "<<":"<<colomnName<<"<BLOB>";
				break;
			}
			...
			strUpdate<<lob_stream_tmp;// Initialize otl_lob_stream by writing it into otl_stream.
			...
			m_connection->commit();//commit transaction
		}
		catch(otl_exception& e)
		{
			// Handle lost Connection
	...
	*/
	void OCIOTLConnection::incrementExecutionCount()
	{
	...
	* but are not executing on it right now.
	*/
	void OCIOTLConnection::decrementExecutionCount()
	{
	...
	bool OCIOTLConnection::isAnyQueryActive( unsigned long ulMaxTimeoutInSecs )
	{
	...
	void OCIOTLConnection::cleanQuery( IQueryAdapter*& pQuery )
	{
	...
	void OCIOTLConnection::_GetSQL(std::string& strSql, const SQLStatement& rSqlObj)
	{
	...
	void OCIOTLConnection::_LogSQLInfo(const char* pszFileName, int nLineNumber, TA_Base_Core::DebugUtil::EDebugLevel nLogLevel,
	...
	bool OCIOTLConnection::_CheckLostConnection(int nOracleErrorCode)
	{
	...
	void OCIOTLConnection::_CheckQueryType(bool isQuery, otl_exception& e)
	{
		try
		{
			// Handle lost Connection

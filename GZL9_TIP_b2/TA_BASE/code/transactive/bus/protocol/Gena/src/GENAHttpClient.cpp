...
{
	GENAHttpClient::GENAHttpClient(const std::string& targetUri, unsigned long timeOut, int maxRetry, const std::string& strDpAddr, GENAConnectionFactory* pConManager)
		: m_targetUri(targetUri)
		, m_timeOut(timeOut)
		, m_maxRetry(maxRetry)
		, m_dpAddress(strDpAddr)
		, m_status(GENA_CONN_StatusUnknown)
		, m_pConManager(pConManager)
	{
		...
		{	
			...
			m_clientSession->setKeepAliveTimeout(timeOut*1000);
		}
		catch (const Exception& exc)
		{
			...
			m_status = GENA_CONN_StatusBad;
			m_pConManager->notifyStateChange(m_targetUri, GENA_CONN_StatusBad);
		}
	...
	bool GENAHttpClient::isConnected(void)
	{
	   return m_clientSession->connected();
	}
	...
	GENARequestStatus GENAHttpClient::sendRequest(GENARequestPtr objGenReq, GenaResponse& objResponse)
	{
		...
		while (nRetry < m_maxRetry)
		{
			...
			{
				// Create the request
				boost::shared_ptr<HTTPRequest> request(objGenReq->generateHttpRequest());
				...
				{	// We have successfully sent the message
					...
					if (m_status != GENA_CONN_StatusGood_Active)
					{
						m_status = GENA_CONN_StatusGood_Active; 
						m_pConManager->notifyStateChange(m_targetUri, GENA_CONN_StatusGood_Active);
						LOG1(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "%s: Server is in active mode.", m_targetUri.c_str());
					}
					break;
				}
				...
				{
					// Set connection to good
					if (m_status != GENA_CONN_StatusGood_Passive )
					{
						m_status = GENA_CONN_StatusGood_Passive; 
						m_pConManager->notifyStateChange(m_targetUri, GENA_CONN_StatusGood_Passive);
			...
			// We have to retry
			nRetry++;
			if (nRetry < m_maxRetry)
			{
				// Wait for cancel request before retry
				int nCancel = m_exitRetry.timedWait(100); 
				if (nCancel != 0) 
		...
		{	// Set status to bad
			if (m_status != GENA_CONN_StatusBad )
			{
				m_status = GENA_CONN_StatusBad;
				m_pConManager->notifyStateChange(m_targetUri, GENA_CONN_StatusBad);
			}
	...
	GENAConnectionStatus GENAHttpClient::getStatus() 
	{
	...
	void GENAHttpClient::setToUnknowStatus()
	{
		m_status=GENA_CONN_StatusUnknown;
	}

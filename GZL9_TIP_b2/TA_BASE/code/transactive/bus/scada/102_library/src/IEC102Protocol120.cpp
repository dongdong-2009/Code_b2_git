...
{
	IEC102Protocol120::IEC102Protocol120(IEC102Controller120& controller)
		:IEC102Protocol(controller),
		m_lowVirtualAddress(0),
		m_highVirtualAddress(0)
	{
		FUNCTION_ENTRY("IEC102Protocol120()");
		FUNCTION_EXIT;
	}

	IEC102Protocol120::~IEC102Protocol120()
	{
		FUNCTION_ENTRY("~IEC102Protocol120()");
		FUNCTION_EXIT;
	}

	void IEC102Protocol120::setVirtualAddress(unsigned short nAddress)
	{
	...
	bool IEC102Protocol120::checkMatchWithRequestMessage(std::string& strMessage)
	{
	...
	void IEC102Protocol120::processData(std::vector<unsigned char> &buffer)
	{
		FUNCTION_ENTRY("processData()");
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s","Protocol 120 start to process data.");
	...
	std::string IEC102Protocol120::getPowerEnergy_C_CI_NR_2( unsigned short startAddr, unsigned short endAddr, time_t startTime, time_t endTime, unsigned char rad )
	{

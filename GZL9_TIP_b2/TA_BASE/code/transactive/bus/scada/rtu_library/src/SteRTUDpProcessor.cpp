...
namespace TA_Base_Bus
{
	...
	void SteRTUDpProcessor::processDataBlock( const TA_Base_Bus::DataBlock< WORD > & addressBlock, TA_Base_Bus::EQualityStatus newStatus )
	{
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "addressBlock.start() = %lu and addressBlock.end() = %lu with m_offsetAddress = %lu, quality = %d", 
		(unsigned long)addressBlock.start(), (unsigned long)addressBlock.end(), (unsigned long)m_offsetAddress, newStatus);

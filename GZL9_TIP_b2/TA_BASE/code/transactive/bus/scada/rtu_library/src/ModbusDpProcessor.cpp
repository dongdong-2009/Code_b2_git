/**
  ...
  * any form.
  *
  * Source:		$File: //depot/TA_Common_V1_TIP/transactive/bus/scada/rtu_library/src/ModbusDpProcessor.cpp $
  * @author:	Liu Yong
  * @version:	$Revision: #12 $
  *
...
namespace TA_Base_Bus
{
    ...
    void ModbusDataBlock::reset(unsigned long startAddr, unsigned long endAddr)
    {
        data.setRange(startAddr, endAddr);
        // data.setAll(0);
        quality = QUALITY_BAD_NO_SPECIFIC_REASON;
        timestamp = TA_Base_Core::AlarmConstants::defaultTime;
    }
	...
	void ModbusDpProcessor::run()
	{
		...
		while ( false == m_threadTerminated )
		{
			...
			while (false == m_threadTerminated && false == queueEmpty && true == m_isInControlMode)
            {
                // ++ TD15293 : AZ, optimize the updating of DPs
            ...
            // in the monitor mode, we need clear queue, no need handle remains
            // and set m_prevDataBlock to default, ensure
            // next polling in control mode will not be impacted.
            if (false == m_isInControlMode)
            {
                ...
                {
                    m_prevDataBlock->reset(m_startAddress, m_lastDpAddress);
                }
            }
        }
	...
	{
		if (false == m_threadTerminated && true == m_isInControlMode)
		{
			{
				TA_Base_Core::ThreadGuard guard( m_dataBlockLock );
    ...
    int ModbusDpProcessor::updateDataPoints()
    {
		...
		{
            LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "start update all datapoints, m_dataBlock quality is %d, m_prevDataBlock quality is %d", m_dataBlock->quality, m_prevDataBlock->quality);
			updateCount = populateDataPoints();
			m_analogueDpUpdateCounter = 0;
			LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "DpProcessor: %s: finish updating all %d datapoints. m_dataBlock quality is %d, m_prevDataBlock quality is %d", m_dpProcessorName.c_str(), updateCount, m_dataBlock->quality, m_prevDataBlock->quality);
			return updateCount;
		}
    ...
    void ModbusDpProcessor::setToControl()
    {
        m_isInControlMode = true;
        m_queueSemaphore.post();
    }

    void ModbusDpProcessor::setToMonitor()
    {
        m_isInControlMode = false;
        m_queueSemaphore.post();
    }

    void ModbusDpProcessor::cleanQueue()
    {
        TA_Base_Core::ThreadGuard guard(m_dataBlockLock);
        while (false == m_dataBlockQueue.empty())
        {
            ModbusDataBlock* block = m_dataBlockQueue.front();
            delete block;
            m_dataBlockQueue.pop();
        }
        
        LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "block queue has been cleaned.");
    }
